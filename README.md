...
| Deploy engine | One imperative API over six backends: `dockerhost`, `podman` (native libpod), native `containerd`, daemonless `bare`, `incus`, and `kubernetes`. Compose, Docker-compatible clients, and Dev Containers all translate into this API. |
...
`--addr 127.0.0.1:5000` keeps this evaluation server reachable from this machine
only, which is what you want while trying Cornus out. It is narrower than the
`:5000` default, and deliberately so: features where a workload dials back into
the server -- client-local mounts, client-side egress, credential delivery, and
workload telemetry -- need an address a container can reach, so drop the `--addr`
flag once you move past this walkthrough. Read
[the security guide](https://cornus.dev/guides/security) before you do: the API is
unauthenticated unless you configure it, and it can build, deploy, and exec.
