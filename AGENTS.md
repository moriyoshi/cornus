...
* ❌ Never emit decomposed Japanese. Always write CJK text in NFKC normal form, so voiced hiragana and katakana are the precomposed single code points (`が` U+304C, `ぱ` U+3071) — never a base kana followed by a combining voiced/semi-voiced sound mark (U+3099 / U+309A). NFKC also folds half-width katakana (`ｶﾞ` -> `ガ`) and full-width ASCII to their canonical forms. Decomposed kana is visually identical to composed kana in every editor and terminal, so it can only be caught by scanning:
  ...
  The one legitimate exception is a `#fragment` inside a link URL: markdown-it-anchor slugifies headings after Unicode decomposition, so anchors into Japanese headings must keep the decomposed code points that `docs:check-anchors` prints. Anything outside a URL fragment is a defect — normalize it to NFKC. See `.agents/docs/QUALITY_GATE.md` section 5.
