...
* [./.agents/docs/TODO.md](./.agents/docs/TODO.md) ... open to-do items extracted from JOURNAL.md during `good-sleep` consolidation. Check and update this file when picking up or finishing work. Its header carries the measured rule that every entry must be re-verified against the tree before acting on it; the reusable form is `.agents/docs/LTM/verification-and-audit-methods.md`.
...
* [./.agents/docs/JA_TRANSLATION_GLOSSARY.md](./.agents/docs/JA_TRANSLATION_GLOSSARY.md) ... Japanese terminology glossary for `docs/` -> `docs/ja/`: preserve-verbatim list, preferred terms, and compound-term rules. Internal translation aid, never published. Read before translating or reviewing `docs/ja/`, and record newly settled terms here in the same change.
* [./.agents/docs/ZH_TRANSLATION_GLOSSARY.md](./.agents/docs/ZH_TRANSLATION_GLOSSARY.md) ... the same for `docs/` -> `docs/zh/` (Simplified Chinese).
...
* ❌ Never emit decomposed Japanese. Always write CJK text in NFKC normal form, so voiced hiragana and katakana are the precomposed single code points (`が` U+304C, `ぱ` U+3071) — never a base kana followed by a combining voiced/semi-voiced sound mark (U+3099 / U+309A). NFKC also folds half-width katakana (`ｶﾞ` -> `ガ`) and full-width ASCII to their canonical forms. Decomposed kana is visually identical to composed kana in every editor and terminal, so it can only be caught by scanning:

  ```sh
  rg -n --pcre2 '[\x{3099}\x{309A}]' --glob '*.md' .
  ```

  **There is no exception.** `#fragment`s in link URLs used to be one: VitePress's bundled slugify (`@mdit-vue/shared`) does `.normalize('NFKD')` and then strips only `[̀-ͯ]`, the Latin combining range, so Japanese voiced marks (U+3099 / U+309A) survived into the heading id and a normally-typed anchor silently failed to resolve. That was an upstream defect, not a contract, and it is now fixed at the source: `docs/.vitepress/config.mts` supplies a `markdown.anchor.slugify` that recomposes to NFC as its last step (after the accent-stripping, so `café` -> `cafe` is unaffected). All heading ids are composed, and anchors are written exactly like the headings they point at. Decomposed kana anywhere — fragment or prose — is now a defect; normalize it to NFKC. See `.agents/docs/QUALITY_GATE.md` section 5.
