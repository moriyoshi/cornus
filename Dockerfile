...
# CNI reference plugins, for a containerized server driving the containerd or bare
# host backend. Those backends realize workload networking by forking these
# binaries themselves (pkg/deploy/internal/hostrun), so without them in the image
# every deploy fails — and it fails LATE, after the image pull and after the
# previous healthy deployment has already been torn down (see the plugin check in
# hostrun's CNIManager.Setup).
#
# Fetched in a stage of its own so the final image needs no curl. Pinned, and
# deliberately the SAME version the E2E runner stages (e2e/container/Dockerfile),
# because a released image on a different plugin version than the one CI exercises
# is an untested configuration shipped by default.
#
# Exactly the four plugins CNIManager.checkPlugins requires, and no more. `firewall`
# is omitted on purpose: hostrun probes for it and chains it when present, but the
# E2E runner does not stage it either, so shipping it here would enable a code path
# nothing tests. macvlan/ipvlan are not shipped because these backends ignore a
# network `driver` outright (they warn about it via UnsupportedNetworkFeatures).
FROM debian:bookworm-slim AS cni
...
RUN apt-get update \
    ...
    && curl -fsSL "https://github.com/containernetworking/plugins/releases/download/${CNI_PLUGINS_VERSION}/cni-plugins-linux-${TARGETARCH}-${CNI_PLUGINS_VERSION}.tgz" \
        | tar xz -C /out/cni ./bridge ./portmap ./host-local ./loopback \
    # Fail the build rather than ship an image whose deploys cannot network, in the
    # same spirit as the store self-assertion in the build stage above: a silently
    # empty /opt/cni/bin would pass every other check here.
    && for p in bridge portmap host-local loopback; do test -x "/out/cni/$p"; done \
    && ls -la /out/cni

FROM debian:bookworm-slim
# runc is the OCI executor the in-process build engine invokes; uidmap enables the
# rootless build path on restrictive hosts.
#
# iptables is what the CNI plugins above shell out to, and it is required rather
# than nice to have: the generated bridge conflist sets ipMasq, and portmap is what
# realizes `ports:`, so both invoke the iptables binary. Measured — without it a
# deploy fails with `failed to locate iptables: exec: "iptables": executable file
# not found in $PATH` from inside the bridge plugin.
RUN apt-get update \
    && apt-get install -y --no-install-recommends \
        runc ca-certificates uidmap iptables \
    && rm -rf /var/lib/apt/lists/*
COPY --from=build /out/cornus /usr/local/bin/cornus
# The default directory hostrun's pluginDirs() resolves to; CORNUS_CNI_BIN_DIR
# overrides it.
COPY --from=cni /out/cni/ /opt/cni/bin/
