...
### The gate cannot see a test that proves nothing

Every command above answers one question: *did anything turn red?* None of them
answers *would this have turned red if the code were wrong?* A test written from
your model of the fix, rather than from the code, inherits that model's blind
spots — and it passes this gate exactly as quietly as a test that works. So
before you run the gate:

1. **Audit the code, then write the test.** Read what the code actually does and
   state it; pin that, not your intent for it.
2. **Ask what a PASS would look like if the claim were false.** If there is no
   answer, the check is not evidence yet, whatever it prints.
3. **Neutralize it.** Break the fix on purpose, confirm the test fails with the
   diagnostic you intended, then restore. ❌ A COMPILE error is not a valid
   neutralization: it proves the test names a symbol, not that it observes a
   behavior.

Not hypothetical here. On 2026-07-29 `warn_once_linux_test.go` keyed on warning
TEXT, so it could not catch the differently-worded duplicate it was written for;
and a containerd hostname test pinned each side of an invariant separately when
only their AGREEMENT was the contract. Both were green through every command in
section 1. See `AGENTS.md` "Testing".
...
4. Check that every Japanese character you authored is in NFKC normal form.
   ...
   **Every hit is a defect, including inside a link `#fragment`.** Fragments were
   the one exception until 2026-07-29, when the underlying slugify bug was fixed
   (see step 6); the tree carried 34 decomposed fragments across 20 files and they
   have all been recomposed. Retype the run of text, or normalize just that
   string:
   ...
   Whole-file NFKC is now safe (it used to rewrite anchors into ids VitePress
   never emitted), because the ids are composed too. `docs:check-anchors` is still
   what proves it.
...
6. Run the complete documentation gate:
   ...
   VitePress build, validates fragments against the generated heading ids,
   rejects duplicate link targets within one Markdown list, and reports translated
   pages whose English source has changed since they were last synced
   (`docs/.translation-state.json`; see the translation glossaries for the
   look-then-record workflow). The punctuation
   ...
   That gap used to bite hardest in the locale trees. VitePress's bundled slugify
   (`@mdit-vue/shared`) is `str.normalize('NFKD').replace(/[\u0300-\u036f]/g, '')`:
   the NFKD is there to strip Latin accents and the character class covers exactly
   the Latin combining range, so Japanese voiced marks (U+3099 / U+309A) fell
   through and a Japanese heading got an id containing DECOMPOSED kana, while a
   hand-typed fragment was composed. Visually identical everywhere a human would
   look; matched by exact code points in the browser; silently dead.

   **Fixed at the source on 2026-07-29.** `docs/.vitepress/config.mts` supplies
   `markdown.anchor.slugify`, the upstream function with `.normalize('NFC')`
   appended — last, so the accent-stripping it exists for still runs
   (`café` -> `cafe`, `9P` -> `_9p`). All 1,860 built heading ids are composed.
   Write anchors exactly like the heading they point at.

   `docs:check-anchors` resolves every fragment against the ids VitePress actually
   emitted, which is what proves the slugify override is doing its job. Note it can
   only be trusted against a FRESH build: it reads `.vitepress/dist/`, so a heading
   added since the last build reports as dead for that reason alone. Run
   `docs:build` first, or use `docs:check`, which chains them in order. The checker itself lives in the `assess-doc-quality` skill
