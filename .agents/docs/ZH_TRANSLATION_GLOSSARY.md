...
| rootless / rootful | rootless / rootful (保留英文; 容器文档中已通行) |
| socket (a unix domain socket) | socket (保留英文) |
| socket activation | socket activation (保留英文) |
...
| mini map (in the pane chooser) | 缩略地图 |
| pin (the pane chooser) | 固定 |
| gutter (the pinned chooser's strip) | 侧栏 |
| file browser | 文件浏览器 |
