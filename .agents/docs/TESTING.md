...
| `socks5-ingress-tls.star` | ingress-via-conduit, EMULATE **TLS termination** (docker + kube): an `x-cornus-ingress` with `tls: {}` makes the emulated ingress serve `:443` with a generated cert (CA pinned under a temp `XDG_DATA_HOME`); `http_get("https://…", socks5=proxy, insecure=True)` proves the `:443` TLS server terminates the handshake and proxies to the workload |
| `deploy-ingress.star` | automatic ingress creation (kube only): boots a server with `CORNUS_INGRESS_DOMAIN`/`CLASS`/`TLS_ISSUER`, deploys with `ingress={}` and asserts the generated `networking.k8s.io/v1` Ingress has an auto-derived host, the workload Service as its backend, the default class/pathType; a second deploy with an explicit host + path + TLS asserts the override, the `<name>-tls` secret, and the cert-manager cluster-issuer annotation; a third deploy exercises multiple `hosts` with an `@` apex over one shared TLS entry; then `remove` proves the Ingress is GC'd with its owning Deployment |
