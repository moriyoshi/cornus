...
cmd/cornus-e2e/main.go        CLI: parse flags, build a Target, preflight, run scenarios
  └─ pkg/e2e/target.go        Target: local | docker | podman | podman-rootless | containerd | bare | kube — the deploy backend + env
...
|------|---------|---------|
| `--target` | `docker` | Deployment target: `docker`, `podman`, `podman-rootless`, `containerd`, `bare` (daemonless OCI runtime), `kube` (kind), or `local` (build-only, no external env). |
...
| `podman` | `dockerhost` (libpod engine) | `CORNUS_DEPLOY_BACKEND=podman`, `CORNUS_PODMAN_SOCKET`, `CORNUS_ALLOW_BIND_SOURCES=/`, `CORNUS_REGISTRY_SOURCE=off` | no-op — the backend pulls from the same-host registry (libpod takes `tlsVerify` per request, so no `registries.conf` entry is needed) |
| `podman-rootless` | same, against a ROOTLESS podman service | same as `podman` | same |
...
Two global constants are injected. **`TARGET`** is the string `"local"`, `"docker"`,
`"podman"`, `"podman-rootless"`, `"containerd"`, `"bare"`, `"incus"`, or `"kube"`; scenarios branch on it to skip
target-specific assertions. **`CORNUS_BIN`** is the path to the binary UNDER TEST,
for the handful of scenarios that must run it somewhere the harness does not manage —
inside a container or instance they create themselves, to exercise the server's own
view of a containerized host. It is injected rather than left to `getenv` so it cannot
drift from the binary every other builtin uses, which is a live hazard here:
`server-in-container.star` carries a CAUTION about a stale image silently exercising
old server code, having once cost a mutation check that "passed" against a server that
never had the mutation.
...
A scenario that decides it cannot run here says so with `log("<label>: skipped
(<reason>)")`, and the runner now **reports those in the summary** — the per-scenario
line reads `passed (reported a skip)` and a trailing block names each one. That matters
because `E2E_STRICT=1` does NOT cover them: it promotes only the entrypoint's
target-level skips (the incus OCI-version gate, the bare agent image), so before this a
scenario could be registered, parse-checked, listed in a target's subset, and still
never execute while the leg reported "all N scenario(s) passed". Keep to the `skipped
(<reason>)` spelling; a partial-arm skip that keeps going should NOT use it (see
`dockerd-exit-code.star`, which logs "skipping the raw wait-body assertions" and then
continues).
...
| `serve_container` | `serve_container(image, data_dir?, env?)` | Starts the server AS A DOCKER CONTAINER on the target's docker host instead of as a host process, and points every other builtin at it — the one topology a host-run server cannot reach, where the paths cornus hands the daemon and the paths it resolves genuinely diverge. `image` must be a cornus-embedding image (e.g. `cornus:e2e`); `data_dir=False` omits the data-dir bind, which is how a scenario proves what that bind buys. Stopped GRACEFULLY on teardown: the server holds kernel 9P mounts that propagate onto the daemon's filesystem and only it can unwind them. |
| `serve_instance` | `serve_instance(image, name?, env?)` | Starts the server AS AN INCUS INSTANCE on the incusd it manages, and points every other builtin at it. Not a mode of `serve_container`: that one shells out to `docker run`, and the incus leg starts no dockerd (`entrypoint.sh` sets `need_dockerd` for the docker and kube targets only) — on an incus host the server's container has to be made BY incus, which is also the arrangement worth testing, since an instance sits on incusd's bridge alongside its workloads and reaches them with neither host networking nor a companion. `image` is any OCI ref an incus remote can pull (an `--protocol=oci` remote is added on demand; localhost registries over http, matching the backend's own rule). The binary under test is bind-mounted in as a disk device rather than baked into an image, which makes the stale-image hazard structurally impossible. The daemon socket goes in as a **proxy** device — a disk device is idmap-shifted to `nobody` and cornus cannot connect — published under `/tmp` with `CORNUS_INCUS_SOCKET` pointing at it, because a proxy listener needs its parent directory to already exist in the image and `/var/lib/incus` is absent from most. Returns the instance's bridge address, which is also what sibling workloads pull from. Deleted on teardown. |
...
| `server_log` | `server_log()` | Everything the running server has written to stdout/stderr so far. For contracts whose ONLY observable is the server's own log: a TCP port-forward is a raw passthrough with no post-preamble error channel (`pkg/server/deploy_exec.go`), so a setup failure — a backend refusal, a kube RBAC denial, a missing pod — reaches the CLI only as a closed tunnel and its cause is logged server-side. Prefer a client-visible assertion where one exists. |
...
`SCENARIOS` list in the `Makefile`. The podman target runs `SCENARIOS_PODMAN`
(the backend-agnostic core plus `compose-dns-resolution`, the podman-only
`server-in-container-podman`, and the port-forward contrast scenario); the
`podman-rootless` target runs the smaller `SCENARIOS_PODMAN_ROOTLESS`, because
rootlessness changes what is REACHABLE rather than what the libpod engine
encodes — the wire-level coverage is already earned on the rootful leg. Both are
mirrored by `PODMAN_SCENARIOS` / `PODMAN_ROOTLESS_SCENARIOS` in
`e2e/container/entrypoint.sh`, and `TestScenarioSubsetsInSync` fails
`go test ./...` if a pair drifts. The containerd target runs the smaller
...
| `web-conduit-join.star` | `cornus web --publish-in-conduit` JOINS the conduit the agent already runs (docker only): `compose up -d --conduit socks5://.shared:PORT` picks the address, then `web(publish=True)` is called with **no** `conduit=` at all and must find its way there on its own. The load-bearing assertion is ONE ADDRESS, not reachability — a scenario that only fetched the UI would pass against two proxies, which is exactly the bug — so the workload and the UI are both fetched through the port compose chose, and `daemon status` is asserted to show exactly one SOCKS5 banner. Complements `web.star`, which always pins `conduit=` and so exercises the other branch |
...
**The mocked BFF is two halves that have to agree.** `web/src/mock/` is the in-memory
fixture (filesystem, workloads, metrics) shared by the jsdom tests and the standalone dev
server; `web/mock/` is that dev server plus a fake interactive shell. They meet at the
container filesystem: the fake shell answers `cd` / `ls` / `cat` from the SAME trees the
file explorer browses, and reports its working directory over OSC 7 exactly as a real
prompt hook does, so a session's reported cwd is a directory the browser can navigate to
and drop files into. That agreement is what makes the cwd-dependent features — the follow
pairing, a split inheriting the live directory, a terminal pane as a transfer destination
— demonstrable against `npm run dev:mock` rather than only against a real backend, and it
is pinned by `web/mock/faketerm.test.ts`. Note where that test LIVES: `web/mock/` is
written for Node's type stripping and has no `@types/node`, `tsconfig.json` includes only
`src`, so a test importing the dev server from under `src/` drags the whole untyped tree
into the type-check gate. Tests for it therefore sit beside it, and `vitest.config.ts`
includes `mock/**/*.test.ts` for that reason.

**A third agreement: the mock and the real activity detector.** The fake shell can also
impersonate every agent cornus can classify — `agents` lists them, `agent <name>` (or
just the name) plays one — so the workspace's activity badges, the Overview agent column
and every "this session needs you" affordance can be exercised with no backend. It
reports each screen's state but owns no classifier, and must not grow one: a mock that
judged its own screens would agree with itself and prove nothing. The screens instead
live in `web/mock/agent-screens.json` (the OSC title it sets, the lines it paints, and
the state the agent's own bundled herdr manifest classifies those into), and three tests
hold that file to the manifests, one per side:

- `pkg/agentdetect/screens_test.go` — every entry reaches its declared state **through
  the declared rule**. Asserting the winning rule and not merely the state is the point:
  several rules can fire on one screen, and one that lands on "blocked" via unrelated
  evidence keeps passing after the rule it was written for stops working. It also
  requires a screen for every state a manifest can produce (so refreshing the bundle with
  a new agent fails until the mock can show it) and holds every line inside the
  detector's 12-row by 80-column window.
- `cmd/cornus/internal/webbff/agentscreens_test.go` — the **same bytes through a real
  session**: `readLoop`, the OSC scanner that decides what a terminal would paint, the
  headless VT100, and `renderLocked`'s bottom-12 window, before any rule sees them. This
  is what makes "end to end" mean anything; classifying a string exercises none of it.
- `web/mock/agentdemo.test.ts` — the mock's own half: every agent is playable, the state
  it reports is the corpus's, and a blocked screen holds until a human answers rather
  than timing out.

Two constraints on the emission, both load-bearing. Each screen is preceded by
clear-and-home, because a TUI repaints rather than scrolls — without it the previous
step's lines stay in the render window and keep matching their own rules, so a blocked
prompt arrives under a working footer that is still true (this is why the mock's test
plays a whole sequence through ONE session; the Go test gives each entry a fresh one and
cannot catch it). And an entry cannot key on OSC 9;4 progress: the BFF's scanner does not
capture it (`setOSC(u.title, "")`), so the `osc_progress` regions two manifests use are
unreachable in cornus today — see TODO.
...
- **kube — the redirect arm.** The client-local bind redirect keeps the explorer fast
  there, and the arm asserts the two routes that cover everything else: a volume-backed
  path goes through the caretaker's filesystem operator, and an IMAGE-layer path — which
  the caretaker structurally cannot see — goes through tar over exec
  (`pkg/deploy/kubernetes/archive.go`, the kubectl cp mechanism). The image-path copy is
  compared against what `pod_exec` reads, since a status alone would be satisfied by an
  empty file, and its preflight must NOT say `server`: an image path has no operator root,
  so claiming that route means the planner is about to ask the caretaker for something it
  cannot serve. The arm writes through the BFF and reads it back with
...
make e2e-incus-container                      # the incus counterpart: runs the incus subset against
                                              # the incusd the image itself ships (Incus 7.3 from
                                              # Zabbly), started + initialized by the entrypoint
make e2e-podman-container                     # the podman subset against a ROOTFUL podman API
                                              # service the entrypoint starts (no host podman)
make e2e-podman-rootless-container            # the same against a ROOTLESS service, run as the
                                              # image's unprivileged `rootless` user
```

The rootless leg needs nothing rootless on the HOST — the image carries a
`rootless` user with `/etc/subuid` + `/etc/subgid` ranges (without them podman
cannot map a user namespace and every container create fails with "no subuid
ranges found"), plus `slirp4netns`/`passt` for rootless networking and
`netavark`/`aardvark-dns` under `/usr/lib/podman/` for name resolution. The
entrypoint's `start_podman_rootless` starts `podman system service` as that user
on `/run/user/1001/podman/podman.sock`.

Both podman legs point the CLI at the socket under test with `podman --url ...`
in `Setup` and in `probePodman`. Without it the CLI answers from whatever daemon
it defaults to, which on the rootless leg is a DIFFERENT daemon than the server
drives — a preflight that passes for a runtime nothing in the run ever talks to.
...
on pushes to `main` and via manual `workflow_dispatch`. The matrix has eight legs,
all `fail-fast: false` so one flake cannot mask another leg's real regression:
`docker`, `kube`, `containerd`, `bare`, `incus`, `podman`, `podman-rootless`, and
`kube-ingress` (a second kube run with a real ingress controller, narrowed to the
ingress scenarios). Every leg is one privileged `docker run` of the same image —
nothing is installed on the runner host — so the `incus` leg needs no host Incus
(the image ships its own incusd) and the `podman-rootless` leg needs nothing
rootless (the image ships the unprivileged user and its subuid/subgid ranges).

The two podman legs are separate on purpose. Rootlessness changes what is
REACHABLE, not what the libpod engine encodes, and
`deploy-portforward-rootless-podman.star` asserts BOTH halves of that: the forward
must SUCCEED on rootful and be REFUSED by name on rootless. Dropping the rootful
leg would admit an over-broad refusal; dropping the rootless leg would admit a
removed one. The `bare`, `incus`, `podman` and `podman-rootless` legs set
`E2E_STRICT=1` and run a preceding `E2E_PREFLIGHT_ONLY=1` gate step, because a leg
that exists for exactly one backend must fail, not green-skip, when that backend's
daemon/runtime is missing. Note what that flag does per leg: for `bare` and `incus`
it CONVERTS a real self-skip into a failure (a missing crane/agent image, an incusd
too old for OCI). `E2E_STRICT` has no podman arm — the podman bring-up already
fails the target rather than self-skipping — so on those two legs the value is the
gate itself, which names a missing binary or unreachable socket in about a minute
instead of letting it surface as a pile of failed deploys. It builds the runner
image with a `type=gha` layer cache and
