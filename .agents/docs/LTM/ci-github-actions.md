...
## July 2026 Reliability Findings

- Release binary caches separate the architecture-independent module cache from
  the GOOS/GOARCH-specific compiler cache. `setup-go cache: true` does not encode
  the matrix target and can restore incompatible build artifacts.
- Dockerfile BuildKit cache mounts improve reuse within one multi-platform build,
  but `cache-to: type=gha` does not export cache-mount contents between runs.
  Cross-run cache dancing was declined because of signing-job supply-chain
  surface, cache-budget pressure, and weak payoff.
- The E2E runner pins Docker Engine 27.5.1. An unpinned apt install drifted to
  Docker 29 and changed Compose plugin dispatch and attach behavior. The pin is a
  deterministic test environment, not a claim of Docker 28/29 compatibility.
- The yamux race job uses `-race -short` plus a separate non-race full run:
  scheduler coverage remains under the detector while the 16 GiB throughput test
  runs without its known detector timeout.

## Nested Modules and License Bundle Exceptions

`pkg/wire/sqliteab` is a nested Go module and must pass its own
`GOFLAGS=-mod=readonly go vet ./...`; a green root-module gate does not prove its
`go.mod` is tidy. Run `go mod tidy` in that module when its MVS result changes.

`github.com/rootless-containers/proto/go-proto` publishes no license file in its
module zip even though the parent repository is Apache-2.0. `go-licenses` ignores
that module on both save/report paths, then the build deliberately re-injects the
canonical license text and attribution row. Never turn the ignore into missing
shipped attribution.

## Backend, Observability, and License Gates

The E2E workflow covers Docker, Kubernetes, containerd, bare, Incus, and the
controller-backed Kubernetes ingress leg. Bare/Incus perform strict preflight so
an unavailable runtime cannot produce a green job that deployed nothing.

The Kubernetes leg can build with `imbh`/`sable_extern_lib` and install
metrics-server, turning the observability scenario family from self-skips into
real assertions. The first activation found product and fixture defects, which is
the evidence the leg was needed.

The CI license job performs both policy classification and byte-for-byte notices
drift. It reads `LICENSE_TAGS` from the Makefile and obtains module `.Dir` from
`go list`, covering local replacements such as the vendored yamux fork.

Docker 29.2.1 was A/B tested with a runner differing only in Docker version. Most
proxy scenarios pass and foreground attach defects were fixed, but the Dev
Container scenario still times out later in the CLI flow. The Docker 27.5.1 pin
therefore remains a measured compatibility boundary, not an unexamined old pin.

CI logs can lag the dirty working tree in this amend-based repository. Before
triaging a run, compare the tested commit with local HEAD and `origin/main`; a fix
may already exist locally but not in the run under investigation.

## August 2026 CI Corrections

The license job now scans the release-platform union and its byte-for-byte drift gate caught a herdr vendor update without regenerated notices. Release scripts must stay portable: the Windows leg failed on Unix-specific `tee /dev/stderr` before linking, not because observability was unavailable. Concurrent hub tests now wait for the prior registration lifecycle before starting another round.
