...
green (9/9) on a real privileged Docker host. `entrypoint.sh`'s `start_incus`
...
## Implemented Data Plane and Runner

The backend implements `StatPath`, `CopyFrom`, and `CopyTo` through the Incus file
API, proxy-device port forwarding, console-backed logs, and websocket exec/attach.
Console logs have neither per-line timestamps nor stdout/stderr separation, so
unsupported filtering/follow semantics are reported rather than fabricated.

A Debian-based privileged runner with a real Incus daemon has passed all seven
Incus scenarios. The official client remains pinned at
`github.com/lxc/incus/v6` v6.18.0; treat a bump as both API and license-audit work.

## OCI Mapping and Remote Mode

Incus maps `oci.entrypoint`, absolute `oci.cwd`, and numeric `oci.uid`/`oci.gid`.
Command-only overrides, relative working directories, and username-form users
remain unsupported because Incus exposes only whole argv and numeric OCI keys.
Each key was traced through the vendored Incus source to its runtime consumer;
declaration alone is not evidence that an override takes effect.

Server-host binds map to disk devices, named and anonymous volumes map to custom
storage volumes, and anonymous names are recorded on the instance for deletion.
Named volumes outlive deployments and are removed through `VolumeRemover`.
Sysctls, supported ulimits, tmpfs, and shm size also map. The unsupported-field
suite accounts for every top-level DeploySpec field and requires one warning per
unrealized value.

Remote mode uses a sibling companion and a shifted shared storage volume for the
agent socket. It supports TCP/UDP forwarding and SSH-agent relay. Mount relay and
egress remain structurally blocked because a sibling mount cannot propagate into
the application namespace.

Console logs support snapshot tail and live follow. Since, until, and timestamps
remain impossible because the Incus console carries no per-line time metadata.
Attach order favors possible duplication over an unobservable gap between the
snapshot and live stream.

Internal registry credentials must be URL-userinfo-safe. Compact JWS base64url plus
dot satisfies that constraint; a future opaque or standard-base64 credential must
not be substituted without changing the Incus auth transport.
