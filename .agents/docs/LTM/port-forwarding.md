...
- TCP everywhere; UDP on every HOST backend — dockerhost, containerd, bare and incus — via a `/udp`
  mapping suffix (e.g. `5353:53/udp`). kubernetes stays TCP-only — `pkg/portfwd` probe-detects that and
...
- CLI mapping suffix: `[LOCAL:]REMOTE/udp` (e.g. `5353:53/udp`); implemented on every host backend.
  incus implemented it from the start but did not DECLARE `SupportsUDPPortForward`, so the server's
  optional-capability assertion failed and every udp tunnel against it was refused as unsupported —
  fixed 2026-07-31, with `pkg/deploy` guarding both directions (a udp branch without the declaration,
  and a declaration without a branch).
...
- Unit: `TestPortForwardWS` (server preamble + bridge + port decode via an echoing
  `fakeBackend`); `TestForwardPortEchoes` / `TestForwardPortUDPEchoes` / `TestForwardPortRejectsUnknownProto`
  (dockerhost — a local echo listener stands in for the container, the fake engine inspect returns
  `127.0.0.1`); `TestUDPForwardCapabilityValues` + `TestEveryUDPForwardingBackendDeclaresIt`
  (`pkg/deploy`, cross-backend: implements udp <-> declares the capability);
