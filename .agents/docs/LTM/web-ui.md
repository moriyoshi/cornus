...
Loaded projects own workload, mount, forward, graph, and Apply sections;
origin-attributed but unloaded workloads remain in `Other`.

Files and terminals share a tiled model with splits, stacks/tabs, whole-stack
drag, placement prompts, and modal services. File panes browse confined local
