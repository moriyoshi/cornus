...
(2026-08-02) Apply is **MCP-only**. `Server.Apply` is the one core method with an MCP adapter and no
HTTP route: the `POST /.cornus/web/projects/{name}/apply` endpoint, the `applyProject` client
function, and the "Apply (up -d)" button on each loaded project heading were all removed. The UI is a
companion to the CLI, not a second front door to it, so re-deploying belongs in the terminal the user
already has open; an agent driving `project_apply` has no such terminal. `flushWriter` in
`handlers.go` went with the endpoint (`pkg/server` has its own, unrelated).
...
- Node/npm may be absent from a non-interactive `PATH` on a development host; locate the installed
  toolchain and prepend it for the command at hand rather than assuming a fixed location.

## Overview and Tiled Workspaces

The Overview is project-oriented by default and can pivot to workload grouping.
Loaded projects own workload, mount, forward, and graph sections (no Apply — see
above); origin-attributed but unloaded workloads remain in `Other`.

Files and terminals share a tiled model with splits, stacks/tabs, whole-stack
drag, and modal services. Where a pane goes is asked with the on-screen wireframe
targets, not a prompt: the tiling chrome lights the focused tile with a cross of
direction buttons (arrows/hjkl place, Space stacks) and the same overlay serves
move, new-pane, and split. Going TO a pane is the mirror of that gesture —
`prefix s` lists the panes in the corner while the arrows walk the workspace
itself, previewing without moving the focus until Enter. File panes browse confined local
roots and workload filesystems and support cross-mount copy, editing,
upload/download, image preview, breadcrumbs, and keyboard navigation.

Terminal close codes distinguish explicit end, session active elsewhere, lost
transport, and stale session. Ctrl-D closes the pane; ambiguous loss offers
reconnect. The mock BFF must match the production close-code contract.

## Animated Brand Asset

`assets/cornus-logo-animated.svg` is a separate one-shot animation; the static SVG,
PNG, and Illustrator sources remain unchanged. Its six original path geometries
are preserved byte-for-byte and grouped into the outer shell, workload tiles, and
central blossom.

The shell rotates around its vector-area centroid rather than the canvas or
bounding-box center; the inner groups use their measured composition center. Each
layer settles after an integral number of turns, preserving the original final
orientation. `prefers-reduced-motion` disables all animation.

## Unified Workspace and Tiling

Files and Terminal share one tiled Workspace. Tiles hold tab stacks; split, move, next, previous, rotate, close, and open-in-terminal use one command vocabulary and a pane chooser instead of separate placement prompts. The chooser supports keyboard traversal, numbering, a pinned mini-map, scrolling, and viewport panning. Touch uses explicit pick/drag handling and two-finger workspace pan rather than desktop-only HTML drag events.

The workspace may extend beyond the viewport while preserving separate minimum width and height floors. New panes extend bounds only when necessary and use golden-ratio sizing without privileging the source pane. DOM focus routes keys while tile focus represents workspace state. Tests must supply real coordinates and rectangles because jsdom defaults can make edge, centre, and scroll cases pass vacuously.

## Files Workspace

The explorer supports cursor and range selection, rubber-band selection, drag-and-drop, batch copy/move, F5/F6 cross-pane transfer, editor dirty-state preservation across retiling, and opening a chosen directory in a terminal. Activation is centralized as an Open command so pointer, keyboard, palette, and menu paths share semantics. A global toaster owns operation feedback.

## Terminal Introspection and Activity

Terminal tabs learn foreground titles, working directories, and shell PIDs through OSC title, OSC 7, and an in-session PID announcement. String control sequences are removed from the activity detector's screen model. Detection rules scope to the live foreground program, with a herdr-compatible agent classifier and dev fixtures for known agents. Kubernetes E2E distinguishes transport availability from process-classification accuracy.

## Page Structure and Metrics

A fixed header replaces the sidebar. Overview and Workload detail use plain sections and independently scrolling tables, remove columns implied by scope, keep backend facts in the Server card, and show Ingress separately. Apply was removed so the web companion remains an observation and operation surface rather than a second deployment front door. Metrics charts appear beside the project or workload they describe.
