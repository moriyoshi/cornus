...
- SSH agent forwarding is tunnelled, not served over 9P: caller declares `BuildSpec.SSHIDs`; the server creates a temp unix socket per id and, per BuildKit connection, opens a **server-initiated** `S`-tagged yamux stream back to the caller, which proxies to `$SSH_AUTH_SOCK` (`pkg/build/buildwire/ssh.go`).
...
- `pkg/build/buildwire/p9fs.go` — p9-backed `fsutil.FS` adapter (stat wrapping, ENOENT mapping, SkipDir handling, Linkname over the wire).
- `pkg/wire/confinedfs.go` — `confinedAttacher`, `validComponent`, `guard.confinedParent`/`confinedFollow`, `loadDockerignore`, read-only enforcement.
- `pkg/build/buildwire/ssh.go` — SSH agent tunnel (`ServerSession.SSH()`, `serveSSH`, stream tagging, `pipe`, `readLine`).
