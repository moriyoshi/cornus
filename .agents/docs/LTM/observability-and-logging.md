...
- `cmd/cornus/caretaker.go` — caretaker `Setup` with `service.name=cornus-caretaker`. (The former
  `cmd/cornus/mountagent.go` carried the same wiring for the `mount-agent` alias; that file and both
