...
## Evidence and Harness Refresh

This synthesis also incorporates [verification-and-audit-methods.md](./verification-and-audit-methods.md). Consolidation records are navigation hints, not proof: verify changed symbols and surrounding claims against the tree. For a conjunction of refusal guards, isolate each negative fixture so no earlier guard can make the named row pass. Prefer asymmetric protocol oracles, positive controls that prove work occurred, and realistic one-line neutralizations.

Harness cleanup is capability confinement, not argument validation: `remove_all` resolves only beneath harness-owned temporary roots. Identity-sensitive scenarios compare stable workload IDs rather than counts. Live target execution remains separate evidence from `e2e-check`; tagged/cgo observability runners, backend labels, and KEEP cleanup must be verified independently.

The release-platform license union and Windows-script portability failures reinforce the same rule: run the exact matrix path whose behavior is claimed. A green host-native approximation is not evidence about target-specific file selection or shell semantics.
