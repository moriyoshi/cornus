# Docker-Compatible Client Surfaces Synthesis (Compose, providers, Docker proxy, devcontainers)

## Summary

Cornus exposes Docker-compatible clients through one translation and lifecycle stack.
`cornus compose` drives the native Compose model; provider services delegate selected resources
to external plugins; `cornus daemon docker` lets stock Docker clients drive Cornus; and native
devcontainer.json support produces the same Compose project model. These surfaces share
`pkg/client`, `pkg/compose`, deploy-attach session lifetime, build-wire fields, and Docker-parity
semantics for naming, labels, lifecycle, and exec. Provider resources remain external and join
that pipeline through dependency readiness and environment injection rather than deployment.
...
| [compose-provider-services.md](./compose-provider-services.md) | External provider protocol, dependency readiness, environment injection, lifecycle, and reload |
...
- A `provider:` service is mutually exclusive with `image`, `build`, and `deploy`. It has no
  Cornus workload and appears as `provider:<type>` in status.
- The runtime resolves `docker-<type>` or `<type>` on `PATH` and invokes `up`, `down`, or `stop`.
  Start maps to `up`; restart is `stop` followed by `up`.
- Provider stdout is an NDJSON-capable stream, not JSON-only output. `setenv` and `rawsetenv`
  inject values into dependents after readiness; explicit dependent environment wins.
- Foreground reload and detached watch run provider `up` again. Correct reconciliation therefore
  depends on plugin idempotence.
...
- Keep provider execution in the Compose runtime rather than translating it into a synthetic
  deployment. Preserve topological readiness and environment precedence.
...
- `cmd/cornus/internal/composecli/provider.go` - provider process and stream protocol
...
- Provider parser and helper-process tests cover schema exclusion, binary resolution, stream
  events, environment precedence, lifecycle, and reload; a registered E2E uses a real plugin.
...
- Provider output may contain diagnostic non-JSON lines, and repeated `up` must be safe during
  watch/reload reconciliation.
...
## Reconciliation and Credential Refresh

Dockerhost now preserves exact app-container identity for an unchanged plain co-located Apply. Its fingerprint covers the normalized whole `DeploySpec` and resolved image content ID, so a rebuilt mutable tag recreates even when its reference is unchanged. Companions, unknown fingerprints, changed specs, and self-container protection retain the recreate path; `--force-recreate` changes a label inside the fingerprint.

Compose `x-cornus-credentials` maps services to brokered credential requests without flattening credentials into ordinary client-resolved environment values. Backend capability and trust-boundary rules still govern realization. Docker proxy stream compatibility follows the deploy contract: stdcopy for non-TTY, raw bytes for TTY.
