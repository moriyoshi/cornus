...
- Supported service fields: image, build (incl. `target`, string-or-list `cache_from`), command, environment, env_file, ports, volumes (bind + named), restart, depends_on, deploy.replicas. Top-level `volumes:` is parsed; networks and `extends` are unhandled.
