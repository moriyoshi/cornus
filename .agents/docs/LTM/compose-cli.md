...
- `pkg/dockerproxy/translate.go` — `toDeploySpec`, `parseNamedVolume`, `isHostPath`
- `internal/deploy/kubernetes/` — bind-mount rejection, sidecar mounts, `namedPVCName`, `claimName`
- `pkg/build/builder/solve_linux.go` — progressui build output
...
- `pkg/client/client_test.go` — Deploy/List/Status/Delete/Action, Build tar contents + `build-arg` query, `TestClientBuildTargetAndCacheFrom` (target stage over the wire, cache_from -> registry import, empty entry dropped).
...
- `pkg/build/builder/solve_linux_test.go` — `frontendAttrs` target + named-context filtering, empty TargetStage sets no attr, cacheEntries registry import.
...
## Reconciliation and UX Additions

- `up` and `down` detect project-labeled orphans; `--remove-orphans` removes them.
- Independent services launch concurrently while their existing dependency waits
  remain the topology gate.
- `--progress=status|stream` selects a collapsed live line or durable event stream.
- `up --watch` reloads and re-reconciles; one-shot replacement tolerates Kubernetes
  termination/PVC races and reuses stable mount-session ids.
- Missing bind sources are created for Docker parity unless
  `bind.create_host_path: false` requests failure.
- Provider services are detailed in
  [compose-provider-services.md](./compose-provider-services.md).

Client errors must not fail-fast cancel the shared conduit and every peer mount;
transient retries remain scoped where repeating deployment cannot conflict with
attachment state.

## Version Skew and Partial-Apply Cleanup

`api.ServerInfo.Version` lets Compose warn when known client/server builds differ.
Two `dev` builds cannot be distinguished, so stamped releases and reproducible
artifacts remain the real defense against dev-to-dev skew.

Restart-disabled one-shots retain a small transient infrastructure retry budget;
`restart: "no"` means do not restart after application exit, not that scheduling,
mount, or PVC races receive exactly one attempt.

When deploy-attach Apply fails after creating a workload but before dependents
finish, the server deletes the partial workload before dropping its mount session.
Otherwise an orphan can run against an empty/torn-down mount and obscure the
original apply error. Server lifecycle logs key register, apply, readiness,
disconnect, and teardown events by deployment plus session digest.

## Merge, Flag, and Reconciliation Semantics

Multi-file loading preserves YAML key presence and the `!reset` / `!override`
tags before decoding into typed service structs. This permits an override file to
turn inherited booleans off, clear a scalar intentionally, reset a sequence or
mapping, or replace rather than append. Treating zero values as absent cannot
implement Compose merge semantics.

Selective `compose up SERVICE` expands the transitive `depends_on` graph by
default; `--no-deps` opts out. Expansion is restricted to selected/profile-active
plans and emitted in stable project order. The live E2E fixture uses a two-level
fan-out so a one-hop implementation cannot pass.

Flag compatibility is explicit in `flagcompat.go`: implemented flags are mapped,
unsupported lifecycle/image flags are accepted with actionable warnings, and
parser-shape divergences such as `logs -f` are documented at the failure point.
Never accept a flag silently when the requested behavior is not honored.

`--force-recreate` uses a per-process label. Host backends already recreate on
Apply; on Kubernetes the label changes the pod template and forces a rollout.
The remaining higher-level divergence is that ordinary repeated `compose up` and
watch reconciliation still recreate unchanged services on the host backends.
Tests of this behavior must compare instance identity, not sample an
instantaneous running count.

## Apply Identity and Brokered Credentials

Dockerhost fingerprints the normalized whole `DeploySpec` plus resolved image content ID and stamps `cornus.spec-hash`. An unchanged plain co-located deploy reuses exact app instances and starts stopped ones. Changed specs, mutable-tag content changes, companions, unknown fingerprints, and the server's own container take the guarded recreate path. `--force-recreate` remains effective because its per-process label participates in the hash.

`x-cornus-credentials` maps Compose services to brokered credential requests. They remain subject to backend capability and trust-boundary rules rather than becoming ordinary client-expanded environment values.
