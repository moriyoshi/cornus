...
## Review Baselines and Diagnosability

The July 2026 fresh-eyes review found strong local craft indicators but a large
maintenance surface across five deploy backends and independently moving runtime
stacks. Strategic scope, security, and versioning decisions remain in `TODO.md`;
the durable method is to combine source review with execution of shipped
artifacts and the documented first-user path.

Startup rejects unambiguously invalid backend names and reports unwritable
registry stores on write. A universal writability startup failure would reject
valid read-only servers, so any preflight must preserve that mode. Investigations
should distinguish builder privilege, registry acceptance, data-directory
permissions, and deploy readiness rather than treating adjacent symptoms as one
failure.

## July 2026 Consolidation and Product Decisions

The 2026-07-27 through 2026-07-31 journal established several decisions that
should not be re-opened without new evidence:

- Keep all five deploy backends. Bare and Incus were retained, then given
  fail-closed CI legs and substantially stronger unit coverage.
- Continue maintaining the Japanese and Simplified Chinese documentation trees.
  English remains the source of truth; translation freshness is now tracked per
  page.
- Keep the amend-based pre-1.0 history workflow. Its costs are explicit: no
  ordinary bisect/review units, unreachable release tags after another amend, and
  dependence on a preserved reflog.
- Keep `cornus serve` bound to all interfaces by default. Caretaker dial-back
  features need a routable server; a loopback default passed caller audits while
  breaking the topology.
- Do not enable authentication by default until the internal dataplane and
  onboarding path can preserve the no-extra-bother constraint. Scoped internal
  credentials and SSH client identity were implemented without changing that
  policy.

The durable documentation boundaries are now explicit: README is the repository
landing page, `docs/` owns user tasks and interface reference, `ARCHITECTURE.md`
owns system design, `OVERVIEW.md` orients agents, and JOURNAL/LTM/TODO retain
findings, reusable memory, and open work.

The product review's highest-leverage direction is a polished golden path:
existing Compose or Dev Container project, remote build, Kubernetes execution,
local edits, and localhost reachability with minimal setup. Breadth is evidence
