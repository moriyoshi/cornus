...
Recognized devcontainer fields must be either mapped or warned about. The boundary
tests cover unsupported fields individually and also require a definition using
only the supported subset to remain silent.

Single-container `runArgs` maps roughly 35 Docker-run flags through the compose
model, including user, workdir, entrypoint, restart, capabilities, security
options, DNS, mounts, ports, resources, TTY, and init. `build.options` maps cache,
target, build args, labels, network, SSH, hosts, platform-related options, and
resource settings. Compose-based definitions warn when their single-container
`image` or `build` fields would otherwise be ignored.

The Kubernetes E2E verifies backend realization, not merely the synthesized
compose plan: added/dropped capabilities, `/dev/shm` size, and hostname are read
inside the running container. Choose assertions that the target backend actually
maps; unsupported `runArgs` should remain explicit warnings rather than being
smuggled into the fixture.
