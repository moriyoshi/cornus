...
- `cmd/cornus/` — `caretaker.go` (caretaker / caretaker-check subcommands), `netredirect.go` (`net-redirect`, google/nftables over netlink; the former `_linux`/`_other` build-tag split is now one file, and the `mountagent.go` deprecated aliases were removed), `winch_unix.go`/`winch_windows.go` (`notifyResize`, the SIGWINCH cross-build fix).
