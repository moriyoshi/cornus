...
- Resize is an OUT-OF-BAND control-plane call, not in the hijacked stdio stream: `Backend.ExecResize(ctx, execID string, height, width uint) error` in `pkg/deploy/deploy.go`; server route `POST /.cornus/v1/deploy/exec/{id}/resize?h=&w=` (`pkg/server/deploy_exec.go`); `Client.ExecResize` (`pkg/client/client.go`); dockerhost forwards to the daemon's `/exec/{id}/resize` (`engine.go execResize`, plain non-hijack POST).
...
- `pkg/deploy/deploy.go` — `Backend` interface (Stats, StatPath, CopyFrom, CopyTo, ExecCreate, ExecStart, ExecInspect, ExecResize, Attach)
- `pkg/deploy/dockerhost/engine.go` — raw-HTTP Docker pass-throughs, hand-rolled `hijack`/`hijackedConn`, `execResize`, `firstInstanceID`
- `pkg/deploy/kubernetes/kubernetes.go` — remotecommand-based exec/attach; stats/cp not-supported stubs
- `pkg/server/deploy_exec.go` — exec/resize server routes; `handleDeployItem`/`handleDeployStats`/`handleDeployArchive` on the server
- `pkg/client/client.go` — `Client.ExecResize` and the other client methods
...
## Teardown Ordering and Version Boundary

`/stop` and `/rm` once raced the attach-session watcher, allowing the response to
return before a published-port listener closed. Teardown is now single-owner and
the handler waits for session completion before reporting success.

The E2E environment pins Docker 27.5.1. Docker 29 changed Compose plugin dispatch
and foreground attach behavior beyond the implemented subset. The pin prevents
environment drift; Docker 28/29 compatibility remains separate product work.

## Engine API Corrections and Docker 29 Boundary

Named volume deletion and `prune --all` reach `deploy.VolumeRemover` before the
in-memory proxy record is forgotten. In-use volumes return a conflict, unsupported
backends return 501, and no success is reported while backend data survives.
Default prune remains empty because API 1.42+ prunes anonymous volumes by default
and the proxy tracks named volumes.

Docker create requests now carry `Tty`, explicit/default `restart: no`, and
`MaximumRetryCount`; inspect reports the same TTY value so the CLI selects the
correct raw versus stdcopy decoder. Wait returns a real terminal exit code when
known and the explicit unknown value 125 with an error when teardown prevented an
observation.

Foreground `docker run` exposed two attach defects in the create -> attach ->
start ordering: output written before session establishment was lost, and the
stream did not EOF after a short-lived workload exited. The proxy requests log
replay only on that ordering and closes after exit plus a quiet drain interval;
an attach to an already-running container does not replay history. Both unit and
Starlark regression tests were neutralized against the old behavior.

Docker 29.2.1 passes the direct foreground attach scenario after these fixes, but
the full `devcontainer-vscode.star` still times out later in its third-party CLI
flow. Keep the E2E runner on Docker 27.5.1 until that remaining blocker is
characterized. The proxy also still advertises `raw-stream` for non-TTY attach
where dockerd advertises the multiplexed content type.
