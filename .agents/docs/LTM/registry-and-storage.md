...
- `ObjectStore` interface is just `Get` / `Put` / `Stat` / `Delete` / `List` / `Close` (all `ctx`-threaded); adding a backend is ~5 small methods. Registry semantics (sha256 CAS, digest verification, resumable upload staging, manifest/tag/repo indexing) live once in `pkg/storage/cas.go`.
...
- Optional `NativeUploader` capability: filesystem commits by renaming the session file (no copy); S3 (`pkg/storage/s3.go`) streams to S3 multipart; other gocloud backends fall back to local staging (temp file, streamed on commit).
...
- `pkg/storage/drivers_cloud.go` (`//go:build cloudblob`) blank-imports `gocloud.dev/blob/gcsblob` + `azureblob` and sets `const cloudBlobBuilt = true`; `drivers_nocloud.go` (`//go:build !cloudblob`) sets it false.
...
A generic `http(method, url, body?, headers?)` E2E harness builtin (returns `status`/`body`/`headers` keyed by canonical Go header names; registered in `predeclared()` + `predeclaredNames()`) drives `e2e/scenarios/registry-edges.star` against the real `pkg/registry/registry.go`:
...
- `pkg/storage/cas.go` — shared `Backend` layer (CAS, manifests, tags, uploads)
- `pkg/storage/filesystem.go` — native filesystem backend (default)
- `pkg/storage/blob.go` — gocloud blob backend
- `pkg/storage/s3.go` — `s3ObjectStore` with `NativeUploader` multipart
- `pkg/storage/open.go` — ref routing; `openS3Bucket`
- `pkg/storage/drivers_cloud.go` / `drivers_nocloud.go` — cloudblob tag gating
- `pkg/registry/registry.go` — registry HTTP handlers
...
- `pkg/storage/storage_test.go` — full CAS / chunked-upload / manifest / tag / repo / dedup suite, table-driven across filesystem + memory.
- `pkg/registry/registry_test.go` — `TestBackendsConformance` runs go-containerregistry push/pull/tags over filesystem + memory.
- `pkg/storage/s3_test.go` — `TestS3Backend` + `TestS3MultipartUpload`, opt-in via `CORNUS_TEST_S3_ENDPOINT` (e.g. `http://127.0.0.1:5555`), skip otherwise. Multipart test uploads >5 MiB in 3 chunks, reconstructs the session via `GetUpload` between writes (proves sidecar resumption), and asserts wrong-digest commit yields `ErrDigestMismatch` with no blob.
- `pkg/storage/cloudblob_test.go` (`//go:build cloudblob`) — opt-in GCS/Azure round-trips via `CORNUS_TEST_GCS` / `CORNUS_TEST_AZBLOB`; both PASS against fake-gcs-server and Azurite with zero code changes; `drivers_nocloud_test.go` asserts the clear default-build error.
...
`storage.Backend.Usage` lists `blobs/sha256/` and stats each object, tolerating a
blob deleted between List and Stat. `GET /.cornus/v1/storage` combines CAS usage
with block-cache files/bytes; pure host-native re-export reports zero CAS. It is
read-only and authn-governed but not part of the cheap `/.cornus/v1/info` probe.

`cornus storage usage --format text|json` consumes `api.StorageUsage`. The walk is
O(blob count), potentially one HEAD per S3 object. Quota enforcement remains a
