...
`pkg/svcforward` discovers service backends with `discovery.k8s.io/v1` EndpointSlices rather than deprecated core/v1 Endpoints. Setup and automatic forwarding retain port selection and readiness filtering without the deprecation warning. Tests must model EndpointSlices rather than fake Endpoints objects the production resolver no longer reads.
