...
The "barehost and containerdhost" framing holds for the runtime slices (OCI spec options, netns, hosts, volumes, CNI) but NOT for the Stats layer, which has since become the shared Docker-stats vocabulary for every backend — see the Stats paragraph below.
...
Stats is used well beyond the two daemonless backends and is best read as the project-wide Docker-stats vocabulary rather than as host-runtime code. `StreamStats` plus a sampler is also how kubernetes (`pkg/deploy/kubernetes/stats.go:180`, sampling the metrics API) and incus (`pkg/deploy/incushost/stats_linux.go:31`, sampling instance state) serve stats, and dockerhost decodes the daemon's own stats frame into `hostrun.DockerStats` (`pkg/deploy/dockerhost/dockerhost.go:652`) rather than defining a parallel type. A change to the Stats types is therefore a five-backend change, not a two-backend one.
