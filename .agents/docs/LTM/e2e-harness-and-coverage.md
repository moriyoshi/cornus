...
- Hub multi-replica validation is IN the Starlark suite as of 2026-07-28: `hub-multireplica-redis.star` (docker target, real Redis container) and `hub-multireplica-kube.star` (kube target, the API server as the store), both in `SCENARIOS`. It used to be two shell scripts with their own `make e2e-multireplica*` targets, because the harness drove exactly ONE `cornus serve`; `serve(name=, addr=)` now starts additional replicas with their own data dirs and handles, leaving `h.server`/`h.client`/`h.registryHost` — what every other builtin means by "the server" — untouched. The move was not cosmetic: as scripts the two drifted apart (one gained a JWT-only posture, the other did not) with nothing to catch it, since a script is not parse-checked, not resolve-checked, and invisible to `TestPredeclaredNamesInSync`.
...
- `/home/moriyoshi/src/cornus/cmd/cornus-e2e/` — runner binary (`--target`, `--check`, `--preflight`, `--skip-preflight`, `--keep`, `--cornus`, `--compose`, `--storage`).
- `/home/moriyoshi/src/cornus/pkg/e2e/harness.go` — builtins, per-role data dirs, `predeclared()`/`predeclaredNames()`.
- `/home/moriyoshi/src/cornus/pkg/e2e/value.go`, `target.go` — Starlark value plumbing; DockerTarget/KubeTarget.
- `/home/moriyoshi/src/cornus/pkg/e2e/preflight.go` (+ `preflight_test.go`) — Capability probes, `targetNeeds`/`scenarioNeeds`.
- `/home/moriyoshi/src/cornus/pkg/e2e/ftp_test.go` — in-process fake FTP server (PASV bogus-host, PORT dial-back) unit tests.
- `/home/moriyoshi/src/cornus/e2e/scenarios/*.star` + fixture dirs (e.g. `build-edge/`, `build-upload/`, `ftp/` — a nested Go module with its own go.mod, compiled in CI via `cd e2e/scenarios/ftp && go build ./...`).
- `/home/moriyoshi/src/cornus/e2e/container/{Dockerfile,entrypoint.sh,appimage.Dockerfile,multus-daemonset-thick.yml}` — all-in-one runner.
- `pkg/e2e/multireplica.go` — the replica/store/spoke builtins (`serve(name=)`, `redis`, `cornus_bg`, `tcp_echo`, `dial_echo`) behind the two hub multi-replica scenarios. The delivery target is the in-process `tcp_echo()`; the committed static `e2e/echoserver/` it replaced was removed 2026-07-28 with the shell scripts.
- `/home/moriyoshi/src/cornus/Makefile` — `SCENARIOS` list and all `e2e-*` targets.
- `/home/moriyoshi/src/cornus/pkg/build/builder/lock_linux.go` — engine.lock fail-fast (found by the baseline).
...
- **Hub multi-replica**: `hub-multireplica-redis.star` on the docker target (needs docker for the Redis container) and `hub-multireplica-kube.star` on the kube target (reuses the cluster the target already creates). Each gates on `TARGET` and no-ops elsewhere.
...
## Later Harness Lessons

- A scenario that kills a server or workload must clean poison at the start.
  Starlark has no `defer`, and a failed run can leave a same-named workload that
  satisfies `deploy_attach`'s name-based readiness poll on the next run.
- A timeout is containment and diagnostics, not a root-cause fix. Preserve trace
  output and the last daemon call when containing a hanging scenario.
- Real controllers are separate coverage, not a replacement for object-shape
  tests. The kube-ingress leg validates class, annotations, certificates, and
  routing that an API-only scenario cannot.
- `mcp.CommandTransport` is used for stdio E2E so tests cover launch, stdout
  purity, multi-request child lifetime, and stdin-close shutdown.
- Mutation checks must compile and must fail on the intended world assertion.

## Retry and Fixture Invariants

- `http_get(retry_5xx=True)` retries transient startup 5xx responses only within
  the caller's retry window and still returns the last 5xx when the window ends.
  Dial-error retry alone is insufficient for a reverse proxy whose upstream is
  still starting.
- `--scenario-timeout` (default 10m, `CORNUS_E2E_SCENARIO_TIMEOUT`, zero disables)
  bounds one Starlark file, kills children through existing context handling, and
  preserves partial output. It contains a hang; it does not explain it.
- The runner explicitly installs and versions Docker Compose/buildx plugins.
  `docker-ce` only recommends them, so `--no-install-recommends` plus a cold layer
  can otherwise make `docker compose` disappear.
- Create `/run/sshd` immediately before launching OpenSSH in the harness; a
  container's fresh `/run` tmpfs does not retain the package-time directory.
- Use bounded `exec(..., retry=...)` only for asynchronously appearing objects.
  A retry cannot fix querying the wrong object name, such as confusing anonymous
  `-vol-0` PVCs with hashed named-volume PVCs.
- Shared-namespace assertions must scope by project. A failed prior scenario can
  leave policies or workloads, and end-of-file cleanup never runs after failure.
- Avoid fixture routes reserved by the test image itself. `traefik/whoami` owns
  `/api`; longest-match ingress tests use an application path such as `/svc`.

## Fail-Closed Coverage Expansion

`deploy_attach` readiness is gated by the current child process's run-scoped
`deployed <name>` event, not by Status observing any same-named leftover. Status
is read afterward only as the returned report. Early-exit and timeout diagnostics
include captured child output.

Bare and Incus have dedicated strict CI legs. `E2E_STRICT=1` converts unavailable
daemons and failed companion-image preparation from green skips to failures;
`E2E_PREFLIGHT_ONLY=1` proves the target before the scenario suite begins.
Fault injection established the fail-closed behavior before the real suites were
credited.

The observability family originally self-skipped because the runner did not link
the store. Tagged cgo runner builds, metrics-server installation, and dedicated
failure rules make the scenarios execute. A self-skip that no dedicated leg ever
forces is not coverage.

Multi-replica Redis and Kubernetes scripts were replaced by Starlark scenarios
plus opt-in multi-server builtins. Scenario cleanup records only resources the
scenario created; daemon-wide `List` sweeps are forbidden because a local Docker
daemon may contain unrelated user workloads.

High-value scenario design rules from the expansion:

+- Assert resulting workload identity/state before trusting CLI output.
+- Use multi-hop fixtures when testing transitive graph expansion.
+- Keep a survivor when testing withdrawal so loss of the whole shared service
+  cannot masquerade as removal of one registration.
+- Gate controller behavior on controller existence and fail if an installed
+  prerequisite is unhealthy.
+- Clean same-named fixtures at scenario start because Starlark has no `defer` on
+  failure.
+- Preflight scanners must recognize capabilities used through shell commands,
+  not only dedicated builtins.

## Safe Cleanup and Identity Assertions

`remove_all` confines resolved targets beneath harness-owned temporary roots; it is a sandbox, not a thin `rm -rf` argument check. Idempotence and replacement scenarios compare stable workload identity rather than counts or names.

Live runs remain essential. The containerized runner formerly let observability scenarios self-skip because its binary lacked the store; the tagged cgo path now executes them. Target labels and KEEP/cleanup behavior need independent checks so a fixture on the wrong backend cannot validate its own expectation.
