...
- TLS serving cert/key and client CA hot-reload on file mtime change (`pkg/server/tlsreload.go`), so cert-manager renewals apply without a pod restart; the Helm chart optionally renders a cert-manager `Certificate`.
...
**dockerhost** (`pkg/deploy/dockerhost/policy.go`):
...
- Server wiring (`pkg/server/server.go`): `defaultBackendFactory(cfg)` builds the policy from env and ALWAYS appends `cfg.MountsDir()` to the allowed prefixes, so deploy-attach mounts (rewritten to `<DataDir>/mounts/<session>/...`) work without operator opt-in while a raw `POST /.cornus/v1/deploy` with `Source:/` is rejected.
- E2E: `pkg/e2e/target.go` sets `CORNUS_ALLOW_BIND_SOURCES=/` in the DockerTarget serve env so `deploy-config.star` still passes; production stays default-deny.
...
`pkg/server/auth.go`: an `authenticator` built from env in `New()`, wired as `Server.Handler() = otelHandler(s.auth.wrap(s.mux))` (auth inside otel so 401s are traced). Verifiers are tried static-first, then each JWT verifier:
...
- Scope semantics (**rewritten 2026-07-28**; the old line described `authtoken.CaretakerOnly` and "an unscoped JWT stays a full credential", both of which are gone). `authtoken.Grant(scope) (Access, error)` resolves a space-separated scope into an allowlist: `api` -> full, `registry:push` -> push+pull, `registry:pull` -> pull, `caretaker` -> the attach endpoint alone, `peer` -> inter-replica forwarding alone. A scope naming none of them — including an empty one — grants NOTHING, and `authtoken.Issue` refuses to mint such a token. `authtoken.Allows(access, endpoint)` is the matrix; its zero values deny.
- Whose scope claim COUNTS is decided by who holds the signing key: cornus's installation secret (`internal`, `sshJWT`) and the operator's own keys (`CORNUS_JWT_HS256_SECRET`, `CORNUS_JWT_PUBLIC_KEY` — what `cornus token issue` signs with) are authoritative; a JWKS is a third party's key set the operator cannot mint with, so its tokens' `scope` grants nothing and `pkg/authscope` decides instead. Cluster ServiceAccount tokens have no scope claim at all and are reachable only through that map. See [ARCHITECTURE.md](../../../ARCHITECTURE.md), *Security -> Third-party tokens: scope mapping and token exchange*.
...
**API policy** (`pkg/server/apipolicy.go`):
...
- `pkg/deploy/dockerhost/policy.go` — dockerhost privilege `Policy`, `PolicyFromEnv`, `PermissivePolicy`
- `internal/deploy/kubernetes` — `Backend.allowPrivileged`, `checkPrivilege`, `caretakerConfigEnv`, `parseSecretRef`
- `pkg/server/auth.go` — `authenticator`, verifiers, `authenticate`, `Subject`/`Identity`, enforcement
- `pkg/server/jwks.go` — `jwksResolver`, `verifyJWKS`, `pickKey`, `jwksAlgs`
- `pkg/server/apipolicy.go` — `CORNUS_API_POLICY` model, `loadAPIPolicy`, action checks
- `pkg/server/tlsreload.go` — `tlsConfig`, `certReloader`, `caReloader`
- `pkg/server/server.go` — `defaultBackendFactory` policy wiring, `Handler()` middleware chain (otel → auth → registryAuthz → mux)
- `internal/authtoken` — shared JWT `Claims`/`Scope` model, `Issue`, `Grant`, `Allows`
- `pkg/caretaker/caretaker.go` — `Config.Token`, `applyEnvToken`, bearer header on the attach handshake
...
## Scoped Internal and External Credentials

`authtoken.Grant` replaces the former two-state `CaretakerOnly` predicate with a
default-deny access model. Empty or unknown-only scopes grant nothing; known
operator scopes remain authoritative. The minting path refuses a scope that
grants no Cornus access, so the server cannot issue a cryptographically valid dud.

Internal dataplane credentials use a separate installation issuer and fixed
audience. Cornus mints only the purpose needed at each edge:
`registry:push` for BuildKit, `registry:pull` for deploy backends, and
`caretaker` for sidecars. The installation secret is either an environment-safe
value of at least 32 bytes or a mode-checked `0600` data-dir file. Helm preserves
one shared secret across replica upgrades.

All five backends carry pull authentication. Docker uses `X-Registry-Auth`,
containerd and bare use resolver credentials, Incus embeds a short-lived
URL-userinfo-safe credential in its OCI source, and Kubernetes refreshes an
ownerless namespace dockerconfig Secret.

Third-party JWT/JWKS identity is mapped to Cornus scopes by explicit claim rules
or `CORNUS_JWT_DEFAULT_SCOPE`. A third party's own `scope` claim never grants
Cornus authority. Operator-held signing keys keep authoritative Cornus scopes and
fall through to the map only when they name none.

`POST /.cornus/v1/auth/exchange` implements the RFC 8693 exchange. Downscoping is
checked by endpoint containment rather than enum ordering, and server-only
`caretaker`/`peer` scopes are not issuable to clients. Client profiles support
exchange plus shared token caching. A cached exchanged token rejected with 401
is discarded and refreshed at most once, only for rewindable requests, and never
for the expected CornusSSH challenge handshake.

SSH-key authentication uses versioned same-endpoint challenge handshakes under
`/.cornus/v1/auth/*`. Proofs bind purpose and canonical request fields, reject
RSA/SHA-1, and cache by stable profile identity rather than a transient local
port-forward address.

## SSH Session Revocation Semantics

Deleting an enrolled SSH key blocks new session minting but does not revoke an already issued cached session before its TTL. E2E coverage proves the warm-cache success and the post-cache-clear minting refusal.

Isolation tests redirect the token cache explicitly. `CORNUS_AGENT_DIR` does not currently override an existing `XDG_RUNTIME_DIR`, and ambient E2E sessions can otherwise pollute the developer's runtime cache. The precedence and harness-wide isolation decisions remain in `TODO.md`.
