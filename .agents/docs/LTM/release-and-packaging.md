...
## All-In-One Observability Artifacts

Released serving artifacts include the embedded Collector and built-in
observability store. The server-only store is selected by build tags; `cornus
observe` is a client and does not need the store linked locally.

Linux release binaries remain fully static despite the Rust-backed store. The
musl build supplies a `libgcc_s.a` shim pointing to `libgcc_eh.a`, uses external
static linking, and keeps Go DNS/user lookup through `netgo` and `osusergo`. The
container image deliberately uses glibc cgo because its build and runtime distro
match.

`--obs` is tri-state: unspecified follows `obsstore.Compiled()`, `--obs` requests
the feature, and `--no-obs` disables it. `cornus version --features --format
json` is a build-pipeline contract; the image and binary jobs require
`"obsstore":"yes"` so a mistyped tag cannot silently ship a stub.

Five binary targets ship because upstream imbh-go has no Windows/arm64 archive.
Native per-target runners make the artifacts executable where they are built,
which permits feature assertions instead of trusting cross-compilation.

Before the next release, measure the emulated arm64 image leg: linking the large
Rust archive under qemu may be too slow or memory-heavy. If it does not hold,
build images on native architecture runners and merge manifests before signing.
