...
## Reconcile Pressure and Replacement Races

The client-go config defaults of QPS 5 / Burst 10 self-throttle a multi-service
Compose reconciliation. Cornus defaults to QPS 50 / Burst 100, overridable with
`CORNUS_KUBE_QPS` and `CORNUS_KUBE_BURST`; API priority/fairness remains the
server-side protection.

On one-shot replacement, an anonymous workload-owned PVC may still exist but be
terminating after foreground Job deletion. `ensurePVC` reuses a live claim, waits
boundedly for a terminating claim to disappear, and creates a fresh one. Blindly
accepting AlreadyExists adopts a doomed claim and leaves the new pod
Unschedulable.

A lenient caretaker mount readiness check is load-bearing: Compose workloads can
carry optional/failing binds. Requiring every target to prove 9P fstype blocked
otherwise-valid one-shots, so readiness remains best-effort rather than an
all-mounts barrier.

## Field Coverage, Jobs, and Delete Races

`List` enumerates Deployments, Jobs, and optional Knative Services, matching the
object families already handled by Status and Delete. A restart-disabled workload
is a Job; omitting Jobs made one-shot logs invisible to the recorder even though
the workload itself completed correctly.

Applying a Deployment by name must inspect lifecycle state, not only existence. A
terminating Deployment accepts an update and then disappears with it. The backend
waits for deletion and creates a fresh object, with re-fetch handling for a delete
that begins during the update retry.

The per-field contract drives the real fake-client Apply path and asserts both
silence and resulting cluster objects. It exposed duplicate warnings on every
attachment Apply and silent drops for stop signal, init, port host IP, volume
driver options, healthcheck start interval, SELinux, stateless attachments, and
additional Knative ports.

Knative warns only when `Replicas > 1`; unset and one remain silent. The warning
matters because the result is scale-to-zero rather than a fixed single replica.
NetworkAttachment sub-fields are grouped by consequence: isolation, IPAM shape,
IPv6, and Docker-object concepts. In particular, `internal: true` cannot remove
the pod's primary cluster egress and must not be accepted silently.

## Image-Layer Files and Ingress Reporting

Image-layer stat, get, and put use tar over `pods/exec`, avoiding any assumption that a node-visible container filesystem exists while preserving server-side validation.

The web Overview reports Ingress separately from Server. Native and emulated ingress are different realization paths; two UI surfaces reading one mock fixture are not evidence that both live paths agree.
