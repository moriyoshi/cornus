...
The Docker-host flow includes “Docker host (server in a container)”. It emits a
`cornus-compose.yaml` artifact with the daemon socket, `rshared` data-directory
bind, explicit `CORNUS_DATA`, and server port matching the saved connection
profile. Preflight guidance comes before startup so a path-mapping failure is
reported before workloads silently receive empty binds.
