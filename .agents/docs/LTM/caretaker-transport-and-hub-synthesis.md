...
- `pkg/wire/writablefs.go`, `confinedfs.go` — confined 9P attachers (ro + rw)
- `cmd/cornus/` — `hub.go`, `caretaker.go`, `commands.go` (the `mountagent.go` deprecated aliases
  were removed)
  (`--local-mount`)
...
- Live: `deploy-mounts.star`, `deploy-hub.star` (known dind-flaky on kube), `deploy-hub-udp.star`,
  `deploy-proxy-mounts.star`; `hub-multireplica-redis.star` (real Redis) and
  `hub-multireplica-kube.star` (real kind KubeStore).
