...
- A pod's mounts share one deploy-attach session id: `applyWithSidecarMounts` (`pkg/server/deploy_attach.go`) mints one id per Deployment and stamps it on every `AttachMount` with one shared `RelayURL` (`CORNUS_ADVERTISE_URL`), so grouping by (server, session) — later (server) — yields exactly one connection per pod.
...
- `pkg/wire/datagram.go`: `WriteDatagram`/`ReadDatagram` (2-byte big-endian length prefix, max 65535) preserve datagram boundaries over the yamux byte stream; `BridgeDatagram` couples a framed stream to a connected UDP socket (one conn.Read = one datagram).
...
| `hub.RedisStore` (`pkg/hub/redisstore.go`) | `CORNUS_HUB_REDIS` | per-service provider hash + per-replica alive-TTL liveness (5s heartbeat, 15s TTL, pipelined EXISTS filter of dead owners); disjoint per-replica partitions, no CRDT |
...
- Redis: `e2e/scenarios/hub-multireplica-redis.star` (docker target) — real Redis container + two real `cornus serve` replicas + two `cornus hub` spokes; `PING-MULTIREPLICA` round-tripped B → forward to A → hosting spoke → echo. Proves real RESP/TTL + real cross-process WebSocket forwarding (which miniredis cannot).
- KubeStore: `e2e/scenarios/hub-multireplica-kube.star` (kube target) — real kind cluster as the store, two replicas with `CORNUS_HUB_STORE=kube`; CRD self-installed, both replica Leases appeared, `PING-KUBESTORE` round-tripped through the forward path.
...
- `e2e/scenarios/deploy-hub.star`, `e2e/scenarios/deploy-hub-udp.star` (both in the Makefile SCENARIOS list), `e2e/scenarios/hub-multireplica-redis.star`, `e2e/scenarios/hub-multireplica-kube.star` (both in SCENARIOS; the former shell scripts and `e2e/echoserver` were removed 2026-07-28)
- Design docs: `.agents/docs/HUB_NETWORK_DESIGN.md`, `.agents/docs/HUB_PHASE0_CARETAKER_MUX.md`

## Test Coverage

- `pkg/server/caretaker_relay_test.go` — full caller→server→caretaker path in-process, two mounts over one muxed connection read back over real 9P; `TestCaretakerUnifiedMount`; `TestCaretakerMountUnknownSession` (per-stream close on the unified endpoint)
- `pkg/hub/registry_test.go` — round-robin, `RemoveConn`, `TestRegistryDeliverTarget`
- `pkg/server/hub_test.go` — `TestHubRelaysToRegisteredService`, `TestHubUnknownServiceCloses`, `TestHubRoleEndToEnd`, `TestHubDeliveryEndToEnd`, `TestHubPolicyEnforced`, `TestHubRegisterPolicy`, `TestHubMTLSIdentityIsAuthoritative` (in-process CA + RequireAndVerifyClientCert), `TestHubCatalog`
- `pkg/server/hub_multireplica_test.go` — two in-process `*Server` + miniredis: cross-replica delivery, cross-replica dial-direct, liveness expiry (in CI)
...
- k8s injection: `TestSyntheticIP`, `pkg/deploy/kubernetes/hub_test.go` (record IP == Reach listen IP; export modes), `TestHubWithMountsSingleCaretaker`, `TestHubProxyRejected`
- caretaker: `TestCaretakerURL`, `TestGroupByServer` (formerly `TestGroupMounts`); CLI: `hubRoleFromFlags` parsing
- E2E: `deploy-hub.star`, `deploy-hub-udp.star` (real dind+kind runs, both passed), `hub-multireplica-redis.star`, `hub-multireplica-kube.star`
...
- dind harness traps (from the KubeStore validation): the e2e image ships no socat/python3, which is why the harness provides the in-process `tcp_echo()` builtin rather than a mounted echo binary; `kind create cluster` already writes ~/.kube/config (use it directly); dockerd startup remounts /tmp as tmpfs in the dind image, wiping dirs created before it — put work dirs under $HOME and create them after dockerd/kind come up.
...
## Credential Forwarding and Peer Authentication

A deploy-attach session owns its mounts, credentials, and egress, so
cross-replica credential routing reuses the existing
`mountServiceName(session)` record. No second credential-routing record exists.
The owner replica resolves the live session and re-checks
`AllowsCredential(name)`; the forwarding replica cannot authorize a fact it does
not hold.

JWT/JWKS/mTLS-only replicas authenticate peer forwards through keys distributed by
the hub store. Each auth-enabled Redis or KubeStore replica maintains a mode-checked
P-256 key, publishes its public half under replica liveness, and sends short-lived
`scope=peer` ES256 credentials. The peer scope reaches only the three forward
routes. Store misses, stale leases, malformed keys, and algorithm confusion fail
closed.

`CORNUS_AUTH_TOKEN` retains precedence for mixed-version rollout compatibility.
After all replicas understand peer credentials, installations can remove the
shared static token.

The former Redis/Kubernetes multi-replica shell scripts are now ordinary Starlark
scenarios. Multi-server support was added as opt-in harness builtins, so the rest
of the suite remains single-server and both live paths run through the normal CI
matrix.
