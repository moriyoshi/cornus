...
Reboot recovery includes companion caretakers. It reconstructs their role bundle, reconnects network-namespace references, and restores desired-running state without treating the companion as an app replica. Unit tests cover state and namespace repointing; rootfs remount remains a live-runtime concern.
