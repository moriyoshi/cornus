...
A compose project name and server connection are identity; conduit settings are
configuration. Reusing a project name against a different server is rejected
because the agent cannot migrate its existing deploy-attach sessions. Changing
the conduit on the same server is reconciled in place.

`Project.Rebind` marks live exposure registrations dirty and the ordinary Apply
path moves them. Mount sessions remain intact. The agent acquires the new
connection/conduit before releasing the old and defers the release until
registration withdrawal completes, preserving shared refcounts.

Conduit keys canonicalize the default mode before both acquisition and release.
Without that rule a frontend registered with an empty mode was released under a
different key and leaked indefinitely. Rebinds are serialized per project so
concurrent updates cannot leave stored configuration and live exposure on
different conduits.

## Adoption: the exception to sharing-by-identity

(2026-08-05) Sharing by structural `Identity` equality has two failure modes, both
fixed at the point where the key is taken.

**Equivalent spellings used to fork.** `Identity` hashes the RAW
`Socks5Listen` / `Socks5Suffix` / `Socks5BareServiceNames`, but the engine
substitutes its own defaults for the empty ones. `--conduit socks5` (listen `""`)
and `--conduit socks5://.shared:1080` therefore described ONE proxy under TWO keys,
and the second frontend to arrive forked a conduit that could only fail to bind.
`canonicalConduitCfg` now applies `socks5.DefaultListen` / `DefaultSuffix` /
bare-names-true for socks5, and drops the suffix entirely when `Socks5Resolve` is
set (explicit rules replace the suffix default, so it reaches nothing).

**`cornus web --publish-in-conduit` now ADOPTS rather than keys.** A browser has one
proxy setting, so the published UI has no legitimate opinion about
listen/suffix/ingress — and `clientconn.ConduitConfig` never filled in ingress the
way `compose up -d` does, which made a fork the everyday case rather than the
exception. `WebSpec.JoinConduit` (protocol v8) demotes `Request.Conduit` to a
fallback; the agent picks a SHARED (non-session-local) socks5 conduit of the same
`connState`, preferring an exact identity match and then the most-shared one, with
an explicit sort because Go randomizes map order and >1 candidate implies different
bind addresses. The client sets it unless `--conduit` NAMED an address or a suffix
(`ConduitSpec.HasListen`/`HasSuffix`) — a bare `socks5`, `CORNUS_CONDUIT`, and the
profile are ambient defaults, not pins.

Two consequences worth keeping:

- The published NAME had to move agent-side (`WebSpec.Name` may now be empty;
  `Response.WebName` carries the answer back). `webbff.Config.PublishedName` is the
  Host allow-list, and router locals are consulted BEFORE the rules — so a name
  derived from the REQUESTED suffix while publishing in a conduit with a different
  one resolves through the proxy and is then refused with **421**. Reachable-and-
  refused, citing a flag that is mutually exclusive with this mode.
- `webFrontend.egKey` must be the ADOPTED key. `releaseConduitLocked` returns
  silently for an unknown key, so the requested key leaks the conduit with no error
  anywhere — the same shape as the mode-canonicalization bug above.

**Bind conflicts now name their holder.** `ensureConduitLocked` pre-flights a socks5
acquire: a live conduit on the same effective address under a different key is
refused with the frontend holding it (walking `a.webs`/`a.projects`/`a.dockers` for
a matching `egKey` — no reverse index) and the differing settings (`conduitDiff`,
modelled on `connDiff`), instead of passing through `bind: address already in use`.
An ephemeral (`:0`) bind never collides. Holders outside the agent (a foreground
`cornus socks5`) are invisible to it, so the raw bind error stays the backstop.

A FOLLOWER model — re-homing a published UI whenever the connection's shared conduit
changes, so either command order works — was designed and deliberately rejected.
Eviction is only legal when every rider of the colliding conduit is also a follower,
so it fixes `web`-then-`compose` and nothing else (a docker frontend can never be a
follower: its conduit `Add`s are a closure over `es.eg` that nothing replays). It
also inverts the acquire-before-release discipline `rebindProjectLocked` relies on —
