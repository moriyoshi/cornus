...
## Workspace and Filesystem Surface Refresh

The browser's Files and Terminal surfaces are now one extensible tiled Workspace. Shared commands drive split, move, traversal, rotation, placement, and cross-pane file transfer through a keyboard- and touch-capable chooser. DOM focus routes input while tile focus records workspace intent; tests must provide geometry and coordinates because jsdom has no trustworthy layout.

Filesystem mutation is server-planned. Copy and move normalize paths, preflight collisions and descendant operations, and stream through the BFF or caretaker rather than buffering in the browser. Kubernetes image-layer operations use confined tar streams over `pods/exec`. Terminal OSC title, cwd, and PID signals feed tab naming and live foreground-process classification only after control strings are removed from the visible-screen activity model.

The web surface remains operational rather than declarative: Apply is absent. Overview separates Server and Ingress facts, scopes tables and metrics to their surrounding section, and treats shared mock data as fixture convenience rather than evidence that live realization paths agree.
