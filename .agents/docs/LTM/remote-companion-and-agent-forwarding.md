...
## Unified Host Companion

Dockerhost and containerdhost plan one per-replica companion carrying `Mounts`,
`Egress`, `PortForward`, and `AgentRelay`. `companionPlan` unions binds and
privileges before app creation: mounts require privileged kernel 9P, transparent
egress adds `NET_ADMIN` and `Mark=egressMark`, and the mark exempts every role's
own dials.

Telemetry remains a separate unprivileged companion but can open a server-bound
caretaker connection for the telemetry mux. Bare supports mounts/egress; Incus
uses a sibling PortForward/AgentRelay companion but cannot propagate mounts from
that sibling. Container-level crash/OOM now has a wider blast radius, while the
in-process supervisor still restarts individual roles.

## Incus Companion Extension

Incus remote mode now realizes a per-replica companion with PortForward and
AgentRelay roles. Incus cannot join a sibling's network namespace, so the
companion is a sibling on the same bridge and dials the application address;
`PortForwardRole.Host` carries that distinction. A shifted custom storage volume
shares the agent socket between instances without assuming access to the remote
daemon's filesystem.

Companions are excluded from user replica ordering and included in lifecycle and
volume cleanup. Client-local mounts and relay egress remain unsupported because a
mount made in a sibling cannot propagate into the application's mount namespace.

Incus direct and companion UDP forwarding are implemented and now declared through
`SupportsUDPPortForward`. The same remote-companion UDP path on dockerhost and
containerdhost still needs live coverage; those implementations historically used
the wrong framed-datagram bridge at the server side.
