...
How Cornus itself is delivered and stood up, as opposed to what Cornus does to user workloads.
Release artifacts include a GHCR multi-arch image, prebuilt static CLI binaries with a
`SHA256SUMS` manifest, and an OCI Helm chart, all version-stamped and keyless-signed. Kubernetes
installation is represented by Helm and a raw manifest that must agree on image and backend
configuration. The k3s-first Quick start bootstraps Cornus with Cornus, while `cornus setup`
builds connection profiles and installation artifacts for guided scenarios. The Docker-host
server-in-container topology is supported and preflighted; containerd container installation
retains explicit networking and CNI requirements. Changes to one surface often affect the rest.
...
| [setup-wizard.md](./setup-wizard.md) | Interactive profiles, discovery, verification, navigation, and generated install artifacts |
| [in-container-server-mode.md](./in-container-server-mode.md) | Docker server-in-container installation and remaining containerd runtime requirements |
...
- **Checksums + keyless cosign signing** (GitHub OIDC, no long-lived keys): a `SHA256SUMS`
  ...
  `releases/latest/download/` curl), signed via `cosign sign-blob --bundle` producing a single
  Sigstore `SHA256SUMS.bundle` asset (cosign v3's default new-bundle format carries signature +
  Fulcio cert + Rekor proof in one file), NOT the old separate `SHA256SUMS.sig` /
  `SHA256SUMS.pem` pair; the image is signed by its manifest-list DIGEST (never by tag);
...
### Guided and in-container installation

- `cornus setup` maps one flat answer set into one atomic `clientconfig.Context`. Discovery and
  verification seams keep scenario-specific capability checks testable and repeatable.
- The Docker-host container scenario emits `cornus-compose.yaml` with the daemon socket, an
  `rshared` data-directory bind, explicit `CORNUS_DATA`, and a server port matching the saved
  profile. It presents preflight guidance before startup.
- `pkg/hostenv` and `pkg/hostcheck` distinguish merely running in a container from proven path
  divergence and fail configurations that would silently bind empty host paths.
- Dockerhost in-container mode is the supported guided path. Containerd path translation and
  log-shim staging are implemented, but a complete installation still requires host networking,
  host-visible runtime paths, root, and CNI plugins inside the image.
...
- **Changing setup scenarios**: keep question order, profile values, generated artifacts, and
  verification in sync. Re-run plain and rich UI flow tests because scripted response queues can
  remain green while answering the wrong question after an insertion.
...
- `cmd/cornus/internal/setupwiz/`, `cmd/cornus/setup.go` - guided setup and artifacts.
- `pkg/hostenv/`, `pkg/hostcheck/` - in-container path proof and startup preflight.
...
Setup flow/model tests cover navigation, defaults, discovery, verification, and artifact values.
`server-in-container.star` exercises the generated Docker topology and fail-loud host-path
mapping against a real daemon.
...
- A daemon socket bind alone does not make server-in-container mode safe. The data directory must
  be host-visible, and mount-serving requires the documented propagation.
- Containerd needs host networking and in-image CNI plugins in addition to translated paths.

## Dependency Compliance Refresh

This synthesis also incorporates [dependency-license-compliance.md](./dependency-license-compliance.md). Shipped notices and policy scanning use the union of all released `GOOS/GOARCH` dependency graphs, and vendored runtime source such as `third_party/herdr` must update attribution in the same change. Known overrides retain module-specific provenance.

The unresolved axis is cgo reachability: release binaries use `CGO_ENABLED=1`, so platform-union completeness does not by itself prove that cgo-selected modules are covered. Rust crates statically linked through `libimbhgo.a` likewise require an upstream notices manifest. Treat both as explicit release-compliance gaps rather than silently equating Go-module enumeration with the complete shipped artifact.
