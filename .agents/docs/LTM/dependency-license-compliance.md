...
## Released-Platform Union

Notices and policy scanning use the union of modules reachable from all five released `GOOS/GOARCH` targets. `scan_licenses.py --platforms` unions `go list -deps` results and warns when platform resolution disagrees. This captures Windows-, Darwin-, and amd64-specific shipped dependencies.

Known overrides carry per-module provenance. Vendoring a runtime dependency, including `third_party/herdr`, requires regenerating notices. The remaining reachability gap is cgo because release binaries use `CGO_ENABLED=1`.
