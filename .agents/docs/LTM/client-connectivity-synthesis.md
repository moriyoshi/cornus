...
## Kubernetes Service Discovery Refresh

Automatic cluster forwarding now resolves ready backends through `discovery.k8s.io/v1` EndpointSlices instead of deprecated core/v1 Endpoints. Port name and number selection remain part of the same `pkg/svcforward` contract. Fakes and setup tests must construct EndpointSlices; an Endpoints fixture no longer exercises production discovery.
