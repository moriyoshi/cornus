...
## Machine-Checked Field and Capability Contracts

Every backend now maintains two sides of the DeploySpec contract: supported specs
that must map and remain silent, and one row per unsupported top-level field that
must warn exactly once. A reflection guard fails when an exported field is added
without a backend decision. Counting warnings per single-field spec catches both
identical and differently worded duplicates.

The guard is intentionally top-level. A recursive leaf gate produced mostly
false positives because wholesale-ignored parents need no sub-field decision and
many mapped parents are realized in shared server, caretaker, ingress, telemetry,
or hostrun code. Nested worklists are audited by following the actual realization
path.

Client-session-dependent blocks are invalid on the stateless deploy endpoint.
The server now rejects `Credentials` there rather than relying on backend-specific
warnings. Relay egress remains stateless only when the selected policy needs no
client relay.

Optional capability interfaces need agreement between behavior and declaration.
The Incus UDP path existed while the declaration was absent; containerd mounts
were declared but routed to a message claiming incapacity. Guards must inspect
capability values and caller routing, not only method names.

Canonical predicates define when a backend has anything to refuse. Empty ingress,
client-emulated ingress, and `healthcheck: NONE` must remain silent, with positive
controls proving the warning paths still exist.

## Stream Framing and TTY

Non-TTY logs and attach output use Docker stdcopy framing; TTY streams are raw PTY bytes. The proxy content type and bridge must follow that distinction, and all backends must agree. Docker returns raw TTY output, while Kubernetes must propagate `spec.TTY` into attach instead of always applying `muxWriters`. Cover both TTY and non-TTY arms in backend and proxy tests.
