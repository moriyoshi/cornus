...
- Read-write mounts work end to end: a writable confined 9P attacher (`pkg/wire/writablefs.go`) keeps writes jailed to the export root; a pod write propagates back to the caller's local dir.
...
The transport reuses buildwire rather than extracting an `internal/wire` package up front: `Serve9PBacking`/`Backing9PSocket` were already exported, and extraction would have meant moving the security-critical `confinedfs.go` and splitting the BuildKit-coupled half of `ninep_backing.go`. Thin exported wrappers were added instead (`pkg/wire/export.go`: `Dial`/`Accept`/`OpenTagged`/`AcceptTagged`/`TagControl`, later `OpenBacking`/`Pipe`/`AcceptConn`/`DialConn`); `internal/deploywire` imports buildwire. (An `internal/wire` package did later materialize for `MeteredConn`.)

### dockerhost path

- `MountManager.Prepare` kernel-9p-mounts each local mount onto `<DataDir>/mounts/<session>/<name>` (config: `MountsDir` in `pkg/config/config.go`) and rewrites `Spec.Mounts[i].Source`; the caller's spec is never mutated.
...
| sidecar `startupProbe` running `cornus caretaker-check` (self-contained via `/proc/self/mountinfo`, no util-linux; `cornus mountcheck` before the caretaker unification) | gates the app container until the mount is live — restores the "files present before the entrypoint" guarantee, the analogue of dockerhost's synchronous `mount(2)`-before-start |
...
- Gating: a `deploy.RemoteCapable` marker interface (`Remote() bool`), read once at backend
  construction (`remoteModeEnabled` in `pkg/server/server.go`, which reads `remoteModeEnvs` — the
  single source for the four `CORNUS_*_REMOTE` names, so the variable the factory honours and the
  variable an error message tells the operator to set cannot drift apart). Without this, the
  unconditional type-assertion `backend.(deploy.MountingBackend)` in the mounts-dispatch branch would
  always succeed the moment dockerhost implemented it, silently stealing every client-local-mount
  deploy away from the existing, working `applyWithHostMounts` fast path — daemon co-location cannot
  be auto-detected. A backend that doesn't implement `RemoteCapable` at all (kubernetes, which has no
  host-mount fallback to protect) is treated as always eligible.
- **A false answer from that gate does NOT mean the same thing on every backend, and reading it as
  "fall back to the co-located path" is wrong for containerd** (corrected 2026-07-31; four code
  comments, a test docstring, and both reference pages in all three locales had said otherwise):
  - dockerhost and bare DO have the co-located path — `hostcheck.UsesHostMountFastPath` is the single
    source for that classification, and `pkg/server`'s mounts-dispatch asks it rather than comparing
    `Backend.Name()` against literals. So for them, false genuinely selects it.
  - containerd has no such path at all. For it, false means client-local mounts are UNAVAILABLE, which
    is why `clientLocalMountsUnavailable` names `CORNUS_CONTAINERD_REMOTE` instead of reporting the
    backend as incapable. The old message ("client-local mounts are not supported by the containerd
    backend") was false: the capability is one environment variable away, and the message withheld the
    variable. Only the `remote` FIELD comment in `containerdhost/backend_linux.go` had it right —
    "instead of having client-local mounts be unsupported".
...
`cmd/cornus/commands.go`: `cornus deploy --server <url> --local-mount SRC:DST[:ro]` runs foreground with streaming and a Ctrl-C graceful `down`. The sidecar sets `Args: ["caretaker"]` (`pkg/deploy/kubernetes/kubernetes.go`); `CORNUS_AGENT_IMAGE` defaults to the app image because the Cornus image's ENTRYPOINT is `cornus`. It read `Args: ["mount-agent", ...]` here until the caretaker unification folded the standalone mount-agent into the caretaker and the alias was removed.
...
- `pkg/server/deploy_attach.go` — attach handler + backend dispatch (sidecar vs host vs error)
- `pkg/server/mount_relay.go` — session registry + mount relay (originally `/.cornus/v1/deploy/mount/`, later unified onto `/.cornus/v1/caretaker/attach`)
- `pkg/server/server.go` — `/.cornus/v1/deploy/attach` route
- `pkg/wire/export.go` — buildwire wrappers (`Dial`, `Accept`, `OpenTagged`, `AcceptTagged`, `TagControl`, `OpenBacking`, `Pipe`, `AcceptConn`, `DialConn`)
- `pkg/wire/writablefs.go` — writable confined 9P attacher
- `pkg/wire/metered.go` — `MeteredConn`, `Backing9PSocketMetered`
- `pkg/deploy/deploy.go` — `MountingBackend`, `AttachMount`
- `internal/deploy/kubernetes/` — `ApplyWithMounts`, `deploymentWithMounts`, shared `applyDeployment`, bind-mount rejection in `Apply`
- `pkg/client/client.go` — `DeployAttach`, host-path classification, `wsAttachURL(base, path)`
- `pkg/config/config.go` — `MountsDir`
- `cmd/cornus/commands.go` — `--server` / `--local-mount` flags
- `mount-agent` / `mountcheck` were top-level subcommands in `cmd/cornus/mountagent.go`. Both were
  REMOVED (no published release carried them); the functionality lives under `cornus daemon` and
  `cmd/cornus/cli_surface_test.go` asserts the two names stay absent from the command tree.
...
- `TestMountRelayLocalFastPath` / `TestMountRelayCrossReplica` (`pkg/server/mount_multireplica_test.go`; the
  former `TestMountRelayServesCallerExport`) — in-process: a fake MountingBackend registers the session,
  the test plays the pod side, and a real p9 client over the relay reads the caller's marker file back.
  `TestMountForwardUnknownSession` (formerly `TestMountRelayUnknownSession`) covers the no-such-session arm (registry + relay + confined export, no root); `TestMountRelayUnknownSession`
...
## Writable Export Compatibility

`pkg/wire/writablefs.go` decorates `localfs`; embedding `templatefs.NoopFile`
silently supplies unsupported defaults unless each needed operation is delegated.
`Lock` inherited ENOSYS and broke Turbopack lockfiles, so it now delegates to the
inner local file. `O_APPEND` reached `os.OpenFile` and then failed when the 9P
server used `WriteAt`; the export strips it because the `cache=none` kernel client
already positions append writes at EOF.

The privileged kernel-9P regression is gated by `CORNUS_REPRO_9P=1`. Turbopack's
remaining persistent-cache failure is different: `mmap` is unavailable with
`cache=none`, and relaxing that mode is a coherence decision.

Stable mount-session ids are reused during re-apply when no live registry entry
owns the baked id. This preserves mounts across repeated `compose up`; it is not
automatic reconnect without re-apply.

## Containerd Routing Correction

Local containerd implements `MountingBackend` but has no kernel-9P fast path.
Without `CORNUS_CONTAINERD_REMOTE`, client-local mounts are rejected with a
message that names the remote-mode variable; the former documentation incorrectly
claimed a local fast path existed. In remote mode the companion path is the
realized route.

`hostcheck.UsesHostMountFastPath` is the single classification shared by startup
preflight and deploy routing. The remote-mode variable map likewise feeds both
the backend factory and operator guidance. Agreement tests do not replace
behavioral reachability checks: two copies can agree and both still describe the
wrong product behavior.

Whether local containerd should implicitly start the privileged companion without
the remote flag remains a maintainer decision requiring a live containerd E2E
run.
