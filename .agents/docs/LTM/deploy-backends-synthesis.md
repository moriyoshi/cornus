...
TTY is a cross-backend stream contract. Non-TTY logs and attach output are stdcopy-framed; TTY output is raw PTY data. Docker's daemon already makes this distinction, and Kubernetes attach must propagate `spec.TTY` instead of unconditionally multiplexing. Proxy content types and tests need both arms.

Barehost reboot recovery includes companion caretakers: restore their role bundle and network-namespace references without counting them as app replicas. Kubernetes file inspection and transfer use tar over `pods/exec`, not node-visible rootfs paths. These are backend-specific realizations beneath shared observable contracts, not reasons to weaken the contract.
