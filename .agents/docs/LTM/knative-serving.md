...
- The typed fake clientset does not know CRDs; the ksvc backend needs the dynamic fake + fake discovery (copy `pkg/deploy/kubernetes/internal/netdriver/netdriver_test.go`). Register the NAD/CNP list kinds too or `Delete`'s netdriver GC panics.
...
## Speculative Probe RBAC Rule

`deleteKnative`, `knativeStatus`, and `knativeExists` treat Kubernetes Forbidden
like NotFound. These are speculative probes used even for plain Deployments on a
cluster where Knative APIs exist. A ServiceAccount that lacks
`serving.knative.dev` rights could not have created a ksvc, so requiring those
rights merely to delete or inspect a non-Knative workload would violate least
privilege. A dynamic-fake Forbidden reactor pins this behavior.

## Vendored Test Installation and Scale Semantics

The E2E runner vendors Knative Serving, default-domain, and Kourier manifests at
`knative-v1.15.0` with `SHA256SUMS`. The installer verifies the files before
apply and falls back to upstream URLs only when explicitly asked to try another
version. Container images remain digest-pinned external pulls; vendoring all image
archives would disproportionately enlarge the shared runner.

A Knative service ignores fixed replica counts because autoscaling, including
scale-to-zero, owns the number. `Replicas > 1` therefore produces a warning while
zero/unset and one remain quiet. Additional published ports also warn; the API
comment formerly promised that warning before the implementation existed.
