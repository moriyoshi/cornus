...
- `pkg/wire/ninep_backing.go`, `pkg/build/buildwire/ninep_backing.go`, and the server-side
  `ServeBlock*` call sites (`pkg/server/caretaker_attach.go`, `pkg/server/mount_relay.go`) - production
  feature wiring. (This read `pkg/deploy/.../ninep_backing.go` until 2026-07-31; there is no such file
  under `pkg/deploy`, and the ellipsis hid that rather than standing in for a real path.)
...
## Wire and Disk Reference

The shared transport is one WebSocket containing yamux streams selected by a
one-byte tag and assigned QoS classes. A block frame starts with `u32 length`,
`u8 op`, `u8 flags`, `u16 reserved`, and `u64 request id`; HELLO negotiates
features before READ, READRANGE, WRITE, and FSYNC traffic.

`DiskStore` shards entries by key prefix and stores a sparse `.data` file beside
a JSON `.idx` sidecar. Unread blocks consume no data-file space. Entries survive
restarts and are governed by the file-cache cap and GC. `ARCHITECTURE.md` carries
the byte layouts and sequence diagrams; user docs use the container / cache /
caller view.

## DiskStore Scratch Allocation

Partial writes and `HashRange` reuse a per-store `sync.Pool` of chunk-sized
scratch buffers. DiskStore keeps no in-memory chunk, so pooled buffers are
call-local, completely overwritten by `readData`, never returned to callers, and
do not share MemStore's aliasing concern.

The SQLite benchmark showed allocations falling from roughly 21-23 MB/op to
3.4-4.8 MB/op while throughput remained fsync-bound. The improvement is reduced
GC pressure, not a latency claim.

The regression test uses a byte ceiling rather than allocation count and includes
a correctness control proving the chunk remains present with the expected hash.
Without that control, a bad caller hash made 199 of 200 writes take a no-op early
return and the test passed with the pool removed. Measurement tests must prove the
measured work happened.
