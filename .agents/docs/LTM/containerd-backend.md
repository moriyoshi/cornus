...
## Later Correctness Findings

The containerd E2E suite exposed a leaked CNI network from an earlier scenario:
portmap DNAT existed but targeted an unreachable address, producing
`EHOSTUNREACH` rather than the `ECONNREFUSED` of an unpublished port. Persistent
host backends make scenario ordering and cleanup part of the test surface.

Container UTS hostname and managed `/etc/hosts` must agree. Both containerd and
bare now use `hostrun.InstanceHostname(spec, id)`; testing the rule alone is not
enough, so the hosts-file caller is checked too.

The per-field audit added warnings for unsupported stop, init, stdin, DNS,
restart-attempt, reservation, network-address, SELinux, and volume metadata
semantics while preserving the fields containerd genuinely realizes. Client-local
mounts require `CORNUS_CONTAINERD_REMOTE`; no local kernel-9P fast path exists.

Instance log files are removed on both application and companion deletion. The
binary log shim command name is one exported production constant shared with both
CLI registrations, eliminating a test-owned duplicate literal.
