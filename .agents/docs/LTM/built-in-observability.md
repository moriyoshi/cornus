...
## Queryable Workload Metrics

The web dashboard queries the built-in store and places scoped charts beside the workloads and projects they describe. Kubernetes samples require unique timestamps per series: duplicate timestamps poisoned PromQL reads even though ingestion and SQL inspection showed rows. Tests now exercise the actual PromQL path instead of treating recorded rows as proof of queryability.

Histogram rows remain a separate limitation where the store cannot resolve canonical bucket, sum, and count selectors; `cornus observe query` SQL is the diagnostic path until that contract is settled.
