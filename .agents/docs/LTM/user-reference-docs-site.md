...
- `.agents/docs/JA_TRANSLATION_GLOSSARY.md`, `.agents/docs/ZH_TRANSLATION_GLOSSARY.md`,
  `.agents/docs/QUALITY_GATE.md` - internal terminology and documentation/localization gate.
...
Topics were folded into task-oriented Guides, and navigation labels were
reconciled with page H1s. The `assess-doc-quality` skill owns the anchor checker;
it prunes generated/dependency trees, resolves relative and suffixed routes, and
counts every fragment candidate. A VitePress build alone does not validate
localized fragments.

Japanese authored text is NFKC except for decomposed kana in URL fragments that
must match VitePress's emitted ids. Schema type literals remain verbatim,
`デプロイスペック` is preferred, and `リファレンス` is used for documentation
senses. Never normalize a whole locale file blindly.

State tool requirements without naming one contributor's personal tool manager
or path. Agent skills are documents and belong in repository-wide scans.

## Web Screenshot Workflow

The `web-screenshot` skill captures the local SPA or another page with Playwright,
places VitePress assets under `docs/public/screenshots/`, and embeds them as
`/screenshots/<name>.png` from every locale. Its helper supports retina scale,
full-page or selector crops, wait conditions, and color scheme.

An npx-installed ESM package is not automatically resolvable from a script's own
directory. The helper discovers npx `node_modules/.bin` paths, resolves
`playwright` with `createRequire`, dynamically imports the resolved entry, and
handles CommonJS interop through `mod.chromium ?? mod.default?.chromium`. This
avoids a committable skill-local `node_modules` tree.

## Translation Freshness and Tooling

English remains the source for Japanese and Simplified Chinese. Structural audits
catch missing headings, blocks, and frontmatter, while per-page state in
`docs/.translation-state.json` records the SHA-256 of the English page at the last
review. A stale digest means the source changed, not automatically that the
translation is wrong; review before updating it.

`translation_state.py update --path` accepts source-relative paths and fails
atomically when a requested path matches nothing. The former behavior printed
`recorded 0` and exited successfully, leaving the operator believing stale pages
had been reviewed.

The translation audit compares links and inline code order-insensitively but
multiplicity-sensitively. Localized fragments are ignored when comparing link
identity, code/link content inside fenced examples is excluded, and multiline
inline-code whitespace is folded. These advisory comparisons are unit-tested at
the function level because the audit's successful exit status does not prove its
warning logic.

`make test-scripts` discovers documentation and translation tests per script
directory. A single root discovery is inappropriate because the scripts are
standalone modules that manipulate their own import paths. The target itself was
fault-injected to confirm a failing directory produces a non-zero make result.

README is now a concise landing page rather than a second reference manual. It
names maturity and security posture, offers a source-verified loopback Docker
evaluation, and routes users to the canonical VitePress guides and architecture.
