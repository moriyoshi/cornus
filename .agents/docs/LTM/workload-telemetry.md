...
## Tagged Runner and Mux Verification

The containerized E2E runner switches from a static Go build to a glibc cgo build
when `BUILD_TAGS` contains `imbh`; `sable_extern_lib` is required and the build
fails loudly if the external archive cannot be resolved. This runner and its
caretaker sidecar share the Debian runtime, so dynamic glibc linkage is
intentional there.

`observability-telemetry-mux.star` now executes with a real Collector and
store-backed server on the Kubernetes leg. An explicitly present empty telemetry
block is distinct from an absent block: the former requests the server mux with
no external endpoint. The Starlark builtin must retain that presence information
instead of unpacking directly into a Go string.

The metrics path needs metrics-server and a first successful scrape. A dedicated
leg treats an installed-but-unavailable APIService or an absent observability
store as failure, not as a green skip.
