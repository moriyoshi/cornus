# Build Engine Synthesis (delegation, transport, caches, lazy contexts)
...
its confined caller-side export, delegation from an unprivileged server to a privileged builder,
build-cache import/export across three backends, the lazy bind-mount mechanism that serves large
contexts on demand, and the choice of BuildKit worker (runc default, containerd via
`CORNUS_BUILD_WORKER=containerd`). The source documents share the same seams (`SolveInput`,
`buildwire.BuildSpec`, the confined 9P attacher, `.dockerignore` handling), so an agent touching
any one of them needs the invariants from all of them.
...
| [builder-delegation.md](./builder-delegation.md) | Raw build relay, capability probing, and lazy supervised privileged builder |
...
- BuildKit viability is a capability question, not an euid question. Cornus probes a real bind
  mount before deciding that the local process can host the engine.
- `CORNUS_BUILDER_URL` selects an external builder. Authorization is checked before relay, POST
  bodies remain streaming, and attach connections are raw-spliced so the existing buildwire
  protocol is not reimplemented.
- Without an explicit builder, a server with a usable local container runtime can lazily build a
  helper image from its own running executable and start a supervised privileged builder. Using
  the current executable preserves version and feature-tag agreement.
- The helper uses host networking so build exports can reach the delegating server. Its registry
  mode must be the delegating server's already-resolved mode; resolving an independent default
  can make export behavior disagree with registry write support.
...
- Keep delegation as transport and lifecycle policy around the existing build endpoints. Do not
  introduce a second build protocol or buffer request bodies in the relay.
...
- `pkg/server/build_relay.go`, `pkg/builderctr/` — external relay, capability probe, helper image,
  and supervised privileged builder lifecycle
...
- Delegation: URL normalization, authorization-before-connect, streaming POST, raw attach relay,
  mount-capability probing, and registry-mode propagation.
...
- Never use uid 0 as proof that BuildKit mount operations are available, and never pull a
  nominally matching helper image when the running binary can build the exact helper.
