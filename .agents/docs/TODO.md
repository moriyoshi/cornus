...
## Conduit join: silent fallback and the non-loopback shared conduit (opened 2026-08-09)

> **SUPERSEDED the same day** by the conduit rendezvous redesign (JOURNAL, "the conduit rendezvous design, as built"), which retires the
> shared-vs-session-local model outright rather than patching these two symptoms. Both items
> below are dissolved by that design, not fixed by it: there is no "nothing to join" fallback
> once the address IS the identity, and `.shared` is retired rather than extended. They are kept
> here only until the design's stage 3-4 lands, because until then the CURRENT code still has
> both defects. Do not fix them in place — check the design first.

- [ ] **`cornus web --publish-in-conduit` starts a second proxy in SILENCE when there is nothing
      to join.** `resolveWebConduitLocked` (`cmd/cornus/internal/clientagent/web.go:254`) emits
      warnings only for the "joined, but not the conduit your settings describe" cases; when
      `pickSharedConduit` returns `ok == false` it falls straight through to
      `ensureConduitLocked` and the user sees a normal-looking `SOCKS5 proxy listening on
      127.0.0.1:1080` banner with no hint the join was attempted and failed. This is what made a
      real report take a code read to explain. Add a warning naming what was looked for (shared
      socks5 conduits on this connection) and what happened instead. Neutralize by making
      `pickSharedConduit` return a candidate and confirming the warning disappears.

- [ ] **A shared non-loopback conduit is expressible in the config schema but not in the
      `--conduit` grammar.** `ParseConduitSpec`
      (`cmd/cornus/internal/clientconn/clientconn.go:423`) treats only the host `.shared` as
      shared and hard-codes `spec.Listen = "127.0.0.1:" + port` at `:426`; every other authority
      sets `SessionLocal = true`, which `pickSharedConduit` filters out. So
      `--conduit socks5://0.0.0.0:PORT --allow-non-loopback` is necessarily un-joinable, and
      `--conduit socks5://.shared:PORT --allow-non-loopback` silently stays on loopback.
      `set-context --conduit-mode` cannot express it either (`cmd/cornus/config.go:494-496`
      rejects a session-local URL). But a hand-written context (`conduit.socks5.listen:
      0.0.0.0:PORT`) DOES yield a shared non-loopback conduit, because `configFromContext`
      (`clientconn.go:130-137`) never sets `SessionLocal` — so this is a grammar gap to close,
      not a capability to add. Let `.shared` carry a full `host:port`;
      `clientconduit.Config.Validate` (`pkg/clientconduit/clientconduit.go:242`) is already the
      single enforcement point and is reached only from `clientconduit.Start` (`:212`), so
      adoption stays correctly unvalidated. Check `ARCHITECTURE.md:1453-1465` and
      `docs/**/cli/web.md` in the same change — both currently describe `.shared[:port]` as
      port-only. Verified by reading only; the hand-written-profile workaround has not been run.

- [ ] **`docs/guides/networking.md:44-48` walks the reader into the trap.** The guide DOES state
      that "a `socks5://` URL with an authority spins up a private, session-local proxy" (line
      31), but fifteen lines later the `--allow-non-loopback` example is
      `cornus compose up --conduit 'socks5://0.0.0.0:10080' --allow-non-loopback` with no
      restatement — and nothing anywhere connects session-local to "therefore
      `cornus web --publish-in-conduit` cannot join it". The two facts are stated pages apart and
      never composed, which is how a reader arrives at exactly this command. Add the consequence
      at the example, and cross-link it from the `--publish-in-conduit` docs; mirror into
      `ja`/`zh`.

## Conduit rendezvous redesign (opened 2026-08-09)

Design: JOURNAL, "the conduit rendezvous design, as built". Journal: "the conduit becomes an
addressable rendezvous (design + first slice)". Stage 1's address/coverage core is DONE and
gated (`pkg/conduithost/addr.go`, `dualstack.go`, `addr_test.go`); everything below is open.

- [x] **DONE 2026-08-09: Stage 1 rest, the rendezvous directory and control protocol.**
      `pkg/conduithost` does create-or-join under a per-port flock, with dial-based stale
      reaping and connection-scoped registrations. See the JOURNAL entry "conduit rendezvous:
      the directory and the control protocol" for the three portability findings (Go binds
      `0.0.0.0` as dual-stack `[::]`; Windows has no usable `ECONNREFUSED`; Windows needs a
      real lock, not `agentproc`'s no-op) and the test that was passing for the wrong reason.

- [x] **DONE 2026-08-09: Stage 1a, listener replication and takeover.** `pkg/listenerpass`
      implements the per-OS seam; `conduithost` fetches a replica over a dedicated `OpAdopt`
      connection at join time and `Joiner.Takeover` runs the flock election with registration
      replay. The zero-refusal measurement across a deliberate ownerless interval is
      `TestAddressNeverRefusesDuringTakeover`. Remaining gaps recorded below. Original note
      follows. Replicate AT JOIN TIME, never at exit — a SIGKILLed host
      hands nothing over. Windows specifics and the IOCP disassociation trap are recorded in the
      design. **The load-bearing test is a connect loop across a SIGKILL asserting zero
      ECONNREFUSED**, not "a survivor became host" — the latter passes even if the address went
      unbound for a second, which is the whole failure being designed against. Election must be
      driven with several concurrent survivors or the flock is untested.

- [x] **DONE 2026-08-09: Stage 2, per-claim dialers in `pkg/socks5`.** The dialer rides on
      the alias claim, `Result.Dialer` carries it out of `Resolve`, and the proxy falls back
      to its own when nil. `AliasSpec`/`Router.Claim` is the full registration form. See the
      JOURNAL entry "per-claim dialers: one conduit, several servers". Original note follows.
- [ ] ~~**Stage 2: `Result.Dialer` in `pkg/socks5`.**~~ `Proxy` holds one `portfwd.Dialer`
      (`socks5.go:523`), so consolidating two different servers onto one address needs the dialer
      per registration. Neutralize with two registrations pointing at two fakes, asserting each
      is dialed.

- [x] **DONE 2026-08-09: stage 3 part two, conduits bind through the rendezvous.**
      `clientconduit.WithRendezvous` + `rendezvousConduit`; the agent passes
      `clientagent.ConduitRegistry()` (`<agent dir>/conduit`). Two processes on one address
      now consolidate. See the JOURNAL entry "stage 3, part two". What remains of stage 3 is
      retiring `pickSharedConduit` and the `Socks5SessionLocal` branch. Original note follows.
- [ ] ~~**Stage 3 (part two): agent hosts through `conduithost`.**~~ Part one is DONE —
      `socks5.Serve` (caller-owned listener) and `clientconduit.Registrar` (the
      conduithost <-> router bridge, with recovery). What remains is the swap in
      `clientagent`: bind through `conduithost.Open` instead of `ensureConduitLocked`, serve
      with `socks5.Serve` on the returned listener, and retire `pickSharedConduit` and the
      `Socks5SessionLocal` branch. See the JOURNAL entry "stage 3, part one". Original note
      follows.
- [ ] ~~**Stage 3: agent hosts through `conduithost`.**~~ `up -d` and `daemon docker` stop using
      `ensureConduitLocked`'s map; `pickSharedConduit` and the whole `Socks5SessionLocal` branch
      retire.

- [x] **DONE 2026-08-09: stage 4, foreground sessions join.** `composecli.runForeground`,
      `cornus deploy`'s foreground conduit and `cornus socks5` all pass
      `WithRendezvous(clientagent.ConduitRegistry())`. The original report's scenario now
      JOINS instead of forking, and fails loudly on the publish step pending stage 5.
      Original note follows.
- [ ] ~~**Stage 4: foreground joins.**~~ `runForeground` (`composecli/commands.go:411`) and
      `cornus socks5` (`cmd/cornus/socks5.go:65`). This is the stage that closes the original
      report.

- [ ] **Stage 5: `--publish-in-conduit` hosts its own BFF** and publishes it with
      `register-local` over a unix socket. Deletes the agent-side web hosting, `agentSelfView`,
      the `socketAgentView` round-trip, and the two hand-written copies of `ToBFFIngress`. Note
      the UI's lifetime ALREADY equals the `cornus web` process's lifetime (`handleWebServe`
      parks and reaps on EOF), so nothing is lost by moving it out of the agent.

- [ ] **Stage 6: grammar and docs.** Retire `.shared` with a deprecation warning; rewrite
      `ARCHITECTURE.md:1451-1477`, `docs/guides/networking.md`, `docs/**/cli/web.md` and the
      `ja`/`zh` mirrors; an E2E scenario per hosting mode. The networking guide's
      `--allow-non-loopback` example is the exact command from the original report.

## Pre-release security review follow-ups (2026-08-09)

From `.agents/docs/SECURITY_REVIEW-2026-08-09.md` — a design review ahead of the first public
release. The first pass was documentation-only; **the three blockers and the builder item were
then VERIFIED against the tree the same day, by reading and by running throwaway tests with
controls** (evidence in the review's "Verification against the code" section, summarized in
JOURNAL). Items still marked *(verify)* remain doc-derived inferences the code may refute.

Release blockers — all CONFIRMED. Suggested order: builder credential, then privilege policy,
then the bind gate, then the trust-domain statement.

- [ ] **CONFIRMED — Gate a non-loopback bind with no verifier configured.** Verified: there is
      no gate, and the startup logging is *inverted* — `logListenScope` (`cmd/cornus/serve.go:139`)
      returns early unless the address is loopback-only, so `:5000`, `0.0.0.0:5000` and `[::]:5000`
      are all SILENT while `127.0.0.1:5000` gets an INFO line; nothing anywhere logs that auth is
      off. Also found: `deploy/k8s/cornus.yaml:154` is `type: NodePort` on `30500` with no
      `CORNUS_AUTH_*` env, so the shipped quick-start manifest publishes the unauthenticated
      build/deploy/exec API on every node — fix that alongside the examples. `cornus serve` defaults to
      `:5000` on every interface with auth off, in front of build/deploy/exec/push. Proposal:
      loopback stays permissive; a non-loopback bind with no verifier refuses to start unless
      `--allow-anonymous` / `CORNUS_ALLOW_ANONYMOUS=1` states the intent — the same gate
      `cornus socks5 --allow-non-loopback` and `cornus web --allow-non-loopback` already have.
      Also fix `docs/introduction/installation.md`, whose `docker run --privileged -p 5000:5000
      -v /var/run/docker.sock` example publishes remote root on the host with no warning
      attached (the `cli/serve.md` warning box should travel with it), and audit the Helm chart
      / `deploy/k8s/cornus.yaml` for the same combination.
- [ ] **CONFIRMED (highest priority) — Extend the workload privilege policy past
      `spec.Privileged`.** Verified by execution on both paths. Zero-value `hostpolicy.Policy`
      ALLOWED all of capAdd `SYS_ADMIN`/securityOpt `unconfined`/devices `/dev/sda`/`pidMode:
      host`/`ipcMode: host`, while the controls (`Privileged`, bind `/`) were denied. With a fake
      clientset and `allowPrivileged=false`, `capabilities.add=[SYS_ADMIN SYS_PTRACE]`,
      `hostPID=true` and `hostIPC=true` all landed on the pod while `Privileged: true` was
      rejected in the same run — `hostPID` + `SYS_PTRACE` is a textbook escape past the gate.
      Sites: dockerhost `engine.go:1438,1496,1505`; hostrun `spec_linux.go:86,102,115`;
      kubernetes `kubernetes.go:2544,2936,2943`. Ahead of the bind gate in priority: that one an
      operator can correct today with `--addr`, this is a control that does not do what its name,
      comments, docs and LTM entry all say. `deploy-spec.md` accepts
      `capAdd`, `securityOpt` (dockerhost "passes them verbatim"), `devices`, `pidMode: host`
      and `ipcMode: host` with no gate. `devices` bypasses the bind-source prefix check
      entirely. Fold them under `CORNUS_ALLOW_PRIVILEGED` (or a finer
      `CORNUS_ALLOW_CAPABILITIES` / `CORNUS_ALLOW_DEVICES`) in both `dockerhost.Policy.validate`
      and the kubernetes `checkPrivilege` parity gate. Note this contradicts
      `LTM/auth-and-security.md`'s "Privileged was the only remaining user-controlled escalation
      knob" — correct that line too. *(verify)*
- [ ] **CONFIRMED — State the trust domain, or scope authorization to resources.** Verified:
      `apiPolicy.Allow(identity, action)` takes no resource argument, so "which deployment" is
      inexpressible. With `{"ci-bot":["deploy"],"nobody":[]}`, the `nobody` identity — granted
      NOTHING — got 200 on archive GET/HEAD, logs and the deployment list, and cleared the gate on
      `fsop op=get`/`op=list` (501 only because the test backend has no FSOperator), while the
      control `POST .../stop` returned 403 in the same run. Gate: `pkg/server/deploy.go:177-200`. `CORNUS_API_POLICY` gates
      verbs, never ownership: any `deploy` identity reaches every workload, and archive GET is a
      "pure read", so an identity with an empty action list can stream files out of any
      container. Minimum for this release: one paragraph in `docs/architecture/security.md`
      saying authenticated clients share one trust domain and read endpoints are governed by
      authentication alone. Consider gating archive GET behind an action as a cheap partial fix.

- [ ] **CONFIRMED, PROMOTED TO BLOCKER — The auto-started builder needs a credential.**
      `builderctr.go:392-445`: the container's `Env` is exactly `["CORNUS_BUILDER_AUTO=false"]`
      (plus `CORNUS_DEPLOY_BACKEND=dockerhost` in re-export mode) — no credential of any kind —
      with `Privileged: true`, `NetworkMode: host`, `RestartPolicy: unless-stopped` and the host
      Docker socket bound. Host networking puts that unauthenticated API on the HOST's
      `127.0.0.1:5099`, reachable by every local process, and it outlives the server.
      **In the default re-export mode it composes with the privilege-policy gap above into a
      clean local privesc**: that mode sets `CORNUS_DEPLOY_BACKEND=dockerhost` and binds the host
      Docker socket, so posting a deploy carrying `pidMode: host` + `capAdd: [SYS_ADMIN,
      SYS_PTRACE]` — which the builder's own default-deny `PolicyFromEnv` does not stop — runs it
      on the host's daemon. Fixing either breaks the chain; fix both. In CAS mode (explicit
      `--storage`) no socket is mounted and that chain does not close, but an unauthenticated
      privileged build API is arbitrary code execution regardless. Remedy: mint an internal credential with
      the existing installation-secret machinery, or move the builder to a unix socket.

- [ ] **Correct the LTM claim that `Privileged` was the last escalation knob.**
      `.agents/docs/LTM/auth-and-security.md` — "hostPath binds were already refused outright by
      the k8s backend, so `Privileged` was the only remaining user-controlled escalation knob" —
      is disproved by the run above. Fix it in place when the policy gap is closed.

High:
- [ ] **Qualify policy identities by the verifier that asserted them.** mTLS CN, JWT `sub` from
      each verifier, SSH-enrolled name and exchanged subject all collapse to one flat string, so
      every configured verifier can assert any identity — including any CA in the
      `--tls-client-ca` bundle issuing `CN=admin`. Namespace the key (`mtls:`, `jwt:iss#sub`,
      `ssh:`), or document that the client CA must be dedicated and `sub` namespaces must not
      collide. Consider URI SAN over CN.
- [ ] **Require `CORNUS_JWT_AUDIENCE` when a JWKS source is configured.** Without it, any token
      that issuer minted for any relying party authenticates — for a cluster JWKS that is every
      pod's default-audience ServiceAccount token. The docs' own exchange example already passes
      `--audience cornus`; the server should insist on what its example demonstrates. Relatedly,
      warn on `CORNUS_JWT_DEFAULT_SCOPE=api` and log at `warn` what it granted.
- [ ] **`CORNUS_EGRESS_GATEWAY` should require a policy and block link-local by default.** With
      the gateway on and `CORNUS_EGRESS_POLICY` unset the server dials anything a workload names,
      including `169.254.169.254` (the node's cloud identity). `cornus socks5` already refuses
      loopback and link-local when exposed — the server-side gateway should not be weaker than
      the client-side proxy.
- [ ] **Web UI / MCP hardening.** (a) Ship a startup-generated bearer token in the printed URL
      (`--token`, `--no-token`) — loopback is not an authentication boundary on a shared host,
      and the surface is exec + persistent terminals + `file_write`. (b) Make
      `--allow-non-loopback` with no `--allow-host` a hard error instead of a warning that
      silently drops the only rebinding guard. (c) Confirm the non-WebSocket BFF and MCP
      handlers validate `Origin`, not only `Host` — the retained same-origin default covers the
      websockets, and MCP's own localhost protection is deliberately disabled. *(verify)*
- [ ] **Hub service-name squatting.** With `CORNUS_HUB_REGISTER_POLICY` unset any spoke registers
      any name, and the caretaker dns role's synthetic-IP mapping makes the interception
      invisible to the victim app. Document it in `docs/guides/hub.md`; consider defaulting, when
      auth is on, to "an identity may register only names it owns".
- [ ] **Name the hub store as a trust root.** `peer` JWTs verify against a public key read from
      Redis / the `HubEndpoint` CRs with `sub == kid`, so store write access mints peer
      credentials and rewrites delivery routing. Document that `CORNUS_HUB_REDIS` needs AUTH and
      TLS and that the CRs/Leases must be RBAC-restricted.

Medium (see the review for the full statement of each):

- [ ] No revocation for issued sessions, exchanged tokens, or mTLS certs (no CRL/OCSP); the only
      primitive is rotating a signing key. Document the model; consider a `jti` denylist.
- [ ] No content trust in build → push → deploy, and the `dockerhost` re-export view recomputes
      digests, so the digest pinning `deploy-spec.md` recommends does not hold there.
- [ ] Credential endpoints are authorized by the network namespace: reachable by every container
      in the pod including ephemeral debug containers, and with `aws-imds` + `wellKnown` any
      app-level SSRF becomes credential theft with no hop-limit equivalent. Add both to the
      credentials guide's "Trust" section.
- [ ] `CORNUS_INGRESS_ENFORCE_DOMAIN` should default to true whenever `CORNUS_INGRESS_DOMAIN` is
      set — otherwise a deploy claims any hostname cluster-wide.
- [ ] Make `tunnel` a non-implied API action, and warn in `docs/guides/tunnels.md` that a tunnel
      publishes a workload with no authentication in front of it.
- [ ] Workload log recording is on by default (168h at rest) — document location, permissions and
      retention; consider making logs opt-in separately from metrics.
- [ ] Default a `CORNUS_MAX_BUILD_CONTEXT_BYTES`, cap registry uploads, and rate-limit
      authentication failures (SSH challenge, token exchange, docker-login Basic). The JWKS
      unknown-kid refetch is already rate-limited — extend the same instinct.
- [ ] `/.cornus/v1/info` is auth-exempt: name it as a fingerprinting surface and keep it minimal.

Documentation and process:

- [ ] **Add `SECURITY.md`** — disclosure contact, embargo expectations, supported versions. The
      cheapest item on this list and the most conspicuous omission for a public repo.
- [ ] Add a consolidated **threat model** page naming the actors and the standing assumptions
      (a build is remote code execution; an authenticated client is an operator; a workload is
      untrusted; the hub store, client CA and configured issuers are trusted; loopback is not an
      authentication boundary).
- [ ] Add a **hardening checklist** page an operator follows top to bottom — the recipes exist in
      `guides/security.md` but the reader has to assemble them.
- [ ] Document the least-privilege story for the server itself: the quickstart is `--privileged`
      + `docker.sock`, i.e. root on the host by construction. Give the reduced variants.
- [ ] Add a key inventory table: `installation.key`, `peer.key`, the enrollment secret,
      `auth/authorized_keys`, TLS material — locations, modes, rotation, replica sharing.
- [ ] State what lands in logs and in the activity flight record (identities, image refs, env
      keys?). The internal "never log credential values" note is not a user-facing guarantee.

- [ ] **Conduit migration: two things still unverified.** (a) The Windows path
      (`WSADuplicateSocket`, `LockFileEx`, `WSAECONNREFUSED`) has never executed — bindings and
      the IOCP disassociation rule are checked against x/sys v0.46.0 and Go's own
      `net/file_windows.go`, but nothing has run it. (b) `conduithost`'s takeover tests model
      host death with `Host.Close`, not a real SIGKILL; `pkg/listenerpass` proves the
      descriptor crosses a process boundary, but a cross-process conduit kill (with the
      connect loop asserting zero ECONNREFUSED) belongs with the stage 3/4 wiring, where there
      are real processes to kill.

- [x] **DONE 2026-08-09: published names (locals) now have sequences and claim identity.**
      `RegisterLocal` returns a `LocalReg` with a `LocalHandle`; `UnregisterLocal(handle)`
      withdraws exactly that claim and restores the one beneath; `RegisterLocalSeq` is the
      replay path. `ingressemu.NewMux`'s register callback returns its own undo, so the
      `unregister` callback is gone. Fixes the live defect described below. Original note
      follows.
- [ ] ~~**Published names (locals) need the same last-claim-wins treatment as aliases.**~~
      `socks5.Router.RegisterLocal` replaces (which matches the decision), but
      `UnregisterLocal(host, port)` deletes the subject outright and cannot tell which claim
      is withdrawing. **This is a live defect today**: if project A publishes `app.test` and
      project B then publishes it, A's teardown destroys B's registration. Needs ordered
      claims plus a handle returned by `RegisterLocal`, which ripples into
      `ingressemu.NewMux`'s `unregister func(host string, port int)` callback
      (`pkg/ingressemu/ingressemu.go:235`) and four call sites in
      `pkg/clientconduit/clientconduit.go:441-504`. Aliases were done 2026-08-09; see the
      JOURNAL entry "conduit name collisions: last claim wins, and leaving repoints".

- [x] ~~**Report the repoint.**~~ DONE 2026-08-09: `Registrar.Logf` reports a name taking
      or handing back a claim, gated on `Router.Recovering()` so a takeover's restorations
      are not narrated as movements. Original note follows.
- [ ] ~~**Report the repoint (superseded).**~~ `RegisterAlias`/`UnregisterAlias` now return
      `(winner, changed)`, but no caller uses it yet. `clientconduit.socks5Conduit.Add`
      (`:426`) and its withdrawal goroutine (`:429`) should surface "the short name `web` now
      resolves to demo-web (shop left)" — otherwise a member leaving repoints a name with
      nothing to notice.

- [x] **DONE 2026-08-09: claims carry a host-minted sequence number.** `socks5.aliasClaim`
      has a seq and the list is ordered by it; `RegisterAliasSeq` is the replay path;
      `conduithost.Registrar` takes a `Registration` struct carrying the seq, and `Takeover`
      replays through `RegisterAt`. See the JOURNAL entry "claim sequence numbers: precedence
      that survives its host". Locals still have neither sequences nor claim identity — that
      remains open above.

- [x] **DONE 2026-08-09: single-accepter enforced by a held accept lease.** Per-address
      `<port>/<key>.accept`, taken before advertising and released after the listener closes,
      non-blocking so followers stay cancellable. The step-past path is gone: an unresponsive
      host is refused with `UnresponsiveError`, and a wedged host halts the conduit until it
      is killed (the accepted cost). Per-process legs turned out to be unnecessary once the
      lease guarantees the previous holder is dead before anyone rebinds. Original note
      follows.
- [ ] ~~**The single-accepter invariant is unenforced, and one code path actively breaks it.**~~
      `conduithost.withLock` is a create-or-join mutex, not a held lease, so nothing stops two
      participants accepting; and `electLocked` deliberately steps past an unresponsive host
      while that host still holds a replica of the same listening socket. If it recovers,
      connections split silently (39/21, measured). Decide between a HELD accept flock (a
      wedged host halts the conduit until killed) and an accept lease with epoch fencing (a
      wedged host can be stepped past and yields on recovery), then implement it before
      stage 3 wires any of this to real traffic. See the JOURNAL entry
      "re-examining 'flock is enough' after the takeover was built".

- [x] **DONE 2026-08-09: recovery window wired to takeover.** `conduithost.Recoverer` is an
      optional registrar capability; `Takeover` opens the window before replaying and only
      when hosting, and reports through Logf when the registrar cannot. `Config.RecoveryWindow`
      defaults to 5s. Still needs the stage-3 registrar to forward `BeginRecovery` to
      `socks5.Router.SetRecoveryUntil`.

- [ ] **Consider closing the recovery window early.** It currently always runs to its
      deadline, so a deployment-qualified name (`demo-web.cornus.internal`, which needs no
      claim) stalls for the whole window during a takeover. If the new host knew how many
      participants were expected back it could close as soon as they had all re-registered —
      but that count is exactly what the mirror would have provided, so it may not be worth
      it. Measure the stall first.

- [x] **DONE 2026-08-09: a joiner can publish names.** `publishOverSocket` lends the conduit
      a unix socket for the addressless listener, carried in the `KindLocal` payload's
      `Upstream`; the host dials it through `unixLocalDialer`. Hosts publish the same way, so
      a published name replays after a takeover like any other claim. INGRESS is still
      host-only. Original note follows.
- [ ] ~~**A joiner cannot publish names or serve ingress.**~~ `rendezvousConduit.AddLocal` and
      `AddIngress` refuse when this process joined rather than hosts, because both terminate
      in the publishing process and reaching them from the host needs the register-local
      upstream socket. That is stage 5 (`cornus web --publish-in-conduit` hosting its own
      BFF). Refused loudly, not silently.

- [ ] **A joiner's registrations are dialed through the HOST's tunnel.** Per-claim dialers
      exist and are tested, but `clientconduit`'s Registrar returns nil for every peer because
      the joiner's `ConnSpec` is not carried on the wire yet. Consolidating two projects on
      DIFFERENT servers would tunnel both to the host's server. Add `ConnSpec` to the alias
      payload and resolve it host-side, the way `clientagent` already does for `up -d`.

- [ ] **Find out whether anything else resolves the agent dir from the environment.**
      `clientagent.ConduitRegistry()` did, and wiring it into `ensureConduitLocked` silently
      pointed the unit tests at the user's real `/run/user/<uid>/cornus`, where they bound
      real ports and left advertisements behind. Fixed by injection, but `Socket()`,
      `statePath()` and `logPath()` resolve the same way — worth checking which tests touch
      them and whether the same class of accident is already latent.

- [x] **DONE 2026-08-09: `cornus web --publish-in-conduit` hosts its own BFF.** It joins the
      conduit in its own process, serves the BFF on a memlisten and publishes it with
      `AddLocal`; `publishRequest` became `publishConduitConfig`. The direct-serving `--addr`
      mode is untouched. Original note follows.
- [ ] ~~**Stage 5's last step: move `cornus web --publish-in-conduit` off the agent.**~~ The
      mechanism is done (a joiner can publish a name the host serves), but `cornus web` still
      asks the agent to host its BFF via `web-serve`. Hosting it in `cornus web`'s own process
      and publishing with `AddLocal` deletes `agentSelfView`, the `socketAgentView` round-trip
      and the duplicated `ToBFFIngress` — and it is what actually closes the 2026-08-09 report.

- [ ] **Ingress is still host-only in a joined conduit.** `rendezvousConduit.AddIngress`
      refuses when this process joined, because native mode tunnels through the host's dialer
      and emulate mode terminates TLS in the publishing process. Emulate could use the same
      lent-socket trick as `AddLocal`; native needs the host to resolve the joiner's
      controller. Refused loudly today.

- [x] **DONE 2026-08-09: deleted the agent's web hosting.** About 925 lines, plus the
      `web-serve`/`web-stop` actions and `WebSpec`; ProtocolVersion is now 9. `Inventory.Webs`
      turned out to have NO reader outside the agent's own tests, so the feared
      `daemon status` regression did not exist. Original note follows.
- [ ] ~~**Delete the agent's now-unreachable web hosting.**~~ `doWebServe`, `handleWebServe`,
      `webFrontend`, `reapWeb`, `agentSelfView`, `defaultPublishedName` and the
      `web-serve`/`web-stop` actions are dead from this CLI now that `cornus web` hosts its
      own BFF. Deleting them also removes `Inventory.Webs`, which `cornus daemon status`
      (`cmd/cornus/daemon.go`) and the E2E harness read — so the change has a blast radius
      and wants its own pass, including whether `daemon status` should instead report
      published names from the conduit registry.

- [x] **DONE 2026-08-09: E2E web scenarios pass against the new publish path.** All five
      (`web`, `web-conduit-join`, `web-fs`, `web-agent-detect`, `web-terminal-introspect`) on
      the docker target. `web-conduit-join`'s `daemon status` assertion — exactly ONE proxy,
      at the compose-chosen address — also confirms removing `Inventory.Webs` broke nothing.
      Original note follows.
- [ ] ~~**Re-run the E2E web scenarios against the new publish path.**~~ `web.star` and
      `web-conduit-join.star` drive `cornus web --publish-in-conduit` through the harness,
      which reads the published name off the client's own output line. The line is unchanged,
      but the mechanism underneath it is entirely new and has not been exercised end to end
      with a real server.


- [ ] **Document the terminal typeface settings in `docs/guides/web-ui.md` (optional).** The
      guide does not enumerate settings — it explains behaviours that need explaining, and it
      names **Settings -> Terminal -> Shell candidates** only in passing — so the three new
      rows (Font, Font size, Line height, added 2026-08-09) were left out rather than
      half-documenting the screen. Each row carries a full description in the UI already. If
      it is written up, the one genuinely non-obvious point is the fallback: three of the five
      bundled families have no box-drawing subset, so a TUI's frames come from the browser's
      monospace and can differ in width. Doing this means `docs/ja/` and `docs/zh/` too
      (see the `translate-documents` skill).

- [ ] **Consider whether the E2E web scenarios should assert the font assets are embedded.**
      `pkg/webui/dist` now carries 42 woff2 files (692 KiB) that only reach a user through
      `//go:embed`. Nothing outside the Vite build checks they are actually served — a broken
      asset path would show up as terminals silently rendering in the fallback face, which is
      indistinguishable from the default setting.

- [x] ~~**`Router.RegisterAliasSeq` has no caller.**~~ DONE 2026-08-09: deleted;
      `AliasSpec`/`Claim` is the surviving form. `AliasSpec` + `Router.Claim` superseded it
      and the Registrar uses `Claim`; only doc comments still mention it. `RegisterLocalSeq`
      IS used, so the pair is asymmetric. Either delete it or route `Claim` through it — dead
      API left by a refactor, found by checking exported symbols for non-test callers rather
      than by reading.
