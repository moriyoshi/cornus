...
2. **Build engine** — an in-process BuildKit solver (no separate `buildkitd`) with
   ...
   When the server cannot perform the mounts BuildKit requires, it can relay those same
   endpoints to an explicit builder or lazily start and supervise a privileged builder
   container made from the running Cornus executable.
3. **Deploy engine** — an imperative, pluggable deploy backend with **five backends**:
   ...
   runs them directly through an OCI runtime with no daemon at all, `incus` deploys
   OCI images as Incus application containers (Incus 6.3+), and `kubernetes`
...
default-deny workload privilege policies on one axis. Diagnostics are split into four
complementary planes: Cornus process telemetry, workload telemetry through an embedded
Collector, the built-in workload observability store (with automatic runtime log capture),
and the append-only activity flight recorder. An optional Prometheus `/metrics` endpoint
exposes Cornus process metrics. See the root `ARCHITECTURE.md` for both.

## Recent runtime capabilities

Kubernetes can realize a first-class Knative Serving descriptor as a native Service
with autoscaling and scale-to-zero; other targets preserve its portable deployment
form while warning that Knative behavior is unavailable. Workloads can also opt into
an embedded OpenTelemetry Collector. It exports directly to a configured backend or,
by default, into the built-in Cornus store; when Cornus is the destination, the
caretaker mux can carry the export without workload network egress or a server token.

## Deployment model

Cornus ships as a container image (plus prebuilt all-in-one CLI binaries) and runs both
on a **local Docker host** (as a host process or containerized server) and as a
first-class **Kubernetes service** (StatefulSet + PVC + Service + RBAC; Helm chart
under `deploy/helm/cornus`). Pre-built multi-arch images are released to
`ghcr.io/moriyoshi/cornus` (semver tags), with third-party license attribution
bundled inside the image. Releases attach CLI binaries for linux and macOS
(amd64/arm64) and Windows (amd64) with a `SHA256SUMS` manifest; Linux artifacts
are fully static. The Helm chart is published as an OCI artifact, and every artifact is
signed with keyless cosign. Both the image and downloadable binaries embed the built SolidJS web UI;
the frontend is built once before Go compilation so every artifact serves the same interface.
The shipped k8s manifests/chart preset the `kubernetes` deploy backend. The README
keeps a loopback-only Docker-host evaluation path; the full user quick start under
`docs/introduction/quick-start.md` stands the system up Docker-free on single-node
k3s. The registry and deploy subsystems need no special privileges; the build engine
needs root, a rootless user-namespace stack, or builder delegation — see the root
`ARCHITECTURE.md` "Running with the right privileges".

## Interfaces

* HTTP: `/v2/*` (registry); `/.cornus/v1/info`; build and deploy under
  `/.cornus/v1/build[(/attach)]` and `/.cornus/v1/deploy[/{name}[/{action}]]`
  (including `deploy/attach` and `deploy/exec/*`); `/.cornus/v1/ingress-tunnel`
  plus its `/channel/*` transport; caretaker, hub, and cross-replica forwarding
  under `/.cornus/v1/{caretaker/attach,hub/{catalog,forward},mount/forward}`;
  `/.cornus/v1/{gc,storage,activity}`; `/.cornus/v1/volume/*`; optional
  observability query, Grafana-compatible, and OTLP ingest routes under
  `/.cornus/v1/{obs,otlp}/*`; `/healthz`; `/readyz`; and an opt-in Prometheus
  `/metrics`.
* CLI (kong): `serve`, `setup`, `config`, `build`, `push`, `deploy`, `exec`, `port-forward`,
  `socks5`, `tunnel`, `ingress-tunnel`, `compose`, `web`, `daemon`, `hub`, `token`,
  `activity`, `observe`, `storage`, `health`, `version` (plus hidden sidecar-facing aliases
  such as `caretaker` / `caretaker-check`). `cornus config` manages
...
* `cornus compose`: a Docker Compose-compatible command group (`up`/`down`/`ps`/`build`/
  ...
  `cmd/cornus/internal/composecli`. Compose `provider:` services are external resources rather
  than deployments: the client runs their plugin lifecycle, gates dependents on successful
  provisioning, and injects the environment values the plugin reports.
* `cornus web`: unauthenticated browser UI for workloads, Compose projects and dependency
  graphs, mounts, tunnels/forwards, configuration files, logs, and exec. It binds loopback by
  default. Explicit `--allow-non-loopback` and Host allow-list controls support off-host
  access. It runs a
  client-side BFF because Compose structure and live agent sessions do not exist in the
  server's flattened workload API. The BFF's operation core also powers co-hosted HTTP MCP
  and stdio-only MCP modes. Structured deployment-origin metadata provides lineage, but only
  a locally loaded Compose project supplies the project definition and editable files.
...
with reader-facing architecture adapted from the root `ARCHITECTURE.md`, built and deployed to
