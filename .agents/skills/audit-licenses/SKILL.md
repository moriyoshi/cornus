...
**Build tags are load-bearing.** `go list -deps` reports the dependency graph for
the build configuration it is given, so scanning with a shorter tag list silently
omits real shipped dependencies rather than failing. `scan_licenses.py` therefore
defaults `--tags` to the shipped set, READ FROM the Makefile's `LICENSE_TAGS` —
the same source `ci.yml`'s notices-drift gate reads, so the two cannot diverge.
(It leaves an existing `-tags=` in `GOFLAGS` alone, which is how CI passes its
own.) Keep `LICENSE_TAGS` in step with the Dockerfile's `BUILD_TAGS` when the
shipped set changes; nothing else needs updating. Measured on 2026-07-28: scanning
untagged produced notices listing **257** modules where the shipped configuration
links **352**. The missing 95 were the whole otelcol collector tree plus
`imbh-go` (Apache-2.0), `sable` (MIT) and their arrow/flatbuffers dependencies,
whose bindings compile as stubs without `imbh`/`sable_extern_lib`. Nothing
warned; the notices just came out shorter.
...
Measured 2026-08-09: **352** shipped Go modules and **16** bundled npm packages,
all permissive except **nine MPL-2.0** HashiCorp libraries and the **five
OFL-1.1** `@fontsource` font packages (both weak-copyleft, both compatible).
**Zero** GPL/LGPL/AGPL, zero review items. `third_party/yamux` is modified
MPL-2.0 and must retain its license. If a run diverges from this baseline, a
dependency changed — investigate the delta.

Note on the Go count: 352 is the LINUX shipped configuration
(`GOOS=linux`, `LICENSE_TAGS`), which is the only one released — see the
Makefile's `docker-*` targets. A notices file listing Windows-only modules
(`Microsoft/hcsshim`, `go-winio`, `danieljoos/wincred`, `yusufpapurcu/wmi`, …) was
generated under different assumptions; those do not compile into any shipped
artifact and were dropped on the 2026-08-09 regeneration. Over-listing is not a
compliance failure, but it is noise, and it hides real drift.
