...
# OFL-1.1 sits here rather than in PERMISSIVE. Its grant is broad — the fonts may be
# embedded, redistributed and sold as part of a larger work — but it carries two
# file-level conditions that are copyleft in shape: a derivative FONT must itself be
# released under OFL, and it may not keep the original's Reserved Font Name. Neither
# reaches Cornus's own code, and the fonts ship byte-for-byte as fontsource cut them, so
# retaining the license text is the whole of the obligation. Same treatment as MPL-2.0,
# for the same reason.
WEAK_COPYLEFT = {"MPL-2.0", "MPL-1.1", "EPL-2.0", "CDDL-1.0", "CC-BY-4.0", "OFL-1.1"}
...
def classify(text):
    ...
    # OFL BEFORE MIT — third trap, same shape as the MPL one above. The SIL Open Font
    # License opens with "Permission is hereby granted, free of charge, to any person
    # obtaining a copy of the Font Software", so the MIT test below matches it word for
    # word and every bundled @fontsource package would be reported as MIT. Order is
    # load-bearing here exactly as it is for MPL. See references/policy.md.
    if "sil open font license" in tl or "open font license" in tl:
        m = re.search(r"open font license[,\s]+version\s*([0-9.]+)", tl)
        return "OFL-" + (m.group(1) if m else "1.1")
