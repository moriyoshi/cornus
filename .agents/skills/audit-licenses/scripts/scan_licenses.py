...
KNOWN_MODULE_LICENSES = {
    ...
    "github.com/rootless-containers/proto/go-proto": "Apache-2.0",
}
...
def scan_go(repo, pkgs):
    ...
    for path, ver in sorted(mods.items()):
        ...
        if lic in ("NO-LICENSE-FILE", "UNKNOWN") and path in KNOWN_MODULE_LICENSES:
            lic = KNOWN_MODULE_LICENSES[path]
            fn = "(verified from source headers; LICENSE at repo root, not in submodule)"
