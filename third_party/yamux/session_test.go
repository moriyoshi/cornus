...
func TestSession_PartialReadWindowUpdate(t *testing.T) {
	testConnTypes(t, func(t testing.TB, clientConn, serverConn io.ReadWriteCloser) {
		conf := testConfNoKeepAlive()
		// This test does a single partial Read of the whole flood and asserts the
		// resulting window update equals exactly the bytes read. That only holds if
		// the flood is buffered in full before the Read: the fork's DATA-frame cap
		// (defaultMaxDataFrame, 128 KiB) would otherwise split a 256 KiB flood into
		// two frames, and on a real socket the second frame can lag the sender's
		// drain signal, so the Read returns only the first frame and credits a
		// smaller (off-by-one) update. Pin MaxDataFrame above the window so the
		// flood is one atomically-buffered frame and the assertion is deterministic.
		conf.MaxDataFrame = conf.MaxStreamWindowSize
