...
third-party-licenses:
	...
	@LDFLAGS="$$($(GO) run github.com/moriyoshi/imbh-go/cmd/imbhgo-fetch@$(IMBH_VERSION) -print-env 2>/dev/null | sed -n 's/^export CGO_LDFLAGS="\(.*\)"$$/\1/p')"; \
...
.PHONY: e2e-kube
e2e-kube: build
	E2E_KNATIVE=$(E2E_KNATIVE) E2E_INGRESS_NGINX=$(E2E_INGRESS_NGINX) $(BIN)/cornus-e2e --target kube $(if $(KEEP),--keep,) $(E2E_FLAGS) $(SCENARIOS)
