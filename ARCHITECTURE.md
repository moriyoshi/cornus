...
There is deliberately **no in-container exec route**. Splicing `cp`/`mv` into somebody
else's image is unreliable in ways that compound — a distroless image has no shell, busybox
and GNU disagree on directory-onto-directory, an exec's exit code is only as trustworthy as
the runtime's inspect call, and exec is a wider privilege than a file operation. `execServer`
is served instead by **`deploy.FSOperator`** (`api.FSOpRequest`: stat, list, get, put,
mkdir, remove, rename, copy), whose refusals carry machine-readable codes so a caller can
tell "tell the user" from "do this the long way instead".

On kubernetes this is not an optimization but the only way in: that backend's archive trio
is unsupported outright, so a volume-backed path could be listed (exec works) and then
neither read, written, nor copied. `addFSOpRole` mounts the app's managed volumes into
whichever caretaker the pod already has and the operator serves them over tag `S`. The
caretaker shares only the app's *network* namespace, so it has its own mount and PID
namespace and can serve **exactly what was mounted into it** — image-layer paths keep the
archive route. The BFF learns whether a backend has an archive at all and routes
accordingly; a backend with no operator relays, silently.

The UI has no authentication, so `requireLoopback` accepts only `localhost` or a
loopback IP listen address. That constraint shapes the explorer directly: a bind-mount
...
- **dockerhost** is supported with the Docker socket and data dir bind-mounted;
  `rshared` propagation is needed for client-local mounts. Host networking (or
  explicit remote mode) is needed when Cornus must dial workload container IPs.
- **containerd path handling is complete**: data-dir-backed OCI mounts, log
  files, and the staged log-shim executable use host spellings. The full
  containerized-containerd mode is not complete, however. Its CNI plumbing must
  run in the host network namespace and the image must contain CNI plugins, so
  `--network host`, CNI packaging, and the corresponding E2E leg remain required
  before this topology is advertised as supported. Preflight warns about the
  network-namespace mismatch; a missing plugin already fails loudly when a
  deploy attempts CNI setup.
...
published port would add a failure mode, not remove one.
