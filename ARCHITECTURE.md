...
Two rules keep "one config" from splintering into several proxies that then fight
over one address. First, `canonicalConduitCfg` substitutes the socks5 engine's own
defaults (listen, suffix, bare-names) *before* the identity is taken, so the empty
and explicit spellings of one proxy key one conduit rather than two. Second,
`cornus web --publish-in-conduit` is the one frontend that **adopts** rather than
keys: a browser has a single proxy setting, so the published UI has no legitimate
opinion about listen/suffix/ingress and instead joins whichever shared socks5
conduit the connection already has, deciding this agent-side under `a.mu` where the
live set is knowable atomically. Its `--conduit` therefore *pins* settings (used
exactly, and only when there is nothing to join) rather than selecting a conduit.
Where a genuine collision remains — two sessions wanting one address with different
settings — the agent refuses before binding and names the frontend already holding
it, instead of passing through `bind: address already in use`.
