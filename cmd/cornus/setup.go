...
type SetupCmd struct {
	// Scenario presets the deployment scenario, skipping the opening picker. The
	// valid names come from the wizard's own scenario list (setupwiz.ScenarioNames),
	// so they cannot drift from what the picker offers; an unknown one is reported
	// with the full list, which is the picker's job when there is no picker.
	Scenario string `kong:"name='scenario',placeholder='NAME',help='Skip the scenario picker and configure the named scenario directly (an unknown name is rejected with the list of valid ones).'"`
}
...
func (c *SetupCmd) Run(cli *CLI) error {
	...
	// Resolve the preset BEFORE any prompting, so a typo fails immediately
	// instead of after the first question.
	run := func(wiz *setupwiz.Wizard) error { return wiz.Run(cli.rootContext()) }
	if c.Scenario != "" {
		sc, perr := setupwiz.ParseScenario(c.Scenario)
		if perr != nil {
			return perr
		}
		run = func(wiz *setupwiz.Wizard) error { return wiz.RunScenario(cli.rootContext(), sc) }
	}
	wiz := setupwiz.NewWizard(d, setupwiz.NewUI(d), path)
	if err := run(wiz); err != nil {
