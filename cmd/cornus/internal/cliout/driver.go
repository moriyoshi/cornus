...
type Driver struct {
	...
	// progressStyle is the resolved live-progress preference (status vs stream),
	// read from CORNUS_PROGRESS at New and overridable via SetProgressStyle (the
	// compose --progress flag). It gates whether Progress starts a live region.
...
func New(o Options) *Driver {
	...
	// An unrecognized CORNUS_PROGRESS falls back to the default (ParseProgressStyle
	// returns ProgressStatus with ok=false); the compose --progress flag rejects a
	// bad value at the flag layer instead.
	progressStyle, _ := ParseProgressStyle(env("CORNUS_PROGRESS"))
...
// SetProgressStyle overrides the progress style resolved from CORNUS_PROGRESS.
// The compose commands call it to apply their --progress flag, which takes
// precedence over the environment. Call it before starting any Progress.
func (d *Driver) SetProgressStyle(s ProgressStyle) { d.progressStyle = s }

// ProgressStyle reports the resolved progress style (default ProgressStatus).
func (d *Driver) ProgressStyle() ProgressStyle { return d.progressStyle }
