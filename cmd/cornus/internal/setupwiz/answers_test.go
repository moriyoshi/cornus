...
func TestBuildContext(t *testing.T) {
	...
	t.Run("ssh-key-auth stores selectors not key material", func(t *testing.T) {
		c := BuildContext(Answers{Name: "laptop", KeyAuthIdentityFile: "/keys/id_ed25519"})
		if c.KeyAuth == nil || c.KeyAuth.IdentityFile != "/keys/id_ed25519" || c.KeyAuth.Name != "laptop" {
			t.Fatalf("key-auth: %+v", c.KeyAuth)
		}
		if c.Token != "" {
			t.Fatalf("key-auth unexpectedly stored a bearer token: %q", c.Token)
		}
	})
}
...
func TestSetContextCommandGoldens(t *testing.T) {
	cases := []struct {
		...
		{
			name: "key-auth",
			a:    Answers{Name: "laptop", Server: "https://x", KeyAuthFingerprint: "SHA256:abc"},
			want: "cornus config set-context laptop --server https://x --key-auth-fingerprint SHA256:abc --key-auth-name laptop",
		},
	}
	names := map[string]string{"local": "local", "token-redacted": "url", "shell-quoting": "q", "ssh": "devbox", "key-auth": "laptop"}
