...
func TestWizardServerNotReadyGuidesAndSkipsLiveWork(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.yaml")
	ui := &scriptUI{
		selects:  []int{0, backendIndex(backendBare), 2}, // Local; runtime: Bare; skip the unit offer
		inputs:   []string{"", ""},                       // server (default), name (default "local")
		confirms: []bool{false, true},                    // not set up, make current
	}
...
func TestWizardBackReasksPreviousStep(t *testing.T) {
	...
	ui := &progUI{replies: []progReply{
		...
		{i: backendIndex(backendBare)}, // runtime: Bare
...
func TestWizardBackFromRuntimePickerReasksTheConfirm(t *testing.T) {
	ui := &progUI{replies: []progReply{
		...
		{i: backendIndex(backendIncus)}, // runtime: Incus
	}}
...
func TestScenarioOptionsCoverEveryScenario(t *testing.T) {
	...
	const last = ScenarioIncusContainer
...
func TestLocalBackendsCoverEveryDrivableRuntime(t *testing.T) {
	want := []string{backendDocker, backendPodman, backendContainerd, backendBare, backendIncus, backendKubernetes}
...
func TestScenarioNamesCoverEveryScenario(t *testing.T) {
	names := ScenarioNames()
	if got, want := len(names), int(ScenarioIncusContainer)+1; got != want {
...
// backendIndex is the picker position of a backend, looked up rather than
// written down.
//
// These selections used to be literal indices, which meant INSERTING a backend
// silently changed which one every later test picked — the assertions still
// passed, against a different runtime than the comment claimed. Adding podman
// between Docker and containerd is exactly that edit, and it broke two tests
// loudly only because they happened to assert the resulting backend by name.
func backendIndex(key string) int {
	for i, b := range localBackends {
		if b.Key == key {
			return i
		}
	}
	panic("no such backend in the local picker: " + key)
}
