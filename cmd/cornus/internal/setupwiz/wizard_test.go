...
// The scenario picker maps the selected INDEX straight to Scenario(idx), so the
// option list and the enum are one data structure split across two files. A
// mismatch does not fail to compile — it silently runs the wrong flow for the
// chosen label, which is about the worst way for this to break.
func TestScenarioOptionsCoverEveryScenario(t *testing.T) {
	// Highest enum value, kept in step with the const block.
	const last = ScenarioDockerContainer
	if got, want := len(scenarioOptions), int(last)+1; got != want {
		t.Fatalf("scenarioOptions has %d entries, want %d — the picker's index maps directly to Scenario", got, want)
	}
...
func TestDockerContainerScenarioHasItsOwnGuidance(t *testing.T) {
	if got := docPathFor(ScenarioDockerContainer); got != "/guides/server-in-a-container" {
		t.Errorf("docPathFor = %q, want the server-in-a-container guide", got)
	}
	steps := stepsFor(ScenarioDockerContainer)
	if len(steps) == 0 || !strings.Contains(steps[0], "preflight") {
		t.Errorf("first step = %v, want the preflight to lead (the binds are the difficulty)", steps)
	}
}
