...
var backendPrereqs = map[string]string{
	...
	backendPodman: "Needs the podman API socket, and cornus must be told which one: `CORNUS_PODMAN_SOCKET=$XDG_RUNTIME_DIR/podman/podman.sock` " +
		"after `systemctl --user enable --now podman.socket` (rootless), or the rootful `/run/podman/podman.sock` after `sudo systemctl enable --now podman.socket`. " +
		"Alternatively `CORNUS_PODMAN_SERVICE=1` makes cornus run `podman system service` itself, which needs only the podman binary on `PATH`. " +
		"Nothing is inferred: with neither set, the server refuses to start.",
...
var backendCaveats = map[string]string{
	...
	backendPodman: "Rootless podman is the trap. A rootless workload's network namespace is not routable from the host, so `cornus port-forward` and " +
		"`cornus tunnel` CANNOT reach it by dialing — they need `CORNUS_PODMAN_REMOTE=1`, which reaches the workload through a per-instance companion " +
		"and itself needs `CORNUS_AGENT_IMAGE` and `CORNUS_ADVERTISE_URL`. Deploy, logs and exec work either way. A ROOTFUL podman has none of this.",
...
// containerdRunCommand is the `ctr run` that starts a containerized server on a
// containerd host.
//
// `ctr`, not `docker`, and that is load-bearing rather than stylistic: cornus
// resolves its own container by asking the containerd it deploys through, so it can
// only find itself when THAT containerd created the server's container. A server
// docker started on a containerd host lands in docker's own containerd, on another
// socket, where the lookup finds nothing and the operator must supply
// CORNUS_HOST_PATH_MAP instead.
func containerdRunCommand(a *Answers) string {
	port := portOf(a.Server)
	dataDir := orDefault(a.ContainerDataDir, "/srv/cornus")
	return "ctr run -d --net-host --privileged" +
		" --mount type=bind,src=/run/containerd/containerd.sock,dst=/run/containerd/containerd.sock,options=rbind:rw" +
		" --mount type=bind,src=" + dataDir + ",dst=/var/lib/cornus,options=rbind:rw" +
		" --mount type=bind,src=/run/cornus,dst=/run/cornus,options=rbind:rw" +
		" --env CORNUS_DATA=/var/lib/cornus --env CORNUS_DEPLOY_BACKEND=containerd" +
		" " + containerImage + " cornus cornus serve --addr :" + port
}

// incusContainerRunCommand is the run command that starts a server beside incusd.
//
// Only the daemon socket is bound: this backend hands incusd no path of its own, so
// there is no data dir to make host-visible. --network host is what gives it a route
// to the incus bridge, which it cannot otherwise reach or join.
//
// Spelled with `docker`, but nothing here is docker-specific — `podman run` takes
// the identical flags, and an incus host may well have neither. That is why the
// setup steps lead with running the server AS an instance, which needs no second
// container runtime at all.
func incusContainerRunCommand(a *Answers) string {
	port := portOf(a.Server)
	return "docker run -d --name cornus --network host --restart unless-stopped" +
		" -p 127.0.0.1:" + port + ":" + port +
...
// setupContainerdContainer leads with the preflight for the same reason the docker
// one does, and names what each bind buys: two of the three fail silently without it.
func setupContainerdContainer(a *Answers) []string {
	dataDir := orDefault(a.ContainerDataDir, "/srv/cornus")
	return []string{
		"Start the server: `" + containerdRunCommand(a) + "`",
		"Cornus discovers the " + dataDir + " to /var/lib/cornus correspondence by asking containerd about its own container, so no CORNUS_HOST_PATH_MAP is needed — but only because `ctr` created that container. Start it with docker instead and cornus cannot find itself.",
		"Check the binds first if you can: `" + containerImage + " daemon preflight` with the same mounts and CORNUS_DEPLOY_BACKEND=containerd. It refuses to start on a configuration where no deploy could work.",
	}
}

// setupIncusContainer has no bind to check beyond the socket, so it leads with the
// routing consequence instead — the only thing that can go wrong here.
func setupIncusContainer(a *Answers) []string {
	return []string{
		"Simplest on an incus host: run the server AS an incus instance on this daemon. It then sits on incusd's bridge alongside its workloads, so it needs neither host networking nor a companion, and no second container runtime. Expose the daemon socket to it with a PROXY device, not a disk device — a disk device is id-shifted to `nobody` inside an unprivileged instance and cornus cannot connect — and publish it where the parent directory already exists in the image, since `/var/lib/incus` is absent from most: `incus config device add cornus incusd proxy listen=unix:/tmp/incus-daemon.sock connect=unix:/var/lib/incus/unix.socket bind=instance mode=0660` then `incus config set cornus environment.CORNUS_INCUS_SOCKET=/tmp/incus-daemon.sock`",
		"Or run it in a container beside incusd (any runtime — `podman run` takes the same flags): `" + incusContainerRunCommand(a) + "`",
		"`--network host` is what gives that container a route to the incus bridge. Without it, port-forward, tunnels and caretaker dial-backs cannot reach a workload — deploys and host-published ports still work.",
	}
}
...
var (
	...
	nextContainerdContainerHint = []string{
		"The socket bind is what lets cornus discover its own host paths; the /run/cornus bind is where it pins each instance's network namespace and hands the path to containerd, which reopens it in the host's mount namespace; `--net-host` puts the CNI bridge and the port-publishing NAT rules on the host rather than inside this container. The published image already carries the CNI plugins and iptables.",
	}
	nextIncusContainerHint = []string{
		"Only the daemon socket is bound: incusd owns every path this backend hands it, so there is nothing to translate. What `--network host` buys is a route to the incus bridge — which a server running as an incus instance already has, and needs no docker or podman to get.",
	}
...
func guideFor(a *Answers) scenarioGuide {
	...
	case ScenarioContainerdContainer:
		return scenarioGuide{Synopsis: containerdRunCommand(a), Setup: setupContainerdContainer(a), Next: append(append([]string{}, nextContainerdContainerHint...), nextDockerContainer...), Doc: setupGuideURL + "#in-a-container"}
	case ScenarioIncusContainer:
		return scenarioGuide{Synopsis: incusContainerRunCommand(a), Setup: setupIncusContainer(a), Next: append(append([]string{}, nextIncusContainerHint...), nextDockerContainer...), Doc: setupGuideURL + "#in-a-container"}
	}
