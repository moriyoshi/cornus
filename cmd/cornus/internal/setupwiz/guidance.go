...
// stepsDockerContainer leads with the preflight, because the binds are the
// whole difficulty here and getting one wrong fails silently at deploy time
// rather than at startup.
var stepsDockerContainer = []string{
	"check the binds before committing: docker compose -f cornus-compose.yaml run --rm cornus daemon preflight",
	"start the server: docker compose -f cornus-compose.yaml up -d",
	"confirm it is reachable: cornus version --health",
...
func docPathFor(s Scenario) string {
	...
	case ScenarioDockerContainer:
		return "/guides/server-in-a-container"
	default:
		return "/cli/setup"
	}
}
