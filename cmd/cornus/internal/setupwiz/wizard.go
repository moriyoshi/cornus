...
var scenarioDefs = []scenarioDef{
	...
	{ScenarioContainerdContainer, "containerd-container", "containerd host (server in a container)", "run the server as a container on this containerd host"},
	{ScenarioIncusContainer, "incus-container", "Incus host (server in a container)", "run the server as a container beside this incusd"},
}
...
func (w *Wizard) scenarioSteps(ctx context.Context, a *Answers, f *clientconfig.File, origCurrent string) []step {
	...
	case ScenarioDockerContainer, ScenarioContainerdContainer:
		steps = append(steps, w.dockerContainerSteps(a)...)
	case ScenarioIncusContainer:
		steps = append(steps, w.incusContainerSteps(a)...)
	}
...
func scenarioBackend(a *Answers) string {
	...
	case ScenarioSSHBare:
		return backendBare
	case ScenarioSSHIncus, ScenarioIncusContainer:
		return backendIncus
...
var localBackends = []struct {
	...
	{backendPodman, Option{Label: "Podman", Desc: "rootful or rootless; needs the podman API socket, or the podman binary"}},
...
// incusContainerSteps asks only for the server URL. Unlike the docker and
// containerd container scenarios there is no data-directory question, because this
// backend hands incusd no path of its own: incusd owns every path it ever sees, so
// there is nothing to bind and nothing to translate. Asking would imply a bind that
// does nothing.
func (w *Wizard) incusContainerSteps(a *Answers) []step {
	return []step{
		w.inputStep(Question{Title: "Server URL", Default: "http://127.0.0.1:5000", Validate: validateServerURL},
			func() string { return a.Server }, func(v string) { a.Server = v }),
	}
