...
import (
	...
	"cornus/cmd/cornus/internal/ingresspub"
...
type UpCmd struct {
	...
	Watch               bool                 `kong:"name='watch',help='Watch the loaded compose files and env files for edits and automatically reload the configuration and re-reconcile the running services (recreating changed services, starting added ones, and tearing down ones removed from the file). Works in the foreground and, with -d, in the background agent.'"`
	NoForwardPorts      bool                 `kong:"name='no-forward-ports',help='Do not auto-forward published service ports to local listeners.'"`
	NoAttach            bool                 `kong:"name='no-attach',help='Do not stream service logs in the foreground (still holds mounts/forwards until Ctrl-C). Unlike docker compose, this is a project-wide switch, not a list of services to leave unattached.'"`
	NoDeps              bool                 `kong:"name='no-deps',help='Do not also bring up the depends_on dependencies of the named services. Only meaningful with an explicit service list (without one, every service is selected already).'"`
	ForceRecreate       bool                 `kong:"name='force-recreate',help='Recreate the workloads even when nothing about them changed. Container backends recreate on every up regardless; on kubernetes this is what forces a rollout of an otherwise identical Deployment.'"`
	Timeout             int                  `kong:"name='timeout',short='t',default='-1',help='Accepted for docker compose compatibility and NOT honored: the cornus deploy API carries no per-call shutdown timeout. Set stop_grace_period on the service instead.'"`
	NoLogPrefix         bool                 `kong:"name='no-log-prefix',help='Do not prefix streamed log lines with the service name.'"`
	Progress            string               `kong:"name='progress',default='',help='Progress display on an interactive terminal: status (one collapsed line per service that mutates in place as it comes up; the default) or stream (append-only scrolling lines, a full log). Overrides CORNUS_PROGRESS. Ignored for piped/plain/json output.'"`
	RemoveOrphans       bool                 `kong:"name='remove-orphans',help='Remove workloads for services no longer defined in the Compose file (left behind after a service was removed or renamed), like docker compose up --remove-orphans. Without it, up only warns about them.'"`
	...
	AllowNonLoopback    bool                 `kong:"name='allow-non-loopback',help='Permit the SOCKS5 conduit to bind a non-loopback address (e.g. --conduit socks5://0.0.0.0:1080). Refused by default: the conduit proxy has no authentication and dials arbitrary destinations from this host, so off-host it is an open proxy for anyone who can reach it.'"`
	...
	IngressEmulateCA    string               `kong:"name='ingress-emulate-ca',type='path',help='Emulate-mode: PEM CA certificate that signs the per-host leaf certs (with --ingress-emulate-ca-key). Empty uses a generated, persisted session CA. Takes precedence over CORNUS_INGRESS_EMULATED_CA and the profile.'"`
	IngressEmulateCAKey string               `kong:"name='ingress-emulate-ca-key',type='path',help='Emulate-mode: PEM CA private key paired with --ingress-emulate-ca. Takes precedence over CORNUS_INGRESS_EMULATED_CA_KEY and the profile.'"`
...
// --no-forward-ports into ModeNone, the --conduit override into the profile
// precedence, and --allow-non-loopback into the bind-exposure opt-in. It
// validates the result here rather than leaving it to clientconduit.Start,
// because `up -d` starts the conduit inside the background agent: without this,
// a bad selector would only be reported after the agent is already up.
func (c *UpCmd) conduitCfg(rt *runtime) (clientconduit.Config, error) {
	mode := c.Conduit
	if c.NoForwardPorts {
		mode = clientconduit.ModeNone
	}
	cfg, err := rt.conduitConfig(mode)
	if err != nil {
...
func (c *UpCmd) resolveIngress(ctx context.Context, rt *runtime, cfg *clientconduit.Config) error {
	return rt.resolveIngress(ctx, cfg, c.IngressConduit, c.IngressEmulateCA, c.IngressEmulateCAKey)
}

func materializeNativeTLS(rt *runtime, names []string, cfg clientconduit.Config) error {
	for _, name := range names {
		plan := rt.plans[name]
		if err := clientconduit.MaterializeNativeTLS(&plan.Spec, cfg); err != nil {
			return fmt.Errorf("service %s: %w", name, err)
		}
		// Emulate mode: the ingress is a client-side reverse proxy reached through
		// the conduit, so flag it client-emulated and the deploy backend skips the
		// cluster Ingress object (and its TLS Secret) for it. Without this the server
		// tries to create a real Ingress the emulate workflow never wanted — which
		// fails at deploy (e.g. no ingress RBAC / controller), aborting the up.
		clientconduit.MarkClientEmulatedIngress(&plan.Spec, cfg)
	}
	return nil
}
...
func (c *UpCmd) Run(cli *Cmd, r *clientconn.Resolver, d *cliout.Driver) error {
	...
	defer rt.cleanup()
	// One /.cornus/v1/info fetch feeds every up-front check below (memoized on the
	// runtime); best-effort, so an unreachable server leaves every field unknown and
	// each check falls back to its pre-existing behavior.
	info := rt.serverInfo(context.Background())
	// Warn up front if the client and server builds differ — a skew silently sends a
	// deploy down the wrong path (best-effort; no-op when versions match or are
	// unknown).
	warnServerVersionSkew(info, d)
	// Apply --progress before any Progress region is created (the build below opens
	// one), so the status/stream preference governs the whole command.
	if err := applyProgressFlag(d, c.Progress); err != nil {
		return err
	}
	...
	// docker compose parity: an explicit service list also brings up what those
	// services depend on, unless --no-deps. Announced when it adds anything, so a
	// selection that grew is never a surprise (and --no-deps is discoverable).
	if !c.NoDeps && len(c.Services) > 0 {
		expanded, added := expandDependencies(rt, names)
		if len(added) > 0 {
			d.Info("also starting dependencies of %v: %v (--no-deps to skip)", c.Services, added)
		}
		names = expanded
	}
	// Flags accepted for docker compose compatibility that cornus cannot honor,
	// or that mean something different here. Warned before anything is built or
	// deployed, so the notice is not buried under a build log. See flagcompat.go.
	warnShutdownTimeout(d, c.Timeout, "up")
	if c.NoAttach {
		warnNoAttachDivergence(d, c.Services)
	}

	if err := c.applyServiceOverrides(rt, names); err != nil {
		return err
	}

	// Refuse a depends_on condition this backend can never satisfy (today:
	// service_healthy where the backend runs no probe engine) before anything is
	// built or deployed, so the user gets one actionable message instead of a
	// dependency timeout after the workloads are already up.
	if err := validateDependencyConditions(rt, selectedSet(names)); err != nil {
		return err
	}
	...
	// Orphan handling (docker compose parity): a workload that belongs to this
	// project but whose service is gone from the Compose file is an orphan.
	// --remove-orphans tears them down now (before deploying, so a reused name is
	// free); otherwise up just warns and points at the flag. Project-wide even for a
	// partial `up SERVICE`, matching docker — an orphan is by definition a service
	// absent from the file, so no in-file service is ever caught by this. Best-effort
	// on a bounded, signal-cancellable context so it never wedges the deploy.
	{
		octx, ostop := signalContext()
		known := rt.knownResources()
		if c.RemoveOrphans {
			err := removeOrphans(octx, rt.client, rt.projectName, known, d, true)
			ostop()
			if err != nil {
				return err
			}
		} else {
			warnOrphans(octx, rt.client, rt.projectName, known, d)
			ostop()
		}
	}

	cfg, err := c.conduitCfg(rt)
	if err != nil {
		return err
	}
	...
	// --watch always needs the background agent (it holds the file watcher and
	// re-reconciles on edit), even for an all-fire-and-forget project that would
	// otherwise deploy statelessly and exit — see upDetached's watch handling.
	if c.Detach && (c.Watch || needsBackgroundAgent(specs, string(cfg.Mode))) {
		return c.upDetached(cli, rt, cfg, names)
	}

	ctx, stop := signalContext()
	defer stop()
	return c.runForeground(ctx, cli, rt, names)
}

// selectedSet renders the service names this up is bringing up as a lookup set,
// so a dependency check ignores depends_on targets outside the selection (they
// are never deployed here, and nothing ever waits on them).
...
// reloadDescriptor builds the watch inputs for a detached `up -d --watch`: the
// absolute file set to watch and the ReloadSpec the agent re-execs on change.
// The reload re-runs this exact CLI invocation (os.Args) in the ORIGINAL working
// directory with the ORIGINAL environment, so the agent's re-plan resolves the
// same compose files, relative --env-file paths, and env-driven settings the
// developer ran with. Returns zero values when --watch is off.
func (c *UpCmd) reloadDescriptor(rt *runtime) ([]string, *clientagent.ReloadSpec, error) {
	if !c.Watch {
		return nil, nil, nil
	}
	cwd, err := os.Getwd()
	if err != nil {
		return nil, nil, fmt.Errorf("--watch: resolving working directory: %w", err)
	}
	reload := &clientagent.ReloadSpec{
		Argv: append([]string(nil), os.Args[1:]...),
		Dir:  cwd,
		Env:  os.Environ(),
	}
	return rt.watchFiles, reload, nil
}

// applyServiceOverrides applies the project-wide client-side overrides to each
// selected service's plan spec, in place on rt.plans: the --egress flags
// (overriding any compose egress: block), env-mode proxy settings, the
// --telemetry-* flags (overriding any x-cornus-telemetry: block), and the
// workload lineage origin (project name + the client host/user/dir/git). Every
// call is a no-op when its inputs are unset. Shared by the initial Run and by a
// --watch reload (which re-plans rt.plans and must re-apply these).
func (c *UpCmd) applyServiceOverrides(rt *runtime, names []string) error {
	...
	for _, n := range names {
		...
		// --force-recreate: stamp a per-process token so the spec differs from the
		// live one and the workload is genuinely replaced (see forceRecreateLabel
		// for why a label is enough on every backend). The compose plan's own map
		// is never mutated — it is copied first, so a --watch reload re-planning
		// from the file starts clean.
		if c.ForceRecreate {
			labels := make(map[string]string, len(p.Spec.Labels)+1)
			for k, v := range p.Spec.Labels {
				labels[k] = v
			}
			labels[forceRecreateLabel] = recreateToken
			p.Spec.Labels = labels
		}
...
func (c *UpCmd) runForeground(ctx context.Context, cli *Cmd, rt *runtime, names []string) error {
	cfg, err := c.conduitCfg(rt)
	if err != nil {
		return err
	}
	if err := c.resolveIngress(ctx, rt, &cfg); err != nil {
		return err
	}
	if err := materializeNativeTLS(rt, names, cfg); err != nil {
		return err
	}
	// Bind through the conduit rendezvous, so this foreground session's proxy is
	// JOINABLE. Without it the proxy lived in this process and nothing else could
	// reach it — a `cornus web --publish-in-conduit` in another terminal could only
	// fork a second one beside it, which is the report this whole redesign came from.
	conduit, err := clientconduit.Start(ctx, rt.forwardDialer, cfg, clientconduit.WithRendezvous(clientagent.ConduitRegistry()))
	if err != nil {
		return err
	}
	d := rt.driver()
	...
	// Route the engine's deploy diagnostics through each service's live status line
	// rather than slog: a transient diagnostic (a workload still starting — an image
	// still pulling, a brief CrashLoopBackOff — that later comes up fine) or a
	// terminal failure folds onto that service's one mutating line in fancy+TTY
	// status mode, and falls back to a Warn/Error notice otherwise. statuses is
	// populated after the build's own progress region is torn down (below) and
	// captured here by reference; the engine only fires this reporter during a
	// mounted service's attach, which runs in the post-build goroutines — by then
	// statuses is set.
	var statuses map[string]*serviceStatus
	project := clientagent.NewProject(rt.client, conduit, clientagent.WithDeployReporter(func(n clientagent.DeployNotice) {
		if st := statuses[n.Service]; st != nil {
			st.diagnostic(n.Message, n.Terminal)
			return
		}
		if n.Terminal {
			d.Error("%s: %s", n.Service, n.Message)
		} else {
			d.Warn("%s: %s", n.Service, n.Message)
		}
	}))
	teardown := func() {
		project.Close()
	...
	// Provider services (compose-spec `provider:`) run an external plugin instead
	// of a container; prepare their readiness channels so a dependent can gate on
	// a provider's plugin `up` completing (see runProviderService / injectProviderEnv).
	rt.initProviderState(names)
	...
	// One evolving status line per service (see serviceStatus). Created here — after
	// buildServices tore down its own live region — so only one live program owns
	// the terminal at a time. A no-op overlay off a fancy TTY or under
	// --progress=stream, where the append-only notices below carry the output.
	statuses = make(map[string]*serviceStatus, len(names))
	for _, name := range names {
		statuses[name] = newServiceStatus(d, prog, name)
	}
	...
	for i, name := range names {
		...
		grp.Go(func() (rerr error) {
			// Finish this service's status line exactly once, from the goroutine's
			// outcome: a clean bring-up — or a Ctrl-C that suppressCascaded folds to
			// nil — finishes it done; a genuine failure finishes it failed.
			defer func() {
				if rerr == nil && gctx.Err() == nil {
					status.done()
				} else {
					status.fail()
				}
			...
			return suppressCascaded(gctx, func() error {
				...
				if err := waitForDependencies(gctx, rt, rt.client, name, selected, d, status, reconcilePollInterval, completionWaitTimeout); err != nil {
					return err
				}
				plan := rt.plans[name]
				// A provider service delegates to an external plugin: run its `up`,
				// record its env for dependents, and signal readiness — nothing is
				// built, deployed, exposed, or hook-run.
				if plan.Provider != nil {
					return rt.runProviderService(gctx, name, d)
				}
				spec := serviceSpec(rt, builtTags, name)
				if spec.Image == "" {
					return fmt.Errorf("service %q has neither image nor build", name)
				}

				// Services deploy fire-and-forget UNLESS they need a live session: a
				// client-local mount, a client-side-egress relay mode, or a brokered
				// credential all run off this client for the workload's lifetime, so the
				// deploy must hold a deploy-attach session (the session-holding branch
				// below), not a stateless POST. See needsHeldSession.
				if !needsHeldSession(spec) {
					if err := retryTransientDeploy(gctx, func() error {
						_, derr := rt.client.Deploy(gctx, spec)
						return derr
					}); err != nil {
					...
					if completion[name] {
						st = reportCompletion(gctx, rt.client, name, spec.Name, d, prog, status, reconcilePollInterval, completionWaitTimeout)
					} else {
						st = reportReconcile(gctx, rt.client, name, spec.Name, d, prog, status, reconcilePollInterval, reconcileWaitTimeout)
					}
					if gctx.Err() != nil {
						return nil
					}
				...
				// NB: the mounted (deploy-attach) path is deliberately NOT wrapped in
				// retryTransientDeploy. Retrying an attach re-runs project.Apply, which
				// mints a fresh mount session id and can re-apply the workload — itself a
				// "connection replaced under a fresh id" event that orphans the id baked
				// into the running pod (the stale-mount-session failure mode). The
				// server-side waitJobGone fix already prevents the still-terminating race
				// on this path, so a bounded client retry here would add churn, not safety.
				if err := expose(gctx, name, spec, false); err != nil {
					return fmt.Errorf("deploy %s: %w", name, err)
				}
				outcomes[i].mounted = true
				d.Event(svcEvent(name, "up", heldSessionReason(spec)))
				...
				out := hookLines.Writer(d.Out(), name+" | ")
				defer out.Close()
				if err := rt.runMountedServiceHooks(gctx, name, spec.Name, d, prog, status, out); err != nil {
					return fmt.Errorf("lifecycle %s: %w", name, err)
				}
				return nil
			}())
	...
	noForward := cfg.Mode == clientconduit.ModeNone
	if !holdForeground(c.Detach, len(names), mountedCount, noForward) {
		teardown()
		return nil
	}
	// A service declaring `x-cornus-ingress.tunnel` publishes the whole PROJECT
	// through one public URL: that is the point of an ingress tunnel, and a tunnel
	// per service would defeat it. The first declaring service (in dependency
	// order) supplies the host mode and host.
	if opt := projectIngressTunnelOpt(rt, names); opt != nil {
		stopTunnel := ingresspub.Publish(ctx, d, rt.client, rt.tunnelDefaults, opt, rt.projectName, "")
		prev := teardown
		teardown = func() {
			stopTunnel()
			prev()
	...
	// --watch: record the initial deployed mount-free spec keys so a reload only
	// re-deploys (recreates) a mount-free service whose spec actually changed, and
	// announce that the session is watching for edits.
	deployedKeys := map[string]string{}
	if c.Watch {
		for _, name := range mountFree {
			deployedKeys[name] = specKey(serviceSpec(rt, builtTags, name))
		}
		d.Info("watching %d compose/env file(s) for changes; edit and save to reload.", len(rt.watchFiles))
	}

	// Hold until Ctrl-C or an external `down`. With --watch, also reload and
	// re-reconcile whenever a loaded compose/env file is edited, then keep holding
	// the new service set. Each hold iteration owns a child context so a reload (or
	// exit) promptly stops that iteration's log stream, deployment watch, and file
	// watcher.
	for {
		...
		for _, name := range names {
			// Provider services have no server deployment to watch; a Status call on
			// their resource always errors, which watchGone reads as "not gone",
			// wedging the self-exit. Skip them.
			if rt.plans[name].Provider != nil {
				continue
			}
			resources = append(resources, rt.plans[name].Resource)
		}
		iterCtx, iterCancel := context.WithCancel(ctx)
		gone := watchGone(iterCtx, rt.client, resources, reconcilePollInterval)

		// Attach to the services' container logs and stream them, prefixed by service
		// name, until this hold iteration ends (Ctrl-C, external down, or a reload
		// that rebuilds the set). --no-attach opts out (still holds mounts/forwards).
		...
		if !c.NoAttach {
			...
			go func() {
				...
				if err := rt.streamLogs(iterCtx, names, opts, !c.NoLogPrefix, d.Out(), d.Err()); err != nil && iterCtx.Err() == nil {
					d.Error("streaming service logs: %v", err)
				}
			}()
		}

		// A file watcher over the current loaded set, feeding one notification per
		// debounced change. Rebuilt each iteration because a reload can change the set.
		changed := make(chan struct{}, 1)
		var watcher *filewatch.Watcher
		if c.Watch {
			watcher = filewatch.New(rt.watchFiles, 0, 0)
			go func() {
				if watcher.Wait(iterCtx) {
					select {
					case changed <- struct{}{}:
					case <-iterCtx.Done():
					}
		...
		// exit tears down the held sessions and removes the mount-free deployments
		// this foreground up created (Ctrl-C stops everything it brought up, matching
		// `docker compose up`; mounted services were already removed by project.Close).
		exit := func() error {
			endIter()
			teardown()
			removeDeployments(rt.client, rt.plans, mountFree, d)
		...
		case <-gone:
			...
			return exit()
		case <-changed:
			d.Info("compose files changed; reloading...")
			endIter()
			newNames, newMountFree, err := c.reloadAndReconcile(ctx, cli, rt, project, names, mountFree, deployedKeys)
			if err != nil {
				// Keep holding the previously-loaded set rather than exiting; the user
				// can fix the file and save again to retry. rt is left unchanged on error.
				d.Error("reload failed: %v", err)
			} else {
				names = newNames
				mountFree = newMountFree
			}
		}
	}
}

// specKey is a stable content hash of a resolved deploy spec, used by a --watch
// reload to detect whether a mount-free service's spec actually changed (and so
// needs a server-side recreate) rather than re-deploying every service on every
// edit.
func specKey(spec api.DeploySpec) string {
	b, _ := json.Marshal(spec)
	sum := sha256.Sum256(b)
	return hex.EncodeToString(sum[:])
}

// serviceSpec resolves a service's deploy spec for reconcile, substituting the
// freshly built image tag for a build: service and folding in any environment
// variables contributed by provider services this one depends_on (compose-spec
// `provider:`); the latter is a no-op unless a provider dependency has run.
func serviceSpec(rt *runtime, builtTags map[string]string, name string) api.DeploySpec {
	plan := rt.plans[name]
	spec := plan.Spec
	if plan.Build != nil {
		spec.Image = builtTags[name]
	}
	rt.injectProviderEnv(name, &spec)
	return spec
}

// reloadAndReconcile re-parses the (edited) compose/env files and drives the
// running project to the new desired state, reusing the live connection, conduit,
// and session engine. It re-plans into a temporary runtime and only commits the
// new project state onto rt after the reconcile succeeds, so a parse/build/deploy
// error leaves rt (and thus the ongoing hold) on the previously-loaded set. It
// returns the new selected service names and the new mount-free set, and updates
// deployedKeys in place. Mounted services are recreated on a spec change and torn
// down when removed (via the engine's ApplyExact); mount-free services are
// re-deployed only when their spec changed, and deleted server-side when removed.
func (c *UpCmd) reloadAndReconcile(ctx context.Context, cli *Cmd, rt *runtime, project *clientagent.Project, prevNames, prevMountFree []string, deployedKeys map[string]string) (names, mountFree []string, err error) {
	d := rt.driver()
	// Re-plan into a shallow copy so the live connection fields are shared but the
	// project-derived fields are only committed onto rt on full success.
	tmp := *rt
	if err := cli.loadProjectInto(&tmp); err != nil {
		return prevNames, prevMountFree, fmt.Errorf("reload: %w", err)
	}
	names, err = tmp.selectServices(c.Services)
	if err != nil {
		return prevNames, prevMountFree, fmt.Errorf("reload: %w", err)
	}
	if err := c.applyServiceOverrides(&tmp, names); err != nil {
		return prevNames, prevMountFree, err
	}
	cfg, err := c.conduitCfg(&tmp)
	if err != nil {
		return prevNames, prevMountFree, err
	}
	if err := c.resolveIngress(ctx, &tmp, &cfg); err != nil {
		return prevNames, prevMountFree, err
	}
	if err := materializeNativeTLS(&tmp, names, cfg); err != nil {
		return prevNames, prevMountFree, err
	}
	builtTags, err := tmp.buildServices(ctx, buildPlans(&tmp, names), c.SSH, buildOverrides{})
	if err != nil {
		return prevNames, prevMountFree, err
	}

	// Re-provision provider services first (in selection order) so an edited
	// provider config takes effect and the dependent deploy loop below sees fresh
	// env (see injectProviderEnv). This mirrors the detached path, whose reload
	// re-execs the CLI and thus re-runs provider `up` too; providers are required
	// to be idempotent.
	for _, name := range names {
		if tmp.plans[name].Provider != nil {
			if err := tmp.runProviderReload(ctx, name, d); err != nil {
				return prevNames, prevMountFree, err
			}
		}
	}

	// Build the desired engine set and re-deploy any mount-free service whose spec
	// changed (or is new). Mounted/relay services are handled by the engine below.
	desired := make([]clientagent.Service, 0, len(names))
	newKeys := map[string]string{}
	for _, name := range names {
		// Provider services were re-provisioned above; they are not engine-managed.
		if tmp.plans[name].Provider != nil {
			continue
		}
		spec := serviceSpec(&tmp, builtTags, name)
		if spec.Image == "" {
			return prevNames, prevMountFree, fmt.Errorf("service %q has neither image nor build", name)
		}
		if !needsHeldSession(spec) {
			mountFree = append(mountFree, name)
			key := specKey(spec)
			newKeys[name] = key
			if deployedKeys[name] != key {
				if err := retryTransientDeploy(ctx, func() error {
					_, derr := rt.client.Deploy(ctx, spec)
					return derr
				}); err != nil {
					return prevNames, prevMountFree, fmt.Errorf("deploy %s: %w", name, err)
				}
			}
			desired = append(desired, clientagent.Service{Name: name, Spec: spec, ForwardPorts: len(spec.Ports) > 0, ForwardOnly: true})
			continue
		}
		desired = append(desired, clientagent.Service{Name: name, Spec: spec, ForwardPorts: len(spec.Ports) > 0})
	}

	// Reconcile the engine to exactly the desired set: recreate changed mounted
	// services, expose new ones, and tear down (mount + exposure) any removed.
	statuses, err := project.ApplyExact(ctx, desired)
	if err != nil {
		return prevNames, prevMountFree, fmt.Errorf("reload reconcile: %w", err)
	}

	// Delete server-side any mount-free deployment removed from the file — the
	// engine only withdrew its exposure; the fire-and-forget workload is ours to
	// remove. Uses the pre-reload rt.plans for the resource names. Mounted removals
	// were already torn down by ApplyExact.
	nowSet := make(map[string]bool, len(names))
	for _, n := range names {
		nowSet[n] = true
	}
	for _, n := range prevMountFree {
		if nowSet[n] {
			continue
		}
		if err := rt.client.Delete(ctx, rt.plans[n].Resource); err != nil {
			d.Warn("removing %s: %v", n, err)
		}
	}

	// Success: commit the new project state onto rt.
	rt.project = tmp.project
	rt.projectName = tmp.projectName
	rt.plans = tmp.plans
	rt.order = tmp.order
	rt.baseDir = tmp.baseDir
	rt.watchFiles = tmp.watchFiles
	rt.hooks = tmp.hooks
	rt.initialize = tmp.initialize
	for k := range deployedKeys {
		delete(deployedKeys, k)
	}
	for k, v := range newKeys {
		deployedKeys[k] = v
	}

	// Summarize the reconcile.
	prevSet := make(map[string]bool, len(prevNames))
	for _, n := range prevNames {
		prevSet[n] = true
	}
	added, removed, recreated := 0, 0, 0
	for _, n := range names {
		if !prevSet[n] {
			added++
		}
		if statuses[n] == clientagent.StatusRecreated {
			recreated++
		}
	}
	for _, n := range prevNames {
	...
	d.Info("reloaded: %d service(s) desired (%d added, %d recreated, %d removed).", len(names), added, recreated, removed)
	return names, mountFree, nil
}
...
// deploymentLister lists all deployments on the server — the read side of orphan
// detection. A narrow interface over *client.Client so the logic is unit-testable
// with a scripted fake.
type deploymentLister interface {
	List(ctx context.Context) ([]api.DeployStatus, error)
}

// orphanClient is what removeOrphans needs: list deployments, then delete each
// orphan and watch it drain (reportTeardown), the way `down` tears services down.
type orphanClient interface {
	deploymentLister
	teardownClient
}

var _ orphanClient = (*client.Client)(nil)

// findOrphans returns the resource names of deployments that belong to this
// compose project (Origin.Project == projectName) but whose service is no longer
// defined in the Compose file — workloads left behind after a service was removed
// or renamed, the way `docker compose` detects orphans. known is the set of
// resource names of every service currently in the project (ALL profiles, so a
// service disabled by an inactive profile is not mistaken for an orphan). The
// result is sorted for deterministic reporting. A deployment with no recorded
// origin (Origin nil — e.g. a raw `cornus deploy`, or one deployed before lineage
// was stamped) or one owned by another project is never claimed: cornus only
// removes workloads it can prove belong to this project.
func findOrphans(list []api.DeployStatus, projectName string, known map[string]struct{}) []string {
	var orphans []string
	for _, st := range list {
		if st.Origin == nil || st.Origin.Project != projectName {
			continue
		}
		if _, ok := known[st.Name]; ok {
			continue
		}
		orphans = append(orphans, st.Name)
	}
	sort.Strings(orphans)
	return orphans
}

// removeOrphans deletes the project's orphaned workloads (findOrphans), the way
// `docker compose {up,down} --remove-orphans` does: it lists deployments, filters
// to this project's orphans, deletes each, and — when wait — watches the cluster-
// side teardown drain (reportTeardown) before returning, exactly like `down`. A
// list failure is returned so the caller decides whether it is fatal; a per-orphan
// delete failure is warned and the remaining orphans still proceed. Honors ctx
// cancellation (Ctrl-C) mid-wait. Prints nothing when there are no orphans.
func removeOrphans(ctx context.Context, cl orphanClient, projectName string, known map[string]struct{}, d *cliout.Driver, wait bool) error {
	list, err := cl.List(ctx)
	if err != nil {
		return fmt.Errorf("listing deployments to find orphans: %w", err)
	}
	for _, name := range findOrphans(list, projectName, known) {
		if err := cl.Delete(ctx, name); err != nil {
			d.Warn("removing orphan %s: %v", name, err)
			continue
		}
		if !wait {
			d.Event(svcEvent(name, "orphan removed", ""))
			continue
		}
		// Watch the teardown drain to the final "removed", like `down`, using the
		// resource name as the label (an orphan has no compose service name).
		reportTeardown(ctx, cl, name, name, d, reconcilePollInterval, teardownWaitTimeout)
		if ctx.Err() != nil {
...
// warnOrphans lists deployments and warns about the project's orphaned workloads
// without removing them — the advisory `docker compose up` prints when it finds
// orphans, pointing the user at --remove-orphans. Best-effort: a list failure is
// swallowed so it never blocks `up`. A no-op when there are no orphans.
func warnOrphans(ctx context.Context, cl deploymentLister, projectName string, known map[string]struct{}, d *cliout.Driver) {
	list, err := cl.List(ctx)
	if err != nil {
		return
	}
	orphans := findOrphans(list, projectName, known)
	if len(orphans) == 0 {
		return
	}
	d.Warn("found orphan workloads not defined in the Compose file: %s. If you removed or renamed these services, run `down --remove-orphans` (or `up --remove-orphans`) to remove them.", strings.Join(orphans, ", "))
}
...
// needsHeldSession reports whether a service can only be realized on a live
// deploy-attach session, so it must NOT be deployed with a stateless POST:
//
//   - client-local bind mounts, served over 9P for the workload's lifetime;
//   - a client-side-egress relay mode (proxy/transparent), which routes the
//     workload's traffic back through this client;
//   - brokered credentials, minted on this client on demand (an env-kind
//     delivery is minted once at deploy time, but still over the session).
//
// Every caller that classifies a service as "deploy it and forget it" uses this
// one predicate: a spec that needs a session but takes the stateless path
// deploys green and then never gets its mounts, its egress, or its credentials.
func needsHeldSession(spec api.DeploySpec) bool {
	return len(spec.Mounts) > 0 || spec.Egress.NeedsRelay() || spec.Credentials.NeedsSession()
}

// heldSessionReason renders, for the `up` event line, WHY a service is holding a
// session — the same three reasons needsHeldSession tests, in the same order. It
// exists because one held service can be held for a reason that has nothing to
// do with 9P, and reporting "streaming over 9P" for a credentials-only service
// sends anyone debugging it to the wrong subsystem.
func heldSessionReason(spec api.DeploySpec) string {
	var why []string
	if len(spec.Mounts) > 0 {
		why = append(why, "mounted; streaming over 9P")
	}
	if spec.Egress.NeedsRelay() {
		why = append(why, "relaying client-side egress")
	}
	if spec.Credentials.NeedsSession() {
		why = append(why, "brokering credentials")
	}
...
func (c *UpCmd) upDetached(cli *Cmd, rt *runtime, cfg clientconduit.Config, names []string) error {
	...
	// Provider services (compose-spec `provider:`) run an external plugin; prepare
	// their readiness channels (see runForeground's identical step).
	rt.initProviderState(names)
	...
	for i, name := range names {
		...
		grp.Go(func() error {
			return suppressCascaded(gctx, func() error {
				...
				// below; the wait still runs here so it covers both branches. The
				// detached path keeps its standalone per-service tasks (nil status), so
				// --progress still collapses/streams them but they are not folded into a
				// held foreground line.
				if err := waitForDependencies(gctx, rt, rt.client, name, selected, d, nil, reconcilePollInterval, completionWaitTimeout); err != nil {
					return err
				}
				plan := rt.plans[name]
				// A provider service runs its external plugin `up` here (recording env
				// for dependents) instead of being deployed or handed to the agent.
				if plan.Provider != nil {
					return rt.runProviderService(gctx, name, d)
				}
				spec := serviceSpec(rt, builtTags, name)
				...
				// this client, and a brokered credential is minted here on demand, so — like
				// a client-local mount — either needs the agent to hold a live deploy-attach
				// session (the daemonService below), not a stateless POST. See needsHeldSession.
				if !needsHeldSession(spec) {
					if err := retryTransientDeploy(gctx, func() error {
						_, derr := rt.client.Deploy(gctx, spec)
						return derr
					...
					var st api.DeployStatus
					if completion[name] {
						st = reportCompletion(gctx, rt.client, name, spec.Name, d, prog, nil, reconcilePollInterval, completionWaitTimeout)
					} else {
						st = reportReconcile(gctx, rt.client, name, spec.Name, d, prog, nil, reconcilePollInterval, reconcileWaitTimeout)
					}
					if gctx.Err() != nil {
						return nil
					}
	...
	// has mounts or a per-service forward; --watch always needs it too, so the agent
	// holds the watcher (and re-reconciles on edit) even for an all-fire-and-forget
	// project with nothing else to hand off.
	if len(daemonSvcs) == 0 && !socks5Mode && !c.Watch {
		return nil
	}

	// --watch: capture how the agent should re-invoke this CLI to reload the project
	// after an edit — the same argv in the same cwd/env, so the re-plan sees the
	// developer's original working directory and environment (see ReloadSpec).
	watchFiles, reload, err := c.reloadDescriptor(rt)
	if err != nil {
		return err
	}
	...
	if len(daemonSvcs) > 0 {
		...
		emitAgentWarnings(d, resp.Warnings)
		...
		for _, s := range hookSvcs {
			...
			hgrp.Go(func() error {
				return suppressCascaded(hgctx, func() error {
					...
					defer out.Close()
					if err := rt.runMountedServiceHooks(hgctx, s.Name, s.Spec.Name, d, prog, nil, out); err != nil {
	...
	} else if socks5Mode || c.Watch {
		// A socks5 project with no mount/forward services still needs the agent up to
		// host the proxy; a watched project with no held services still needs it to
		// hold the watcher. Register a (possibly empty) project so the agent holds it.
		resp, err := clientagent.Send(socket, clientagent.Request{
			Action: "up", Project: rt.projectName, Conn: rt.connSpec, Conduit: clientagent.ToWireConduit(cfg),
			Watch: c.Watch, WatchFiles: watchFiles, Reload: reload,
		})
		if err != nil {
...
func emitAgentWarnings(d *cliout.Driver, warnings []string) {
...
type DownCmd struct {
	...
	// RemoveOrphans also removes workloads that belong to this project but whose
	// service is gone from the Compose file (removed or renamed), matching
	// `docker compose down --remove-orphans`.
	RemoveOrphans bool `kong:"name='remove-orphans',help='Also remove workloads for services no longer defined in the Compose file (left behind after a service was removed or renamed), like docker compose down --remove-orphans.'"`
	// RMI and Timeout are accepted for docker compose compatibility and warn
	// rather than act; see flagcompat.go for why neither can be honored.
	RMI     string `kong:"name='rmi',help='Accepted for docker compose compatibility and NOT honored (local|all): cornus has no image-removal API. Warns and continues with the teardown.'"`
	Timeout int    `kong:"name='timeout',short='t',default='-1',help='Accepted for docker compose compatibility and NOT honored: the cornus deploy API carries no per-call shutdown timeout. Set stop_grace_period on the service instead.'"`
}
...
func (c *DownCmd) Run(cli *Cmd, r *clientconn.Resolver, d *cliout.Driver) error {
	// A bad --rmi value is rejected before the project is even loaded: it is a
	// typo in the command line, not a fact about the deployment.
	switch c.RMI {
	case "", "local", "all":
	default:
		return fmt.Errorf("--rmi: unsupported value %q (want local or all)", c.RMI)
	}
	rt, err := cli.load(r, d)
	if err != nil {
		return err
	...
	// Compatibility flags that cannot be honored: warn up front, then do the
	// teardown the user actually asked for (see flagcompat.go).
	warnShutdownTimeout(d, c.Timeout, "down")
	warnRemoveImages(d, c.RMI)
	...
	for i := len(names) - 1; i >= 0; i-- {
		name := names[i]
		// A provider service is torn down by its external plugin's `down`, not a
		// server delete (it has no server deployment).
		if plan := rt.plans[name]; plan.Provider != nil {
			if _, err := rt.providerRunner.run(ctx, *plan.Provider, rt.projectName, name, providerDown, d); err != nil {
	...
	// --remove-orphans also tears down workloads that belong to this project but
	// whose service is gone from the Compose file. Done before the volume sweep so
	// an orphan holding a named volume is gone by the time --volumes runs.
	if c.RemoveOrphans {
		if err := removeOrphans(ctx, rt.client, rt.projectName, rt.knownResources(), d, c.Wait); err != nil {
			return err
		}
		if ctx.Err() != nil {
			return ctx.Err()
...
//
// The default table is SERVICE | NAME | IMAGE | STATUS, deliberately NOT docker
// compose's NAME IMAGE COMMAND SERVICE CREATED STATUS PORTS. Three of docker's
// columns describe a local container and have no cornus equivalent to print:
// api.DeployStatus carries no command, no creation time and no port bindings,
// because a deployment is a spec applied to a backend that may not even have the
// concept (a kubernetes Deployment's "ports" are a Service's, and its "created"
// is a ReplicaSet's). Fabricating them would be worse than omitting them. What
// cornus adds instead is SERVICE first — the compose identity, which is what a
// user actually looks up — with NAME as the backend resource it maps to.
//
// Scripting is served by the formats that promise stability rather than by the
// column set: --format json (every field, machine-readable), --quiet (resource
// ids) and --services (service names).
type PsCmd struct {
	...
	Format   string `kong:"name='format',default='table',help='Output format: table (default; SERVICE/NAME/IMAGE/STATUS — cornus deployment columns, not docker compose ones) or json.'"`
}
...
func psRows(order []string, plans map[string]compose.ServicePlan, byResource map[string]api.DeployStatus) []psRow {
	...
	for _, name := range order {
		plan := plans[name]
		// A provider service is managed by its external plugin, not the server, so
		// it has no deployment to report; show its provider type in the image column
		// and a "provider" status rather than a misleading "not created".
		if plan.Provider != nil {
			rows = append(rows, psRow{Service: name, Name: plan.Resource, Image: "provider:" + plan.Provider.Type, Status: "provider"})
			continue
		}
...
type BuildCmd struct {
	...
	Pull     bool     `kong:"name='pull',help='Always attempt to pull a newer version of each base image (build.pull).'"`
	Push     bool     `kong:"name='push',help='Push the built images. Already the default — every cornus compose build pushes to the cornus registry, which is where the deploy pulls from — so this only prints where they went.'"`
	Quiet    bool     `kong:"name='quiet',short='q',help='Do not print build progress. Failures are still reported.'"`
}
...
func (c *BuildCmd) Run(cli *Cmd, r *clientconn.Resolver, d *cliout.Driver) error {
	...
	ov := buildOverrides{noCache: c.NoCache, args: args, pull: c.Pull, quiet: c.Quiet}
	plans := buildPlans(rt, names)
	if len(plans) > 0 {
		// --push asks for what every compose build already does; say where the
		// images actually went instead of swallowing the flag (see flagcompat.go).
		// After the plan check so a project with nothing to build makes no
		// registry-resolving call.
		if c.Push && !c.Quiet {
			noteAlwaysPush(d, rt.registryHostFor(ctx))
		}
		if _, err := rt.buildServices(ctx, plans, c.SSH, ov); err != nil {
			return err
		}
	}
	if len(plans) == 0 && !c.Quiet {
...
func (c *RestartCmd) Run(cli *Cmd, r *clientconn.Resolver, d *cliout.Driver) error {
	warnShutdownTimeout(d, c.Timeout, "restart")
...
func (c *StopCmd) Run(cli *Cmd, r *clientconn.Resolver, d *cliout.Driver) error {
	warnShutdownTimeout(d, c.Timeout, "stop")
...
func runAction(cli *Cmd, r *clientconn.Resolver, d *cliout.Driver, requested []string, action string) error {
	...
	for _, name := range names {
		// A provider service is driven by its external plugin, not a server
		// deployment: map the lifecycle action to the plugin's own command(s).
		if plan := rt.plans[name]; plan.Provider != nil {
			for _, pc := range providerLifecycle[action] {
				if _, err := rt.providerRunner.run(ctx, *plan.Provider, rt.projectName, name, pc, d); err != nil {
...
// projectIngressTunnelOpt returns the ingress-tunnel opt-in for the project: the
// first service (in dependency order) that declares one. An ingress tunnel is
// project-scoped by nature — every service's declared host sits behind the one
// URL — so several services declaring it still yield a single tunnel.
func projectIngressTunnelOpt(rt *runtime, names []string) *api.IngressTunnelOpt {
	for _, name := range names {
		plan := rt.plans[name]
		if plan.Provider != nil {
			continue
		}
		in := plan.Spec.Ingress
		if in != nil && in.Tunnel != nil && in.Tunnel.Enabled {
			return in.Tunnel
		}
	}
	return nil
