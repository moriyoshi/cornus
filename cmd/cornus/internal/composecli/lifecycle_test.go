...
func (f *fakeRunner) ExecStart(ctx context.Context, id string, cfg api.ExecStartConfig) (net.Conn, error) {
	// The exec is created with no TTY, so a real backend's stream is
	// stdcopy-MULTIPLEXED — the fake must be too, or it models a wire format no
	// backend produces and lets a demux bug through. (It did: runExec copied the
	// stream raw for as long as this fake sent unframed bytes.)
	return &fakeConn{Reader: bytes.NewReader(stdcopyFrame(1, "out:"+id+"\n"))}, nil
}

// stdcopyFrame builds one stdcopy frame: stream id, 3 reserved bytes, a
...
func TestRunContainerHooksOrderAndConfig(t *testing.T) {
	...
	// Assert the EXACT bytes, not a substring.
	//
	// A substring check is what let a demux bug live here: the exec is created
	// without a TTY, so a real backend's stream is stdcopy-multiplexed, and this
	// path used to copy it through raw. The 8-byte frame headers do not stop
	// "out:e1" from appearing, so `strings.Contains` passed while every hook log
	// carried "\x01\x00\x00\x00\x00\x00\x00\x09" before each line.
	if got, want := out.String(), "out:e1\nout:e2\nout:e3\n"; got != want {
		t.Errorf("hook output = %q, want %q\n"+
			"stray NULs mean the stdcopy frame headers were not stripped", got, want)
	}
	if bytes.ContainsRune(out.Bytes(), 0) {
		t.Errorf("hook output contains NUL bytes (%q); stdcopy frame headers leaked through", out.String())
	}
}
