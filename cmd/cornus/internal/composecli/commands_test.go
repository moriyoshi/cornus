...
func TestNeedsBackgroundAgent(t *testing.T) {
	...
	creds := api.DeploySpec{Credentials: &api.CredentialSpec{
		Sources: []api.CredentialSource{{Name: "claude", Backend: "claude-code"}},
	}}
	emptyCreds := api.DeploySpec{Credentials: &api.CredentialSpec{}}
	...
	cases := []struct {
		...
		// Same shape as the relay-only bug above: a credential-only service has no
		// mounts and no ports, but its sources are minted on THIS client, so `up -d`
		// without the agent would deploy a workload whose credential fetches then
		// have nobody to answer them.
		{"credentials-only, no conduit", []api.DeploySpec{creds}, clientconduit.ModeNone, true},
		{"credentials-only, port-forward", []api.DeploySpec{creds}, clientconduit.ModePortForward, true},
		{"an empty credential spec brokers nothing", []api.DeploySpec{emptyCreds}, clientconduit.ModeNone, false},
	}
...
// TestNeedsHeldSession pins the predicate that decides, at three separate call
// sites, whether a service takes the stateless-POST path. A spec that needs a
// session but is classified session-free deploys green and then silently never
// gets its mounts, its egress, or its credentials — so each reason is asserted
// on its own, not just in combination.
func TestNeedsHeldSession(t *testing.T) {
	cases := []struct {
		name string
		spec api.DeploySpec
		want bool
	}{
		{"plain", api.DeploySpec{}, false},
		{"client mount", api.DeploySpec{Mounts: []api.Mount{{}}}, true},
		{"relay egress", api.DeploySpec{Egress: &api.EgressSpec{Mode: "proxy"}}, true},
		{"env egress", api.DeploySpec{Egress: &api.EgressSpec{Mode: "env"}}, false},
		{"brokered credential", api.DeploySpec{Credentials: &api.CredentialSpec{
			Sources: []api.CredentialSource{{Name: "db", Backend: "static"}},
		}}, true},
		// An env-kind delivery is fetched once at deploy time, but still over the
		// session — so it needs one just the same.
		{"env-kind credential delivery", api.DeploySpec{Credentials: &api.CredentialSpec{
			Sources: []api.CredentialSource{{Name: "db", Backend: "static", Deliveries: []api.CredentialDelivery{{Kind: "env", EnvVar: "K"}}}},
		}}, true},
		{"empty credential spec", api.DeploySpec{Credentials: &api.CredentialSpec{}}, false},
		{"ports alone are not a session", api.DeploySpec{Ports: []api.PortMapping{{}}}, false},
	}
	for _, tc := range cases {
		if got := needsHeldSession(tc.spec); got != tc.want {
			t.Errorf("%s: needsHeldSession = %v, want %v", tc.name, got, tc.want)
		}
	}
}

// TestHeldSessionReason asserts the `up` line names the reason a service is
// held. Reporting "streaming over 9P" for a service held only for credentials
// (or only for egress) points debugging at the wrong subsystem.
func TestHeldSessionReason(t *testing.T) {
	creds := &api.CredentialSpec{Sources: []api.CredentialSource{{Name: "db", Backend: "static"}}}
	cases := []struct {
		name string
		spec api.DeploySpec
		want string
	}{
		{"mounts", api.DeploySpec{Mounts: []api.Mount{{}}}, "mounted; streaming over 9P"},
		{"egress relay", api.DeploySpec{Egress: &api.EgressSpec{Mode: "proxy"}}, "relaying client-side egress"},
		{"credentials", api.DeploySpec{Credentials: creds}, "brokering credentials"},
		{"mounts and credentials", api.DeploySpec{Mounts: []api.Mount{{}}, Credentials: creds},
			"mounted; streaming over 9P; brokering credentials"},
	}
	for _, tc := range cases {
		if got := heldSessionReason(tc.spec); got != tc.want {
			t.Errorf("%s: heldSessionReason = %q, want %q", tc.name, got, tc.want)
		}
	}
}

// TestUpConduitCfgAllowNonLoopback pins the opt-in end of `compose up --conduit
// socks5://<non-loopback>`: refused by default, accepted with
// --allow-non-loopback, and — critically — refused/accepted HERE, before Run
// branches to the detached agent, since `up -d` starts the conduit out of process
// where the error would otherwise surface only after the agent is up.
func TestUpConduitCfgAllowNonLoopback(t *testing.T) {
	t.Setenv("CORNUS_CONDUIT", "")
	rt := &runtime{conduitConfig: (&clientconn.Conn{}).ConduitConfig}

	if _, err := (&UpCmd{Conduit: "socks5://0.0.0.0:10080"}).conduitCfg(rt); err == nil {
		t.Error("conduitCfg = nil error, want the non-loopback refusal without --allow-non-loopback")
	} else if !strings.Contains(err.Error(), "--allow-non-loopback") {
		t.Errorf("error = %v, want it to point at the opt-in", err)
	}

	cfg, err := (&UpCmd{Conduit: "socks5://0.0.0.0:10080", AllowNonLoopback: true}).conduitCfg(rt)
	if err != nil {
		t.Fatalf("conduitCfg with --allow-non-loopback: %v", err)
	}
	if !cfg.Socks5AllowNonLoopback {
		t.Error("Socks5AllowNonLoopback = false, want the flag carried into the conduit config")
	}
	if cfg.Socks5Listen != "0.0.0.0:10080" {
		t.Errorf("Socks5Listen = %q, want the requested address", cfg.Socks5Listen)
	}

	// --no-forward-ports wins over the selector, so the loopback policy never fires
	// for a conduit that binds nothing.
	if _, err := (&UpCmd{Conduit: "socks5://0.0.0.0:10080", NoForwardPorts: true}).conduitCfg(rt); err != nil {
		t.Errorf("conduitCfg with --no-forward-ports = %v, want nil", err)
	}
}
