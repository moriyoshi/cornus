...
// --build-arg / --pull / --quiet) into resolveBuildRequest and buildServices.
// The zero value — what `up --build` passes — changes nothing.
type buildOverrides struct {
	noCache bool
	args    map[string]string // merged over the compose build.args
	// pull is OR'd with each service's build.pull, like noCache with
	// build.no_cache: the flag can turn base-image pulling on for a build that
	// did not ask for it, never off for one that did.
	pull bool
	// quiet suppresses build progress rendering (compose build --quiet). It is a
	// property of the invocation, not of the build request, so it never reaches
	// the wire and never changes a build's identity (see buildRequestKey).
	quiet bool
}
...
func (r *runtime) resolveBuildRequest(ctx context.Context, plan compose.ServicePlan, cliSSH []string, ov buildOverrides) (client.BuildRequest, error) {
	...
	return client.BuildRequest{
		...
		// --pull (ov) OR build.pull always re-resolves the base image.
		NoCache:          ov.noCache || bp.NoCache,
		Labels:           bp.Labels,
		Pull:             ov.pull || bp.Pull,
...
func (r *runtime) buildServices(ctx context.Context, plans []compose.ServicePlan, cliSSH []string, ov buildOverrides) (map[string]string, error) {
	...
	tags, err := runBuildGroups(ctx, r.client, groups, reportDriver(r.driver(), ov.quiet))
	if err != nil {
		return nil, err
	}
	return tags, nil
...
// reportDriver returns the driver the build progress renders through: d itself,
// or — for `compose build --quiet` — one whose streams are discarded.
//
// Suppressing at the driver rather than teaching every reporter path a quiet
// mode is deliberate: progress RENDERING is the only thing --quiet suppresses,
// and a build failure still travels back as an error (carrying the build's own
// diagnostics) through runBuildGroups' return value, which the command layer
// prints. So a quiet build that fails is still diagnosable, which is what
// separates "don't print progress" from "don't tell me anything".
func reportDriver(d *cliout.Driver, quiet bool) *cliout.Driver {
	if !quiet {
		return d
	}
	return cliout.New(cliout.Options{Stdout: io.Discard, Stderr: io.Discard, Output: "plain"})
}
