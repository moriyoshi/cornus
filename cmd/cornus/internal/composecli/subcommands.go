...
// warnServerVersionSkew warns when the server's advertised build version differs
// from this client's. A client and server built from different generations can
// silently disagree — e.g. a client that predates a feature never sends the flag
// the server now expects, so the request takes a wrong path and fails with a
// confusing deep error rather than "your client is out of date". This turns the
// skew into one clear line at the start of a deploy. Best-effort: info is the
// runtime's memoized GET /.cornus/v1/info, which is the ZERO ServerInfo when the
// fetch failed (the deploy path reports real connectivity problems), and the check
// is skipped unless BOTH versions are known and genuinely differ — two identical
// builds (including two "dev" builds from the same tree) cannot be told apart, and
// are left alone rather than warned about spuriously.
func warnServerVersionSkew(info api.ServerInfo, d *cliout.Driver) {
	if d == nil || Version == "" || !versionSkew(Version, info.Version) {
		return
	}
	d.Warn("client version %s differs from server version %s: a client/server skew can behave inconsistently (a feature one side supports the other may not); rebuild or upgrade so both match", Version, info.Version)
}

// versionSkew reports whether a client/server version pair should be flagged: both
// known (non-empty) and genuinely different. Two identical builds — including two
// "dev" builds from the same tree — are equal and NOT flagged (they cannot be told
// apart), so this only ever fires on a real, visible mismatch.
func versionSkew(clientVer, serverVer string) bool {
	return clientVer != "" && serverVer != "" && clientVer != serverVer
}
