...
// warnServerVersionSkew fetches the server's advertised build version and warns when
// it differs from this client's. A client and server built from different
// generations can silently disagree — e.g. a client that predates a feature never
// sends the flag the server now expects, so the request takes a wrong path and fails
// with a confusing deep error rather than "your client is out of date". This turns
// the skew into one clear line at the start of a deploy. Best-effort: an Info error
// is ignored (the deploy path reports real connectivity problems), and the check is
// skipped unless BOTH versions are known and genuinely differ — two identical builds
// (including two "dev" builds from the same tree) cannot be told apart, and are left
// alone rather than warned about spuriously.
func warnServerVersionSkew(ctx context.Context, cl *client.Client, d *cliout.Driver) {
	if cl == nil || d == nil || Version == "" {
		return
	}
	info, err := cl.Info(ctx)
	if err != nil || !versionSkew(Version, info.Version) {
		return
	}
