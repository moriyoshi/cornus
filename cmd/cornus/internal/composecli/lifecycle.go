...
func runExec(ctx context.Context, cl execRunner, resource, user, workDir string, argv []string, out io.Writer) error {
	...
	// The exec was created with no TTY (ExecStartConfig{}), so its output is
	// stdcopy-multiplexed and must be demultiplexed rather than copied through —
	// a raw copy writes the 8-byte frame headers into the hook log. Both streams
	// go to `out` deliberately: a lifecycle hook's stderr is part of its log, and
	// merging them here preserves that while still stripping the framing.
	_, _ = stdcopy.StdCopy(out, out, conn)
	conn.Close()
