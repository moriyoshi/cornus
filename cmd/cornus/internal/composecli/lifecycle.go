...
func waitRunningThenHooks(ctx context.Context, poll statusPoller, cl execRunner, service, deployName, resource string, h *devcontainer.Hooks, d *cliout.Driver, prog *cliout.Progress, out io.Writer, pollInterval, timeout time.Duration) error {
	if h == nil {
		return nil
	}
	reportReconcile(ctx, poll, service, deployName, d, prog, pollInterval, timeout)
...
func (r *runtime) runMountedServiceHooks(ctx context.Context, name, deployName string, d *cliout.Driver, prog *cliout.Progress, out io.Writer) error {
	return waitRunningThenHooks(ctx, r.client, r.client, name, deployName, r.plans[name].Resource, r.hooks[name], d, prog, out, reconcilePollInterval, reconcileWaitTimeout)
}
