...
type runtime struct {
	...
	applyIngress func(ctx context.Context, cfg *clientconduit.Config, cliMode string)
...
func (c *Cmd) load(r *clientconn.Resolver, d *cliout.Driver) (*runtime, error) {
	full, baseDir, hooks, initialize, err := c.resolveProject()
	if err != nil {
		return nil, err
	}
	...
	return &runtime{
		...
		cleanup:          cn.Cleanup,
		kubeLogs:         kubeLogs,
		forwardDialer:    cn.Dialer(viaServer),
		conduitConfig:    cn.ConduitConfig,
		applyIngress:     cn.ApplyIngress,
		connSpec: clientagent.ConnSpec{
			ConfigFile: absCfg,
			Context:    r.Context,
...
func (r *runtime) resolveIngress(ctx context.Context, cfg *clientconduit.Config, cliMode string) {
	if r.applyIngress != nil {
		r.applyIngress(ctx, cfg, cliMode)
	}
}
...
func (c *Cmd) resolveProject() (proj *compose.Project, baseDir string, hooks map[string]*devcontainer.Hooks, initialize *devcontainer.LifecycleCommand, err error) {
	switch {
	case c.Devcontainer != "":
		return loadDevcontainer(c.Devcontainer)
	case len(c.Files) == 1 && isDevcontainerFile(c.Files[0]):
		return loadDevcontainer(c.Files[0])
	case len(c.Files) > 0:
		proj, err = compose.LoadWithOptions(c.loadOptions(), c.Files...)
		return proj, filepath.Dir(c.Files[0]), nil, nil, err
	}
	if found := discoverComposeFile(); found != "" {
		proj, err = compose.LoadWithOptions(c.loadOptions(), found)
		return proj, filepath.Dir(found), nil, nil, err
	}
	if _, e := devcontainerFor("."); e == nil {
		return loadDevcontainer(".")
	}
	return nil, "", nil, nil, fmt.Errorf("no compose file or devcontainer found (looked for compose.yaml, compose.yml, docker-compose.yaml, docker-compose.yml, .devcontainer/devcontainer.json, .devcontainer.json)")
}
