...
type Cmd struct {
	...
	Host         string   `kong:"name='host',short='H',aliases='server',env='CORNUS_HOST',help='cornus server endpoint (alias: --server). Falls back to the selected connection profile, then http://localhost:5000.'"`
...
type runtime struct {
	...
	// tunnelDefaults are the profile's public-tunnel defaults (credential file
	// path, ingress host mode), used when a service declares `ingress.tunnel`.
	// nil when the profile sets none.
	tunnelDefaults *clientconfig.Tunnel
	...
	// serverInfoMemo caches GET /.cornus/v1/info for this command run so the
	// several up-time checks that need it (the client/server version skew warning,
	// the deploy backend's capabilities) cost one request between them. Nil until
	// the first serverInfo call; a FAILED fetch memoizes the zero ServerInfo, so a
	// server that cannot answer is asked once and every field reads as "unknown".
	// Resolved before the per-service goroutines start, so single-goroutine command
	// execution guards it as it does registryHostMemo.
	serverInfoMemo *api.ServerInfo
	...
	// for --no-forward-ports). A malformed selector (bad socks5:// URL) is reported
	// as an error rather than deferred to clientconduit.Start. Captures the
	// connection profile. Never nil.
	conduitConfig func(cliMode string) (clientconduit.Config, error)
...
// serverInfo fetches the server's self-description (GET /.cornus/v1/info) once
// per runtime and memoizes it, so the checks that need it share one request.
//
// Best-effort by contract: a missing client or a failed fetch memoizes and
// returns the ZERO ServerInfo rather than an error. Every caller therefore has to
// treat an empty field as "unknown" and fall back to the behavior it had before
// the field existed — connectivity problems are reported by the deploy path
// itself, not by a capability probe.
func (r *runtime) serverInfo(ctx context.Context) api.ServerInfo {
	if r.serverInfoMemo != nil {
		return *r.serverInfoMemo
	}
	var info api.ServerInfo
	if r.client != nil {
		if got, err := r.client.Info(ctx); err == nil {
			info = got
		}
	}
	r.serverInfoMemo = &info
	return info
}

// backendInfo reports the deploy backend's advertised self-description, or nil
// when the server did not report one (an older server, or a backend it could not
// construct). Callers must treat nil as "unknown" and assume the backend is
// capable, so a capability that cannot be learned never fails a command.
func (r *runtime) backendInfo() *api.BackendInfo {
	if r.serverInfoMemo == nil {
		return nil
	}
	return r.serverInfoMemo.Backend
}
...
func (c *Cmd) resolveProject() (proj *compose.Project, baseDir string, watchFiles []string, hooks map[string]*devcontainer.Hooks, initialize *devcontainer.LifecycleCommand, err error) {
	...
	case len(c.Files) > 0:
		...
		// A -f value that is not a path at all is usually `logs -f` meant as
		// --follow; say so rather than reporting a missing file (see flagcompat.go).
		return proj, filepath.Dir(c.Files[0]), files, nil, nil, explainFileFlagError(c.Files, err)
	}
