...
//     "healthy". A backend that never reports health (containerd, bare, incus)
//     can never satisfy this, which is why the condition is rejected UP FRONT
//     rather than polled here — see validateDependencyConditions.
...
// backendReportsHealth reports whether the server's deploy backend runs a health
// probe engine and fills api.InstanceStatus.Health — the deploy.HealthReporter
// capability, advertised as api.BackendInfo.ReportsHealth by GET /.cornus/v1/info.
//
// A capability we could not LEARN reads as present: a server older than the field,
// one whose backend could not be constructed to ask, or a runtime that never
// fetched /info all yield nil, and the caller then behaves exactly as it did
// before this check existed (poll, and time out if the state never comes). Only a
// backend that positively answered "I do not report health" is refused, so no
// client-side list of backend names decides this.
func backendReportsHealth(rt *runtime) bool {
	b := rt.backendInfo()
	return b == nil || b.ReportsHealth
}

// backendLabel names the deploy backend for an error message, falling back to a
// neutral word when the server reported capabilities but no name.
func backendLabel(rt *runtime) string {
	if b := rt.backendInfo(); b != nil && b.Name != "" {
		return fmt.Sprintf("%q", b.Name)
	}
	return "configured"
}

// errHealthUnsupported is what a REQUIRED `depends_on: condition: service_healthy`
// gets on a backend that cannot report container health: the condition can never
// be met, so the only honest outcomes are this error now or the same failure
// dressed up as a timeout minutes later. It names the dependent service, the
// dependency, the backend, and the two ways out (the condition that works, or a
// backend that reports health).
func errHealthUnsupported(rt *runtime, service, dependency string) error {
	return fmt.Errorf("service %q depends on %q with condition: %s, but the %s deploy backend does not report container health (it has no probe engine, so the healthcheck is dropped and %q can never be observed healthy): use condition: %s, or deploy to a backend that reports health",
		service, dependency, compose.DependsOnHealthy, backendLabel(rt), dependency, compose.DependsOnStarted)
}

// healthConditionUnsatisfiable reports whether dep is a depends_on entry this
// backend can never satisfy: `condition: service_healthy` (and only that — no
// other condition consults Health) against a backend that reports no health. A
// provider dependency (compose-spec `provider:`) is exempt: it is satisfied by its
// plugin `up` completing, not by any container status.
func healthConditionUnsatisfiable(rt *runtime, dep compose.Dependency) bool {
	if dep.Condition != compose.DependsOnHealthy {
		return false
	}
	if plan, ok := rt.plans[dep.Service]; ok && plan.Provider != nil {
		return false
	}
	return !backendReportsHealth(rt)
}

// validateDependencyConditions rejects a depends_on condition the target backend
// can never produce BEFORE the up builds or deploys anything, so the failure lands
// as one actionable message instead of as a generic dependency timeout after the
// workloads are already running.
//
// Today that is exactly `condition: service_healthy` on a backend without a probe
// engine (see errHealthUnsupported). Only a REQUIRED dependency fails the up: an
// optional one keeps its documented "warn and continue" behavior, and is left to
// waitForDependencies, which warns once per service as it skips the wait — warning
// here too would print the same notice twice. Dependencies outside `selected` are
// ignored, matching waitForDependencies: nothing waits on them.
//
// Services are visited in name order so a project with several offending services
// always reports the same first one.
func validateDependencyConditions(rt *runtime, selected map[string]bool) error {
	...
	for _, name := range names {
		...
		for _, dep := range svc.DependsOn {
			if !selected[dep.Service] || !healthConditionUnsatisfiable(rt, dep) {
				continue
			}
			if dep.Required {
				return errHealthUnsupported(rt, name, dep.Service)
			}
		}
	}
	return nil
...
//
// Backend limitations:
//   - service_healthy needs a backend that runs a probe engine and reports
//     InstanceStatus.Health. On one that does not (containerd, bare, incus, which
//     drop the healthcheck at Apply time) the condition is unsatisfiable BY
//     CONSTRUCTION, so it is refused immediately here — a required dependency
//     returns the actionable error errHealthUnsupported builds, an optional one
//     warns and is skipped — instead of polling a field that will never change
//     until the full timeout elapses. `up` normally rejects this before deploying
//     anything (validateDependencyConditions); this is the same check at the seam
...
func waitForDependencies(ctx context.Context, rt *runtime, cl statusPoller, serviceName string, selected map[string]bool, d *cliout.Driver, status *serviceStatus, poll, timeout time.Duration) error {
	...
	for _, dep := range svc.DependsOn {
		...
		// A condition the backend can never produce is refused rather than waited
		// on: polling it only converts an unsatisfiable dependency into a timeout,
		// after burning the whole (multi-minute) bound. `up` already validated this
		// before deploying; repeating it here keeps the guarantee at the seam that
		// does the waiting, for every caller.
		if healthConditionUnsatisfiable(rt, dep) {
			if dep.Required {
				return errHealthUnsupported(rt, serviceName, dep.Service)
			}
			d.Warn("%s: optional dependency %q cannot be %s on the %s deploy backend (it does not report container health); continuing without waiting",
				serviceName, dep.Service, compose.DependsOnHealthy, backendLabel(rt))
			continue
		}
		cond := dep.Condition
		if cond == "" {
