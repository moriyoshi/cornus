...
func TestWaitForDependenciesBlocksThenProceeds(t *testing.T) {
	...
	err := waitForDependencies(context.Background(), rt, p, "web", selected, testDriver(&out), time.Millisecond, 5*time.Second)
	if err != nil {
...
func TestWaitForDependenciesRequiredTimeout(t *testing.T) {
	...
	selected := map[string]bool{"web": true, "db": true}
	var out bytes.Buffer
	err := waitForDependencies(context.Background(), rt, p, "web", selected, testDriver(&out), time.Millisecond, 20*time.Millisecond)
	if err == nil {
...
func TestReportReconcileTransitions(t *testing.T) {
	...
	var out bytes.Buffer
	d := testDriver(&out)
	st := reportReconcile(context.Background(), p, "web", "web", d, d.Progress(), time.Millisecond, 5*time.Second)
...
func TestReportReconcileTimeout(t *testing.T) {
	...
	var out bytes.Buffer
	d := testDriver(&out)
	st := reportReconcile(context.Background(), p, "web", "web", d, d.Progress(), time.Millisecond, 20*time.Millisecond)
...
func TestReportReconcileContextCancel(t *testing.T) {
	...
	go func() {
		reportReconcile(ctx, p, "web", "web", d, d.Progress(), time.Millisecond, time.Hour)
		close(done)
	}()
	select {
	case <-done:
...
func TestReportReconcileViaClient(t *testing.T) {
	...
	var out bytes.Buffer
	d := testDriver(&out)
	st := reportReconcile(context.Background(), client.New(srv.URL), "web", "web", d, d.Progress(), time.Millisecond, 5*time.Second)
	if len(st.Instances) != 1 || !st.Instances[0].Running {
...
func TestReportCompletionExitsPromptly(t *testing.T) {
	...
	go func() {
		st = reportCompletion(context.Background(), p, "job", "job", d, d.Progress(), time.Millisecond, time.Hour)
		close(done)
	}()
	select {
	case <-done:
...
func TestReportCompletionTimeout(t *testing.T) {
	...
	var out bytes.Buffer
	d := testDriver(&out)
	reportCompletion(context.Background(), p, "job", "job", d, d.Progress(), time.Millisecond, 20*time.Millisecond)
...
func TestReportReconcileRemovedWhileWaiting(t *testing.T) {
	...
	var st api.DeployStatus
	go func() {
		st = reportReconcile(context.Background(), p, "web", "web", d, d.Progress(), time.Millisecond, time.Hour)
		close(done)
	}()
	select {
	case <-done:
