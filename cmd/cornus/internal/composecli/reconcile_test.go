...
// withBackend stamps the deploy backend capabilities a server would have
// advertised via GET /.cornus/v1/info onto rt, standing in for the fetch the up
// path memoizes. A runtime that never calls it reports no backend at all — the
// "unknown, assume capable" case an older server produces.
func withBackend(rt *runtime, name string, reportsHealth bool) *runtime {
	rt.serverInfoMemo = &api.ServerInfo{Backend: &api.BackendInfo{Name: name, ReportsHealth: reportsHealth}}
	return rt
}

// TestWaitForDependenciesHealthUnsupported: on a backend that reports no health,
// a REQUIRED service_healthy dependency must fail immediately with an actionable
// error, never poll, and never consume the timeout.
func TestWaitForDependenciesHealthUnsupported(t *testing.T) {
	p := &scriptedPoller{steps: []pollStep{
		{st: status(inst("db-0", "running", true))}, // healthy is never reported here
	}}
	rt := withBackend(depRuntime("web", compose.DependsOn{{Service: "db", Condition: compose.DependsOnHealthy, Required: true}}), "containerd", false)
	selected := map[string]bool{"web": true, "db": true}
	var out bytes.Buffer
	start := time.Now()
	err := waitForDependencies(context.Background(), rt, p, "web", selected, testDriver(&out), nil, time.Millisecond, time.Hour)
	if err == nil {
		t.Fatal("waitForDependencies = nil, want an unsupported-condition error")
	}
	for _, want := range []string{`service "web"`, `depends on "db"`, compose.DependsOnHealthy, `"containerd"`, "does not report container health", "use condition: " + compose.DependsOnStarted} {
		if !strings.Contains(err.Error(), want) {
			t.Errorf("error %q missing %q", err, want)
		}
	}
	if p.count() != 0 {
		t.Errorf("dependency was polled %d times, want 0 (the condition is unsatisfiable)", p.count())
	}
	if elapsed := time.Since(start); elapsed > 5*time.Second {
		t.Errorf("wait took %s; it must not consume the timeout", elapsed)
	}
}

// TestWaitForDependenciesHealthUnsupportedOptional: the same dependency, not
// required, warns and continues WITHOUT waiting (the pre-existing "warn and
// proceed" contract, just not paid for with the full timeout).
func TestWaitForDependenciesHealthUnsupportedOptional(t *testing.T) {
	p := &scriptedPoller{steps: []pollStep{{st: status(inst("db-0", "running", true))}}}
	rt := withBackend(depRuntime("web", compose.DependsOn{{Service: "db", Condition: compose.DependsOnHealthy, Required: false}}), "bare", false)
	selected := map[string]bool{"web": true, "db": true}
	var out bytes.Buffer
	err := waitForDependencies(context.Background(), rt, p, "web", selected, testDriver(&out), nil, time.Millisecond, time.Hour)
	if err != nil {
	...
	if p.count() != 0 {
		t.Errorf("dependency was polled %d times, want 0", p.count())
	}
	if !strings.Contains(out.String(), "continuing without waiting") {
		t.Errorf("expected an optional-dependency warning; got:\n%s", out.String())
	}
}

// TestWaitForDependenciesHealthSupported: the identical project on a backend that
// DOES report health keeps polling to satisfaction, unchanged.
func TestWaitForDependenciesHealthSupported(t *testing.T) {
	p := &scriptedPoller{steps: []pollStep{
		{st: status(healthInst("db-0", "running", true, "starting"))},
		{st: status(healthInst("db-0", "running", true, "healthy"))},
	}}
	rt := withBackend(depRuntime("web", compose.DependsOn{{Service: "db", Condition: compose.DependsOnHealthy, Required: true}}), "dockerhost", true)
	selected := map[string]bool{"web": true, "db": true}
	var out bytes.Buffer
	if err := waitForDependencies(context.Background(), rt, p, "web", selected, testDriver(&out), nil, time.Millisecond, 5*time.Second); err != nil {
...
// TestWaitForDependenciesStartedUnaffected: service_started never consults
// health, so it must behave identically on both kinds of backend.
func TestWaitForDependenciesStartedUnaffected(t *testing.T) {
	for _, tc := range []struct {
		name          string
		backend       string
		reportsHealth bool
	}{
		{"health-reporting backend", "dockerhost", true},
		{"backend without health", "containerd", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			p := &scriptedPoller{steps: []pollStep{
				{st: status(inst("db-0", "pending", false))},
				{st: status(inst("db-0", "running", true))},
			}}
			rt := withBackend(depRuntime("web", compose.DependsOn{{Service: "db", Condition: compose.DependsOnStarted, Required: true}}), tc.backend, tc.reportsHealth)
			selected := map[string]bool{"web": true, "db": true}
			var out bytes.Buffer
			if err := waitForDependencies(context.Background(), rt, p, "web", selected, testDriver(&out), nil, time.Millisecond, 5*time.Second); err != nil {
				t.Fatalf("waitForDependencies = %v, want nil", err)
			...
			if p.count() < 2 {
				t.Errorf("expected the wait to poll until running, got %d polls", p.count())
			}
		})
	}
}

// TestValidateDependencyConditions covers the planning-time gate: it must reject
// a required service_healthy dependency on a backend without health BEFORE
// anything is deployed, and leave every other combination alone.
func TestValidateDependencyConditions(t *testing.T) {
	cases := []struct {
		...
		backend       string
		reportsHealth bool
		unknown       bool // server advertised no backend at all
		wantErr       bool
	}{
		{name: "healthy required, no health", condition: compose.DependsOnHealthy, required: true, backend: "containerd", wantErr: true},
		{name: "healthy required, health reported", condition: compose.DependsOnHealthy, required: true, backend: "kubernetes", reportsHealth: true},
		{name: "healthy required, backend unknown", condition: compose.DependsOnHealthy, required: true, unknown: true},
		{name: "healthy optional, no health", condition: compose.DependsOnHealthy, backend: "incus"},
		{name: "started, no health", condition: compose.DependsOnStarted, required: true, backend: "containerd"},
		{name: "completed, no health", condition: compose.DependsOnCompleted, required: true, backend: "bare"},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			rt := depRuntime("web", compose.DependsOn{{Service: "db", Condition: c.condition, Required: c.required}})
			if !c.unknown {
				withBackend(rt, c.backend, c.reportsHealth)
			}
			err := validateDependencyConditions(rt, map[string]bool{"web": true, "db": true})
			if c.wantErr != (err != nil) {
				t.Fatalf("validateDependencyConditions = %v, wantErr %v", err, c.wantErr)
			}
		})
	}
}

// TestValidateDependencyConditionsSkipsUnselected: a dependency outside the
// selection is never waited on, so it must not fail the up either.
func TestValidateDependencyConditionsSkipsUnselected(t *testing.T) {
	rt := withBackend(depRuntime("web", compose.DependsOn{{Service: "db", Condition: compose.DependsOnHealthy, Required: true}}), "containerd", false)
	if err := validateDependencyConditions(rt, map[string]bool{"web": true}); err != nil {
		t.Fatalf("validateDependencyConditions = %v, want nil (db not selected)", err)
	}
}
