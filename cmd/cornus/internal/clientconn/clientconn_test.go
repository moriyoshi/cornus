...
func TestParseConduitSpec(t *testing.T) {
	ok := []struct {
		...
		// A non-loopback bind address is carried through verbatim: whether it is
		// ALLOWED is clientconduit.Config.Validate's call, made where the
		// --allow-non-loopback opt-in is visible. Rejecting it here would make the
		// opt-in unreachable, since the flag cannot un-fail a parse error.
		{"wildcard bind", "socks5://0.0.0.0:1080", ConduitSpec{Mode: clientconduit.ModeSocks5, Listen: "0.0.0.0:1080", HasListen: true, SessionLocal: true, SessionLocalSet: true}},
		{"wildcard bind v6", "socks5://[::]:1080", ConduitSpec{Mode: clientconduit.ModeSocks5, Listen: "[::]:1080", HasListen: true, SessionLocal: true, SessionLocalSet: true}},
		{"any-interface port only", "socks5://:1080", ConduitSpec{Mode: clientconduit.ModeSocks5, Listen: ":1080", HasListen: true, SessionLocal: true, SessionLocalSet: true}},
		{"non-loopback ip", "socks5://192.168.1.5:1080", ConduitSpec{Mode: clientconduit.ModeSocks5, Listen: "192.168.1.5:1080", HasListen: true, SessionLocal: true, SessionLocalSet: true}},
	}
	for _, tc := range ok {
...
func TestConduitConfig(t *testing.T) {
	...
	t.Run("bare socks5 override keeps profile socks5 settings", func(t *testing.T) {
		...
		cfg := mustConduitConfig(t, cn, "socks5")
	...
	t.Run("url override replaces listen and suffix, keeps resolve", func(t *testing.T) {
		t.Setenv("CORNUS_CONDUIT", "")
		cn := connWithProfileAndEnv(t, profile)
		cfg := mustConduitConfig(t, cn, "socks5://127.0.0.1:1099?suffix=.demo.internal")
	...
	t.Run("flag url beats env, env beats profile", func(t *testing.T) {
		...
		cn := connWithProfileAndEnv(t, profile)
		cfg := mustConduitConfig(t, cn, "socks5://127.0.0.1:1099")
		if cfg.Socks5Listen != "127.0.0.1:1099" {
	...
	t.Run(".shared overrides a profile back to the shared proxy", func(t *testing.T) {
		...
		cn := connWithProfileAndEnv(t, profile)
		cfg := mustConduitConfig(t, cn, "socks5://.shared") // ...but the flag forces shared
		if cfg.Socks5SessionLocal {
	...
	t.Run(".shared with a port pins the shared proxy address", func(t *testing.T) {
		t.Setenv("CORNUS_CONDUIT", "")
		cn := connWithProfileAndEnv(t, profile)
		cfg := mustConduitConfig(t, cn, "socks5://.shared:1085")
		if cfg.Socks5SessionLocal {
	...
	t.Run("no profile, no override defaults to port-forward", func(t *testing.T) {
		t.Setenv("CORNUS_CONDUIT", "")
		cn := &Conn{}
		if cfg := mustConduitConfig(t, cn, ""); cfg.Mode != clientconduit.ModePortForward {
			t.Errorf("Mode = %q, want port-forward", cfg.Mode)
		}
	})

	// A non-loopback bind address survives resolution, carrying the address the user
	// asked for. Enforcement is clientconduit.Config.Validate, which the caller runs
	// after folding in --allow-non-loopback. Before this, ParseConduitSpec rejected
	// it outright, which no flag could override.
	t.Run("non-loopback bind is resolved, not rejected", func(t *testing.T) {
		t.Setenv("CORNUS_CONDUIT", "")
		cn := &Conn{}
		cfg := mustConduitConfig(t, cn, "socks5://0.0.0.0:10080")
		if cfg.Mode != clientconduit.ModeSocks5 {
			t.Errorf("Mode = %q, want socks5", cfg.Mode)
		}
		if cfg.Socks5Listen != "0.0.0.0:10080" {
			t.Errorf("Socks5Listen = %q, want the requested non-loopback address", cfg.Socks5Listen)
		}
		// ...and it is still refused until the caller opts in.
		if err := cfg.Validate(); err == nil {
			t.Errorf("Validate() = nil, want a refusal without --allow-non-loopback")
		}
		cfg.Socks5AllowNonLoopback = true
		if err := cfg.Validate(); err != nil {
			t.Errorf("Validate() with the opt-in = %v, want nil", err)
		}
	})

	// A selector that ParseConduitSpec rejects must surface THAT diagnosis here, at
	// the flag. It used to be kept as an opaque mode, so clientconduit.Start later
	// reported the unrelated `unknown conduit mode "http://x"` for a URL whose real
	// fault the parser had already diagnosed precisely.
	t.Run("malformed override is reported, not kept opaque", func(t *testing.T) {
		for _, tc := range []struct{ name, override, want string }{
			{"wrong scheme", "http://x", "scheme must be socks5 or socks5h"},
			{"stray path", "socks5://127.0.0.1:1080/foo", `unexpected path "/foo"`},
			{"unknown query param", "socks5://127.0.0.1:1080?foo=1", `unknown query parameter "foo"`},
		} {
			t.Run(tc.name, func(t *testing.T) {
				t.Setenv("CORNUS_CONDUIT", "")
				cn := &Conn{}
				cfg, err := cn.ConduitConfig(tc.override)
				if err == nil {
					t.Fatalf("ConduitConfig(%q) = %+v, want an error", tc.override, cfg)
				}
				if !strings.Contains(err.Error(), tc.want) {
					t.Errorf("error = %v, want it to mention %q", err, tc.want)
				}
				if strings.Contains(err.Error(), "unknown conduit mode") {
					t.Errorf("error = %v, want the parse diagnosis, not Start's unknown-mode fallback", err)
				}
			})
		}
	})
...
// mustConduitConfig resolves a conduit selector that the test expects to be valid.
func mustConduitConfig(t *testing.T, cn *Conn, override string) clientconduit.Config {
	t.Helper()
	cfg, err := cn.ConduitConfig(override)
	if err != nil {
		t.Fatalf("ConduitConfig(%q): %v", override, err)
	}
	return cfg
}
