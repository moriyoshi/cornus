...
func TestParseConduitSpec(t *testing.T) {
	ok := []struct {
		...
		{"empty suffix is still present", "socks5://:1085?suffix=", ConduitSpec{Mode: clientconduit.ModeSocks5, Listen: ":1085", HasListen: true, HasSuffix: true, SessionLocal: true, SessionLocalSet: true}},
...
func TestConduitConfig(t *testing.T) {
	...
	t.Run("bare socks5 override keeps profile socks5 settings", func(t *testing.T) {
		...
		cn := &Conn{ProfileConduit: profile}
