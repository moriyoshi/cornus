...
type Flags struct {
	...
	ViaMux      *bool    `kong:"name='telemetry-via-mux',negatable,help='Carry the workload telemetry over the caretaker existing connection to the cornus server instead of dialing it directly. On by default when cornus is the destination and the backend is kubernetes; --no-telemetry-via-mux forces the direct dial.'"`
}

// Set reports whether any telemetry flag was provided.
func (f Flags) Set() bool {
	return f.Endpoint != "" || f.Protocol != "" || len(f.Headers) > 0 ||
		f.Insecure || len(f.Signals) > 0 || f.ServiceName != "" || f.Debug || f.ViaMux != nil
}
...
func (f Flags) Apply(spec *api.DeploySpec) error {
	...
	// Only an EXPLICIT --telemetry-via-mux / --no-telemetry-via-mux writes the
	// field; a nil pointer leaves the spec undecided so the server can default it.
	if f.ViaMux != nil {
		t.ViaMux = f.ViaMux
	}
	if f.Debug {
