...
func TestApplyAcceptsEndpointless(t *testing.T) {
	// Telemetry flags with no endpoint are accepted: an empty endpoint asks the
	// cornus server to export to its own built-in observability store. Requiring
	// one here would make `--telemetry` unusable for exactly the users the store
	// exists for. A deploy path with no store to default to rejects it later, with
	...
	if !spec.Telemetry.Active() {
		t.Error("telemetry flags did not enable telemetry")
	}
	if spec.Telemetry.Endpoint != "" {
		t.Errorf("endpoint = %q, want empty", spec.Telemetry.Endpoint)
	}
}
