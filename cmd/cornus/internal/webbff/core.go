...
//
// An upstream 4xx keeps its own status. Collapsing those into 502 said "the
// gateway failed" about what was really the caller's own bad request, and it
// broke a live feature: the metrics dashboard renders "nothing has ever reported
// this" by branching on the store's 400 for an unresolved metric name
// (MetricPanel.tsx fetchSource), which no 502 can ever satisfy — so every panel
// for a metric this deploy backend does not record failed as an error toast
// instead. The browser mock returns the real 400, so the UI tests passed while
// the product did not.
//
// A 5xx stays 502 on purpose: THAT one really is "the upstream failed", and the
// BFF answering 500 would claim the failure as its own. Either way the body is
// the upstream's own "<status>: <message>", so the real code is never lost.
func writeErr(w http.ResponseWriter, err error) {
	...
	if errors.As(err, &ae) && ae.StatusCode >= 400 && ae.StatusCode < 500 {
		http.Error(w, ae.Error(), ae.StatusCode)
		return
	}
...
func (s *Server) Workloads(ctx context.Context) ([]webWorkload, error) {
	...
	for _, st := range list {
		...
		row := webWorkload{Name: st.Name, Service: byService[st.Name], Image: st.Image, Created: true, Instances: st.Instances, Origin: st.Origin}
...
func (s *Server) Tunnels(ctx context.Context) (webTunnelsResponse, error) {
	...
	for _, st := range list {
		...
		resp.Tunnels = append(resp.Tunnels, webTunnel{Service: byService[st.Name], Workload: st.Name, TunnelStatus: ts})
	}
...
//
// Reached only from the `project_apply` MCP tool. The web UI deliberately exposes
// no apply — it accompanies the CLI rather than duplicating it — so unlike every
// other method here this one has an MCP adapter and no HTTP route. The re-exec is
// what buys the exact `up` semantics (reconcile, background agent handoff for
// mounts/conduit) without duplicating composecli's CLI-coupled reconcile engine.
...
//
// ExitKnown exists because ExitCode alone cannot express "the command ran but we
// could not read its status", and the zero value of an int is indistinguishable
// from success. Docker reports an exec as still Running for a moment after its
// stdio stream closes, and ExecInspect can fail outright, so a caller that reads
// ExitCode==0 as "it worked" will eventually be wrong. It is deliberately the
// POSITIVE spelling: a value nobody filled in means "unknown", which is the safe
// answer, so a fake or a future call site cannot claim success by omission.
//
// Informational callers (ExecRun, and through it the MCP exec_run tool) still get
// stdout/stderr when the status is unknown — the command did run. Callers that
// sequence anything destructive behind the result must refuse instead; see
// containerExecOK.
type ExecResult struct {
	Stdout    string `json:"stdout"`
	Stderr    string `json:"stderr"`
	ExitCode  int    `json:"exitCode"`
	ExitKnown bool   `json:"exitKnown"`
}
...
func (s *Server) ExecRun(ctx context.Context, name string, cmd []string) (ExecResult, error) {
	return execCapture(ctx, s.client, name, "", cmd, maxToolCapture)
}

// ---- activity ----

// webActivity is the flight record as this surface returns it.
//
// LiveInstance travels with the records because without it they are misleading:
// the serving process's OWN lifetime record is legitimately open (it has not
// exited), and a reader that could not tell it apart from a crashed one would
// report a healthy server as having died.
type webActivity struct {
	Events       []activity.Event `json:"events"`
	LiveInstance string           `json:"liveInstance,omitempty"`
}

// Activity returns the server's flight records: what it and its caretakers were
// doing, and what they did not finish.
//
// since accepts an RFC3339 instant or a duration back from now ("2h"), parsed
// here rather than by the caller so the web and MCP surfaces agree on what a
// time window means.
func (s *Server) Activity(ctx context.Context, since, kind string, unfinished bool) (webActivity, error) {
	ts, err := activity.ParseSince(since)
	if err != nil {
		return webActivity{}, statusErr(http.StatusBadRequest, "since: %v", err)
	}
	events, live, err := s.client.Activity(ctx, ts, kind, unfinished)
	if err != nil {
		return webActivity{}, err
	}
	if events == nil {
		events = []activity.Event{}
	}
	return webActivity{Events: events, LiveInstance: live}, nil
}
