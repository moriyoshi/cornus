...
func (ts *termSession) info() termInfo {
	// Both detector reads happen BEFORE ts.mu is taken, and through the detector's
	// own accessors. `agent` stopped being immutable when the /proc probe started
	// replacing it with the live foreground program, so reading the field directly
	// was a data race; and taking d.mu under ts.mu would make this the one place in
	// the file that orders the two locks, which the probe path deliberately avoids.
	agent, detState := "", sessionState("")
	if ts.det != nil {
		agent, detState = ts.det.currentAgent(), ts.det.current()
	}
	ts.mu.Lock()
	defer ts.mu.Unlock()
	// State is reported only while the session is alive: a dead session's last
	// screen is stale, so the UI shows "ended", not a frozen activity badge.
	state := ""
	if ts.det != nil && ts.alive {
		state = string(detState)
	}
