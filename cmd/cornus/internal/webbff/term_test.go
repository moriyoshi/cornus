...
// lastCreate is the ExecConfig the manager last handed the backend. Some of what the
// web UI asks for — a working directory above all — is never echoed back in the session
// list, so the config the backend was given is the only place to observe it.
func (f *fakeExec) lastCreate(t *testing.T) api.ExecConfig {
	t.Helper()
	f.mu.Lock()
	defer f.mu.Unlock()
	if len(f.creates) == 0 {
		t.Fatal("no ExecCreate reached the backend")
	}
...
// TestTermCreateWorkingDir pins the working directory the web UI sends when a terminal
// is opened from the file browser: it has to reach the backend as ExecConfig.WorkingDir,
// because nothing downstream ever echoes it back — the session list does not carry it,
// so a test that watched the response could not tell "the shell started in /srv" from
// "the shell started wherever the image says".
//
// The three cases below are one claim each about createTermRequest.dir, not three
// spellings of one: absolute passes through, absent stays absent (so a plain terminal
// does not suddenly acquire a cwd), and a relative value is DROPPED rather than
// forwarded — the point being that the backend never sees a path it would resolve
// against a directory neither side chose.
func TestTermCreateWorkingDir(t *testing.T) {
	for _, tc := range []struct {
		name string
		req  createTermRequest
		want string
	}{
		{"absolute", createTermRequest{Workload: "web", Dir: "/srv/app"}, "/srv/app"},
		{"uncleaned", createTermRequest{Workload: "web", Dir: "/srv/app/../app/"}, "/srv/app"},
		{"absent", createTermRequest{Workload: "web"}, ""},
		{"relative", createTermRequest{Workload: "web", Dir: "srv/app"}, ""},
		{"escaping", createTermRequest{Workload: "web", Dir: "../etc"}, ""},
	} {
		t.Run(tc.name, func(t *testing.T) {
			fe := &fakeExec{}
			mgr := newTermManager(fe)
			sess, err := mgr.Create(context.Background(), tc.req.Workload, tc.req.cmd(), tc.req.dir())
			if err != nil {
				t.Fatalf("Create: %v", err)
			}
			t.Cleanup(func() { mgr.Kill(sess.id) })
			if got := fe.lastCreate(t).WorkingDir; got != tc.want {
				t.Errorf("ExecConfig.WorkingDir = %q, want %q", got, tc.want)
			}
		})
	}
}
