...
func (s *Server) loadProject() error {
	...
	if len(files) == 0 {
		// No project is not the same as no roots. `--local-root` names directories
		// directly, and browsing one without a compose file is its whole point — so
		// the root set is built on this path too, not only on the project path.
		s.buildLocalRoots()
		return nil
	}
...
func (s *Server) routes(mux *http.ServeMux) {
	...
	mux.HandleFunc("POST /.cornus/web/fs/move", s.handleFsMove)
	mux.HandleFunc("POST /.cornus/web/fs/preflight", s.handleFsPreflight)
	...
	// Observability store (workload logs/traces/metrics). These are plain
	// request/response reads; the live tails above stay WebSocket.
	mux.HandleFunc("GET /.cornus/web/observe/logs", s.handleObserveLogs)
	mux.HandleFunc("GET /.cornus/web/observe/traces", s.handleObserveTraces)
	mux.HandleFunc("GET /.cornus/web/observe/trace/{id}", s.handleObserveTrace)
	mux.HandleFunc("GET /.cornus/web/observe/metrics", s.handleObserveMetrics)
	mux.HandleFunc("GET /.cornus/web/observe/status", s.handleObserveStatus)
	...
	// POST, not GET: the candidate list is the caller's own state and travels in
	// the body. See shells.go.
	mux.HandleFunc("POST /.cornus/web/workloads/{name}/shells", s.handleShellsDiscover)
}
...
type webConfigResponse struct {
	...
	// ClientIngress is how the CLIENT realizes ingress (the conduit's own ingress
	// mode), the counterpart to the SERVER's advertised front door in
	// Server.Ingress. It lives on /config rather than /tunnels because it is a
	// setting, not a live tunnel — and next to AgentLive, which is what tells a
	// reader whether an empty list means "routes none" or "nobody was there to ask".
	ClientIngress []AgentIngress `json:"clientIngress,omitempty"`
	Version       string         `json:"version"`
}

func (s *Server) handleConfig(w http.ResponseWriter, r *http.Request) {
	// One Status() call, not two: it round-trips the agent's control socket, and
	// two calls could straddle an agent restart and report a live agent whose
	// ingress settings came from before it — or the reverse.
	...
	if agent != nil {
		resp.ClientIngress = agent.Ingress
	}
...
// webWorkload deliberately carries no backend name, though api.DeployStatus has
// one. That field exists so an error message and the `cornus.backend` log
// attribute can say WHICH target answered — it is constant across a response, not
// a per-workload fact: a server builds its deploy backend once (pkg/server's
// getBackend memoizes a single s.backend from server config alone), every backend
// stamps its own Name() on every status it returns, and this BFF proxies exactly
// one server. Relaying it per row only ever produced the same string N times. The
// server-scoped fact is /config's server.backend (api.ServerInfo.Backend).
...
type webTunnel struct {
	// Service is the compose service this tunnel's deployment realizes, joined
	// here rather than left to the browser: the UI's forward tables are keyed by
	// service, and the tunnel rows sat beside them naming only the resource. It
	// is empty for a deployment outside the loaded project — there is no service
	// name to give one, and no plan to look it up in.
	Service  string `json:"service,omitempty"`
...
// No apply route, and do not add one back: this HTTP surface serves a browser UI
// that accompanies the CLI rather than replacing it, so re-deploying a project is
// `cornus compose up -d` in the terminal, not a button. `Server.Apply` stays — the
// `project_apply` MCP tool calls it directly for agents, which have no terminal of
