...
// TestArchivelessDeadEndExplainsItself is the other half of the archiveless kube
// shape: the backend has no archive AND the caretaker will not serve this path,
// so the fallback chain has run out. That is a legitimate state — the caretaker
// serves the workload's volumes and nothing else — but the user used to be handed
// `structured filesystem operations are unsupported here: web`, which names an
// internal sentinel and says nothing about what to do.
//
// The assertions are about the CONTENT of the message, because content is the
// entire feature here. In particular the server's own reason has to survive: it
// is what separates "the pod's caretaker has not connected yet" (wait) from "this
// path is not in a volume" (nothing to wait for), and those need opposite
// responses from whoever is reading.
func TestArchivelessDeadEndExplainsItself(t *testing.T) {
	const reason = "no caretaker root serves /etc"
	mux := http.NewServeMux()
	mux.HandleFunc("/.cornus/v1/deploy/{name}/archive", func(w http.ResponseWriter, r *http.Request) {
		io.Copy(io.Discard, r.Body)
		w.WriteHeader(http.StatusNotImplemented)
		w.Write([]byte(`{"error":"cp/archive not supported on the kubernetes backend"}`))
	})
	mux.HandleFunc("POST /.cornus/v1/deploy/{name}/fsop", func(w http.ResponseWriter, r *http.Request) {
		// What the server sends when the operator refuses a path: 501 carrying the
		// classified refusal, which is also how the archive-absent case answers.
		w.WriteHeader(http.StatusNotImplemented)
		json.NewEncoder(w).Encode(api.FSOpResponse{Error: reason, Code: api.FSErrUnsupported})
	})
	srv := httptest.NewServer(mux)
	t.Cleanup(srv.Close)
	...
	for _, tc := range []struct {
		...
		{"CopyFrom", func() error {
			_, err := fs.CopyFrom(context.Background(), "web", "/etc/hosts", io.Discard)
			return err
		}},
		{"CopyTo", func() error {
			return fs.CopyTo(context.Background(), "web", "/etc", strings.NewReader("TAR"), api.CopyToOptions{})
		}},
	} {
		t.Run(tc.op, func(t *testing.T) {
			err := tc.run()
			if err == nil {
				t.Fatal("a dead-ended transfer reported success")
			}
			msg := err.Error()
			for _, want := range []string{
				"/etc",    // WHICH path is out of reach
				"web",     // ...in which workload
				reason,    // the server's own reason, not swallowed
				"volumes", // the actionable constraint
			} {
				if !strings.Contains(msg, want) {
					t.Errorf("message %q does not mention %q", msg, want)
				}
			}
			// The sentinel stays reachable for code that routes on it, even though
			// it is no longer what the user reads.
			if !errors.Is(err, client.ErrFSOpUnsupported) {
				t.Errorf("the ErrFSOpUnsupported sentinel was dropped from the chain: %v", err)
			}
			// And it still carries a status, so the explorer answers 501 rather
			// than the generic 500 an unclassified error would get.
			var se *statusError
			if !errors.As(err, &se) || se.code != http.StatusNotImplemented {
				t.Errorf("want a statusError(501), got %#v", err)
			}
		})
	}
}
