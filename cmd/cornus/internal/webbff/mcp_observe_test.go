...
// TestObserveMetricsPreservesUpstreamStatus is about the metrics dashboard, not
// about status codes for their own sake.
//
// The store answers a metric name nothing has ever reported with a 400 and a
// "metric ... is not resolved" diagnostic — not with an empty result. The UI is
// built on exactly that: MetricPanel.fetchSource renders the panel as an ABSENCE
// when it sees a 400 whose body matches /not resolved/, and re-throws anything
// else. On a kubernetes server that is every host-backend metric (container CPU
// time, network I/O, disk I/O), so this is the ordinary path, not an edge.
//
// The BFF used to collapse every upstream failure into 502, which no amount of
// body-matching can rescue: the panel raised an error toast for a metric that
// simply does not exist on this backend. The browser mock returns the real 400,
// which is why the UI's own tests never saw it.
//
// A 5xx is asserted alongside deliberately: it must NOT be passed through, or the
// BFF would be claiming the upstream's failure as its own.
func TestObserveMetricsPreservesUpstreamStatus(t *testing.T) {
	for _, tc := range []struct {
		...
		{"unresolved metric", http.StatusBadRequest,
			`obsstore: promql: metric "container_cpu_time" is not resolved`, http.StatusBadRequest},
		{"store failure", http.StatusInternalServerError, "store exploded", http.StatusBadGateway},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			s := testServer(t, up, fakeAgentView{status: &AgentStatus{}})
			h, err := s.Handler()
			if err != nil {
				t.Fatal(err)
			}
			req := httptest.NewRequest("GET",
				"/.cornus/web/observe/metrics?query=container_cpu_time&since=1h&step=1m", nil)
			// Through the whole Handler, so the answer is what a browser really
			// gets: guardHost included, which needs a Host it serves under.
			req.Host = "127.0.0.1:5000"
			rec := httptest.NewRecorder()
			h.ServeHTTP(rec, req)

			if rec.Code != tc.want {
				t.Errorf("status = %d, want %d (upstream said %d)", rec.Code, tc.want, tc.upstream)
			}
			// The diagnostic has to survive whichever status is chosen: it is what
			// the UI matches on, and what a human reads when it does not.
			if !strings.Contains(rec.Body.String(), tc.body) {
				t.Errorf("body %q lost the upstream diagnostic %q", strings.TrimSpace(rec.Body.String()), tc.body)
			}
		})
	}
}
