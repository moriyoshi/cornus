...
type AgentStatus struct {
	...
	Banners []string
	// Ingress is how this CLIENT realizes ingress, one entry per live conduit that
	// routes any. Empty is the answer "no conduit routes ingress" — distinguishable
	// from "no agent answered" only because that case yields no AgentStatus at all,
	// so a consumer must branch on the nil status, never on this being empty.
	Ingress []AgentIngress
}

// AgentIngress mirrors the agent's clientagent.AgentIngress. It is duplicated
// rather than imported for the same reason AgentStatus is: clientagent hosts this
// package, so importing it back would be an import cycle.
...
type Config struct {
	...
	// ConfigPath, Context, and Host are replayed as flags when the `project_apply`
	// MCP tool re-execs `cornus compose up -d`.
	ConfigPath string
	Context    string
	Host       string

	// LocalRoots are extra browsable directories for the file explorer, on top of
	// the ones the Compose project implies. They exist because the implied set is
	// exactly "the project directory and its bind-mount sources", which is a
	// guess about what the developer is working on rather than an answer: a
	// scratch directory, a sibling checkout, or a data drive is browsable to them
	// and invisible here. Absolute (the agent host has its own working directory),
	// and subject to the same browsableSource refusals as any other root.
	LocalRoots []LocalRootSpec
	...
	// AllowedHosts are extra Host header values guardHost accepts, on top of the
	// loopback spellings and PublishedName. It exists for an off-host bind
	// (`cornus web --addr --allow-non-loopback`), where the name a browser reaches
	// this origin under is a LAN address or hostname only the operator knows;
	// `--allow-host` names them so the DNS-rebinding pin can stay on.
	AllowedHosts []string

	// AllowAnyHost disables the Host pin entirely. It is what an off-host bind
	// falls back to when the operator named no hosts at all: the bind is already
	// deliberately network-reachable and we cannot enumerate the names it answers
	// to, so pinning would only break legitimate access. Callers that set it must
	// say so out loud — `cornus web` warns on startup.
	AllowAnyHost bool
...
// LocalRootSpec is one operator-declared browsable directory (`cornus web
// --local-root`). Label is what the source switcher shows; empty means the
// directory's base name.
type LocalRootSpec struct {
	Label    string
	Path     string
	ReadOnly bool
}
...
type Server struct {
	...
	// refusedRoots are bind-mount sources that were deliberately NOT exposed
	// (browsableSource said no — a pseudo-filesystem, or the filesystem root). Kept
	// so the UI can explain the absence instead of a root just going missing.
	refusedRoots []fsRefusedRoot
	// hostMounts maps a deployment name to what backs each of its container paths
	// (see fsmounts.go). Built in loadProject, never mutated after.
	hostMounts map[string][]hostMount
	...
	// fsopKnown memoizes whether a workload's backend can serve structured
	// filesystem operations (see serverFSOps). Probed lazily and re-probed after
	// fsopProbeTTL, so a caretaker that connects late is not written off; guarded
	// because the explorer's handlers run concurrently.
	fsopMu    sync.Mutex
	fsopKnown map[string]fsopProbe

	// shellsKnown memoizes which interactive shells a workload has, keyed by the
	// workload AND the resolved candidate list (see shells.go). Probing costs one
	// exec per candidate that is absent, so a terminal-heavy session would otherwise
	// re-walk the same list for every pane it opens.
	shellsMu    sync.Mutex
	shellsKnown map[string]shellProbe
...
//
// Config.AllowedHosts widens the pin (for an off-host bind reached by LAN address
// or hostname) and Config.AllowAnyHost removes it; both require the operator to
// have asked for an off-host bind in the first place.
func (s *Server) guardHost(next http.Handler) http.Handler {
	if s.cfg.AllowAnyHost {
		return next
	}
	...
	for _, h := range s.cfg.AllowedHosts {
		if h = strings.ToLower(strings.Trim(h, "[]")); h != "" {
			allowed[h] = true
		}
	}
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !allowed[host] && !isLoopbackHost(host) {
			http.Error(w, "unrecognized Host header (pass `cornus web --allow-host "+host+"` to serve this name)", http.StatusMisdirectedRequest)
			return
		}
		next.ServeHTTP(w, r)
	})
