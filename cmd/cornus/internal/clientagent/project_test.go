...
// ingressCalls returns a snapshot of the recorded AddIngress calls.
func (c *recordingConduit) ingressCalls() []ingressCall {
	c.mu.Lock()
	defer c.mu.Unlock()
	return append([]ingressCall(nil), c.ingress...)
}

// Rebind is the conduit dimension of the reconcile: when a same-name `up` arrives
// with changed conduit/ingress settings, the agent acquires the conduit the new
// settings ask for and rebinds the live project onto it. The exposure dimension
// must move across (ports, alias, AND the ingress registration), while the mount
// dimension — a healthy 9P deploy-attach session — must be left completely alone.
func TestRebindMovesExposureAndKeepsMount(t *testing.T) {
	oldConduit := &recordingConduit{}
	newConduit := &recordingConduit{}
	att := &countingAttacher{}
	p := NewProject(att, oldConduit)
	t.Cleanup(p.Close)

	svc := mountedService()
	svc.Spec.Ingress = &api.IngressSpec{Enabled: true, Hosts: []string{"web.example.test"}}
	if status, err := p.StartService(svc); err != nil || status != StatusStarted {
		t.Fatalf("first up = %q, %v; want %q", status, err, StatusStarted)
	}

	if !p.Rebind(newConduit) {
		t.Fatal("Rebind reported no change for a different conduit")
	}
	// The rebind itself performs no teardown; the reconcile that follows moves the
	// exposure, which is why the service reports "recreated".
	if status, err := p.StartService(svc); err != nil || status != StatusRecreated {
		t.Fatalf("re-up after rebind = %q, %v; want %q", status, err, StatusRecreated)
	}

	if got := att.count(); got != 1 {
		t.Fatalf("attach count = %d, want 1 (a conduit change must not rebuild the mount dimension)", got)
	}
	if got := len(oldConduit.addCalls()); got != 1 {
		t.Fatalf("old conduit Add count = %d, want 1 (nothing may re-register in the conduit we left)", got)
	}
	adds := newConduit.addCalls()
	if len(adds) != 1 {
		t.Fatalf("new conduit Add count = %d, want 1 (the exposure must move across)", len(adds))
	}
	if adds[0].name != "proj-web" || len(adds[0].ports) != 1 || len(adds[0].aliases) != 1 || adds[0].aliases[0] != "web" {
		t.Fatalf("re-registered exposure = {name:%q ports:%v aliases:%v}, want {proj-web, 1 port, [web]}", adds[0].name, adds[0].ports, adds[0].aliases)
	}
	ing := newConduit.ingressCalls()
	if len(ing) != 1 || ing[0].spec == nil || len(ing[0].spec.Hosts) != 1 || ing[0].spec.Hosts[0] != "web.example.test" {
		t.Fatalf("ingress registrations in the new conduit = %+v, want the service's declared host", ing)
	}

	// Level-triggered: with nothing changed, the next reconcile rebuilds neither
	// dimension.
	if status, err := p.StartService(svc); err != nil || status != StatusUpToDate {
		t.Fatalf("re-up after the rebind settled = %q, %v; want %q", status, err, StatusUpToDate)
	}
	if got := len(newConduit.addCalls()); got != 1 {
		t.Fatalf("new conduit Add count = %d after an unchanged re-up, want 1", got)
	}
	if got := att.count(); got != 1 {
		t.Fatalf("attach count = %d after an unchanged re-up, want 1", got)
	}
}

// Rebinding onto the conduit the project is already on changes nothing, so no
// dimension is rebuilt — the guard that keeps an `up` with unchanged settings from
// churning the exposures.
func TestRebindToSameConduitIsNoOp(t *testing.T) {
	conduit := &recordingConduit{}
	att := &countingAttacher{}
	p := NewProject(att, conduit)
	t.Cleanup(p.Close)
	...
	if p.Rebind(conduit) {
		t.Fatal("Rebind reported a change for the conduit the project is already on")
	}
	if p.Rebind(nil) {
		t.Fatal("Rebind reported a change for a nil conduit")
	}
	if status, err := p.StartService(mountedService()); err != nil || status != StatusUpToDate {
		t.Fatalf("re-up = %q, %v; want %q", status, err, StatusUpToDate)
	}
	if got := len(conduit.addCalls()); got != 1 {
		t.Fatalf("Add count = %d, want 1 (a no-op rebind must not re-register)", got)
	}
	if got := att.count(); got != 1 {
		t.Fatalf("attach count = %d, want 1", got)
	}
}
