...
func newMountController(attacher Attacher, reporter DeployReporter) *mountController {
	return &mountController{attacher: attacher, reporter: reporter, live: map[string]*mountSession{}}
}

// report surfaces a deploy notice: through the user-facing reporter when one is
// set (an interactive frontend), otherwise falling back to slog with severity
// matching the event — a transient diagnostic is a warning, a terminal failure an
// error. ctx carries the request's logger for the slog fallback.
func (m *mountController) report(ctx context.Context, n DeployNotice) {
	if m.reporter != nil {
		m.reporter(n)
		return
	}
	...
	if n.Terminal {
		log.ErrorContext(ctx, "deploy error", "service", n.Service, "error", n.Message)
	} else {
		log.WarnContext(ctx, "deploy warning", "service", n.Service, "error", n.Message)
	}
}
...
func (m *mountController) ensure(opCtx context.Context, name string, spec api.DeploySpec, fingerprint string) (context.Context, bool, error) {
	...
	// Stream this service's deploy logs and surface its deploy diagnostics as the
	// attach runs; the shared session handles the readiness/hold/self-exit mechanics.
	// The server streams both transient diagnostics (a workload still starting — an
	// image pull in progress, a brief CrashLoopBackOff, a one-off status-poll error
	// that later succeeds; Event.Done unset) and terminal failures (Done set). Route
	// them by severity so a transient diagnostic that resolves on its own is a
	// user-facing warning, not an alarming error: a bare `e.Err != ""` used to log
	// EVERY diagnostic — transient ones included — through slog as an "error", so the
	// noise leaked past the interactive reporter even when the deploy came up fine.
	s := attachsession.Open(m.attacher, spec, attachsession.WithEventHook(func(e deploywire.Event) {
		if e.Log != "" {
			fmt.Print(e.Log)
		}
		if e.Err != "" {
			m.report(opCtx, DeployNotice{Service: name, Message: e.Err, Terminal: e.Done})
		}
	}))
...
type exposureReg struct {
	...
	fingerprint string
	forwards    []string
	// dirty marks a registration whose fingerprint still matches the desired service
	// but whose CONDUIT was replaced under it (see rebind). The next ensure must
	// therefore re-register it even though nothing about the service changed.
	dirty bool
}
...
func (e *exposureController) ensure(parent context.Context, name string, svc Service, fingerprint string, reparent bool) ([]string, bool, error) {
	...
	cur := e.live[name]
	e.mu.Unlock()
	if cur != nil && cur.fingerprint == fingerprint && !reparent && !cur.dirty {
...
// rebind points the controller at a different conduit and marks every live
// registration dirty, so the next reconcile re-registers each service in the new
// conduit through the ordinary ensure path (withdraw the old registration, add the
// new one). It performs no teardown itself: the old conduit must stay live until
// its registrations have been moved, and only the reconcile knows the parent
// context each service's exposure hangs off. Reports whether the conduit actually
// changed. Callers must serialize this with reconciles (Project.Rebind holds the
// reconcile lock).
func (e *exposureController) rebind(conduit clientconduit.Conduit) bool {
	e.mu.Lock()
	defer e.mu.Unlock()
	if conduit == nil || conduit == e.conduit {
		return false
	}
	e.conduit = conduit
	for _, reg := range e.live {
		reg.dirty = true
	}
	return true
}
