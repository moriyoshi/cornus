...
func (a *Agent) doUp(req Request) Response {
	...
	entry, err := a.ensureProject(req.Project, req.Conn, req.Conduit)
	if err != nil {
	...
	return Response{OK: true, Running: p.Running(), Forwards: p.Forwards(), Statuses: statuses, Banners: entry.banners}
}
...
func (a *Agent) ensureProject(name string, spec ConnSpec, egCfg ConduitCfg) (*projectEntry, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *Agent) removeProject(name string) {
	...
	if entry.active != 0 || len(entry.project.Running()) != 0 {
		a.mu.Unlock() // still in use or non-empty — keep it
		return
	}
