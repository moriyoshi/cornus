...
// Rebind points the project's EXPOSURE dimension at a different conduit and
// reports whether that changed anything. It is how a same-name `up` whose conduit
// configuration changed is reconciled rather than silently ignored: the agent
// acquires the conduit the new configuration asks for and rebinds the live project
// onto it.
//
// It is a level-triggered edit of the desired state, not an imperative teardown —
// it only swaps the conduit and marks the live registrations dirty, so the
// reconcile that follows (the Apply of the same `up`) moves each service through
// the ordinary exposure ensure path. Two consequences matter:
//
//   - The MOUNT dimension is untouched by construction. Its fingerprints do not
//     change and its attacher is fixed for the project's lifetime, so a conduit or
//     ingress settings change never restarts a healthy 9P deploy-attach session; the
//     re-registered exposures re-parent on the very same live mount contexts.
//   - The old conduit is still live when its registrations are withdrawn, because
//     withdrawal happens through each registration's own context during that
//     reconcile. The agent releases its refcount on the old conduit only afterwards,
//     so a conduit another frontend still shares is never closed.
//
// Serialized with Apply/ApplyExact/Remove through reconcileMu.
func (p *Project) Rebind(conduit clientconduit.Conduit) bool {
	p.reconcileMu.Lock()
	defer p.reconcileMu.Unlock()
	return p.exposure.rebind(conduit)
}
