...
// and a conduit (for port exposure).
func NewProject(attacher Attacher, conduit clientconduit.Conduit) *Project {
	return &Project{
		mounts:   newMountController(attacher),
...
// DeploySpec, so any spec change recreates the workload (docker compose recreate
// semantics). encoding/json emits struct fields in order and sorts map keys, so
// equal specs always hash equal.
func mountFingerprint(s Service) (string, error) {
	return hashJSON("mount", s.Name, s.Spec)
}
