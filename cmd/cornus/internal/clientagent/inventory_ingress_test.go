...
// ingressConduitReq brings up a conduit carrying ing, through the docker frontend.
//
// It used to publish a web UI, because that was an entry point that took a conduit
// config; the agent no longer hosts web UIs, and the docker frontend serves the same
// purpose here. SOCKS5 specifically, because only a SOCKS5 conduit answers CAInfo —
// a port-forward conduit returns nil, so it could never show that the Trust lines
// come from the LIVE conduit rather than from the configuration.
func ingressConduitReq(t *testing.T, server string, ing *clientconduit.IngressConfig) Request {
	t.Helper()
	cfg := socks5Conduit()
	cfg.Ingress = ing
	return Request{
...
func TestInventoryReportsEmulatedClientIngress(t *testing.T) {
	...
	resp := a.doDockerServe(ingressConduitReq(t, "http://fake:5000", &clientconduit.IngressConfig{
...
func TestInventoryReportsNoIngressWhenTheConduitRoutesNone(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			a := newTestAgent(t, fakeResolve(nil))
			resp := a.doDockerServe(ingressConduitReq(t, "http://fake:5000", tc.ing))
			if !resp.OK {
				t.Fatalf("docker-serve = %+v", resp)
			}
			inv := a.inventory()
...
func TestInventoryDeduplicatesIdenticalClientIngress(t *testing.T) {
	...
	for _, s := range []struct{ server, name string }{
		...
		if resp := a.doDockerServe(ingressConduitReq(t, s.server, ing())); !resp.OK {
	...
	if resp := a.doDockerServe(ingressConduitReq(t, "http://three:5000", other)); !resp.OK {
		t.Fatalf("docker-serve three = %+v", resp)
	}
	inv := a.inventory()
