...
type ConfigSetContextCmd struct {
	...
	TokenExchange      bool   `kong:"name='token-exchange',negatable,help='Trade the profile credential for a short-lived Cornus credential via OAuth 2.0 Token Exchange (RFC 8693), cached between commands. Requires a server with a JWT/JWKS verifier and a scope map.'"`
	TokenExchangeScope string `kong:"name='token-exchange-scope',help='Narrow the exchanged credential to this scope (e.g. registry:pull). Empty takes whatever the server scope map grants; a scope it does not grant is refused, never silently reduced.'"`

	KeyAuthIdentityFile string `kong:"name='key-auth-identity-file',type='path',help='SSH private-key file used to mint short-lived Cornus sessions.'"`
	KeyAuthFingerprint  string `kong:"name='key-auth-fingerprint',help='SHA256 fingerprint of the key to use from SSH_AUTH_SOCK.'"`
	KeyAuthName         string `kong:"name='key-auth-name',help='Enrollment name used by cornus auth enroll.'"`
	KeyAuthScope        string `kong:"name='key-auth-scope',help='Requested session scope (default api).'"`
	KeyAuthTTL          string `kong:"name='key-auth-ttl',help='Requested session lifetime as a Go duration (default 1h, maximum 24h).'"`
...
func (c *ConfigSetContextCmd) Run(cli *CLI) error {
	...
	// --token-exchange is negatable, so an explicit --no-token-exchange has to be
	// able to turn it back off; keying only on "is it true" would make disabling
	// impossible without hand-editing the config.
	if c.TokenExchange || c.TokenExchangeScope != "" {
		if ctx.TokenExchange == nil {
			ctx.TokenExchange = &clientconfig.TokenExchange{}
		}
		ctx.TokenExchange.Enabled = c.TokenExchange
		if c.TokenExchangeScope != "" {
			ctx.TokenExchange.Scope = c.TokenExchangeScope
		}
	}
	...
	if c.KeyAuthIdentityFile != "" || c.KeyAuthFingerprint != "" || c.KeyAuthName != "" || c.KeyAuthScope != "" || c.KeyAuthTTL != "" {
		if ctx.KeyAuth == nil {
			ctx.KeyAuth = &clientconfig.KeyAuth{}
		}
		if c.KeyAuthIdentityFile != "" {
			ctx.KeyAuth.IdentityFile = c.KeyAuthIdentityFile
		}
		if c.KeyAuthFingerprint != "" {
			ctx.KeyAuth.KeyFingerprint = c.KeyAuthFingerprint
		}
		if c.KeyAuthName != "" {
			ctx.KeyAuth.Name = c.KeyAuthName
		}
		if c.KeyAuthScope != "" {
			ctx.KeyAuth.Scope = c.KeyAuthScope
		}
		if c.KeyAuthTTL != "" {
			ctx.KeyAuth.TTL = c.KeyAuthTTL
		}
	}
