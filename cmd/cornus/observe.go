...
func (r observeStatusResult) Human(p cliout.Printer) {
	...
	if m := s.Metrics; m != nil {
		...
		// Not a fault, so it is not shouted like the two above: the backend simply
		// republishes one reading to several polls (metrics-server does, its scrape
		// being coarser than the sampling interval). Worth showing, because it is
		// the honest explanation for a series that is sparser than the interval
		// suggests — and for why raising the interval would cost nothing.
		if m.Stale > 0 {
			p.Line("  repeated  %d readings the backend re-served (source is coarser than %s)", m.Stale, m.Interval)
		}
