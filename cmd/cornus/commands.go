...
func (c *PushCmd) Run(cli *CLI) error {
	...
	if tok := pushToken(cli, destRef.Context().RegistryStr()); tok != "" {
...
type DeployCmd struct {
	...
	IngressController   string               `kong:"name='ingress-controller',help='Native-mode ingress controller Service to tunnel to, as <namespace>/<service>[:httpPort/httpsPort]. Empty learns it from the server (GET /.cornus/v1/info). Takes precedence over CORNUS_INGRESS_CONTROLLER and the profile.'"`
...
// dockerhost by default, containerd, bare (the daemonless OCI-runtime backend),
// or incus (OCI images as Incus application containers) — mirroring the server's
// selection (kubernetes stays server-only; deploying into a cluster goes through
// `cornus deploy --server`). Supported values are "" / "dockerhost" (the
// default), "containerd", "bare", and "incus"; any other value — including
// "kubernetes", which only the server honors — falls through to dockerhost with
// a warning.
func localBackend(cli *CLI) (deploy.Backend, error) {
	switch v := os.Getenv("CORNUS_DEPLOY_BACKEND"); v {
	case "podman":
		return dockerhost.New(
			dockerhost.WithFlavor(dockerhost.FlavorPodman),
			dockerhost.WithPolicy(dockerhost.PermissivePolicy()),
		)
	case "containerd":
	...
	case "incus":
		return incushost.New(
			incushost.Config{DataDir: cli.resolveConfig().DataDir},
			incushost.WithPolicy(hostpolicy.Permissive()),
		)
	default:
		if v != "" && v != "dockerhost" {
...
func (c *DeployCmd) runRemote(base context.Context, d *cliout.Driver, cn *clientconn.Conn, spec api.DeploySpec) error {
	...
	if err := c.materializeNativeTLS(ctx, cn, &spec); err != nil {
		return err
	}
	if c.Detach {
	...
	err := cl.DeployAttach(ctx, spec, func(e deploywire.Event) {
		...
		if e.Ready {
			fwdOnce.Do(func() {
				...
				stopTunnel = ingresspub.Publish(ctx, d, cl, cn.Tunnel, ingresspub.OptFor(spec), "", spec.Name)
			})
		}
...
// conduitCfg resolves the session conduit config for this deploy: --no-forward-ports
// folded into ModeNone, --conduit layered over CORNUS_CONDUIT and the profile, and
// --allow-non-loopback into the bind-exposure opt-in. It validates the result up
// front so a bad selector is reported at the flag rather than after the workload is
// already deployed. The override layer is returned alongside, so the caller can
// resolve via-server and the ingress config from the same layering. Ingress is NOT
// applied here: --no-forward-ports must be able to short-circuit before that (it can
// fetch the server's /info).
func (c *DeployCmd) conduitCfg(cn *clientconn.Conn) (clientconduit.Config, clientconn.Config, error) {
	mode := c.Conduit
	if c.NoForwardPorts {
		mode = clientconduit.ModeNone
	}
	override, err := clientconn.ConfigFromOptions(c.ViaServer, mode, c.IngressConduit, c.IngressController, c.IngressEmulateCA, c.IngressEmulateCAKey)
	if err != nil {
		return clientconduit.Config{}, clientconn.Config{}, err
	}
	cfg := cn.ConduitConfigFor(override)
...
func (c *DeployCmd) materializeNativeTLS(ctx context.Context, cn *clientconn.Conn, spec *api.DeploySpec) error {
	cfg, override, err := c.conduitCfg(cn)
	if err != nil {
		return err
	}
	cn.ApplyIngressConfig(ctx, &cfg, override)
	return clientconduit.MaterializeNativeTLS(spec, cfg)
}
...
func (c *DeployCmd) startConduit(ctx context.Context, d *cliout.Driver, cn *clientconn.Conn, spec api.DeploySpec) (clientconduit.Conduit, error) {
	...
	cn.ApplyIngressConfig(ctx, &cfg, override)
	viaServer := cn.ResolveConfig(override).ViaServer
	// Joinable, for the same reason as the compose foreground: a session's proxy is
	// only useful to a browser if everything else can be reached through it too.
	eg, err := clientconduit.Start(ctx, cn.Dialer(viaServer != nil && *viaServer), cfg, clientconduit.WithRendezvous(clientagent.ConduitRegistry()))
	if err != nil {
...
func (c *DeployCmd) runRemoteDetached(ctx context.Context, d *cliout.Driver, cl *client.Client, endpoint string, spec api.DeploySpec) error {
	if err := checkDetachedConduitOptions(c); err != nil {
		return err
	}
...
func checkDetachedConduitOptions(c *DeployCmd) error {
	var opts []string
	if c.Conduit != "" {
		opts = append(opts, "--conduit")
	}
	if c.AllowNonLoopback {
		opts = append(opts, "--allow-non-loopback")
	}
	if c.IngressConduit != "" {
		opts = append(opts, "--ingress-conduit")
	}
	if c.IngressController != "" {
		opts = append(opts, "--ingress-controller")
	}
	if c.IngressEmulateCA != "" {
		opts = append(opts, "--ingress-emulate-ca")
	}
	if c.IngressEmulateCAKey != "" {
		opts = append(opts, "--ingress-emulate-ca-key")
	}
	if len(opts) == 0 {
		return nil
	}
	return fmt.Errorf("%s cannot be used with --detach: detached deploys have no client session for conduits or ingress; drop --detach or use cornus port-forward", strings.Join(opts, ", "))
}
...
// pushToken resolves the bearer token `cornus push` should present to destReg,
// the destination registry host.
//
// CORNUS_TOKEN wins outright, matching the precedence every other command
// documents. Failing that it falls back to the SELECTED CONTEXT's static token —
// but only when that context's server is the same host as the destination, so a
// push to an unrelated registry (ghcr.io, Docker Hub) can never be handed a
// cornus credential. Without the host check, storing a token in a profile would
// silently widen where it is sent, which is the opposite of what the
// destination-scoped keychain below exists to prevent.
//
// The profile is read directly rather than through cli.resolveConn: resolving a
// connection can start a port-forward for a pf-only profile and can mint a
// kube-auth token, and neither side effect belongs in a registry copy. Two costs
// follow, both preferable to those side effects and both escapable via
// CORNUS_TOKEN: a kube-auth-only profile contributes no token here, and a
// per-project cornus-context override's token is not layered on (that layering,
// with its provenance vetting, lives in Resolver.ResolveWith).
//
// Any failure to load the config is silently ignored — push must keep working
// with no config file at all.
func pushToken(cli *CLI, destReg string) string {
	...
	_, cc, err := f.Resolve(cli.Context)
	if err != nil || cc == nil || cc.Token == "" || cc.Server == "" {
		return ""
	}
	if !sameRegistryHost(cc.Server, destReg) {
		return ""
	}
	return cc.Token
}

// sameRegistryHost reports whether a context server URL and a registry host
// refer to the same host:port. The context server is a URL ("http://127.0.0.1:5000")
// while a registry host is bare ("127.0.0.1:5000"), so they need normalizing
// before comparison.
func sameRegistryHost(server, registry string) bool {
	host := server
...
// only when the requested resource's registry host matches registry, and falls
// through to the ambient Docker credentials for every other host. This scopes the
// token to the destination registry so a cross-registry crane.Copy cannot leak it
// to the source host, while still letting a copy READ from a private source the
// caller is already logged in to.
...
// otherwise delegates to authn.DefaultKeychain (the Docker config credential
// store), which yields anonymous credentials when the host has no entry.
// Returning authn.Anonymous unconditionally here used to break a cross-registry
// copy whose SOURCE was private even when `docker login` could authenticate it.
