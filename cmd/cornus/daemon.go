...
type DaemonCmd struct {
	...
	Preflight      PreflightCmd         `kong:"cmd,help='Check whether this process can drive the container runtime of the configured deploy backend (run it inside the image you intend to serve from).'"`
...
// agentEnsureRunning and agentSend are the background-agent calls DockerProxyCmd
// makes, as vars so a test can drive Run without spawning a real agent process.
//
// Without them nothing could observe what this command PRINTS, and printing is
// the part that broke: the conduit banner naming the session-local SOCKS5 proxy
// address is only ever emitted here (see the loop in Run), so dropping it left a
// working proxy nobody could address. A test that only checks the agent RETURNS
// Response.Banners stays green through exactly that regression — which is what
// the attestation audit found.
var (
...
func (c *DockerProxyCmd) Run(r *clientconn.Resolver, d *cliout.Driver) error {
	...
	socket, err := agentEnsureRunning()
	if err != nil {
		return fmt.Errorf("start background agent: %w", err)
	}
	resp, err := agentSend(socket, req)
	if err != nil {
	...
	// The conduit banner names the SOCKS5 proxy address, and it is the ONLY place
	// a session-local proxy's address is ever printed — the agent binds it on a
	// port the caller did not choose. Dropping it here left a `daemon docker`
	// session with a working proxy nobody could address, while `compose up`
	// (commands.go) and `cornus web` (web.go) both print the same lines. The agent
	// has always returned them in the docker-serve response; only this path
	// discarded them.
