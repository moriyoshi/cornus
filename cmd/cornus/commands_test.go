...
func TestCheckDetachedConduitOptions(t *testing.T) {
	cases := []struct {
		...
		{name: "allow-non-loopback", set: func(c *DeployCmd) { c.AllowNonLoopback = true }},
...
// TestDeployConduitCfgAllowNonLoopback pins the opt-in end of `cornus deploy
// --conduit socks5://<non-loopback>`: refused by default, accepted with
// --allow-non-loopback. Resolution happens before the workload is applied, so a
// refusal costs nothing and an accepted bind is the address the user asked for.
func TestDeployConduitCfgAllowNonLoopback(t *testing.T) {
	t.Setenv("CORNUS_CONDUIT", "")
	cn := &clientconn.Conn{}

	_, _, err := (&DeployCmd{Conduit: "socks5://0.0.0.0:10080"}).conduitCfg(cn)
	if err == nil {
	...
	cfg, _, err := (&DeployCmd{Conduit: "socks5://0.0.0.0:10080", AllowNonLoopback: true}).conduitCfg(cn)
	if err != nil {
