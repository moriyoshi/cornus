...
import (
	...
	// GitHub support, the same source/delivery pair one layer over: the
	// `github-cli` SOURCE reads the developer's own `gh auth login` by running
	// `gh auth token`, and the `github-proxy` DELIVERY provider injects it into
	// the workload's REST calls (GITHUB_API_URL override) so the token never
	// enters the container. REST only — git-over-HTTPS does not ride the proxy.
...
type CLI struct {
	...
	// VersionFlag makes `cornus --version` work, the spelling every other CLI
	// answers to. It prints the `version` kong.Var (wired in main() to the same
	// build-stamped string the `version` subcommand reports) and exits 0. The
	// subcommand stays: it is the one that also carries --features.
	VersionFlag kong.VersionFlag `kong:"name='version',help='Print the cornus version and exit.'"`
	...
	Auth          AuthCmd          `kong:"cmd,help='Enroll and manage SSH public keys used for short-lived client sessions.'"`
	...
	IngressTunnel IngressTunnelCmd `kong:"cmd,name='ingress-tunnel',help='Host a public tunnel in front of the server ingress, so every host and path a project declares is reachable through one URL (unlike cornus tunnel, which exposes a single workload port).'"`
	...
	Activity      ActivityCmd      `kong:"cmd,help='Read the flight records: what the server and its caretakers were doing, and what they did not finish.'"`
	Observe       ObserveCmd       `kong:"cmd,help='Query a WORKLOAD\\'s recorded telemetry (logs, traces, metrics) from the server built-in observability store. Distinct from cornus activity, which reads cornus own records.'"`
	Storage       StorageCmd       `kong:"cmd,help='Server storage administration (non-destructive disk-usage report).'"`
...
// parserOptions is the kong configuration main() parses with.
//
// It is a function rather than an inline argument list so a test can build the
// SAME parser instead of a lookalike. Two of these options are user-visible
// behavior with no other observer — ShortUsageOnError decides what a mistyped
// flag prints, and Vars feeds --version — and asserting them against a replica
// option list would prove only that the replica is self-consistent. Both were
// closed as done and neither had a test; see .agents/docs/TODO.md.
func parserOptions() []kong.Option {
	return []kong.Option{
		kong.Name("cornus"),
		kong.Description("All-in-one container registry, build engine, and deploy engine."),
		// ShortUsageOnError, not UsageOnError: the full help for this CLI is ~170
		// lines, which scrolls the actual error message off an 80x24 terminal. A
		// one-line usage hint plus the error is what the user needs; `--help` is
		// there for the rest.
		kong.ShortUsageOnError(),
		// Feeds the `--version` VersionFlag above.
...
func main() {
	...
	var cli CLI
	kctx := kong.Parse(&cli, parserOptions()...)
