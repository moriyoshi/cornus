...
// resolveAuthToken picks the tunnel credential to inject. See
// ingresspub.ResolveAuthToken — one implementation serves the tunnel commands
// and the declarative `ingress.tunnel` publisher alike.
func resolveAuthToken(token, tokenFile string) (string, error) {
	return ingresspub.ResolveAuthToken(token, tokenFile)
}
