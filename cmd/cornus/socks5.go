...
func (c *Socks5Cmd) Run(cli *CLI) error {
	...
	cfg, err := cn.ConduitConfig(clientconduit.ModeSocks5)
	if err != nil {
		return err
	}
	...
	// The ad-hoc proxy joins the rendezvous too, so `cornus socks5` and a compose
	// session asking for one address share it instead of racing for the bind.
	eg, err := clientconduit.Start(ctx, cn.Dialer(cn.ViaServer(c.ViaServer)), cfg, clientconduit.WithRendezvous(clientagent.ConduitRegistry()))
	if err != nil {
		return err
	}
	defer eg.Close()
