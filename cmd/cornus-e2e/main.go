...
type CLI struct {
	Target              string        `kong:"name='target',default='docker',enum='docker,podman,podman-rootless,containerd,bare,incus,kube,local',help='Deployment target: docker, podman, podman-rootless, bare (daemonless OCI runtime), incus (Incus OCI application containers), kube (kind), or local (build-only, no external env).'"`
	...
	PodmanSocket        string        `kong:"name='podman-socket',help='Podman API socket for --target podman (default: CORNUS_PODMAN_SOCKET). Not discovered: the backend refuses to guess, and so does this harness.'"`
...
func (c *CLI) Run() error {
	...
	var failures int
	// scenario -> the self-skip messages it logged (see e2e.Harness.Skips).
	...
	for _, scenario := range c.Scenarios {
		...
		if s := h.Skips(); len(s) > 0 {
			skipped[scenario] = s
			fmt.Printf("✓ %s passed (reported a skip)\n", scenario)
			continue
		}
		fmt.Printf("✓ %s passed\n", scenario)
	}
	...
	// A skip and a pass are indistinguishable in that count, which is how a scenario
	// can be registered, parse-checked, listed in a target's subset, and still never
	// execute — server-in-container-containerd.star did exactly that for four
	// consecutive green runs while being reported as covering the topology it names.
	// Printing them costs a few lines and removes the class of mistake rather than
	// one instance of it, so they are printed even though the run passed.
	if len(skipped) > 0 {
		fmt.Printf("\n%d of them reported a skip and may not have exercised anything:\n", len(skipped))
		for _, scenario := range c.Scenarios {
			for _, msg := range skipped[scenario] {
				fmt.Printf("  %s: %s\n", scenario, msg)
			}
		}
	}
...
func buildTarget(c *CLI) (e2e.Target, error) {
	...
	case "podman":
		return &e2e.PodmanTarget{Socket: c.PodmanSocket}, nil
	case "podman-rootless":
		return &e2e.PodmanTarget{Socket: c.PodmanSocket, Rootless: true}, nil
	case "containerd":
