...
| [`incus launch`](https://linuxcontainers.org/incus/) against an OCI remote | the incus deploy backend applies a whole Compose project or deploy spec to an Incus 6.3+ host as application containers, with the same CLI you use for Docker and Kubernetes |
...
`DeploySpec` imperatively across Docker, containerd, Incus, and Kubernetes alike.
