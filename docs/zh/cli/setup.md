...
对于两个 **Kubernetes** 场景，向导还会探测服务器公布的 ingress (`/.cornus/v1/info`)，并询问是否通过 SOCKS5 conduit 访问工作负载的 ingress 主机。建议的合理默认值是：服务器公布 ingress controller 时为 **native** (隧道连接到检测到的 ingress controller)，只暴露 ingress 域名时为 **emulate** (带生成证书的客户端反向代理)，否则为 **off**。你的选择会写入配置文件的 `conduit.ingress` 块并选择 socks5 conduit。请参阅[公共 ingress](/zh/topics/ingress)。
