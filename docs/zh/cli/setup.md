...
- **Docker 主机 (在容器中运行服务端)** — 在此 docker 主机上把服务端本身作为容器运行。配置文件是普通的回环配置；这个场景真正提供的是运行用的产物，因为绑定挂载才是全部难点，而写错一个会静默失败。参见[在容器中运行服务端](/zh/guides/server-in-a-container)。
...
对于 SSH 场景，向导会询问是否为远程主机写入 `cornus.service` systemd 单元；对于 Kubernetes 场景，它会询问是否写入 `cornus-values.yaml` helm values 片段；对于容器安装场景，它会提供一份 `cornus-compose.yaml`，其中包含套接字绑定、带 `rshared` 的数据目录绑定，以及与刚保存的配置文件相匹配的发布端口。每项都会在写入前询问 (写入文件、输出到标准输出、跳过)，并在已有文件时要求确认覆盖。
