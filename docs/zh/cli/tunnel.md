...
Tunnel 后端在**服务器**上由 `CORNUS_TUNNEL_BACKEND` 选择（默认 `ngrok`）；其他后端为 `ssh`（SSH reverse-tunneling）、`cloudflare`（Cloudflare Tunnel）和 `tailscale`（Tailscale Funnel）。各后端所需条件参见[公网隧道 — 后端](/zh/topics/tunnels)，分步设置说明参见[隧道指南](/zh/guides/tunnels)。
...
端口必须位于 `1..65535`。连接由 `--server` 解析，否则使用选定连接配置文件（参见 [`cornus config`](/zh/cli/config)）。更完整的说明参见[公网隧道](/zh/topics/tunnels)。
