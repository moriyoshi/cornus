...
| 指标 | 单位 | 标签 | 含义 |
|---|---|---|---|
| `container_cpu_time` | 秒 | `cornus_replica`, `cpu_mode` | 累积 CPU 时间。请配合 `rate()` 使用。 |
...
| `container_disk_io` | 字节 | `cornus_replica`, `disk_io_direction` | 累积块 I/O。Kubernetes 上不可用。 |
| `cornus_container_memory_limit` | 字节 | `cornus_replica` | 存在限制时所强制的上限。 |
| `cornus_container_pids` | 个数 | `cornus_replica` | 进程与线程数。 |
