...
团队将自主 AI agent (编码 agent、数据 agent) 作为集群工作负载运行，但其到 LLM API (例如 Anthropic API) 的调用只能经由*您*的网络离开: API 只能通过开发机上的企业代理、VPN 或 SASE 路径访问，集群中并不存在该路径。此外，API key 必须在运行时代理，绝不能写入镜像、deploy spec 或 pod spec。Cornus 同时解决这两个问题: [Egress](/zh/guides/egress) 将 agent 的 API 出站调用路由回您的机器，[凭据](/zh/guides/credentials) 则交付一个只存在于您笔记本电脑上的 secret。

## 使用的功能

- [Egress](/zh/guides/egress)——deploy spec 的 `egress:` block (mode 和 PAC 风格 route script) 仅将 `api.anthropic.com` 发往调用方网络。
- [凭据](/zh/guides/credentials)——deploy spec 的 `credentials:` block 将 API key 交付给 agent，而不让它进入镜像。
- [`cornus deploy`](/zh/cli/deploy)——前台 `--server` session；relay egress 和 credential fetch 都需要它。
- [会话通道](/zh/guides/networking#会话通道-port-forward-与-socks5)——使用 `--conduit` 选择访问工作负载自身端口的方式。
...
```sh
export ANTHROPIC_API_KEY=sk-ant-...      # 自己机器上保留
```
...
```yaml
name: agent
image: localhost:5000/coding-agent@sha256:1c2d...   # 固定 digest；不含密钥

env:
  AGENT_TASK: "triage the backlog"

egress:
  mode: proxy                 # caretaker 转发代理，经服务器中继回来
  default: cluster            # 未匹配目标从 Pod 自身网络出站
  script: |
...
credentials:
  sources:
    - name: anthropic
      backend: env                       # 从调用方侧环境变量签发
      config: { var: ANTHROPIC_API_KEY } # 非密钥: 只传输变量名
      deliveries:
        - kind: env
          envVar: ANTHROPIC_API_KEY      # 注入 agent 容器
```
    ...
    - name: anthropic
      ...
      deliveries:
        - kind: endpoint
          ...
          # upstream: https://my-gateway    # 可选: Anthropic 兼容网关
```
...
**另请参阅:** [Egress](/zh/guides/egress) · [凭据](/zh/guides/credentials) · [deploy spec](/zh/reference/deploy-spec) · [`cornus deploy`](/zh/cli/deploy) · [使用远程集群](/zh/guides/remote-clusters)
