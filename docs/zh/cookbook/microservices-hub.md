...
- 工作负载到工作负载 hub overlay 及其 relay model——见[工作负载 hub](/zh/topics/hub)。
- In-cluster workload 的 deploy spec `hub:` block——见[Deploy spec](/zh/reference/deploy-spec)。
- 从任意位置（含笔记本电脑）加入 overlay 的 `cornus hub`——见 [`cornus hub`](/zh/cli/hub) 和[网络操作](/zh/guides/networking)。
...
**另请参阅：**[网络操作](/zh/guides/networking) · [工作负载 hub](/zh/topics/hub) · [Deploy spec](/zh/reference/deploy-spec) · [`cornus hub`](/zh/cli/hub)
