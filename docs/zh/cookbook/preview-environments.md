...
- [公共 tunnel](/zh/guides/tunnels)——shareable URL 的底层模型。
...
# 示例: 显示 https://abcd-1234.ngrok-free.app，将其粘贴到 PR
```
...
ports:
  - { host: 8080, container: 80 }
ingress:
  enabled: true          # host 自动派生为 <name>.<CORNUS_INGRESS_DOMAIN>
  tls: { }               # 通过服务器默认 cluster-issuer 提供 HTTPS
YAML

cornus deploy -f preview.yaml --detach
# 审阅者访问 https://app-pr-123.preview.example.com
```
...
**另请参阅:** [构建镜像](/zh/guides/building-images) · [部署工作负载](/zh/guides/deploying-workloads) · [网络与 conduit](/zh/guides/networking) · [隧道](/zh/guides/tunnels) · [Cookbook](/zh/cookbook/)
