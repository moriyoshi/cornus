...
```sh
kubectl apply -f https://raw.githubusercontent.com/moriyoshi/cornus/v0.0.0/deploy/k8s/cornus.yaml
...
如果希望使用引导式流程，请运行交互式向导 [`cornus setup`](/zh/cli/setup)。它会选择部署场景、创建并验证配置文件，然后显示剩余的设置步骤。

有关连接远程或没有 ingress 的集群，请参见[连接配置](/zh/reference/connection-config)和[使用远程集群](/zh/guides/remote-clusters)。
...
cornus compose logs demo-web               # 相同日志，无需 kubectl
```
...
```sh
# 在集群内构建并推送到镜像仓库。--builder 经 9P-on-WebSocket 将上下文和
# 密钥流式传输到 Cornus Pod，因此主机无需 Docker，也无需构建权限:
...
# 从原生 spec 部署——所有上层接口最终都转换为该 schema。
# 它使用当前连接配置文件 (显式 --server 会覆盖该配置):
...
* [使用远程集群](/zh/guides/remote-clusters)——将 CLI 指向远程集群。
* [隧道](/zh/guides/tunnels)——将工作负载公开暴露。
* [工作负载 Hub](/zh/guides/hub)——按名称访问其他工作负载。
