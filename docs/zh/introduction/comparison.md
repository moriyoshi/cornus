...
| 面向 OCI remote 的 [`incus launch`](https://linuxcontainers.org/incus/) | incus 部署后端可将整个 Compose 项目或 deploy spec 作为应用容器应用到 Incus 6.3+ 主机上，使用的 CLI 与面向 Docker 和 Kubernetes 时完全相同 |

最接近的单二进制类比是 [Werf](https://werf.io/)，它同样从一个二进制文件构建并部署到 Kubernetes；但 Werf 由 Git 驱动，仍依赖外部镜像仓库和基于 Helm 的应用，而 Cornus 由 Compose / devcontainer 驱动，提供自己的镜像仓库，并在 Docker、containerd、Incus 和 Kubernetes 上以命令式方式调谐 `DeploySpec`。
