...
| 保持主机干净 | [作为容器](#in-a-container) | `docker-container`、`containerd-container`、`incus-container` |
...
cornus 通过 Podman 的**原生 libpod API** 驱动它，而不是 Docker 兼容端点。

**需要:** podman 的 API socket，并且必须告诉 cornus 用哪一个。它不做任何推断: 两个变量都未设置时，服务器会拒绝启动，而不是去猜你指的是哪个 daemon。

```sh
# rootless (桌面与 CI 上的常见形态)
...
启用 `podman.socket` **不会**把 Podman 变成守护进程: 该 unit 是 socket activation，服务按需启动、空闲即退出。

如果你完全不想启用 socket unit，也可以让 cornus 自己运行该服务 — 只需要 `PATH` 上有 `podman` 二进制:

```sh
CORNUS_DEPLOY_BACKEND=podman CORNUS_PODMAN_SERVICE=1 cornus serve
```

远程 podman 可经 SSH 连接，直接使用 `podman system connection` 已保存的目标:

```sh
export CORNUS_PODMAN_SOCKET="ssh://core@host/run/user/1000/podman/podman.sock"
```

**rootless 注意事项:** rootless workload 的网络命名空间在宿主机上不可路由，因此 [`cornus port-forward`](/zh/cli/port-forward) 和 [`cornus tunnel`](/zh/cli/tunnel) 无法通过直连到达它。设置 `CORNUS_PODMAN_REMOTE=1`，改为经 per-instance companion 到达 workload；该模式还需要 `CORNUS_AGENT_IMAGE` 和 `CORNUS_ADVERTISE_URL`。部署、日志和 exec 在两种情况下都可用，**rootful** podman 则没有这些限制。

**检查:** `cornus health`。

**更多:** [`podman` 后端](/zh/reference/deploy-backends#podman)。
...
## 作为所管理主机上的容器 {#in-a-container}

服务端本身作为容器运行在它所管理的主机运行时上。在 docker 上，难点完全在于绑定挂载，而写错了并不会在启动时失败——它会在部署时静默失败。
...
### 在 containerd 或 Incus 主机上

两者都受支持，且配置方式差异足够大，值得在动手前先读一读:

- **containerd** 用 `ctr run` 而不是 `docker run`，这一点很关键: cornus 通过向即将用于部署的那个 containerd 询问来确定自己所在的容器，因此只有当正是那个 containerd 创建了服务端容器时，它才能找到自己。此外还需要 `--net-host` 和 `/run/cornus` 绑定，缺少任何一个，预检都会拒绝启动。请用 `cornus setup --scenario containerd-container`。
- **Incus** 只需要守护进程的套接字 (该后端不会把自己的任何路径交给 incusd) ，另外还需要 `--network host` 以获得通往 incus 网桥的路由。如果把服务端作为 incus 实例运行，这两者都不需要。请用 `cornus setup --scenario incus-container`。
