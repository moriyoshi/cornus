...
- 这里的 `--conduit` 是**固定**地址，而不是选择 conduit: 指定地址 (`--conduit socks5://127.0.0.1:1080`、`--conduit socks5://?suffix=.demo.internal`) 表示"就用这个地址"，这也是你在多个代理之中有意做出选择的方式。bare 的 `--conduit socks5`、`CORNUS_CONDUIT` 和 profile 都没有指定地址，因此 UI 会前往已经在运行的代理 — 包括另一个终端里前台 `compose up` 启动的那一个。
