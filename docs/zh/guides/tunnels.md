...
通过 [`cornus tunnel`](/zh/cli/tunnel) 将工作负载端口暴露到公网的分步设置说明，每种后端一节。有关背后的模型——凭据注入流程，以及监听器模型与上游模型的形态差异——请参阅[公网隧道](/zh/topics/tunnels)。四种后端共享同一个客户端命令；仅服务器端的 `CORNUS_TUNNEL_BACKEND` 及其对应后端的环境变量有所不同。

```sh
cornus tunnel [--authtoken TOKEN | --authtoken-file FILE] [--proto http|tcp] <name> <port>
```

**安全地传递凭据。** `--authtoken TOKEN` 会将 secret 直接放入 argv，机器上的其他用户可以通过 `ps` 读取它，shell 也常会将其写入历史记录——除了快速的本地测试外都应避免这种方式。按优先顺序推荐：完全不提供凭据 (服务器已有默认值——见下文)、使用对应后端的环境变量 (kong 会自动将其读入 `--authtoken`，因此该值不会出现为命令行参数)、或使用 `--authtoken-file FILE` (从文件读取 secret，使其既不出现在 argv 中也不出现在历史记录中)。下面的每个步骤都采用环境变量或文件的形式。
...
**另请参阅：** [cornus tunnel](/zh/cli/tunnel)、[公网隧道](/zh/topics/tunnels)
...
4. 告诉 cornus 如何推导公网 URL。如果中继会在 SSH session banner 中打印自己的 URL (sish、serveo、pinggy 都会这样做)，可以自动获取：
   ...
   ```
5. 提供 SSH 凭据——未加密的私钥 PEM 或密码，取决于中继接受哪一种。由于中继的 SSH 握手发生在**服务器**端而非客户端，这通常是整个 cornus 服务器共用的一个服务身份，而非按调用者区分的凭据，因此建议将其设置一次作为服务器端默认值，让客户端完全无需提供凭据：
   ```
...
- 不支持受密码保护的私钥 — 请使用未加密的密钥，或改用密码认证。
- 没有 known-hosts 文件、没有固定的主机密钥，也没有选择不安全模式时，连接会被拒绝，而不是信任未经验证的主机。
- 不支持 SSH agent 集成：这里的凭据是直接交给服务器的原始密钥/密码，而不是客户端 `ssh-agent` 按需生成的签名。基于 agent 的认证需要服务器的 SSH 握手能够访问一个只有客户端才能连接的 agent——即在隧道请求连接上做 agent 转发——而该后端目前尚未实现这一点。

**另请参阅：** [cornus tunnel](/zh/cli/tunnel)、[公网隧道](/zh/topics/tunnels)、[服务器环境变量](/zh/reference/server-env-vars)
...
**另请参阅：** [cornus tunnel](/zh/cli/tunnel)、[公网隧道](/zh/topics/tunnels)、[Helm chart 值](/zh/reference/helm-values)
