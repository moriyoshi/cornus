...
以上内容暴露的都是**一个工作负载的一个端口**。因此，包含多个服务的项目需要多条隧道和多个互不相关的 URL，而服务通过 [`x-cornus-ingress`](/zh/guides/ingress)声明的主机名和路径一个也用不上。

[`cornus ingress-tunnel`](/zh/cli/ingress-tunnel)发布的是入口本身:
...
只有一个 URL，而请求会像在生产环境中一样，按主机和路径抵达正确的服务。在 Kubernetes 上，隧道把流量交给真实的集群入口控制器，因此适用集群自身的路由规则和 TLS 证书；在其他所有后端上，则由服务器自己路由已声明的主机和路径。

### 你的后端提供哪一层

各后端送达对端的内容不同，这决定了能保留多少信息:

| 层 | 后端 | TLS | `Host` |
| --- | --- | --- | --- |
| HTTP | `ngrok` (http)、`cloudflare`、`tailscale` | 在提供方边缘终止，进入 cornus 的一段是纯文本 HTTP | 提供方的主机名，除非它认可了你的主机名 |
| Raw TCP | `ssh`、`ngrok` (`--proto tcp`) | 端到端——客户端的 TLS 原样抵达集群入口控制器 | 与客户端发送的完全一致 |

对于 HTTP 层隧道，访问者看到的是提供方的证书，而入口上的 `tls:` 块管辖的是集群原生的前端入口，而非隧道 URL。

端到端 TLS 需要两个条件同时成立: 一条原样传输字节的 raw TCP 隧道，**以及**一个真实的集群入口控制器来终止它。自行路由入口的服务器没有可提供的证书，因此它会拒绝 `--proto tcp`，而不是交给你一个只要有人请求 `https` 就会失败的 URL。

### 获得你自己的主机名

如果隧道的公共主机名能够与某个入口主机名**相等**，那么请求就完全不需要调整——应用收到的正是它期望的内容，TLS 也可以做到端到端。`ngrok` 通过账户下的保留域名或自定义域名可以做到；`ssh` 后端在中继按请求的 bind 主机路由时也可以，例如 [sish](https://github.com/antoniomika/sish)。Cloudflare quick tunnel 和 Tailscale Funnel 做不到。

无法做到时，cornus 会把提供方的主机名路由到你的入口主机，应用则始终看到浏览器所在的主机名——因此重定向和 Cookie 仍然可达。完整说明以及何时应当覆盖默认行为，参见 [Host 处理](/zh/cli/ingress-tunnel#host-处理)。

### 不使用任何隧道抵达入口

如果运维者只需要在服务器已经接入的网络上访问入口，那么直接绑定前端入口即可，完全不需要隧道:

```sh
CORNUS_INGRESS_LISTEN=:8080 cornus serve
```

把已声明主机名的 DNS (或 `/etc/hosts` 条目) 指向该地址即可。它提供的正是入口隧道所置于前端的同一张路由表，因此也是检查主机和路径是否按预期解析的最简单方式。
