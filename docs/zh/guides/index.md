...
### [Compose 扩展与兼容性](/zh/guides/compose-support)
...
### [在容器中运行服务端](/zh/guides/server-in-a-container)
...
启用 OpenTelemetry trace、metric 和 log；添加 Prometheus `/metrics` 端点；配置日志；并接入存活和就绪探针。或者启用内置存储, 无需运行任何后端, 就能获得*你自己工作负载*的日志、追踪和指标 —— 可搜索、比容器存活得更久, 并且可以从 Grafana 查询。
