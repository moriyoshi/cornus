...
- 与 dockerhost 相比的已知差距: 不支持 mount、volume、user network、healthcheck 以及 command/entrypoint 覆盖；`cornus attach` 不受支持(请改用 [`cornus exec`](/zh/cli/exec))；日志来自 instance 控制台，因此 `--follow`/`--since`/`--tail` 不适用；`cornus stats` 的 CPU 百分比偏低。每个未映射的字段都会按名称告警。
