...
Ingress 是[客户端侧 egress](/zh/guides/egress)的入站对应能力: 它为工作负载已发布的端口请求一个公共 **HTTP(S)** 前端入口，使服务可通过真实 hostname 访问，而不仅能通过[端口转发](/zh/guides/networking)或[隧道](/zh/guides/tunnels)访问。它面向工作负载已发布的端口，因此 spec 至少要发布一个端口。

Ingress 通过 deploy spec 的 `ingress:` block 或 Compose 的可移植 `x-cornus-ingress:` extension 显式启用，绝不会隐式打开。

## 由谁来实现

你声明的主机和路径在任何环境下都一样；不同的是由谁来提供它们:

| 后端 | 实现方式 |
| --- | --- |
| `kubernetes` | 一个面向工作负载 `ClusterIP` Service 的真实 `networking.k8s.io/v1` **Ingress** 对象，由集群的 ingress 控制器提供服务 |
| `dockerhost`、`containerd`、`bare`、`incus` | **cornus 服务器自身**: 它维护一张主机/路径路由表并代理到工作负载 |

只有 Kubernetes 这条路径会创建集群对象，因此下文的 class name、annotation 和 cert-manager TLS 设置仅适用于该路径。其他环境下由服务器路由同一份声明，这正是 Compose 项目在笔记本上和在集群中表现一致的原因。

::: warning 服务器端前端入口仅用于开发
服务器端 ingress 路由的存在，是为了让 Compose 项目在笔记本上和在集群中表现一致。它**不是**生产级 ingress: 不终止自己的 TLS，不做速率限制或访问控制，其错误页面还会显示未能到达的内部工作负载名称和容器端口——调试时有用，但放在陌生人可访问的地址上并不合适。

请将它用于开发和预览。任何长期或面向公众的用途，都应部署到带有真实 ingress 控制器的集群；只要控制器可用，cornus 就会连接它。
:::

## 如何访问

仅仅声明 ingress 并不会让它从你的机器上可达——那需要 DNS 指向某个地址，而预览环境很少具备这一点。弥合这一差距有三种方式，大致按常用程度排列:

- [`cornus ingress-tunnel`](/zh/cli/ingress-tunnel)为整个项目提供一个**公共 URL**，并代为完成路由。参见[将 ingress 公开发布](#将-ingress-公开发布)。
- [SOCKS5 conduit](#通过-conduit-从本机访问-ingress)从你自己的机器上路由已声明的主机名，不做任何公开暴露。
- `CORNUS_INGRESS_LISTEN` 把服务器的前端入口绑定到一个地址，你可以让 DNS 或 `/etc/hosts` 条目指向它。
...
Native 路径会在 deploy 请求中发送私钥材料。因此 Cornus 会在请求序列化之前拒绝经由远程明文 HTTP 的调用；请使用 HTTPS、SSH 隧道配置文件，或诸如 Kubernetes 端口转发这样的 loopback 端点。密钥绝不会出现在状态或诊断输出中。完整字段参见[连接配置参考](/zh/reference/connection-config#ingresscertificate)。
...
[`cornus ingress-tunnel`](/zh/cli/ingress-tunnel)在 ingress 前面放置一个公共 URL——作用域声明的每个主机名和路径都位于这一个地址之后:
...
这正是它与[`cornus tunnel`](/zh/cli/tunnel)的区别: 后者只暴露单个工作负载端口，对你声明的主机和路径一无所知。

如果希望每次 `compose up` 时都发布，而不是手动运行命令，可以把它声明在所适用的 ingress 旁边:
...
隧道凭据仍然来自客户端——连接配置文件中的 `authtoken-file`，或 `CORNUS_TUNNEL_AUTHTOKEN`——绝不会来自会被提交到仓库的 compose 文件。各后端的能力参见[隧道指南](/zh/guides/tunnels#隧道化入口而非端口)；在分享该 URL 之前唯一值得弄清楚的一点——应用看到的是隧道主机名还是你声明的主机名——参见 [Host 处理](/zh/cli/ingress-tunnel#host-处理)。
