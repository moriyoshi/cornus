...
JWKS 只能证明调用方**是谁**。ServiceAccount token 不携带 cornus 的 `scope` claim，因此服务器还需要一份 **scope map** 来说明该 identity 可以触及什么，否则每次调用都会被一个明确说明原因的 401 拒绝:
...
以精确的 `sub` 作为匹配键，正是这次授予具有针对性的原因: 集群中任何其他 ServiceAccount 都能签发同样有效、audience 同样正确的 token，只有这份 map 决定它不是 operator。参见 [scope 映射](/zh/guides/security#将第三方-claim-映射到-cornus-scope)。
