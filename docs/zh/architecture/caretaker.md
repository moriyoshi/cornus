...
| `hub` | 是 | 注册 hosted service，并经 [hub](/zh/architecture/networking#工作负载到工作负载-hub)访问 peer。 |
| `docker` | 是 (client API) | 在 pod loopback 运行 Docker Engine API proxy，并通过 `DOCKER_HOST` 声明 (如下) 。 |
| `otel` | 否 | 运行内置 OpenTelemetry Collector，在 pod 回环地址接收应用的 OTLP，并将其导出到配置的后端 (参见[可观测性](/zh/guides/observability#工作负载遥测)) 。 |
...
- [部署工作负载](/zh/guides/deploying-workloads)——实际中的 mount 和 session。
- [Egress](/zh/guides/egress)——实际中的模式、路径和 PAC 策略。
- [凭据](/zh/guides/credentials)——源后端和交付类型。
