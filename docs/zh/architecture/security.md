...
安全是分层且**可选**的: 未配置任何内容时，Cornus 是适用于本地开发的 pass-through。下列每一层均由配置启用，并且**一旦启用即 fail closed**——错误 policy 会导致 hard startup error，需要 identity 时拒绝空 identity，配置错误的 verifier 会拒绝而非放行。本页说明模型；[安全与认证](/zh/guides/security)指南介绍设置和加固步骤。
...
SSH 公钥客户端身份验证为 opt-in。配置后，服务器会为注册与会话签发提供用途绑定的 SSH 签名 challenge，并拒绝 SHA-1 RSA 签名。注册会轮换模式为 `0600` 的一次性 secret，并且只保存公钥。会话 proof 返回以注册名称为身份的短期 scoped JWT。可写注册仅用于单副本；多副本部署使用声明式 `CORNUS_AUTHORIZED_KEYS` 与 `CORNUS_AUTH_KEYSTORE=none`。
...
- **Hub policy 基于 verified identity。**mTLS 下 spoke identity 来自 client certificate，不来自其自身声明。参见[hub](/zh/architecture/networking#discovery-和-policy)。
...
- [安全与认证](/zh/guides/security)——配置所有 verifier 和 TLS mode，以及加固步骤。
