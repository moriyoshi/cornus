...
挂载 relay 与凭据 relay 走的是同一条转发路径——也是同一条路由记录。带有客户端侧 attachment 的 deploy-attach session 只发布一条记录，位于保留的 `cornus.mount/<digest>` name 之下 (只放 session id 的摘要，因为原始 id 是一个 capability) ；落到错误 replica 上的 caretaker stream，会经 `GET /.cornus/v1/mount/forward` 或 `GET /.cornus/v1/cred/forward` splice 到 owner。这里有意不设单独的凭据记录: 一个 deploy-attach session 同时拥有某个 pod 的挂载与凭据，因此凭据路由直接继承挂载记录的 TTL / Lease liveness 与崩溃 GC，而不是在第二套需要保持同步的生命周期里重复一遍。

这两跳的授权在 **owner** replica 上强制。Owner 在自己的本地 registry 中重新解析该 session，并把请求的挂载名或凭据名重新对照该 live session 的允许列表检查；转发方 replica 不持有任何 session 状态，从不被征询，也无法为某个 name 背书，所以伪造的转发无法绕过这项检查。这与 hub 的投递转发方向相反: 投递转发中 reach policy 由转发方强制、由 owner 信任。reach policy 以 caller identity 为键，而只有转发方认证过调用者；凭据的权限则是仅 owner 持有的 per-session capability。两个 handler 都不会再次转发，因此 stale 记录不会形成环路。
