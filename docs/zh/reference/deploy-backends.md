...
五个 host 后端均支持同一核心 spec 字段 (`name` / `image` / `replicas` / `restart` / `env` / `ports`) ，其中 `dockerhost` / `podman` / `containerd` / `bare` 还共享客户端本地 9P bind mount、Compose user network 和已发布端口转发，因此可不变地在这四者之间移动同一工作流。`incus` 仍是例外，但差距已比过去窄得多: 它映射核心字段、`entrypoint` 覆盖、服务器主机 bind mount、托管 volume、`sysctls`、`ulimits`、`tmpfs`、`shmSize` 和 healthcheck，但不支持客户端本地 9P bind mount、user network 以及仅有 command 的覆盖——而且它现在对每个无法映射的字段都会告警，不再静默丢弃任何东西。个别字段仅映射到部分后端时，[deploy spec 参考](/zh/reference/deploy-spec)会逐字段说明。
...
客户端来源的凭据在 host 后端上**完全不需要 companion**，因此也不需要 `CORNUS_ADVERTISE_URL` 或 `CORNUS_AGENT_IMAGE`，因为同机的服务器可以自行完成每一种交付。[`env` 交付](/zh/guides/credentials)在部署时解析并在创建容器时写入其环境变量; `file` 渲染到服务器自身的数据目录下并以只读方式绑定; `endpoint` 则是服务器在*工作负载 network namespace 内*绑定的监听器，它在没有原先那个边车的情况下保持了与 kubernetes 相同的安全模型 (命名空间边界即为授权) 。在会把 ID 重映射进用户命名空间的运行时上，cornus 以工作负载真正读取时所用的 ID 拥有凭据文件，因此非 root 的工作负载也能读取自己的凭据; 在这类运行时上数据目录会变为可遍历 (`0711` — 可穿过但不可列出) ，而其中的密钥仍为 `0600`。两种重映射运行时走的路径不同。rootless 的 `podman` 在容器存在之前就会报告其映射，因此服务器可以解析出所有者并一次写对。`incus` 把映射记录在实例本身上，因此写文件的那一刻无从查询: 它会先以停止状态创建实例，待映射存在后接管该目录的所有权，然后再启动。有一种情形会被拒绝而不是错误地交付——当各副本被分配了不同的 ID 范围时 (`security.idmap.isolated=true`) ，一个宿主目录只能有单一所有权，因此副本 0 能读取的凭据对其余副本都不可读。另外 `incus` 达成这一切时并不是 AttachingBackend: 它的 companion 是兄弟实例，无法承载挂载或出站流量，但这丝毫不妨碍服务器自行实现凭据交付。在 remote 模式下它们都会被拒绝，因为那里运行时的路径与进程 ID 都不是这台服务器能解析的。
...
尽管 containerd 自身不运行任何探针，healthcheck 在这里依然可用: 由 cornus 运行探针并报告结果 (见[健康检查](#健康检查)) 。

**相对 `dockerhost` 的已知缺口:** attach 仅输出。目前不测试也不支持 rootless containerd。
...
healthcheck 在这里同样可用，而且本来就没有别的东西能运行它: 在这个后端上 cornus 本身就是运行时，因此它的探针引擎与上文的 restart 监督机制并列 (见[健康检查](#健康检查)) 。

**相对 `dockerhost` 的已知缺口:** 与 `containerd` 一样，attach 仅输出。目前不支持 rootless，且会明确报错。
...
- **结构化文件操作** (`deploy.FSOperator`，即 Web 文件浏览器所用的能力) 走另一条通道: 守护进程的 SFTP。文件 API 根本无法表达 rename，而这几乎就是该能力存在的理由——归档原语已经能 stat 和复制，却无法移动任何东西。该通道对镜像没有任何要求，因此在 distroless 应用容器上也能工作，并以单一根 (`/`) 提供实例的整个文件系统。
...
- **`healthcheck`** 受支持，而且这是 cornus 出力最多的一个后端: Incus 不暴露任何 instance 级别的探针，因此由 cornus 通过守护进程的 exec API 自行运行检查 (见[健康检查](#健康检查)) 。这个 exec 也是三个后端里形态最好的一个——它的操作元数据带有命令真实的返回码，而 `bare` 后端的 OCI 运行时只能把非零退出报告为一个错误。

**与 `dockerhost` 相比的已知差距。** 除上述日志和 stats 的注意事项外，incus 后端不映射: **仅有 command** 的覆盖、客户端本地 9P bind mount、Compose user `networks` 以及 `knative`。
...
每个后端都会报告工作负载的健康状态，但走的是三条不同的路径。`dockerhost` / `podman` 把 [`healthcheck`](/zh/reference/deploy-spec#healthcheck) 交给守护进程，由它运行并在 inspect 时给出答案。`kubernetes` 把它转成 exec liveness 和 readiness probe，并把 readiness 投射为所报告的健康状态。`containerd`、`bare` 和 `incus` 没有自己的探针引擎，因此**由 cornus 自行运行探针**。

最后这一组正是本节存在的理由。一个不报告健康状态的后端并不只是少了一列: 带 `depends_on: condition: service_healthy` 的 Compose 文件在那里会被**提前拒绝**，因为诚实的答案胜过几分钟后才超时的依赖。在 cornus 自行运行探针之前，这一点让一大类普通的 Compose 文件根本无法部署到这三个后端上。

**状态机是 Docker 的**，这是刻意为之: `service_healthy` 比对的是 Docker 的词汇，因此这里的差异就是真实 Compose 文件能否收敛的差异。

- 探针以 `0` 退出即通过，其他一切都算失败——包括根本无法执行的探针，以及超出 `timeout` 的探针。
- **在 `start_period` 期间，失败不计入 `retries`**，状态保持 `starting`。第一次成功即转为 `healthy` 并结束 start period，此后的失败即使该期间尚未走完也照常计数。
- 此后 `retries` 次**连续**失败转为 `unhealthy`；任何一次成功都会转回 `healthy` 并清零计数。
- 字段为空时采用 Docker 的默认值: `interval` 30s、`timeout` 30s、`retries` 3、`start_period` 0。
- `test` 使用 Docker 的形式——`["CMD", ...]` 直接 exec，`["CMD-SHELL", s]` 通过 `/bin/sh -c` 运行 `s`，`["NONE"]` 禁用继承而来的检查。

cornus 的探针引擎有两件事是委托给守护进程的后端做不到的。一是它支持 **`start_interval`** (在 start period *期间*更频繁地探测) ，而 `kubernetes` 后端会告警说自己无法表达它——Kubernetes 的 Probe 自始至终只有一个 period。二是它保证**每个 instance 同时只有一个探针在执行**: 比 interval 更慢的探针只会推迟下一次，而不会堆积。

**跨重启的行为。** 停止工作负载会解除其探针 (健康状态读作空，而不是 `unhealthy`——已停止的容器并不是正在失败的容器) ，启动则会重新装载。值得说明的是 **cornus 服务器**重启这一情况: healthcheck 本身持久化在工作负载所在之处——`containerd` 上是容器 label，`bare` 上是 instance 记录，`incus` 上是 instance 配置——因此对仍在运行、或即将被 restart monitor 复活的 instance，探测会恢复。**不会**保留的是探针的*状态*: 每个重新装载的 instance 都会退回 `starting`，直到它的第一次探测返回，而 Docker 把自己的状态保存在比 cornus 活得更久的守护进程里。被显式停止的 instance 保持解除状态。`incus` 没有可以挂载这件事的启动 reconcile 流程，因此重新装载由第一次 `cornus deploy status` 或 `cornus deploy list` 触发——这已经足够，因为 `service_healthy` 正是靠轮询 status 来收敛的。

**健康状态是报告，不是控制。** 没有任何东西会因为容器变得不健康而重启它；没有编排器时 Docker 也不会这么做。[`restart` 策略](/zh/reference/deploy-spec)响应的是容器*退出*，那是另一回事。
