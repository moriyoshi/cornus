...
| `CORNUS_ADVERTISE_URL` | — | — | mount-agent / caretaker sidecar 回拨的 cornus URL。Kubernetes backend 的 client-local mount 必需；`dockerhost`/`containerd` 上通过 `CORNUS_DOCKER_REMOTE`/`CORNUS_CONTAINERD_REMOTE` 启用同一 sidecar 路径时同样必需。 |
...
| `CORNUS_ACTIVITY_MAX_BYTES` | — | `8388608` | 飞行记录日志 (`<数据目录>/activity`) 的大小上限，并在旁保留一份此前的世代。参见 [cornus activity](/zh/cli/activity)。 |
...
| `CORNUS_DEPLOY_BACKEND` | — | `dockerhost` | 部署后端: `dockerhost`、`kubernetes` (`k8s`) 、`containerd`、`bare` 或 `incus`。无法识别的值会导致启动错误。如果接受 `docker` 之类的近似值，就会在选择 `dockerhost` 的同时悄然让镜像仓库脱离主机原生重新导出。 |
...
| `CORNUS_INCUS_REMOTE` | — | off | 为与 `CORNUS_DOCKER_REMOTE`/`CORNUS_CONTAINERD_REMOTE`/`CORNUS_BARE_REMOTE` 对称而被接受，但该 backend 上的 caretaker companion 路径**尚未实现**: 不会创建 companion，mount 和端口转发行为也不会改变。它唯一的效果是服务器不再预先拒绝 `cornus exec --forward-agent`，随后由 backend 告警并忽略。请不要设置它。 |
