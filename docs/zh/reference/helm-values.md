...
此外，chart 始终创建 `<release>-cornus-installation`，并将其值作为 `CORNUS_INSTALLATION_SECRET` 传入。这个共享内部 key 使启用认证的构建和所有部署后端都能使用同机 registry；它本身不会启用客户端认证。Helm `lookup` 在 upgrade 和副本之间保留已安装的值。单独的 `helm template` 无法查询 cluster，因此每次 render 的 Secret 都包含新的随机值；该输出差异只是表面现象，不会轮换已安装的 Secret。

SSH 公钥同样为 opt-in。设置声明式密钥后，chart 将其作为 `CORNUS_AUTHORIZED_KEYS` 传入。多副本模式还会设置 `CORNUS_AUTH_KEYSTORE=none`，因为 StatefulSet 的每个副本都有独立 PVC，运行时注册无法一致地更新所有副本。

| Value | 默认值 | 说明 |
| --- | --- | --- |
| `auth.ssh.authorizedKeys` | `""` | 以换行分隔的 OpenSSH `authorized_keys` 条目。设置后启用 SSH 公钥客户端身份验证。多副本模式下这些条目仍然有效，但运行时注册被禁用。 |
...
| Value | 默认值 | 说明 |
| --- | --- | --- |
| `rbac.create` | `true` | 为 in-cluster Kubernetes deploy backend (包括带 Cornus label 的 registry pull Secret) 以及 `replicas > 1` 时 kube-native hub store 授予 RBAC (HubEndpoint CR、Lease、CRD self-install) 。Lease verb 同时覆盖 `gc.lease`。 |
