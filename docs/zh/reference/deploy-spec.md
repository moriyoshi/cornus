...
Compose 文件或 devcontainer 会在内部转换为相同的规范，因此这里的每个字段也都可通过 [`cornus compose`](/zh/cli/compose) 使用。六个后端: `dockerhost` (默认)、`podman`、`containerd`、`bare`、`incus` 和 `kubernetes`，都位于同一接口之后并遵循相同规范，但并非每个字段都能映射到每个后端。当源码记录了每后端行为时，会在字段描述中说明。`incus` 后端映射的字段子集最窄 (不支持 client-local mount、user network 以及仅有 command 的覆盖；`workingDir` 和 `user` 仅在绝对路径形式和数值形式下映射)；它会逐字段告警而不是静默丢弃，[部署后端](/zh/reference/deploy-backends#incus)集中列出了这些差距。
...
| `command` | []string | 否 | 镜像 `CMD` | 覆盖镜像的默认命令 (Docker `CMD`): 即传给仍然生效的镜像 `ENTRYPOINT` 的参数。在 kubernetes 上它会放入容器的 `Args`，从而保留镜像 entrypoint。在 `incus` 上，未同时设置 `entrypoint` 的 `command` 无法映射并会告警——请把 `entrypoint` 也设上。 |
| `entrypoint` | []string | 否 | 镜像 `ENTRYPOINT` | 覆盖镜像 entrypoint (Docker `ENTRYPOINT` / Kubernetes 容器 `command`)。设置后，`command` 提供其参数；空值保留镜像默认值。在 `incus` 上它会变成 instance 的 `oci.entrypoint`，整体替换镜像 argv。 |
...
| `credentials` | [CredentialSpec](#credentialspec) | 否 | — | 将客户端签发的短期凭据代理至工作负载。仅在 **kubernetes** 上实现；目前每个主机后端都会拒绝它。Compose service 用 `x-cornus-credentials:` 设置它。请参阅[凭据](/zh/guides/credentials)。 |
...
| `user` | string | 否 | 镜像默认值 | 进程运行时使用的用户 (及可选组): `uid`、`uid:gid`、`user` 或 `user:group`。kubernetes 仅映射**数值型** `uid[:gid]`，无法表示用户名；`incus` 有同样的限制 (`oci.uid`/`oci.gid`) ，并且会整体拒绝 `uid:groupname` 而不是丢掉组。 |
| `workingDir` | string | 否 | 镜像默认值 | 容器工作目录 (compose `working_dir`)。在 `incus` 上仅当为**绝对路径**时才映射 (`oci.cwd`) ；相对路径会告警。 |
...
| `origin` | [Origin](#origin) | 否 | — | 工作负载**来源**: 所属项目以及生成它的客户端主机 / 用户 / 目录 / Git 仓库。CLI 自动填充，服务器记录经过身份验证的 subject。通过 status/list 返回。 |
...
| `ingress` | [IngressSpec](#ingressspec) | 否 | — | 声明已发布工作负载端口的 HTTP(S) 主机 / 路径路由。Kubernetes 会创建原生 Ingress；主机后端由 cornus 服务器实现相同路由。请参阅 [Ingress](/zh/guides/ingress)。 |
| `knative` | [KnativeSpec](#knativespec) | 否 | — | 将工作负载部署为 Knative Serving Service (serverless、autoscaling、scale-to-zero)。仅在集群提供 `serving.knative.dev` 的 **kubernetes** 后端实现；其他情况下会警告并忽略 (工作负载作为普通容器运行)。通常由 `serving.knative.dev/v1` descriptor loader 填充——参见 [`cornus deploy`](/zh/cli/deploy)。 |
| `agentForward` | bool | 否 | `false` | 为此部署接入 caretaker `AgentRelayRole`，使 `cornus exec --forward-agent` / `cornus compose exec --forward-agent` 能够把本地 ssh-agent 中继进 exec 会话。**仅限 kubernetes**，按部署启用 (dockerhost/containerdhost 则改由后端范围的 `CORNUS_DOCKER_REMOTE` / `CORNUS_CONTAINERD_REMOTE` 控制；它们已经为每个部署运行每实例 companion) 。Compose 服务通过 `x-cornus-agent-forward: true` 设置。 |
| `telemetry` | [TelemetrySpec](#telemetryspec) | 否 | — | 在 caretaker 中运行内置 OpenTelemetry Collector，并自动把工作负载的 `OTEL_*` env 接到它上面。导出到 `endpoint`；未设置时导出到 cornus 自己的存储。适用于所有后端。Compose: `x-cornus-telemetry:` (service 或 project level)；CLI: `--telemetry-*`。参见[可观测性](/zh/guides/observability#工作负载遥测)。 |
...
工作负载的来源 (`origin`), 即部署来自哪里. CLI 从客户端环境填充 `subject` 之外的所有字段 (`cornus deploy` 记录工作目录, `cornus compose` 记录项目名称和 Compose 文件目录); 服务器会用经过身份验证的请求身份覆盖 `subject`, 并**丢弃客户端提供的值**, 从而将声明的来源与经过验证的身份分开. 所有字段均为尽力而为. 每个后端会将其持久化为 `cornus.origin.*` 容器标签 (dockerhost / containerd) 、记录字段 (bare) 或对象注解 (kubernetes), 并通过 [`cornus deploy`](/zh/cli/deploy) / status / list 返回.

| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `project` | string | 否 | — | Compose 项目名称或 `cornus deploy --project`。 |
| `host` | string | 否 | — | 客户端计算机主机名。 |
| `user` | string | 否 | — | 客户端操作系统用户。 |
| `directory` | string | 否 | — | 启动部署的客户端侧绝对目录。 |
| `git` | [GitOrigin](#gitorigin) | 否 | — | `directory` 的 Git 来源。 |
| `subject` | string | 否 | — | 由服务器记录的身份验证 ID (JWT subject)。 |

#### GitOrigin

来源 `directory` 的 Git 来源 (`origin.git`)，由客户端声明且尽力而为。

| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `remote` | string | 否 | — | `origin` 远程 URL。 |
| `branch` | string | 否 | — | 已检出的分支 (detached HEAD 时为空)。 |
| `commit` | string | 否 | — | 完整的 HEAD commit SHA。 |
| `dirty` | bool | 否 | `false` | 是否有未提交的更改。 |
...
| `noCreateHostPath` | bool | 否 | `false` | 拒绝自动创建缺失的调用方本地绑定源. 默认行为与 Compose `bind.create_host_path: true` 一致, 会创建空目录; Compose `bind.create_host_path: false` 会设置此字段. 对服务器主机挂载忽略. |
...
| `ip` | string | 否 | — | 以 CIDR 形式固定此成员在该网络上的 IPv4 地址 (例如 `10.222.14.7/24`)。仅在由 Multus 实现的 kubernetes 网络上生效。`dockerhost` 忽略它 (libnetwork 原生完成成员的地址分配与解析，且 CIDR 形式不是合法的 Docker endpoint 地址)；`containerd` 和 `bare` 会告警并忽略 —— 它们的 bridge CNI 使用无预留的 host-local IPAM，因此实例会取自动分配范围内的下一个空闲地址。 |
...
在 caretaker 中运行内置 OpenTelemetry Collector (Compose 的 service 或 project level `x-cornus-telemetry:`, CLI 的 `--telemetry-*`). 应用将 OTLP 发送到 pod-loopback receiver, Collector 导出到 `endpoint`; 未指定时则导出到 cornus 自己的[内置可观测性存储](/zh/guides/observability#内置存储). 后端自动注入工作负载的 `OTEL_*` env. 适用于所有后端. 参见[可观测性](/zh/guides/observability#工作负载遥测). 需要启用 collector 的镜像 (`-tags otelcol`, 发布镜像已设置).

| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `enabled` | bool | 否 | `false` | 开启 telemetry. 非空 `endpoint` 也会开启; 仅使用 `x-cornus-telemetry: {}` 即足以启用. |
| `endpoint` | string | 否 | 服务器自身的存储 | 要导出到的 OTLP 后端 (`grpc` 使用 `host:port`, `http/protobuf` 使用 URL). **留空会导出到 cornus 的[内置可观测性存储](/zh/guides/observability#内置存储)**, 由服务器在部署时填入. 如果部署路径没有可作为默认值的存储 (本地 `cornus deploy`, 或未使用 `--obs` 启动的服务器), 空 endpoint 会被拒绝, 并显示说明两种补救方法的消息. |
| `protocol` | string | 否 | `grpc` | 导出协议: `grpc` 或 `http/protobuf` (同时选择向应用公布的 receiver port: 4317 或 4318). |
| `headers` | map[string]string | 否 | — | 静态导出 header (例如身份验证 token). 在 **kubernetes** 上通过 Deployment 拥有的 Secret 和 `secretKeyRef` 投影, 因此 pod spec 中不会出现值. |
...
| `viaMux` | bool | 否 | cornus 为目的地时开启 | 通过 caretaker 连接将导出传送到 cornus 服务器, 而不是从工作负载网络直接拨号. 在运行 telemetry caretaker 的每个后端上, 只要 endpoint 解析为此服务器, 就默认开启; 设置 `false` 可强制直接拨号. 显式指定第三方 `endpoint` 时保持关闭, 因为没有可复用的连接. 需要 `CORNUS_ADVERTISE_URL`. Compose: `via_mux`. 参见[可观测性](/zh/guides/observability#它通过-cornus-连接传输). |
...
将客户端来源的凭据代理至工作负载 (`credentials`)。密钥值由客户端签发 (绝不携带在本 spec 中)，并通过 cornus 服务器和 caretaker sidecar 交付。在 **kubernetes** 后端上，通过客户端在工作负载整个生命周期内保持的会话实现；`cornus deploy --detach` 和所有主机后端 (`dockerhost`、`containerd`、`bare`、`incus`) 都会拒绝它。Compose service 用 `x-cornus-credentials:` (service 或项目级) 设置它，并且同样支持 `cornus compose up -d`——由项目的后台 agent 持有会话。请参阅[凭据](/zh/guides/credentials)。
| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
...
| `backend` | string | 是 | — | 签发凭据的客户端侧后端 (例如 `aws-sts`、`github-cli`、`static`、`exec`)。在调用方机器上使用调用方自身的云/API 凭据运行。 |
...
| `deliveries` | [][CredentialDelivery](#credentialdelivery) | 否 | — | 容器使用凭据的方式。空值有效 (可获取但不会呈现)。 |
...
| `provider` | string | 否 | `generic` | **endpoint 类型。** `generic` 提供 cornus 原生 JSON 协议 (`GET /credentials/<name>`)；`aws-imds` 和未来适配器会将同一凭据渲染成云 SDK 预期的格式；注入认证信息的代理 (`anthropic-proxy`、`openai-proxy`、`github-proxy`) 自行持有凭据，并把它加到工作负载的 API 调用上。 |
...
| `upstream` | string | 否 | provider 默认值 | **endpoint 类型、auth-proxy 提供方。** 覆盖代理转发的供应商 API (例如兼容 Anthropic/OpenAI 的网关，或 `https://ghe.corp/api/v3` 这样的 GitHub Enterprise Server REST 基址)。不含密钥。 |
...
容器健康探测 (`healthcheck`)，以 Docker healthcheck 为模型。`test` 使用 Docker 的 `CMD` 形式: 第一个元素是 `CMD` (执行其余元素)、`CMD-SHELL` (通过 shell 运行单个字符串) 或 `NONE` (禁用任何继承的 healthcheck)。

每个后端都支持它，走三条路径之一: `dockerhost`/`podman` 把它交给守护进程，`kubernetes` 把它转成 exec liveness (及 readiness) probe，而 `containerd`/`bare`/`incus` 没有自己的探针引擎，因此由 cornus 自行运行探针。三者的状态机都是 Docker 的——规则、默认值，以及 cornus 重启对正在进行的探针有何影响，见[健康检查](/zh/reference/deploy-backends#健康检查)。
...
::: warning kubernetes
`startInterval` 是 kubernetes 后端唯一无法表达的字段——Kubernetes 的 Probe 只有一个 period，start period 期间用的也是它——它会告警而不是静默丢弃。运行 cornus 自有探针引擎的后端支持它。
:::
...
| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `mode` | string | 否 | `env` | `env` 将 `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` / `ALL_PROXY` 传入容器 (所有后端, 无中继). `proxy` 让 caretaker 运行经服务器中继的 HTTP CONNECT + SOCKS5 转发代理; `transparent` 通过 nftables redirect 捕获并中继所有出站 TCP. 中继模式在 Kubernetes sidecar 或主机后端 companion caretaker 中运行; `incus` 尚不支持. |
...
声明已发布工作负载端口的 HTTP(S) 主机 / 路径路由 (`ingress`)。Kubernetes 会在 `ClusterIP` Service 前创建原生 Ingress。`dockerhost`、`containerd`、`bare` 和 `incus` 由 cornus 服务器实现相同路由。必须发布至少一个端口。请参阅 [Ingress](/zh/guides/ingress)。
| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
...
| `tunnel` | [IngressTunnelOpt](#ingresstunnelopt) | 否 | — | 前台远程部署成功后，通过公共隧道发布此 ingress。 |
...
前台 `cornus deploy --server` 成功后，通过服务器的隧道提供方发布 ingress。服务器在 apply 时不处理此块，凭据保留在客户端侧。`--detach` 和本地部署不会启动它，它只在前台会话期间持续存在。凭据文件来自所选连接配置文件的 `tunnel` 块；未设置时回退到 `NGROK_AUTHTOKEN`。可选隧道发布失败不会使成功的部署变为失败。

| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
| `enabled` | bool | 否 | `false` | 部署后发布。 |
| `hostMode` | string | 否 | 配置文件默认值，然后是 `auto` | `auto`、`passthrough`、`alias` 或 `rewrite`。 |
| `host` | string | 否 | 自动选择 | 有多个 ingress 时要发布的主机。 |
...
| `minScale` | int | 否 | `0` | autoscaling 下限 (`autoscaling.knative.dev/minScale`). `0` 允许 scale-to-zero. |
| `maxScale` | int | 否 | `0` | autoscaling 上限 (`autoscaling.knative.dev/maxScale`). `0` 表示无限制. |
| `target` | int | 否 | — | 每个 replica 的 autoscaling 目标 (`autoscaling.knative.dev/target`): 并发请求数, 或 `rps` 指标下的每秒请求数. |
| `concurrency` | int | 否 | `0` | 每个 replica 的并发请求硬上限 (revision `containerConcurrency`). `0` 表示无限制. |
...
| `timeoutSeconds` | int | 否 | `300` | 单个请求的最长时间 (revision `timeoutSeconds`). |
| `port` | int | 否 | first published | Knative 路由到的单一容器端口. 非零值必须与一个已发布端口匹配. |
| `annotations` | map[string]string | 否 | — | 合并到 revision template, 以提供以上字段之外的 autoscaling 参数 (冲突时以上字段优先). |
## 另请参阅
