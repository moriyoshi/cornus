...
Compose 文件或 devcontainer 会在内部转换为相同的规范，因此这里的每个字段也都可通过 [`cornus compose`](/zh/cli/compose) 使用。五个后端: `dockerhost` (默认)、`containerd`、`bare`、`incus` 和 `kubernetes`，都位于同一接口之后并遵循相同规范，但并非每个字段都能映射到每个后端。当源码记录了每后端行为时，会在字段描述中说明。`incus` 后端映射的字段子集最窄 (不支持 mount、volume、user network、healthcheck 以及 command/entrypoint 覆盖)；它会逐字段告警而不是静默丢弃，[部署后端](/zh/reference/deploy-backends#incus)集中列出了这些差距。
...
| `knative` | [KnativeSpec](#knativespec) | 否 | — | 将工作负载部署为 Knative Serving Service。仅在支持它的 **kubernetes** 后端实现。 |
...
| `telemetry` | [TelemetrySpec](#telemetryspec) | 否 | — | 在 caretaker 中运行 OpenTelemetry Collector，并自动设置工作负载的 `OTEL_*` env。适用于所有后端。 |
...
将客户端来源的凭据代理至工作负载 (`credentials`)。密钥值由客户端签发，并通过 cornus 服务器和 caretaker sidecar 交付。在前台 **kubernetes** 会话中实现；`--detach` 和所有主机后端 (`dockerhost`、`containerd`、`bare`、`incus`) 都会拒绝它。请参阅[凭据](/zh/guides/credentials)。
| 字段 | 类型 | 必填 | 默认值 | 说明 |
| --- | --- | --- | --- | --- |
