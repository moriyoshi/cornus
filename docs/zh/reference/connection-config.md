...
描述用于访问远程 Docker/containerd 主机上 cornus 服务器的 SSH 连接。配置后，普通命令会透明使用它，无需逐个命令指定隧道 flag。`addr` 可以是 `ssh_config` 主机 alias，因此除非 `no-ssh-config` 将其禁用，否则会继续应用常规的用户、端口、身份、代理和主机密钥设置。
