...
| --- | --- | --- | --- |
| `CORNUS_ADDR` | `--addr` | `:5000` | HTTP listen address for `/v2/*` and `/.cornus/v1/*`. All interfaces by default, because a containerized caretaker dials back to the server; set `CORNUS_ADDR=127.0.0.1:5000` to restrict it to this machine when workloads do not need to reach it. See [Listen address and exposure](/cli/serve#listen-address-and-exposure). |
...
| `CORNUS_ADVERTISE_URL` | — | — | The cornus URL a mount-agent / caretaker sidecar dials back to. Required for client-local mounts on the kubernetes backend, and on `dockerhost`/`containerd` when `CORNUS_DOCKER_REMOTE`/`CORNUS_CONTAINERD_REMOTE` opts into the same sidecar path. It is demanded only when something actually dials back: a co-located server realizing client-local mounts on its own host, or an [`env`-kind credential](/guides/credentials) it resolves at deploy time, needs neither this nor `CORNUS_AGENT_IMAGE`. |
...
| `CORNUS_HOST_NETWORK` | — | auto-detected | `1`/`0`: states outright whether this server's container shares the host's network namespace, overriding what self-inspection concluded. On the CNI-based host backends (`containerd`, `bare`) an isolated namespace means published ports are NAT'd inside the server's own container and are invisible on the host, so a server that DISCOVERS one refuses to start — set `0` to acknowledge that and serve anyway, or `1` when the server already has host networking but cannot tell. `bare` has no daemon to ask, so this is its only answer. A malformed value is a startup error. See [Running the server in a container](/guides/server-in-a-container). |
| `CORNUS_REPLICA_ID` | — | — | Stable identity for this replica; used by the distributed hub store and the GC leader gate, and as both `sub` and `kid` in automatic peer-forward JWTs. |
| `CORNUS_DEPLOY_BACKEND` | — | `dockerhost` | Deploy backend: `dockerhost`, `podman`, `kubernetes` (`k8s`), `containerd`, `bare`, or `incus`. An unrecognized value is a startup error — a near-miss such as `docker` would otherwise select `dockerhost` while silently dropping the registry out of host-native re-export. |
...
| `CORNUS_INSTALLATION_SECRET` | — | generated under `CORNUS_DATA` | Installation-wide internal signing key. When client auth is enabled, Cornus uses it only to mint short-lived, scoped credentials for its own registry build/pull traffic. Set the same value on every replica; it does not enable client auth by itself. |
| `CORNUS_AUTH_KEYSTORE` | — | automatic | SSH-key enrollment store mode: `file` writes `<CORNUS_DATA>/auth/authorized_keys`; `none` disables runtime enrollment while retaining declarative keys. With no value, the file store activates only when key auth is otherwise configured or the store already exists. |
| `CORNUS_AUTHORIZED_KEYS` | — | — | Newline-separated OpenSSH `authorized_keys` entries. Setting it enables SSH-public-key client authentication; these declarative entries are read-only through the API. |
...
| `CORNUS_JWT_SCOPE_MAP` | — | — | Path to a YAML/JSON scope map: rules that turn a third-party issuer's claims into a cornus scope. Required for JWKS-verified tokens, which carry no cornus `scope` claim. See [scope mapping](/guides/security#map-third-party-claims-to-cornus-scopes). |
| `CORNUS_JWT_DEFAULT_SCOPE` | — | — | Catch-all scope for tokens no `CORNUS_JWT_SCOPE_MAP` rule matched (e.g. `api`). Appended behind the map, so explicit rules keep deciding. |
...
| `CORNUS_PODMAN_SOCKET` | — | none | The Podman API endpoint for the `podman` backend: a path, a `unix://`/`tcp://` URL, or an `ssh://` destination. There is **no default** and no probing of `CONTAINER_HOST`/`DOCKER_HOST`/stock paths — with neither this nor `CORNUS_PODMAN_SERVICE` set, the server refuses to start, so which daemon it drove is always answerable from the configuration. Setting both is an error. |
| `CORNUS_PODMAN_SERVICE` | — | off | Makes cornus run `podman system service` itself on a private socket and supervise it, so no `podman.socket` unit need be enabled. Needs only the `podman` binary on `PATH`. Mutually exclusive with `CORNUS_PODMAN_SOCKET`. |
| `CORNUS_PODMAN_REMOTE` | — | off | Opts the `podman` backend into the always-on per-instance remote companion, exactly as `CORNUS_DOCKER_REMOTE` does for `dockerhost`. **Required for `cornus port-forward`/`cornus tunnel` against a ROOTLESS podman**, whose workload namespaces are not routable from the host — without it those commands refuse immediately rather than timing out. Needs `CORNUS_AGENT_IMAGE` and `CORNUS_ADVERTISE_URL`. |
...
| `CORNUS_CONTAINERD_REMOTE` | — | off | Opts the `containerd` backend into the same always-on per-instance remote-companion sidecar as `CORNUS_DOCKER_REMOTE`, joining each instance's pinned network namespace, whether or not the deploy uses `--mount` (a companion container/task performs the kernel 9P mount, relayed via a shared host directory with `rshared`/`rslave` OCI mount options, and the companion also reroutes `cornus port-forward`/`cornus tunnel`/`cornus exec --forward-agent`). Does **not** make containerd itself remote-reachable (its client dialer is unix-socket-only) — it changes how port-forward/exec-agent-forwarding are realized on an otherwise still-co-located daemon, and it is what makes client-local mounts available at all on this backend: unlike `dockerhost`/`bare` there is no kernel-9p fast path to fall back on, so with this unset a deploy carrying `--mount` is refused (naming this variable). Needs `CORNUS_AGENT_IMAGE` and `CORNUS_ADVERTISE_URL`. See [deploy backends](/reference/deploy-backends). |
| `CORNUS_DOCKER_SOCK` | — | `$XDG_RUNTIME_DIR/cornus-docker.sock` | Unix socket the CLIENT-side [`cornus daemon docker`](/cli/daemon) proxy LISTENS on. It does **not** configure the `dockerhost` backend, which reads `DOCKER_HOST`. |
...
| `CORNUS_INCUS_STORAGE_POOL` | — | `default` | Incus storage pool the backend creates custom volumes in: a deployment's managed `volumes`, and in remote mode the companion's shared agent volume. A deployment with neither touches no pool at all — instances take their root disk from the project's profile. |
...
| `CORNUS_INCUS_REMOTE` | — | off | Turn on the caretaker-companion path, as `CORNUS_DOCKER_REMOTE`/`CORNUS_CONTAINERD_REMOTE`/`CORNUS_BARE_REMOTE` do for their backends: every replica gets a companion instance carrying the PortForward and AgentRelay caretaker roles, port-forward traffic is rerouted through it instead of dialing the instance address, and `cornus exec --forward-agent` becomes available (it is refused up front without this). Needs `CORNUS_AGENT_IMAGE` and `CORNUS_ADVERTISE_URL`; a deploy fails up front if either is missing. It does not bring client-local mounts or egress — see [deploy backends](/reference/deploy-backends#incus). |
...
With client auth enabled, selecting either distributed store also enables
automatic peer-forward credentials. Each replica keeps a private ECDSA P-256
key at `$CORNUS_DATA/peer.key`, publishes only its public key under the store's
existing heartbeat/Lease lifetime, and sends a five-minute `peer`-scoped ES256
JWT to the three `/.cornus/v1/*/forward` endpoints. No additional env variable
is required. When `CORNUS_AUTH_TOKEN` is set, that legacy shared token keeps
absolute precedence for mixed-version rolling upgrades.
...
| `CORNUS_TOKEN_CACHE` | `auto` | Where the CLI keeps short-lived credentials (minted SSH-key sessions, exchanged tokens): `auto` (the OS keyring, falling back to a file), `keyring`, `file`, or `none`. `none` mints afresh every invocation — set it where a credential at rest is unacceptable. The file backend lives under `$XDG_RUNTIME_DIR/cornus/tokens` (0600 in a 0700 directory), which is tmpfs-backed and cleared at logout. |
...
| `CORNUS_GH_BIN` | `gh` on PATH | Path to the GitHub CLI the `github-cli` [credential](/guides/credentials) source runs. Read on the machine that holds the deploy session, since that is where the token is minted. An explicit `config.command` in the spec still wins. |
