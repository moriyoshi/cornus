...
Independently, the chart always creates `<release>-cornus-installation` and
passes its value as `CORNUS_INSTALLATION_SECRET`. This shared internal key lets
authenticated builds and all deploy backends use the co-located registry; it
does not turn client authentication on. Helm `lookup` preserves the installed
value across upgrades and replicas. A bare `helm template` cannot query the
cluster, so its rendered Secret contains a fresh random value on every render;
that output difference is cosmetic and does not rotate an installed Secret.

SSH public keys are also opt-in. When declarative keys are set, the chart passes
them as `CORNUS_AUTHORIZED_KEYS`. For multiple replicas it additionally sets
`CORNUS_AUTH_KEYSTORE=none`, because each StatefulSet replica has a separate PVC
and runtime enrollment could not update every replica consistently.

| Value | Default | Description |
| --- | --- | --- |
| `auth.ssh.authorizedKeys` | `""` | Newline-separated OpenSSH `authorized_keys` entries. Setting it enables SSH-public-key client authentication. In multi-replica mode the entries remain active but runtime enrollment is disabled. |
...
| Value | Default | Description |
| --- | --- | --- |
| `rbac.create` | `true` | Grant RBAC for the in-cluster kubernetes deploy backend, including Cornus-labelled registry pull Secrets, and, when `replicas > 1`, the kube-native hub store (HubEndpoint CRs, Leases, CRD self-install). The Lease verbs also cover `gc.lease`. |
