...
Server-side fallbacks for workloads that opt into [ingress](/topics/ingress) (deploy spec `ingress:` / Compose `x-cornus-ingress:`). Leave every field empty (the default) to require each workload to specify its own host, so nothing is auto-exposed.
...
| `tls.clientCA` | `false` | Verify client certs (mTLS) using `ca.crt` from the Secret. A verified cert's CommonName becomes the caller identity (see `CORNUS_API_POLICY` in [Auth and TLS](/topics/auth-and-tls)). |
...
Opt-in JWT verification for the server API (the kube-auth turnkey path). Each set value renders the matching `CORNUS_JWT_*` env; leaving all empty renders nothing (auth stays off unless configured elsewhere). See [Auth and TLS](/topics/auth-and-tls).
...
Setting `replicas > 1` switches the workload-to-workload [hub](/topics/hub)
