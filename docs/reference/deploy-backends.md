...
Client-sourced credentials need **no companion at all** on the host backends — and therefore neither `CORNUS_ADVERTISE_URL` nor `CORNUS_AGENT_IMAGE` — because a co-located server can do each delivery itself. An [`env` delivery](/guides/credentials) is resolved at deploy time and set in the container's environment at create; a `file` is rendered under the server's own data dir and bound read-only; an `endpoint` is a listener the server binds *inside the workload's network namespace*, which keeps the kubernetes security model (the namespace boundary is the authorization) without the sidecar that used to provide it. One gap remains. Where a runtime remaps ids into a user namespace (rootless `podman`), cornus asks it for the mapping and owns the credential file as the ids the workload will actually read it with, so a non-root workload reads its own credential; the data directory becomes traversable (`0711` — walkable, not listable) on those runtimes, while the secrets in it stay `0600`. The exception is `file` on `incus`, refused for a timing reason rather than a permissions one: incus records an instance's id map ON THE INSTANCE, and a credential file must be written before the instance exists, since it reaches the workload as a disk device in the create request — and the daemon exposes no id-map base of its own to ask beforehand. `env` and `endpoint` work there. `incus` reaches all of this WITHOUT being an AttachingBackend at all: its companion is a sibling instance that cannot carry mounts or egress, but nothing about that stops the server realizing a credential itself. All of them are declined in remote mode, where the runtime's paths and process ids are not this server's to resolve.
...
**Known gaps vs `dockerhost`.** Beyond the log and stats caveats above, the incus backend does not map: a **command-only** override, `healthcheck`, client-local 9P bind mounts, Compose user `networks`, and `knative`. `healthcheck` is permanent rather than pending — nothing in Incus exposes an instance-level probe, so a workload that goes unhealthy without exiting keeps being reported as running.
