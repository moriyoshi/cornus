...
The four host backends honor the same core spec fields (`name` / `image` / `replicas` / `restart` / `env` / `ports`), and `dockerhost` / `containerd` / `bare` additionally share the client-local 9P bind mounts, Compose user networks, and published-port forwarding, so the same workflow moves across those three unchanged. `incus` is the outlier: it maps the core fields but not mounts, volumes, user networks, healthchecks, or command/entrypoint overrides — it warns per field rather than dropping them silently. Where an individual spec field maps onto only some backends, the [deploy spec reference](/reference/deploy-spec) says so per field.
...
On the host backends (`dockerhost`, `containerd`, `bare`), everything a workload needs running *beside* it — client-local mounts, client-side egress, and, in remote mode, the port-forward rerouting and ssh-agent relaying described below — is realized by **one companion `cornus caretaker` container per replica**, sharing that replica's network namespace. So a single deploy can combine client-local mounts with client-side egress. Client-sourced credentials are the one attachment the host backends do not realize yet; they remain `kubernetes`-only.
...
- **[`cornus port-forward`](/cli/port-forward) and [`cornus tunnel`](/cli/tunnel)** dial the instance's own routable IPv4 (from its instance state) directly, for both TCP and UDP — no companion sidecar involved.
...
**Known gaps vs `dockerhost`.** Beyond the log and stats caveats above, the incus backend does not currently map: `command` / `entrypoint` overrides (the image's own entrypoint runs), `user`, `workingDir`, `mounts` (including client-local 9P bind mounts), managed `volumes`, `healthcheck`, Compose user `networks`, and `knative`. Each of these logs a warning naming the field when it is set, so nothing is dropped silently. `CORNUS_INCUS_REMOTE` is accepted for parity with the other host backends' remote-companion flag, but the caretaker-companion path is **not yet implemented** here, so it realizes none of the mounts or port-forward rerouting the other backends get from it. Setting it has no observable effect at all: [`cornus exec --forward-agent`](/cli/exec) is still refused up front, since this backend declares the feature unavailable rather than letting the server infer it from the flag. Leave it unset.
