...
Privilege handling is **default-deny**: privileged containers and host bind mounts are refused unless explicitly allowed (`CORNUS_ALLOW_PRIVILEGED`, `CORNUS_ALLOW_BIND_SOURCES`). See [Auth and TLS](/topics/auth-and-tls).
...
Note this is distinct from **workload** privilege, which is default-deny regardless of how the server runs: privileged containers and host bind mounts are refused unless explicitly allowed (`CORNUS_ALLOW_PRIVILEGED`, `CORNUS_ALLOW_BIND_SOURCES`; see [Auth and TLS](/topics/auth-and-tls)).
...
- [Remote workflows](/topics/remote-workflows) — driving the kubernetes backend from a laptop.
