...
| `credentials` | [CredentialSpec](#credentialspec) | no | — | Brokers short-lived client-minted credentials into the workload. Realized on **kubernetes** only; every host backend currently rejects them. See [Credentials](/guides/credentials). |
...
Brokers client-sourced credentials into a workload (`credentials`). The secret value is minted on the client (never carried in this spec) and delivered through the cornus server and the caretaker sidecar. Realized on **kubernetes** over a foreground `cornus deploy --server` session. `--detach` and every host backend (`dockerhost`, `containerd`, `bare`, and `incus`) reject it. See [Credentials](/guides/credentials).
