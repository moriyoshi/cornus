...
`--allow-non-loopback`, with `--allow-host` extending the Host allow-list. The browser
deliberately does not apply projects; `cornus compose up` remains the deployment front door.
...
- **Profiles** are kubeconfig-style contexts managed by `cornus config`:
  ...
  key-auth or kube-auth block. One resolver threads the selected context into every client
  command, with two precedence chains: explicit flag > context server > auto
  port-forward for the endpoint, and `CORNUS_TOKEN` > SSH-key session > kube-auth mint > static
...
- **Kube-auth**: a profile can mint a short-lived, audience-scoped
  ...
  server.
- **SSH-key auth**: a profile stores only an identity-file path or ssh-agent
  fingerprint. After enrollment, the CLI proves possession through the already
  resolved TLS/tunnel transport and receives a short-lived scoped session. The
  interactive client caches it privately for the background agent; neither the
  private key nor minted token enters the durable profile.
...
- [Compose, devcontainers & docker](/guides/compose-devcontainers-docker) —
  the workflows.
- [Working with remote clusters](/guides/remote-clusters) — profiles and kube-auth setup.
