...
[backends table](/guides/tunnels#backends) for what each needs and the
...
[ingress](/guides/ingress) for the full field reference.
...
[networking and conduits](/guides/networking) for configuration and precedence.
...
### Forwarding mounts and credentials

The mount relay and the credential relay ride that same forwarding path — and
the same routing record. A deploy-attach session with client-side attachments
publishes one record under a reserved `cornus.mount/<digest>` name (the session
id's digest only, since the raw id is a capability), and a caretaker stream that
lands on the wrong replica is spliced to the owner through
`GET /.cornus/v1/mount/forward` or `GET /.cornus/v1/cred/forward`. There is
deliberately no separate credential record: one deploy-attach session owns both
a pod's mounts and its credentials, so credential routing inherits the mount
record's TTL / Lease liveness and crash GC instead of duplicating them under a
second lifecycle to keep in sync.

Authorization for those two hops is enforced at the **owner** replica. The owner
re-resolves the session in its own local registry and re-checks the requested
mount or credential name against that live session's allow-list; the forwarding
replica holds no session state, is never consulted, and cannot vouch for a name,
so a forged forward cannot bypass the check. This is the reverse of the hub
delivery forward, where reach policy is enforced by the forwarder and trusted by
the owner — reach policy keys on caller identity, which only the forwarder
authenticated, whereas a credential's authority is a per-session capability that
only the owner holds. Neither handler ever re-forwards, so a stale record cannot
form a loop.

## Related pages

- [Networking and conduits](/guides/networking) — conduit modes plus recipes for
  forwarding and reaching workloads.
- [Tunnels](/guides/tunnels) — tunnel backends and per-backend setup.
- [The workload hub](/guides/hub) — the overlay's relay model and usage.
- [Ingress](/guides/ingress) — the ingress field reference.
- [Working with remote clusters](/guides/remote-clusters) — connection profiles.
