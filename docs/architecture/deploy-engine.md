...
| `incus` | The Incus daemon REST API over its unix socket, via the official Go client | Runs OCI images as Incus **application containers** (Incus 6.3+). Incusd owns the pull (through `skopeo`/`umoci` on its own host), the instance, and its networking; published ports become `proxy` devices. Linux-only. Deliberately the narrowest backend — see the gaps below. |
...
- **The record store replaces the metadata DB.**
  `<DataDir>/bare/records/<id>/record.json` (atomic writes) holds
  image/snapshot/IPs/ports/policy plus the
...
The `incus` backend (`CORNUS_DEPLOY_BACKEND=incus`) sits at the opposite end
from `bare`: instead of cornus owning more of the stack, it owns *less* than on
any other backend. Incus 6.3+ can run an OCI image as an **application
container**, so cornus hands incusd an image reference and a config map and lets
the daemon own the pull, the rootfs, the instance, and its networking. It talks
the Incus REST API over the local unix socket via the official Go client
(`github.com/lxc/incus/v6/client`), and is Linux-only like the other two host
backends.

- **The `incusConn` seam.** `incus.InstanceServer` is a ~100-method interface
  whose mutating calls return async `Operation`s. The backend never touches it
  directly: it goes through a narrow internal interface whose methods return
  already-waited plain values (the real adapter runs `Operation.Wait`), with exec
  the one exception that returns the live operation because it needs the
  control-channel websocket. A fake implementation drives every unit test, so
  `go test ./...` needs no live incusd. Missing instances are detected with
  Incus's typed 404 check rather than string matching, and mapped to the shared
  not-found error.
- **Everything arbitrary lives under `user.*`.** Incus only accepts free-form
  config keys in that namespace, so cornus's identity and provenance labels are
  stored as `user.cornus.managed` / `user.cornus.app` / `user.cornus.origin.*`
  (plus Compose `labels:`), while env goes to `environment.*`, resource caps to
  `limits.*`, and restart policy to the boolean `boot.autorestart` — which is why
  `on-failure:N` loses its attempt cap here, as it does on `containerd`.
- **Apply recreates**, because Incus refuses to delete a running instance: stop,
  delete, then create `Replicas(spec)` fresh instances with `Start: true`.
  Published ports become `proxy` devices on **replica 0 only**, the same
  one-DNAT-target-per-host-port contract the other backends follow.
- **The data plane is what the daemon can express, and no more.** Logs are the
  instance console — one raw PTY stream with no timestamps and no stdout/stderr
  split — so `--since`/`--follow`/`--tail` are warned about and cannot be
  honored; stats come from `GetInstanceState` through the shared Docker-stats
  encoder but carry no host CPU total, so CPU% reads low; `cp` rides the instance
  file API, which reports neither size nor symlink target; `ForwardPort` splices
  to the instance's own routable IPv4 (UDP included, unlike kubernetes); and
  **attach is a deliberate unsupported error**, since Incus offers a PID-1
  console rather than docker-attach stream semantics. Exec is fully supported,
  with the TTY size remembered across the exec-create/control-channel race.
- **Deliberately unmapped**, each with a per-field warning rather than a silent
  drop: command/entrypoint overrides, `user`, `workingDir`, mounts, volumes,
  healthchecks, user networks, and knative. The caretaker-companion optional
  interfaces (client-local mounts, egress) are not implemented, so unlike
  `containerd`/`bare` this backend simply does not advertise those capabilities.
- **Version pinning.** Incus `v6.19.0+` requires `runtime-spec v1.3.0`, which
  breaks the vendored containerd `oci` package, so the module pins incus
  `v6.18.0`; bumping it further means bumping containerd first. The *daemon* may
  of course be newer — CI exercises this backend against Incus 7.x.
...
Compose `networks:` are supported on four of the five backends. On **dockerhost**
they are native Docker networks (create-if-absent, member-less managed networks
reaped on delete). On **containerd** and **bare** they are the generated CNI
bridges above, with names resolving via hosts-file sync. **incus** is the
exception: instance networking belongs to incusd, so a spec carrying user
networks deploys onto the daemon's own network with a warning naming the field.
On **kubernetes** the compose `driver` selects a provider pipeline:
