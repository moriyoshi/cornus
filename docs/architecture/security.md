...
caller identity on the request context for authorization. Client-token issuance
(`cornus token issue`) is an operator/CLI action, not a general HTTP minting
endpoint. Separately, an installation-wide signing key exists only while client
auth is enabled: Cornus uses it to mint short-lived `registry:push` and
`registry:pull` credentials for its own build/deploy dataplane. Those credentials
are accepted before operator verifiers, marked internal in request context, and
issued only for known co-located registry hosts. Kubernetes stores a 12-hour
pull credential in the ownerless namespace `cornus-registry-pull` Secret and a
supervised loop refreshes it every four hours; host backends mint per pull.
Kubernetes caretaker sidecars get a **scoped**
...
SSH-public-key client authentication is opt-in. When configured, the server
offers purpose-bound SSH signing challenges for enrollment and session minting;
SHA-1 RSA signatures are rejected. Enrollment rotates a `0600` one-time secret and
stores public keys only. Session proof returns a short-lived scoped JWT with the
enrolled name as its identity. Writable enrollment is for one replica;
multi-replica deployments use declarative `CORNUS_AUTHORIZED_KEYS` and
`CORNUS_AUTH_KEYSTORE=none`.
