...
| [`cornus ingress-tunnel`](/ja/cli/ingress-tunnel) | デプロイメントまたは Compose プロジェクトで宣言した ingress を単一のパブリックトンネルで公開します。 |
| [`cornus config`](/ja/cli/config) | リモート cornus サーバーへ到達するための接続プロファイル (コンテキスト) を管理します。 |
| [`cornus setup`](/ja/cli/setup) | サーバー接続を対話形式で設定して検証します。 |
| [`cornus auth`](/ja/cli/auth) | 短時間有効なクライアントセッション用の SSH 公開鍵を登録して管理します。 |
| [`cornus compose`](/ja/cli/compose) | Compose / devcontainer プロジェクト向けの Docker Compose 互換クライアントです。 |
| [`cornus web`](/ja/cli/web) | ブラウザー UI とクライアント側 BFF を提供します (既定はループバック)。 |
| [`cornus daemon`](/ja/cli/daemon) | Docker API フロントエンドと統合バックグラウンドエージェントの制御です。 |
| [`cornus hub`](/ja/cli/hub) | ワークロード間オーバーレイに spoke として参加します。 |
| [`cornus activity`](/ja/cli/activity) | サーバーのアクティビティフライトレコードを読み取ります。 |
| [`cornus storage`](/ja/cli/storage) | レジストリストレージを調査して移行します。 |
| [`cornus observe`](/ja/cli/observe) | 保存済みのワークロードログ、トレース、メトリクスを照会します。 |
