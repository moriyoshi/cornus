...
| メトリクス | 単位 | ラベル | 意味 |
|---|---|---|---|
| `container_cpu_time` | 秒 | `cornus_replica`, `cpu_mode` | 累積 CPU 時間。`rate()` で使います。 |
...
| `container_disk_io` | バイト | `cornus_replica`, `disk_io_direction` | 累積ブロック I/O。Kubernetes では利用できません。 |
| `cornus_container_memory_limit` | バイト | `cornus_replica` | 設定されている場合の適用中の上限。 |
| `cornus_container_pids` | 個数 | `cornus_replica` | プロセスとスレッド。 |
