...
リモートの Docker/containerd ホスト上の cornus サーバーへ接続するための SSH 接続を記述します。設定後は通常のコマンドが透過的にこれを使うため、コマンドごとのトンネルフラグは不要です。`addr` には `ssh_config` のホスト alias を指定できます。`no-ssh-config` で無効にしない限り、通常のユーザー、ポート、ID、プロキシ、ホストキーの設定が適用されます。
