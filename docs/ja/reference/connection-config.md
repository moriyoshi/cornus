...
| `conduit` | [Conduit](#conduit) | ポート転送 | クライアントセッションがデプロイメントのポートを呼び出し元に公開する方法。`CORNUS_CONDUIT` env var と `--conduit` フラグより低い、最下位 precedence レイヤーです。[リモート workflows](/ja/topics/remote-workflows) を参照してください。 |
...
mTLS と bearer 認証のサーバー側については [Auth and TLS](/ja/topics/auth-and-tls) を参照してください。
...
- [リモート workflows](/ja/topics/remote-workflows) - conduit、ポート転送、リモートサーバーの操作。
- [Auth and TLS](/ja/topics/auth-and-tls) - bearer トークン、mTLS、cluster-minted ID。
