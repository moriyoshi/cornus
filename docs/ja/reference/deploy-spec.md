...
| `knative` | [KnativeSpec](#knativespec) | no | — | ワークロードを Knative Serving Service としてデプロイします。対応する **kubernetes** バックエンドでのみ実現されます。 |
...
| `telemetry` | [TelemetrySpec](#telemetryspec) | no | — | caretaker 内で OpenTelemetry Collector を実行し、ワークロードの `OTEL_*` env を自動設定します。全バックエンド対応。 |
...
caretaker 内で組み込み OpenTelemetry Collector を実行します (Compose の service または project level の `x-cornus-telemetry:`、CLI の `--telemetry-*`)。アプリは pod-loopback receiver へ OTLP を送り、Collector は `endpoint`、または未指定時には cornus 自身の[組み込みオブザーバビリティストア](/ja/guides/observability#組み込みストア)へ export します。バックエンドはワークロードの `OTEL_*` env を自動注入します。全バックエンド対応です。[オブザーバビリティ](/ja/guides/observability#ワークロードのテレメトリー)を参照してください。collector 有効イメージが必要です (`-tags otelcol`、リリースイメージでは設定済み)。
...
client-sourced 資格情報をワークロードに仲介します (`credentials`)。シークレット値はクライアント上で発行され、cornus サーバーと caretaker sidecar 経由で配送されます。フォアグラウンドの **kubernetes** セッションで実現され、`--detach` とすべてのホストバックエンド (`dockerhost`、`containerd`、`bare`、`incus`) は拒否します。[資格情報](/ja/guides/credentials) を参照してください。
| フィールド | 型 | 必須 | 既定 | 説明 |
| --- | --- | --- | --- | --- |
