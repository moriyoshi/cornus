...
| `credentials` | [CredentialSpec](#credentialspec) | no | — | 短命なクライアント発行の資格情報をワークロードへ仲介します。**kubernetes** でのみ実現され、現在はすべてのホストバックエンドが拒否します。[資格情報](/ja/guides/credentials) を参照してください。 |
...
client-sourced 資格情報をワークロードに仲介します (`credentials`)。シークレット値はクライアント上で発行され (この spec には決して含まれません)、cornus サーバーと caretaker sidecar 経由で配送されます。**kubernetes** 上で、フォアグラウンドの `cornus deploy --server` セッションによって実現されます。`--detach` とすべてのホストバックエンド (`dockerhost`、`containerd`、`bare`、`incus`) は拒否します。[資格情報](/ja/guides/credentials) を参照してください。
| フィールド | 型 | 必須 | 既定 | 説明 |
| --- | --- | --- | --- | --- |
...
- [エグレス](/ja/guides/egress) - `egress` block の詳細。
- [イングレス](/ja/guides/ingress) - `ingress` block の詳細。
- [資格情報](/ja/guides/credentials) - `credentials` block の詳細。
- [ワークロード間 hub](/ja/guides/hub) - `hub` block とワークロード間オーバーレイ。
