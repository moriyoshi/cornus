...
五つの host バックエンドは同じ核となる仕様フィールド (`name` / `image` / `replicas` / `restart` / `env` / `ports`) を尊重し、さらに `dockerhost` / `podman` / `containerd` / `bare` はクライアントローカル 9P バインドマウント、Compose user ネットワーク、公開ポート転送も共有します。そのためこの四つの間では同じワークフローを変更せず移動できます。例外は依然として `incus` ですが、その差はかつてよりずっと狭くなりました。核となるフィールド、`entrypoint` の上書き、サーバーホストのバインドマウント、管理対象ボリューム、`sysctls`、`ulimits`、`tmpfs`、`shmSize`、ヘルスチェックは map しますが、クライアントローカル 9P バインドマウント、user ネットワーク、command だけの上書きには対応しません。そして map できないフィールドについては必ず警告し、黙って捨てることはもうありません。特定バックエンドにしか対応しない仕様フィールドは[デプロイスペックリファレンス](/ja/reference/deploy-spec)にフィールドごとに記載されています。
...
Web UI のファイルエクスプローラーは、バックエンドが提供する経路でワークロードのファイルへ到達します。`dockerhost` / `podman`、`containerd`、`bare` は、コンテナの init プロセス経由でそのルートファイルシステムを読むことで **構造化ファイル操作** (`deploy.FSOperator`) を提供します。したがって 1 つのワークロード内での rename やコピーは、全バイトを呼び出し元へ往復させずにその場で完了します。`incus` もこれを提供しますが、経路が異なります。読むべきローカルパスがないため、デーモン自身がインスタンスへ張る SFTP チャネルを使います。こちらはイメージ側に何も要求しません。`kubernetes` は同じことを caretaker 経由で行います。`/proc/<pid>/root` 経路には root が必要で (root 所有のコンテナの `/proc/<pid>/root` を読むのは特権操作です) 、非ローカルまたは rootless なデーモンでは拒否されます。そこでは pid がこのサーバーから到達できるものを指さないためです。拒否された場合はリレーにフォールバックするため、エクスプローラーはどちらでも動作し、失われるのは高速経路だけです。
...
クライアント由来の資格情報は、host バックエンドでは **companion を一切必要としません**。したがって `CORNUS_ADVERTISE_URL` も `CORNUS_AGENT_IMAGE` も不要です。同じホスト上にあるサーバーが、どの配送方法も自分で行えるからです。[`env` 配送](/ja/guides/credentials)はデプロイ時に解決してコンテナ作成時に環境変数へ設定し、`file` はサーバー自身のデータディレクトリ配下へ描画して読み取り専用でバインドし、`endpoint` は *ワークロードのネットワーク名前空間の中に* サーバーがバインドするリスナーです。最後のものは、かつてそれを担っていたサイドカーなしで Kubernetes と同じセキュリティモデル (名前空間の境界が認可そのものである点) を保ちます。ID をユーザー名前空間へ再マップするランタイムでは、cornus がワークロードの実際に読む側の ID で資格情報ファイルを所有させるため、非 root のワークロードでも自分の資格情報を読めます。そうしたランタイムではデータディレクトリが辿れるようになりますが (`0711`。辿れるだけで一覧はできません) 、そこに置かれるシークレットは `0600` のままです。再マップするランタイムは 2 つあり、経路が異なります。rootless の `podman` はコンテナが存在する前にマップを報告するため、サーバーが所有者を解決し、最初から正しい ID でファイルを書けます。`incus` はマップをインスタンス自身に記録するため、ファイルを書く時点では問い合わせ先がありません。そこでインスタンスを停止状態で作成し、マップが存在してからディレクトリの所有権を移し、そのあとで起動します。1 つだけ、誤って配送する代わりに拒否される場合があります。レプリカごとに異なる ID 範囲が割り当てられている場合 (`security.idmap.isolated=true`) 、1 つのホストディレクトリは単一の所有権しか持てないため、レプリカ 0 が読める資格情報は残りのレプリカからは読めません。なお `incus` はこれらを AttachingBackend にならずに実現しています。companion が兄弟インスタンスであるためマウントやエグレスは運べませんが、そのことはサーバー自身が資格情報を実現することを何ら妨げません。いずれもリモートモードでは拒否されます。そこではランタイムのパスもプロセス ID もこのサーバーが解決できるものではないためです。
...
containerd 自身は probe を実行しませんが、ヘルスチェックはここでも動作します。cornus が probe を実行して結果を報告します ([ヘルスチェック](#ヘルスチェック)を参照)。

**`dockerhost` との既知の差:** attach は output-only です。ルートレス containerd は現在未検証・未対応です。
...
ヘルスチェックはここでも動作します。そしてこのバックエンドでは、ほかに実行できるものが最初から存在しませんでした。cornus 自身がランタイムなので、その probe エンジンは上記の restart 監督機構の隣に置かれています ([ヘルスチェック](#ヘルスチェック)を参照)。

**`dockerhost` との既知の差:** `containerd` と同様、attach は output-only です。ルートレスは現在対象外で、明確にエラーになります。
...
- **構造化ファイル操作** (`deploy.FSOperator`。Web のファイルエクスプローラーが使うもの) は別のチャネル、デーモンの SFTP を使います。ファイル API は rename をまったく表現できず、それはこの機能が存在する理由そのものに近いためです (アーカイブ系のプリミティブは stat とコピーはできても、何も move できません) 。このチャネルはイメージ側に何も要求しないため distroless のアプリケーションコンテナでも動作し、インスタンスのファイルシステム全体を単一のルート (`/`) として提供します。
...
- **`healthcheck`** は尊重されます。そして cornus が最も多くを担ったのがこのバックエンドです。Incus はインスタンスレベルの probe を公開しないため、cornus がデーモンの exec API を通じて自分でチェックを実行します ([ヘルスチェック](#ヘルスチェック)を参照)。この exec は 3 つのバックエンドの中で最も形が良く、操作のメタデータがコマンドの実際の返り値を持ちます。`bare` バックエンドの OCI ランタイムは非ゼロ終了をエラーとしてしか報告しません。

**`dockerhost` との既知の差。** 上記のログと stats の注意点に加えて、incus バックエンドは次を map しません。**command だけ**の上書き、クライアントローカル 9P バインドマウント、Compose の user `networks`、`knative`。
...
どのバックエンドもワークロードの health を報告しますが、その経路は 3 種類あります。`dockerhost` / `podman` は [`healthcheck`](/ja/reference/deploy-spec#ヘルスチェック) をデーモンに渡し、デーモンが実行して inspect で答えます。`kubernetes` は exec liveness probe と readiness probe に変換し、readiness を報告する health に射影します。`containerd`、`bare`、`incus` には自前の probe エンジンがないため、**cornus が自分で probe を実行します**。

この最後のグループがこの節の存在理由です。health を報告しないバックエンドでは、単に列が欠けるだけでは済みません。`depends_on: condition: service_healthy` を含む Compose ファイルは**事前に拒否されます**。数分後にタイムアウトする依存関係よりも、正直な答えのほうがましだからです。cornus が自前で probe を実行するようになるまで、この 3 つのバックエンドではごく普通の Compose ファイルの多くがデプロイできませんでした。

**状態機械は Docker のものです。**これは意図的な選択です。`service_healthy` は Docker の語彙と照合されるため、ここでの差は実際の Compose ファイルが収束するかどうかの差になります。

- probe が `0` で終了すれば成功、それ以外は失敗です。probe をそもそも実行できなかった場合や、`timeout` を超えた場合も失敗に含まれます。
- **`start_period` の間、失敗は `retries` に数えられず**、状態は `starting` のままです。最初の成功で `healthy` に変わり、そこで start period は終わります。以後の失敗は、期間が経過していなくても通常どおり数えられます。
- それ以降は `retries` 回**連続した**失敗で `unhealthy` になります。成功すれば `healthy` に戻り、カウントはクリアされます。
- 空のフィールドには Docker の既定値が入ります。`interval` 30s、`timeout` 30s、`retries` 3、`start_period` 0。
- `test` は Docker の form を使います。`["CMD", ...]` はそのまま exec し、`["CMD-SHELL", s]` は `s` を `/bin/sh -c` 経由で実行し、`["NONE"]` は継承したチェックを無効化します。

cornus の probe エンジンが、デーモンに委譲するバックエンドにはできないことが 2 つあります。1 つは **`start_interval`** (start period の*間*だけ probe の間隔を詰める) を尊重することです。`kubernetes` バックエンドはこれを表現できないと警告します。Kubernetes の Probe は全期間を通じて period が 1 つしかないためです。もう 1 つは、インスタンスごとに **probe を同時に 1 つしか実行しない**ことです。interval より遅い probe は次の probe を遅らせるだけで、積み上がりません。

**再起動をまたいだ挙動。** ワークロードを停止すると probe は解除されます (health は `unhealthy` ではなく空になります。停止したコンテナは失敗しているコンテナではありません)。開始すると再び武装します。明記しておく価値があるのは **cornus サーバー**の再起動です。ヘルスチェック自体はワークロードのある場所に永続化されているため (`containerd` ではコンテナのラベル、`bare` ではインスタンスレコード、`incus` ではインスタンス設定)、まだ動作しているインスタンス、および restart monitor がこれから復帰させるインスタンスについては probe が再開されます。**残らない**のは probe の*状態*のほうです。再武装したインスタンスは最初の probe が返るまで `starting` に戻ります。Docker は自身の状態を cornus より長生きするデーモンに保持しています。明示的に停止されたインスタンスは解除されたままです。`incus` にはこれを載せられる起動時の reconcile パスがないため、再武装は最初の `cornus deploy status` または `cornus deploy list` が引き金になります。`service_healthy` は status のポーリングで収束するので、これで十分です。

**health は報告であって制御ではありません。** unhealthy になったからといってコンテナを再起動するものは何もありません。Docker もオーケストレーターなしではそうしません。[`restart` policy](/ja/reference/deploy-spec) が反応するのはコンテナの*終了*であり、これは別の事象です。
