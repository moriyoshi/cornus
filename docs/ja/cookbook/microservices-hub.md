...
- ワークロード間ハブオーバーレイと、その中継方式。[ワークロードハブ](/ja/topics/hub)を参照してください。
- クラスター内ワークロード向けデプロイスペックの `hub:` ブロック。[デプロイスペック](/ja/reference/deploy-spec)を参照してください。
- ノート PC を含む任意の場所からオーバーレイに参加するための `cornus hub`。[`cornus hub`](/ja/cli/hub)と[ネットワークのレシピ](/ja/guides/networking)を参照してください。
...
アクセスは二つの任意のポリシーマトリクスで管理され、設定されている場合にだけ適用されます。**到達**マトリクス (呼び出し元 ID から許可する呼び出し先サービス、`CORNUS_HUB_POLICY`) と、**登録**マトリクス (ID からホスト可能なサービス名、`CORNUS_HUB_REGISTER_POLICY`) です。スポークは `identity` を宣言しますが、mTLS では検証済みクライアント証明書の CommonName から ID を取得します。したがって、ポリシーではスポークが偽造できない資格情報をキーとして使用します。ID とポリシーの仕組みは[ワークロードハブ](/ja/topics/hub)を参照してください。
...
**関連項目:** [ネットワークのレシピ](/ja/guides/networking) · [ワークロード間 hub](/ja/topics/hub) · [デプロイスペック](/ja/reference/deploy-spec) · [`cornus hub`](/ja/cli/hub)
