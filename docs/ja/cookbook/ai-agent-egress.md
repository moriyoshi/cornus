...
- [資格情報](/ja/guides/credentials) — デプロイスペックの `credentials:` block で、イメージに入れず API キーをエージェントへ配送します。
- [`cornus deploy`](/ja/cli/deploy) — 中継エグレスと資格情報取得の両方に必要な、フォアグラウンドの `--server` セッション。
- [セッションの conduit](/ja/guides/networking#セッションの-conduit-ポート転送と-socks5) — エージェント自身のポートに到達する方法を選ぶ `--conduit`。
