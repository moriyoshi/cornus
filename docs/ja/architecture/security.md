...
セキュリティは層状で、**明示的に有効化する方式**です。何も設定しない Cornus は、ローカル開発向けのパススルーとして動作します。以下の各レイヤーは設定で有効になり、**有効化後はフェイルクローズ**します。不正なポリシーは起動時の重大なエラーとなり、ID が必要な場所では空の ID を拒否し、設定を誤った検証器は通過させず拒否します。このページではモデルを説明します。設定は[認証と TLS](/ja/topics/auth-and-tls)、強化のレシピは[セキュリティガイド](/ja/guides/security)を参照してください。
...
- [認証と TLS](/ja/topics/auth-and-tls) — すべての verifier と TLS モードの設定。
- [セキュリティガイド](/ja/guides/security) — hardening レシピ。
