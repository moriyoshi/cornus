...
クライアントは接続 ID を事前に解決して work と一緒に送ります。エージェントのプロセス環境は spawn 時点で固定されるためです。エージェントは同じプロファイル logic に基づいて再解決するので、バックグラウンド compose セッションはプロファイルのトークン、TLS、key-auth、kube-auth を取得できます。同じサーバーを対象にする work は **1 本の接続と 1 つの conduit** を共有します。そのため、単一の SOCKS5 プロキシで docker コンテナと compose サービスの両方へ名前で到達できます。
...
UI には認証がなく、既定ではループバックにバインドします。ホスト外へバインドするには `--allow-non-loopback` が必要で、`--allow-host` によって Host 許可リストを拡張します。ブラウザーは意図的にプロジェクトを適用しません。デプロイのフロントドアは引き続き `cornus compose up` です。

## 接続プロファイルとリモートクラスター

クラスター *内*にある Cornus サーバーへ到達するには、以前は手作業の `kubectl port-forward` と手作業で用意したトークンが必要でした。接続プロファイルはサーバー側の変更なしに、その穴をクライアント側で埋めます。

- **プロファイル** は `cornus config` で管理される kubeconfig 風のコンテキストです。エンドポイント、TLS material、任意のポート転送対象、任意の key-auth または kube-auth ブロックを持ちます。1 つの resolver が選択されたコンテキストをすべてのクライアントコマンドへ通し、エンドポイントには明示的フラグ > コンテキストサーバー > エンドポイントの自動ポート転送、資格情報には `CORNUS_TOKEN` > SSH 鍵セッション > kube-auth 発行 > 静的プロファイルトークンという 2 つの precedence chain を適用します。
...
- **SSH 鍵認証**: プロファイルが保存するのは秘密鍵ファイルのパスまたは ssh-agent のフィンガープリントだけです。登録後、CLI は解決済みの TLS またはトンネル転送経路を通じて鍵の所有を証明し、短時間有効な範囲限定セッションを受け取ります。対話クライアントはバックグラウンドエージェント用に非公開でキャッシュしますが、秘密鍵も発行済みトークンも永続プロファイルには保存しません。
