...
**conduit はアドレスによって識別されます。** どこも提供していないアドレスを要求すればプロキシを起動し、すでに提供されているアドレス — 別のセッション、バックグラウンドエージェント、誰のものであれ — を要求すればそこに**参加**し、自分のワークロードの名前をすでに動作しているプロキシへ登録します。ブラウザーのプロキシ設定 1 つですべてに到達できるのはこのためです。自分の Compose サービス、別セッションのサービス、`cornus daemon docker` のコンテナ、そして Web UI のすべてに届きます。

参加の判定は完全一致ではなく「包含」です。`0.0.0.0:1080` のプロキシは `127.0.0.1:1080` への要求も提供します。逆は成り立ちません。ループバックだけにバインドされたプロキシは後から範囲を広げられないため、そのポートを保持している間に `0.0.0.0` を要求すると、保持しているプロセスを名指しして拒否されます。

ポートを持たない `socks5://` URL (または明示的な `:0`) はプライベートです。ほかから見つけるための合意されたアドレスが存在しないため、誰も参加しません。意図的に自分専用のプロキシを立てたい場合はこれを使います。SOCKS5 CONNECT は TCP 専用です。単独で使うアドホックなプロキシは [`cornus socks5`](/ja/cli/socks5) です。
