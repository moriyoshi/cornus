...
- [`cornus ingress-tunnel`](/ja/cli/ingress-tunnel)は、プロジェクト全体に 1 つの**パブリック URL** を与え、ルーティングも代行します。[イングレスをパブリックに公開する](#イングレスをパブリックに公開する)を参照してください。
- [SOCKS5 conduit](#conduit-経由で自分のマシンからイングレスに到達する)は、宣言したホスト名を自分のマシンからルーティングします。パブリックには一切公開されません。
...
トンネルの資格情報は引き続きクライアント側から渡されます (接続プロファイルの `authtoken-file`、または `CORNUS_TUNNEL_AUTHTOKEN`)。リポジトリにコミットされる compose ファイルからは渡されません。各バックエンドで何ができるかは[トンネルガイド](/ja/guides/tunnels#ポートではなくイングレスをトンネルする)を、URL を共有する前に理解しておくべき唯一の点 — アプリが認識するのがトンネルのホスト名か宣言したホスト名か — については[Host の扱い](/ja/cli/ingress-tunnel#host-の扱い)を参照してください。
