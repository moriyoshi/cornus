...
イングレスは[クライアント側エグレス](/ja/guides/egress)の受信側に当たる機能です。ワークロードの公開ポートに対するパブリックな **HTTP(S)** のフロントドアを要求するため、サービスは[ポート転送](/ja/guides/networking)や[トンネル](/ja/guides/tunnels)だけでなく、実際のホスト名で到達可能になります。ワークロードの公開ポートの前段となるため、仕様は少なくとも 1 つのポートを公開しなければなりません。

イングレスはデプロイスペックの `ingress:` ブロックまたは Compose のポータブル拡張 `x-cornus-ingress:` で明示的に有効化します。暗黙に有効になることはありません。

## 何が実現するか

宣言するホストとパスはどの環境でも同じです。異なるのは、それを提供する主体です。

| バックエンド | 実現する主体 |
| --- | --- |
| `kubernetes` | ワークロードの `ClusterIP` Service の前段に立つ実際の `networking.k8s.io/v1` **Ingress** オブジェクト。クラスターのイングレスコントローラーが提供します |
| `dockerhost`、`containerd`、`bare`、`incus` | **cornus サーバー自身**。ホストとパスのルーティングテーブルを保持し、ワークロードへプロキシします |

クラスターオブジェクトを作成するのは Kubernetes の経路だけなので、以下で説明するクラス名、アノテーション、cert-manager の TLS 設定はそこにのみ適用されます。それ以外の環境ではサーバーが同じ宣言をルーティングします。これにより Compose プロジェクトはノート PC 上でもクラスター上でも同じように振る舞います。

::: warning サーバー側フロントドアは開発用です
サーバー側のイングレスルーティングは、Compose プロジェクトがノート PC 上でもクラスター上でも同じように振る舞うようにするために存在します。**本番用のイングレスではありません**。自前の TLS 終端はなく、レート制限もアクセス制御も行わず、エラーページは到達できなかった内部ワークロード名とコンテナポートを表示します。デバッグ中には有用ですが、見知らぬ相手が到達できるアドレスでは望ましくありません。

開発とプレビュー用途に使ってください。恒久的なもの、あるいはパブリックなものには、実際のイングレスコントローラーを備えたクラスターへデプロイしてください。cornus は利用可能な場合そちらへ接続します。
:::

## 到達方法

イングレスを宣言しただけでは、自分のマシンから到達できるようにはなりません。そのためには DNS がどこかを指している必要があり、プレビュー環境にそれがあることはまれです。この隔たりを埋める方法は 3 つあり、必要になる頻度の高い順に挙げます。

- [`cornus ingress-tunnel`](/ja/cli/ingress-tunnel)は、プロジェクト全体に 1 つの**パブリック URL** を与え、ルーティングも代行します。[イングレスをパブリックに公開する](#イングレスをパブリックに公開する)を参照してください。
- [SOCKS5 conduit](#conduit-経由で自分のマシンからイングレスに到達する)は、宣言したホスト名を自分のマシンからルーティングします。パブリックには一切公開されません。
- `CORNUS_INGRESS_LISTEN` はサーバーのフロントドアを、DNS や `/etc/hosts` のエントリで指せるアドレスにバインドします。
...
デプロイスペックは 1 列目の camelCase のフィールド名を使い、Compose の `x-cornus-ingress` 拡張は 2 列目の snake_case のキーを使います ([Compose サービスを公開する](#compose-サービスを公開する)を参照)。
...
native の経路はデプロイ要求で秘密鍵の実体を送信します。そのため Cornus は、リモートのプレーン HTTP 越しの場合は要求のシリアライズ前に拒否します。HTTPS、SSH トンネルのプロファイル、または Kubernetes のポート転送のようなループバックのエンドポイントを使ってください。鍵がステータスや診断出力に現れることはありません。フィールドの一覧は[接続設定リファレンス](/ja/reference/connection-config#ingresscertificate)を参照してください。
...
[`cornus ingress-tunnel`](/ja/cli/ingress-tunnel)はイングレスの前面にパブリック URL を置きます。スコープが宣言したホスト名とパスのすべてが、1 つのアドレスの背後に入ります。
...
これが[`cornus tunnel`](/ja/cli/tunnel)との違いです。後者は単一のワークロードポートを公開するだけで、宣言されたホストやパスについては何も関知しません。

コマンドを実行する代わりに `compose up` のたびに公開したい場合は、対象のイングレスの隣で宣言します。
...
トンネルの資格情報は引き続きクライアント側から渡されます (接続プロファイルの `authtoken-file`、または `CORNUS_TUNNEL_AUTHTOKEN`)。リポジトリにコミットされる compose ファイルからは渡されません。各バックエンドで何ができるかは[トンネルガイド](/ja/guides/tunnels#ポートではなくイングレスをトンネルする)を、URL を共有する前に理解しておくべき唯一の点 — アプリが認識するのがトンネルのホスト名か宣言したホスト名か — については[Host の扱い](/ja/cli/ingress-tunnel#host-の扱い)を参照してください。
