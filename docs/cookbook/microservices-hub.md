...
- The workload-to-workload hub overlay and its relay model — see [the workload hub](/guides/hub).
- The `hub:` block in the deploy spec for in-cluster workloads — see [Deploy spec](/reference/deploy-spec).
- `cornus hub` to join the overlay from anywhere, including a laptop — see [`cornus hub`](/cli/hub).
...
[the workload hub](/guides/hub) for the identity and policy model.
...
**See also:** [The workload hub](/guides/hub) · [Networking and conduits](/guides/networking) · [Deploy spec](/reference/deploy-spec) · [`cornus hub`](/cli/hub)
