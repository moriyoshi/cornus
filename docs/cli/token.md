...
| `--scope` | — | `api` | Scope: `api` (full access) or `caretaker` (the caretaker attach endpoint only). A token must name one: an empty scope grants nothing and is refused at mint time. |
