...
| `--ingress-conduit` | `CORNUS_INGRESS_CONDUIT` | — | Reach a workload ingress (`x-cornus-ingress`) through the SOCKS5 conduit: `native` (tunnel to the real cluster ingress controller), `emulate` (a client-side reverse proxy with a generated cert), or `off`. Requires `--conduit-mode socks5`. See [Public ingress](/topics/ingress). |
