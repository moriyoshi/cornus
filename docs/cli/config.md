...
Each context describes how to reach a server: its base URL, SSH-key session,
bearer token or ServiceAccount-minted auth, TLS material, an optional automatic port-forward to
...
| `--key-auth-identity-file` | — | — | SSH private-key path used to mint short-lived Cornus sessions. |
| `--key-auth-fingerprint` | — | — | SHA256 fingerprint of the key to select from `SSH_AUTH_SOCK`, or to pin an identity file. |
| `--key-auth-name` | — | — | Human-readable name used when enrolling the key. |
| `--key-auth-scope` | — | `api` | Scope requested for SSH-key sessions. |
| `--key-auth-ttl` | — | `1h` | Requested SSH-key session lifetime (maximum `24h`). |
