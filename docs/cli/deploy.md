...
default), `containerd`, `bare`, or `incus`. Any other value — including
`kubernetes`, which only the server honors — falls back to `dockerhost` with a
warning. See [Deploy backends](/reference/deploy-backends).
...
(`dockerhost` default, `containerd`, `bare`, or `incus`).
