...
supported on every host backend — dockerhost, containerd, bare, and incus;
Kubernetes port-forward is TCP-only, so such mappings are skipped with a warning.

The connection is resolved from `--server`, otherwise from the selected
connection profile (see [`cornus config`](/cli/config)). See
[Networking and conduits](/guides/networking) for how port-forward fits the
...
Forward a UDP port (any host backend; not Kubernetes):
