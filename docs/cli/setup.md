...
- **Docker host (server in a container)** — run the server itself as a container
  ...
  adds is the run artifact, since the bind mounts are the whole difficulty and
  getting one wrong fails silently. See
...
helm values snippet; for the container-install scenario it offers a
`cornus-compose.yaml` with the socket bind, the `rshared` data-dir bind, and a
published port matching the profile it just saved. Each is ask-before-write ({write to a file, print to stdout,
