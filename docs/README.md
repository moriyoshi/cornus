...
npm run docs:build    # production build into .vitepress/dist (also the route check)
npm run docs:check    # check punctuation, build, fragments, and duplicate targets
npm run docs:preview  # serve the production build locally
```

`npm run docs:build` fails on dead internal links, so a clean build is the primary
correctness signal after editing cross-page links. It does not validate URL
fragments or detect two entries in one list that lead to the same target;
`npm run docs:check` adds both checks and rejects full-width parentheses or
colons in authored prose while leaving literal code and URL destinations alone.
...
The deployment artifact is always rebuilt from the Markdown source. Never edit
or publish `docs/.vitepress/dist/` by hand.
