...
routing a workload's outbound traffic through the caller, is [client-side
egress](/topics/egress).
...
- **`endpoint`** — the caretaker serves the credential from a loopback HTTP
  ...
  [IMDS-compatible endpoints (AWS)](#imds-compatible-endpoints-aws) below.
...
app change — the credential is still minted client-side (typically by the
[`aws-sts`](#source-backends) backend) and delivered by the caretaker. The adapter
is pure HTTP with no AWS SDK dependency of its own, and it answers two shapes over
one endpoint:
...
the caretaker runs a loopback reverse proxy to the vendor API and **injects the
auth header itself**, so the workload calls the LLM with no key of its own. It
sets `ANTHROPIC_BASE_URL` / `OPENAI_BASE_URL` on the app, strips any client-sent
auth, and adds the real credential per request. So a coding-agent workload can
ride **your own** Claude Code / Codex login without the secret ever entering the
container.
...
`upstream` points the proxy at any Anthropic- or OpenAI-compatible gateway
instead of the vendor default (`https://api.anthropic.com` /
...
neither read the raw secret nor spoof it. The full field reference is in the
[deploy spec](/reference/deploy-spec). For the task-oriented recipes, see the
[Credentials](/guides/credentials) guide.
