...
feature, handing a workload a caller-minted secret, is [credential
brokering](/topics/credentials); for the *inbound* direction — giving a workload a
public hostname — see [Ingress](/topics/ingress).
...
SASE** networks (the sanctioned egress path lives on the client side).
...
For a one-off deploy, select the mode and first-match routing rules on the CLI:
...
corporate PAC file drops in. It is set as `script:` on the egress spec and, when
present, supersedes `rules`. It is evaluated by a sandboxed pure-Go JS engine
with the standard pure PAC builtins (`shExpMatch`, `dnsDomainIs`, `isInNet`, ...),
a bounded per-call deadline, and **fail-closed-to-`deny`** on error or timeout.
The runtime has no ambient authority: no `require`, no live network or DNS I/O
(a name resolves only to the destination IP the caller already knows), and
deterministic `Date` / random, so evaluation is reproducible across the
caretaker, server, and client evaluation points.
...
See the [deploy spec](/reference/deploy-spec) for the `EgressSpec` fields and
[`cornus deploy`](/cli/deploy) for the CLI flags. For the task-oriented recipes,
see the [Egress](/guides/egress) guide.
