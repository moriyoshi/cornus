...
const TREE: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { link: '/guides/server-setup', text: { en: 'Setting up a server', ja: 'サーバーをセットアップする', zh: '设置服务端' } },
      ...
      { link: '/guides/compose-support', text: { en: 'Compose extensions and compatibility', ja: 'Compose の拡張と互換性', zh: 'Compose 扩展与兼容性' } },
      { link: '/guides/web-ui', text: { en: 'The browser UI', ja: 'ブラウザー UI', zh: '浏览器 UI' } },
      { link: '/guides/remote-clusters', text: { en: 'Working with remote clusters', ja: 'リモートクラスターで作業する', zh: '使用远程集群' } },
      { link: '/guides/remote-docker-hosts', text: { en: 'Remote container hosts over SSH', ja: 'SSH 経由のリモートコンテナホスト', zh: '通过 SSH 访问远程容器主机' } },
      { link: '/guides/server-in-a-container', text: { en: 'Running the server in a container', ja: 'サーバーをコンテナで実行する', zh: '在容器中运行服务端' } },
  ...
  },
  {
    ...
    items: [
      ...
      { link: '/cli/ingress-tunnel', text: { en: 'cornus ingress-tunnel', ja: 'cornus ingress-tunnel', zh: 'cornus ingress-tunnel' } },
      ...
      { link: '/cli/auth', text: { en: 'cornus auth', ja: 'cornus auth', zh: 'cornus auth' } },
      ...
      { link: '/cli/activity', text: { en: 'cornus activity', ja: 'cornus activity', zh: 'cornus activity' } },
      { link: '/cli/observe', text: { en: 'cornus observe', ja: 'cornus observe', zh: 'cornus observe' } },
...
// slugify recomposes VitePress's heading ids to NFC.
//
// The upstream slugify (@mdit-vue/shared, bundled into VitePress) is:
//
//   const rCombining = /[\u0300-\u036f]/g
//   str.normalize('NFKD').replace(rCombining, '')...
//
// NFKD is there to strip Latin accents (café -> cafe), and rCombining covers
// exactly the Latin combining range. Japanese voiced/semi-voiced sound marks are
// U+3099 / U+309A, OUTSIDE that range — so NFKD splits ド into ト + U+3099 and
// nothing puts it back. The id then contains decomposed kana that looks
// identical to composed kana in every editor, meaning a normally-typed link into
// a Japanese heading silently fails to resolve.
//
// Recomposing at the end fixes the ids at the source, so anchors are written the
// same way as the headings they point at. Ordering matters: NFC runs last, after
// the accent-stripping the upstream function exists to do, so café -> cafe is
// unaffected. `docs:check-anchors` is what proves both halves.
const rControl = /[\u0000-\u001f]/g
