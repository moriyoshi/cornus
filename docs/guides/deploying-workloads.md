...
- Known gaps vs dockerhost: no mounts, volumes, user networks, healthchecks, or command/entrypoint overrides; `cornus attach` is unsupported (use [`cornus exec`](/cli/exec)); logs come from the instance console, so `--follow`/`--since`/`--tail` do not apply; `cornus stats` reports CPU% low. Each unmapped field is warned about by name.
