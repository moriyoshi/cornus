...
five [deploy backends](/reference/deploy-backends). The local backend is chosen
...
- Known gaps vs dockerhost: attach is output-only. Healthchecks work — containerd runs no probes itself, so cornus runs them (see [healthchecks](/reference/deploy-backends#healthchecks)).
...
Run the same OCI image as an [Incus](https://linuxcontainers.org/incus/) application container, supervised by incusd.

```sh
CORNUS_DEPLOY_BACKEND=incus cornus deploy -f app.yaml
```

- Linux-only; needs **Incus 6.3+** (earlier releases have no OCI support) and access to the daemon socket (`CORNUS_INCUS_SOCKET`, default `/var/lib/incus/unix.socket`). Choose the project with `CORNUS_INCUS_PROJECT`.
- **`skopeo` and `umoci` must be installed where incusd runs** — the daemon, not cornus, flattens the OCI image. If incusd pulls from a plain-HTTP registry (cornus's own, for instance), list it in `CORNUS_INCUS_INSECURE_REGISTRIES` *and* mark it insecure in the daemon host's `/etc/containers/registries.conf.d/`, or skopeo will refuse it.
- Instances are named `cornus-<app>-<replica>`; published ports become Incus `proxy` devices on replica 0.
- Known gaps vs dockerhost: no client-local bind mounts, user networks, or command-only overrides; `cornus attach` is unsupported (use [`cornus exec`](/cli/exec)); logs come from the instance console, so `--follow`/`--since`/`--tail` do not apply; `cornus stats` reports CPU% low. Each unmapped field is warned about by name. Server-host bind mounts, managed volumes, an `entrypoint` override, and healthchecks all map — the healthcheck by cornus running the probe over the daemon's exec API, since Incus exposes none of its own.

**See also:** [deploy backends](/reference/deploy-backends#incus), [server env vars](/reference/server-env-vars)
...
**See also:** [remote clusters](/guides/remote-clusters), [deploy backends](/reference/deploy-backends)
...
**See also:** [cornus deploy](/cli/deploy), [remote clusters](/guides/remote-clusters)
...
- Client-local mounts are served from the server's own `<DataDir>/mounts` area and are always permitted, so they work without relaxing the host-privilege policy.
...
write-heavy mounts. Two suffixes opt into a server-side file cache:

```sh
cornus deploy -f app.yaml --server https://cornus.example.com \
  --local-mount ./models:/models:ro,cache \
  --local-mount ./db:/var/lib/app:async
```

- Add `,cache` to declare an immutable read-only source — a read-through cache for datasets or model weights. It uses the server per-file cache and implies `:ro`.
...
- Both need the server's file cache enabled. Set `CORNUS_BLOCK_COHERENCE=subhash,subfill` in both the server and deploy-caller environments, plus `CORNUS_BLOCK_READAHEAD=64k` or a larger cap, as a starting point for database-shaped async mounts. See [server environment variables](/reference/server-env-vars#remote-9p-file-cache-and-writable-mounts) and [client-local bind mounts](/architecture/deploy-engine#client-local-bind-mounts) for how the caching works.

**See also:** [cornus deploy](/cli/deploy), [Networking and conduits](/guides/networking)
...
**See also:** [cornus port-forward](/cli/port-forward), [networking](/guides/networking), [remote clusters](/guides/remote-clusters)
