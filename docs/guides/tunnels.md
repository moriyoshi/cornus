...
## Tunnelling the ingress instead of a port

Everything above exposes **one port of one workload**. A project with several
services therefore needs several tunnels and several unrelated URLs, and none of
the hostnames or paths your services declare with
[`x-cornus-ingress`](/guides/ingress) apply.

[`cornus ingress-tunnel`](/cli/ingress-tunnel) publishes the ingress itself:
...
One URL, and requests reach the right service by host and path exactly as they
would in production. On Kubernetes the tunnel hands traffic to the real cluster
ingress controller, so the cluster's own routing rules and TLS certificates
apply; on every other backend the server routes the declared hosts and paths
itself.

### Which layer your backend delivers

The backends differ in what arrives at the far end, and that decides how much can
be preserved:

| Layer | Backends | TLS | `Host` |
| --- | --- | --- | --- |
| HTTP | `ngrok` (http), `cloudflare`, `tailscale` | terminated at the provider's edge; the leg into cornus is plain HTTP | the provider's hostname, unless it granted yours |
| Raw TCP | `ssh`, `ngrok` (`--proto tcp`) | end to end — the client's TLS reaches the cluster ingress controller untouched | exactly what the client sent |

For an HTTP-layer tunnel the provider's certificate is what your visitors see, and
the `tls:` block on your ingress governs the cluster-native front door rather than
the tunnel URL.

End-to-end TLS needs both halves: a raw-TCP tunnel to carry the bytes untouched,
**and** a real cluster ingress controller to terminate them. A server routing the
ingress itself has no certificate to serve, so it refuses `--proto tcp` rather
than hand you a URL that fails the moment anyone requests `https`.

### Getting your own hostname

If the tunnel's public hostname can be made to *equal* an ingress hostname, nothing
about the request needs adjusting — the app sees exactly what it expects, and TLS
can be end to end. `ngrok` can do this with a reserved or custom domain on the
account; the `ssh` backend can wherever the relay routes by requested bind host,
as [sish](https://github.com/antoniomika/sish) does. Cloudflare quick tunnels and
Tailscale Funnel cannot.

When it is not possible, cornus routes the provider's hostname to your ingress host
instead, and the app keeps seeing the hostname the browser is on — so redirects and
cookies stay reachable. See
[host handling](/cli/ingress-tunnel#host-handling) for the full picture and for
when to override it.

### Reaching the ingress without any tunnel

An operator who only needs the ingress reachable on the network the server already
sits on can skip tunnels entirely by binding the front door directly:

```sh
CORNUS_INGRESS_LISTEN=:8080 cornus serve
```

Point DNS (or an `/etc/hosts` entry) for the declared hostnames at that address.
This serves the same routing table an ingress tunnel fronts, so it is also the
simplest way to check that your hosts and paths resolve the way you intended.
