...
Step-by-step setup for exposing a workload port publicly through
[`cornus tunnel`](/cli/tunnel), one recipe per backend. For the model behind
it — the credential-injection flow and the listener vs. upstream provider
shapes — see [Public tunnels](/topics/tunnels). All four backends share the
same client command; only the server-side `CORNUS_TUNNEL_BACKEND` and its
per-backend environment variables change.

```sh
cornus tunnel [--authtoken TOKEN | --authtoken-file FILE] [--proto http|tcp] <name> <port>
```

**Passing the credential safely.** `--authtoken TOKEN` puts the secret
directly in argv, which any other user on the machine can read via `ps` and
which shells often write to history — avoid it for anything but a quick local
test. Prefer, in order: no credential at all (the server has a default —
see below), the backend's env var (kong reads it into `--authtoken`
automatically, so the value never appears as a command-line argument), or
`--authtoken-file FILE` (reads the secret from a file, keeping it out of both
argv and history). The recipes below use the env-var / file forms throughout.
...
**See also:** [cornus tunnel](/cli/tunnel), [Public tunnels](/topics/tunnels)
...
**See also:** [cornus tunnel](/cli/tunnel), [Public tunnels](/topics/tunnels), [Server env vars](/reference/server-env-vars)
...
**See also:** [cornus tunnel](/cli/tunnel), [Public tunnels](/topics/tunnels), [Helm chart values](/reference/helm-values)
