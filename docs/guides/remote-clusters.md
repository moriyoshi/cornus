...
The JWKS proves **who** the caller is. A ServiceAccount token carries no cornus
`scope` claim, so the server also needs a **scope map** saying what that identity
may reach, or every call is refused with a 401 that says so:
...
Keying on the exact `sub` is what makes the grant specific: any other
ServiceAccount in the cluster can mint an equally valid, correctly-audienced
token, and only the map decides that it is not an operator. See
[scope mapping](/guides/security#map-third-party-claims-to-cornus-scopes).
