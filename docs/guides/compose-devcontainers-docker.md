...
| Workspace and runtime | `workspaceFolder`, `workspaceMount`, `mounts`, `forwardPorts`, `appPort`, `containerEnv`, `remoteEnv`, `overrideCommand`, `containerUser`, `remoteUser` |
...
are editor-specific `customizations` and `build.options`. For a single-container
definition, `runArgs` supports only `--privileged`; other arguments are ignored
with a warning. For a Compose-based definition, all `runArgs` are ignored with a
warning. `containerUser` selects the lifecycle-command user but does not change
the container's main process user. Other schema fields are outside Cornus's
supported boundary.
