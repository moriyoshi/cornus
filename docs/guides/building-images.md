...
Run the build on a remote cornus server, streaming the context, build-context dirs, and secrets over 9P/WebSocket.
...
- `--builder` accepts a `ws://` / `wss://` or `http(s)://` base URL (env `CORNUS_BUILDER`); a selected connection profile that names a server also routes the build remotely.
- `--lazy` serves the `--build-context` dirs on demand, so only the bytes the build actually reads cross the wire. Lazy is not supported on the `containerd` build worker.

**See also:** [cornus build](/cli/build), [remote clusters](/guides/remote-clusters), [remote workflows](/topics/remote-workflows)
...
**See also:** [cornus build](/cli/build), [security](/guides/security)
