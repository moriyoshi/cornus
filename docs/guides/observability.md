...
**Metrics** screen of [`cornus web`](/guides/web-ui#metrics-dashboard) — CPU, memory,
...
there are no network counters, disk counters, or process counts, and CPU arrives
as an instantaneous rate (`container_cpu_usage`) rather than the cumulative
`container_cpu_time` the host backends record. The fuller set lives behind the
kubelet's Summary API, which needs a `nodes/proxy` grant that reaches every
kubelet in the cluster; that is not a reasonable trade for three more metric
families.

The memory *limit* is reported normally, because it is not a measurement: Cornus
reads it back off the pod spec it wrote.
...
claims. `cornus observe status` lists them under `metrics.unsupported`, and the
web dashboard leaves those panels out entirely. Run `cornus daemon preflight` to
check the RBAC grant.
:::
