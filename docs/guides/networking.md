...
# Pin the proxy's bind address and suffix in one value:
...
cornus compose up --conduit socks5                    # the profile's address
cornus compose up --conduit 'socks5://127.0.0.1:1085' # that address, joining if it is already served
cornus compose up --conduit 'socks5://'               # a private proxy on an ephemeral port
cornus deploy --server http://cornus.example:5000 --conduit socks5 -f deploy.yaml
```

**A conduit is identified by its address.** Ask for an address nothing is serving
and you start the proxy; ask for one that is already served — by another session,
by the background agent, by anyone — and you JOIN it, registering your workloads'
names into the proxy that is already running. That is what makes one browser proxy
setting reach everything: your compose services, another session's, `cornus daemon
docker` containers, and the web UI.

Joining is by coverage rather than exact match, so a proxy on `0.0.0.0:1080` also
serves a request for `127.0.0.1:1080`. The reverse is not true: a proxy bound only
to loopback cannot be widened, so asking for `0.0.0.0` while it holds the port is
refused, naming the process that holds it.

A `socks5://` URL with **no port** (or an explicit `:0`) is private: there is no
agreed address for anything else to find, so nothing else joins it. That is how you
deliberately run a proxy of your own. SOCKS5 CONNECT is TCP-only. The standalone
ad-hoc proxy is [`cornus socks5`](/cli/socks5).

The bind address in a `socks5://` selector is loopback-only by default: the
conduit proxy has no authentication and dials arbitrary destinations from your
machine, so off-host it is an open proxy for anyone who can reach it. Add
`--allow-non-loopback` to accept that exposure deliberately:

```sh
# Refused: an off-host bind with no opt-in.
cornus compose up --conduit 'socks5://0.0.0.0:10080'
# Accepted: the same proxy, exposure acknowledged.
cornus compose up --conduit 'socks5://0.0.0.0:10080' --allow-non-loopback
```

The opt-in is needed by whoever **binds** the address. A later session that joins a
proxy already serving it does not need the flag — the exposure was decided when the
proxy was created — but it does say so, so an off-host proxy is never something you
end up in without being told.

The flag exists on [`compose up`](/cli/compose) and [`deploy`](/cli/deploy)
alongside `--conduit`, and on the standalone
[`cornus socks5`](/cli/socks5) alongside `--listen`. Without it the bind is
refused up front, before anything is deployed.
...
On the dockerhost backend the server dials the workload's container IP, so it needs a
route to it. Two cases where that is not automatic, both of which report the cause
rather than timing out:

- **`macvlan` / `ipvlan` networks.** A macvlan container cannot be reached from its own
  host — that is the driver's design, not a misconfiguration. If the workload is also on
  a bridge network, cornus dials it there; if it is on macvlan only, `port-forward` says
  so and your options are to publish a port, add a bridge network, or connect from
  another machine.
- **A remote `DOCKER_HOST`.** Container IPs on another machine's daemon mean nothing
  locally. Set `CORNUS_DOCKER_REMOTE=1` (with `CORNUS_AGENT_IMAGE` and
  `CORNUS_ADVERTISE_URL`) so the server reaches workloads through a per-instance
  companion.

A cornus server that is itself a container has a third case, handled automatically —
see [Server in a container](/guides/server-in-a-container#reaching-workloads).

The other host backends answer the same question differently:

- **`containerd` and `bare` need nothing.** Cornus builds their networks itself, with
  CNI, in whatever network namespace cornus is in — so the workload is always somewhere
  cornus can reach. What a containerized server costs there is a published port, not a
  port-forward: without host networking the port-publishing NAT rules land inside the
  server's own container. On containerd cornus can detect that and refuses to start;
  see [Server in a container](/guides/server-in-a-container#containerd).
- **`incus`** instances sit on incusd's own bridge, which the daemon host can always
  route to. A server containerized *beside* incusd cannot, and cannot join that bridge
  either — a cornus container is not an incus instance, so there is no equivalent of
  the docker self-attach. Give that container the host's network namespace, or run the
  server as an incus instance itself (then it is on the bridge alongside its workloads,
  and cornus recognizes it), or set `CORNUS_INCUS_REMOTE=1` (with `CORNUS_AGENT_IMAGE`
  and `CORNUS_ADVERTISE_URL`) to reach instances through a per-instance companion.
  `port-forward` names whichever of these is missing rather than timing out. See
  [Server in a container](/guides/server-in-a-container#incus).
...
- `cornus web --publish-in-conduit` needs no conduit settings of its own: it
  **joins** whichever shared conduit the agent is already running for the
  connection. Naming an address or a suffix in its `--conduit` pins those settings
  instead, which is how you would deliberately run a second proxy.
- `compose up -d` and `cornus daemon docker` still key on their own settings, so
  keep those two consistent with each other (the same `--conduit` URL, or both
  relying on the profile). Divergent `listen`/`suffix` values make the second
  command's proxy collide with the first on its bind address — the agent now
  refuses that by naming the session already holding the address, rather than
  passing the raw bind error through.
