...
- `--conduit` here **pins** settings rather than selecting a conduit: naming an
  address or a suffix (`--conduit socks5://.shared:1080`,
  `--conduit socks5://?suffix=.demo.internal`) means "use exactly these", which is
  how you deliberately run a second, separate proxy — or choose between several.
  A bare `--conduit socks5`, `CORNUS_CONDUIT`, and the profile name no settings, so
  they join like the flagless case.
- If the agent happens to run more than one shared conduit for this connection, the
  UI joins the most-shared one and says which; the banner line printed at startup
  names the address it landed on.
