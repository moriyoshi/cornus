...
- `src/views/` — one file per screen. Five are routed (`src/index.tsx`): Overview,
  WorkloadDetail, Metrics, Workspace, Settings. The rest — Workloads, Projects,
  Mounts, Tunnels — are sections the Overview and detail pages compose, not
  destinations of their own.
- `src/views/Workspace.tsx` — the tiled screen, whose panes are either file
  browsers (`views/files/`) or terminals (`views/terminal/`) in one tree. It was
  two screens, Files and Terminal, until they merged; `views/tiling/` is the
  payload-agnostic chrome both had always shared.
...
The mock shell can also impersonate any agent cornus knows how to classify, which is
how the activity badges, the Overview agent column and the "needs you" affordances
are exercised without one:

```
agents            # list them (claude, codex, cursor, gemini, grok, …)
agent claude      # play one — or just type its name
```

A run shows a working screen, stops on the prompt it is waiting for (the session
badges as blocked until you press Enter), then returns to idle. The screens are real
ones from `mock/agent-screens.json`, and each state is the verdict the agent's own
detection manifest reaches on that screen — checked from the Go side, so what you see
here is what a live backend would report.
