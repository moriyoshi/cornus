...
export interface Settings {
  ...
  paneZoom: boolean;
  // Which typeface a terminal pane is painted in. An id from TERMINAL_FONTS, resolved
  // through termFont() at the point of use rather than trusted here — see that function
  // for why an unrecognised value has to land on the default rather than reach xterm.
  terminalFont: TerminalFontId;
  // The unzoomed size a terminal is painted at, in px. The BASE, not the final size: a
  // pane's own zoom level multiplies it (see termType in components/Term.tsx), so this is
  // what every terminal starts at rather than what every terminal is. Resolved through
  // termFontSize(), which snaps it to the offered set.
  terminalFontSize: number;
  // The leading between a terminal's rows, as a multiplier of the font size (xterm's own
  // lineHeight unit). Read through termLineHeight(), which clamps it: this is the one
  // number in this file that can break a pane outright if a corrupt blob puts a zero in
  // it, because xterm divides by the cell height to count rows.
...
export function defaultSettings(): Settings {
  ...
  // The terminal's font is the BROWSER'S monospace, at no extra leading. The four bundled
  // families are each a better terminal face than a generic default, which is why they are
  // offered — but "monospace" is the font this user already chose, in their own browser
  // settings, for exactly this kind of text, and picking over it for them is not a default's
  // job. It is also the only option that fetches nothing. Size 13px and leading 1 for a
  // plainer reason: they are what the terminal already was, so nobody's panes re-flow
  // because these settings arrived.
  return {
    ...
    terminalLineHeight: DEFAULT_TERM_LINE_HEIGHT,
...
export function setTerminalFont(v: TerminalFontId): void {
  store.setSettings((s) => ({ ...s, terminalFont: v }));
}

export function setTerminalFontSize(v: number): void {
  store.setSettings((s) => ({ ...s, terminalFontSize: v }));
}

export function setTerminalLineHeight(v: number): void {
  store.setSettings((s) => ({ ...s, terminalLineHeight: v }));
}
