...
export interface Config {
  ...
  agentSocket: string;
  agentLive: boolean;
  // How the CLIENT realizes ingress, one entry per live conduit that routes any —
  // the counterpart to `server.ingress`, which is the front door the SERVER
  // advertises. The two are independent: a cluster with a real controller can
  // still be reached through a client-side emulated proxy.
  //
  // Absent/empty is TWO different answers, and `agentLive` is what tells them
  // apart: live agent + no entries means no conduit routes ingress; no live agent
  // means nobody was there to ask, so the setting is unknown. A UI that renders
  // one sentence for both states asserts something it does not know.
  clientIngress?: ClientIngress[];
  version: string;
}

// ClientIngress mirrors webbff.AgentIngress: one conduit's ingress settings.
export interface ClientIngress {
  // "native" tunnels transparently to the real cluster ingress controller;
  // "emulate" runs a client-side HTTP(S) reverse proxy. Never empty — a conduit
  // with ingress off contributes no entry rather than a mode-less one.
  mode?: "native" | "emulate";
  // The suffix ingress hosts are derived under. Empty means the conduit falls back
  // to its service-host suffix stem.
  domain?: string;
  // The native-passthrough target. Absent in emulate mode, where there is no
  // controller to reach.
  controller?: {
    kubeContext?: string;
    namespace?: string;
  ...
  // The conduit's own account of the TLS material a browser has to accept: the
  // emulated per-host certificates and the fallback CA. Human-readable lines, not
  // paths to parse. Empty in native mode, where the real controller serves its own.
  trust?: string[];
}
...
export interface ObsMetricsStatus {
  ...
  failed: number;
  dropped: number;
  // stale counts readings the backend re-served unchanged, which the recorder
  // skips rather than writing twice. Not a fault: on Kubernetes the source is
  // metrics-server, whose scrape window is coarser than the sampling interval.
  // It is the answer to "why is this line sparser than the interval promises?"
  stale?: number;
  // unsupported names the workload metric families this server's deploy backend
  // has no source for at all, in the RECORDED spelling (OpenTelemetry dots:
  // `container.network.io`), which is not how a query spells them. Fold it with
  // `promName` in views/metrics/catalog.ts before comparing against a panel.
  //
  // Absent means no claim rather than full support — an older server does not
  // send the field — so a consumer must default to showing everything.
  unsupported?: string[];
}
