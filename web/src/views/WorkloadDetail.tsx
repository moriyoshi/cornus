...
import { DEFAULT_RANGE, panelsFor, rangeById, supported } from "./metrics/catalog";
...
function WorkloadMetrics(props: { workload: string }) {
  ...
  // Guarded, because reading a resource accessor RE-THROWS its error. Absent
  // means "no claim yet", which `supported` reads as "show everything".
  const unsupported = () => (obs.error ? undefined : obs()?.metrics?.unsupported);
  ...
  return (
    <>
      ...
      <Show when={!storeOff()}>
        ...
        <div class="panels">
          {/* Same filter as the dashboard: a family the deploy backend has no
              source for is dropped rather than drawn as a permanently blank
              chart. Both screens read it off the one status poll each already
              makes. */}
          <For each={supported(panelsFor("workloads"), unsupported())}>
            {(panel) => (
