        ...
        <table class="grid">
          <thead>
            <tr>
              ...
              <th>Kind</th>
              <th>Modified</th>
              {/* The spacer column. It has no header because it names nothing: it is where
                  the table's leftover width goes, so the name column can size to its
                  names. See .fs-pad. */}
              <th class="fs-pad" aria-hidden="true" />
            </tr>
          </thead>
          <tbody>
            <Show when={!atRoot()}>
            ...
            <For each={atRoot() ? current()?.refused : undefined}>
              {(r) => (
                <tr class="fs-refused">
                  <td class="fs-name-cell">
                    <span class="fs-name muted" title={r.path}>
                      🚫 {r.path}
                    </span>
                  </td>
                  <td colspan="4">
                ...
                >
                  <td class="fs-name-cell">
                    ...
                    >
                      ...
                      {/* The cell clips at its right edge (see .fs-name-cell), so the
                          tail of a long name is only recoverable from the tooltip. */}
                      <span class="fs-name" title={e.name}>
                        {e.name}
                      </span>
