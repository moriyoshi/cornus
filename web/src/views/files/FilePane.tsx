...
import { promptText, confirmModal, promptChoice } from "../../modal";
...
// The folder an in-app drag started from, shared by every pane because they are all
// instances of this module. A pane needs it to refuse a drop that would put the items
// back where they already are, and dragover cannot ask the payload: the dataTransfer is
// in protected mode there, exposing its TYPE list but none of its data.
...
// formatBytes renders a transfer total. formatSize above takes an entry; this takes a
// plain count, which is what a preflight reports.
...
export default function FilePane(props: {
  ...
  // the listing has changed for the rows that DID succeed.
  ...
  // only). DND_TYPE carries the in-app payload — virtual paths, so the receiving pane
  // never needs to know which pane sent them.
  const DND_TYPE = "application/x-cornus-fs";
  ...
  const rowDrag = (name: string): DragSourceSpec => ({
    start: (via: DragVia) => {
      ...
      setDragFrom(path()); // so no pane offers a drop back into this same folder
  ...
  // Copy or move? For a mouse the answer is already in the gesture: Shift makes it a move,
  // so the destructive one is the one you ask for. A FINGER HAS NO MODIFIER TO HOLD, and
  // the alternatives all put the choice somewhere it cannot be seen — a second long press,
  // a separate drop zone per verb, a mode set before the drag and forgotten during it. So an
  // emulated drop asks, once it has landed somewhere legal and knows what it is asking
  // about, and a dismissed question is a drop that never happened.
  const askCopyOrMove = async (destDir: string, items: DropItem[]): Promise<boolean | null> => {
    const what = items.length === 1 ? `"${baseOf(items[0].path)}"` : `${items.length} items`;
  ...
  // receiveInto is the in-app transfer itself, with no gesture left in it: N items, named
  // by virtual path, into one destination folder. Three callers, one implementation — a
  // drop (the folder the pointer was over), `receive` below (THIS pane's folder, when a
  // cross-pane command chose this pane), and transferElsewhere (a typed path).
  //
  // The first two are why it belongs to the DESTINATION pane. Almost everything it does is
  // about where the files land — the ghost rows, the arrival flash, the read-only refusal —
  // and run from the source pane a cross-pane copy would draw its ghosts in the wrong
  // listing. The third caller is the exception that has to be handled rather than assumed
  // away: a typed path may name a folder this pane knows nothing about, so each of those
  // destination-local behaviours is now conditioned on the destination actually being here.
  const receiveInto = async (destDir: string, incoming: DropItem[], move: boolean) => {
    ...
    // round trip to be told the same thing. It used to say "cannot DROP a folder into
    // itself" — the gesture's own word — which stopped being true when this became
    // reachable from a command as well. The OPERATION is the word both routes share, and
    ...
    // Ghosts go up BEFORE the preflight, not after it. Their whole job is that the pane
    // admits the transfer the instant it is dropped; deferring them until a round trip
    // answers would leave the drop looking like it did nothing. Anything the preflight
    // then refuses is wiped below — it was never a file.
    //
    // Ghosts only make sense for a drop into the folder this pane is showing; one onto a
    // subfolder row lands out of sight, and flashArrived marks that folder instead.
    const ghosting = destDir === path();
    if (ghosting) addPending(items.map((i) => ({ name: baseOf(i.path), dir: i.dir })));
    const wipeGhosts = (paths: string[]) => {
    ...
    wipeGhosts(refused.map((p) => p.from));
    ...
    wipeGhosts(going.filter((p) => !ok.includes(p)));
    report(items.length, failed, move ? "moved" : "copied");
    flashArrived(ok.map(baseOf), destDir);
    props.refreshAll(); // the SOURCE pane is usually a different one
  };

  // confirmTransfer decides whether a drop needs the user's word before it runs, and asks
  ...
  // nobody reads.
  const confirmTransfer = async (plan: FsPreflightResult, allowed: number, move: boolean): Promise<boolean> => {
    const refused = plan.items.filter((p) => p.action === "refused");
