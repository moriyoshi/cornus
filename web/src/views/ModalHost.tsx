...
function TextBody(props: { req: Extract<ModalRequest, { kind: "text" }>; onCancel: () => void }) {
  ...
  onMount(() => {
    input.focus();
    // Selected by default, because the initial value is normally a suggestion the first
    // keystroke should replace. caretAtEnd is for the case where it is not a suggestion but
    // the start of the user's own typing — see modal.ts.
    if (props.req.caretAtEnd) input.setSelectionRange(input.value.length, input.value.length);
    else input.select();
  });
...
function ChoiceBody(props: { req: Extract<ModalRequest, { kind: "choice" }>; onCancel: () => void }) {
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    if (e.key === "Escape") {
      e.preventDefault();
      props.onCancel();
      // Up/Down alias Left/Right unconditionally: they are the only way through the
      // "list" layout, and in a row they are what a hand reaching for arrow keys is as
      // likely to press. The traversal is linear either way.
  ...
  return (
    <div class="modal-choice" tabindex={-1} ref={panel} onKeyDown={onKeyDown}>
      <div class="modal-options" classList={{ list: props.req.layout === "list" }}>
      ...
      {/* The hint names the axis the layout actually reads along, so it never tells the
          reader to press a key that moves the selection sideways off a column. */}
      <p class="modal-hint muted">
        {props.req.layout === "list" ? "↑ ↓" : "← →"} to move · Enter to choose · Esc to cancel
      </p>
    </div>
  );
}
