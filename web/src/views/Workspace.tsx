...
import { TreeNode, PickBackdrop, PANE_TAG, type TileCtx } from "./tiling/panes";
...
export default function Workspace() {
  ...
  const commit = (tree: Node<PaneData>) => setState("tree", reconcile(tree));
  ...
  const splitAt = (paneId: string, dir: Dir, before: boolean) => {
    const { tree, newPaneId } = splitPane(snapshot(), paneId, dir, inheritLive, before);
  ...
  const refuseDestination = (id: string, from: string): string | undefined => {
    ...
    if (!pane) return "gone";
    // A terminal is a place you can cd to, not a place files land. Refused by kind rather
    // than by the absence of a `path`, so the row says what it is instead of the file
    // pane's "the mount list".
  ...
  return (
    <div class="workspace">
      ...
      <Show when={ctx.choose.selected()}>
        <PaneChooser ctx={ctx} tree={() => state.tree} />
      </Show>
    </div>
  );
}
