...
import { setUnservedMetrics, setUnsupportedMetrics } from "../../mock/metrics";
...
// Panels the deploy backend has no source for are removed, not drawn empty.
//
// The kubernetes case is the one this exists for: metrics.k8s.io reports CPU
// (as a rate) and memory and nothing else, so four of the seven workload panels
// could never fill on that backend and each one used to render as a labelled
// blank chart explaining that it would stay blank.
describe("Metrics (families the backend has no source for)", () => {
  // Exactly what pkg/deploy/kubernetes reports, in the RECORDED spelling the
  // server sends — dots, not underscores. Written out rather than derived, so a
  // change to either spelling fails here instead of silently agreeing with
  // itself.
  const KUBE_UNSUPPORTED = [
    "container.cpu.time",
    "cornus.container.pids",
    "container.network.io",
    "container.disk.io",
  ];

  it("drops them from the dashboard and keeps the ones that do report", async () => {
    setUnsupportedMetrics(KUBE_UNSUPPORTED);
    renderMetrics();

    // The instantaneous CPU panel is kubernetes's own, so it must survive while
    // the cumulative one goes.
    await panel("CPU (instantaneous)");
    await panel("Memory in use");
    for (const gone of ["CPU", "Network I/O", "Disk I/O", "Processes"]) {
      expect(screen.queryByRole("heading", { name: gone })).toBeNull();
    }
  });

  it("says which ones it dropped, so a missing chart is answerable", async () => {
    setUnsupportedMetrics(KUBE_UNSUPPORTED);
    renderMetrics();
    await panel("Memory in use");

    const note = await screen.findByText(/Not collected on this deploy backend/);
    for (const title of ["CPU", "Network I/O", "Disk I/O", "Processes"]) {
      expect(note.textContent).toContain(title);
    }
  });

  // The absence of a claim is not a claim of absence. A server that sends no
  // list — an older one, or a backend that declares nothing — must get the whole
  // dashboard, or an upgrade path silently blanks it.
  it("shows every panel when the server declares nothing", async () => {
    renderMetrics();
    for (const title of ["CPU", "CPU (instantaneous)", "Memory in use", "Network I/O", "Disk I/O", "Processes"]) {
      await panel(title);
    }
    expect(screen.queryByText(/Not collected on this deploy backend/)).toBeNull();
  });

  // Server panels are a different population with a different source (the cornus
  // process itself), so a deploy backend's gaps must not touch them — including
  // the note, which would otherwise describe panels the reader is not looking at.
  it("leaves the server scope alone", async () => {
    setUnsupportedMetrics(KUBE_UNSUPPORTED);
    renderMetrics("?scope=server");
    await panel("Server CPU");
    await panel("Goroutines");
    expect(screen.queryByText(/Not collected on this deploy backend/)).toBeNull();
  });
});
