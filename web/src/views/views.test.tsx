...
import { PIN_MIN_VIEWPORT_PX } from "./tiling/gutter";
import { ZOOM_HOME, ZOOM_SCALES, keepZooms, zoomLevels } from "./tiling/zoom";
import { EXISTING_SHARE, GOLDEN, MIN_PANE_WIDTH_REM } from "./tiling/grow";
import { MAP_MAX_W_REM, bulletGrid } from "./tiling/minimap";
...
// --- the geometry a reveal reads ---------------------------------------------------------
// `revealTile` measures two boxes — the scroll container and the tile — and scrolls the
// container by the difference. jsdom lays nothing out, so every rect is zero, every difference
// is zero and nothing is ever scrolled: a test driven through the DOM would pass exactly as
// happily for a reveal that had been deleted. So the two boxes are given a geometry here.
//
// ON THE PROTOTYPE, and that is the load-bearing part: a split rebuilds the tile elements, so a
// stub installed on the nodes would be thrown away by the very operation under test.
const REVEAL_VIEWPORT: [number, number] = [0, 400];
// One row of tiles across a 400px screen. The spans are deliberately not interchangeable — the
// first is WIDER THAN THE SCREEN and overhangs its left edge, the second sticks out past its
// right — because which tile was revealed is read out of how far the container scrolled, and two
// tiles that scrolled it equally would make that reading meaningless.
const REVEAL_TILES: [number, number][] = [
  [-500, 100],
  [100, 500],
];

const revealRect = ([start, end]: [number, number]): DOMRect =>
  ({
    x: start,
    y: 0,
    left: start,
...
// laidOut gives the scroll container and the tiles a place, for the length of one test. Anything
// else keeps whatever jsdom says, so the components that measure themselves are unaffected.
function laidOut(spans: [number, number][] = REVEAL_TILES) {
  const real = Element.prototype.getBoundingClientRect;
  const stub = function (this: Element): DOMRect {
    if (this.classList.contains("workspace-body")) return revealRect(REVEAL_VIEWPORT);
    if (this.classList.contains("stack")) {
      const span = spans[tiles().indexOf(this as HTMLElement)];
      if (span) return revealRect(span);
    }
...
// Every scroll the workspace's container was asked to make, in order. jsdom implements no
// scrollBy, so this is the spy AND the implementation: nothing actually moves, which is what
// keeps each assertion about the reveal's own arithmetic rather than about an accumulated
// offset. `revealTile` guards the call, so without this stub the reveal is silently a no-op —
// hence a test that asserts a scroll HAPPENED as well as how far.
function watchScroll(): { left: number; top: number }[] {
  const seen: { left: number; top: number }[] = [];
  Object.defineProperty(Element.prototype, "scrollBy", {
    value(this: Element, how: ScrollToOptions) {
      if (this.classList.contains("workspace-body")) seen.push({ left: how.left ?? 0, top: how.top ?? 0 });
    },
    configurable: true,
    writable: true,
  });
...
//
// Module-scope because both kinds of pane receive this drag: a file pane into the folder
// it is showing, a terminal pane into the directory its session reports.
...
// The panes in document order, by the identity their tabs carry. Two panes showing the
// same thing are indistinguishable by label, so this is the only way to say that a
// particular pane is the one that moved.
const paneIds = () =>
  Array.from(document.querySelectorAll<HTMLElement>(".tab")).map((e) => e.dataset.paneId);
...
describe("Overview (project-oriented dashboard)", () => {
  ...
  it("names the deploy backend in the Server card, with what it can do", async () => {
    ...
    // Ingress is NOT here any more, in either direction: it moved to its own
    // section, and a card that kept a stale copy would let the two disagree.
    expect(labels.map((d) => d.textContent)).not.toContain("Ingress");
  });
...
// The Ingress section: two independent halves of one setting — the front door the
// SERVER advertises, and how THIS CLIENT's conduit realizes ingress. Every
// assertion below reads through the section element (or through the <dt> a value
// hangs off), never "the string appears somewhere on the page": the fixture puts
// "shop.test" in both halves and in a workload row, so a bare text lookup would
// pass while the value sat under the wrong label, in the wrong half, entirely.
describe("Overview ingress section", () => {
  let restore: () => void;
  beforeEach(() => {
    restore = installMockFetch();
  });
  ...
  // Renders the Overview and returns its ingress section. It renders rather than
  // taking an already-rendered page so the empty-state test below can seed, render,
  // cleanup, and seed again inside one `it` — the two states it compares are only
  // reachable from two different configs.
  const section = async () => {
    renderView(Overview);
    await screen.findByRole("heading", { name: "Ingress" });
    // The heading is static markup and appears before the config resource answers,
    // so waiting on it alone would read a section still saying "loading…" — and an
    // absence assertion against a loading section passes for the wrong reason.
    await waitFor(() =>
      expect(document.getElementById("ingress")!.textContent).not.toContain("loading…"),
    );
    return document.getElementById("ingress") as HTMLElement;
  };
  // The value rendered under a given label, scoped to one <h3> block — which is what
  // keeps "Controller" in the front-door half from answering for the client half's
  // row of the same name.
  const valueUnder = (el: HTMLElement, label: string) =>
    [...el.querySelectorAll("dt")].find((d) => d.textContent === label)?.nextElementSibling
      ?.textContent;
  const labelsIn = (el: HTMLElement) => [...el.querySelectorAll("dt")].map((d) => d.textContent);
  const half = (s: HTMLElement, heading: string) => {
    const h = [...s.querySelectorAll("h3")].find((x) => x.textContent === heading)!;
  ...
  // The reason the section exists: these three facts ride on the wire and the
  // Server card rendered none of them, because its ~150px value column had no room.
  it("names the class, listen address and controller the Server card could not fit", async () => {
    seedServerInfo({
      backend: { name: "kubernetes", reportsHealth: true },
      ingress: {
        domain: "shop.test",
        class: "nginx",
        emulated: false,
    ...
    const front = half(await section(), "Front door");
    expect(valueUnder(front, "Mode")).toBe("cluster");
    expect(valueUnder(front, "Base domain")).toBe("shop.test");
    expect(valueUnder(front, "Class")).toBe("nginx");
    const ctl = valueUnder(front, "Controller")!;
    expect(ctl).toContain("ingress-nginx/ingress-nginx-controller");
    expect(ctl).toContain("80");
    expect(ctl).toContain("443");
    // A cluster front door has no listen address of its own to report, so the row
    // is absent rather than dashed — a dash would read as "bound to nothing".
    expect(valueUnder(front, "Listen")).toBeUndefined();
  });

  // An emulated front door with no CORNUS_INGRESS_LISTEN is reachable only through
  // an ingress tunnel. That is a live state, not a missing value, and it has to
  // read as one.
  it("says an emulated front door is tunnel-only when it is bound to nothing", async () => {
    seedServerInfo({ ingress: { domain: "shop.test", emulated: true } });
    const front = half(await section(), "Front door");
    expect(valueUnder(front, "Mode")).toBe("emulated");
    expect(valueUnder(front, "Listen")).toContain("ingress tunnel");
    // And the controller absence is named rather than dropped: it is WHY a client
    // has to fall back to emulating ingress itself.
    expect(valueUnder(front, "Controller")).toContain("none discovered");
  });

  // A server omits `ingress` when it has no front door to advertise OR when a
  // cluster lookup failed. Neither is "this server has no ingress", so the section
  // must describe the silence and invent no mode.
  it("reports an unadvertised front door without claiming the server has none", async () => {
    seedServerInfo({ backend: { name: "dockerhost", reportsHealth: true } });
    const front = half(await section(), "Front door");
    expect(front.textContent).toContain("advertised no ingress front door");
    expect(front.querySelector(".badge")).toBeNull();
  });

  // The client half — the one the Server card could not have shown at all, since
  // the server does not know it. Emulate mode mints certificates the machine does
  // not trust yet, so the trust lines are what turn a TLS error into an action.
  it("describes how this client realizes ingress, including what to trust", async () => {
    const client = half(await section(), "This client");
    expect(valueUnder(client, "Mode")).toBe("emulate");
    expect(valueUnder(client, "Domain")).toBe("shop.test");
    // The row's presence and its content asserted apart, so a dropped row and a
    // wrong path fail with different diagnostics.
    expect(labelsIn(client)).toContain("Trust");
    expect(valueUnder(client, "Trust")).toContain("/home/dev/.cache/cornus/ingress-ca.pem");
    // Emulate mode has no controller to reach, so no row claims one.
    expect(valueUnder(client, "Controller")).toBeUndefined();
  });

  it("names the controller a native client tunnels to, and reports no trust of its own", async () => {
    seedConfig({
      clientIngress: [
        {
          mode: "native",
    ...
    const client = half(await section(), "This client");
    expect(valueUnder(client, "Mode")).toBe("native");
    expect(labelsIn(client)).toContain("Controller");
    const ctl = valueUnder(client, "Controller")!;
    expect(ctl).toContain("ingress-nginx/ingress-nginx-controller");
    expect(ctl).toContain("kind-cornus");
    // The real controller serves its own certificate, so there is nothing for the
    // browser to be told to trust — and an empty Trust row would imply otherwise.
    expect(valueUnder(client, "Trust")).toBeUndefined();
  });

  // THE assertion this design exists for. "The agent routes no ingress" and "no
  // agent answered, so the setting is unknown" are different answers, and an
  // implementation that collapsed them would still render a plausible sentence in
  // both cases — which is exactly what nobody would notice.
  it("tells a client that routes no ingress apart from one it could not ask", async () => {
    seedConfig({ clientIngress: [], agentLive: true });
    const routesNone = half(await section(), "This client").textContent!;
    expect(routesNone).toContain("routes no ingress");
    expect(routesNone).not.toContain("unknown");

    cleanup();
    seedConfig({ clientIngress: [], agentLive: false });
    const noAgent = half(await section(), "This client").textContent!;
    expect(noAgent).toContain("unknown");
    expect(noAgent).not.toContain("routes no ingress");
  });

  // A section that vanishes when it has nothing to report leaves a reader unable to
  // tell "nothing configured" from "this dashboard never had that section", so both
  // halves stay present and speak.
  it("keeps both halves on the page when there is nothing to report", async () => {
    seedConfig({ server: undefined, clientIngress: [], agentLive: false });
...
describe("Workspace (file browser)", () => {
  ...
  // A name is one line, always. Wrapping made a row as tall as its name was long, and this
  // listing is a thing you scan down and sweep across: the band select measures rows by
  // offsetTop, so a row whose height depends on its text is a row you have to look at to
  // predict.
  //
  // Two halves, because either alone passes for the wrong reason. The CELL has to be the one
  // the clipping rule names — a row still rendering `td.wrap` wraps no matter how the rule
  // reads — and the RULE has to clip against a definite WIDTH: nowrap alone in a
  // content-sized table column just widens the column, which is the same row-shape problem
  // wearing a horizontal scrollbar.
  it("keeps a listing row one line tall, however long the name", async () => {
    renderView(Workspace);
    fireEvent.click(await screen.findByText("project"));
    await screen.findByText("compose.yaml");

    const names = Array.from(document.querySelectorAll<HTMLElement>(".fs-list tbody .fs-name"));
    expect(names.length).toBeGreaterThan(0);
    for (const name of names) {
      const cell = name.closest("td")!;
      if (cell.colSpan > 1) continue; // ".." spans the row: a control, not a name to clip
      expect(cell.className).toContain("fs-name-cell");
      expect(cell.className).not.toContain("wrap");
      // The tail is off the screen now, so it has to be somewhere: the title is where.
      expect(name.getAttribute("title")).toBe(name.textContent);
    }

    const rule = block(cssSource, ".fs-list table.grid td.fs-name-cell {");
    expect(rule).toMatch(/white-space:\s*nowrap/);
    expect(rule).toMatch(/overflow:\s*hidden/);
    expect(rule).toMatch(/text-overflow:\s*ellipsis/);
    // The edge the ellipsis truncates against. A content-sized table column has none, so a
    // nowrap name with no cap widens the table instead of clipping — the same defect with a
    // horizontal scrollbar on it. The cap has to be a LENGTH: a percentage max-width on a
    // table cell is undefined in the table algorithm, and Chromium duly ignores it.
    expect(rule).toMatch(/max-width:\s*min\(\d+cqw,\s*calc\(100cqw - [\d.]+rem\)\)/);
  });

  // The name column takes its content's width; the slack goes to a trailing spacer column.
  // Two things have to hold for that, and neither is visible in jsdom, which computes no
  // layout — so this pins the STRUCTURE the layout rests on, and the widths themselves are
  // measured in a real engine (see the journal entry for 2026-08-09).
  //
  // The colspan sum is the half that is easy to get wrong later: a full-row cell left at 4
  // when the table has 5 columns does not throw, it just stops short of the last column, and
  // the ".." row and the empty-folder line are exactly the rows nobody looks at twice.
  it("gives the listing a spacer column for the slack, and keeps every row spanning it", async () => {
    renderView(Workspace);
    fireEvent.click(await screen.findByText("project"));
    await screen.findByText("compose.yaml");

    const head = Array.from(document.querySelectorAll(".fs-list thead th"));
    expect(head).toHaveLength(5);
    expect(head.at(-1)!.className).toContain("fs-pad");
    expect(head.at(-1)!.textContent).toBe(""); // it names nothing; it is width, not a column of data

    for (const row of Array.from(document.querySelectorAll<HTMLTableRowElement>(".fs-list tbody tr"))) {
      const cells = Array.from(row.cells);
      expect(cells.reduce((n, c) => n + c.colSpan, 0)).toBe(5);
      // A row of real cells ends in the spacer; a full-row cell (".." , "Empty folder.")
      // has already covered that column with its colspan.
      if (cells.length > 1) expect(cells.at(-1)!.className).toContain("fs-pad");
    }

    // The spacer is the column holding `width: 100%` — that is what makes it, and not the
    // name column, the one the leftover width lands on.
    expect(block(cssSource, ".fs-list table.grid .fs-pad {")).toMatch(/width:\s*100%/);
    // And the cap above is in cqw, which is only the LISTING's width if the listing declares
    // itself a container. Without this, cqw silently resolves against the viewport and a
    // narrow split pane gets a name column sized for the whole window.
    expect(block(cssSource, ".fs-list {")).toMatch(/container-type:\s*inline-size/);
  });
  ...
  // --- Drag and drop -----------------------------------------------------------------
  // The transfer stand-in and the drop gesture are module-scope (see dataTransfer above):
  // a terminal pane receives the SAME payload, and two definitions of what a drag carries
  // would let the two receivers be tested against different drags.
  ...
  it("shows the same tile menu whichever kind of pane is focused", async () => {
    ...
    expect(onTerminal).toEqual({
      ...
      "ws:choose": "s",
      "ws:rotate": "Ctrl+O",
      // tmux's select-layout even-horizontal, on SHIFT-h rather than tmux's Alt+1: the
      // number indexes a list of five layouts, four of which this workspace does not have.
      "ws:even-h": "H",
      "ws:close": "x",
      // Resizing, on tmux's own prefix Alt+arrow. Tagged because a modifier on a 6px divider is
      // not a gesture a touch device has, and the ⋮ is where that device finds every operation.
      // Listed here even in a one-tile layout, where all four are disabled with a reason.
      "ws:resize-wider": "Alt+ArrowRight",
      "ws:resize-narrower": "Alt+ArrowLeft",
      "ws:resize-taller": "Alt+ArrowDown",
      "ws:resize-shorter": "Alt+ArrowUp",
    });
...
describe("Workspace (terminals)", () => {
  ...
  // REGRESSION: a tile could be dragged past its neighbour ONCE, and then neither tile
  // would ever cross the other again.
  //
  // The whole-stack handle used to be registered with the stack id its component held at
  // MOUNT. That id is not stable: a move rebuilds the tree, and reconcile then merges the
  // new shape into the store by `id`, so the stack sitting at a given position simply
  // becomes a different object — while the StackView above it stays mounted, because
  // TreeNode's Switch only re-creates a branch when a node changes KIND. The two tiles
  // therefore swapped their contents and kept each other's drag ids, and every later drop
  // resolved source and destination to the same stack, which moveStack correctly refuses.
  // The handle has to read the id it has NOW, hence the accessor.
  //
  // Pane ids, not tab labels: two empty terminal panes are identical on screen, and the
  // only thing that says WHICH one ended up on the left is the identity the tab carries.
  it("keeps a tile draggable after two tiles have swapped places", async () => {
    renderTerm();
    await splitViaEdge("right");
    expect(document.querySelectorAll(".stack")).toHaveLength(2);
    const [left, right] = paneIds();

    // Carry the right-hand tile past the other one, by the blank of its tab bar.
    dropTabOn(document.querySelectorAll<HTMLElement>(".stack-tabs")[1], 0, "left");
    await waitFor(() => expect(paneIds()).toEqual([right, left]));
    expect(document.querySelectorAll(".stack")).toHaveLength(2);

    // …and back again, which is the step that stopped working.
    dropTabOn(document.querySelectorAll<HTMLElement>(".stack-tabs")[0], 1, "right");
    await waitFor(() => expect(paneIds()).toEqual([left, right]));
    expect(document.querySelectorAll(".stack")).toHaveLength(2);
  });
...
describe("Workspace (one tree, two kinds of pane)", () => {
  ...
  // ---- a terminal as a transfer destination ------------------------------------------
  //
  // A terminal is a place a shell is STANDING, and since the BFF sniffs that directory off
  // OSC 7 it is a folder in the same virtual namespace the file panes browse. So files can
  // be sent to it — by the chooser and by a drop — and the whole feature hangs on the
  // destination being the LIVE directory rather than the `dir` the pane was launched with.
  // Every test below is built so that reading `dir` would land somewhere else.

  // The requests, so a destination can be observed as the thing that was actually asked for
  // rather than inferred from a listing that several paths could produce. Returns the query
  // strings of the transfer calls, which is where the destination folder rides.
  function watchTransfers() {
    ...
    return (op: "copy" | "move") =>
      seen
        .filter((c) => c.method === "POST" && c.url.includes(`/fs/${op}?`))
        .map((c) => new URLSearchParams(c.url.split("?")[1]).get("path"));
  }

  // The terminal pane in these tests: attached to shop-web, LAUNCHED in /app (a directory
  // the container fixture does not even have) and standing in /etc/nginx (one it does). A
  // transfer that read the launch dir would therefore fail outright rather than land
  // somewhere plausible, and the toast would say so.
  //
  // The mock filesystem is stateful for the whole file, so each test below sends a
  // DIFFERENT file: reusing one would meet its own leftover at the destination and stop on
  // the overwrite confirmation, which is a dialog no assertion here is about.
  const seedTermDest = (cwd: string | undefined) => {
    seedPanes([
      { kind: "files", path: "project/many-files" },
      { kind: "term", workload: "shop-web", cmd: ["/bin/ash"], dir: "/app", sessionId: "term-live" },
    ]);
    seedTerminals([
      { id: "term-live", workload: "shop-web", cmd: ["/bin/ash"], alive: true, title: "ash", ...(cwd ? { cwd } : {}) },
    ]);
  };

  it("sends a chosen transfer to the directory the terminal reports, not its launch one", async () => {
    seedTermDest("/etc/nginx");
    const transfers = watchTransfers();
    renderView(Workspace);
    // Wait for the session poll to have ANSWERED: the cwd arrives on it, and a chooser
    // opened in the first frame legitimately has no destination yet.
    await waitFor(() => expect(tabNames()[1]).toBe("shop-web  ash"), { timeout: 5000 });
    fireEvent.click((await within(tiles()[0]).findByText("file-070.png")).closest("tr")!);
    ...
    // The terminal is OFFERED — the half that used to read "a terminal" and be disabled.
    expect(rows[1].classList.contains("disabled")).toBe(false);
    fireEvent.click(rows[1]);

    await waitFor(() => expect(document.querySelector(".pane-chooser")).toBeNull());
    expect(await screen.findByText("copied 1 item")).toBeInTheDocument();
    // WHERE it went, asked of the request: the container path became a virtual path under
    // the workload mount. `shop-web/app` is what reading the pane's `dir` would produce.
    expect(transfers("copy")).toEqual(["shop-web/etc/nginx"]);

    // And it is really there: the source pane walked to the destination finds the file.
    fireEvent.click(within(tiles()[0]).getByText("All"));
    fireEvent.click(await within(tiles()[0]).findByText("shop-web"));
    fireEvent.click(await within(tiles()[0]).findByText("etc"));
    fireEvent.click(await within(tiles()[0]).findByText("nginx"));
    expect(await within(tiles()[0]).findByText("file-070.png")).toBeInTheDocument();
  });

  // The refusal that remains. A shell that never reports where it is cannot receive
  // anything, and the row says WHICH fact refused it — "a terminal" (the old blanket
  // answer) would now be wrong, and the file pane's "the mount list" was never true here.
  it("refuses a terminal that never says where it is, naming that fact", async () => {
    seedTermDest(undefined);
    renderView(Workspace);
    await waitFor(() => expect(tabNames()[1]).toBe("shop-web  ash"), { timeout: 5000 });
    fireEvent.click((await within(tiles()[0]).findByText("file-073.tsx")).closest("tr")!);
    ...
    expect(within(rows[1]).getByText("this shell does not report its directory")).toBeTruthy();
    fireEvent.keyDown(window, { key: "Escape" });
  });

  // The drop is the same transfer by the other gesture, and the terminal has to take it
  // where xterm would otherwise swallow it: an unprevented dragover is not a drop target at
  // all, and the browser then pastes the dragged text into the focused textarea.
  it("takes a drop on the terminal itself, into the directory it reports", async () => {
    seedTermDest("/etc/nginx");
    const transfers = watchTransfers();
    renderView(Workspace);
    await waitFor(() => expect(document.querySelector(".term-drop")).toBeTruthy());
    await waitFor(() => expect(tabNames()[1]).toBe("shop-web  ash"), { timeout: 5000 });
    const term = document.querySelector(".term-drop")!;

    const dragged = "project/many-files/file-071.sql";

    // DRAGOVER: preventDefault is what marks a drop target, and the ring is what says so
    // on screen — the same ring a file pane draws for the same drag.
    const over = createEvent.dragOver(term, { dataTransfer: payload(dragged) });
    fireEvent(term, over);
    expect(over.defaultPrevented).toBe(true);
    await waitFor(() => expect(term.classList.contains("fs-drop-here")).toBe(true));

    dropOn(term, payload(dragged));
    expect(await screen.findByText("copied 1 item")).toBeInTheDocument();
    expect(transfers("copy")).toEqual(["shop-web/etc/nginx"]);
    // The ring is gone with the drag: one left standing would mark a pane that is no
    // longer receiving anything.
    await waitFor(() => expect(term.classList.contains("fs-drop-here")).toBe(false));
    // A copy, so the source still has it.
    expect(await within(tiles()[0]).findByText("file-071.sql")).toBeInTheDocument();
  });

  // Shift is the move, exactly as it is over a file pane. The wire is the only place the
  // two are distinguishable: a regression to copy leaves the destination looking identical.
  it("makes a shifted drop on a terminal a move", async () => {
    seedTermDest("/etc/nginx");
    const transfers = watchTransfers();
    renderView(Workspace);
    await waitFor(() => expect(document.querySelector(".term-drop")).toBeTruthy());
    ...
    // The source pane must be LISTED before the move, or "the row is gone" is true of a
    // listing that never had it.
    expect(await within(tiles()[0]).findByText("file-072.ts")).toBeInTheDocument();

    dropOn(document.querySelector(".term-drop")!, payload("project/many-files/file-072.ts"), true);
    expect(await screen.findByText("moved 1 item")).toBeInTheDocument();
    expect(transfers("move")).toEqual(["shop-web/etc/nginx"]);
    expect(transfers("copy")).toEqual([]);
    // Moved OUT of the source, which is the half a copy would fail — and the pane showing
    // it re-listed itself, which is what refreshAll is for.
    await waitFor(() => expect(within(tiles()[0]).queryByText("file-072.ts")).toBeNull());
  });

  // A terminal with no directory to report refuses the drop as well as the chooser row —
  // and refusing means the event is NOT prevented, so it falls through to whatever encloses
  // the pane instead of being silently eaten.
  it("refuses a drop on a terminal that does not report a directory", async () => {
    seedTermDest(undefined);
    const transfers = watchTransfers();
    renderView(Workspace);
    await waitFor(() => expect(document.querySelector(".term-drop")).toBeTruthy());
    ...
    const dragged = "project/many-files/file-074.go";
    const over = createEvent.dragOver(term, { dataTransfer: payload(dragged) });
    fireEvent(term, over);
    expect(over.defaultPrevented).toBe(false);
    expect(term.classList.contains("fs-drop-here")).toBe(false);

    // And if one arrives anyway — a refused dragover means the browser never fires a drop,
    // but nothing in the DOM enforces that — it is still refused, silently and with no
    // request behind it.
    dropOn(term, payload(dragged));
    await waitFor(() => expect(transfers("copy")).toEqual([]));
    expect(transfers("move")).toEqual([]);
  });
...
describe("Tiling chrome without a pointer (touch)", () => {
  ...
  // Pin the workspace to the DIVIDING layout for one test. Restored per test, like the
  // disposition helper above and for the same reason: it is module-level state that the rest of
  // the file shares.
  const dividing = () => {
    onTestFinished(() => setWorkspaceGrowth("extend"));
    setWorkspaceGrowth("divide");
  };
  ...
  it("opens the palette on the pane tag instead of a menu of its own", async () => {
    ...
    expect(tagged).toEqual([
      ...
      "Rotate panes",
      "Even horizontal",
      // Grouped with the rearranging commands, not the creating ones. Present here in a
      // two-pane layout with the horizontal pair enabled and the vertical pair disabled with a
      // reason — the split was left/right, so there is nothing above or below to resize against.
      "Make pane wider",
      "Make pane narrower",
      "Make pane taller",
      "Make pane shorter",
  ...
  // Run under the DIVIDING layout, which is the only thing here that depends on it: the grab
  // offset is the same code on both paths, but the arithmetic below is written around a split
  // that starts at dead centre, and the extending layout starts it at 0.6. Re-deriving the
  // handle's centre from the live ratio would make the test agree with whatever the code did,
  // which is the one thing an invariant about a dropped offset must not do.
...
// The workspace that is bigger than the screen.
//
// The arithmetic has its own suite (views/tiling/grow.test.ts), which is where the geometry is
// pinned tile by tile. What is left for this file is everything that arithmetic cannot see: that
// the extent reaches the DOM as a canvas the body scrolls, that the setting really does put the
// old behaviour back, that Alt is read off the press rather than the move, and that focus drags
// the view along behind it.
describe("The extending workspace", () => {
  ...
  // The scroll container reports a size, which jsdom otherwise puts at zero — and a zero would
  // make the px-to-extent conversion infinite, so resizeExtend refuses it and the Alt drag would
  // silently do nothing. Stubbed to 400 to match SPLIT_RECT, so a 100px drag is a quarter screen.
  const measurableBody = () => Object.defineProperty(body(), "clientWidth", { value: 400 });

  const altEvent = (type: string, clientX: number, alt: boolean) => {
    const ev = new MouseEvent(type, { clientX, clientY: 200, bubbles: true, button: 0, altKey: alt });
    Object.defineProperty(ev, "pointerId", { value: 1 });
    Object.defineProperty(ev, "isPrimary", { value: true });
    return ev;
  };

  it("adds a pane beside the one it split instead of halving it", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    // One tile: the workspace is exactly the screen, so nothing scrolls and the canvas is the
    // body. This is the state every layout saved before the feature existed loads into.
    ...
    // The workspace is multiplied by the golden ratio; what was already there shares two
    // thirds of that and the new tile gets the remaining third, so the split sits at ⅔.
    expect(parseFloat(canvas().style.width)).toBeCloseTo(GOLDEN * 100, 2);
    expect(canvas().style.height).toBe("100%"); // the other axis is untouched
    expect(growOf(0)).toBeCloseTo(EXISTING_SHARE, 5);
  });

  // The two floors, and that the host reaches for the right one per axis.
  //
  // The viewport is chosen so that ONE shared constant could not produce this result: 900x800
  // puts half the width (450px) under the 640px width floor and half the height (400px) over the
  // 320px height floor, so the two axes must disagree. Under a single 40rem constant the height
  // floor would be 640px and a vertical split would extend too — the test would go green for a
  // build that had never been split into two constants. It also catches the axes being swapped,
  // which is the other way this reads correctly and behaves inside out.
  it("uses the width floor for a left/right split and the height floor for a top/bottom one", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    ...
    // Left/right: halving would leave 450px-wide panes, under the 40rem width floor.
    //
    // The new pane is the FLOOR (640px), not two thirds of 900px (600px), because two thirds
    // would itself be under the floor that made this extend — so the workspace ends up
    // 1 + 640/900 screens wide. Written as that arithmetic rather than as 171.11, so the
    // number says where it comes from.
    // The new tile is the FLOOR (640px on a 900px body), because the rule's own third of the
    // grown workspace (0.539) is under it. The existing tile is still scaled by ⅔φ, so the
    // workspace is that plus the floor.
    const wFloorUnits = (MIN_PANE_WIDTH_REM * 16) / 900;
    const scale = EXISTING_SHARE * GOLDEN;
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(2));
    expect(parseFloat(canvas().style.width)).toBeCloseTo((scale + wFloorUnits) * 100, 2);
    expect(canvas().style.height).toBe("100%"); // the other axis has not been asked yet

    // Top/bottom on the pane that split just made: halving leaves 400px, over the 20rem height
    // floor, so this one is an ordinary split and the workspace does not get any taller.
    // Addressed by index because both tiles now offer a bottom edge.
    const bottoms = await screen.findAllByRole("button", {
      name: "Split pane, new pane on the bottom",
    });
    dwellOnEdge(bottoms[1], "bottom");
    fireEvent.click(bottoms[1]);
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(3));
    expect(canvas().style.height).toBe("100%");
    expect(parseFloat(canvas().style.width)).toBeCloseTo((scale + wFloorUnits) * 100, 2); // no wider
  });

  // The off switch, and the reason it is worth having: it is the OLD CODE, so this asserts the
  // old numbers exactly rather than approximately.
  it("halves the pane and never scrolls under the dividing layout", async () => {
    dividing();
    renderTerm();
    await screen.findAllByRole("combobox");
    await splitViaEdge("right");
    ...
    expect(growOf(0)).toBeCloseTo(0.5, 5);
    expect(canvas().style.width).toBe("100%");
    expect(canvas().style.height).toBe("100%");
  });

  it("persists the extent, and reads a layout saved without one as exactly one screen", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    // seedPanes writes a blob with no `ext` at all — the shape every layout in localStorage had
    // before this feature — so this half is the backward-compatibility case, not a fixture.
    expect(canvas().style.width).toBe("100%");

    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(2));
    await waitFor(() => {
      const saved = JSON.parse(globalThis.localStorage?.getItem(WS_LAYOUT_KEY) || "{}");
      expect(saved.ext.w).toBeCloseTo(GOLDEN, 5);
      expect(saved.ext.h).toBe(1);
    });
  });

  it("grows the workspace on an Alt-dragged divider and trades extent on a plain one", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelector(".split")).toBeTruthy());
    ...
    // PLAIN: the two sides trade, so the workspace is the size it was.
    divider.dispatchEvent(altEvent("pointerdown", 200, false));
    window.dispatchEvent(altEvent("pointermove", 300, false));
    window.dispatchEvent(pointerEvent("pointerup", { clientX: 300, clientY: 200 }));
    expect(parseFloat(canvas().style.width)).toBeCloseTo(wide, 3);

    // ALT: the side being dragged grows and the workspace grows with it. 100px of a 400px
    // screen is a quarter of a viewport.
    divider.dispatchEvent(altEvent("pointerdown", 200, true));
    window.dispatchEvent(altEvent("pointermove", 300, true));
    window.dispatchEvent(pointerEvent("pointerup", { clientX: 300, clientY: 200 }));
    expect(parseFloat(canvas().style.width)).toBeCloseTo(wide + 25, 2);
  });

  // The latch. A drag means what it meant when it began — otherwise letting go of a modifier
  // halfway through would change the gesture under the user's hand, and the divider would stop
  // agreeing with the pointer.
  it("keeps the gesture it started with when Alt is released mid-drag", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelector(".split")).toBeTruthy());
    ...
    // Pressed WITH Alt, moved without: still an extend.
    divider.dispatchEvent(altEvent("pointerdown", 200, true));
    window.dispatchEvent(altEvent("pointermove", 300, false));
    window.dispatchEvent(pointerEvent("pointerup", { clientX: 300, clientY: 200 }));
    expect(parseFloat(canvas().style.width)).toBeCloseTo(wide + 25, 2);

    // Pressed WITHOUT Alt, moved with: still a plain trade.
    const afterExtend = parseFloat(canvas().style.width);
    divider.dispatchEvent(altEvent("pointerdown", 200, false));
    window.dispatchEvent(altEvent("pointermove", 260, true));
    window.dispatchEvent(pointerEvent("pointerup", { clientX: 260, clientY: 200 }));
    expect(parseFloat(canvas().style.width)).toBeCloseTo(afterExtend, 3);
  });

  // Even horizontal, driven through the real prefix path. The menu inventory above asserts the
  // bind STRING; this asserts the key actually reaches the command and rearranges the tiling,
  // which a table of strings cannot say.
  it("puts every tile in one row on prefix H", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    measurableBody();
    // A lopsided layout: split right, then split the new tile downwards twice.
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(2));
    // Addressed by index: both tiles now offer a bottom edge.
    const bottoms = await screen.findAllByRole("button", {
      name: "Split pane, new pane on the bottom",
    });
    dwellOnEdge(bottoms[1], "bottom");
    fireEvent.click(bottoms[1]);
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(3));
    // Stacked tiles mean the workspace has grown taller than the screen.
    expect(parseFloat(canvas().style.height)).toBeGreaterThan(100);

    expect(pressBind("H", false)).toBe("swallow");

    // One row: the tree is now nothing but horizontal splits, and the height is back to a screen.
    await waitFor(() => expect(canvas().style.height).toBe("100%"));
    expect(document.querySelectorAll(".split.v")).toHaveLength(0);
    expect(document.querySelectorAll(".split.h")).toHaveLength(2);
    expect(document.querySelectorAll(".stack")).toHaveLength(3); // no tile was lost
  });

  // The workspace's own borders, as handles. The arithmetic is grow.test.ts's; what only this
  // file can say is that they are RENDERED, wired to the workspace rather than to a tile, and
  // absent under the layout where there is nothing for them to drag.
  const edges = () => ({
    right: document.querySelector<HTMLElement>(".divider.edge-right"),
    bottom: document.querySelector<HTMLElement>(".divider.edge-bottom"),
  });

  it("puts a resize handle on the workspace's right and bottom borders", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    expect(edges().right).toBeTruthy();
    expect(edges().bottom).toBeTruthy();
    // Named for what they resize, and oriented so a screen reader says which way they move.
    expect(edges().right!.getAttribute("aria-orientation")).toBe("vertical");
    expect(edges().bottom!.getAttribute("aria-orientation")).toBe("horizontal");
    // They belong to the CANVAS, not to a tile: a handle inside a `.stack` would move with the
    // pane instead of with the workspace.
    expect(edges().right!.closest(".workspace-canvas")).toBeTruthy();
    expect(edges().right!.closest(".stack")).toBeNull();
  });

  it("offers no border handles under the dividing layout", async () => {
    dividing();
    renderTerm();
    await screen.findAllByRole("combobox");
    // Nothing to drag: that layout pins the workspace to exactly one screen.
    expect(edges().right).toBeNull();
    expect(edges().bottom).toBeNull();
  });

  it("widens the workspace when the right border is dragged out", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(2));
    ...
    // The handle works in CORRECTIONS against the canvas's live edge, so the canvas has to
    // report one. jsdom lays nothing out, so it is stubbed at the width the body was given.
    let edge = 400;
    Object.defineProperty(canvas(), "getBoundingClientRect", {
      value: () => ({ right: edge, bottom: 400, left: 0, top: 0, width: edge, height: 400 }),
      configurable: true,
    });

    const handle = edges().right!;
    handle.dispatchEvent(pointerEvent("pointerdown", { clientX: 400, clientY: 200 }));
    window.dispatchEvent(pointerEvent("pointermove", { clientX: 500, clientY: 200 }));
    window.dispatchEvent(pointerEvent("pointerup", { clientX: 500, clientY: 200 }));

    // 100px of a 400px screen is a quarter of a viewport.
    expect(parseFloat(canvas().style.width)).toBeCloseTo(wide + 25, 2);
  });

  // The recognizer has its own suite (views/tiling/pan.test.ts). What only this file can say is
  // that it is attached to the SCROLL CONTAINER and not to something inside it — a mistake that
  // leaves every unit test green and the workspace immovable.
  it("pans the workspace when two fingers drag across it", async () => {
    renderTerm();
    await screen.findAllByRole("combobox");
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(2));

    // jsdom lays nothing out, so scrollLeft is inert unless it is given somewhere to live.
    let left = 0;
    ...
    const finger = (type: string, id: number, x: number) => {
      const ev = new MouseEvent(type, { clientX: x, clientY: 200, bubbles: true, button: 0 });
      Object.defineProperty(ev, "pointerId", { value: id });
      Object.defineProperty(ev, "pointerType", { value: "touch" });
      return ev;
    };
    // Started on a PANE, not on the container: one finger there belongs to the pane, and the
    // whole point is that the second one hands the gesture to the workspace anyway.
    const pane = document.querySelector(".stack-body")!;
    pane.dispatchEvent(finger("pointerdown", 1, 300));
    pane.dispatchEvent(finger("pointerdown", 2, 400));
    for (let i = 1; i <= 4; i++) {
      window.dispatchEvent(finger("pointermove", 1, 300 - i * 20));
      window.dispatchEvent(finger("pointermove", 2, 400 - i * 20));
    }
    expect(body().scrollLeft).toBe(80);
  });

  // Focus that lands off screen is worse than no focus: the keyboard is somewhere the user
  // cannot see. Asserted as a DISTANCE, on the scroll container, because that is the only form
  // in which the answer can be wrong in the way that matters — the reveal has always called
  // something, and what a browser pass caught it doing was scrolling too little.
  it("scrolls the tile that takes focus into view", async () => {
    laidOut();
    ...
    await splitViaEdge("right");
    await waitFor(() => expect(document.querySelectorAll(".stack")).toHaveLength(2));
    // The split focuses the pane it made, and the tile brought into view is THAT tile: the
    // second one runs to 500px on a 400px screen, so the view moves 100px right. The tile the
    // split came from would have moved it 500px the other way, and "some element was revealed"
    // is not a claim this can make any more.
    await waitFor(() => {
      expect(tiles()[1].classList.contains("focused")).toBe(true);
      expect(seen).toContainEqual({ left: 100, top: 0 });
    });
  });
});
...
describe("Pane chooser", () => {
  ...
  // The walk moves the VIEW even though it moves nothing else. On a workspace bigger than the
  // screen the highlight can land on a tile that is not on it, and a highlight the user cannot
  // see defeats the one thing this mode is for — the arrows are pressed at the speed of
  // looking. Asserted against the PREVIEWED tile's own distance, not merely "something was
  // revealed": the reveal-on-focus effect was already there, so a test that only checked "a
  // scroll happened" would have passed before this behaviour existed at all.
  it("brings the previewed tile into view while leaving the focus alone", async () => {
    await twoTiles();
    // The geometry is installed AFTER the split that made the second tile, because the gesture
    // that performs it stubs the source tile's rect for its own purposes; laying the workspace
    // out here overwrites that and leaves both tiles reading from one description.
    laidOut();
    const seen = watchScroll();
    ...
    await waitFor(() => expect(previewedTile()).toBe(0));
    // FAR ENOUGH TO SHOW ITS LEFT EDGE, which is the whole of what a reveal owes an oversized
    // tile. Tile 0 is 600px wide on a 400px screen and hangs 500px off its left edge, so the
    // view moves 500px left; the browser's own `scrollIntoView({ inline: "nearest" })` answered
    // 300 there — right edge flush, left edge still off screen, the walk apparently arriving in
    // the middle of a pane. And the focus has not moved to it.
    await waitFor(() => expect(seen).toContainEqual({ left: -500, top: 0 }));
    expect(focusedTile()).toBe(1);
    expect(seen).not.toContainEqual({ left: 100, top: 0 }); // tile 1's distance: never asked for

    // Leaving without choosing pans back to where the user started, the same way Escape puts
    // the focus back. A walk that costs nothing has to cost nothing to leave, too.
    seen.length = 0;
    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
    await waitFor(() => expect(seen).toContainEqual({ left: 100, top: 0 }));
  });

  // A COMMIT ENDS THE MODE AND MOVES THE FOCUS AS TWO WRITES, so the reveal runs twice: once for
  // the tile being left (the preview is gone, the focus has not moved yet) and once for the tile
  // chosen. The first has already aimed a smooth scroll somewhere else by the time the second
  // measures, so the second must speak even when it has nothing to move — a zero scroll is what
  // aborts the one in flight. Skipping it sent a committed choice 3647px to the pane the user had
  // just walked away from, which no jsdom test could see and this one can: the geometry puts the
  // chosen tile exactly where it belongs, so the correct last word IS the zero.
  it("still scrolls for the chosen tile when it is already in place", async () => {
    await twoTiles();
    laidOut([
      [0, 600], // tile 0: wider than the 400px screen and already flush with its left edge
      [600, 1000], // tile 1: a screen's width, entirely off to the right
    ]);
    const seen = watchScroll();
    expect(pressBind("s", false)).toBe("swallow");
    await waitFor(() => expect(chooser()).toBeTruthy());
    ...
    fireEvent.keyDown(window, { key: "Enter" });
    await waitFor(() => expect(chooser()).toBeNull());
    await waitFor(() => expect(focusedTile()).toBe(0));
    // The last word belongs to the tile that was CHOSEN. Both reveals are recorded — the tile
    // being left asks for 600 — and what matters is which of them ran last.
    await waitFor(() => expect(seen.at(-1)).toEqual({ left: 0, top: 0 }));
    expect(seen).toContainEqual({ left: 600, top: 0 }); // the stale one it has to overrule
  });

  // The panel's OWN list scrolls once the workspace holds more panes than it has room for, and
  // the walk moves the highlight without touching that scroll — so the mark walks out of the box
  // and the panel goes on reporting a position nobody can see. Tab is what made it obvious (it
  // reaches every pane, so it reaches the ones past the fold), but the arrows and the digits land
  // on the same rows and had the same problem.
  //
  // Asserted on WHICH element was revealed, and specifically that it is the chooser's row rather
  // than the tile: `revealTile` was already scrolling the workspace on every step, so a test that
  // only counted scrollIntoView calls would have passed before this existed.
  it("scrolls its own list to the row the walk moved to", async () => {
    const seen: Element[] = [];
    ...
    // The row the mark ARRIVED at, inside the panel — not the row it left, and not the tile that
    // row stands for out on the workspace.
    await waitFor(() => expect(seen).toContain(rows()[0]));
    expect(seen).not.toContain(rows()[1]);
    expect(seen.filter((el) => el.classList.contains("pane-chooser-item"))).toHaveLength(1);

    // Every route to the mark, not just the one that exposed the bug: the arrows and the digits
    // reach rows past the fold too, and a reveal wired to the Tab handler alone would leave those
    // exactly as broken while passing the row above.
    seen.length = 0;
    fireEvent.keyDown(window, { key: "ArrowRight" });
    await waitFor(() => expect(seen).toContain(rows()[1]));
    seen.length = 0;
  ...
  // THE MINI MAP. The list is ordered but not positioned, and once the workspace can be
  // several screens wide (the extending layout) that is the thing it cannot say: "row 5 of 7"
  // is not an answer to "where". The map answers it, and the tests below are about the two
  // ways it could answer it wrongly — by drawing a layout the tree does not describe, and by
  // agreeing with the list about which pane is which while disagreeing about where it is.
  //
  // The map is OPT-IN (Settings -> Workspace -> Mini map), so every test below that wants to
  // see one says so, and puts the setting back. The pair that checks the switch itself is
  // "is off unless it is turned on".
  const withMiniMap = () => {
    onTestFinished(() => setPaneMiniMap(false));
    setPaneMiniMap(true);
  };
  const mapCells = () => Array.from(document.querySelectorAll<HTMLElement>(".pane-chooser-map-tile"));
  // The map's geometry, as the numbers actually written into the style attribute. jsdom lays
  // nothing out, so these percentages are the whole of what can be checked here — which is
  // exactly right, because they are also the whole of what the component decides.
  const cellBoxes = () =>
    mapCells().map((c) => ({
  ...
  const currentCell = () => mapCells().findIndex((c) => c.classList.contains("current"));
  const filledCell = () => mapCells().findIndex((c) => c.querySelector(".pane-number.current"));
  const mapView = () => document.querySelector<HTMLElement>(".pane-chooser-map-view");

  // THE SWITCH, on one workspace in both states. Two tiles is exactly the case the map is for,
  // so an absent map here is the setting doing the work and not the tile count — which is the
  // only way to tell this apart from the guard tested further down.
  it("is off unless it is turned on", async () => {
    await twoTiles();
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(rows()).toHaveLength(2);
    expect(document.querySelector(".pane-chooser-map")).toBeNull();
    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
    ...
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(mapCells()).toHaveLength(2);
    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
  });

  it("draws the tiles side by side when the split was a column", async () => {
    withMiniMap();
    await twoTiles();
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    ...
    expect(mapCells()).toHaveLength(2);
    // Same numbers as the list and the plates, so a rectangle and a row are the same pane.
    expect(mapCells().map((c) => c.textContent?.trim())).toEqual(["1", "2"]);
    // …and each rectangle names the tile it maps, in the tiles' own order. Everything below
    // compares cell i with tile i, and this is the assertion that earns the right to.
    expect(mapCells().map((c) => c.dataset.stackId)).toEqual(tiles().map((t) => t.dataset.stackId));

    const box = cellBoxes();
    // A ROW: the second starts where the first ends, they share the full height, and between
    // them they cover the workspace. The three claims are separate on purpose — two tiles
    // both drawn at left: 0 would still "cover the workspace" by the third one alone.
    expect(box[0].left).toBe(0);
    expect(box[1].left).toBeCloseTo(box[0].w, 6);
    expect(box[0].w + box[1].w).toBeCloseTo(100, 6);
    expect(box.map((b) => b.top)).toEqual([0, 0]);
    expect(box.map((b) => b.h)).toEqual([100, 100]);
  });

  // The same workspace split the other way. Run only against sizes, a map that transposed x
  // for y would pass every assertion in the test above; this is the pair that makes the axis
  // itself the claim.
  it("draws them stacked when the split was a row", async () => {
    withMiniMap();
    renderView(Workspace);
    await screen.findByText("project");
    await splitViaEdge("bottom");
    ...
    expect(box[0].top).toBe(0);
    expect(box[1].top).toBeCloseTo(box[0].h, 6);
    expect(box[0].h + box[1].h).toBeCloseTo(100, 6);
    expect(box.map((b) => b.left)).toEqual([0, 0]);
    expect(box.map((b) => b.w)).toEqual([100, 100]);
  });

  // The map carries the mode's two marks, and they are on different rectangles for the whole
  // of the walk — the same negative assertion the rest of this suite is built on, asked of
  // the picture rather than of the list.
  it("moves the highlight on the map without moving the fill", async () => {
    withMiniMap();
    await twoTiles();
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(currentCell()).toBe(1);
    expect(filledCell()).toBe(1);

    fireEvent.keyDown(window, { key: "ArrowLeft" });
    expect(currentCell()).toBe(0);
    expect(filledCell()).toBe(1);
    // …and the map agrees with the workspace it is a map of. Checked here rather than taken
    // on trust: the map is drawn from `tileRects` and the tiles from the DOM, and the whole
    // point of the map is that those two orders are the same one.
    expect(previewedTile()).toBe(0);
    expect(focusedTile()).toBe(1);
  ...
  it("commits the pane whose rectangle was clicked", async () => {
    withMiniMap();
    await twoTiles();
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    ...
    fireEvent.click(mapCells()[0]);
    await waitFor(() => expect(chooser()).toBeNull());
    expect(focusedTile()).toBe(0);
  });

  // One tile is not a map. The picture would be a single rectangle filling the box, saying
  // only what the one row beneath it already says.
  it("is not drawn for a workspace with one tile", async () => {
    withMiniMap();
    // TWO PANES IN ONE TILE — the only single-tile workspace the chooser opens on at all
    // (with a single pane the command is disabled, "only one pane open"). That makes this
    // the case the guard is actually for: two rows in the list, two tabs sharing one place
    // on screen, and so nothing whatever for a map to say about where they are.
    renderView(Workspace);
    await screen.findByText("project");
    runCommand("ws:new");
    await waitFor(() => expect(document.querySelector(".pane-pick-overlay")).toBeTruthy());
    fireEvent.keyDown(window, { key: "Enter" }); // the centre target: stack it as a tab
    await waitFor(() => expect(document.querySelectorAll(".tab")).toHaveLength(2));
    expect(tiles()).toHaveLength(1);

    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(rows()).toHaveLength(2); // the list still has both panes...
    expect(document.querySelector(".pane-chooser-map")).toBeNull(); // ...and the map is absent
    expect(mapCells()).toHaveLength(0);
  });

  // A TILE IS A STACK, so it can hold several panes sharing one place on screen — and the cell
  // draws every one of their numbers, because a number is what ties a rectangle to a row and to
  // a tab. The two halves of the rule are checked on ONE workspace at two map sizes, so what
  // differs between them is the room and nothing else. A pair of separate workspaces could pass
  // both while the cell was really keying off something other than how much room it has.
  const cellFor = (i: number) =>
    document.querySelector<HTMLElement>(
      `.pane-chooser-map-tile[data-stack-id="${tiles()[i].dataset.stackId}"]`,
    )!;
  const bulletsIn = (el: HTMLElement) =>
    Array.from(el.querySelectorAll<HTMLElement>(".pane-number")).map((b) => b.textContent?.trim());
  const ellipsesIn = (el: HTMLElement) => el.querySelectorAll(".pane-chooser-map-more").length;

  // jsdom lays nothing out, so the map's box comes from `mapBox` over whatever the scroll
  // container reports — which is what makes the room controllable here at all: a measured 3000
  // x 500 workspace draws a 20rem-wide map, and a 500 x 500 one draws an 8rem-wide one.
  const measureWorkspace = (w: number, h: number) => {
    const body = document.querySelector<HTMLElement>(".workspace-body")!;
    for (const [k, v] of [
      ["clientWidth", Math.min(w, 1000)],
      ["clientHeight", Math.min(h, 500)],
      ["scrollWidth", w],
      ["scrollHeight", h],
    ] as const) {
      Object.defineProperty(body, k, { value: v, configurable: true });
    }
    fireEvent.scroll(body);
  };

  // Stack `n` extra panes onto the focused tile, through the app's own placement wireframe.
  async function stackTabs(tile: number, n: number) {
    const tabsIn = () => tiles()[tile].querySelectorAll(".tab").length;
    const from = tabsIn();
    for (let i = 1; i <= n; i++) {
      runCommand("ws:new");
      await waitFor(() => expect(document.querySelector(".pane-pick-overlay")).toBeTruthy());
      fireEvent.keyDown(window, { key: "Enter" }); // the centre target: stack it as a tab
      await waitFor(() => expect(tabsIn()).toBe(from + i));
    }
  }

  // The map's box is written in rem, so the tests below can work out what a cell's own size is
  // and therefore how many numbers fit on ONE line of it. That is what makes "they wrapped"
  // checkable without layout: more numbers drawn than a line can hold is rows being used.
  const mapRem = () => {
    const st = document.querySelector<HTMLElement>(".pane-chooser-map")!.style;
    return { w: parseFloat(st.width), h: parseFloat(st.height) };
  };
  const cellRem = (i: number) => {
    const st = cellFor(i).style;
    const box = mapRem();
    return { w: (parseFloat(st.width) / 100) * box.w, h: (parseFloat(st.height) / 100) * box.h };
  };

  it("wraps the numbers down the cell, and elides only once the rows run out too", async () => {
    withMiniMap();
    await twoTiles();
    await stackTabs(1, 5); // six tabs in the right-hand tile, one place on screen
    expect(tiles()).toHaveLength(2);

    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());

    // ROOMY: a 6:1 workspace draws a 20rem map that is still a few rem tall, so the tabbed
    // cell is several bullets across AND more than one row down.
    measureWorkspace(3000, 500);
    await waitFor(() => expect(bulletsIn(cellFor(1))).toHaveLength(6));
    expect(bulletsIn(cellFor(1))).toEqual(["2", "3", "4", "5", "6", "7"]);
    expect(ellipsesIn(cellFor(1))).toBe(0);
    // THEY WRAPPED, and this is how that is visible without layout: more numbers are drawn
    // than one line of this cell could hold. Before the numbers wrapped, the count WAS the
    // line, so this assertion could not have passed at any cell size.
    const line = bulletGrid(cellRem(1).w, cellRem(1).h).cols;
    expect(line).toBeGreaterThan(0);
    expect(bulletsIn(cellFor(1)).length).toBeGreaterThan(line);
    // The single-pane tile is unaffected: one pane, one number, no ellipsis.
    expect(bulletsIn(cellFor(0))).toEqual(["1"]);
    expect(ellipsesIn(cellFor(0))).toBe(0);

    // FLAT: a 10:1 workspace draws a map barely one bullet tall, so the rows are gone and the
    // cell is back to a single line — which six numbers do not fit on.
    measureWorkspace(3000, 300);
    await waitFor(() => expect(ellipsesIn(cellFor(1))).toBeGreaterThan(0));
    expect(bulletGrid(cellRem(1).w, cellRem(1).h).rows).toBe(1);
    const shown = bulletsIn(cellFor(1));
    expect(shown.length).toBeGreaterThan(0);
    expect(shown.length).toBeLessThan(6);
    // What is kept is the window around the tab the cell STANDS FOR — the visible one, which
    // is the last tab created. A window that simply took the first N would have dropped it,
    // and the map would go quiet during exactly the Tab walk it is being read for.
    expect(shown).toContain("7");

    // FLATTER STILL: not even one row. A clipped glyph would be worse than an empty rectangle
    // and a clipped ellipsis worse than that, so the cell says nothing at all.
    measureWorkspace(4000, 250);
    await waitFor(() => expect(bulletsIn(cellFor(1))).toHaveLength(0));
    expect(ellipsesIn(cellFor(1))).toBe(0);
    expect(bulletGrid(cellRem(1).w, cellRem(1).h).rows).toBe(0);

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
  });

  // Each number is its own target — the only way to reach a BACKGROUND tab by pointing at it,
  // since stacked tabs share one place on screen and the cell as a whole can only stand for
  // the visible one. Note this does NOT prove the click is stopped at the number: the cell
  // underneath commits the same pane by then, so letting it through is invisible here. The
  // test that proves it is the refused one below, and it is the reason the guard exists.
  it("commits the background tab whose number was clicked", async () => {
    withMiniMap();
    await twoTiles();
    await stackTabs(1, 1); // two tabs; the new one is visible and focused

    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    measureWorkspace(3000, 500);
    await waitFor(() => expect(bulletsIn(cellFor(1))).toHaveLength(2));
    expect(bulletsIn(cellFor(1))).toEqual(["2", "3"]);

    // The focus is on pane 3, the tab that was just made. Pane 2 is behind it.
    const filled = () => rows().findIndex((r) => r.querySelector(".pane-number.current"));
    expect(filled()).toBe(2);

    fireEvent.click(cellFor(1).querySelectorAll<HTMLElement>(".pane-number")[0]);
    await waitFor(() => expect(chooser()).toBeNull());

    // Re-open and read where the focus went. Pane 2 — the one whose number was clicked — and
    // NOT pane 3, which is what the cell underneath the number would have committed.
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(filled()).toBe(1);
    fireEvent.keyDown(window, { key: "Escape" });
  ...
  // NO REFUSED BULLET TEST any more, and no refusals in the map at all. A map is drawn only for
  // the mode's OWN question (see `asking` in PaneChooser), and that is the one purpose that
  // refuses nothing — every refusal the app can produce belongs to a question a caller brought,
  // and those are drawn as a list with no picture. The case the deleted test covered — a greyed
  // bullet whose click fell through to the cell and committed the previewed pane — is now
  // unreachable rather than guarded. Refusals themselves are unaffected and still tested: they
  // live on the rows, which is where their reasons are.

  // WHERE THE SCREEN IS — the map's one piece of information that nothing else in the app
  // states. It comes from a measurement, and jsdom measures nothing, so the container is
  // given the measurement by hand: three viewports wide, scrolled one across. What is being
  // checked is not the arithmetic (that is minimap.test.ts) but that the component is
  // listening to the right element and reading it at all.
  it("frames what is on screen, and only once there is something off it", async () => {
    withMiniMap();
    await twoTiles();
    const body = document.querySelector<HTMLElement>(".workspace-body")!;
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    // Unmeasured. NOT "everything fits": the two are indistinguishable on screen, and drawing
    // a confident 2px frame around a measurement nobody took is the failure this guards.
    expect(mapView()).toBeNull();

    const fake = (k: string, v: number) => Object.defineProperty(body, k, { value: v, configurable: true });
    fake("clientWidth", 1000);
    fake("clientHeight", 500);
    fake("scrollWidth", 3000);
    fake("scrollHeight", 500);
    fake("scrollLeft", 1000);
    // The live route: the map re-reads on the container's own scroll event, which is what
    // makes the frame travel with the reveal each arrow key triggers.
    fireEvent.scroll(body);

    await waitFor(() => expect(mapView()).toBeTruthy());
    expect(parseFloat(mapView()!.style.width)).toBeCloseTo(100 / 3, 6);
    expect(parseFloat(mapView()!.style.left)).toBeCloseTo(100 / 3, 6);
    expect(parseFloat(mapView()!.style.height)).toBe(100);
    // The box takes the measured shape too: 3000x500 is 6:1, so the width budget binds and
    // the height gives way rather than the map being drawn square.
    const map = document.querySelector<HTMLElement>(".pane-chooser-map")!;
    expect(map.style.width).toBe(`${MAP_MAX_W_REM}rem`);
    expect(parseFloat(map.style.height)).toBeCloseTo(MAP_MAX_W_REM / 6, 6);
  ...
  // …and it does not STOP at the tile, which the test above cannot see: there, every pane the
  // key could reach was a tab of the one tile, so a Tab confined to that tile answers it
  // identically. Here the layout is the other extreme — two tiles of one pane each, where the
  // confined key had nowhere to go and did nothing at all — so this is the row that fails if
  // Tab is still tile-scoped. The two are a pair; neither alone says what the key does.
  it("crosses the tile boundary with Tab, wrapping both ways", async () => {
    await twoTiles();
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(rowText()).toEqual(["All", "project"]);
    ...
    fireEvent.keyDown(window, { key: "Tab" });
    expect(selectedRow()).toBe(0);
    // The mode's whole promise, restated on the new key: the highlight is on one tile and the
    // focus is still on the other. A Tab that activated as it went would look the same in the
    // list and be a different mode.
    expect(previewedTile()).toBe(0);
    expect(focusedTile()).toBe(1);
    ...
    // And the hint names it, on a layout where nothing is stacked. It used to be gated on the
    // previewed tile having tabs to walk, so this list would have advertised no Tab at all
    // while the key worked — worse than the reverse, because a key nobody is told about is a
    // key nobody presses.
    expect(document.querySelector(".pane-chooser-keys")!.textContent).toMatch(/Tab next pane/);
  ...
  // PINNED — the chooser taken out of the mode it normally is and stood in a gutter. Every
  // test below is about the same distinction, asked in a different place: a floating chooser IS
  // the mode, and a pinned one merely REPORTS it. So the panel outlives Enter, the scrim and
  // the keyboard do not, and between walks a click on a row means what pointing at a pane
  // means everywhere else rather than answering a question nobody asked.
  //
  // The gutter's own geometry — that the workspace really gives up the width, that the strip
  // lands on the named side — is measured in a browser (.agents-workspace/tmp/measure-gutter.mjs).
  // jsdom lays nothing out, so what can be settled here is which classes and which behaviours,
  // and those are asserted rather than the pixels they imply.
  const pinButton = () => document.querySelector<HTMLButtonElement>(".pane-chooser-pin");
  const scrim = () => document.querySelector(".pane-chooser-scrim");
  // The two roles the panel can be in, told apart by the one class that says which. A caller's
  // question opens a CARD beside the gutter rather than in it, so from here on a query that
  // does not say which panel it means is a query that can pass on the wrong one.
  const gutterPanel = () => document.querySelector<HTMLElement>(".pane-chooser.pinned");
  const cardPanel = () => document.querySelector<HTMLElement>(".pane-chooser:not(.pinned)");
  const panels = () => document.querySelectorAll(".pane-chooser").length;
  const rowsIn = (p: HTMLElement) =>
    Array.from(p.querySelectorAll<HTMLElement>(".pane-chooser-item"));
  const disabledIn = (p: HTMLElement) => rowsIn(p).map((r) => r.classList.contains("disabled"));
  const titleIn = (p: HTMLElement) =>
    p.querySelector<HTMLElement>(".pane-chooser-title")?.textContent?.trim();
  const workspaceEl = () => document.querySelector<HTMLElement>(".workspace")!;
  const chooserTitle = () =>
    document.querySelector<HTMLElement>(".pane-chooser-title")?.textContent?.trim();
  const withPin = () => {
    onTestFinished(() => {
      setPaneChooserPinned(false);
      setPaneChooserSide("auto");
    });
    setPaneChooserPinned(true);
  };
  // jsdom reports a 1024px window and never changes its mind, so the width gate is driven
  // here. The resize event is not decoration: it is the ONLY thing that re-reads the gate, so
  // a test that set the property alone would be checking a value nothing had read.
  const setViewportWidth = (px: number) => {
    Object.defineProperty(window, "innerWidth", { value: px, configurable: true, writable: true });
    fireEvent(window, new Event("resize"));
  };

  it("pins from its own corner and then outlives the mode that opened it", async () => {
    await twoTiles();
    expect(chooser()).toBeNull();

    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    // Floating, which is the state the pin's faded look is about.
    expect(chooser()!.classList.contains("pinned")).toBe(false);
    expect(workspaceEl().classList.contains("chooser-pinned")).toBe(false);
    expect(pinButton()!.getAttribute("aria-pressed")).toBe("false");
    expect(scrim()).toBeTruthy();

    fireEvent.click(pinButton()!);
    await waitFor(() => expect(chooser()!.classList.contains("pinned")).toBe(true));
    expect(workspaceEl().classList.contains("chooser-pinned")).toBe(true);
    expect(pinButton()!.getAttribute("aria-pressed")).toBe("true");
    // Pinning mid-walk does not end the walk, and the panel keeps the keyboard: it holds
    // aria-activedescendant, so handing focus back to the workspace here would leave a
    // screen reader following nothing.
    expect(chooser()!.getAttribute("aria-activedescendant")).toBeTruthy();
    expect(document.activeElement).toBe(chooser());

    // THE POINT OF THE FEATURE: Enter answers the question and the panel stays. Its
    // aria-activedescendant goes with the walk, because nothing is being walked any more.
    fireEvent.keyDown(window, { key: "Enter" });
    await waitFor(() => expect(scrim()).toBeNull());
    expect(chooser()).toBeTruthy();
    expect(chooser()!.classList.contains("pinned")).toBe(true);
    expect(chooser()!.getAttribute("aria-activedescendant")).toBeNull();

    // And unpinning takes it away entirely — there is no walk left for it to report.
    fireEvent.click(pinButton()!);
    await waitFor(() => expect(chooser()).toBeNull());
    expect(workspaceEl().classList.contains("chooser-pinned")).toBe(false);
  });

  // The one thing a permanent panel must not do. A standing copy of the mode's keyboard would
  // eat Escape, Enter, Tab, the digits and hjkl from whatever pane the user is actually typing
  // in — which is not a chooser with a bug, it is a workspace nobody can use.
  //
  // The two halves are a PAIR on purpose. "Escape was not swallowed" is true of a panel that
  // never registered the handler and of one that registered it and did nothing useful; only
  // the armed half, where the very same key IS swallowed, says the handler is real.
  it("leaves the keyboard alone between walks, and takes it during one", async () => {
    await twoTiles();
    withPin();
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(scrim()).toBeNull();
    // No key hint either: every line of it names something a keypress does, and none of it is
    // true while the keys belong to the panes.
    expect(document.querySelector(".pane-chooser-keys")).toBeNull();
    // And it did not TAKE the keyboard on the way in. The panel focuses itself when a walk
    // arms, because it holds aria-activedescendant then; doing it on mount would pull the
    // keyboard out of a live shell every time the setting was switched on or the page reloaded.
    expect(document.activeElement).not.toBe(chooser());

    // fireEvent returns false once preventDefault has been called, which is the whole of what
    // "swallowed" means here.
    ...
    expect(fireEvent.keyDown(window, { key: "2" })).toBe(true);
    expect(fireEvent.keyDown(window, { key: "h" })).toBe(true);
    // …and none of it armed anything: the highlight is still on the pane holding the focus.
    expect(selectedRow()).toBe(1);
    expect(focusedTile()).toBe(1);

    runCommand("ws:choose");
    await waitFor(() => expect(scrim()).toBeTruthy());
  ...
  // Between walks the panel is a list of places, so a click is what a click on a place means.
  // Armed, the same click answers a question — which may be refused, and for a transfer is not
  // a focus change at all. The distinction is invisible in the markup and total in effect.
  it("goes to the pane whose row was clicked, with no mode in sight", async () => {
    await twoTiles();
    withPin();
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(focusedTile()).toBe(1);
    ...
    fireEvent.click(rows()[0]);
    await waitFor(() => expect(focusedTile()).toBe(0));
    // No mode was entered on the way: the panel is still there and nothing is previewing.
    expect(chooser()).toBeTruthy();
    expect(scrim()).toBeNull();
    expect(previewedTile()).toBe(-1);
    // The highlight followed the focus, which is the only thing a list that is not being
    // walked has to say.
    expect(selectedRow()).toBe(0);
  });

  // A CANCELLED QUESTION LEAVES NOTHING BEHIND. The mode ends by clearing the preview, not by
  // forgetting what was asked — `purpose` still holds the last one — so a standing panel that
  // read it unconditionally would go on greying rows for a transfer that ended minutes ago,
  // and a click on one of them would silently do nothing at all.
  //
  // This is the only test here that can tell the two halves of a click apart. For the plain
  // chooser they are the same thing by construction: committing a pane and activating one both
  // end in `activate`. It takes a purpose that REFUSES something to make the stale reading
  // visible, and a refused row that is not already focused to make it visible as a movement.
  it("forgets a cancelled purpose's refusals rather than enforcing them for ever", async () => {
    const pane = (id: string, data: PaneData) => ({ id, data });
    const term = (sessionId: string): PaneData => ({
      kind: "term",
      workload: "shop-web",
      cmd: ["/bin/ash"],
      sessionId,
    ...
    globalThis.localStorage?.setItem(
      ...
      JSON.stringify({
        tree: {
          ...
          dir: "h",
          ratio: 0.5,
          a: { type: "stack", id: "t0", active: 0, panes: [pane("src", term("term-live"))] },
          b: {
            type: "stack",
            id: "t1",
            active: 0,
            panes: [pane("dest", { kind: "files", path: "project" }), pane("other", term("term-two"))],
          },
        },
        focused: "src",
      }),
    ...
    seedTerminals([
      { id: "term-live", workload: "shop-web", cmd: ["/bin/ash"], alive: true, cwd: "/srv/app" },
      { id: "term-two", workload: "shop-web", cmd: ["/bin/ash"], alive: true, cwd: "/srv/app" },
    ]);
    renderView(Workspace);
    await waitFor(() => expect(tiles()).toHaveLength(2));
    withPin();
    ...
    // Follow refuses a terminal by kind, so two of the three rows grey out — the premise for
    // everything below, asserted rather than assumed.
    await waitFor(() =>
      expect(allCommands().find((c) => c.id === "ws:follow-terminal")?.disabled).toBeUndefined(),
    );
    runCommand("ws:follow-terminal");
    await waitFor(() => expect(cardPanel()).toBeTruthy());
    // The refusals are the CARD's, and the gutter beside it never took them: they belong to a
    // question, and the gutter is not the one being asked.
    expect(disabledIn(cardPanel()!)).toEqual([true, false, true]);
    expect(disabledIn(gutterPanel()!)).toEqual([false, false, false]);

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(scrim()).toBeNull());
    // The question is over, so nothing is refused any more.
    expect(cardPanel()).toBeNull();
    expect(disabledIn(gutterPanel()!)).toEqual([false, false, false]);
    expect(document.querySelector(".pane-chooser-why")).toBeNull();

    // And the row the question refused is pressable again. Under the stale reading this click
    // lands on `commit`, which drops a refused pane on the floor: the focus would stay where
    // it is and the panel would look exactly as it does now.
    expect(focusedTile()).toBe(0);
    fireEvent.click(rowsIn(gutterPanel()!)[2]);
    await waitFor(() => expect(focusedTile()).toBe(1));
    expect(selectedRow()).toBe(2);
  });

  // A CALLER'S QUESTION DOES NOT GO IN THE GUTTER. The gutter is furniture — the plain question
  // asked permanently, read while working — and "Copy 3 items to…" is a modal question that has
  // to be answered. Putting one in the other replaces the thing being read with something that
  // must be dealt with, in the one part of the screen that was supposed to stay still.
  it("gives a caller's question a card beside the gutter rather than the gutter itself", async () => {
    const pane = (id: string, data: PaneData) => ({ id, data });
    globalThis.localStorage?.setItem(
      WS_LAYOUT_KEY,
      JSON.stringify({
        tree: {
          ...
          a: {
            ...
            id: "t0",
            active: 0,
            panes: [pane("src", { kind: "term", workload: "shop-web", cmd: ["/bin/ash"], sessionId: "term-live" })],
          },
          b: { type: "stack", id: "t1", active: 0, panes: [pane("dest", { kind: "files", path: "project" })] },
        },
        focused: "src",
      }),
    );
    ...
    await waitFor(() => expect(tiles()).toHaveLength(2));
    withPin();
    await waitFor(() => expect(gutterPanel()).toBeTruthy());
    expect(panels()).toBe(1);

    // The mode's OWN question still goes in the gutter — that is what the gutter is.
    runCommand("ws:choose");
    await waitFor(() => expect(titleIn(gutterPanel()!)).toBe("Choose a pane"));
    expect(panels()).toBe(1);
    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(titleIn(gutterPanel()!)).toBe("Panes"));

    await waitFor(() =>
      expect(allCommands().find((c) => c.id === "ws:follow-terminal")?.disabled).toBeUndefined(),
    );
    runCommand("ws:follow-terminal");
    await waitFor(() => expect(panels()).toBe(2));
    expect(titleIn(cardPanel()!)).toBe("Follow this terminal in…");
    // …and the gutter is untouched by it: still its own list, still saying where the focus is.
    expect(titleIn(gutterPanel()!)).toBe("Panes");
    expect(gutterPanel()!.getAttribute("aria-activedescendant")).toBeNull();
    // The card is on the OTHER SIDE. Sharing one would put it on top of the panel it was split
    // out of, which is most of the way back to sharing it.
    expect(gutterPanel()!.classList.contains("left")).toBe(true);
    expect(cardPanel()!.classList.contains("right")).toBe(true);
    // And the gutter is scenery while the card's scrim is up — its rows move the focus, which
    // is the one thing a modal question must not allow behind its own back.
    expect(gutterPanel()!.classList.contains("inert")).toBe(true);

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(panels()).toBe(1));
    expect(gutterPanel()!.classList.contains("inert")).toBe(false);
  });

  // A caller's question is a LIST OF DESTINATIONS: no picture of the workspace, and no mark
  // saying where the focus is. The map answers "where is that pane", which is a navigation
  // question; a transfer picks by name, by kind and by which rows are greyed. And the focus
  // mark reports a fact no answer to this question can change — a transfer deliberately leaves
  // the focus where it is.
  //
  // Asserted against the SAME workspace with the same setting on, once for each question, so
  // that an absent map is the question doing it and not the setting, the tile count or the pin.
  it("draws a caller's question as a plain list — no map, no focus mark", async () => {
    withMiniMap();
    const pane = (id: string, data: PaneData) => ({ id, data });
    globalThis.localStorage?.setItem(
      WS_LAYOUT_KEY,
    ...
    // The mode's own question, unpinned: map, focus mark, pin — all as before.
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(mapCells()).toHaveLength(2);
    expect(rows().filter((r) => r.querySelector(".pane-number.current"))).toHaveLength(1);
    expect(rows().filter((r) => r.getAttribute("aria-current"))).toHaveLength(1);
    expect(pinButton()).toBeTruthy();
    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());

    // The same panel, the same setting, a caller's question: a list and nothing else.
    await waitFor(() =>
      expect(allCommands().find((c) => c.id === "ws:follow-terminal")?.disabled).toBeUndefined(),
    );
    runCommand("ws:follow-terminal");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(document.querySelector(".pane-chooser-map")).toBeNull();
    // Scoped to the PANEL. The standing number on the tab is filled too, and it should be: it
    // is the `paneNumbersInTabs` copy, out on the workspace, and this question is not about it.
    expect(chooser()!.querySelector(".pane-number.current")).toBeNull();
    expect(rows().filter((r) => r.getAttribute("aria-current"))).toHaveLength(0);
    expect(pinButton()).toBeNull();
    // What it does NOT drop: the rows, their numbers, and the reasons two of them cannot be
    // answers. Those are the question.
    expect(rowNumbers()).toEqual(["1", "2"]);
    expect(rows().map((r) => r.classList.contains("disabled"))).toEqual([true, false]);
    expect(screen.getByText("a terminal")).toBeTruthy();

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
  });

  // …AND THE ROWS LOSE THEIR STATE BADGES with them. "working", "needs you" and "following"
  // say what a pane is DOING, which is exactly what a tab is for; in a list of destinations
  // they are answers to a question nobody asked, and an amber "needs you" beside a row reads
  // as a warning about sending there — a refusal the purpose never made.
  //
  // Three observation points, and all three are needed. The chooser's own row is the claim;
  // the SAME row under the mode's own question is the positive control, without which an
  // absent badge would prove only that the poll had not answered; and the TAB is the control
  // in the other direction, because "hide the badge" implemented one layer too low would take
  // it off the tab bar too and every assertion here would still pass.
  it("strips a caller's question of the rows' state badges, and only there", async () => {
    const pane = (id: string, data: PaneData) => ({ id, data });
    globalThis.localStorage?.setItem(
      WS_LAYOUT_KEY,
      JSON.stringify({
    ...
    // Blocked from the first frame, so the badge is a property of the seeded world rather
    // than of a poll a later assertion would race.
    seedTerminals([
      {
        id: "term-live",
        workload: "shop-web",
    ...
    await waitFor(() => expect(tiles()).toHaveLength(2));
    const tabBadges = () =>
      Array.from(document.querySelectorAll(".tab-label .badge")).map((b) => b.textContent);
    const chooserBadges = () =>
      Array.from(chooser()?.querySelectorAll(".badge") ?? []).map((b) => b.textContent);
    // The tab has it, which is what makes the rest of this test able to fail.
    await waitFor(() => expect(tabBadges()).toEqual(["needs you"]));

    // THE POSITIVE CONTROL. The mode's own question is a list of PLACES TO GO, and a pane
    // that is waiting for a human is a reason to go there — so the badge belongs.
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(chooserBadges()).toEqual(["needs you"]);
    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());

    // THE CLAIM. Same workspace, same session, same row — a caller's question, and the badge
    // is gone from it.
    await waitFor(() =>
      expect(allCommands().find((c) => c.id === "ws:follow-terminal")?.disabled).toBeUndefined(),
    );
    runCommand("ws:follow-terminal");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(chooserBadges()).toEqual([]);
    // The NAME survives, which is the half of the row that answers the question. Without
    // this, "the label renders nothing at all" passes the line above.
    expect(rowText()).toEqual(["shop-web  /bin/ash", "project"]);
    // THE OTHER CONTROL: the tab bar is untouched. The badge was dropped from this list, not
    // from the app.
    expect(tabBadges()).toEqual(["needs you"]);

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
    // And it comes BACK to the chooser the moment the plain question is asked again — the
    // strip is a property of the question, not a one-way edit to the row.
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(chooserBadges()).toEqual(["needs you"]);
    fireEvent.keyDown(window, { key: "Escape" });
  ...
  // …AND ON THE COMMAND THE RULE WAS WRITTEN FOR. The test above proves the mechanism with
  // `ws:follow-terminal`, which is the caller's question that is cheapest to raise; this one
  // raises an actual *Copy … to…*, because "a caller's question" is `purpose() !== goToPane`
  // and nothing but running the command says which side of that line a transfer falls on.
  //
  // A terminal is a real destination for a transfer — it takes files into its shell's working
  // directory — so it is a row here for its own sake, not scenery.
  it("strips the badge from a real Copy … to another pane as well", async () => {
    const pane = (id: string, data: PaneData) => ({ id, data });
    globalThis.localStorage?.setItem(
      WS_LAYOUT_KEY,
      JSON.stringify({
        tree: {
          ...
          dir: "h",
          ratio: 0.5,
          a: { type: "stack", id: "t0", active: 0, panes: [pane("browse", { kind: "files", path: "project" })] },
          b: {
            type: "stack",
            id: "t1",
            active: 0,
            panes: [pane("sh", { kind: "term", workload: "shop-web", cmd: ["/bin/ash"], sessionId: "term-live" })],
          },
        },
        focused: "browse",
      }),
    ...
    const chooserBadges = () =>
      Array.from(chooser()?.querySelectorAll(".badge") ?? []).map((b) => b.textContent);
    await waitFor(() =>
      expect(Array.from(document.querySelectorAll(".tab-label .badge")).map((b) => b.textContent)).toEqual([
        "needs you",
      ]),
    );

    // Something to send. `files:copy` is disabled until a row is picked, so this is what
    // makes the command runnable at all rather than a no-op that would pass every line below.
    fireEvent.click((await within(tiles()[0]).findByText("compose.yaml")).closest("tr")!);
    await waitFor(() =>
      expect(allCommands().find((c) => c.id === "files:copy")?.disabled).toBeUndefined(),
    );
    runCommand("files:copy");
    await waitFor(() => expect(chooser()).toBeTruthy());
    // The question really is the transfer's, and not the mode's own — the thing that decides
    // whether any of this applies.
    expect(document.querySelector(".pane-chooser-title")?.textContent).toBe(
      'Copy "compose.yaml" to…',
    );
    expect(chooserBadges()).toEqual([]);
    // The badged pane is still A ROW; it lost its badge, not its place in the list. The
    // escape hatch trails it, as it does for every transfer — it is not a pane, has no badge
    // to lose, and is here so that a shortened list would fail rather than pass quietly.
    expect(rowText()).toEqual(["project", "shop-web  /bin/ash", "an arbitrary location"]);

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(chooser()).toBeNull());
  });

  // The panel's two voices. A purpose's title is a QUESTION and only makes sense while it is
  // being asked; the standing panel says what it is a list of. Getting this wrong leaves "Copy
  // 3 items to…" sitting in the gutter for the rest of the session, over rows that would do
  // nothing of the sort.
  it("borrows the mode's title and marks while a walk is up, then gives them back", async () => {
    await twoTiles();
    withPin();
    await waitFor(() => expect(chooserTitle()).toBe("Panes"));
    expect(selectedRow()).toBe(1); // where the focus is
    const filled = () => rows().findIndex((r) => r.querySelector(".pane-number.current"));
    expect(filled()).toBe(1);

    runCommand("ws:choose");
    await waitFor(() => expect(chooserTitle()).toBe("Choose a pane"));
    fireEvent.keyDown(window, { key: "ArrowLeft" });
    // Now the two marks come apart, which is what says a walk is up at all.
    expect(selectedRow()).toBe(0);
    expect(filled()).toBe(1);

    fireEvent.keyDown(window, { key: "Enter" });
    await waitFor(() => expect(chooserTitle()).toBe("Panes"));
    // Back to one mark on one row — the row the walk moved the focus to.
    expect(selectedRow()).toBe(0);
    expect(filled()).toBe(0);
  });

  // The map has the same two states as the list, and takes them from the same place: the panel
  // hands both the one answer to "which pane wears the highlight". Between walks that is the
  // focused pane, so the picture says "you are here" rather than going blank until a mode is
  // entered — which is what it would do if it read the preview for itself.
  it("marks the tile the focus is in on the map, with nothing armed", async () => {
    withMiniMap();
    await twoTiles();
    withPin();
    await waitFor(() => expect(mapCells()).toHaveLength(2));
    expect(currentCell()).toBe(1);
    expect(filledCell()).toBe(1);

    fireEvent.click(rows()[0]);
    await waitFor(() => expect(currentCell()).toBe(0));
    expect(filledCell()).toBe(0);
    expect(scrim()).toBeNull();
  });

  // A placement is a mode too, and the pinned panel is the first thing that can be on screen
  // during someone ELSE's mode. Its rows move the focus, and moving the focus while the
  // workspace is waiting to be told where a new pane goes is the one thing that surface is not
  // supposed to allow.
  it("goes inert while a placement is waiting for an answer", async () => {
    await twoTiles();
    withPin();
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(chooser()!.classList.contains("inert")).toBe(false);

    runCommand("ws:new");
    await waitFor(() => expect(document.querySelector(".pane-pick-overlay")).toBeTruthy());
    expect(chooser()!.classList.contains("inert")).toBe(true);

    fireEvent.keyDown(window, { key: "Escape" });
    await waitFor(() => expect(document.querySelector(".pane-pick-overlay")).toBeNull());
    expect(chooser()!.classList.contains("inert")).toBe(false);

    // The class is half of it; the sheet is the other half, and jsdom applies none of it. The
    // click has to reach the scrim UNDER the panel — which is what cancels a placement — so
    // this is `pointer-events`, not a z-index that would merely stop it being drawn on top.
    expect(block(cssSource, ".pane-chooser.pinned.inert {")).toMatch(/pointer-events:\s*none/);
  });

  it("opens the gutter on the side the setting names", async () => {
    await twoTiles();
    withPin();
    // "auto" under a left-to-right document: the START of the reading direction, which is the
    // corner the panel floats in — so pinning moves it outward rather than across the screen.
    await waitFor(() => expect(workspaceEl().classList.contains("chooser-pinned")).toBe(true));
    expect(workspaceEl().classList.contains("chooser-left")).toBe(true);
    expect(chooser()!.classList.contains("left")).toBe(true);

    setPaneChooserSide("right");
    await waitFor(() => expect(workspaceEl().classList.contains("chooser-left")).toBe(false));
    expect(chooser()!.classList.contains("right")).toBe(true);
    setPaneChooserSide("left");
    await waitFor(() => expect(workspaceEl().classList.contains("chooser-left")).toBe(true));

    // The class is only half the claim — the other half is in the sheet, which jsdom does not
    // apply. `order` rather than a second set of positions is what makes one class enough.
    expect(block(cssSource, ".workspace.chooser-left > .pane-chooser.pinned {")).toMatch(
      /order:\s*-1/,
    );
    expect(block(cssSource, ".workspace.chooser-pinned {")).toMatch(/flex-direction:\s*row/);
  });

  // THE FLOATING PANEL FOLLOWS THE LANGUAGE, and nothing else — `paneChooserSide` is a choice
  // about a permanent strip of screen, and a card that appears for a keystroke is not that.
  // Both halves asserted, because either alone would pass against a panel with no anchor at
  // all: exactly one side class, and the sheet's rule for it.
  it("floats in the corner the reading direction starts at", async () => {
    await twoTiles();
    runCommand("ws:choose");
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(chooser()!.classList.contains("left")).toBe(true);
    expect(chooser()!.classList.contains("right")).toBe(false);
    // …and the class is not decoration: jsdom applies no CSS, so the rule that turns it into a
    // position is read out of the sheet. `left`/`right` rather than `inset-inline-start`,
    // because the document's computed direction is not the question being asked — see gutter.ts.
    expect(block(cssSource, ".pane-chooser.left {")).toMatch(/left:\s*var\(--space-4\)/);
    expect(block(cssSource, ".pane-chooser.right {")).toMatch(/right:\s*var\(--space-4\)/);
    expect(block(cssSource, ".pane-chooser {")).not.toMatch(/^\s*(left|right):/m);

    // An explicit GUTTER side does not move it: it is not in a gutter.
    setPaneChooserSide("right");
    onTestFinished(() => setPaneChooserSide("auto"));
    await new Promise((r) => setTimeout(r, 0));
    expect(chooser()!.classList.contains("left")).toBe(true);
  });

  // "auto" means the START of the READING direction, so a right-to-left document mirrors both
  // the gutter and the floating corner. The document's own `dir` is what states that, and it
  // outranks the language for the reason it does everywhere: it is a statement, not an
  // inference. Asserted as a PAIR — the two must not be able to end up on opposite sides.
  it("mirrors both the gutter and the floating corner for a right-to-left document", async () => {
    await twoTiles();
    onTestFinished(() => document.documentElement.removeAttribute("dir"));
    ...
    withPin();
    await waitFor(() => expect(workspaceEl().classList.contains("chooser-pinned")).toBe(true));
    expect(workspaceEl().classList.contains("chooser-left")).toBe(false);
    expect(chooser()!.classList.contains("right")).toBe(true);
  });

  // DESKTOP ONLY, and the ONLY honest way to say so is to show the same setting doing nothing
  // here and something a moment later. The positive control at the end is the whole test: an
  // absent panel proves nothing on its own — the feature could be broken outright, the setting
  // could have failed to save, the render could have thrown.
  it("ignores the pin on a window too narrow to spare the gutter", async () => {
    onTestFinished(() => setViewportWidth(1024)); // jsdom's own width, for the next test
    renderView(Workspace);
    await screen.findByText("project");

    setViewportWidth(PIN_MIN_VIEWPORT_PX - 1);
    withPin();
    // Nothing: no panel, no gutter, and no pin offered inside the floating chooser either —
    // a control that appears everywhere and works in one place is worse than an absent one.
    expect(chooser()).toBeNull();
    expect(workspaceEl().classList.contains("chooser-pinned")).toBe(false);

    setViewportWidth(PIN_MIN_VIEWPORT_PX);
    await waitFor(() => expect(chooser()).toBeTruthy());
    expect(workspaceEl().classList.contains("chooser-pinned")).toBe(true);
    expect(pinButton()).toBeTruthy();
  });

  // The other half of "desktop only", and the one that matters on a tablet: it passes the
  // width and fails on the pointer. Same shape as the test above — the setting is left ON
  // throughout, and only the device's answer changes.
  it("ignores the pin where a finger is the only pointer", async () => {
    const real = window.matchMedia;
    onTestFinished(() => {
      window.matchMedia = real;
      setViewportWidth(1024);
    });
    window.matchMedia = ((q: string) => ({ ...real(q), matches: /pointer:\s*coarse/.test(q) })) as
    ...
    // Back to a mouse. The gate is re-read on a resize (the pointer half is asked afresh every
    // time — see ../../pointer), so this is what a keyboard case being attached looks like.
    window.matchMedia = real;
...
describe("Settings", () => {
  ...
  // A label's accessible name is its whole text content run together, with no separator —
  // the Font row reads "FontThe typeface every terminal pane is painted in...". So "Font" is
  // a prefix of "Font size" here with no word boundary between the title and the description
  // to anchor against, and a /^Font/ regex matches BOTH rows; findByLabelText throws on that
  // rather than picking one. A predicate says which row is meant without depending on the
  // wording of either description.
  const fontRowLabel = (text: string) => text.startsWith("Font") && !text.startsWith("Font size");

  // Same shape as the placement test above and for the same reason: a select wired to the
  // wrong setter looks right until the page is reloaded, so the assertion is on the blob.
  it("chooses the terminal font, and persists it", async () => {
    renderView(Settings);
    const sel = (await screen.findByLabelText(fontRowLabel)) as HTMLSelectElement;
    // The browser's own monospace is the default and the first option — it is the one that
    // respects the font the user already chose in their browser, and the only one that
    // fetches nothing.
    ...
    fireEvent.change(sel, { target: { value: "victor-mono" } });
    const saved = JSON.parse(globalThis.localStorage?.getItem("cornus.settings") || "{}");
    expect(saved.terminalFont).toBe("victor-mono");
  });

  it("chooses the terminal font size, and persists it as a number", async () => {
    renderView(Settings);
    const sel = (await screen.findByLabelText(/^Font size/)) as HTMLSelectElement;
    // 13px is what the terminal was before the setting existed.
    expect(sel.value).toBe("13");
    // The list starts at 10 and not lower: below that the zoom table's small steps collide
    // and become keypresses that do nothing (see termFont.test.ts, which asserts the
    // property across every entry here).
    ...
    const saved = JSON.parse(globalThis.localStorage?.getItem("cornus.settings") || "{}");
    expect(saved.terminalFontSize).toBe(16);
  });

  it("chooses the terminal line height, and persists it as a number", async () => {
    renderView(Settings);
    const sel = (await screen.findByLabelText(/^Line height/)) as HTMLSelectElement;
    expect(sel.value).toBe("1");
    fireEvent.change(sel, { target: { value: "1.4" } });
    const saved = JSON.parse(globalThis.localStorage?.getItem("cornus.settings") || "{}");
    // A NUMBER, not the "1.4" the DOM hands back. termLineHeight rejects a non-number
    // outright rather than coercing it, so a setter that forgot the Number() would persist a
    // string that silently resolved back to the default on the next load — the setting would
    // appear to save and then not stick.
    expect(saved.terminalLineHeight).toBe(1.4);
  });

  // The preview is the only feedback the picker gives, so it has to follow BOTH controls.
  // Asserting the inline style is the point: the sample is what tells a user what they
  // picked, and a preview stuck on the default reads as a setting that did nothing.
  it("paints the preview in the chosen font and leading", async () => {
    const { container } = renderView(Settings);
    const preview = container.querySelector(".term-font-preview") as HTMLElement;
    expect(preview.style.fontFamily).toBe("monospace");
    expect(preview.style.fontSize).toBe("13px");
    expect(preview.style.lineHeight).toBe("1");

    fireEvent.change(await screen.findByLabelText(fontRowLabel), {
      target: { value: "cascadia-code" },
    });
    fireEvent.change(await screen.findByLabelText(/^Font size/), { target: { value: "20" } });
    fireEvent.change(await screen.findByLabelText(/^Line height/), { target: { value: "1.5" } });
    // Quoted, and with the fallback tail intact — the sample has to fall back exactly the
    // way a real pane does, or it would misreport a font whose subset is missing glyphs the
    // text uses (the box-drawing characters in it are precisely that case).
    expect(preview.style.fontFamily).toBe('"Cascadia Code", monospace');
    // px, not a token: the point of previewing a size is that it is the size you get.
    expect(preview.style.fontSize).toBe("20px");
    expect(preview.style.lineHeight).toBe("1.5");
  });
  ...
  it("renders its groups as ordinary sections, not cards", () => {
    ...
    // Every setting is the same row, whatever control it carries: six checkboxes, seven
    // selects (new-pane placement, new-pane sizing, chooser gutter side, prefix combination,
    // terminal font, terminal font size, terminal line height), and the shell-candidates textarea. (This screen
    // has been down to one group and back up to two, so the uniform-row rule is what has to
    // survive the churn — hence the shape asserted per row rather than a count alone.)
    ...
    // The font preview is the one block in the stack that is NOT a row, and it must stay
    // that way: a `.setting-row` is a `<label>`, and a label wrapping a sample with no
    // control in it is a click target that does nothing. It still carries `.setting-text`,
    // which is what keeps it aligned with the rows around it.
    const previews = Array.from(container.querySelectorAll(".setting-preview"));
    expect(previews).toHaveLength(1);
    expect(previews[0].tagName).toBe("DIV");
    expect(previews[0].matches(".setting-row")).toBe(false);
    expect(previews[0].querySelector(".setting-text > .setting-title")).not.toBeNull();
  });
});
