...
// nextPaneId is the pane after `fromId` in LAYOUT order, wrapping at the end — tmux's
...
// findIndex returns -1, so the wrap lands on 0. Focus that has fallen off the tree is
// recovered instead of propagated, the same way loadLayout recovers it with firstPaneId.
export function nextPaneId<P>(tree: Node<P>, fromId: string): string {
  const panes = allPanes(tree);
  return panes[(panes.findIndex((p) => p.id === fromId) + 1) % panes.length].id;
}
