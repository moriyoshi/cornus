...
import { addTab } from "./layout";
import type { Dir, Ext, Node, Pane } from "./layout";
import {
  applyMovePane,
  applyMoveStack,
  applySplit,
...
export interface TileDragOps<P> {
  snapshot: () => Node<P>;
  // The workspace's current extent, and a commit that carries the new one. Every rearrange can
  // change it — a tile leaving shrinks the workspace and a tile arriving grows it — so the two
  // travel together rather than the host recomputing an extent from a tree that cannot express
  // one. Under the dividing layout the extent that comes back is the one that went in.
  ext: () => Ext;
  commit: (tree: Node<P>, ext: Ext) => void;
  // The smallest a pane may be along an axis, in viewport extents — what decides whether a
  // split extends the workspace or just halves the tile. Supplied by the host because it is the
  // only side that can measure a screen; see applySplit.
  floor: (dir: Dir) => number;
...
export function createTileDrag<P>(ops: TileDragOps<P>): TileDrag<P> {
  ...
  const land = (tree: Node<P>, ext: Ext, focus: string | undefined) => {
    if (focus) ops.setFocused(focus);
    ops.commit(tree, ext);
  };

  // Read once per operation rather than at module scope: the setting is live, and a user who
  // changes it mid-session should get the new behaviour on their next drag, not their next
  // reload.
  const grows = () => workspaceExtends();

  const stackById = (srcId: string, destId: string) => {
    const { tree, ext } = applyStackPane(grows(), ops.snapshot(), ops.ext(), srcId, destId);
    land(tree, ext, srcId);
  };
  const moveById = (srcId: string, destId: string, z: DropZone) => {
    const { dir, before } = dirBefore(z);
    const { tree, ext } = applyMovePane(
      grows(),
      ops.snapshot(),
      ops.ext(),
      srcId,
  ...
  const stackStackById = (srcStackId: string, destId: string) => {
    const { tree, ext, focus } = applyStackStack(grows(), ops.snapshot(), ops.ext(), srcStackId, destId);
    land(tree, ext, focus);
  };
  const moveStackById = (srcStackId: string, destId: string, z: DropZone) => {
    const { dir, before } = dirBefore(z);
    const { tree, ext, focus } = applyMoveStack(
      grows(),
      ops.snapshot(),
      ops.ext(),
      srcStackId,
  ...
  const placeAt = (destId: string, z: DropZone) => {
    let made: { tree: Node<P>; ext: Ext; newPaneId: string };
    if (z === "stack") {
      ...
      // pane to copy from. A tab adds no tile, so the workspace does not change size.
      made = { ...addTab(ops.snapshot(), destId, (pending ?? ops.fresh)()), ext: ops.ext() };
    } else {
      const { dir, before } = dirBefore(z);
