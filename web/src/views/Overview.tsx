...
import IngressSection from "./Ingress";
...
// the Ingress section, the one session-wide setting too wide for a card (it has
// two independent halves: the server's front door and this client's conduit) —
// then one section per compose project, each carrying that project's workloads,
...
export default function Overview() {
  ...
  return (
    <>
          ...
          <Show when={config()} fallback={<span class="muted">loading…</span>}>
            {(c) => (
              <dl class="kv">
                ...
                {/* No Ingress row here. It was one — a mode chip and the base domain
                    — and it could never be more: this card's value column is ~150px,
                    so the class, the emulated front door's listen address and the
                    discovered controller Service all rode on the wire unrendered for
                    want of room. Ingress is now its own section below the cards,
                    where the server's front door sits beside the CLIENT's conduit
                    ingress — the half this card could not have shown at all. */}
                <dt>Status</dt>
                <dd>
                  <Show
                    when={!c().serverError}
      ...
      {/* Between the session-wide cards and the per-project sections, because that
          is what it is: ingress is one session-wide setting, but a two-part one too
          wide for the card row — and it scopes every project section below it. */}
      <IngressSection config={config()} />
