...
import { hasMetric, mockMetrics, notResolved, obsStatus, setUnservedMetrics } from "./metrics";
...
// is reachable from a fixture that always reports a healthy dockerhost. A test asks
// for one with seedServerInfo(); installMockFetch resets to the fixture. An
// override rather than a mutation of fx.config, which is a shared module-level
// const that would carry the change into every later test.
let serverInfoOverride: { value: Config["server"] } | undefined;
export function seedServerInfo(server: Config["server"]): void {
  serverInfoOverride = { value: server };
}
...
export function resolve(method: string, path: string, body = ""): Response {
  ...
  switch (true) {
    case rel === "/config":
      return json(serverInfoOverride ? { ...fx.config, server: serverInfoOverride.value } : fx.config);
    case rel === "/workloads":
    ...
    case rel === "/observe/status":
      return storeOff ? json({ error: OBS_OFF }, 501) : json(obsStatus);
