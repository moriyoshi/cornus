...
// truth. GET endpoints return fixtures; mutating verbs return {result:"ok"}.
//
...
import { hasMetric, mockMetrics, notResolved, obsStatus } from "../src/mock/metrics.ts";
...
const server = createServer((req, res) => {
  ...
  if (rel === "/terminals" && req.method === "POST") {
    readBody(req).then((raw) => {
      ...
      // `dir` is the working directory the pane asked for — what "Open in a terminal"
      // sends, and what a split inheriting a live cwd sends. The mock honours it (see
      // newShell) rather than dropping it, or every session would report the same
      // directory however it was opened and nothing downstream of the reported cwd
      // could be exercised here.
      let dir: string | undefined;
      try {
        const parsed = JSON.parse(raw || "{}");
        if (parsed.workload) workload = String(parsed.workload);
        if (typeof parsed.dir === "string" && parsed.dir.trim()) dir = parsed.dir.trim();
        if (Array.isArray(parsed.cmd) && parsed.cmd.length) cmd = parsed.cmd.map(String);
        // `cmdline` is the typed form the real BFF splits with a shell-words parser;
        // whitespace is close enough for a mock nobody quotes paths in.
        else if (typeof parsed.cmdline === "string" && parsed.cmdline.trim()) {
          cmd = parsed.cmdline.trim().split(/\s+/);
        }
      } catch {
        // fall through with defaults
      }
      json(res, mockTerms.create(workload, cmd, dir));
    });
    return;
  }
  // Shell discovery. The mock stands in for a busybox image: ash and sh, no bash —
  // so a pane opened here lands on /bin/ash and the auto-pick is visible rather
  // than being indistinguishable from the old hardcoded /bin/sh.
  const shellsM = rel.match(/^\/workloads\/([^/]+)\/shells$/);
  if (shellsM && req.method === "POST") {
    readBody(req).then((raw) => {
      let candidates: string[][] = [];
      try {
        const parsed = JSON.parse(raw || "{}");
        if (Array.isArray(parsed.candidates)) {
          candidates = parsed.candidates.map((c: unknown) => String(c).trim().split(/\s+/));
        }
      } catch {
        // fall through with an empty list
      }
      json(res, { candidates, found: [["/bin/ash"], ["/bin/sh"]] });
    });
    return;
  }
  ...
  switch (true) {
    ...
    case rel === "/observe/status":
      return json(res, obsStatus);
    case rel === "/observe/metrics": {
      const metric = url.searchParams.get("query") ?? "";
      // Same 400-with-a-diagnostic the real store answers for a metric nothing
      // has reported; the Metrics view renders it as "no data", not as an error.
      if (!hasMetric(metric)) return res.writeHead(400).end(notResolved(metric));
      return json(
        res,
        mockMetrics(
          metric,
