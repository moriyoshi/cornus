...
# shadow the other.
#
# web-svc is rooted at `/svc` and web-root at `/`, both on `shared.example.com`. The
...
# The longer path is `/svc`, not `/api`: whoami reserves `/api` for a JSON dump that
# would mask the plaintext `Hostname:` echo (see the .yaml note).
#
...
wait(name = "e2e-web-svc", running = 1, timeout = "180s")
log("✓ both services up; two ingresses share shared.example.com (/ and /svc)")
...
cases = [
    ("/svc", "backend-svc"),  # exact request to the longer prefix -> web-svc
    ("/svc/v1/x", "backend-svc"),  # deeper under /svc -> still web-svc
    ("/", "backend-root"),  # only / matches -> web-root
    ("/other", "backend-root"),  # /svc is not a prefix of /other -> web-root
    ("/svcx", "backend-root"),  # element-boundary: /svcx is NOT under /svc -> web-root
]
