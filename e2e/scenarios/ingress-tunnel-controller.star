...
def run(ssh):
    ...
    # them in its routing table, hence the generous retry — and retry_until=200
    # rather than retry_5xx alone, because that pre-ingest moment is answered by
    # the controller's DEFAULT backend with a 404, not a 5xx, so retry_5xx returns
    # it verbatim and the assertion fails on sync latency. The negative 404
    # assertion further down deliberately keeps the plain retry: once these two
    # positives have passed, the controller is provably caught up, so a 404 there
    # means "no rule declares this Host" rather than "not yet".
    r = http_get(url = url + "/", host = "front.example.com", retry = "120s", retry_5xx = True, retry_until = 200)
    ...
    r = http_get(url = url + "/api", host = "front.example.com", retry = "120s", retry_5xx = True, retry_until = 200)
