...
if TARGET != "kube":
    ...
    # Retry: the positive path above can pass during the brief window before the
    # CNI has programmed the deny (nothing is blocked yet), so poll until the
    # stranger is actually blocked rather than asserting once and racing
    # enforcement. A non-enforcing CNI keeps returning "nginx" and this times out.
    denied = ""
    for _ in range(15):
        denied = pod_exec(app = "npe-stranger", cmd = "wget -qO- --timeout=8 http://web 2>&1 | grep -om1 nginx || echo BLOCKED")
        if "BLOCKED" in denied:
            break
        sleep(duration = "2s")
