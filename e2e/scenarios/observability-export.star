...
if TARGET == "local":
    ...
    if probe.get("status", 0) != 200:
        ...
        deploy(
            ...
            restart = "no",
            command = ["sh", "-c", "echo %s; sleep 30" % marker],
        )
    ...
    # No CORNUS_OBS at all. The receive route must still exist, because there IS
    # somewhere to put telemetry — just not locally. This is the configuration a
    # plain pure-Go build can serve, so it is asserted even when the store leg
    # above skipped.
    gwUp = otlp_collector()
    gw = serve(env = {"CORNUS_OBS_EXPORT_ENDPOINT": "http://" + gwUp})
    gwBase = "http://" + gw
