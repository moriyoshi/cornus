...
if TARGET != "kube":
    ...
    # Second negative control: a token that is every bit as VALID as the first —
    # same cluster issuer, same JWKS, same "cornus" audience, unexpired — minted
    # for a DIFFERENT ServiceAccount, which the server's scope map does not name.
    # This is the assertion that separates "the signature and audience checked
    # out" from "the operator's policy granted this caller something", and no
    # amount of signing can satisfy it.
    cornus(
        "config", "set-context", "unmapped",
        "--pf-namespace", NS, "--pf-service", "cornus-auth", "--pf-remote-port", "5000",
        "--kube-auth-service-account", "cornus-auth", "--kube-auth-audience", "cornus",
        env = env,
    )
    ...
    # Negative: a VALID cluster token for an unmapped ServiceAccount. It verifies
    # against the same JWKS with the same audience, so everything the JWKS
    # verifier checks passes; only the scope map declines it. Proves authority
    # comes from the operator's policy and not from the issuer's signature — and
    # that a cluster token cannot grant itself access merely by existing.
    out = cornus("--context", "unmapped", "compose", "-f", app, "ps", env = env, expect_fail = True)
    assert_contains(out, "401",
                    "an unmapped ServiceAccount must be refused (got %r)" % out)
    log("✓ a valid cluster token for an unmapped ServiceAccount is refused — the scope map is what grants")
