...
if TARGET == "local":
    ...
    if probe.get("status", 0) != 200:
        ...
        # Long-lived deliberately: a `restart="no"` spec becomes a Job on kube, and
        # (*kubernetes.Backend).List enumerates Deployments only, so the recorder
        # never tails it. See the gated one-shot section of observability-store.star
        # — the query surfaces this scenario covers are not the place to pin it.
        deploy(
            name = "observe-app",
            image = "busybox:latest",
            command = ["sh", "-c", "echo %s-one; echo %s-two; sleep 600" % (marker, marker)],
        )
