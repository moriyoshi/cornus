...
if TARGET != "docker":
    ...
    # `docker run -t`: the container must actually get a pseudo-TTY.
    #
    # `Tty` was missing from the proxy's create-request struct, so it was dropped
    # at JSON decode and EVERY proxied container was non-TTY no matter what the
    # CLI asked for. Nothing noticed, because the `-it` exec block above allocates
    # its PTY on the EXEC, which was parsed correctly — the container's own TTY
    # was never asserted anywhere.
    #
    # The test asks the container itself rather than trusting inspect: `[ -t 1 ]`
    # is the same isatty() check the programs that care (shells, progress bars,
    # anything colourizing output) make. Both arms are asserted, because a proxy
    # that hardcoded Tty:true would pass the positive one alone.
    tty_probe = "if [ -t 1 ]; then echo ISATTY; else echo NOTTY; fi; sleep infinity"
    for name, flags, want_out in [("ttyon", ["-t"], "ISATTY"), ("ttyoff", [], "NOTTY")]:
        argv = ["-H", host, "run", "-d"] + flags + ["--name", name, "alpine:3.20", "sh", "-c", tty_probe]
        docker(*argv)
        wait(name = name, running = 1, timeout = "180s")
        out = docker("-H", host, "logs", name)
        assert_contains(out, want_out,
                        "`docker run %s` produced %r: the container's stdout is not what the CLI asked for"
                        % (" ".join(flags), out))
        # inspect must agree: the CLI reads Config.Tty from here to choose its
        # attach/logs stream decoder (raw for a TTY, stdcopy-multiplexed
        # otherwise), so a container whose reported Tty disagrees with the one it
        # actually has makes the CLI mis-frame the stream it receives.
        ins = docker("-H", host, "inspect", name, "--format", "{{.Config.Tty}}")
        want = "true" if flags else "false"
        assert_contains(ins, want, "inspect Config.Tty = %r, want %r" % (ins, want))
        docker("-H", host, "rm", "-f", name)
    log("✓ `docker run -t` allocates a real pseudo-TTY and inspect reports it (both arms)")
