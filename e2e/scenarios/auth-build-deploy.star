...
if TARGET != "docker":
    ...
    # Reaches Docker Hub directly, so it inherits the registry's own bad days (a
    # 502 from registry-1.docker.io failed the docker leg on 2026-08-06). An
    # upstream 5xx is not a statement about cornus: retry, then report the last
    # attempt's output rather than a bare exit status. Same treatment as cli.star.
    pull = {"code": 0, "output": ""}
