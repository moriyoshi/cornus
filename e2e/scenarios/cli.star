...
if TARGET != "docker":
    ...
    # The only step in the suite that reaches Docker Hub directly, so it is the
    # only one exposed to the registry's own bad days: on 2026-08-06 the docker
    # leg failed here on a 502 Bad Gateway from registry-1.docker.io. An upstream
    # 5xx says nothing about cornus, so retry before failing the scenario — and
    # report the last attempt's output, or the failure reads as "exit status 1".
    pull = {"code": 0, "output": ""}
    for attempt in range(3):
        if attempt > 0:
            sleep("5s")
        pull = sh(cmd = "docker pull alpine:3.20")
        if pull["code"] == 0:
            break
    assert_eq(pull["code"], 0, "docker pull alpine:3.20 failed 3 times: %s" % pull["output"])
