...
#
#   - On docker/containerd/bare/incus the archive primitives work, so the explorer moves
#     bytes through the RELAY. Client-local bind mounts need root on those targets to
#     kernel-9p-mount (see deploy-mounts.star), so the redirect is out of reach here.
#   - On kubernetes the archive trio is explicitly unsupported (StatPath/CopyFrom/CopyTo
#     all answer "cp/archive not supported ... use kubectl cp"), so the relay CANNOT
#     work — and the bind redirect, realized by the mount sidecar, is not an optimization
#     there but the only reason the explorer is usable at all.
#
...
""" % ro))
    ...
    # THE PAIRED NEGATIVE, and the reason the redirect matters on this backend at all.
    #
    # Read the split precisely, because the first version of this assertion got it wrong
    # and the run corrected it: kubernetes cannot do cp/archive
    # (StatPath/CopyFrom/CopyTo), but it CAN exec — so an image path still LISTS (the
    # listing is a shell glob over an exec) and cannot be COPIED. Asserting that listing
    # fails was simply false, and the failure is what exposed the workdir defect below.
    ...
    # ...and the copy is the part that cannot work here.
    cp = http(
        method = "POST",
    ...
    assert_true(cp["status"] != 200,
                "kubernetes has no cp/archive support, so copying an image path must fail; got %d" % cp["status"])
    log("✓ preflight routed to the client; the image path listed but could not be copied, as this backend requires")
    ...
    #
    # This is the section the archive arm cannot have, and it is the whole reason the
    # operator exists. On kubernetes StatPath/CopyFrom/CopyTo are unsupported outright,
    # so before the operator a volume-backed path could be LISTED (exec works) and then
    # neither read, written, nor copied — the explorer showed a directory whose every
    # file answered 501. The caretaker mounts the app's volumes and serves them
    ...
    # whole thing in the pod — and on THIS backend that is not merely faster, it is the
    # only way it can happen, since there is no archive to relay through.
    vol_copy = http(
        method = "POST",
    ...
    proj = temp_dir()
    compose_file = proj + "/compose.yaml"
    write_file(path = compose_file, content = compose_yaml("alpine:3.20", ""))
