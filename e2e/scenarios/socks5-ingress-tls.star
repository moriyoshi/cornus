...
#
# retry_5xx: `wait` above only proves the CONTAINER is running, not that nginx has
# bound :80 inside it. In that window the emulated ingress cannot dial its upstream
# and answers its own 502 — an HTTP response, so the plain retry loop (which only
# absorbs dial errors) returns it verbatim and the assertion fails on startup
# latency rather than on TLS. The other emulated-ingress scenarios
# (socks5-ingress.star, socks5-ingress-longest-match.star) already pass it; this one
# was the outlier and flaked on the CI docker target.
resp = http_get(url = "https://app.native-cert.example.test/", socks5 = proxy, ca_file = cert_file, retry = "30s", retry_5xx = True)
