...
if TARGET == "local":
    ...
    if probe.get("status", 0) != 200:
        ...
        # the kubernetes backend realizes it as a Job, which the recorder cannot
        # see at all — see the gated one-shot section at the end of this file. The
        # zero-touch recording claim is about workloads in general, so it is
        # asserted here on the shape that works on every backend, and the one-shot
        # gap is pinned separately rather than folded into this assertion.
        deploy(
            name = "obs-probe",
            image = "busybox:latest",
            command = ["sh", "-c", "echo %s-out; echo %s-err 1>&2; sleep 600" % (marker, marker)],
        )
        ...
        # --- 7. one-shot workloads (KNOWN-FAILING on kube) ---------------------
        # A run-to-completion workload is the case a durable store exists for most:
        # by the time anyone asks, the container is gone, so the store is the ONLY
        # place its output can be. The assertion below states that, and is
        # deliberately not weakened.
        #
        # It fails on the kube target today, and the cause is not in the
        # observability layer: `restart="no"` is one-shot (deploy.IsOneShot), which
        # the kubernetes backend realizes as a **Job** rather than a Deployment
        # (pkg/deploy/kubernetes/job.go, pinned by deploy-oneshot.star) — while
        # (*kubernetes.Backend).List enumerates ONLY Deployments
        # (pkg/deploy/kubernetes/kubernetes.go). The recorder drives its tails off
        # List (pkg/server/logrecorder.go reconcile), so a one-shot workload is
        # never tailed, nothing is recorded, and nothing reports an error. The
        # observed failure is an empty query result:
        #
        #   assert_contains failed: "" does not contain "cornus-obs-e2e-marker-out"
        #
        # Gated (rather than deleted) so the assertion stays in the tree and stays
        # parse+resolve-checked by `make e2e-check` — the same handling as
        # auth-build-deploy.star and dockerd-exit-code-nonzero.star. Drop the gate
        # once List covers Jobs; see TODO.md "One-shot workloads are invisible to
        # the kubernetes backend's List".
        if getenv(name = "E2E_OBS_ONESHOT"):
            one_marker = "cornus-obs-oneshot-marker"

            # Kept alive for the whole poll window on purpose. Whether the record
            # OUTLIVES the workload is a second question; this pins the first one,
            # that a one-shot workload is recorded at all — so the failure reads as
            # "never recorded" rather than as a race with its exit.
            deploy(
                name = "obs-oneshot",
                image = "busybox:latest",
                restart = "no",
                command = ["sh", "-c", "echo %s; sleep 600" % one_marker],
            )
            wait(name = "obs-oneshot", timeout = "60s")

            one_found = ""
            for _ in range(30):
                one_got = cornus("observe", "logs", "--server", base, "--service", "obs-oneshot", "--limit", "50")
                if one_marker in one_got:
                    one_found = one_got
                    break
                sleep("1s")
            assert_contains(
                one_found,
                one_marker,
                "a one-shot workload's output was never recorded; its container is gone, so the store " +
                "is the only place it could be read from",
            )
            log("✓ a one-shot workload's output was recorded")
            remove(name = "obs-oneshot")
