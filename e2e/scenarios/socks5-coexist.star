...
if TARGET != "docker":
    ...
    # ONE agent, not two. Everything below judges the two proxies by what they
    # serve, which two independent agents would satisfy just as well — so pin the
    # premise first: a single `cornus daemon status` (one control socket) must
    # inventory BOTH projects. This is the "through the background agent" half of
    # the claim; the traffic assertions are the "coexist" half.
    inv = cornus("daemon", "status")
    log(inv)
    assert_contains(inv, "coexa", "the shared-conduit project is not held by this agent")
    assert_contains(inv, "coexb", "the session-local-conduit project is not held by the SAME agent")
    # ...and it banners BOTH listeners, so the two proxies are two listeners in one
    # process rather than one proxy answering for both projects.
    assert_contains(inv, shared, "the agent does not report the SHARED proxy listener %s" % shared)
    assert_contains(inv, local, "the agent does not report the SESSION-LOCAL proxy listener %s" % local)
    log("✓ a single agent inventories both projects and both proxy listeners")
