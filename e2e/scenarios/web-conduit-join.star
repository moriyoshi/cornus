...
#
# A conduit is a rendezvous at an ADDRESS: the first process to ask for one binds
# it and hosts a control socket beside it, and anyone later asking for an address it
# covers joins over that socket. So the UI does not "adopt" anything — it asks the
# rendezvous registry where a proxy is already running and goes there.
#
# This scenario predates that design and was written against the one before it,
# where sharing was emergent from two configurations hashing to the same
# clientconduit.Identity and a `cornus web` resolved its own configuration slightly
# differently (its config path never filled in ingress the way `compose up -d`
# does) — so the everyday case was a SECOND proxy that then collided on one bind
# address. The assertions below are unchanged because they were never about the
# mechanism; they are about the outcome.
#
...
if TARGET != "docker":
    ...
    # The compose session picks the address; the UI must find its way here on its
    # own. (`.shared:PORT` is the legacy spelling for "loopback, this port"; a plain
    # socks5://127.0.0.1:PORT means the same thing now that the address IS the
    # identity.)
    p = free_port()
    proxy = "127.0.0.1:" + p
