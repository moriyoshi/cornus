...
if TARGET != "docker":
    ...
    def measure(name, label):
        ...
        dt_w = timed(name, "dd if=/dev/zero of=/data/seq bs=%d count=%d conv=fsync 2>/dev/null" % (bs, mb))
        bench_record(label + "-seq-write", dt_w, unit = "s", extra = {"MB": mb, "MBps": mb / dt_w})

        # Sequential read-back.
        dt_r = timed(name, "dd if=/data/seq of=/dev/null bs=%d 2>/dev/null" % bs)
        ...
        dt_wal = timed(name, "i=0; while [ $i -lt %d ]; do dd if=/dev/zero of=/data/w bs=4096 count=1 conv=fsync 2>/dev/null; i=$((i+1)); done" % n)
