...
#   IMBH_VERSION   imbh-go release supplying libimbhgo.a (default: v0.1.0)
#
...
: "${OUT:?OUT is required}"
VERSION="${VERSION:-dev}"
IMBH_VERSION="${IMBH_VERSION:-v0.1.0}"
...
# OUT is normally relative (the workflow uploads it by that name, and a relative
# path keeps the Windows leg working under git-bash, where an absolute POSIX path
# would not survive the trip into a native Windows `go build`). A bare relative
# name is not executable, so derive an explicit path for the checks below.
case "${OUT}" in
  /* | ./* | *:*) EXE="${OUT}" ;;
  *) EXE="./${OUT}" ;;
esac

# Refuse to publish an artifact whose observability store silently no-ops. A
# mistyped tag still COMPILES — it just selects the stub — and every other signal
# here (exit status, file size, `file` output) would look fine, so assert against
# what the binary reports about itself. This is the payoff of building natively:
# the artifact is runnable on the machine that produced it.
echo "==> verifying the shipped feature set"
"${EXE}" version --features --output json | tee /dev/stderr | grep -q '"obsstore":"yes"' || {
  echo "ERROR: ${OUT} does not carry the observability store" >&2
  exit 1
}
"${EXE}" version --features --output json | grep -q '"otelcollector":"yes"' || {
  echo "ERROR: ${OUT} does not carry the embedded OpenTelemetry Collector" >&2
  exit 1
}
