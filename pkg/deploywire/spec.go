...
type Event struct {
	...
	// ExitCode is the status the workload SETTLED with, as
	// api.DeployStatus.TerminalExitCode resolved it. nil means UNKNOWN — the
	// workload was still running when the session ended, or the backend cannot
	// report a code at all (barehost/incushost never fill
	// api.InstanceStatus.ExitCode).
	//
	// The server stamps it on TERMINAL events only, and reads it from the backend
	// BEFORE deleting the workload: that is the last moment the code exists
	// anywhere, so a caller holding only the session (rather than polling status)
	// still learns how its workload ended.
	//
	// Wire compatibility: the field is optional in both directions. An older
	// server omits it and a new caller decodes nil = unknown — exactly the answer
	// it could give before the field existed. An older caller ignores it
	// (encoding/json drops unknown members). A version-skewed pair therefore
	// degrades to "cannot say", never to a fabricated 0.
