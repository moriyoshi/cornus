...
// TestSessionDirIsTraversable is a regression test for a failure that looked
// like a 9P problem for several rounds and was a directory mode.
//
// os.MkdirTemp always creates 0700. The CONTAINER RUNTIME has to walk the
// session directory to reach the mountpoints inside it, so on a runtime that
// does not run as this server's user — rootless podman — the deploy failed
// outright with `statfs .../sess-<id>/m0: permission denied`. That error names
// the MOUNTPOINT, which is what sent the investigation at v9fs access modes for
// several rounds; the mountpoint was 0755 and its PARENT was 0700.
//
// 0711 rather than 0755: traversable, not listable. The directory's name is
// random, so it stays an unguessable capability.
//
// Prepare returns early when there are no LocalMounts, so the spec must carry
// one to reach the creation at all. It then fails on the nil session — after
// the directory exists, which is the whole point of ordering the check this way.
func TestSessionDirIsTraversable(t *testing.T) {
	base := t.TempDir()
	m := NewMountManager(base)
	t.Cleanup(m.Teardown)

	spec := DeployAttachSpec{
		Spec:        api.DeploySpec{Mounts: []api.Mount{{Source: "/src", Target: "/data"}}},
		LocalMounts: []LocalMount{{Index: 0, Name: "m0"}},
	}
	if _, err := m.Prepare(nil, spec); err == nil {
		t.Fatal("Prepare with a nil session unexpectedly succeeded; this test relies on it " +
			"failing AFTER the session directory is made")
	}
	if m.sessDir == "" {
		t.Fatal("Prepare returned before creating the session directory, so this test no longer " +
			"observes the mode it exists to pin")
	}
	fi, err := os.Stat(m.sessDir)
	if err != nil {
		t.Fatalf("stat session dir: %v", err)
	}
	perm := fi.Mode().Perm()
	if perm&0o011 != 0o011 {
		t.Fatalf("session dir mode = %#o, want the traverse bits set for group and other: a "+
			"runtime that does not run as this server's user cannot reach the mountpoints "+
			"inside it, and the deploy fails naming the MOUNTPOINT rather than this directory", perm)
	}
	if perm&0o044 != 0 {
		t.Errorf("session dir mode = %#o is listable; traversal is what a runtime needs, and "+
			"listing exposes every live mount session on the host", perm)
	}
}
