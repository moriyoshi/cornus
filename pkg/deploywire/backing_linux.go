...
func Mount9P(sock, target string, readOnly, writeback bool) error {
	...
	// No dfltuid/dfltgid here, and that is a measured decision rather than an
	// omission. They are the obvious lever for making a 9P mount present the ids
	// a workload can use — needed by any runtime that remaps ids into a user
	// namespace — and under 9p2000.L they do nothing: the kernel accepts them
	// (they appear in /proc/mounts) and never applies them, because the option is
	// documented as a fallback for when the SERVER supplies no ids and a
	// 9p2000.L getattr always does.
	//
	// Measured: mounting with dfltuid=100999,dfltgid=100999 left a
	// server-root-owned file reading as 0:0 inside the container, with the
	// options visibly present on the mount. So id mapping for 9P mounts has to
	// happen in cornus's own 9P server, in the ids it reports — not here.
