...
// readOnly reports whether the registry has no content store to write to (pure
// re-export mode). Write verbs — blob upload, manifest PUT, blob/manifest DELETE
// — are rejected in that mode; the authoritative store is the local runtime's
// (the daemon or containerd), mutated there, not through /v2/*.
func (r *Registry) readOnly() bool { return r.store == nil }
