...
// armedPanicStore wraps a hub.Store and, once armed, panics exactly once on the
// next Catalog() call — a seam for driving a panic through the notifier's poll
// loop without touching production code. The initial subscribe snapshot reads
// Catalog before arming, so it never panics.
type armedPanicStore struct {
	hub.Store
	armed atomic.Bool
	fired atomic.Bool
}

func (p *armedPanicStore) Catalog() []string {
	if p.armed.Load() && p.fired.CompareAndSwap(false, true) {
		panic("synthetic panic in the catalog poll loop")
	}
	return p.Store.Catalog()
}

// TestCatalogNotifierSupervisedAcrossPanic proves the poll loop is a supervised
// child (supervisor.Restart): a panic in one poll iteration is recovered and the
// loop relaunched, so catalog changes keep flowing to watchers rather than being
// silently lost. If the panic crashed the goroutine (pre-fix) the change below
// would never arrive and the test would either crash the process or time out.
func TestCatalogNotifierSupervisedAcrossPanic(t *testing.T) {
	reg := hub.NewRegistry()
	reg.Register("c1", "web", "10.0.0.1:80", "")
	store := &armedPanicStore{Store: reg}
	n := newCatalogNotifier(store, 5*time.Millisecond)
	...
	recv := func(want string) {
		...
		case got, ok := <-ch:
			...
			if strings.Join(got, ",") != want {
				t.Fatalf("catalog = %v, want %q", got, want)
			}
		case <-time.After(5 * time.Second):
			t.Fatalf("timed out waiting for catalog %q", want)
		}
	}

	// Initial snapshot (read before arming, so it does not panic).
	recv("web")

	// Arm the panic: the next poll-loop Catalog() call panics once. The supervisor
	// must recover it and relaunch the loop.
	store.armed.Store(true)

	// A subsequent registration must still be delivered — proof the loop survived
	// the panic (was restarted) and kept polling.
	reg.Register("c2", "db", "10.0.0.2:5432", "")
	n.changed()
	recv("db,web")

	// The panic really did fire (otherwise this test proves nothing).
	if !store.fired.Load() {
		t.Fatal("the synthetic catalog panic never fired; the test did not exercise recovery")
	}
}
