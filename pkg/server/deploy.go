...
func (s *Server) handleDeployCollection(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		// A client-sourced credential is minted ON THE CALLER'S MACHINE for the
		// lifetime of a deploy-attach session (see api.CredentialSource: "Backend
		// names the CLIENT-side source backend ... It runs on the caller's machine").
		// This endpoint has no session, so there is nobody to mint it and the
		// workload's fetch would find no source. Refuse rather than deploy something
		// that cannot work: the deploy used to succeed and the credential simply never
		// arrived, which surfaces as an application error far from the cause.
		//
		// The CLI already refuses this combination in checkDetachable, but that is
		// client-side only — a direct API caller, or any non-cornus client, reached
		// backend.Apply with credentials set and nothing stopped it. The wording
		// deliberately matches the CLI's so the two are recognisably the same rule.
		if spec.Credentials != nil && len(spec.Credentials.Sources) > 0 {
			writeJSON(w, http.StatusBadRequest, map[string]string{
				"error": "a stateless deploy cannot broker client-sourced credentials (they are minted on the client for the session's lifetime); deploy without --detach, or remove spec.credentials",
			})
			return
		}
		stampOriginSubject(&spec, Identity(r))
		// Point telemetry with no endpoint at this server's own store, before any
		// backend sees the spec, so every backend keeps consuming a concrete
		// endpoint exactly as before.
		s.normalizeTelemetry(r.Context(), &spec, backend.Name())
...
//	POST   /.cornus/v1/deploy/{name}/fsop      -> structured filesystem operation
func (s *Server) handleDeployItem(w http.ResponseWriter, r *http.Request) {
	...
	if hasAction {
		...
		case "fsop":
			// Same read/write split as archive, but the method cannot make it: every
			// fsop is a POST, so the OP decides. Anything not on the read list is
			// gated, which is the safe default for an op set that will grow.
			switch api.FSOp(r.URL.Query().Get("op")) {
			case api.FSOpStat, api.FSOpList, api.FSOpGet:
			default:
		...
		case "archive":
			...
			return
		case "fsop":
			s.handleDeployFSOp(w, r, backend, name)
			return
		case "exec":
	...
	case http.MethodDelete:
		...
		// A deleted deployment has nothing to tunnel to and nothing to route to;
		// tear its tunnel down and withdraw its ingress routes too.
		s.tunnels.stop(name)
		s.ingress.remove(r.Context(), name)
...
// atoiOrZero parses a non-negative integer, yielding 0 for anything else.
...
func (s *Server) handleDeployLogs(w http.ResponseWriter, r *http.Request, backend deploy.Backend, name string) {
	...
	opts := api.LogOptions{
		...
		Since:      q.Get("since"),
		Until:      q.Get("until"),
		// Absent or unparseable means instance 0, which is what every caller
		// meant before the field existed.
		Instance: atoiOrZero(q.Get("instance")),
	}
...
func (s *Server) handleDeployStats(w http.ResponseWriter, r *http.Request, backend deploy.Backend, name string) {
	...
	lw := newLazyFlushWriter(w)
	opts := api.StatsOptions{Stream: stream, Instance: atoiOrZero(q.Get("instance"))}
	if err := backend.Stats(r.Context(), name, opts, lw); err != nil {
