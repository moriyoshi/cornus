...
//
// podman is given an endpoint here rather than being skipped. It is the one
// backend with no default socket, so startup deliberately refuses it when
// neither selector is set (see resolveRegistrySource) — that refusal is the
// subject of TestValidateDeployBackendRejectsPodmanWithNoEndpoint. What THIS
// test is about is the NAME being accepted, so the endpoint is configured to
// isolate the two concerns.
func TestValidBackendsStillResolve(t *testing.T) {
	for _, b := range append([]string{""}, knownDeployBackends...) {
		...
		t.Setenv("CORNUS_PODMAN_SOCKET", "/run/podman/podman.sock")
		if _, err := resolveRegistrySource(config.Config{}); err != nil {
