...
// severs in-flight ForwardPort bridges on teardown; done closes when the accept
// loop has exited; closeFn tears down the backend tunnel (and any shim listener).
...
func (m *tunnelManager) start(backend deploy.Backend, name, token string, ag agent.Agent, port int, proto string) (api.TunnelStatus, error) {
	...
	ts := &tunnelSession{port: port, cancel: cancel, done: make(chan struct{})}
	...
	go m.serve(sctx, backend, name, ts, accept)
	if old != nil {
