	...
	return deploywire.DeployAttachSpec{
		Spec: api.DeploySpec{
			...
			Credentials: &api.CredentialSpec{Sources: []api.CredentialSource{{
				...
				Deliveries: []api.CredentialDelivery{{Kind: "endpoint", Provider: "generic"}},
			}}},
		},
...
func TestCredentialEnvDeliveryFetch(t *testing.T) {
	...
	as := deploywire.DeployAttachSpec{
		Spec: api.DeploySpec{
			...
			Credentials: &api.CredentialSpec{Sources: []api.CredentialSource{{
				...
				Deliveries: []api.CredentialDelivery{{Kind: "env", EnvVar: "OPENAI_API_KEY"}},
			}}},
		},
		CredentialSources: []deploywire.CredentialBacking{{
	...
	for _, d := range creds[0].Deliveries {
		if d.Kind == "env" {
	...
	as := deploywire.DeployAttachSpec{
		Spec: api.DeploySpec{
			...
			Credentials: &api.CredentialSpec{Sources: []api.CredentialSource{{
				...
				Deliveries: []api.CredentialDelivery{{Kind: "endpoint", Provider: "anthropic-proxy"}},
			}}},
		},
		CredentialSources: []deploywire.CredentialBacking{{
