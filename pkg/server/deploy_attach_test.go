...
// TestDeployAttachApplyErrorRemovesWorkload proves the apply-error path REMOVES the
// (possibly partially-created) workload, not just drops the session. A backend can
// create the Job/Deployment before a later dependent step (PVC/Service/Ingress)
// fails; leaving it running orphaned it against a torn-down mount session — the pod
// then started on an empty mount and failed confusingly downstream. On an apply
// error the server must Delete it, like the not-ready path.
func TestDeployAttachApplyErrorRemovesWorkload(t *testing.T) {
	fb := &fakeBackend{applyErr: errors.New("create ingress: forbidden")}
	srv := newTestServer(t, fb)
	defer srv.Close()
	...
	// The session ends on the apply error (surfaced to the client via Serve's return
	// and/or a terminal event — either way Serve returns). The behavior under test is
	// the server-side cleanup that follows: it must Delete the partial workload.
	_ = deploywire.Serve(ctx, wsURL, as, nil, func(deploywire.Event) {}, nil, wire.ClientTransport{})
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		if fb.wasDeleted("web") {
			return
	...
	t.Fatal("apply error must remove the partial workload (it was left orphaned)")
}
...
// dockerhost never silently steals its deploys away from the existing co-located
// fast path (applyWithHostMounts) unless the operator has explicitly opted in
// (CORNUS_DOCKER_REMOTE).
//
// The gate does NOT mean "fall back to the co-located path" in general: only
// dockerhost and bare have one (hostcheck.UsesHostMountFastPath). For containerd
// a false answer means client-local mounts are unavailable, which is what
// TestClientLocalMountsUnavailableNamesTheKnob covers.
...
// TestDeployAttachApplyErrorReachesTheCaller pins what the test above
// deliberately left open ("via Serve's return and/or a terminal event — either
// way Serve returns"). That hedge hid a real bug: the terminal event raced the
// handler's deferred session Close and was usually lost, so the caller saw a
// clean EOF and could only read it as success. `cornus deploy` printed nothing
// about the failure and exited 0, with the reason only in the server log.
//
// Both halves are asserted because the CLI needs both: the callback drives what
// the user sees, the returned error drives the exit code.
func TestDeployAttachApplyErrorReachesTheCaller(t *testing.T) {
	const applyErr = "create ingress: forbidden"
	fb := &fakeBackend{applyErr: errors.New(applyErr)}
	srv := newTestServer(t, fb)
	defer srv.Close()
	...
	as := deploywire.DeployAttachSpec{Spec: api.DeploySpec{Name: "web", Image: "localhost:5000/web:v1"}}
	err := deploywire.Serve(ctx, wsURL, as, nil, func(e deploywire.Event) {
		if e.Err != "" {
			gotErrEvent = e.Err
		}
	}, nil, wire.ClientTransport{})
	...
	if err == nil {
		t.Fatal("Serve returned nil on a failed apply; the CLI would exit 0 on a deploy that never happened")
	}
	if !strings.Contains(err.Error(), applyErr) {
		t.Errorf("Serve error = %q, want it to carry the backend's reason %q", err, applyErr)
	}
	if !strings.Contains(gotErrEvent, applyErr) {
		t.Errorf("caller saw error event %q, want the backend's reason %q — this is what the user is shown", gotErrEvent, applyErr)
	}
}

// A rejected spec is refused before any backend call, on a different return path
// from the apply error above; it must reach the caller just the same.
func TestDeployAttachSpecRejectionReachesTheCaller(t *testing.T) {
	...
	// No image: rejected by the handler's own validation.
	as := deploywire.DeployAttachSpec{Spec: api.DeploySpec{Name: "web"}}
	err := deploywire.Serve(ctx, wsURL, as, nil, func(deploywire.Event) {}, nil, wire.ClientTransport{})
	if err == nil || !strings.Contains(err.Error(), "requires name and image") {
		t.Fatalf("Serve error = %v, want the spec-validation reason", err)
	}
}
