...
func TestPeriodicGCSupervisedAcrossPanic(t *testing.T) {
	...
	s.gcRun = func(context.Context) (gcResponse, error) {
		...
		ran <- struct{}{} // a post-panic run reached its normal (non-panicking) body
		return gcResponse{}, nil
	}

	// Drive the supervised loop with an injected, unbuffered tick channel instead
	// of a free-running wall-clock ticker. This exercises the real supervisor
	// panic→restart path (s.sup, supervisor.Restart) while making the assertion
	// deterministic: the test controls exactly how many ticks the loop sees, so no
	// stray tick can re-set gcRunning after we stop sending. (The old real-ticker
	// version was flaky under load: a fast free-running ticker kept the loop
	// perpetually busy, so gcRunning read true on nearly every poll sample.)
	ticks := make(chan time.Time)
	s.gcTicks = ticks
	s.gcInterval = time.Millisecond // only affects restart-unrelated timers; unused here
	s.startPeriodicGC()
	defer s.closeResources()

	// Tick 1 → the run panics; the supervisor recovers it and restarts the loop
	// after its backoff. Sending is fire-and-forget: the loop receives, then panics.
	ticks <- time.Now()

	// Tick 2 blocks until the *restarted* loop is receiving again — natural
	// synchronization on the supervisor restart, no wall-clock guessing. Its run
	// takes the normal path and signals ran.
	ticks <- time.Now()
	select {
	case <-ran:
	case <-time.After(10 * time.Second):
		t.Fatalf("no post-panic run after 2 ticks (calls=%d); the supervisor should have restarted the loop", calls.Load())
	}

	// With no further ticks in flight, gcRunning must settle back to false and
	// stay there — proving runGCTick's defer cleared the guard across the panic.
	// Deterministic now: nothing re-sets it, so this only waits for the single
	// in-flight post-panic run to finish clearing it.
	deadline := time.Now().Add(10 * time.Second)
	for s.gcRunning.Load() {
		if time.Now().After(deadline) {
			t.Fatal("gcRunning left true after the panicking run — a future tick would wrongly skip as \"already running\"")
		}
		time.Sleep(time.Millisecond)
	}
}
