...
// TestExecCreateFailureIsLogged: a failed exec CREATE must reach the server log,
// not just the requester.
//
// This handler can answer with a status and does — 500 with the reason in the
// body — which is why it was the one exec path that logged nothing. The body goes
// to whoever asked; the server's own record stayed empty, so a failed exec left no
// trace for an operator, and the ABSENCE of a warning could not be used as
// evidence about which layer failed. That is what made the incus `exec_run`
// failure of 2026-08-06 (E2E run 31095177832) undiagnosable after the fact: the
// scenario reported only that the tool errored, and the server log — which does
// carry a WARN for every OTHER exec failure — was silent, so silence meant
// "either the backend did not fail, or the create failed and never said so".
//
// The assertion is on the log, not on the 500: the status was always correct.
func TestExecCreateFailureIsLogged(t *testing.T) {
	var buf bytes.Buffer
	prev := slog.Default()
	slog.SetDefault(slog.New(slog.NewTextHandler(&buf, &slog.HandlerOptions{Level: slog.LevelDebug})))
	defer slog.SetDefault(prev)

	fb := &fakeBackend{execCreateErr: errors.New("incus: deployment \"web\" has no instance 0")}
	srv := newTestServer(t, fb)
	defer srv.Close()
	...
	logged := buf.String()
	if !strings.Contains(logged, "WARN") || !strings.Contains(logged, "exec create") {
		t.Fatalf("a failed exec create left no WARN in the server log, so an operator investigating "+
			"a failed exec has nothing to read and a silent log cannot rule out this path.\nlog: %q", logged)
	}
	// The cause is the whole point — a warning that says only "it failed" is the
	// same dead end with more lines.
	if !strings.Contains(logged, "has no instance 0") {
		t.Errorf("the warning drops the backend's reason: %q", logged)
	}
}
...
//
// It used to also require the substring "TCP-only", which pinned a parenthetical
// that explained KUBERNETES's pods/portforward subresource to whoever hit this
// branch — including the backend that hit it by mistake (incus, which implements
// udp and had only forgotten to declare the capability). The fake backend here is
// named "fake", so the assertion was demanding an explanation about kubernetes in
// a rejection that names neither. Now the message states the fact and no cause,
// and this test asserts on the fact.
func TestPortForwardUDPRejectedByBackend(t *testing.T) {
	...
	if !strings.Contains(ack.Error, "not supported") || !strings.Contains(ack.Error, fb.Name()) {
		t.Fatalf("ack error = %q, want a UDP-unsupported rejection naming the backend", ack.Error)
	}
	if strings.Contains(ack.Error, "kubernetes") || strings.Contains(ack.Error, "TCP-only") {
		t.Errorf("ack error = %q: it explains a kubernetes limitation to a caller on the %q backend. "+
			"The rejection must state the fact and leave the cause to the backend that has it.", ack.Error, fb.Name())
	}
