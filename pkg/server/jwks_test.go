...
func TestJWKSFileVerifyAndRotate(t *testing.T) {
	...
	// A scope map, because a JWKS token's own scope claim grants nothing: these
	// keys belong to a third party, so what its tokens may reach is the
	// operator's policy to state (see pkg/authscope). This test is about key
	// SELECTION and rotation, so the map is a permissive catch-all.
	a := &authenticator{jwks: &jwksResolver{src: &jwksFileSource{path: path}}, scopeMap: catchAllScopeMap(t)}
	h := a.wrap(okHandler())
...
func TestJWKSRejectsHS256AndMismatch(t *testing.T) {
	...
	hs, err := authtoken.Issue(authtoken.IssueOptions{Subject: "x", Scope: authtoken.ScopeAPI, TTL: time.Hour, HS256Secret: []byte("0123456789abcdef0123456789abcdef")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestJWKSURLRefetchOnUnknownKid(t *testing.T) {
	...
	a := &authenticator{jwks: &jwksResolver{src: src}, scopeMap: catchAllScopeMap(t)}
	h := a.wrap(okHandler())
