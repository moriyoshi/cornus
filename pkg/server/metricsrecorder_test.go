...
// --- repeat suppression ------------------------------------------------------
//
// A backend may re-serve one reading to several polls, and the kubernetes
// backend necessarily does: metrics-server stamps each sample with the kubelet
// scrape time and republishes it unchanged for 15-30s, while the recorder polls
// every 15s. Recording both puts two datapoints at one timestamp in one series,
// which the store's PromQL engine refuses to evaluate — so ONE duplicate makes
// every query of that metric fail, over every window, while `observe status`
// reports readings recorded. Observed on a live k3s cluster on 2026-08-04.
//
// The four tests below pin the guard from both sides. Suppressing a repeat is
// only correct if a genuinely new reading still gets through, so
// TestAdvancingReadingIsRecordedEveryCycle is not a nicety: without it, a guard
// that recorded each replica exactly once forever would pass the first test.

func TestRepeatedBackendReadingIsRecordedOnce(t *testing.T) {
	src := &fakeSampler{
		list:    []api.DeployStatus{{Name: "web"}},
		samples: map[string]api.ResourceSample{"web#0": sample()},
	}
	...
	// The backend was asked each cycle — suppression is about what is WRITTEN,
	// not about polling less. A guard that stopped sampling would also stop
	// noticing when the source finally advances.
	if got := len(src.sampled()); got != 3 {
		t.Errorf("backend was sampled %d times, want 3", got)
	}
	if got := len(c.bodies); got != 1 {
		t.Errorf("%d exports reached the acceptance path, want 1: the same reading was written more than once", got)
	}
	if r.Sampled() != 1 {
		t.Errorf("Sampled() = %d, want 1", r.Sampled())
	}
	if r.Stale() != 2 {
		t.Errorf("Stale() = %d, want 2", r.Stale())
	}
	if r.Failed() != 0 || r.Dropped() != 0 {
		t.Errorf("a repeat was miscounted as a fault: Failed()=%d Dropped()=%d", r.Failed(), r.Dropped())
	}
}

func TestAdvancingReadingIsRecordedEveryCycle(t *testing.T) {
	s := sample()
	src := &fakeSampler{
		list:    []api.DeployStatus{{Name: "web"}},
		samples: map[string]api.ResourceSample{"web#0": s},
	}
	c := &collector{}
	r := newTestMetricsRecorder(src, c)
	r.collect(context.Background())

	// The source scraped again: same replica, new timestamp.
	s.Time = s.Time.Add(30 * time.Second)
	src.samples["web#0"] = s
	r.collect(context.Background())

	if got := len(c.bodies); got != 2 {
		t.Errorf("%d exports reached the acceptance path, want 2: a NEW reading was suppressed", got)
	}
	if r.Sampled() != 2 {
		t.Errorf("Sampled() = %d, want 2", r.Sampled())
	}
	if r.Stale() != 0 {
		t.Errorf("Stale() = %d, want 0: an advancing reading is not a repeat", r.Stale())
	}
}

// A backend that leaves Time zero is stamped with the wall clock by
// buildWorkloadMetrics, so each of its readings lands at a distinct time by
// construction and none of them may be suppressed. Treating the zero value as
// "the same timestamp as last cycle" would silence such a backend completely
// after its first reading.
func TestUnstampedReadingIsNeverSuppressed(t *testing.T) {
	s := sample()
	s.Time = time.Time{}
	src := &fakeSampler{
		list:    []api.DeployStatus{{Name: "web"}},
	...
	if got := len(c.bodies); got != 2 {
		t.Errorf("%d exports reached the acceptance path, want 2: an unstamped reading was suppressed", got)
	}
	if r.Stale() != 0 {
		t.Errorf("Stale() = %d, want 0", r.Stale())
	}
}

// A reading that was shed or refused was never written, so the next cycle must
// still be free to write it. Remembering it as recorded would turn one moment of
// backpressure into a permanent hole for that replica — until the source
// happened to advance.
func TestReadingIsRememberedOnlyOnceItIsIn(t *testing.T) {
	src := &fakeSampler{
		list:    []api.DeployStatus{{Name: "web"}},
		samples: map[string]api.ResourceSample{"web#0": sample()},
	}
	...
	r.collect(context.Background())
	if got := len(c.bodies); got != 1 {
		t.Errorf("%d exports reached the acceptance path after the shed cleared, want 1", got)
	}
	if r.Sampled() != 1 {
		t.Errorf("Sampled() = %d, want 1: the shed reading was never retried", r.Sampled())
	}
}

// The last-recorded index must track the LIVE workload set. Without pruning it
// grows by one entry per (name, replica) ever deployed, which on a long-lived
// server is a leak paid for by a feature nobody asked to be permanent.
func TestDepartedWorkloadReleasesItsIndexEntry(t *testing.T) {
	src := &fakeSampler{
		list:    []api.DeployStatus{{Name: "web"}, {Name: "api"}},
		samples: map[string]api.ResourceSample{"web#0": sample(), "api#0": sample()},
	}
	c := &collector{}
	r := newTestMetricsRecorder(src, c)
	r.collect(context.Background())
	if got := len(r.lastSample); got != 2 {
		t.Fatalf("lastSample holds %d entries after sampling two workloads, want 2", got)
	}

	src.list = []api.DeployStatus{{Name: "web"}}
	r.collect(context.Background())
	if _, ok := r.lastSample[sampleKey("api", 0)]; ok {
		t.Error("the removed workload's entry is still held")
	}
	if got := len(r.lastSample); got != 1 {
		t.Errorf("lastSample holds %d entries, want 1", got)
	}

	// And the last workload leaving must empty it, not stop short at the early
	// return for "no targets".
	src.list = nil
	r.collect(context.Background())
	if got := len(r.lastSample); got != 0 {
		t.Errorf("lastSample holds %d entries after every workload was removed, want 0", got)
	}
}
...
// capSampler is a fakeSampler that also declares what it cannot fill.
type capSampler struct {
	*fakeSampler
	unsupported []api.SampleField
}

func (c capSampler) UnsupportedMetrics() []api.SampleField { return c.unsupported }

// TestUnsupportedMetricsTranslatesFieldsToMetricNames pins the one place the two
// vocabularies meet. A backend describes itself in ResourceSample fields (it has
// no business knowing OTel's names), the dashboard filters on metric names, and a
// mistranslation here is silent in both directions: an unmapped field hides
// nothing, and a wrong name hides the wrong panel.
func TestUnsupportedMetricsTranslatesFieldsToMetricNames(t *testing.T) {
	src := capSampler{
		fakeSampler: &fakeSampler{list: []api.DeployStatus{{Name: "web"}}},
		unsupported: []api.SampleField{api.SampleFieldCPUTime, api.SampleFieldNetwork, api.SampleFieldDisk},
	}
	r := newMetricsRecorder(
		func(string, []byte) error { return nil },
		func() (metricsSource, error) { return src, nil },
		time.Hour,
	)
	got := r.UnsupportedMetrics()
	want := []string{metricCPUTime, metricNetIO, metricDiskIO}
	sort.Strings(got)
	sort.Strings(want)
	if !slices.Equal(got, want) {
		t.Errorf("UnsupportedMetrics = %v, want %v", got, want)
	}
}

// TestEveryFieldMapsToARecordedMetric is the guard the translation above needs.
// A field added to api without an entry here is dropped silently — the backend
// declares a gap, the map has no name for it, and the dashboard keeps offering a
// chart that can never fill. That failure is invisible without this test.
func TestEveryFieldMapsToARecordedMetric(t *testing.T) {
	all := []api.SampleField{
		api.SampleFieldCPUTime, api.SampleFieldCPUCores,
		api.SampleFieldMemUsage, api.SampleFieldMemLimit,
		api.SampleFieldPids, api.SampleFieldNetwork, api.SampleFieldDisk,
	}
	for _, f := range all {
		if metricNameForField[f] == "" {
			t.Errorf("api.SampleField %q has no metric name, so a backend declaring it unsupported is ignored", f)
		}
	}
	if len(metricNameForField) != len(all) {
		t.Errorf("metricNameForField has %d entries for %d known fields; the two lists have drifted", len(metricNameForField), len(all))
	}
}

// A backend that declares nothing is claiming nothing, which the dashboard reads
// as full support. Reporting a non-empty list here would hide panels on every
// backend that simply has not been given a declaration.
func TestBackendWithNoDeclarationReportsNothingUnsupported(t *testing.T) {
	src := &fakeSampler{list: []api.DeployStatus{{Name: "web"}}}
	r := newMetricsRecorder(
		func(string, []byte) error { return nil },
		func() (metricsSource, error) { return src, nil },
		time.Hour,
	)
	if got := r.UnsupportedMetrics(); len(got) != 0 {
		t.Errorf("UnsupportedMetrics = %v, want empty for a backend that declares nothing", got)
	}
}

// A backend with no metrics source at all records NO workload family, which is a
// stronger claim than any per-field declaration and the one that matters most:
// every workload panel on that server is permanently empty.
func TestBackendWithNoSamplerReportsEveryFamilyUnsupported(t *testing.T) {
	r := newMetricsRecorder(
		func(string, []byte) error { return nil },
		func() (metricsSource, error) { return listOnlySource{}, nil },
		time.Hour,
	...
	got := r.UnsupportedMetrics()
	if len(got) != len(metricNameForField) {
		t.Fatalf("UnsupportedMetrics = %v (%d), want all %d families", got, len(got), len(metricNameForField))
	}
	if !slices.IsSorted(got) {
		t.Errorf("UnsupportedMetrics = %v, want sorted so the status payload does not churn between reads", got)
	}
}

// No backend yet is not a claim that nothing works — it is no information at all,
// and a server still starting up must not answer with a list that would blank the
// whole dashboard.
func TestNoBackendReportsNoClaim(t *testing.T) {
	r := newMetricsRecorder(
		func(string, []byte) error { return nil },
		func() (metricsSource, error) { return nil, errors.New("no backend configured yet") },
		time.Hour,
	)
	if got := r.UnsupportedMetrics(); got != nil {
		t.Errorf("UnsupportedMetrics = %v, want nil while no backend is resolved", got)
	}
}
