...
// AllowTunnel reports whether identity may host a public tunnel — the dedicated
// "tunnel" action, which the broader "deploy" action implies. The implication is
// what keeps existing policies working: a deploy-capable identity could always
// open a tunnel before the action existed, and still can. The refinement's value
// is tunnel-ONLY identities, e.g. a CI job permitted to publish a preview URL
// without being able to apply or delete anything.
func (p *apiPolicy) AllowTunnel(identity string) bool {
	return p.Allow(identity, "tunnel") || p.Allow(identity, "deploy")
}
