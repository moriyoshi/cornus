...
// handleStorageUsage serves GET /.cornus/v1/storage: a NON-destructive disk-usage
// report — the read-only counterpart to POST /.cornus/v1/gc. It surfaces the
// current registry CAS footprint (blob count + bytes) and, when enabled, the
// block-cache footprint, so an operator can observe disk consumption without
// running a sweep. It is authn-governed (behind the auth middleware) but not gated
// on a policy action; it never mutates state.
//
// Computing CAS usage lists and Stats every blob, so it is O(blob count) in store
// round-trips — cheap on the filesystem backend, notably more expensive on S3 (see
// storage.Backend.Usage). Only GET is accepted.
func (s *Server) handleStorageUsage(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		writeJSON(w, http.StatusMethodNotAllowed, map[string]string{"error": "method not allowed"})
		return
	}
	var resp api.StorageUsage
	if s.store != nil {
		u, err := s.store.Usage(r.Context())
		if err != nil {
			writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "storage usage failed: " + err.Error()})
			return
		}
		resp.CASBlobs = u.Blobs
		resp.CASBytes = u.Bytes
	}
	if s.cfg.FileCacheEnabled {
		if b, files, err := blockcache.DiskUsage(s.cfg.FileCacheDir); err == nil {
			resp.FileCacheBytes = b
			resp.FileCacheFiles = files
		}
	}
	writeJSON(w, http.StatusOK, resp)
}
