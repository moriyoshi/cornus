...
func TestRegistryAuthzInternalBypass(t *testing.T) {
	s := &Server{apiPolicy: newAPIPolicy(map[string][]string{
		"operator": {"deploy"},
		"puller":   {"pull"},
	})}
	...
	for _, req := range []struct{ method, path string }{
		...
		{http.MethodPut, "/v2/app/manifests/latest"},
	} {
		...
		h.ServeHTTP(rec, r)
		if rec.Code != http.StatusOK {
			t.Fatalf("internal %s %s: code = %d, want 200", req.method, req.path, rec.Code)
		}
	}

	// The marker is registry-specific; it must not bypass any API handler's own
	// policy checks through this middleware.
	r := httptest.NewRequest(http.MethodPost, "/.cornus/v1/deploy", nil)
	r = r.WithContext(withInternal(r.Context()))
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, r)
	if rec.Code != http.StatusOK {
		t.Fatalf("non-registry path through registryAuthz: code = %d, want pass-through", rec.Code)
	}
}
