...
func New(cfg config.Config, st *storage.Backend) (*Server, error) {
	...
	s.newBackend = func() (deploy.Backend, error) {
		return defaultBackendFactory(cfg, s.remoteCompanions, s.host)
	}
...
func defaultBackendFactory(cfg config.Config, companions *remotecompanion.Registry, host hostSetup) (deploy.Backend, error) {
	...
	case "incus":
		...
		return incushost.New(incushost.Config{DataDir: cfg.DataDir}, incushost.WithPolicy(pol),
			incushost.WithRemote(os.Getenv("CORNUS_INCUS_REMOTE") != ""),
			incushost.WithAgentImage(os.Getenv("CORNUS_AGENT_IMAGE")),
