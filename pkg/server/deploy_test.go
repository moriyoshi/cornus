...
type fakeBackend struct {
	...
	// applyErr, when set, is returned by Apply (modelling an apply that fails — e.g.
	// a dependent PVC/Ingress create — after the workload object was created).
	applyErr error
	// actionErr, when set, is returned by Start/Stop/Restart (modelling e.g. a
	// backend's deploy.ErrNotFound for a missing name).
	actionErr error
	// execCreateErr, when set, is returned by ExecCreate — the failure mode that
	// used to leave no trace in the server log at all (see
	// TestExecCreateFailureIsLogged).
	execCreateErr error
...
func (f *fakeBackend) Apply(_ context.Context, spec api.DeploySpec) (api.DeployStatus, error) {
	...
	f.mu.Unlock()
	if err != nil {
		// Model a partial apply: the workload object exists, but a dependent step
		// failed — so the caller must still clean it up.
