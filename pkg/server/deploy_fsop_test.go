...
// TestKubeNoTarRoutesToTheCaretakerFallback closes the loop between three files
// that have to agree and are never compiled together in one test otherwise: the
// kubernetes backend's "this image has no tar" error, streamErrStatus's mapping,
// and the 501 the web BFF keys its fallback on.
//
// The chain matters because breaking it is silent. Reword the backend error and a
// distroless workload's archive failure stops being 501; the client stops falling
// back to the caretaker; and volume-backed transfers — which worked before the
// tar route existed at all — start failing, on exactly the images that most need
// the fallback. Nothing else in the tree would catch that.
func TestKubeNoTarRoutesToTheCaretakerFallback(t *testing.T) {
	err := kubernetes.ErrNoTarForTest("web-abc123")
	if got := streamErrStatus(err); got != http.StatusNotImplemented {
		t.Errorf("streamErrStatus(kubernetes no-tar) = %d, want 501 — the web BFF only falls "+
			"back to the caretaker on a 501, so any other status turns a working (slower) "+
			"transfer into a failure. Error was: %v", got, err)
	}
}
