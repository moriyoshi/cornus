...
// fakeRemoteNoAgentRelayBackend is the bare/incus shape: RemoteCapable and in
// remote mode, but explicitly declaring via AgentForwardCapable that it runs no
// caretaker AgentRelayRole. A dedicated wrapper rather than methods on
// fakeBackend, for the method-promotion reason fakeRemoteBackend documents.
type fakeRemoteNoAgentRelayBackend struct {
	fakeBackend
}

func (f *fakeRemoteNoAgentRelayBackend) Remote() bool { return true }

func (f *fakeRemoteNoAgentRelayBackend) AgentForwardEnabled(_ context.Context, _ string) (bool, error) {
	return false, nil
}

// TestAgentForwardRejectedWhenRemoteButNoAgentRelay covers the bare/incus case:
// Remote() is true because client-local mounts go through a caretaker companion,
// but no companion carries an AgentRelayRole — bare's mount-relay companion has
// none, and incus starts no companion at all.
//
// Inferring the answer from Remote() alone made the server accept the request
// and inject SSH_AUTH_SOCK pointing at a socket nothing listens on, so the
// failure only surfaced inside the container ("Error connecting to agent: No
// such file or directory"). An explicit AgentForwardCapable answer must win over
// the Remote() inference, on both the exec-create and exec-agent-channel gates.
func TestAgentForwardRejectedWhenRemoteButNoAgentRelay(t *testing.T) {
	srv := newTestServer(t, &fakeRemoteNoAgentRelayBackend{})
	defer srv.Close()
	...
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusBadRequest {
		t.Fatalf("exec-create status = %d, want 400 (the backend runs no agent relay)", resp.StatusCode)
	}
	...
	if err == nil {
		sess.Close()
		t.Fatal("exec-agent-channel upgrade succeeded; want it refused (the backend runs no agent relay)")
	}
}
