...
// opts reports the options the provider was last started with.
func (p *fakeTunnelProvider) opts() tunnel.Options {
	p.mu.Lock()
	defer p.mu.Unlock()
	return p.lastOpts
}
...
// dyingProvider hands back a session whose Accept fails immediately, standing in
// for a relay that drops on its own (provider outage, quota lapse, network gone)
// rather than one torn down by the server.
type dyingProvider struct{ url string }

func (p *dyingProvider) Start(context.Context, tunnel.Credential, tunnel.Options) (tunnel.Session, error) {
	return &dyingSession{url: p.url}, nil
}

type dyingSession struct{ url string }

func (s *dyingSession) URL() string               { return s.url }
func (s *dyingSession) Accept() (net.Conn, error) { return nil, net.ErrClosed }
func (s *dyingSession) Close() error              { return nil }

// TestTunnelStatusInactiveAfterRelayDies proves a tunnel whose relay died under
// us stops reporting Active with a URL. Handing back the URL of a dead relay
// sends the caller to an endpoint that no longer answers, which is far worse
// than saying the tunnel is down.
func TestTunnelStatusInactiveAfterRelayDies(t *testing.T) {
	fb := &fakeBackend{}
	m := &tunnelManager{name: "fake", backend: &dyingProvider{url: "https://dead.example"}, sessions: map[string]*tunnelSession{}}

	st, err := m.start(fb, "web", "tok", nil, 8080, "http")
	if err != nil {
		t.Fatalf("start: %v", err)
	}
	// start reports the tunnel it just established; the relay dies right after.
	if !st.Active {
		t.Fatal("start should report the freshly established tunnel as active")
	}

	deadline := time.Now().Add(5 * time.Second)
	for m.status("web").Active {
		if time.Now().After(deadline) {
			t.Fatal("status still reports a tunnel whose accept loop has exited")
		}
		time.Sleep(time.Millisecond)
	}
	if got := m.status("web"); got.URL != "" {
		t.Errorf("status of a dead tunnel returned URL %q; want none", got.URL)
	}
	m.closeAll()
}
