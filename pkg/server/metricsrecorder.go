...
type metricsRecorder struct {
	...
	// lastSample remembers the timestamp of the last reading RECORDED for each
	// (workload, replica), so a backend that re-serves one reading does not write
	// it twice. See sampleOne for why that is not a hypothetical.
...
// Stale reports how many readings were dropped because the backend re-served a
// timestamp already recorded. A high count next to a low Sampled means the
// recorder is polling faster than its source refreshes — resolution is bounded
// by the source, not by the interval, and the interval could be relaxed.
func (r *metricsRecorder) Stale() int64 { return r.stale.Load() }

// UnsupportedMetrics names the workload metric families this server's deploy
// backend can never record, in the recorder's own vocabulary (the dotted OTLP
// names, which the store resolves to their underscored PromQL spelling at query
// time).
//
// This is the counters' missing half. Sampled, Failed, Dropped and Stale all
// explain why a series a backend CAN produce is empty; nothing explained a series
// it will never produce at all, and the two are indistinguishable from a query
// returning nothing. The web dashboard uses this to stop offering charts that
// cannot fill — see web/src/views/metrics/catalog.ts — rather than inferring
// permanence from a run of empty results, which would hide an idle workload's
// charts too.
//
// Asked of the backend on every call rather than cached at construction: the
// backend is resolved lazily (r.source) and can be swapped on a running server,
// and this is a status route, not a hot path.
func (r *metricsRecorder) UnsupportedMetrics() []string {
	src, err := r.source()
	if err != nil {
		return nil
	}
	...
	if !ok {
		// No metrics source at all. Every workload family is unsupported here,
		// which is a stronger statement than any per-field declaration and the
		// honest one: this backend records none of them, ever.
		return allWorkloadMetricNames()
	}
	caps, ok := sampler.(deploy.MetricsCapabilities)
	if !ok {
		return nil
	}
	var out []string
	for _, f := range caps.UnsupportedMetrics() {
		if name, ok := metricNameForField[f]; ok {
			out = append(out, name)
		}
	}
	return out
...
// metricNameForField maps a neutral sample field onto the metric family
// buildWorkloadMetrics records from it. It is the only place the two vocabularies
// meet: pkg/deploy names ResourceSample fields, the store holds metric names, and
// a backend should not have to know the second to describe itself in the first.
var metricNameForField = map[api.SampleField]string{
	api.SampleFieldCPUTime:  metricCPUTime,
	api.SampleFieldCPUCores: metricCPUUsage,
	api.SampleFieldMemUsage: metricMemUsage,
	api.SampleFieldMemLimit: metricMemLimit,
	api.SampleFieldPids:     metricPids,
	api.SampleFieldNetwork:  metricNetIO,
	api.SampleFieldDisk:     metricDiskIO,
}

// allWorkloadMetricNames is every family the recorder can emit, sorted so the
// status payload does not churn between reads.
func allWorkloadMetricNames() []string {
	out := make([]string, 0, len(metricNameForField))
	for _, name := range metricNameForField {
		out = append(out, name)
	}
	sort.Strings(out)
	return out
...
func (r *metricsRecorder) collect(ctx context.Context) {
	...
	for _, st := range list {
		...
		for i := 0; i < max(1, len(st.Instances)); i++ {
			...
			live[sampleKey(st.Name, i)] = true
		}
	}
	r.replicas.Store(int64(len(targets)))
	// Before the early return, so a server whose last workload was removed also
	// releases its entries.
	r.forgetDeparted(live)
	if len(targets) == 0 {
		return
	}
...
func (r *metricsRecorder) sampleOne(ctx context.Context, sampler deploy.MetricsSampler, st api.DeployStatus, replica int) {
	...
	if r.alreadyRecorded(st.Name, replica, sample.Time) {
		r.stale.Add(1)
		return
	}
	...
	// Marked only once the reading is actually in, so a shed or failed write is
	// retried on the next cycle instead of being remembered as recorded.
	r.noteRecorded(st.Name, replica, sample.Time)
	r.sampled.Add(1)
}

// sampleKey identifies one (workload, replica) for the last-recorded index. The
// NUL separator is what keeps "a\x001" from colliding with a workload literally
// named "a\x001" — Kubernetes and Docker names cannot contain it.
func sampleKey(name string, replica int) string {
	return name + "\x00" + strconv.Itoa(replica)
}

// alreadyRecorded reports whether this exact reading has already been written.
//
// A backend is free to re-serve one reading to several polls, and the kubernetes
// backend necessarily does: its source is metrics-server, which stamps each
// sample with the KUBELET SCRAPE time and holds that value until its next scrape
// (~15-30s), while the recorder polls every ObsMetricsInterval (15s by default).
// Two polls therefore routinely observe the same reading with the same
// timestamp.
//
// Writing both is not a harmless duplicate. It puts two datapoints at one
// timestamp in one series, and the store's PromQL engine rejects such a series
// outright — so a single duplicate makes EVERY query of that metric fail with
// "duplicate timestamps in one PromQL series", not just the affected window. The
// symptom is the worst possible shape: `observe status` reports thousands of
// readings recorded while every chart and query for them is empty. Observed on a
// live k3s cluster on 2026-08-04.
//
// A zero timestamp is never stale: buildWorkloadMetrics stamps those with the
// wall clock, so each such reading lands at a distinct time by construction.
func (r *metricsRecorder) alreadyRecorded(name string, replica int, t time.Time) bool {
	if t.IsZero() {
		return false
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.lastSample[sampleKey(name, replica)].Equal(t)
}

// noteRecorded remembers the timestamp of a reading that reached the store.
func (r *metricsRecorder) noteRecorded(name string, replica int, t time.Time) {
	if t.IsZero() {
		return
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	r.lastSample[sampleKey(name, replica)] = t
}

// forgetDeparted drops last-recorded entries for targets the current cycle no
// longer covers, so the index tracks the live workload set rather than growing
// once per name ever deployed on a long-lived server.
func (r *metricsRecorder) forgetDeparted(live map[string]bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	for k := range r.lastSample {
		if !live[k] {
			delete(r.lastSample, k)
		}
	}
}
