...
//
// ctx is the connection's context; it is used for logging/trace correlation only —
// the stream's lifetime is the mux's.
func (s *Server) hubControl(ctx context.Context, hc *hubConn, mux *yamux.Session, stream net.Conn) {
	defer stream.Close()
	...
	for {
		...
		for _, svc := range reg.Services {
			...
			var err error
			if svc.Addr != "" {
				err = s.hub.Register(hc.id, svc.Name, svc.Addr, svc.Protocol)
			} else {
				err = s.hub.RegisterDeliver(hc.id, svc.Name, mux)
			}
			if err != nil {
				// ERROR, not warn: with a distributed store this is the failure that
				// makes a service silently unreachable from every OTHER replica while
				// this one keeps resolving it locally — the asymmetry an operator
				// cannot otherwise explain. The connection is deliberately NOT failed:
				// the local registration stands (relays landing here still work) and
				// the store re-drives the shared write in the background, so tearing
				// down a healthy spoke would only make the outage worse. It does not
				// count as a catalog change: nothing that peers can see happened.
				logging.FromContext(ctx).ErrorContext(ctx, "hub: service registered locally but not in the shared registry; peer replicas cannot reach it until the write lands",
					"service", svc.Name, "identity", identity, "error", err)
				continue
			}
			registered = true
		}
...
// server's legacy shared credential (forwardToken) when configured, otherwise a
// short-lived peer-scoped JWT signed by this replica. The explicit shared token
// has absolute precedence so mixed-version rollouts keep their byte-identical
// Authorization header. When auth is off no header is sent. A wss:// peer is
// verified against s.forwardTLS
...
func (s *Server) dialForward(ctx context.Context, forwardAddr, path string, lines ...string) (net.Conn, error) {
	...
	} else if s.peerKey != nil {
		token, err := s.peerForwardToken()
		if err != nil {
			return nil, err
		}
...
//
// Trust model: the auth middleware accepts a peer-scoped credential exactly on
// the three forward endpoints (a full credential remains a superset). The reach
// policy was ALREADY enforced by the peer replica before it forwarded, so we do
// not re-run it here; we trust the authenticated peer's authorization. Only
// reachable in a multi-replica deployment.
