...
func (s *Server) relayBuildPost(w http.ResponseWriter, r *http.Request, base string) {
	...
	// A relayed build is still a build this server served, so it is counted like
	// any other (see recordBuild). Every early return below is a build that failed
	// before the builder ever saw it, which is exactly an outcome=error datapoint.
	start := time.Now()
	req, err := http.NewRequestWithContext(r.Context(), http.MethodPost, out, r.Body)
	if err != nil {
		s.recordBuild(r.Context(), start, true, err)
	...
	// The builder's own status is the only outcome this server can see. Whether
	// the IMAGE built is in-band in the stream just forwarded verbatim (the
	// trailing BUILD OK / BUILD FAILED line), which is why the datapoint is
	// labelled delegated=true rather than pretending to know.
	if copyErr == nil && resp.StatusCode != http.StatusOK {
		copyErr = fmt.Errorf("builder returned %s", resp.Status)
	}
	s.recordBuild(r.Context(), start, true, copyErr)
}
...
func (s *Server) relayBuildAttach(w http.ResponseWriter, r *http.Request, upstream string) {
	// Counted like every other served build (see recordBuild). Nothing about the
	// COMPILE is knowable here — this is a byte-for-byte splice of a protocol this
	// server deliberately does not parse — so the datapoint is delegated=true and
	// its outcome is about the splice: did the caller reach a builder at all.
	start := time.Now()
	down, err := wire.AcceptConn(w, r)
	if err != nil {
		// Already hijacked on most failures; nothing useful to write back. Not
		// counted: the caller never got a session, so no build was served.
		return
	}
	defer down.Close()
	...
	wire.Pipe(down, up)
	s.recordBuild(r.Context(), start, true, nil)
}
