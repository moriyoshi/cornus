...
func (s *Server) applyWithHostAttachments(r *http.Request, sess *deploywire.ServerSession, backend deploy.Backend, can deploy.ServerDelivers) (api.DeployStatus, func(), error) {
	spec := sess.Spec.Spec
	teardown := func() {}
	// The host directories a deploy.LateIDCredentialBinder must take ownership of
	// between creating the workload's container and starting it. Empty for every
	// other backend, which has already had its files written with the right ids.
	var credDirs []string
	...
	status, err := applyOwningCredentials(r.Context(), backend, spec, credDirs)
	if err != nil {
		teardown()
		return api.DeployStatus{}, nil, err
	}
