...
func (s *Server) handleDeployAttach(w http.ResponseWriter, r *http.Request) {
	...
	default: // mounts only
		if mb, ok := backend.(deploy.MountingBackend); ok && useSidecarMounts(backend) {
			...
			// Co-located host backends: the server kernel-9P-mounts each client
			// export under <DataDir>/mounts and rewrites Mount.Source, and the
			// backend binds the mountpoint like any host path.
			//
			// bare shares the server's mount namespace outright (it runs runc as
			// the server's own child), so the mount is directly visible to the
			// container. dockerhost's daemon may instead resolve that path in the
			// HOST's namespace while the server sits in a container of its own —
			// still co-located, but not identical — which is why the mountpoint is
			// translated before Apply rather than passed through.
			status, cleanup, err = s.applyWithHostMounts(r, sess, backend)
		} else {
			sess.Finish(deploywire.Event{Err: "client-local mounts are not supported by the " + backend.Name() + " backend", Done: true})
			return
		}
	}
	if cleanup == nil {
	...
	_ = backend.Delete(context.Background(), spec.Name)
	cleanup()
	sess.Finish(deploywire.Event{Done: true})
}
...
// applyWithHostMounts path — a daemon on a DIFFERENT host cannot be detected, so
// this is never inferred.
//
// A cornus containerized on the daemon's OWN host is not that case and must not
// land here: it can still mount for itself, and does, with only the resulting
// paths translated for the runtime (hostVisibleMountSources). pkg/hostenv
// detects that shape precisely so it does not get mistaken for a remote daemon
// and saddled with a companion per instance.
...
// attachmentsDescription names the client-side attachments a session declares,
// for error messages on the shared applyWithAttachments path (which serves both
// kubernetes and, since it became an AttachingBackend, dockerhost).
func attachmentsDescription(sess *deploywire.ServerSession) string {
	var what []string
	if len(sess.Spec.LocalMounts) > 0 {
		what = append(what, "client-local mounts")
	}
	if len(sess.Spec.CredentialSources) > 0 {
	...
	if len(what) == 0 {
		return "client-side attachments"
	}
...
func (s *Server) applyWithAttachments(r *http.Request, sess *deploywire.ServerSession, backend deploy.AttachingBackend) (api.DeployStatus, func(), error) {
	...
	adv := os.Getenv("CORNUS_ADVERTISE_URL")
	if adv == "" {
		// Names the attachments actually requested: this path now serves the host
		// backends too (an egress-only deploy on dockerhost lands here since it
		// became an AttachingBackend), where "client-sourced credentials" and
		// "in-cluster pod caretaker" would both be wrong.
		return api.DeployStatus{}, nil, fmt.Errorf("%s on the %s backend require CORNUS_ADVERTISE_URL (the cornus URL the caretaker dials back on)",
			attachmentsDescription(sess), backend.Name())
	}
	id := s.mountSessionID(r.Context(), backend, sess.Spec.Spec.Name)
	cleanup := s.registerAttachSession(r.Context(), id, sess.Spec.Spec.Name, sess)
...
func (s *Server) applyWithHostMounts(r *http.Request, sess *deploywire.ServerSession, backend deploy.Backend) (api.DeployStatus, func(), error) {
	...
	// Record each mount write-ahead, so one stranded by a crash is recoverable
	// rather than an untraceable mountpoint (see pkg/activity).
	...
	rewritten, err = s.hostVisibleMountSources(rewritten)
	if err != nil {
		mm.Teardown()
		return api.DeployStatus{}, nil, err
	}
