...
func TestHubRegisterPolicy(t *testing.T) {
	...
	// The spoke's registrations are withdrawn by the server's deferred RemoveConn
	// when it notices the connection dropped — asynchronously with respect to the
	// client-side Close. Without waiting for that, the "rogue" round below can
	// still reach the PREVIOUS ("trusted") round's registration and report the
	// rogue one as accepted. The catalog is served from the same registry, so a
	// name's absence there is exactly the signal that RemoveConn has landed.
	waitCatalogEmpty := func() {
		t.Helper()
		for i := 0; i < 200; i++ {
		...
		t.Fatal("registrations were never withdrawn after the spoke disconnected")
	}
