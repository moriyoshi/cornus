...
// toggle; bare/incus' flat "never"), or backend is RemoteCapable and currently
// in remote mode (dockerhost/containerdhost, a backend-wide toggle). A backend
// implementing neither means agent-forwarding is not available; err is only
// non-nil on an unexpected backend error, not a plain "not supported" answer.
//
...
// dockerhost/containerdhost implies an always-on companion carrying
// AgentRelayRole, but for bare/incus does not: they set Remote() for the
// sidecar-mount path and run no agent relay at all. Inferring from Remote()
// alone made the server accept --forward-agent there and inject an SSH_AUTH_SOCK
// pointing at a socket nothing listens on, so the failure surfaced inside the
// container as "Error connecting to agent: No such file or directory" instead of
// as a clean API rejection.
