...
//
// handleDeployExecCreate calls it too, even though that handler CAN still answer
// with a status and does (500 with the reason in the body). The body reaches
// whoever made the request; it does not reach the server log, and an exec failure
// that leaves no trace there is invisible to everyone else — which is exactly the
// hole the incus `exec_run` flake of 2026-08-06 fell into. Its E2E failure named
// no cause and the server log was silent, so the ABSENCE of a warning could not
// be used to exclude the backend path. Every other exec failure logs; the create
...
// forward an ssh-agent: either backend implements deploy.AgentForwardCapable
// and answers for itself (kubernetes' per-deployment api.DeploySpec.AgentForward
// toggle; bare's flat "never"; incus' remote-mode answer), or backend is
// RemoteCapable and currently in remote mode (dockerhost/containerdhost, a
// backend-wide toggle). A backend implementing neither means agent-forwarding is
// not available; err is only non-nil on an unexpected backend error, not a plain
// "not supported" answer.
//
// AgentForwardCapable is consulted FIRST because it is an explicit answer and
// Remote() is only a proxy for one. Remote() means "this backend realizes
// client-local features through a caretaker companion" — which for
// dockerhost/containerdhost/incus implies an always-on companion carrying
// AgentRelayRole, but for bare does not: it sets Remote() for the sidecar-mount
// path and runs no agent relay at all. Inferring from Remote() alone made the
// server accept --forward-agent there and inject an SSH_AUTH_SOCK pointing at a
// socket nothing listens on, so the failure surfaced inside the container as
// "Error connecting to agent: No such file or directory" instead of as a clean
// API rejection.
...
const agentForwardNotAllowedMsg = "ssh-agent forwarding into exec requires either a remote-mode dockerhost/containerdhost/incus backend (CORNUS_DOCKER_REMOTE / CORNUS_CONTAINERD_REMOTE / CORNUS_INCUS_REMOTE) or, on kubernetes, a deployment applied with AgentForward set"
...
func (s *Server) handleDeployExecCreate(w http.ResponseWriter, r *http.Request, backend deploy.Backend, name string) {
	...
	if cfg.ForwardAgent {
		...
		if err != nil {
			logStreamHandlerErr(r, "exec create", name, err)
	...
	// Wrapping composes with the env injection above rather than competing with it:
	// that one adds a variable, this one re-spells the command, and the wrapper
	// inherits the environment it is handed. Order still matters for readability —
	// the env is settled before the command that will see it is rewritten.
	//
	// Refusal is silent BY DESIGN. Only a POSIX shell can be wrapped, and a session
	// that cannot announce its pid is a session with no pid, not a failure: the
	// caller falls back to whatever it did before. Erroring here would turn an
	// optional introspection nicety into a reason a terminal will not open.
	if cfg.AnnouncePID {
		if wrapped, ok := shells.WrapAnnouncePID(cfg.Cmd); ok {
			cfg.Cmd = wrapped
		}
	}
...
// its ForwardPort can bridge proto "udp" tunnels (framed datagrams). Every host
// backend implements it — dockerhost, containerd, bare and incus; kubernetes
// does not, because its pods/portforward subresource is TCP-only.
//
// Absence reads as "cannot", which is the only sane default but also means a
// backend that CAN forward datagrams and simply forgot to say so is
// indistinguishable from one that cannot. incus was in exactly that state, so
// pkg/deploy's TestEveryUDPForwardingBackendDeclaresIt now checks the two
// against each other.
...
func (s *Server) handleDeployPortForward(w http.ResponseWriter, r *http.Request, backend deploy.Backend, name string) {
	...
	if cfg.Protocol == "udp" {
		...
		if u, ok := backend.(udpPortForwarder); !ok || !u.SupportsUDPPortForward() {
			// No cause is stated: the reason is the backend's, not this handler's,
			// and the parenthetical that used to be here explained KUBERNETES's
			// TCP-only subresource to whoever hit the branch — which was wrong for
			// the one backend that actually reached it by mistake (incus, whose
			// ForwardPort implements udp but did not declare it).
			ack.Error = fmt.Sprintf("UDP port-forward is not supported by the %s backend; only tcp tunnels can be forwarded there", backend.Name())
		}
