...
type catalogNotifier struct {
	...
	stop chan struct{} // closes the poll loop; nil while no subscriber exists
}
...
// run is the recheck loop: on a kick or a poll tick it reads the catalog and,
// when its hash moved, delivers the new snapshot to every subscriber.
func (n *catalogNotifier) run(stop chan struct{}) {
