...
func (s *Server) handleBuildAttach(w http.ResponseWriter, r *http.Request) {
	...
	// Recorded HERE and not only on POST /.cornus/v1/build, because this is the path
	// the CLI actually takes: `cornus build` and `cornus compose build` go through
	// buildwire, so a server whose only builds arrived this way reported
	// cornus.builds flat at zero while building all day. Same instruments and the
	// same `outcome` attribute as the tar-upload path, so the two routes accumulate
	// into one series rather than into two spellings of the same fact.
	//
	// After loadWait on purpose: in docker-daemon mode the image is not usable until
	// the load finishes, so a build whose load failed is an "error" here, and the
	// duration is the whole wait a caller experienced.
	s.recordBuild(r.Context(), start, false, buildErr)
