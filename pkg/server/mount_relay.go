...
//
// The error is returned rather than swallowed because a routing record that never
// lands has a specific, confusing symptom: a pod scheduled anywhere but this replica
// gets its mount stream reset and the operator sees only "no replica currently owns
// this mount session" on a session that is very much alive here (logMountReset).
// The caller logs it; it is not fatal to the attach, since pods landing on this
// replica mount fine and the store re-drives the write.
func (s *Server) registerMountSession(id string) error {
	if !s.hubDistributed() {
		return nil
	}
	name := mountServiceName(id)
	return s.hub.RegisterDeliver(name, name, nil)
}
...
// dies with the owner's liveness key (Redis TTL) or Lease/Pod ownerRef (kube) — but
// a delete that FAILS while this replica lives on does need cleanup, so the error is
// returned (the store queues the retry; the caller logs).
func (s *Server) unregisterMountSession(id string) error {
	if !s.hubDistributed() {
		return nil
	}
	return s.hub.RemoveConn(mountServiceName(id))
}
...
//
// Trust model: identical to /.cornus/v1/hub/forward. The auth middleware accepts
// a peer-scoped credential exactly on the forward endpoints (a full credential
// remains a superset); the session id remains the per-mount capability, checked
// here against the live session it names.
