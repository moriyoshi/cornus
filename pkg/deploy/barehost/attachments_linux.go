...
func (b *Backend) ApplyWithAttachments(ctx context.Context, spec api.DeploySpec, mounts []deploy.AttachMount, creds []deploy.AttachCredential, egress *deploy.AttachEgress) (api.DeployStatus, error) {
	appSpec, err := deploy.RealizeCredentials(spec, creds, "bare",
		"the server realizes env, file and endpoint deliveries itself when it runs the workload as its own child; a delivery arriving here could not be realized that way — in practice remote mode — and this backend starts no caretaker to serve it instead")
	if err != nil {
