...
func (b *Backend) firstRunningInstance(ctx context.Context, name string) (*instanceRecord, error) {
	return b.runningInstanceAt(ctx, name, 0)
}

// runningInstanceAt resolves a deployment's idx-th instance (0-based) and
// requires it to be running.
//
// recordsForApp sorts by the recorded Replica ordinal, so the index IS the
// replica number rather than whatever order the record directories happened to
// be read in. An out-of-range index is ErrNotFound, never a fallback to the
// first: a caller enumerating replicas has to be able to tell a departed one
// from a different one's numbers.
func (b *Backend) runningInstanceAt(ctx context.Context, name string, idx int) (*instanceRecord, error) {
	...
	if idx < 0 || idx >= len(recs) {
		return nil, fmt.Errorf("bare: deployment %q has no instance %d (%d known): %w", name, idx, len(recs), deploy.ErrNotFound)
	}
	rec := recs[idx]
