...
func (s *supervisor) onExit(ctx context.Context, id string, ranFor time.Duration) {
	...
	// Claim the attempt against a FRESH read under the per-record lock. restartInstance
	// runs the OCI runtime and can take seconds; a Stop or Delete that landed in that
	// window must not be clobbered by writing back the record read before it. The same
	// locked read re-checks the intent, so a stop detected here undoes the resurrection
	// instead of leaving a container running that the record says is stopped.
	updated, uerr := s.b.updateRecord(id, func(r *instanceRecord) error {
		if !r.DesiredRunning || r.ExplicitlyStopped {
			return errRecordUnchanged
		}
		r.RestartCount++
		return nil
	})
	switch {
	case errors.Is(uerr, errRecordUnchanged) || errors.Is(uerr, fs.ErrNotExist):
		log.Info("restart raced a stop or delete; stopping the instance again")
		s.unwatch(id)
		s.b.stopInstance(ctx, id)
		return
	case uerr != nil:
		log.Warn("persist restart count failed", "error", uerr)
	default:
		rec = updated
	}
...
func (b *Backend) reconcile() {
	...
	for _, rec := range recs {
		...
		// Companions run in a SECOND pass: one joins its app instance's netns, so
		// it can only be repointed once that app's fresh pin exists.
		...
		// Re-arm health probing. The engine's STATE is in-memory and legitimately
		// resets across a server restart (Docker keeps its own in a daemon that
		// outlives cornus), but the probing itself must come back: the healthcheck
		// is persisted on the record precisely so it can be re-read here. Without
		// this the workload would report no health until someone redeployed it —
		// silent, because it keeps running, and surfacing much later as a compose
		// `depends_on: service_healthy` that hangs. Instances this pass leaves
		// stopped are skipped above, matching Stop's own unwatch.
	...
	b.reconcileCompanions(ctx, recs, log, hostrun.NetnsAlive)
}

// reconcileCompanions is reconcile's second pass, over companion caretakers only.
// It runs after every app instance has been recovered and relaunched, because a
// companion joins its app's pinned netns: repointing it any earlier would name a
// namespace the app pass is about to replace.
//
// The reboot decision is per ROLE, not per container (companionRecoverable): a
// host reboot severs the client connections the mount and egress caretakers relay
// through, so those are left stopped rather than resurrected into a state where
// they cannot serve. Their DesiredRunning is cleared so the supervisor stops
// retrying them; the app instance itself is untouched and keeps running.
// alive is the netns liveness probe (hostrun.NetnsAlive in production; injected
// by tests), matching needsRebootRecovery's shape.
func (b *Backend) reconcileCompanions(ctx context.Context, recs []*instanceRecord, log *slog.Logger, alive func(string) bool) {
	for _, rec := range recs {
		if !isCompanionRec(rec) || !rec.DesiredRunning || rec.ExplicitlyStopped {
			continue
		}
		running := false
		if st, serr := b.rt.State(ctx, rec.ID); serr == nil && st.Status == runcStateRunning && st.Pid > 0 {
		...
		if !running {
			app := appInstanceFor(recs, rec)
			needs, err := companionNeedsRebootRecovery(rec, app, alive)
			if err != nil {
				log.Warn("reconcile companion bundle unreadable", "companion", rec.ID, "error", err)
				continue
			}
			if needs {
				if !companionRecoverable(rec.Role) {
					if _, uerr := b.updateRecord(rec.ID, func(r *instanceRecord) error {
						r.DesiredRunning = false
						return nil
					}); uerr != nil {
						log.Warn("reconcile companion stand-down failed", "companion", rec.ID, "error", uerr)
					}
					log.Info("reconcile left companion stopped after host reboot (its client peer is gone)",
						"companion", rec.ID, "role", rec.Role)
					continue
				}
				if err := b.recoverCompanion(ctx, rec, app); err != nil {
					log.Warn("reconcile companion reboot recovery failed", "companion", rec.ID, "error", err)
					continue
				}
				log.Info("reconcile recovered companion after host reboot",
					"companion", rec.ID, "role", rec.Role, "netns", app.NetNS)
			}
		}
		if err := b.launchSupervised(ctx, rec); err != nil {
			log.Warn("reconcile companion relaunch failed", "companion", rec.ID, "error", err)
		}
	}
}
