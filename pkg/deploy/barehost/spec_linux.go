...
func warnUnsupported(ctx context.Context, log *slog.Logger, spec api.DeploySpec) {
	...
	if spec.Credentials != nil {
		...
		log.WarnContext(ctx, "bare backend ignores credentials: minting a client-sourced credential needs a live deploy-attach session, and a stateless apply has none, so nothing is injected and the workload sees none of the declared credentials")
	}
