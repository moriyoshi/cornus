...
// containerdhost; and Telemetry/Egress/Mounts are realized outside this package.
// None of those is a silent drop, so none of them warns.
//
// Credentials is the one field that is BOTH: the attach path realizes its env
// deliveries and clears the block, while a stateless apply cannot mint anything
// and must say so. It has a row because this table drives the STATELESS path.
var unsupportedFieldCases = []struct {
	...
	// A STATELESS apply cannot mint a client-sourced credential — there is no
	// session behind it. The attach path is a different story: it realizes env
	// deliveries and clears the block, so this warning cannot fire there.
...
func supportedSpec() api.DeploySpec {
	return api.DeploySpec{
		...
		// Honoured since the probe engine landed (health_linux.go): persisted on the
		// instance record and probed by cornus, so apply must stay SILENT about it.
		...
		// Credentials is deliberately NOT here: on the stateless path this
		// backend cannot mint one, and it now says so. Listing it as fully
		// honored is what let the silent drop pass for as long as it did.
	}
}
