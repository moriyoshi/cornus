...
type caretakerConfig struct {
	...
	// TelemetryRelay carries the Collector's exports over a caretaker connection
	// to the server instead of a direct dial. Mirrors caretaker.TelemetryRelayRole
	// field-for-field, for the same reason Otel mirrors otelcollector.Config: the
	// JSON wire form must match caretaker.Config without importing pkg/caretaker.
	TelemetryRelay *caretakerTelemetryRelayRole `json:"telemetryRelay,omitempty"`
	Mark           int                          `json:"mark,omitempty"`
	Token          string                       `json:"token,omitempty"`
}

// caretakerTelemetryRelayRole mirrors caretaker.TelemetryRelayRole.
type caretakerTelemetryRelayRole struct {
...
type companionSpec struct {
	...
	extraEnv   map[string]string // extra container env (e.g. the app's egress proxy vars)
...
func (b *Backend) startCompanion(ctx context.Context, cs companionSpec) (retErr error) {
	...
	compSpec := api.DeploySpec{
		...
		Env:        companionEnv(string(raw), cs.extraEnv),
...
// companionEnv builds a companion's container env: its caretaker config plus any
// extra variables the caller supplies (the app's egress proxy vars, so a
// companion sharing the workload's netns honours the same "proxy"-mode policy).
func companionEnv(cfgJSON string, extra map[string]string) map[string]string {
	env := make(map[string]string, len(extra)+1)
	for k, v := range extra {
		env[k] = v
	}
	env["CORNUS_CARETAKER_CONFIG"] = cfgJSON
	return env
}
