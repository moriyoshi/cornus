...
// backend's only companion is the mount-relay one ApplyWithMounts starts, which
// carries no caretaker AgentRelayRole (and no PortForwardRole), so there is
// nothing listening on the SSH_AUTH_SOCK path an exec would be given. Declaring
// this explicitly stops the server inferring the feature from Remote() and
// letting the failure surface inside the container instead of at the API.
//
// Implementing the always-on companion here would make this a real answer; see
