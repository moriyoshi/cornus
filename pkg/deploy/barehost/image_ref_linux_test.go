...
func TestImageResolverConsultsRegistryCredentialsForExactRef(t *testing.T) {
	var gotRef string
	s := &imageStore{
		insecure: map[string]bool{},
		creds: func(_ context.Context, ref string) (deploy.RegistryCredential, bool, error) {
			gotRef = ref
			return deploy.RegistryCredential{Username: "cornus-internal", Password: "pull-token"}, true, nil
		},
	}
	if _, err := s.resolver(context.Background(), "localhost:5000/app:v1"); err != nil {
		t.Fatal(err)
	}
	if gotRef != "localhost:5000/app:v1" {
		t.Fatalf("credential resolver ref = %q", gotRef)
	}

	wantErr := errors.New("mint failed")
	s.creds = func(context.Context, string) (deploy.RegistryCredential, bool, error) {
		return deploy.RegistryCredential{}, false, wantErr
	}
	if _, err := s.resolver(context.Background(), "localhost:5000/app:v1"); !errors.Is(err, wantErr) {
...
func TestNewImageStoreRejectsAnUnknownSnapshotter(t *testing.T) {
	_, err := newImageStore(t.TempDir(), "btrfs", nil)
	if err == nil {
...
func TestNewImageStoreHonorsTheNativeEscapeHatch(t *testing.T) {
	s, err := newImageStore(t.TempDir(), "native", nil)
	if err != nil {
