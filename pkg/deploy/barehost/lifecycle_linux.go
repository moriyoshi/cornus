...
func (b *Backend) createInstance(ctx context.Context, spec api.DeploySpec, pulled pulledImage, replica int, hooks applyHooks) (retErr error) {
	...
	rec := &instanceRecord{
		...
		Healthcheck:    healthcheckToRecord(spec.Healthcheck),
...
func (b *Backend) Delete(ctx context.Context, name string) error {
	// Disarm while the records still exist: unwatchHealth reads them for the ids.
...
func (b *Backend) Stop(ctx context.Context, name string) error {
	// Disarm first: a probe racing the teardown would exec into an init being
	// killed and grade a requested stop as a health failure.
	b.unwatchHealth(ctx, name)
	recs, err := b.recordsForApp(name)
	if err != nil {
		return err
	...
	for _, rec := range recs {
		...
		// resurrect it; the startup reconcile honors it too. The amendment goes
		// through the per-record lock so a supervisor (in-process or shim) mid
		// read-modify-write cannot write the pre-stop record back over it.
		_, _ = b.updateRecord(rec.ID, func(r *instanceRecord) error {
			r.DesiredRunning = false
			r.ExplicitlyStopped = true
			return nil
		})
		// Outside the lock: stopSupervised blocks on the shim's control socket and
		// a SIGTERM grace period, and the shim needs the record lock to observe the
		// stop it is being told about.
...
func (b *Backend) Start(ctx context.Context, name string) error {
	...
	for _, rec := range recs {
		...
		// the record the shim reads reflects the desired-running intent. Under the
		// per-record lock, and re-read: a supervisor may have bumped the tally or
		// stamped an exit since recordsForApp listed them.
		fresh, err := b.updateRecord(rec.ID, func(r *instanceRecord) error {
			r.DesiredRunning = true
			r.ExplicitlyStopped = false
			r.RestartCount = 0
			return nil
		})
		if err != nil {
		...
		// the in-process supervisor per CORNUS_BARE_SHIM. Outside the lock: it runs
		// the runtime and waits on the shim's control socket.
		if err := b.launchSupervised(ctx, fresh); err != nil {
			return err
		}
	}
	// Re-arm probing: the instances were stopped (hence unwatched), and the
	// healthcheck comes back off their records, which is what makes this work
...
func (b *Backend) instanceStatus(ctx context.Context, rec *instanceRecord) api.InstanceStatus {
	is := api.InstanceStatus{ID: rec.ID, State: "exited", Health: b.health.State(rec.ID)}
...
// BindsCredentialDir implements deploy.CredentialBinder. This backend runs runc
// as the server's own child, so it shares the server's mount namespace outright
// and a path the server wrote is visible to the workload with no translation at
// all — the strongest form of the capability. False in remote mode, where the
// companion, not this process, would have to reach the path.
