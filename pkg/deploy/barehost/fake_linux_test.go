...
type fakeRuntime struct {
	...
	// createHook, when set, runs at the START of Create, before the fake takes its
	// own lock, so a test can land another backend operation in the middle of an
	// in-flight (re)launch — the window the record-lock races live in. Running it
	// outside the lock is what lets the hook call back into the fake.
	createHook func(id string)
}

// setCreateHook installs the Create interleaving hook.
func (f *fakeRuntime) setCreateHook(fn func(id string)) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.createHook = fn
}
