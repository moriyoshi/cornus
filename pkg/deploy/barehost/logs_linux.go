...
func (b *Backend) Logs(ctx context.Context, name string, opts api.LogOptions, w io.Writer) error {
	...
	// recordsForApp is sorted by Replica, so the index IS the replica ordinal.
	if opts.Instance < 0 || opts.Instance >= len(recs) {
		return fmt.Errorf("bare: deployment %q has no instance %d (%d running): %w", name, opts.Instance, len(recs), deploy.ErrNotFound)
	}
	return streamRawLog(ctx, recs[opts.Instance].LogPath, opts, w)
}
...
// n < 0 keeps the whole file; n == 0 seeks to EOF so no existing content is
// replayed, which is docker's `--tail 0` semantics and what Attach relies on to
// honor Logs:false (it passes Tail:"0" to mean "no history"). Treating 0 as
// "whole file" made `attach` without logs replay the entire log. It reads the
// file to find the boundary; M1 logs are small, so a full read is acceptable
// (the logfmt reader in M4 tails by record).
func seekToLastLines(f *os.File, n int) error {
	...
	if n == 0 {
		if _, err := f.Seek(0, io.SeekEnd); err != nil {
			return fmt.Errorf("bare: seek log to end: %w", err)
		}
		return nil
	}
