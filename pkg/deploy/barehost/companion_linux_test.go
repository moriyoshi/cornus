...
// backend's companion actually does.
//
// Remote() is true in CORNUS_BARE_REMOTE mode because client-local mounts are
// realized through a companion — but that companion carries only mount roles
// (startMountCompanion), never a caretaker AgentRelayRole, and it only exists at
// all when ApplyWithMounts runs. The server used to infer agent-forward support
// from Remote() alone and inject an SSH_AUTH_SOCK pointing at a socket nothing
// listens on, so `ssh` inside an exec failed with "Error connecting to agent: No
// such file or directory" instead of the request being refused up front.
// Declaring deploy.AgentForwardCapable makes the answer explicit.
//
// If an always-on companion carrying AgentRelayRole ever lands here (see
// .agents/docs/JOURNAL.md, "unified companion caretaker", 2026-07-25), this test
// should start failing — that is the point.
func TestAgentForwardNotOffered(t *testing.T) {
	b := &Backend{remote: true}
	if !b.Remote() {
		t.Fatal("Remote() = false, want true (the fixture sets remote)")
	}
	enabled, err := b.AgentForwardEnabled(context.Background(), "web")
	if err != nil {
		t.Fatalf("AgentForwardEnabled: %v", err)
	}
	if enabled {
		t.Error("AgentForwardEnabled = true, but no companion carries an AgentRelayRole")
	}
}
