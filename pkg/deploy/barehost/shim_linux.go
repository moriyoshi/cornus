...
func (s *shim) run() (retErr error) {
	...
	for {
		...
		// Stamp the observed exit onto a FRESH read under the per-record lock. A
		// Stop/Delete may have landed while the init was dying, so the shim amends
		// only the two fields it owns instead of writing back the whole record it
		// read a container-lifetime ago — writing that stale copy is what used to
		// resurrect an explicitly-stopped workload (recordlock_linux.go).
		rec, err = s.updateRecord(func(r *instanceRecord) error {
			r.LastExitCode = code
		...
		// Restart per policy after capped backoff, then CLAIM the attempt: the same
		// locked read that bumps the tally re-checks the desired state, so a Stop
		// that raced the backoff aborts the restart rather than being overwritten
		// by it (errRecordUnchanged), and a Delete surfaces as a missing record.
		time.Sleep(backoffFor(rec.RestartCount))
		if _, err := s.updateRecord(func(r *instanceRecord) error {
			if s.stopping.Load() || !r.DesiredRunning || r.ExplicitlyStopped {
...
func (s *shim) readRecord() (*instanceRecord, error) { return readRecordFile(s.recordDir) }

// writeRecord publishes a whole record. It is only for the round-trip seam the
// tests pin — the supervise loop amends records through updateRecord, so a stale
// in-memory copy can never be written back over the server's intent.
func (s *shim) writeRecord(rec *instanceRecord) error {
	lk, err := lockRecordDir(s.recordDir)
	if err != nil {
		return err
	}
	defer lk.release()
	return writeRecordFile(s.recordDir, rec, shimRecordTmpName)
}

// updateRecord read-modify-writes the shim's record under the per-record lock
// shared with the server (recordlock_linux.go). mutate must not block: the lock
// is held while it runs, and the server takes the same lock from its Stop/Start/
// Delete paths.
func (s *shim) updateRecord(mutate func(*instanceRecord) error) (*instanceRecord, error) {
	return updateRecordAt(s.recordDir, shimRecordTmpName, mutate)
}
