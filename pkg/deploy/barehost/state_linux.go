...
type instanceRecord struct {
	...
	// Healthcheck is the workload's health probe, persisted so the probe engine
	// can re-arm after a SERVER restart. The probe STATE legitimately resets then
	// (Docker keeps its own in the daemon, which outlives cornus), but losing the
	// DEFINITION would mean health never came back for an already-deployed
	// workload — a silent regression that only surfaces much later, as a compose
	// dependency that hangs. Persisted structurally for the same reason Origin is:
	// this backend has no daemon label store to hide it in.
	Healthcheck *api.Healthcheck `json:"healthcheck,omitempty"`
