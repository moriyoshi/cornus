...
func newImageStore(dataDir, snapPref string, creds deploy.RegistryCredentials) (*imageStore, error) {
...
func (s *imageStore) resolver(ctx context.Context, ref string) (remotes.Resolver, error) {
	...
	if s.creds != nil {
		credential, ok, err := s.creds(ctx, ref)
		if err != nil {
			return nil, err
		}
		if !ok {
...
func (s *imageStore) pull(ctx context.Context, ref string) (pulledImage, error) {
	...
	res, err := s.resolver(ctx, ref)
	if err != nil {
		return pulledImage{}, fmt.Errorf("bare: resolve registry credential for %s: %w", ref, err)
	}
