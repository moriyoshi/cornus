...
//
// CPUCores always, because the cgroup and runtime sources are both cumulative.
// Network only when this backend runs a gVisor sandbox: the guest's netstack is
// invisible from the host and sampleRuntime therefore leaves Networks empty, for
// every workload, permanently — which is what makes it a backend property rather
// than a per-reading gap. An unsandboxed bare backend reads /proc/PID/net/dev and
// reports network normally.
func (b *Backend) UnsupportedMetrics() []api.SampleField {
	out := []api.SampleField{api.SampleFieldCPUCores}
	if b.sandboxed {
		out = append(out, api.SampleFieldNetwork)
	}
	return out
}

// Stats streams Docker-format stats JSON for the deployment's opts.Instance-th
// replica: one object then EOF when opts.Stream is false, else one per second.
func (b *Backend) Stats(ctx context.Context, name string, opts api.StatsOptions, w io.Writer) error {
	rec, err := b.runningInstanceAt(ctx, name, opts.Instance)
	if err != nil {
		return err
	}
	return hostrun.StreamStats(ctx, w, rec.ID, name, opts.Stream, b.sampler(ctx, rec.ID))
}
...
func (b *Backend) SampleMetrics(ctx context.Context, name string, instance int) (api.ResourceSample, error) {
	rec, err := b.runningInstanceAt(ctx, name, instance)
	if err != nil {
		return api.ResourceSample{}, err
	}
	s, err := b.sampler(ctx, rec.ID)()
	if err != nil {
		return api.ResourceSample{}, err
	}
...
// sampler returns the per-read closure both Stats and SampleMetrics use, so the
// two can never end up reading different cgroup files for the same container.
func (b *Backend) sampler(ctx context.Context, id string) func() (hostrun.StatsSample, error) {
	...
	return func() (hostrun.StatsSample, error) {
		if b.sandboxed {
			...
			return b.sampleRuntime(ctx, id)
		}
		return b.sampleCgroup(ctx, id, unified)
	}
}
