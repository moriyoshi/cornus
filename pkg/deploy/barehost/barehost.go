...
// RuncStateRoot is the go-runc state root (--root) the bare backend hands the
// OCI runtime. It lives on tmpfs under /run so a host reboot wipes the runtime's
// "running" view — exactly the semantic the reboot-recovery reconcile relies on,
// paralleling the tmpfs netns pins under /run/cornus/netns.
//
// Exported because it is observable from outside the backend: anything that
// inspects or cleans up the bare backend's containers must pass the same --root
// to the runtime, or it addresses a different (empty) state directory and
// silently sees nothing. The E2E harness's bare Teardown does exactly that. This
// file carries no build tag, so the constant is importable on every platform
// even though the implementation that uses it is linux-only.
...
// this backend's companion caretakers — the telemetry collector companion, and
// (with WithRemote(true)) the mount-relay companion, which sources its own image
// from the deploy.AttachMount instead when one is supplied.
//
// NOTE: unlike dockerhost/containerdhost, this backend has no ALWAYS-ON remote
// companion; see WithRemote.
...
// registry (pkg/remotecompanion) so ForwardPort could reroute through a
// remote-mode instance's companion.
//
// NOTE: currently accepted and stored but NOT read. This backend runs no
// companion carrying a caretaker PortForwardRole, and ForwardPort dials the
// instance's own IP directly — which is correct here, since a daemonless backend
// is always co-located with the server. It is wired for parity with
// dockerhost/containerdhost and for the unified companion described in
// .agents/docs/JOURNAL.md, "unified companion caretaker" (2026-07-25).
