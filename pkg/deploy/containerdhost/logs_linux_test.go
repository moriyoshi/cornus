...
// Both halves of the log URI are consumed by a containerd shim running on the
// HOST: it execs the binary and opens the log file there. A containerized
// cornus that emitted its own spelling of either would have every task fail its
// IO setup against paths that do not exist on that side.
func TestLogURITranslatesForAContainerizedServer(t *testing.T) {
	dir := t.TempDir()
	b := &Backend{dataDir: dir, mapper: tableMapper{dir: "/srv/cornus"}}
	uri, err := b.logURI("abc")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got, want := u.Query().Get(logShimArg), "/srv/cornus/containerd/logs/abc.log"; got != want {
		t.Errorf("log path in URI = %q, want the host spelling %q", got, want)
	}
	// The binary is staged into the (host-visible) data dir, because the test
	// executable's own path is not host-visible under this mapping.
	if !strings.HasPrefix(u.Path, "/srv/cornus/containerd/bin/cornus-") {
		t.Errorf("shim path = %q, want a staged binary under the host-visible data dir", u.Path)
	}
}

// TestLogRotationIsNeverPerformedOutsideTaskStart pins the documented rotation
// bound (see the rotation notes in logs_linux.go).
//
// Two things are asserted, and the second is the load-bearing one:
//
//   - the residual is real: with a task running, an over-cap live log stays
//     over-cap. Nothing in the read or reconcile paths trims it. This is the
//     limitation, written down as an executable fact so it cannot be quietly
//     misdescribed;
//   - and it must STAY that way until the shim itself learns to rotate. The
//     obvious "fix" — rotating from Logs, Status, List, or the reconcile pass —
//     would not shrink anything at all, because the running shim holds the fd
//     and would go on appending to the renamed inode. Every subsequent line of
//     a live workload would vanish. A test that fails when someone adds that
//     rotation is worth more than one that merely records the byte count.
func TestLogRotationIsNeverPerformedOutsideTaskStart(t *testing.T) {
	t.Setenv(logMaxBytesEnv, "10")
	f := newFakeClient()
	b, _ := newTestBackend(t, f)
	...
	// A live, over-cap log, exactly as a long-running chatty task produces it.
	path := b.logPath("cornus-web-0")
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		t.Fatal(err)
	}
	oversized := []byte(`{"t":"2024-01-01T00:00:00Z","s":"stdout","m":"far past the ten byte cap\n"}` + "\n")
	if err := os.WriteFile(path, oversized, 0o644); err != nil {
		t.Fatal(err)
	}

	// Every surface that reads or sweeps while the task is up.
	if err := b.Logs(ctx, "web", api.LogOptions{}, io.Discard); err != nil {
		t.Fatalf("Logs: %v", err)
	}
	if _, err := b.Status(ctx, "web"); err != nil {
	...
	if err != nil {
		t.Fatalf("live log after read/sweep passes: %v", err)
	}
	if !bytes.Equal(got, oversized) {
		t.Errorf("live log was modified while a shim holds it open (%d -> %d bytes).\n"+
			"Rotating or truncating outside startTask does not reclaim anything — the shim keeps "+
			"appending to the old inode — and silently discards every later line.", len(oversized), len(got))
	}
	if _, err := os.Stat(rotatedLogPath(path)); !os.IsNotExist(err) {
		t.Error("a rotated generation appeared without a task restart; rotation is only safe in startTask")
	}

	// The bound IS enforced at the next cornus-driven restart, and exactly one
	// old generation survives it: never a .2, so the retired half of the
	// footprint stays capped however many times this repeats.
	for i := 0; i < 2; i++ {
		...
		if _, err := os.Stat(rotatedLogPath(path)); err != nil {
			t.Fatalf("restart %d did not rotate the over-cap log: %v", i, err)
		}
		if _, err := os.Stat(rotatedLogPath(path) + ".1"); !os.IsNotExist(err) {
			t.Fatalf("restart %d kept more than one old generation", i)
		}
	}
	...
	if len(entries) != 1 {
		...
		t.Errorf("log dir = %v, want only the single retained generation cornus-web-0.log.1", names)
	}
}

// TestDeleteRemovesInstanceLogFiles pins the retention half of the log
// lifecycle: once a deployment is deleted its container is gone, so nothing can
// ever reach <id>.log / <id>.log.1 again. Leaving them behind grew the log dir
// without bound across deploy/delete cycles.
func TestDeleteRemovesInstanceLogFiles(t *testing.T) {
	f := newFakeClient()
	b, _ := newTestBackend(t, f)
	ctx := context.Background()
	...
	for _, p := range []string{path, rotatedLogPath(path)} {
		if _, err := os.Stat(p); !os.IsNotExist(err) {
			t.Errorf("%s survived Delete (err=%v); deleted deployments must not leak log files", filepath.Base(p), err)
		}
	}
	entries, err := os.ReadDir(filepath.Dir(path))
	if err != nil {
	...
	if len(entries) != 0 {
		t.Errorf("log dir not empty after Delete: %v", entries)
	}
}

// TestDeleteSucceedsWhenLogFilesAreAbsent covers the two ways log reaping can
// find nothing to do — no files at all, and only the rotated generation left —
// and the unlinkable case. None may fail the delete: a half-removed deployment
// is strictly worse than a leaked file.
func TestDeleteSucceedsWhenLogFilesAreAbsent(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("no log files", func(t *testing.T) {
		...
		// Not even the logs directory exists (the shim, which creates it, never
		// ran here).
		if err := b.Delete(ctx, "web"); err != nil {
			t.Fatalf("Delete with no log files: %v", err)
		}
	...
	t.Run("only the rotated generation", func(t *testing.T) {
		f := newFakeClient()
		b, _ := newTestBackend(t, f)
		if _, err := b.Apply(ctx, api.DeploySpec{Name: "web", Image: "img"}); err != nil {
			t.Fatalf("Apply: %v", err)
		...
		if err := os.WriteFile(rotatedLogPath(path), []byte("old\n"), 0o644); err != nil {
			t.Fatal(err)
		}
		if err := b.Delete(ctx, "web"); err != nil {
			t.Fatalf("Delete with a missing live log: %v", err)
		}
		if _, err := os.Stat(rotatedLogPath(path)); !os.IsNotExist(err) {
			t.Error("rotated generation survived Delete")
		}
	})

	t.Run("unlinkable", func(t *testing.T) {
		if os.Geteuid() == 0 {
			t.Skip("root ignores directory write permission, so the file cannot be made unlinkable")
		}
		f := newFakeClient()
		b, _ := newTestBackend(t, f)
		if _, err := b.Apply(ctx, api.DeploySpec{Name: "web", Image: "img"}); err != nil {
		...
		if err := b.Delete(ctx, "web"); err != nil {
			t.Fatalf("Delete must not fail on an unremovable log file: %v", err)
		}
		if len(f.containers) != 0 {
			t.Fatalf("containers left: %v", f.containers)
		}
