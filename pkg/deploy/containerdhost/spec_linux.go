...
func warnUnsupported(ctx context.Context, log *slog.Logger, spec api.DeploySpec) {
	...
	if spec.Credentials != nil {
		...
		// credentials with no client session behind them to mint from. dockerhost
		// has said so for a while; this backend dropped them in silence, which is
...
func containerLabels(spec api.DeploySpec, att hostrun.Attachment, ports []api.PortMapping, logURI string) (map[string]string, error) {
	...
	if hc := spec.Healthcheck; hc != nil && !hc.Disabled() && len(hc.Test) > 0 {
		// Recorded on the container so the probe engine can re-arm after a server
		// restart, when the spec is no longer in hand (health_linux.go).
		data, err := json.Marshal(hc)
		if err != nil {
			return nil, fmt.Errorf("containerd: encode healthcheck label: %w", err)
		}
		l[labelHealthcheck] = string(data)
	}
