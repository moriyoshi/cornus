...
func (b *Backend) apply(ctx context.Context, spec api.DeploySpec, extraMountsFor func(replica int) []specs.Mount, planFor func(replica int) companionPlan) (api.DeployStatus, error) {
	...
	// Every field this backend cannot honor is warned about here, one line each,
	// before anything is created (spec_linux.go).
	...
	withheld, err := b.deleteInstances(ctx, spec.Name)
	if err != nil {
		return api.DeployStatus{}, err
	}
	// The recreate could not clear an instance because it IS this cornus server
	// (self_linux.go). Stop here rather than pressing on: the create below would
	// fail on the surviving container's id with a bare "already exists", and a
	// recreate that did succeed would have killed the process serving the
	// request. Recreating the server is an operation from outside the server.
	if len(withheld) > 0 {
		return api.DeployStatus{}, fmt.Errorf("containerd: refusing to recreate deployment %q: %s is this cornus server's own container; redeploy it from outside this server", spec.Name, strings.Join(withheld, ", "))
	}
	...
	// ONE companion per replica carries every role that replica needs (see
	// startCompanion). It is started whenever the plan contributes a role, and
	// unconditionally in remote mode — where it also carries PortForward and
	// AgentRelay, so a plain Apply still gets its always-on companion.
	if companionNeeded(b.remote, planFor, len(netnsPaths)) {
		agentImage := b.agentImage
		if planFor != nil {
			if img := planFor(0).agentImage; img != "" {
				agentImage = img
			}
		}
		if agentImage == "" {
			return api.DeployStatus{}, fmt.Errorf("containerd: a companion caretaker is needed but no agent image is configured (set CORNUS_AGENT_IMAGE)")
		}
		agentImg, err := b.pullImage(nctx, agentImage)
		if err != nil {
			return api.DeployStatus{}, fmt.Errorf("containerd: pull companion agent image: %w", err)
		}
		for i, netnsPath := range netnsPaths {
			...
			if b.remote {
				plan.binds = append(append([]specs.Mount{}, plan.binds...), propagatedBindMount(b.caretakerAgentDir(spec.Name, i), remotecompanion.AgentScratchDir, "rshared", false))
			}
			if err := b.startCompanion(ctx, spec.Name, netnsPath, i, agentImg, plan); err != nil {
				return api.DeployStatus{}, fmt.Errorf("containerd: start companion caretaker: %w", err)
			}
		}
	}
	...
	if telemetry != nil {
		...
		// the operator configured. Hand it the same variables. "transparent" mode
		// needs nothing here: its nftables redirect is netns-scoped and already
		// captures the collector.
		proxyEnv := egresspolicy.ProxyEnvFrom(spec.Env)
		for i, netnsPath := range netnsPaths {
			if err := b.startTelemetryCompanion(ctx, spec.Name, netnsPath, i, agentImg, telemetry.Role, telemetry.Relay, proxyEnv); err != nil {
				return api.DeployStatus{}, err
			}
		}
	}
	...
	// Arm health probing for the instances just created. Before Status, so the
	// status this returns already reports "starting" rather than "" for a
	// workload that declares a healthcheck.
	b.syncHealth(ctx, spec.Name)
...
func (b *Backend) createInstance(ctx context.Context, spec api.DeploySpec, img ctd.Image, replica int, ports []api.PortMapping, extraMounts []specs.Mount) (netnsPath string, retErr error) {
	...
	// Per-instance /etc/hosts, seeded with the name the container will actually
	// answer to. Passing the instance id here (as this did) while hostrun sets the
	// UTS hostname from spec.Hostname left the two disagreeing: `hostname` inside
	// the container returned "db" and "db" resolved nowhere, so `ping $(hostname)`
	// failed on containerd and worked on bare. Same resolution as barehost's.
	hostsPath, err := b.hosts.Create(id, hostrun.InstanceHostname(spec, id), att.IP)
	if err != nil {
		return "", err
	}
	defer func() {
	...
	// Everything above named paths in CORNUS's mount namespace; from here they
	// are containerd's to open, so translate the ones we own. The single
	// crossing point for the whole instance (see hostpaths_linux.go).
	mounts, err = b.hostMounts(mounts)
	if err != nil {
		return "", err
	}
	...
	// The spec's netns path is resolved by containerd's shim in the HOST's mount
	// namespace, so it crosses here and must be translated. The label above keeps
	// the local spelling, which is what cornus's own reboot recovery reads.
	specNetns, err := b.hostNetnsPath(att.Netns)
	if err != nil {
		return "", err
	}
	c, err := b.client.CreateContainer(nctx, id, img, labels, hostrun.SpecOpts(ctx, "containerd", id, spec, img, specNetns, mounts))
	if err != nil {
...
func (b *Backend) Start(ctx context.Context, name string) error {
	...
	// Starting ourselves is vacuous, and going through the repair/NewTask path
	// for a container that already has this process running in it is not
	// (self_linux.go).
	cs, withheld := b.withoutSelf(ctx, "start", name, cs)
	if len(cs) == 0 {
		if len(withheld) > 0 {
			return fmt.Errorf("containerd: refusing to start deployment %q: %s is this cornus server's own container", name, strings.Join(withheld, ", "))
		}
	...
	// Re-arm probing: the instances were stopped (so unwatched), and the
	// healthcheck is read back off their labels, which is what makes this work
	// after a server restart too.
	b.syncHealth(ctx, name)
	return nil
}
...
func (b *Backend) repairNetns(ctx context.Context, c ctd.Container, labels map[string]string) (bool, error) {
	...
	// Point the container's baked OCI spec at the fresh netns path — in
	// containerd's spelling, since the shim is what reopens it. The label below
	// keeps the local one for the next recovery pass to read.
	specNetns, err := b.hostNetnsPath(att.Netns)
	if err != nil {
		return false, err
	}
	...
	for i, ns := range spec.Linux.Namespaces {
		if ns.Type == specs.NetworkNamespace {
			spec.Linux.Namespaces[i].Path = specNetns
		}
	}
...
func (b *Backend) Stop(ctx context.Context, name string) error {
	b.ensureReconciled(ctx)
	// Disarm first: a probe racing the teardown would exec into a task being
	// killed and log a failure for a stop the operator asked for.
	...
	// Stopping ourselves would kill the process serving this request
	// (self_linux.go).
	cs, withheld := b.withoutSelf(ctx, "stop", name, cs)
	if len(cs) == 0 {
		if len(withheld) > 0 {
			return fmt.Errorf("containerd: refusing to stop deployment %q: %s is this cornus server's own container", name, strings.Join(withheld, ", "))
		}
		return fmt.Errorf("containerd: no instances for deployment %q: %w", name, deploy.ErrNotFound)
	}
	for _, c := range cs {
...
// companionNeeded reports whether any replica will get a companion caretaker, so
// apply only pulls the agent image when one is actually going to run.
func companionNeeded(remote bool, planFor func(replica int) companionPlan, replicas int) bool {
	if remote {
		return true
	}
	if planFor == nil {
