...
func (b *Backend) resolver(ctx context.Context, ref string) (remotes.Resolver, error) {
	...
	if b.creds != nil {
		credential, ok, err := b.creds(ctx, ref)
		if err != nil {
			return nil, err
		}
		if !ok {
			credential = deploy.RegistryCredential{}
		}
		authorizerOpts = append(authorizerOpts, docker.WithAuthCreds(func(string) (string, string, error) {
	...
	return docker.NewResolver(docker.ResolverOptions{
		...
		// With a resolver installed, an unknown host's Basic challenge now reports
		// "no basic auth credentials" instead of "failed to find supported auth
		// scheme"; anonymous Bearer-token negotiation is unchanged.
...
func (b *Backend) pullImage(ctx context.Context, ref string) (ctd.Image, error) {
	...
	resolver, err := b.resolver(ctx, ref)
	if err != nil {
		return nil, fmt.Errorf("containerd: resolve registry credential for %s: %w", ref, err)
	}
	pullOpts := []ctd.RemoteOpt{ctd.WithPullUnpack, ctd.WithResolver(resolver)}
	if b.snapshotter != "" {
