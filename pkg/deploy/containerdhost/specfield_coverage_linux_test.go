...
// what the operator asked for. This backend had six of them at once (proxy, dns,
// hub, docker, agentForward, updateConfig) until deploy.WarnKubernetesOnlyFields
// existed, and nothing in the tree noticed.
//
...
func TestEveryDeploySpecFieldIsMappedOrWarned(t *testing.T) {
	...
	if len(undecided) > 0 {
		t.Errorf("api.DeploySpec fields that containerdhost neither maps nor warns about: %v\n"+
