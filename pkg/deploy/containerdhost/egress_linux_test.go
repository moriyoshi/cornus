...
// egress produces exactly ONE companion task carrying every role, not the two
// that used to join the same netns.
//
// Two mattered because netredirect.Setup(port, 0, egressMark) installs a nat
// OUTPUT chain scoped to the NETWORK NAMESPACE, so it captured the sibling
// companion's traffic too, exempting only loopback and that one mark. An
// unmarked remote companion had its CORNUS_ADVERTISE_URL dial redirected into
// the egress proxy and routed by the egress policy, breaking port-forward /
// tunnel / exec-agent-forwarding on the next redial. With one process there is
// no sibling to capture, and the process-wide Mark covers every role at once.
//
// Only the container topology is asserted here: the caretaker.Config the
// companion is created with rides opaque OCI SpecOpts this package's fake does
// not materialize, the same limitation TestApplyWithEgressTransparent notes. The
// role content is asserted in the dockerhost package, which builds the identical
// config and captures the create body
// (TestRemoteAndEgressShareOneCompanion there).
func TestRemoteAndEgressShareOneCompanion(t *testing.T) {
	...
	for id, c := range f.containers {
		if isCompanion(c.labels) {
			companions++
			// In remote mode the survivor keeps the mount caretaker's identity, so
			// tooling filtering on that label/name keeps working.
			if id != "cornus-web-mount-0" {
				t.Errorf("companion id = %q, want cornus-web-mount-0", id)
			}
		}
	}
	if companions != 1 {
		t.Fatalf("companions = %d, want 1 (every role folds into one caretaker)", companions)
	}
	if _, ok := f.containers["cornus-web-egress-0"]; ok {
		t.Error("a separate egress companion still exists; it should have been folded in")
	}
}
...
func TestApplyWithAttachmentsRejectsRuntimeCredentialDeliveries(t *testing.T) {
	...
	creds := []deploy.AttachCredential{{
		...
		// No Kind: the api documents that this means "endpoint", and the message
		// must say so rather than leaving an empty string mid-sentence.
		Deliveries: []api.CredentialDelivery{{Provider: "generic"}},
	}}
	_, err := b.ApplyWithAttachments(ctx, api.DeploySpec{Name: "web", Image: "nginx:alpine"}, nil, creds, nil)
	if err == nil {
		t.Fatal("an endpoint credential delivery should be rejected on containerd")
	}
	if !strings.Contains(err.Error(), "endpoint") {
		t.Errorf("error = %v, want it to name the endpoint delivery kind", err)
	}
}
