...
// UnsupportedMetrics implements deploy.MetricsCapabilities. The cgroup source
// fills every family except the instantaneous core count, which is kubernetes's
// shape: CPU here is cumulative time, and a rate is the consumer's to derive.
...
func (b *Backend) SampleMetrics(ctx context.Context, name string, instance int) (api.ResourceSample, error) {
	c, err := b.instanceAt(ctx, name, instance)
	if err != nil {
