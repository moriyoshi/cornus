...
// host backends share), and Telemetry/Egress/Mounts are realized outside this
// package (deploy.BuildTelemetryWiring and the pkg/server attachment entry
// points). None of those is a silent drop, so none of them warns.
//
...
func supportedSpec() api.DeploySpec {
	return api.DeploySpec{
		...
		// container and probed by cornus, so apply must stay SILENT about it.
