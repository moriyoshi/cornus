...
func selfIDFromCgroup(content, namespace string) string {
	...
	for _, ref := range selfCgroupRefs(content) {
...
// cgroupRef is one "/<namespace>/<id>" pair read out of a cgroup path.
type cgroupRef struct {
	namespace string
	id        string
}

// selfCgroupRefs mines every containerd-shaped "/<namespace>/<id>" pair out of
// /proc/self/cgroup content, WITHOUT anchoring on any particular namespace.
//
// selfIDFromCgroup above wants one namespace — cornus's own — because it is
// answering "is this container one I could have created?". Self-inspection asks
// a different question and must not anchor: a cornus SERVER container is
// typically created outside cornus's namespace entirely (`nerdctl`/`ctr` put it
// in "default", docker in "moby"), so anchoring would find nothing in exactly
// the topology self-inspection exists for. Measured: a container created with
// `ctr run` reads "0::/default/<id>" from /proc/self/cgroup.
//
// Carrying the namespace alongside the id is what makes the lookup cheap and
// exact — it names the namespace to load from, so no namespace search is needed
// when this succeeds.
//
// Two segments exactly is the whole filter, which is deliberately loose: plenty
// of unrelated two-segment paths exist (a systemd "/system.slice/foo.service"
// parses as namespace "system.slice", id "foo.service"). That is safe because
// nothing here is trusted — a ref only matters if a containerd daemon actually
// holds a container by that id in that namespace, and hostenv then still has to
// confirm the inspect result describes this process.
func selfCgroupRefs(content string) []cgroupRef {
	var refs []cgroupRef
	seen := make(map[cgroupRef]bool)
	for _, line := range strings.Split(content, "\n") {
		...
		ns, id := path.Split(p)
		ref := cgroupRef{namespace: strings.Trim(ns, "/"), id: id}
		if ref.namespace == "" || ref.id == "" || strings.Contains(ref.namespace, "/") || seen[ref] {
			continue
		}
		seen[ref] = true
