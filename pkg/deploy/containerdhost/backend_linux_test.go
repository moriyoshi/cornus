...
func TestRemoveVolume(t *testing.T) {
	...
	dir, err := b.namedVolumeDir("proj_cache")
	if err != nil {
		t.Fatal(err)
	}
...
func TestInstanceMountsAndAnonReap(t *testing.T) {
	...
	wantShared, err := b.namedVolumeDir("shared")
	if err != nil {
		t.Fatal(err)
	}
	if mounts[1].Source != wantShared {
		t.Fatalf("named volume source = %q", mounts[1].Source)
	}
	wantAnon, err := b.anonVolumesDir("web")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.HasPrefix(mounts[2].Source, wantAnon) {
