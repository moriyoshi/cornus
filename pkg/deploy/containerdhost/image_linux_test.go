...
func TestResolverConsultsRegistryCredentialsForExactRef(t *testing.T) {
	var gotRef string
	b := &Backend{creds: func(_ context.Context, ref string) (deploy.RegistryCredential, bool, error) {
		gotRef = ref
		return deploy.RegistryCredential{Username: "cornus-internal", Password: "pull-token"}, true, nil
	}}
	if _, err := b.resolver(context.Background(), "localhost:5000/app:v1"); err != nil {
		t.Fatal(err)
	}
	...
	b.creds = func(context.Context, string) (deploy.RegistryCredential, bool, error) {
		return deploy.RegistryCredential{}, false, wantErr
	}
	if _, err := b.resolver(context.Background(), "localhost:5000/app:v1"); !errors.Is(err, wantErr) {
