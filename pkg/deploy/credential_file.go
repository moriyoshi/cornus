...
// LateIDCredentialBinder is a CredentialBinder whose id map does not EXIST until
// the workload's container does, so the server cannot own the files it wrote
// before calling Apply.
//
// Every other remapping runtime answers IDMap from something that outlives any
// one workload — rootless podman from the libpod service, docker from the daemon
// config — so the server resolves the ownership, writes the files, and Apply is
// an ordinary bind. incus does not: it records the map on the INSTANCE, and an
// instance that does not exist has no map to read. The ordering is a property of
// the runtime, not a shortcoming of the server, so it is named here rather than
// worked around at the call site.
//
// The dirs are handed over rather than discovered because the alternative was a
// marker on api.Mount, and api.Mount is the USER-FACING spec: a field saying
// "chown this source" would let a deploy.yaml aim a server-performed chown at any
// path the bind policy allows. These directories are ones the server itself
// created under its own data dir, and passing them explicitly keeps that true by
// construction.
type LateIDCredentialBinder interface {
	CredentialBinder
	// ApplyOwningCredentialDirs is Apply, plus the host directories whose
	// ownership the backend must correct to the workload's mapped ids once the
	// container exists and BEFORE it starts.
	//
	// Before it starts is the whole point: these directories are bind-mounted, so
	// a chown after start would land on a workload that may already have tried to
	// read the credential and failed. Deferring the correction to just-after-Apply
	// would trade a deterministic failure for a race.
	//
	// The backend owns the ordering and nothing else: it must not write, move or
	// delete anything in dirs, and an empty slice must behave exactly like Apply.
	ApplyOwningCredentialDirs(ctx context.Context, spec api.DeploySpec, dirs []string) (api.DeployStatus, error)
}
