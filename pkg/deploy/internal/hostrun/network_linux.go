...
// netnsDir is the single source of truth NetnsDir (netnsdir.go), kept as a local
// alias so the call sites below read as they always did.
const netnsDir = NetnsDir
...
// bridgeIfPrefix heads every bridge interface cornus creates. The stale-bridge
// sweep below only ever deletes links carrying it, so an interface cornus did
// not name can never be touched.
const bridgeIfPrefix = "crns"

// bridgeIfName derives a <=15-byte bridge interface name for a network.
func bridgeIfName(name string) string {
	sum := sha256.Sum256([]byte(name))
	return bridgeIfPrefix + hex.EncodeToString(sum[:])[:8]
}

// Bridge lifecycle hooks, indirected so the allocator's behavior is testable
// without root (the real implementations live in bridge_linux.go).
var (
	deleteBridgeLink = deleteBridgeByName
	dropStaleBridges = dropStaleBridgesFor
)
...
func (m *CNIManager) EnsureNetworks(names []string) error {
	...
	for _, name := range names {
		...
		// Self-repair: nothing but a leak may already hold this /24, because the
		// allocator gives every live network its own index. A bridge left over
		// from a cornus that died before reaping its network — or from a data dir
		// that was wiped while the host kept running — would otherwise add a
		// duplicate route for the subnet we are about to bring up. See
		// RemoveNetwork for why that breaks the new network's published ports.
		dropStaleBridges(fmt.Sprintf("%s.%d.1", subnetBase(), idx), bridgeIfName(name))
...
// RemoveNetwork deletes a managed network's conflist, its bridge, and frees its
// subnet. The default network is left alone (recreated on demand, keeps index 0).
func (m *CNIManager) RemoveNetwork(name string) error {
	...
	// The CNI bridge plugin creates the bridge on first attach but never deletes
	// it, not even once the last veth has left. Freeing the /24 without deleting
	// the bridge leaves an addressed interface behind, so the next network handed
	// that index brings up a SECOND bridge on the same <base>.<idx>.0/24: the host
	// then holds two routes for one prefix, and the stale (link-down) one wins for
	// part of the traffic — published ports to the live network fail to connect
	// with EHOSTUNREACH. Delete before releasing the index, so a failure here
	// keeps the subnet allocated rather than handing it out over a live bridge.
	if err := deleteBridgeLink(bridgeIfName(name)); err != nil {
		return fmt.Errorf("%s: delete bridge for network %q: %w", m.errPrefix, name, err)
	}
...
func UnsupportedNetworkFeatures(spec api.DeploySpec) []string {
	...
	for _, n := range spec.Networks {
		...
		// n.IP is the member's pinned IPv4 on this network (compose
		// `networks.<name>.ipv4_address`). The generated bridge conflist uses
		// host-local IPAM with no per-member reservation and cornus passes no
		// CNI_ARGS IP= at Setup, so a pinned address is dropped: the instance gets
		// the next free address in the auto-allocated /24 instead of the one the
		// compose file names. It was the one endpoint knob this list forgot, which
		// made it the one that dropped in silence.
		endpoint = endpoint || n.IP != "" || n.IPv6 != "" || n.MAC != "" || n.Priority != 0
	}
	var out []string
	if driver {
	...
	if endpoint {
		out = append(out, "ipv4Address/ipv6Address/macAddress/priority")
	}
	return out
}
