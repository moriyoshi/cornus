...
// TestInstanceHostname pins the name an instance answers to, and exists because
// the two host backends each had their own copy of this three-line rule and they
// disagreed.
//
// barehost resolved spec.Hostname; containerdhost passed the instance id straight
// through — while hostrun set the UTS hostname from the spec on BOTH. So the same
// compose file with `hostname: db` gave a container whose `hostname` said "db" and
// whose /etc/hosts mapped "cornus-web-0": `ping $(hostname)` worked on bare and
// failed on containerd. The UTS name and the hosts entry have to come from one
// place, which is what this function is.
func TestInstanceHostname(t *testing.T) {
	for _, tc := range []struct {
		...
		{"explicit hostname wins", api.DeploySpec{Hostname: "db"}, "cornus-web-0", "db"},
		{"unset falls back to the instance id", api.DeploySpec{}, "cornus-web-0", "cornus-web-0"},
		{"empty hostname is unset", api.DeploySpec{Hostname: ""}, "cornus-web-1", "cornus-web-1"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := InstanceHostname(tc.spec, tc.id); got != tc.want {
				t.Errorf("InstanceHostname(%+v, %q) = %q, want %q", tc.spec, tc.id, got, tc.want)
			}
		})
	}
}

// TestCreateSeedsTheResolvableHostname is the end of that chain: whatever
// InstanceHostname returns must be the name the hosts file actually maps, so the
// container can resolve itself.
func TestCreateSeedsTheResolvableHostname(t *testing.T) {
	h := NewHostsStore(t.TempDir(), "test", "test")
	spec := api.DeploySpec{Hostname: "db"}
	id := "cornus-web-0"
	path, err := h.Create(id, InstanceHostname(spec, id), "10.4.0.9")
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	data, err := os.ReadFile(path)
	...
	if got := string(data); !strings.Contains(got, "10.4.0.9\tdb\n") {
		t.Errorf("the spec hostname is not resolvable from inside the container:\n%s", got)
	}
}
