...
// InstanceHostname is the name an instance answers to: spec.Hostname when set,
// otherwise the instance id.
//
// It exists as one function because the two host backends each had their own copy
// and they DISAGREED. barehost resolved spec.Hostname; containerdhost passed the
// instance id straight through, while hostrun set the UTS hostname from the spec
// either way. So on containerd `hostname` inside the container returned "db" and
// "db" resolved nowhere — `ping $(hostname)` failed there and worked on bare, for
// the same compose file. The UTS name and the hosts entry must come from the same
// place, and now do.
func InstanceHostname(spec api.DeploySpec, id string) string {
	if spec.Hostname != "" {
