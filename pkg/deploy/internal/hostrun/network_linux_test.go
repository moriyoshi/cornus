...
func TestUnsupportedNetworkFeatures(t *testing.T) {
	...
	// A pinned per-member IPv4 (compose `ipv4_address`) is the endpoint knob this
	// list used to forget: host-local IPAM in the generated conflist has no
	// reservation for it and Setup passes no CNI_ARGS IP=, so the instance gets the
	// next free address in the auto-allocated /24 instead. Silently, until now.
	pinned := api.DeploySpec{Networks: []api.NetworkAttachment{{Name: "front", IP: "10.9.0.4"}}}
	if got := UnsupportedNetworkFeatures(pinned); len(got) != 1 || !strings.Contains(got[0], "ipv4Address") {
		t.Fatalf("pinned ipv4 address = %v, want an endpoint-feature warning", got)
	}
}
