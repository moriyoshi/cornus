...
func runtimeOpts(ctx context.Context, backend, id string, spec api.DeploySpec, netnsPath string, mounts []specs.Mount) []oci.SpecOpts {
	...
	// The instance ID is the container's hostname (docker parity), overridden by an
	// explicit compose `hostname`; the managed /etc/hosts seeds a MATCHING entry so
	// it resolves inside the container. Both sides call InstanceHostname so that
	// promise is structural rather than a convention two files have to remember —
	// containerdhost seeded /etc/hosts with the raw id while this set the spec
	// hostname, so `ping $(hostname)` failed there and worked on bare.
	opts := []oci.SpecOpts{oci.WithHostname(InstanceHostname(spec, id))}
