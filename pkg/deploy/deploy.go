...
// RegistryCredential is a username/password pair for one image registry.
type RegistryCredential struct {
	Username string
	Password string
}

// RegistryCredentials resolves a credential for an image reference at pull
// time. ok=false preserves the backend's normal anonymous/ambient behavior.
// Resolving per pull lets callers mint short-lived credentials without storing
// an expiring token on a long-lived backend.
type RegistryCredentials func(ctx context.Context, ref string) (credential RegistryCredential, ok bool, err error)

// RegistryCredentialRefresher is an optional backend capability for credentials
// stored outside the backend process. Kubernetes implements it for the shared
// image-pull Secret that kubelets read independently of Cornus.
type RegistryCredentialRefresher interface {
	RefreshRegistryCredentials(ctx context.Context) error
}
...
// IngressGateway is implemented by a backend that has a REAL ingress front door
// of its own — today only kubernetes, whose cluster ingress controller does the
// Host/path routing and TLS termination that the server-side mux would otherwise
// have to emulate.
//
// A server hosting an ingress tunnel prefers this: handing the tunnel's bytes
// straight to the controller means the cluster's own routing and certificates
// apply, and a raw TCP tunnel carries the client's SNI and Host through
// untouched. A backend that does not implement it (or whose DialIngress errors
// because no controller could be resolved) is served by the server-side mux
// instead.
type IngressGateway interface {
	// DialIngress opens a connection to the ingress controller's HTTP port, or
	// its HTTPS port when https is set. Each accepted tunnel connection gets one.
	DialIngress(ctx context.Context, https bool) (net.Conn, error)
	// IngressHosts reports the hostnames the live ingress object for name serves,
	// or nil when it has none.
	IngressHosts(ctx context.Context, name string) ([]string, error)
}
...
// FSOperator is an optional Backend capability: running one STRUCTURED filesystem
// operation against a deployment, served by whatever process can already see the bytes.
//
// It exists because the archive trio above cannot express most of what a file browser
// does. StatPath/CopyFrom/CopyTo can pack a path and unpack a tar and nothing else: no
// readdir, no delete, no rename, and no way to copy from one place to another without
// dragging every byte out to the caller and straight back in. Worse, on kubernetes all
// three are unsupported outright, so a volume-backed path there cannot be read or written
// at all.
//
// The alternative — splicing `ls`/`mv`/`rm` into the workload's own image — is what this
// deliberately is not. A distroless image has no shell, busybox and GNU disagree on
// directory-onto-directory, an exec's exit code is only as trustworthy as the runtime's
// inspect call, and exec is a wider privilege than a file operation. Each api.FSOp has one
// meaning and one errno-derived answer, whoever serves it.
//
// Realizations differ, exactly as the archive trio's already do (dockerhost proxies to the
// daemon, containerd walks /proc/<pid>/root, incus issues REST calls):
//
//   - The caretaker is the general one. It serves the volumes the backend mounted INTO it
//     and nothing else — it has its own mount and PID namespace, so the app's image layers
//     are out of reach by construction. This is the realization kubernetes needs.
//   - A backend whose volumes are plain directories it owns can serve them in process,
//     which has a property no sidecar can match: it works while the workload is stopped.
//
// A backend that cannot serve a given path answers api.FSErrUnsupported — a POSITIVE
// answer, not an error — and the caller relays the bytes itself instead. A backend with no
// operator at all simply does not implement this interface.
//
// The returned error is a transport or backend failure. An operation the operator refused
// rides in the response's Error and Code with a nil error, because the caller's next move
// depends on which refusal it was and an error string cannot carry that.
type FSOperator interface {
	// FSOp runs req against the named deployment. body is the tar stream for
	// api.FSOpPut and is ignored otherwise; out receives the tar stream for
	// api.FSOpGet and may be nil for every other op.
...
type Backend interface {
	...
	//
	// Framing contract: framing FOLLOWS THE CONTAINER'S TTY. A non-TTY
	// container's output MUST be stdcopy-multiplexed (Docker's 8-byte per-chunk
	// stream header) so a caller can demultiplex stdout/stderr regardless of
	// backend; a TTY container's output MUST be raw, because a PTY merges the
	// two streams at the source and there is nothing to demultiplex.
	//
	// It follows the TTY because that is what the docker-compatible proxy
	// announces and depends on: pkg/dockerproxy types its response with
	// streamContentType(r, spec.TTY) and passes these bytes through untouched,
	// so a backend that framed unconditionally would hand a TTY deployment's
	// client stdcopy headers it was told to read as terminal output. Docker
	// itself behaves this way, which is why dockerhost needs no branch — it
	// passes through whatever the daemon returns. Backends with an unframed
	// stream (kubernetes) wrap it in stdcopy stdout framing for the non-TTY case
	// only.
	//
	// This paragraph previously said implementations must ALWAYS frame, which
	// described neither implementation and was wrong in the direction that
	// matters: it made the correct backend look like the broken one.
	//
	// opts.Instance selects which replica to stream (0-based, over the same
	// ordering Status reports); the zero value is the first, so a caller that
	// does not care is unaffected. An out-of-range index MUST be ErrNotFound,
	// never a silent fallback to instance 0 — a caller enumerating replicas has
	// to be able to tell a departed replica from a different one's output.
	// Streaming several instances at once is the CALLER's job: it opens one call
	// per instance and labels them (see `compose logs --all-replicas` and the
	// observability log recorder).
	...
	// CLI can parse them. Backends without a metrics source return a
	// not-supported error.
	//
	// opts.Instance selects which replica to sample, on the same terms as
	// Logs: 0-based over the ordering Status reports, the zero value being the
	// first, and an out-of-range index ErrNotFound rather than a silent fallback
	// to instance 0.
	//
	// This is the DOCKER-SHAPED view, kept because the docker CLI and the web UI
	// parse it verbatim. A caller that wants the numbers rather than the format
	// should use MetricsSampler instead: the Docker frame carries CPU as a pair of
	// cumulative readings for the CLI to difference, has no room for a backend
	// that can only report a rate, and zeroes precpu on a one-shot read.
...
// configured for that mode — a daemon on a DIFFERENT host cannot be detected,
// so it is always an explicit operator opt-in (e.g. CORNUS_DOCKER_REMOTE),
// never inferred. The server consults it to choose between the two paths; a
// backend that does not implement RemoteCapable (kubernetes, which has no
// host-mount fallback anyway) is always treated as if Remote() were true.
//
// Not to be confused with a cornus CONTAINERIZED on the daemon's own host,
// which pkg/hostenv does detect. That server stays on the co-located fast path
// and performs the mounts itself; only the paths it hands the runtime are
// translated (pkg/server's hostVisibleMountSources). Turning on remote mode
// there would run a companion per instance to solve a problem that host is not
// having.
type RemoteCapable interface {
	Remote() bool
}

// MountSessionReader is an optional Backend extension: report the deploy-attach
// mount session id already baked into name's currently-applied workload (the id
// stamped on its caretaker sidecar), or "" if the workload does not exist or
// carries no session. The server reuses it on re-apply instead of minting a fresh
// one, so the id stays STABLE across client reconnects / re-runs. A per-connection
// id is baked immutably into the pod at deploy time, so minting a new id on
// re-apply orphans the running pod's baked id — the server then has no live backing
// for the id the pod presents and resets its mounts ("connection reset by peer";
// see mount_relay.go logMountReset). Reusing the baked id lets the reconnecting
// client re-register under the id the pod already presents, keeping its mounts live.
type MountSessionReader interface {
	ExistingMountSession(ctx context.Context, name string) (string, error)
}

// AgentForwardCapable is an optional Backend extension: a backend answers for
// itself whether ssh-agent forwarding into an exec session
// (api.ExecConfig.ForwardAgent) is available for a deployment, so the server can
// fail an exec request fast instead of injecting a SSH_AUTH_SOCK that goes
// nowhere. The server consults it BEFORE falling back to inferring the answer
// from RemoteCapable.Remote(). Two kinds of implementer:
//
//   - kubernetes, where it is gated per-DEPLOYMENT: it has no Remote()/
//     co-location concept, and unconditionally running a caretaker sidecar for
//     every deployment just for this would be wasteful, so it is opt-in via
//     api.DeploySpec.AgentForward and AgentForwardEnabled reports whether name's
//     currently-applied spec had it set.
//   - bare, which returns a flat false: it sets Remote() for the sidecar-mount
//     path but runs no companion carrying a caretaker AgentRelayRole, so the
//     Remote()-based inference would wrongly grant the feature.
//   - incus, which answers Remote(): its remote mode does run an always-on
//     companion with an AgentRelayRole, so the two currently agree — but it
//     states the answer rather than relying on the inference, because a future
//     non-remote incus companion (a mount or egress caretaker) would carry no
//     agent relay and would silently re-acquire bare's failure mode.
//
// Backends whose Remote() genuinely does imply an agent relay (dockerhost,
// containerdhost, which run an always-on companion in remote mode) do not need
// this interface.
type AgentForwardCapable interface {
	AgentForwardEnabled(ctx context.Context, name string) (bool, error)
}

// MetricsSampler is an optional Backend extension: read one instance's current
// resource usage in a backend-neutral shape, for recording rather than display.
//
// It is separate from Backend.Stats because the two have genuinely different
// consumers. Stats renders Docker's stats JSON, which the docker CLI and the web
// UI parse verbatim and which therefore cannot change; this returns the numbers,
// so a collector never has to parse the format cornus itself just produced. See
// api.ResourceSample for what that buys.
//
// A backend that has no metrics source simply does not implement this, and the
// collector skips it — the same shape as Preflighter. instance is a 0-based
// replica ordinal with Backend.Logs's contract: out of range is ErrNotFound,
// never a fallback to the first.
type MetricsSampler interface {
	SampleMetrics(ctx context.Context, name string, instance int) (api.ResourceSample, error)
}

// MetricsCapabilities is an optional MetricsSampler extension: the sampler names
// the api.ResourceSample fields it can never fill, whatever the workload is doing.
//
// A ResourceSample already distinguishes absent from zero, which is enough to
// keep a chart honest — an unobservable family produces no series rather than a
// series of zeros. What it cannot do is tell a consumer WHY a family is empty, and
// the two reasons want opposite treatment: a workload that has moved no bytes yet
// should keep its network chart (it will fill in), while a kubernetes workload,
// whose metrics-server source has no network counters at all, should not be
// offered one. The web dashboard hides the second kind; without a declaration it
// would have to infer permanence from a run of empty results, which is exactly the
// guess that gets an idle workload wrong.
//
// Deliberately a NEGATIVE list, unlike HealthReporter's positive capability. The
// supported set is the default and the interesting claim is the exception, so a
// backend that grows a source drops an entry rather than having to remember to add
// one — and a backend that does not implement this at all is read as "fills every
// field", which is the truthful reading for a source with no known gaps.
//
// The answer must be a permanent property of this backend VALUE. It may depend on
// how the backend was configured (barehost declares network unsupported only when
// it is running a gVisor sandbox, whose netstack the host cannot see), but it must
// not vary per workload or per call: a consumer is entitled to ask once.
type MetricsCapabilities interface {
	UnsupportedMetrics() []api.SampleField
}

// HealthReporter is an optional Backend capability: the backend runs a container
// health probe (api.DeploySpec.Healthcheck, i.e. compose `healthcheck:`) and
// fills api.InstanceStatus.Health from it, so a caller can wait for a workload to
// become "healthy" rather than merely Running. dockerhost implements it (Docker's
// own HEALTHCHECK engine) and so does kubernetes (a liveness/readiness probe,
// projected onto replica readiness). The backends with no probe engine —
// containerd, bare, incus — drop the healthcheck with a warning at Apply time and
// do NOT implement this, so their Health is permanently "".
//
// It exists so a CLIENT can refuse a compose `depends_on: condition:
// service_healthy` UP FRONT on a backend that can never produce that state,
// instead of polling a field that will not change until the dependency wait hits
// its timeout and fails generically. The server reports it through
// GET /.cornus/v1/info (api.BackendInfo.ReportsHealth).
//
// Deliberately a POSITIVE capability rather than a list of backends known to lack
// health: a backend that grows a probe engine implements this and is believed at
// once, with no client-side name list to keep in step. The bool return follows
// RemoteCapable's shape so a backend whose probe engine depends on its
// configuration can answer for the instance at hand; a backend that does not
// implement the interface at all reports no health.
type HealthReporter interface {
	ReportsHealth() bool
}

// Preflighter is an optional Backend extension: verify the backend has the cluster
// permissions it needs and report the missing ones, so the server can WARN at
// startup instead of a deploy failing opaquely at reconcile time with a raw
// `forbidden`. Reporting is best-effort — a backend that cannot run the check (the
// self-check itself is denied, or the API is unreachable) returns nil, never an
// error, so preflight never blocks serving. An empty slice means "all clear".
type Preflighter interface {
	Preflight(ctx context.Context) []PermissionGap
}

// PermissionGap is one permission the backend needs but does not have. Core gaps
// (Feature == "") break a fundamental capability and should be surfaced loudly; a
// feature gap gates only that named optional feature and is informational.
type PermissionGap struct {
	Verb     string // e.g. "create"
	Resource string // group-qualified, e.g. "ingresses.networking.k8s.io"
	Feature  string // "" for a core capability; else the feature this grant gates
	Impact   string // human-facing consequence + hint
}
...
// carries the session/relay coordinates the sidecar dials; Deliveries carries the
...
type AttachCredential struct {
	...
	Deliveries []api.CredentialDelivery // runtime deliveries (endpoints/files) served by the caretaker
