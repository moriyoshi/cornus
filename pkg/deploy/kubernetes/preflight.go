...
var preflightChecks = []permissionCheck{
	...
	// Two features ride this one grant, and the second fails silently without
	// it: `compose logs` reports an error a user sees immediately, but the
	// observability store's log recorder would simply record nothing, leaving an
	// empty query result that reads as "the workload said nothing". Naming both
	// here is what turns that into a legible preflight finding.
	{resource: "pods", subresource: "log", verb: "get", impact: "compose logs is unavailable, and the built-in observability store cannot record workload logs (queries return empty rather than failing)"},
	// Fails the same silent way the pods/log grant does: without it the metrics
	// recorder samples nothing and `cornus observe metrics` returns an empty
	// result, which reads as "this workload used no CPU" rather than "cornus was
	// not allowed to look". Feature-gated because a cluster without
	// metrics-server legitimately has no such API to grant.
	{
		group: "metrics.k8s.io", resource: "pods", verb: "get",
		feature: "workload metrics",
		impact:  "cornus cannot read pod CPU/memory: `docker stats` and the web metrics pane are unavailable, and the built-in observability store records no workload metrics (queries return empty rather than failing). Needs metrics-server installed plus this grant.",
	},
	{
