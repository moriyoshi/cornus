...
// waitJobGone polls until no Job named name exists (bounded by ctx), so a
// replace can safely recreate it. A missing Job returns immediately.
func (b *Backend) waitJobGone(ctx context.Context, name string) error {
	...
	return retry.OnError(retry.DefaultBackoff, func(error) bool { return true }, func() error {
		...
		return fmt.Errorf("job %q still terminating", name)
	})
}
