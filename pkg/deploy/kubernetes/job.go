...
func (b *Backend) applyWorkload(ctx context.Context, spec api.DeploySpec, desired *appsv1.Deployment) (api.DeployStatus, error) {
	// Warn once per apply for every spec value this backend accepts but cannot
	// realize and that has no warning at its translation site. This funnel is the
	// one place every apply path passes through exactly once, which is what keeps
	// a spec from collecting the same warning twice.
	warnUnsupportedFields(ctx, spec)
