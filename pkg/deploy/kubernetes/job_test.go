...
// TestWaitJobGoneWaitsOutTermination is the regression guard for a replace that
// races a still-terminating Job. A foreground delete keeps the Job object alive
// until its pods finish their termination grace period (tens of seconds), so
// waitJobGone must genuinely poll until the object disappears — not give up after
// a handful of sub-second retries, which aborted `cornus compose up` whenever a
// running one-shot service (restart: no) had to be re-applied and left the Job
// deleted-but-never-recreated.
func TestWaitJobGoneWaitsOutTermination(t *testing.T) {
	cs := fake.NewSimpleClientset()

	// Report the Job as still present for the first several Get calls (simulating
	// a pod grinding through its grace period), then NotFound. The old 4-attempt
	// budget gave up well before this many polls; the poll loop must outlast it.
	const lingerPolls = 6
	gets := 0
	cs.PrependReactor("get", "jobs", func(action k8stesting.Action) (bool, runtime.Object, error) {
		gets++
		if gets <= lingerPolls {
			return true, &batchv1.Job{ObjectMeta: metav1.ObjectMeta{Name: "init", Namespace: "default"}}, nil
		}
		return true, nil, apierrors.NewNotFound(batchv1.Resource("jobs"), "init")
	})

	b := NewWithClient(cs, "default")
	if err := b.waitJobGone(context.Background(), "init"); err != nil {
		t.Fatalf("waitJobGone must succeed once the Job clears, got %v", err)
	}
	if gets <= lingerPolls {
		t.Fatalf("expected more than %d polls before giving up, made %d", lingerPolls, gets)
	}
}
