...
func TestCredentialProxyEndpoint(t *testing.T) {
	...
	creds := []deploy.AttachCredential{{
		...
		Deliveries: []api.CredentialDelivery{{Kind: "endpoint", Provider: "anthropic-proxy", Upstream: "http://gw.internal:8080"}},
	}}
	...
	// A base URL alone is not enough: Claude Code and the Anthropic SDKs refuse to
	// run without a key variable, so the app container also gets a placeholder the
	// proxy strips on the way out.
	if env["ANTHROPIC_API_KEY"] == "" {
		t.Fatalf("app env missing the placeholder ANTHROPIC_API_KEY: %v", env)
	}
	cfg := decodeCaretakerConfig(t, caretakerCtr(t, pod).Env)
	if len(cfg.Credentials) != 1 || cfg.Credentials[0].Deliveries[0].Provider != "anthropic-proxy" {
		t.Fatalf("credential role = %+v", cfg.Credentials)
	}
	if got := cfg.Credentials[0].Deliveries[0].Upstream; got != "http://gw.internal:8080" {
