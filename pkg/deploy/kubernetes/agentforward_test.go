...
func TestAgentForwardAloneGetsMinimalCaretaker(t *testing.T) {
	...
	// Asserted against the SHARED constructor, not the literal "web/0". The contract is
	// that this equals the key pkg/server registers the exec --forward-agent client
	// channel under; two independent literals would both keep passing while the relay
	// stopped finding anything.
	if want := remotecompanion.AgentRelayInstance("web"); cfg.Instance != want {
		t.Errorf("cfg.Instance = %q, want %q (the key the server looks the agent channel up by)", cfg.Instance, want)
	}
