...
func knativeGuard(spec api.DeploySpec) error {
	...
	// knative's own Route/URL routing. Gate on ingressroute.Enabled (which is false for a
...
func (b *Backend) applyKnativeService(ctx context.Context, spec api.DeploySpec, desired *appsv1.Deployment) (api.DeployStatus, error) {
	// A revision exposes exactly ONE port (chooseKnativePort picks it). The rest are
	// dropped — api.KnativeSpec documents that as happening "with a warning", and it
	// was silent, so a two-port service deployed as a ksvc lost a listener with
	// nothing said. Warn here rather than in knativeService, which has no context.
	if len(spec.Ports) > 1 {
		logging.FromContext(ctx, slog.Group("kubernetes", "deployment", spec.Name)).WarnContext(ctx,
			"knative: a revision exposes exactly one container port; the other published ports are dropped and nothing routes to them",
			"published", len(spec.Ports))
	}
	// A ksvc has no replica field: Knative's autoscaler owns the count, so
	// spec.Replicas is dropped. Left silent, an explicit `replicas: 3` yields
	// scale-to-zero instead — the workload idles down to NO instances, which is
	// the opposite of what the field asked for and shows up as "my service keeps
	// cold-starting", not as a config error.
	//
	// Threshold mirrors the ports warning above, and it is what makes this
	// noise-free rather than a judgement call: spec.Replicas is 0 when unset
	// (pkg/compose/project.go copies compose's deploy.replicas verbatim) and
	// deploy.Replicas() maps <=0 to 1, so only a user who explicitly asked for
	// more than one instance can trip it. Someone who wrote a ksvc the Knative way
	// and never touched replicas never sees this.
	if spec.Replicas > 1 {
		logging.FromContext(ctx, slog.Group("kubernetes", "deployment", spec.Name)).WarnContext(ctx,
			"knative: a revision has no replica count; spec.Replicas is ignored and the autoscaler decides, including scaling to zero when idle — set knative.minScale/maxScale instead",
...
// knativeList reports every cornus-managed Knative Service, for Backend.List.
//
// It returns no error for the three ways Knative can simply be absent — no
// dynamic client, CRDs not served, or a ServiceAccount without
// serving.knative.dev RBAC (Forbidden). List must still return the workloads it
// CAN see in each of those cases; failing the whole listing because an optional
// feature is not installed would break `cornus ps` on every ordinary cluster.
// Same rationale as knativeStatus and deleteKnative.
func (b *Backend) knativeList(ctx context.Context, opts metav1.ListOptions) ([]api.DeployStatus, error) {
	if b.dyn == nil || !b.knativeServed() {
		return nil, nil
	}
	list, err := b.dyn.Resource(knativeServiceGVR).Namespace(b.namespace).List(ctx, opts)
	if err != nil {
		if apierrors.IsNotFound(err) || apierrors.IsForbidden(err) {
			return nil, nil
		}
	...
	for i := range list.Items {
		out = append(out, b.statusOfKnative(ctx, &list.Items[i]))
	}
	return out, nil
}
