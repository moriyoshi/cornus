...
// worked around:
//
//   - **No network or disk counters.** metrics.k8s.io reports CPU and memory and
//     nothing else. The fuller set lives behind the kubelet's Summary API
//     (/api/v1/nodes/{node}/proxy/stats/summary), which needs nodes/proxy — a
//     grant that lets the holder reach every kubelet in the cluster, and one a
//     sane operator denies. Trading that blast radius for two more metric
//     families is not a good deal, so the families are simply absent, and absent
//     means no series at all rather than a series reading zero.
...
func (b *Backend) SampleMetrics(ctx context.Context, name string, instance int) (api.ResourceSample, error) {
	pod, err := b.podAt(ctx, name, instance)
	if err != nil {
	...
	return pm.toResourceSample(), nil
}
...
func (b *Backend) Stats(ctx context.Context, name string, opts api.StatsOptions, w io.Writer) error {
	...
	return hostrun.StreamStats(ctx, w, pod, name, opts.Stream, b.statsSampler(ctx, pod))
}
...
func (b *Backend) statsSampler(ctx context.Context, pod string) func() (hostrun.StatsSample, error) {
