...
func TestJobFromDeployment(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// restart:"no" carries a small transient-failure budget (oneShotTransientRetries),
		// not 0, so a scheduling/mount/PVC race self-heals instead of being fatal on the
		// first attempt; the pod restartPolicy stays Never.
		{"no", 0, corev1.RestartPolicyNever, oneShotTransientRetries},
...
// ttyPod is a running pod whose APP container was created with a TTY, which is
// what a `tty: true` deploy spec produces (the container build sets TTY from
// spec.TTY).
func ttyPod(name string, tty bool) *corev1.Pod {
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      name + "-abc123",
			Namespace: "default",
		...
		Spec: corev1.PodSpec{Containers: []corev1.Container{
			{Name: execContainer, TTY: tty},
		}},
		Status: corev1.PodStatus{Phase: corev1.PodRunning},
	}
}

// TestLogsAreRawForATTYContainer is the fix for a real garbling bug.
//
// pkg/dockerproxy types a logs response with streamContentType(r, spec.TTY) —
// RAW for a TTY container — and then passes the backend's bytes through
// untouched. This backend framed unconditionally, so a `tty: true` deployment's
// client was told to read raw terminal output and handed stdcopy's 8-byte
// headers, which land in the terminal as garbage.
//
// dockerhost never had the bug because it passes through whatever the daemon
// returns, and Docker itself leaves a TTY container's logs unframed. The written
// contract said "always frame", which described neither backend.
func TestLogsAreRawForATTYContainer(t *testing.T) {
	b := NewWithClient(fake.NewSimpleClientset(ttyPod("tty", true)), "default")

	var buf bytes.Buffer
	if err := b.Logs(context.Background(), "tty", api.LogOptions{}, &buf); err != nil {
		t.Fatalf("Logs: %v", err)
	}
	if got := buf.String(); got != "fake logs" {
		t.Fatalf("TTY logs = %q, want the raw bytes %q with no stdcopy header", got, "fake logs")
	}
	// And the framed reading must now FAIL to find a frame, which is the other
	// half: asserting only the string above would pass for framed output whose
	// header happened to be stripped by something else.
	var stdout, stderr bytes.Buffer
	if _, err := stdcopy.StdCopy(&stdout, &stderr, bytes.NewReader(buf.Bytes())); err == nil && stdout.String() == "fake logs" {
		t.Fatal("TTY logs still demultiplex as stdcopy frames, so they are still framed")
	}
}

// TestLogsStayFramedForANonTTYContainer is the other side: the fix must not turn
// every deployment raw. A non-TTY container has two streams and the client is
// told to demultiplex them.
func TestLogsStayFramedForANonTTYContainer(t *testing.T) {
	b := NewWithClient(fake.NewSimpleClientset(ttyPod("plain", false)), "default")

	var buf bytes.Buffer
	if err := b.Logs(context.Background(), "plain", api.LogOptions{}, &buf); err != nil {
		t.Fatalf("Logs: %v", err)
	}
	var stdout, stderr bytes.Buffer
	if _, err := stdcopy.StdCopy(&stdout, &stderr, &buf); err != nil {
		t.Fatalf("non-TTY logs must be stdcopy-framed: %v", err)
	}
	if stdout.String() != "fake logs" {
		t.Fatalf("stdout = %q, want %q", stdout.String(), "fake logs")
	}
...
// TestPodContainerTTYReadsTheAppContainer pins which container decides. A pod
// carries companions (the caretaker sidecar), and reading the wrong one's TTY
// would make the framing depend on a container the user never configured.
func TestPodContainerTTYReadsTheAppContainer(t *testing.T) {
	pod := &corev1.Pod{Spec: corev1.PodSpec{Containers: []corev1.Container{
		{Name: "caretaker", TTY: true},
		{Name: execContainer, TTY: false},
	}}}
	if podContainerTTY(pod) {
		t.Fatal("the caretaker's TTY decided the framing; only the app container's may")
	}
	pod.Spec.Containers[1].TTY = true
	if !podContainerTTY(pod) {
		t.Fatal("the app container's TTY was ignored")
	}
}

// TestCredentialFileCarriesTheAppContainersIDs is the regression test for a bug
// that survived because every test workload ran as root.
//
// creddelivery.WriteFile writes mode 0600 and the caretaker runs as its image's
// user (root), while the app container runs as spec.User via
// securityContext.RunAsUser. They share only the emptyDir the file lands in — so
// a `user: "1000"` deployment could not read its own credential, and the only
// symptom was the application's own "permission denied".
//
// The host backends have chowned since they gained file delivery
// (deploy.CredentialFileOwner); the kubernetes path never did.
func TestCredentialFileCarriesTheAppContainersIDs(t *testing.T) {
	b := NewWithClient(fake.NewSimpleClientset(), "default")
	...
	b.addCredentialRoles(context.Background(), []deploy.AttachCredential{{
		Name: "db",
	...
	if len(cfg.Credentials) != 1 || len(cfg.Credentials[0].Deliveries) != 1 {
		t.Fatalf("credential roles = %+v, want one file delivery", cfg.Credentials)
	}
	d := cfg.Credentials[0].Deliveries[0]
	if d.UID != 1000 || d.GID != 2000 {
		t.Fatalf("file delivery carries uid:gid = %d:%d, want 1000:2000 — the caretaker writes "+
			"0600 as root, so a non-root app cannot read what it wrote", d.UID, d.GID)
	}
}

// TestCredentialFileIDsFollowThePodSpecNotTheSpecUser pins WHERE the ids come
// from. Reading the pod spec means the file's ownership and the process's
// identity are one fact; re-deriving from spec.User would be two renderings that
// can drift, and only their agreement is the contract.
func TestCredentialFileIDsFollowThePodSpecNotTheSpecUser(t *testing.T) {
	// No securityContext at all: the container inherits the image's user, which
	// nothing here can resolve, so the file must be left as written.
	podSpec := &corev1.PodSpec{Containers: []corev1.Container{{Name: execContainer}}}
	if uid, gid := appContainerIDs(podSpec); uid != 0 || gid != 0 {
		t.Fatalf("ids = %d:%d for a container with no securityContext, want 0:0 (leave as written)", uid, gid)
	}
}
