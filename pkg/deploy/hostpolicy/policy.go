...
type Policy struct {
	...
	//
	// The paths compared here are the RUNTIME's, not necessarily this process's.
	// A bind source names a path the daemon opens, so when cornus is
	// containerized on the daemon's host the two spellings differ and the server
	// has already translated the source before Apply. Prefixes must therefore be
	// written as the runtime sees them — which for an operator setting
	// CORNUS_ALLOW_BIND_SOURCES means host paths, the same ones they would pass
	// to `docker run -v`.
...
func (p Policy) Validate(backend string, spec api.DeploySpec) error {
	...
	for _, m := range spec.Mounts {
		...
		if !p.bindAllowed(m.Source) {
			return p.denied(backend, m.Source)
		}
	}
	return nil
}

// denied builds the rejection error for a source bindAllowed refused.
//
// A NON-ABSOLUTE source gets its own wording because the generic "not under an
// allowed prefix" text misleads: nothing the operator can put in
// CORNUS_ALLOW_BIND_SOURCES -- documented as absolute directory prefixes --
// makes a bare name like "cache" or a relative path like "./src" pass the
// prefix test, so the advice reads as a lie. The bare name in particular is
// usually a VOLUME reference that leaked into Mounts, and its home is
// DeploySpec.Volumes, which is not a host bind and needs no allow-listing.
//
// Bare names are deliberately NOT skipped here, i.e. they are not classified as
// volume references and waved through. Docker resolves the bare-name/relative-
// path ambiguity by fiat (a source that is not a path is a volume), and cornus's
// compose translation, dockerproxy (isHostPath) and client (isLocalSource) all
// follow that convention -- but only on the way IN, when they still route the
// entry to spec.Volumes. By the time a raw spec.Mounts entry reaches a backend
// nothing re-routes it: containerdhost/barehost hand Mount.Source verbatim to
// hostrun.OCIBindMount, so a bare name becomes an OCI bind mount with a RELATIVE
// source, resolved by the OCI runtime against a working directory cornus neither
// sets nor can see (go-runc sets no cmd.Dir, and barehost's runtime binary is
// operator-configurable). Under a base of "/" -- what a systemd unit's default
// WorkingDirectory gives -- "etc", "root" or "proc" is exactly the arbitrary
// host bind this package exists to deny on an unauthenticated API. Skipping the
// check would therefore trade a clear error for an undefined mount, and on the
// wrong runtime for a policy bypass, so a source cornus cannot resolve stays
// default-deny; an operator who really means it can still allow-list the name
// verbatim.
func (p Policy) denied(backend, src string) error {
	if !filepath.IsAbs(src) {
		return fmt.Errorf("%s: mount source %q is not permitted by policy: it is not an absolute host path, so what it binds depends on the runtime's working directory. "+
			"For a named volume use the spec's volumes list (a managed volume, not a host bind, and not policy-gated); for a host bind give an absolute path under an allowed prefix (allowed: %s; extend with CORNUS_ALLOW_BIND_SOURCES)",
			backend, src, p.allowedList())
	}
	return fmt.Errorf("%s: host bind source %q is not permitted by policy (allowed prefixes: %s; extend with CORNUS_ALLOW_BIND_SOURCES)", backend, src, p.allowedList())
}
