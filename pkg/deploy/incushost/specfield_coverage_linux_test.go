...
// adding it to the supported spec in TestBuildInstancesPostIsSilentForASpecItFullyHonors)
// or adding a row."
//
...
func TestEveryDeploySpecFieldIsMappedOrWarned(t *testing.T) {
	...
	if len(undecided) > 0 {
		t.Errorf("api.DeploySpec fields that incushost neither maps nor warns about: %v\n"+
