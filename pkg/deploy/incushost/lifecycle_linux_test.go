...
// ConsoleAttach satisfies incusConn; recordingConn only records lifecycle calls.
func (c *recordingConn) ConsoleAttach(name string) (func(io.ReadWriteCloser) error, func(), error) {
	return nil, nil, fmt.Errorf("recordingConn: console not available for %s", name)
}

// CreateVolume / DeleteVolume satisfy incusConn; recordingConn records only
// lifecycle calls, so these are accepted and ignored.
func (c *recordingConn) CreateVolume(pool, name string, config map[string]string) error { return nil }

func (c *recordingConn) DeleteVolume(pool, name string) error { return nil }
