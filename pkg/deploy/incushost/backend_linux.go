...
type Backend struct {
	...
	// client-local mounts and port-forward; the fields are set from the options so
	// the surface matches the other host backends, but NONE of them is read yet —
	// this backend starts no companion. See WithRemote / AgentForwardEnabled and
...
// path (CORNUS_INCUS_REMOTE). See deploy.RemoteCapable. The companion itself is
// NOT implemented yet, so this currently only records the operator's intent —
// see AgentForwardEnabled.
func (b *Backend) Remote() bool { return b.remote }

// AgentForwardEnabled implements deploy.AgentForwardCapable, always false: this
// backend creates no caretaker companion at all, so nothing carries an
// AgentRelayRole and nothing listens on the SSH_AUTH_SOCK path an exec would be
// given. Declaring this explicitly stops the server inferring the feature from
// Remote() and letting the failure surface inside the container instead of at
// the API.
//
// Implementing the companion here would make this a real answer; see
// .agents/docs/JOURNAL.md, "unified companion caretaker" (2026-07-25).
