...
// wsControl is the exec control channel as this backend uses it: a sink for the
// JSON window-resize message. It is an interface rather than *websocket.Conn
// (which satisfies it) purely so the resize logic — including the flush that
// fixes the resize/connect race — can be exercised without standing up a
// websocket server. Nothing about the behaviour changes.
...
// attachControl records the exec's live control channel and flushes any terminal
// size requested before it came up, so a resize that raced ahead of the
// connection still lands. Split out of ExecStart's callback so the race fix is
// testable without a websocket peer.
func (s *execSession) attachControl(c wsControl) {
	s.mu.Lock()
	s.control = c
...
// initialSize returns the PTY size to seed an exec with, and whether one was
// requested before the exec started.
func (s *execSession) initialSize() (width, height int, ok bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (b *Backend) ExecStart(ctx context.Context, execID string, cfg api.ExecStartConfig, conn io.ReadWriteCloser) error {
	...
	if w, h, ok := sess.initialSize(); ok {
		post.Width, post.Height = w, h
	}

	args := &incus.InstanceExecArgs{
		Stdin:    conn,
		DataDone: make(chan bool),
		// Capture the control channel and flush any size requested before it was
		// live, so a resize that raced ahead of the connection still lands.
		Control: func(c *websocket.Conn) { sess.attachControl(c) },
	}
	if post.Interactive {
		args.Stdout = conn
		args.Stderr = conn
