...
func (b *Backend) ForwardPort(ctx context.Context, name string, port int, proto string, conn io.ReadWriteCloser) error {
	...
	if proto == "udp" {
		...
		if err != nil {
			return fmt.Errorf("incus: dial instance udp %s: %w%s", addr, err, b.unreachableHint())
		}
	...
	if err != nil {
		return fmt.Errorf("incus: dial instance %s: %w%s", addr, err, b.unreachableHint())
	}
	return deploy.Bridge(conn, upstream)
}

// unreachableHint names the cause of a failed instance dial when the topology is
// one where the dial could never have worked, and returns "" otherwise.
//
// Only one such topology exists on this backend, and it is the reason remote mode
// exists at all: a cornus in a container with its own netns has no route to
// incusbr0. There is no macvlan-style analogue to check for and no remote-daemon
// case either — New dials a local unix socket and nothing else.
//
// The third condition is selfInstance. A cornus that is itself an instance on this
// daemon looks isolated by every test hostenv can apply — it is in a container with
// its own netns — but it is a PEER of the workloads on incusd's bridge and can dial
// them directly. Blaming the topology there would send an operator to configure
// remote mode or host networking for a dial that failed because nothing was
// listening yet.
func (b *Backend) unreachableHint() string {
	if !b.isolatedNetwork || b.remote || b.selfInstance != "" {
		return ""
	}
	return " (this cornus server runs in a container with its own network namespace, so it has no route to the incus bridge: " +
		"set CORNUS_INCUS_REMOTE=1 with CORNUS_AGENT_IMAGE and CORNUS_ADVERTISE_URL to reach instances through a per-instance " +
		"companion, or give the server container the host's network namespace)"
}
...
// pickIPv4 returns an instance's first global (non-loopback) IPv4 in INTERFACE
// NAME order, ignoring the loopback interface entirely.
//
// The sort is the whole point of this function's shape. It used to range over the
// map directly, and Go randomizes map iteration, so an instance with more than one
// addressed NIC — a profile that adds a second bridged NIC, a macvlan or routed
// NIC beside eth0 — handed back a DIFFERENT address on different calls. When the
// two addresses are not equally reachable (a macvlan NIC cannot be reached from
// its own host at all, the same kernel rule dockerhost's hostIsolatedDriver
// demotion exists for), that is a port-forward that works about half the time,
// which is materially harder to diagnose than one that never works.
//
// Sorting also puts eth0 ahead of eth1 for free, which is the primary NIC by
// every convention incus follows, so the deterministic answer is also the one
// most likely to be routable.
func pickIPv4(network map[string]incusapi.InstanceStateNetwork) string {
	ifaces := make([]string, 0, len(network))
	for iface := range network {
		if iface != "lo" {
	...
	for _, iface := range ifaces {
		for _, a := range network[iface].Addresses {
