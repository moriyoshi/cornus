...
// ensureHealthRearmed restores probing for instances that outlived the cornus
// server, once per backend.
//
// The engine's STATE is in-memory and legitimately resets across a restart —
// Docker keeps its own in a daemon that outlives cornus — but the probing itself
// must come back: the healthcheck is recorded on the instance precisely so it can
// be re-read here. Without this an already-deployed workload would report no
// health until someone redeployed it, which is silent (it keeps running) and
// surfaces much later as a compose `depends_on: service_healthy` that hangs.
//
// Unlike containerd and bare, this backend has no startup reconcile pass to hang
// it off, so it is driven lazily from the two read paths. That is enough to arm
// itself: `service_healthy` converges by POLLING Status, so the first poll is
// what starts the probes it is waiting on.
//
// A failure to list is not consumed — the pass is retried on the next call rather
// than leaving the backend permanently unarmed.
func (b *Backend) ensureHealthRearmed() {
	b.rearmMu.Lock()
	defer b.rearmMu.Unlock()
	if b.rearmed {
		return
	}
	...
	for _, in := range insts {
		...
		if instanceApp(in) == "" {
			continue // not a cornus-managed instance
		}
		// Only a running instance. Probing a stopped one would exec into nothing
		// and report unhealthy where the pre-restart answer was "" — Stop unwatches,
		// and this pass must not undo that across a restart.
		if in.StatusCode != incusapi.Running {
			continue
		}
		// Nor one already being probed. Because this pass is triggered by a READ, it
		// can run after an Apply in the same process, and Watch restarts the loop
		// from `starting` — which would throw away a healthy verdict this server had
		// already earned. A watched instance always has a state; "" means unwatched.
		if b.health.State(in.Name) != "" {
			continue
		}
		b.health.Watch(in.Name, healthcheckFromConfig(in.Config))
	}
