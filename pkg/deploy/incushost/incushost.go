...
// WithRemote records the operator's opt-in to the caretaker-companion path
// (CORNUS_INCUS_REMOTE), mirroring containerdhost.WithRemote /
// barehost.WithRemote. Like those (and unlike dockerhost) it would not make the
// daemon reachable from another host; it only changes how client-local features
// are realized.
//
// NOTE: the companion itself is NOT implemented on this backend — accepted for
// parity so the same server wiring covers every host backend. Client-local
// mounts are ignored with a warning (spec_linux.go), ForwardPort dials the
// instance IP directly, and ssh-agent forwarding is refused outright
// (AgentForwardEnabled). See .agents/docs/JOURNAL.md, "unified companion
// caretaker" (2026-07-25).
func WithRemote(remote bool) Option {
	return func(o *options) { o.remote = remote }
}

// WithAgentImage sets the cornus-embedding image (CORNUS_AGENT_IMAGE) that a
// caretaker companion would run.
//
// NOTE: currently accepted and stored but NOT read — this backend creates no
...
//
// NOTE: currently accepted and stored but NOT read — this backend creates no
// companion, and ForwardPort dials the instance's own IP. See WithRemote.
