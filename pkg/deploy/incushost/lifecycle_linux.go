...
func (b *Backend) Apply(ctx context.Context, spec api.DeploySpec) (api.DeployStatus, error) {
	return b.apply(ctx, spec, nil)
}

// ApplyOwningCredentialDirs implements deploy.LateIDCredentialBinder.
func (b *Backend) ApplyOwningCredentialDirs(ctx context.Context, spec api.DeploySpec, dirs []string) (api.DeployStatus, error) {
	return b.apply(ctx, spec, dirs)
}

// apply is Apply with the credential directories whose ownership must be
// corrected between create and start (empty for a plain Apply).
//
// Instances are created STOPPED and started in a second pass, on ONE path rather
// than only when credentials are present. A branch here would mean the ordering
// that carries credential delivery is the one the E2E suite almost never runs,
// and a create/start split is exactly the kind of change whose failures are
// timing-shaped. It also sharpens a diagnostic: a config incusd accepts at create
// and rejects at start (limits.kernel.* is the known one) now fails with a start
// error naming the instance, instead of inside CreateInstance.
func (b *Backend) apply(ctx context.Context, spec api.DeploySpec, credDirs []string) (api.DeployStatus, error) {
	...
	// The instances now exist and are stopped, which is the ONLY window in which
	// their id map is both knowable and not yet in use. See ownCredentialDirs.
	if err := b.ownCredentialDirs(ctx, spec, credDirs, n); err != nil {
		return api.DeployStatus{}, err
	}
	for i := 0; i < n; i++ {
		name := instanceName(spec.Name, i)
		if err := b.conn.SetInstanceState(name, "start", false, 0); err != nil {
			return api.DeployStatus{}, fmt.Errorf("incus: starting instance %s: %w", name, err)
		}
		if b.remote {
	...
	// status this returns already says "starting" rather than "" for a workload
...
func (b *Backend) Start(ctx context.Context, name string) error {
	if err := b.actOnApp(ctx, name, "start", false); err != nil {
		return err
	}
	// Re-arm probing: the instances were stopped (hence unwatched), and the
	// healthcheck comes back off their config, which is what makes this work after
...
func (b *Backend) Stop(ctx context.Context, name string) error {
	// Disarm first: a probe racing the teardown would exec into an instance being
	// stopped and grade a requested stop as a health failure.
...
func (b *Backend) Restart(ctx context.Context, name string) error {
	b.unwatchHealth(ctx, name)
	if err := b.actOnApp(ctx, name, "restart", true); err != nil {
		return err
	}
	b.syncHealth(ctx, name)
	return nil
...
func (b *Backend) Delete(ctx context.Context, name string) error {
	// Disarm while the instances can still be enumerated for their ids.
	b.unwatchHealth(ctx, name)
