...
type fakeSrv struct {
	...
	consoleBody string
	consoleErr  error
	consolePost incusapi.InstanceConsolePost
	consoleArgs *incus.InstanceConsoleArgs

	// Storage volumes
	createdVolPool string
	createdVol     incusapi.StorageVolumesPost
	createVolErr   error
	deletedVol     []string
	deleteVolErr   error
...
func TestNewScopesCallsToTheConfiguredProjectAndCarriesOptions(t *testing.T) {
	...
	if !b.remote || b.agentImage != "localhost:5000/cornus:dev" || b.companions != reg || b.creds == nil {
...
// ConsoleInstanceDynamic / storage-volume stubs, so the seam adapter's own
// argument shaping is testable without an incusd.
func (s *fakeSrv) ConsoleInstanceDynamic(name string, post incusapi.InstanceConsolePost, args *incus.InstanceConsoleArgs) (incus.Operation, func(io.ReadWriteCloser) error, error) {
	s.consoleInst, s.consolePost, s.consoleArgs = name, post, args
	if s.consoleErr != nil {
		return nil, nil, s.consoleErr
	}
	return &fakeOp{}, func(rwc io.ReadWriteCloser) error {
		_, err := rwc.Write([]byte(s.consoleBody))
		return err
	}, nil
}

func (s *fakeSrv) CreateStoragePoolVolume(pool string, vol incusapi.StorageVolumesPost) error {
	s.createdVolPool, s.createdVol = pool, vol
	return s.createVolErr
}

func (s *fakeSrv) DeleteStoragePoolVolume(pool, volType, name string) error {
	s.deletedVol = []string{pool, volType, name}
	return s.deleteVolErr
}

// TestRealConnConsoleAttachForcesAndIgnoresControl pins the console attach the
// Logs --follow path depends on: it asks for the CONSOLE (not vga) device, and
// it forces the connection because Incus permits a single console client per
// instance — without Force a re-run of `cornus logs --follow` would race the
// teardown of its predecessor's session and fail.
func TestRealConnConsoleAttachForcesAndIgnoresControl(t *testing.T) {
	s := &fakeSrv{consoleBody: "live output\n"}
	c := &realConn{srv: s}
	stream, stop, err := c.ConsoleAttach("cornus-web-0")
	if err != nil {
		t.Fatalf("ConsoleAttach: %v", err)
	}
	if s.consoleInst != "cornus-web-0" {
		t.Errorf("attached to %q", s.consoleInst)
	}
	if s.consolePost.Type != "console" {
		t.Errorf("console type = %q, want %q", s.consolePost.Type, "console")
	}
	if !s.consolePost.Force {
		t.Error("console attach is not forced, so a second follow fails on the daemon's single console slot")
	}
	if s.consoleArgs == nil || s.consoleArgs.Control == nil || s.consoleArgs.ConsoleDisconnect == nil {
		t.Fatalf("console args incomplete: %+v", s.consoleArgs)
	}
	// The control handler is accepted and ignored (cornus never resizes a log
	// console); calling it must not panic.
	s.consoleArgs.Control(nil)

	var sink strings.Builder
	if err := stream(nopRWC{Writer: &sink}); err != nil {
		t.Fatalf("stream: %v", err)
	}
	if sink.String() != "live output\n" {
		t.Errorf("streamed %q", sink.String())
	}

	// stop must close the disconnect channel, and must be safe to call twice —
	// Logs calls it both from its deferred teardown and from its ctx watcher.
	stop()
	stop()
	select {
	case <-s.consoleArgs.ConsoleDisconnect:
	default:
		t.Error("stop did not signal a console disconnect")
	}

	c = &realConn{srv: &fakeSrv{consoleErr: errors.New("console is already connected")}}
	if _, _, err := c.ConsoleAttach("cornus-web-0"); err == nil {
		t.Fatal("an unavailable console must surface as an error")
	}
}

// nopRWC adapts a writer to the io.ReadWriteCloser a console stream takes.
type nopRWC struct{ io.Writer }

func (nopRWC) Read([]byte) (int, error) { return 0, io.EOF }
func (nopRWC) Close() error             { return nil }

// TestRealConnCreateVolumeIsCreateIfAbsent pins the companion's volume
// provisioning: a custom FILESYSTEM volume (a block volume could not be mounted
// by two instances at once), and an already-existing volume is success, since
// re-applying a deployment must reuse the volume it created last time rather
// than failing on a name clash with itself.
func TestRealConnCreateVolumeIsCreateIfAbsent(t *testing.T) {
	s := &fakeSrv{}
	c := &realConn{srv: s}
	if err := c.CreateVolume("default", "cornus-web-0-agent", map[string]string{"security.shifted": "true"}); err != nil {
		t.Fatalf("CreateVolume: %v", err)
	}
	if s.createdVolPool != "default" {
		t.Errorf("created in pool %q", s.createdVolPool)
	}
	if s.createdVol.Name != "cornus-web-0-agent" || s.createdVol.Type != "custom" || s.createdVol.ContentType != "filesystem" {
		t.Errorf("volume request = %+v, want a custom filesystem volume", s.createdVol)
	}
	if s.createdVol.Config["security.shifted"] != "true" {
		t.Errorf("volume config not forwarded: %v", s.createdVol.Config)
	}

	c = &realConn{srv: &fakeSrv{createVolErr: incusapi.StatusErrorf(409, "Volume already exists")}}
	if err := c.CreateVolume("default", "cornus-web-0-agent", nil); err != nil {
		t.Fatalf("an existing volume must be success, got %v", err)
	}

	c = &realConn{srv: &fakeSrv{createVolErr: errors.New("no such storage pool")}}
	if err := c.CreateVolume("nope", "cornus-web-0-agent", nil); err == nil {
		t.Fatal("a real failure must propagate")
	}
}

// TestRealConnDeleteVolumeIsDeleteIfExists pins the mirror-image contract on
// teardown: Delete must stay a no-op success for a deployment that never had a
// companion (or whose pool is gone), or every non-remote delete would start
// failing on a volume that was never created.
func TestRealConnDeleteVolumeIsDeleteIfExists(t *testing.T) {
	s := &fakeSrv{}
	c := &realConn{srv: s}
	if err := c.DeleteVolume("default", "cornus-web-0-agent"); err != nil {
		t.Fatalf("DeleteVolume: %v", err)
	}
	if want := []string{"default", "custom", "cornus-web-0-agent"}; len(s.deletedVol) != 3 ||
		s.deletedVol[0] != want[0] || s.deletedVol[1] != want[1] || s.deletedVol[2] != want[2] {
		t.Errorf("delete args = %v, want %v", s.deletedVol, want)
	}

	c = &realConn{srv: &fakeSrv{deleteVolErr: notFound()}}
	if err := c.DeleteVolume("default", "ghost"); err != nil {
		t.Fatalf("a missing volume must be success, got %v", err)
	}

	c = &realConn{srv: &fakeSrv{deleteVolErr: errors.New("volume is still attached")}}
	if err := c.DeleteVolume("default", "busy"); err == nil {
		t.Fatal("a real failure must propagate")
	}
}
