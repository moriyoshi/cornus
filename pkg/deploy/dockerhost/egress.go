...
func (b *Backend) planEgress(spec api.DeploySpec, egress *deploy.AttachEgress) (api.DeploySpec, companionPlan, error) {
	...
	if err := e.Validate(); err != nil {
		return spec, companionPlan{}, fmt.Errorf("dockerhost: %w", err)
	}
	if e.Mode != "proxy" && e.Mode != "transparent" {
		return spec, companionPlan{}, fmt.Errorf("dockerhost: client-side egress mode %q is not supported (want %q or %q)", e.Mode, "proxy", "transparent")
	}
	if egress.AgentImage == "" {
		return spec, companionPlan{}, fmt.Errorf("dockerhost: client-side egress needs the cornus agent image (set CORNUS_AGENT_IMAGE on the server)")
	}
	port := e.ListenPort
	if port == 0 {
	...
	if e.Mode == "transparent" {
		...
		plan.capAdd = []string{"NET_ADMIN"}
	}
	return appSpec, plan, nil
}
