...
type engineClient struct {
	...
	hostHeader string
	// overTCP records that DOCKER_HOST named the daemon over TCP rather than a
	// unix socket — the only configuration in which the daemon may not be on this
	// machine at all. See the ForwardPort hint.
	overTCP bool
}

// dockerDefaultEndpoint is where a Docker daemon listens when DOCKER_HOST says
// nothing.
const dockerDefaultEndpoint = "unix:///var/run/docker.sock"

// newEngineClient builds a client from DOCKER_HOST (default
// unix:///var/run/docker.sock).
//
// Endpoint parsing and dialing live in pkg/runtimeendpoint, shared with the two
// other hand-rolled Docker clients in the tree (pkg/registry's re-export source
// and pkg/build/builderctr). What stays here is what is genuinely this client's
// own: the unbounded timeout streaming pulls need, and the OpenTelemetry
// wrapping the other two do not do.
func newEngineClient() (*engineClient, error) {
	ep, err := runtimeendpoint.Parse(os.Getenv("DOCKER_HOST"), dockerDefaultEndpoint)
	if err != nil {
		return nil, fmt.Errorf("DOCKER_HOST: %w", err)
	}
	return &engineClient{
		// A nil Transport from the endpoint means http.DefaultTransport, which is
		// what a TCP daemon should get; otelTransport passes that through unchanged
...
func (c *engineClient) doWithHeaders(ctx context.Context, method, path string, body any, headers http.Header) (*http.Response, error) {
	var rdr io.Reader
	if body != nil {
...
func (c *engineClient) imagePull(ctx context.Context, ref string, credential *deploy.RegistryCredential) error {
	...
	resp, err := c.doWithHeaders(ctx, http.MethodPost, "/images/create?"+q.Encode(), nil, headers)
	if err != nil {
		return err
	}
...
func (c *engineClient) imageExists(ctx context.Context, ref string) (bool, error) {
	_, present, err := c.imageInspect(ctx, ref)
	return present, err
}

// imageInspect resolves ref against the daemon (GET /images/{ref}/json),
// returning the image's CONTENT id (the `Id` field, a digest of the image
// config) and whether the daemon has the image at all. A 404 is reported as
// present=false with no error; any other failure is an error, so a caller can
// distinguish "definitely absent" from "could not tell".
//
// The content id is what makes the idempotent-apply fingerprint safe against a
// MUTABLE tag: `compose up --build` rebuilds the same ref with different bytes,
// and a comparison on the ref alone would leave the old container running.
func (c *engineClient) imageInspect(ctx context.Context, ref string) (id string, present bool, err error) {
...
// networkJoin attaches a container to a network with NO endpoint configuration
// (POST /networks/{name}/connect). It is the plain counterpart of
// networkConnect, for attaching a container that is not a member of the
// deployment — cornus's own server container, which joins a workload's network
// only to gain a route to it (selfnet.go). Passing the attachment's DNS aliases
// there would publish the workload's own names onto the server, so this one
// deliberately sends nothing but the container id.
//
// An already-attached container is success, not an error: Docker answers a
// repeat connect with 403 "endpoint with name X already exists in network Y",
// and every apply of the same deployment re-runs this. Only that specific 403 is
// swallowed — a 403 from anything else (an authorizing proxy in front of the
// socket, say) still surfaces.
func (c *engineClient) networkJoin(ctx context.Context, netName, containerID string) error {
	resp, err := c.do(ctx, http.MethodPost, "/networks/"+url.PathEscape(netName)+"/connect",
...
// networkLeave detaches a container from a network (POST
// /networks/{name}/disconnect). A container that is not attached is success:
// Docker reports that as a 500 whose message says so, and the caller
// (reapNetwork) is asking for an end state, not for an event to have happened.
func (c *engineClient) networkLeave(ctx context.Context, netName, containerID string) error {
	resp, err := c.do(ctx, http.MethodPost, "/networks/"+url.PathEscape(netName)+"/disconnect",
		map[string]any{"Container": containerID})
	if err != nil {
		return err
	}
	...
	if resp.StatusCode == http.StatusInternalServerError {
		b, _ := io.ReadAll(io.LimitReader(resp.Body, 8<<10))
		if strings.Contains(string(b), "is not connected to network") {
			return nil
		}
		return fmt.Errorf("docker api: %s: %s", resp.Status, strings.TrimSpace(string(b)))
	}
	return checkStatus(resp, http.StatusOK, http.StatusNotFound)
}

// networkInspect returns a network's labels and the ids of the containers
// currently attached (GET /networks/{name}).
//
// The member IDS rather than their count, because the GC decision is not "is
// anyone attached" but "is anyone attached OTHER than this cornus server" — an
// in-container server that joined the network to route to the workload is a
// member of every network it ever deployed onto, and counting it would make
// `docker compose down`-style network reaping stop working entirely.
func (c *engineClient) networkInspect(ctx context.Context, name string) (labels map[string]string, members []string, err error) {
...
// networkDriver returns a network's Docker driver ("bridge", "macvlan", ...).
// A network that no longer exists reports "", which callers must treat as
// "unknown", never as a driver with properties.
func (c *engineClient) networkDriver(ctx context.Context, name string) (string, error) {
	resp, err := c.do(ctx, http.MethodGet, "/networks/"+url.PathEscape(name), nil)
	if err != nil {
		return "", err
	}
...
type containerSummary struct {
	...
	Image string
	State string
	// Names are the daemon's container names (each with a leading "/"). Instances
	// are named `cornus-<app>-<i>`, so the name is what carries the replica
	// ordinal — the daemon's listing order does not.
	Names  []string
	Labels map[string]string
}
...
func (c *engineClient) containerList(ctx context.Context, label string) ([]containerSummary, error) {
	...
	for _, r := range raw {
		out = append(out, containerSummary{ID: r.Id, Image: r.Image, State: r.State, Names: r.Names, Labels: r.Labels})
	}
	return out, nil
}
...
//
// reachable, when non-empty, names the networks the CALLER has a route to — the
// networks this cornus's own container is attached to, for a server running in a
// container of its own. Docker's inter-network isolation rules drop traffic
// between two bridge networks, so on such a server "the container's IP" is not
// one address but a choice, and only an address on a shared network can be
// dialed. Empty means the caller shares the host's network view (a server
// process on the host, or one in the host's netns), where every bridge is
// routable and the historical preference order applies.
// It is a thin wrapper over containerAddresses + selectIP, kept because the
// two-step form is what ForwardPort needs (it must narrow the reachable set from
// the very same inspect, rather than pay a second one per connection).
func (c *engineClient) containerIP(ctx context.Context, id string, reachable map[string]bool) (string, error) {
	nets, bridgeIP, err := c.containerAddresses(ctx, id)
	if err != nil {
		return "", err
	}
...
// noRouteError is the one verdict for "this container has no address I can
// dial", stated once so the ForwardPort path and containerIP cannot drift.
func noRouteError(id string) error {
	return fmt.Errorf("container %s has no IP address (not running, or on a network the server cannot route to)", id)
}

// containerAddresses returns a container's per-network addresses (name -> IPv4,
// including endpoints with no address yet) and the top-level default-bridge
// IPAddress, from one GET /containers/{id}/json.
//
// Both halves are returned raw, with no preference applied, because the caller
// is what knows which networks it can reach — and because ForwardPort runs once
// per CONNECTION, so the alternative (inspect once to learn the networks, then
// again to resolve an address) would double the daemon traffic of every forward.
func (c *engineClient) containerAddresses(ctx context.Context, id string) (map[string]string, string, error) {
	...
	return networks, raw.NetworkSettings.IPAddress, nil
}

// selectIP chooses the address to dial from a container's per-network addresses
// and its top-level default-bridge one, given the networks the caller can reach
// (nil/empty = the host's view, where everything is routable). "" means no
// dialable address exists.
//
// The top-level IPAddress is the default bridge's, and it is only the right
// answer when the caller can route to the default bridge. Consulting it before
// the reachable set would hand an isolated server the one address it provably
// cannot reach whenever a workload is on both.
func selectIP(networks map[string]string, bridgeIP string, reachable map[string]bool) string {
	if len(reachable) == 0 {
		if bridgeIP != "" {
			return bridgeIP
		}
	} else if _, ok := networks[defaultBridge]; !ok && reachable[defaultBridge] {
		if bridgeIP != "" {
			return bridgeIP
		}
	}
	return pickNetworkIP(networks, reachable)
}

// containerNetworks returns the networks a container is attached to AND
// currently addressed on (GET /containers/{id}/json, NetworkSettings.Networks),
// as name -> IPv4 address. Used for cornus's own container, to learn which
// workload networks it can route to, and for a workload's, to learn which
// networks it would have to join to reach it — see selfnet.go.
//
// Unaddressed endpoints are dropped, and that is the load-bearing part. A
// STOPPED container still reports every network it was created on, each with an
// empty address (measured). Keeping those would make "this workload is not
// running" indistinguishable from "this workload is on a network I cannot see",
// and the on-demand join would answer the first by attaching the server to
// networks it has no reason to be on.
//
// A container in another container's netns reports NetworkMode "container:<id>"
// and carries no Networks of its own; that yields an empty map here, which
// callers read as "nothing known" rather than "attached to nothing".
func (c *engineClient) containerNetworks(ctx context.Context, id string) (map[string]string, error) {
	resp, err := c.do(ctx, http.MethodGet, "/containers/"+id+"/json", nil)
	if err != nil {
		return nil, err
	}
	...
	out := make(map[string]string, len(raw.NetworkSettings.Networks))
	for name, n := range raw.NetworkSettings.Networks {
		if n.IPAddress != "" {
...
type containerInspectResult struct {
	...
	ExitCode int
	Running  bool
	// Pid is the container's init process id AS THE DAEMON SEES IT, so it is a
	// pid in the daemon's namespace and means nothing in a /proc this server
	// cannot resolve. Zero when the container is not running. Used to reach the
	// container's network namespace (/proc/<Pid>/ns/net) — see
	// credential_endpoint.go, which does not trust it without checking.
	Pid int
}
...
func (c *engineClient) containerInspect(ctx context.Context, id string) (containerInspectResult, error) {
	...
	res := containerInspectResult{ExitCode: raw.State.ExitCode, Running: raw.State.Running, Pid: raw.State.Pid}
...
//
// reachable, when non-empty, restricts the choice to networks the caller is
// itself attached to (see containerIP). Within that set the same lexical tie-break
// applies, and "bridge" loses its blanket preference — it is preferred only if
// the caller is on it, because for a containerized server the default bridge is
// frequently the ONE network the workload is on that the server cannot see.
// Returns "" when the sets do not intersect, which is a real answer: it is what
// "no route to this workload" looks like, and the caller turns it into an error
// that says so rather than dialing an address that will time out.
func pickNetworkIP(networks map[string]string, reachable map[string]bool) string {
	if len(reachable) == 0 {
		if ip := networks["bridge"]; ip != "" {
			return ip
		}
	} else if reachable["bridge"] && networks["bridge"] != "" {
		return networks["bridge"]
	}
	names := make([]string, 0, len(networks))
	for name, ip := range networks {
		if ip == "" || (len(reachable) > 0 && !reachable[name]) {
			continue
		}
...
// nonLocal implements Engine: it reports the overTCP signal recorded at
// construction. See the field comment for why a TCP endpoint is the only
// available hint and what it does (and does not) prove.
func (c *engineClient) nonLocal() bool { return c.overTCP }

// usernsRemapped reads Docker's /info SecurityOptions for the userns entry.
//
// Docker advertises the FACT of remapping there but not the mapping itself,
// which lives in /etc/subuid for the remap user. So this is only ever used to
// refuse, never to translate.
func (c *engineClient) usernsRemapped(ctx context.Context) (bool, error) {
	...
	for _, o := range out.SecurityOptions {
		if strings.Contains(o, "name=userns") {
			return true, nil
		}
	}
	return false, nil
...
// execStart implements Engine: it hijacks POST /exec/{id}/start and returns the
// raw stdio stream. The Detach/Tty body is Docker's exec-start payload; for a
// non-TTY exec the process output arrives stdcopy-framed, which is what the
// deploy.Backend contract requires callers to be able to assume.
func (c *engineClient) execStart(ctx context.Context, execID string, tty bool) (net.Conn, error) {
	...
	return c.hijack(ctx, http.MethodPost, "/exec/"+execID+"/start", body)
}

// containerAttach implements Engine: it hijacks POST /containers/{id}/attach
// with the requested stream selection and returns the raw stdio stream.
//
// Docker reads these as "1"/"0" rather than presence, so every flag is written
// explicitly — a bare omission would take the daemon's default rather than the
// caller's false, which for stdin is the difference between an interactive
// session and a silently read-only one.
func (c *engineClient) containerAttach(ctx context.Context, id string, cfg api.AttachConfig) (net.Conn, error) {
	q := url.Values{}
	...
	return c.hijack(ctx, http.MethodPost, "/containers/"+id+"/attach?"+q.Encode(), nil)
}
