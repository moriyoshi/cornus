...
func (b *Backend) startTelemetryCompanion(ctx context.Context, name, appID string, replica int, role otelcollector.Config, relay *deploy.TelemetryRelay, proxyEnv []string) error {
	...
	if relay != nil {
		...
		if serverURL == "" {
			return b.errf("telemetry viaMux requires CORNUS_ADVERTISE_URL (the cornus URL the companion dials)")
		}
	...
	if err != nil {
		return b.errf("marshal telemetry caretaker config: %w", err)
	}
	body := createBody{
		Image: b.agentImage,
	...
	if err != nil {
		return b.errf("create telemetry caretaker: %w", err)
	}
