...
func TestPickNetworkIPDeterministic(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		if got := pickNetworkIP(multi, nil); got != "172.17.0.3" {
	...
	for i := 0; i < 100; i++ {
		if got := pickNetworkIP(noBridge, nil); got != "10.0.1.1" {
	...
	if got := pickNetworkIP(emptyBridge, nil); got != "10.0.1.1" {
		t.Fatalf("pickNetworkIP(empty bridge) = %q, want 10.0.1.1", got)
	}

	if got := pickNetworkIP(map[string]string{}, nil); got != "" {
