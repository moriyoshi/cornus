...
// netConnectBody is the POST /networks/{name}/connect body, as the fake decodes
// it. EndpointConfig is a pointer so a test can tell "absent" (a bare join)
...
type fakeDocker struct {
	...
	connects        []string
	netRemoved      []string
	// attached is the daemon's real endpoint table for containers the backend
	// connected explicitly (network -> set of container ids), maintained by the
	// connect/disconnect handlers. It exists alongside the label-derived
	// membership because cornus's OWN container joins a workload's network
	// WITHOUT carrying the deployment's cornus.networks label — modelling
	// membership from labels alone would make the server's endpoint invisible to
	// the GC path under test. disconnects records "net:containerID".
	attached    map[string]map[string]bool
	disconnects []string
	// connectBodies keeps every decoded POST /networks/{name}/connect body, so a
	// test can assert on what rode along with an attachment (the server's own
	// join must carry no endpoint config).
	connectBodies []netConnectBody
	// refuseConnectFor makes the daemon reject connects for one container id,
	// modelling a daemon that will not let cornus attach itself.
	refuseConnectFor string
	// startErr, when set, is the message every POST /containers/{id}/start
	// answers with (as a 500).
	startErr string
	// netDrivers gives a network's Docker driver ("bridge", "macvlan", ...) for
	// GET /networks/{name}; absent means "bridge". netInspects counts those GETs,
	// which is how a test sees whether the driver lookup is cached.
	netDrivers  map[string]string
	netInspects int
	// ctrInspects counts GET /containers/{id}/json. ForwardPort runs once per
	// CONNECTION, so this is the number a routing change must not quietly grow.
	ctrInspects int
	...
	// images marks refs the daemon already has locally BEFORE any pull (for GET
	// /images/{ref}/json). A ref that has since been pulled is present too — see
	// the handler — because a daemon that pulled an image has it, and the
	// idempotent-apply path inspects the image right after pulling it.
	images map[string]bool
	// imageIDs overrides the content id GET /images/{ref}/json reports for a ref.
	// It is what lets a test model the case the image-content half of the apply
	// fingerprint exists for: the SAME mutable tag now naming different bytes
	// (`compose up --build`). Unset refs get a deterministic id derived from the
	// ref, so a tag that did not change reports the same id twice.
	imageIDs map[string]string
	// statsFor records, in order, the container ids GET /containers/{id}/stats
	// was called for — how a test sees WHICH replica a Stats call resolved to.
	statsFor []string
	// statsBody overrides the stats response body, and statsBodySet says it was
	// set — the two are separate because "" is a MEANINGFUL value here: an empty
	// 200 body is exactly what dockerd returns for a container that has no
	// reading yet, and it is the case the collector most needs to get right.
	statsBody    string
	statsBodySet bool
}

// appCreates counts container-create calls for APP containers, i.e. excluding
// companions (which carry cornus.role). The idempotence tests count what an
// apply did to the WORKLOAD, and a spec whose change also spawns a collector or
// caretaker companion would otherwise inflate the count for a reason unrelated
// to the claim under test.
func (f *fakeDocker) appCreates() int {
	f.mu.Lock()
	defer f.mu.Unlock()
	n := 0
...
// removals counts container DELETEs seen so far.
func (f *fakeDocker) removals() int {
	f.mu.Lock()
	defer f.mu.Unlock()
	return len(f.removed)
}

// networkMembers returns the ids of the containers attached to net: those whose
// cornus.networks label names it, plus anything explicitly connected through
// POST /networks/{net}/connect and not since disconnected.
func (f *fakeDocker) networkMembers(net string) []string {
...
func (f *fakeDocker) handler() http.Handler {
	...
	mux.HandleFunc("/images/", func(w http.ResponseWriter, r *http.Request) {
		...
		f.mu.Lock()
		// A daemon that PULLED an image has it. Modelling that matters: the
		// idempotent-apply path resolves the image's content id right after the
		// pull, and a fake that 404'd there would silently disable the fast path in
		// every test that does not pre-seed `images` — the tests would then pass
		// while observing the pre-fix behaviour.
		present := f.images[ref]
		name, tag := splitRefTag(ref) // the same normalization /images/create records
	...
	mux.HandleFunc("/networks/", func(w http.ResponseWriter, r *http.Request) {
		...
		case action == "connect" && r.Method == http.MethodPost:
			var body netConnectBody
			_ = json.NewDecoder(r.Body).Decode(&body)
			f.connects = append(f.connects, name+":"+body.Container)
			f.connectBodies = append(f.connectBodies, body)
			if f.refuseConnectFor != "" && body.Container == f.refuseConnectFor {
				w.WriteHeader(http.StatusForbidden)
				_, _ = w.Write([]byte(`{"message":"not permitted"}`))
				return
			}
			// dockerd answers a repeat connect with 403 "endpoint ... already
			// exists", which the backend has to treat as success; the fake must
			// reproduce that or a re-apply would look fine here and fail live.
			if f.attached[name][body.Container] {
				w.WriteHeader(http.StatusForbidden)
				_, _ = w.Write([]byte(`{"message":"endpoint with name x already exists in network ` + name + `"}`))
				return
			}
			if f.attached == nil {
				f.attached = map[string]map[string]bool{}
			}
			if f.attached[name] == nil {
				f.attached[name] = map[string]bool{}
			}
			f.attached[name][body.Container] = true
			w.WriteHeader(http.StatusOK)
		case action == "disconnect" && r.Method == http.MethodPost:
			...
			_ = json.NewDecoder(r.Body).Decode(&body)
			f.disconnects = append(f.disconnects, name+":"+body.Container)
			// And a disconnect of something not attached is dockerd's 500, the
			// other status the backend must read as "already in the end state".
			if !f.attached[name][body.Container] {
				w.WriteHeader(http.StatusInternalServerError)
				_, _ = w.Write([]byte(`{"message":"container ` + body.Container + ` is not connected to network ` + name + `"}`))
				return
			}
			delete(f.attached[name], body.Container)
			w.WriteHeader(http.StatusOK)
		case action == "" && r.Method == http.MethodGet:
			...
			for _, id := range f.networkMembers(name) {
				containers[id] = map[string]any{}
			}
			driver := f.netDrivers[name]
			if driver == "" {
				driver = "bridge"
			}
			_ = json.NewEncoder(w).Encode(map[string]any{"Labels": labels, "Containers": containers, "Driver": driver})
	...
	mux.HandleFunc("/containers/", func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(id, "/json") && r.Method == http.MethodGet:
			...
			for n, members := range f.attached {
				if members[cid] {
					attachedTo[n] = true
				}
			}
			...
			// Per-network addresses. dockerd reports the UNION of the networks a
			// container was created on and the ones it has since been connected
			// to, and the union is what matters here: a test that pinned "nets"
			// and ignored later connects would show a container never joining
			// anything, which is the very thing the join paths are tested for.
			nets := map[string]any{}
			if m, ok := c["nets"].(map[string]string); ok {
				for n, addr := range m {
					nets[n] = map[string]any{"IPAddress": addr}
				}
			}
			for n := range attachedTo {
				if _, ok := nets[n]; !ok {
					// A joined-but-unaddressed endpoint would be indistinguishable
					// from "no address"; give it one derived from the network name
					// so a test can tell WHICH network an address came from.
					nets[n] = map[string]any{"IPAddress": "10.0.0." + strconv.Itoa(len(n))}
				}
			}
			_ = json.NewEncoder(w).Encode(map[string]any{
				"NetworkSettings": map[string]any{"IPAddress": ip, "Networks": nets},
				"State":           state,
			})
		case strings.HasSuffix(id, "/stats"):
			cid := strings.TrimSuffix(id, "/stats")
			f.mu.Lock()
			f.statsFor = append(f.statsFor, cid)
			body, set := f.statsBody, f.statsBodySet
			f.mu.Unlock()
			if !set {
				// The default body is opaque to the backend (Stats copies bytes
				// through), so a marker naming the container is enough and makes a
				// mix-up legible.
				body = `{"id":"` + cid + `"}` + "\n"
			}
		...
		case strings.HasSuffix(id, "/stop"):
			f.mu.Lock()
			f.stopped = append(f.stopped, strings.TrimSuffix(id, "/stop"))
			f.mu.Unlock()
			w.WriteHeader(http.StatusNoContent)
		case strings.HasSuffix(id, "/start"):
			cid := strings.TrimSuffix(id, "/start")
			f.mu.Lock()
			// startErr models a daemon that refuses to start any container, for
			// the failures that surface at START rather than at create — network
			// address-pool exhaustion being the one this exists for.
			if f.startErr != "" {
				f.mu.Unlock()
				w.WriteHeader(http.StatusInternalServerError)
				_, _ = w.Write([]byte(`{"message":"` + f.startErr + `"}`))
				return
			}
...
// TestApplyIsIdempotent is the headline of the compose-idempotence contract: a
// second Apply of a spec that has not changed must not touch the workload at all
// — no remove, no create, and no start of an already-running container.
//
// This test used to assert the OPPOSITE (`removed = 1`, "want exactly 1 on
// recreate") under the same name, which is a fair record of how long the
// divergence from `docker compose up` went unquestioned.
func TestApplyIsIdempotent(t *testing.T) {
	...
	f.mu.Lock()
	created, started := len(f.created), len(f.started)
	f.mu.Unlock()
	if created != 1 || started != 1 {
		t.Fatalf("first apply created %d and started %d containers, want 1 of each", created, started)
	}
	...
	f.mu.Lock()
	removedAfter, createdAfter, startedAfter := len(f.removed), len(f.created), len(f.started)
	f.mu.Unlock()
	if removedAfter != 0 {
		t.Errorf("re-applying an unchanged spec removed %d container(s), want 0: `docker compose up` recreates only what changed", removedAfter)
	}
	if createdAfter != created {
		t.Errorf("re-applying an unchanged spec created %d more container(s), want 0", createdAfter-created)
	}
	if startedAfter != started {
		t.Errorf("re-applying an unchanged spec started %d already-running container(s), want 0 — a no-op apply must not bounce the workload", startedAfter-started)
	}
...
func TestImagePullReportsInStreamError(t *testing.T) {
	...
	err = c.imagePull(context.Background(), "reg/web:v9", nil)
...
func TestImagePullSucceedsOnCleanStream(t *testing.T) {
	...
	if err := c.imagePull(context.Background(), "reg/web:v1", nil); err != nil {
		t.Fatalf("imagePull on a clean stream = %v, want nil", err)
	}
}

func TestImagePullSetsRegistryAuthHeader(t *testing.T) {
	var got deploy.RegistryCredential
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	want := deploy.RegistryCredential{Username: "cornus-internal", Password: "short-lived-token"}
	if err := c.imagePull(context.Background(), "reg/web:v1", &want); err != nil {
		t.Fatal(err)
	}
	if got != want {
		t.Fatalf("X-Registry-Auth credential = %+v, want %+v", got, want)
	}
}
...
// TestStatsSelectsInstance pins that Stats resolves opts.Instance rather than
// always taking the first container.
//
// The ordering has to come from the container NAMES, not from the daemon's
// listing order: /containers/json has no specified order (the fake iterates a
// map, which is deliberately randomized), so a backend that trusted it would
// hand replica 2's numbers back for replica 0 on some runs and not others. The
// name carries the ordinal; the listing order carries nothing.
func TestStatsSelectsInstance(t *testing.T) {
	f := &fakeDocker{containers: map[string]map[string]any{}}
	for _, suffix := range []string{"-1", "-2", "-3"} {
		id := "id-web" + suffix
		f.containers[id] = map[string]any{
			"Id":     id,
			"Names":  []string{"/web" + suffix},
			"State":  "running",
	...
	for idx, want := range map[int]string{0: "id-web-1", 1: "id-web-2", 2: "id-web-3"} {
		f.statsFor = nil
		if err := b.Stats(context.Background(), "web", api.StatsOptions{Instance: idx}, io.Discard); err != nil {
			t.Fatalf("Stats(instance=%d): %v", idx, err)
		}
		if len(f.statsFor) != 1 || f.statsFor[0] != want {
			t.Errorf("Stats(instance=%d) sampled %v, want [%s]", idx, f.statsFor, want)
		}
	}

	// Past the end is ErrNotFound, never a silent fallback to instance 0 — a
	// collector enumerating replicas has to be able to tell a departed replica
	// from a different one's numbers.
	f.statsFor = nil
	err := b.Stats(context.Background(), "web", api.StatsOptions{Instance: 3}, io.Discard)
	if !errors.Is(err, deploy.ErrNotFound) {
		t.Errorf("Stats(instance=3) error = %v, want deploy.ErrNotFound", err)
	}
	if len(f.statsFor) != 0 {
		t.Errorf("an out-of-range instance still sampled %v", f.statsFor)
	}
}

// TestSampleMetricsOnAContainerWithNoStats pins how a container that has no
// reading to give is reported.
//
// Docker signals it two different ways depending on how far along the container
// is: an empty 200 body (which decodes as io.EOF) for one that has not started,
// and a well-formed but entirely zeroed frame for one that has stopped. Both are
// "no sample", and both must surface as ErrNotFound so a per-replica collector
// skips that instance quietly.
//
// The empty-body case is not hypothetical: it is what an E2E caught within
// seconds of a real three-replica deploy, where sampling raced container
// creation and every normal startup looked like three backend failures.
func TestSampleMetricsOnAContainerWithNoStats(t *testing.T) {
	for _, tc := range []struct {
		name string
		body string
	}{
		{"empty body (not started yet)", ""},
		{"zeroed frame (stopped)", `{"read":"0001-01-01T00:00:00Z"}` + "\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			f := &fakeDocker{containers: map[string]map[string]any{
				"id-web-1": {
					"Id": "id-web-1", "Names": []string{"/web-1"}, "State": "running",
					"Labels": map[string]string{deploy.LabelApp: "web"},
				},
			}}
			f.statsBody, f.statsBodySet = tc.body, true
			b := newTestBackend(t, f)

			_, err := b.SampleMetrics(context.Background(), "web", 0)
			if !errors.Is(err, deploy.ErrNotFound) {
				t.Errorf("SampleMetrics = %v, want deploy.ErrNotFound so the collector skips rather than counting a failure", err)
			}
		})
	}
}

// A real frame must still decode into the neutral sample.
func TestSampleMetricsDecodesARealFrame(t *testing.T) {
	f := &fakeDocker{containers: map[string]map[string]any{
		"id-web-1": {
			"Id": "id-web-1", "Names": []string{"/web-1"}, "State": "running",
			"Labels": map[string]string{deploy.LabelApp: "web"},
	...
	f.statsBodySet = true
	f.statsBody = `{"read":"2026-07-26T10:00:00Z","cpu_stats":{"cpu_usage":{"total_usage":9000000000,"usage_in_usermode":6000000000,"usage_in_kernelmode":3000000000}},"memory_stats":{"usage":2048,"limit":8192,"stats":{"inactive_file":48}},"pids_stats":{"current":7},"networks":{"eth0":{"rx_bytes":10,"tx_bytes":20}}}` + "\n"
	b := newTestBackend(t, f)
	...
	if s.CPUTime == nil || s.CPUTime.User != 6000000000 {
		t.Errorf("CPUTime = %+v, want user 6e9", s.CPUTime)
	}
	if s.MemUsage != 2000 {
		t.Errorf("MemUsage = %d, want 2000 (usage less the page cache)", s.MemUsage)
	}
