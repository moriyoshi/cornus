...
func (b *Backend) planMounts(ctx context.Context, spec api.DeploySpec, mounts []deploy.AttachMount) (api.DeploySpec, [][]mountSpec, []companionPlan, error) {
	for _, m := range mounts {
		if m.AgentImage == "" {
			return spec, nil, nil, b.errf("client-local mounts via the sidecar path need the cornus agent image (set CORNUS_AGENT_IMAGE on the server)")
		}
	}
	...
	for i := 0; i < replicas; i++ {
		...
		for mi, m := range mounts {
			...
			if err := b.api.volumeEnsure(ctx, api.VolumeSpec{Name: volName}); err != nil {
				return spec, nil, nil, b.errf("create mount volume: %w", err)
			}
			mp, err := b.api.volumeInspect(ctx, volName)
			if err != nil {
				return spec, nil, nil, b.errf("inspect mount volume: %w", err)
			}
...
func (b *Backend) startCompanion(ctx context.Context, name, appID string, replica int, plan companionPlan) error {
	...
	if agentImage == "" {
		return b.errf("a companion caretaker is needed but no agent image is configured (set CORNUS_AGENT_IMAGE)")
	}
	...
	if b.remote {
		...
		if serverURL == "" {
			return b.errf("remote mode requires CORNUS_ADVERTISE_URL (the cornus URL the remote companion dials)")
		}
