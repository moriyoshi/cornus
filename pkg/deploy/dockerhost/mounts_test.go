...
// TestCompanionStoppedBeforeRemoval proves a companion is SIGTERMed and allowed
// to exit before the force remove, rather than being killed outright.
//
// This is not cosmetic. A companion holding a client-local mount performed the
// kernel 9P mount inside its own rshared view of a shared Docker volume, which
// puts that mount in a shared peer group with the HOST's view of the same
// volume. `DELETE /containers/{id}?force=1` SIGKILLs the process, so
// runMountStream's deferred deploywire.Unmount9P never runs and a live 9P mount
// is stranded at /var/lib/docker/volumes/<vol>/_data with its backing socket
// gone. Every later apply of that deployment then dies at container create with
// `invalid mount config for type "bind": stat ...: input/output error`, and
// clearing it needs root on the daemon host — so a single teardown permanently
// breaks the deployment until an operator intervenes.
//
// The app container is deliberately NOT graceful-stopped here: it has no such
// teardown obligation and Apply's recreate path should stay fast.
func TestCompanionStoppedBeforeRemoval(t *testing.T) {
	f := &fakeDocker{}
	b := newRemoteTestBackend(t, f)
	...
	compID := "id-cornus-web-mount-0"
	if err := b.Delete(ctx, "web"); err != nil {
		t.Fatalf("Delete: %v", err)
	}

	stoppedAt := indexOf(f.stopped, compID)
	if stoppedAt < 0 {
		t.Fatalf("companion %q was never stopped before removal; stopped=%v", compID, f.stopped)
	}
	if indexOf(f.removed, compID) < 0 {
		t.Fatalf("companion %q was never removed; removed=%v", compID, f.removed)
	}
	// The app instance keeps its force-remove semantics.
	if got := indexOf(f.stopped, "id-"+instanceName("web", 0)); got >= 0 {
		t.Errorf("app instance was graceful-stopped at %d; only companions need that", got)
	}
}

// indexOf returns the position of v in list, or -1.
