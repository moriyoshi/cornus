...
// PodmanSelfInspector is SelfInspector for the podman flavor.
//
// Separate rather than a parameter on SelfInspector because the two resolve
// their endpoints by entirely different rules: Docker's comes from DOCKER_HOST
// with a stock default, while podman's is an explicit operator choice that fails
// closed when unset (see resolvePodmanAccess). Folding them together would put a
// fail-closed path behind a function whose Docker caller expects a default.
//
// Like SelfInspector, it performs no I/O until hostenv calls it, and hostenv
// only calls it when the process looks containerized.
func PodmanSelfInspector() (hostenv.Inspector, error) {
	access, err := resolvePodmanAccess(podmanEnvOrOS)
	if err != nil {
		return nil, err
	}
	if access.Spawn {
		// A self-spawned service exists only for a Backend that started it; the
		// preflight has no such handle and must not start a second one just to ask
		// a question about this process's identity.
		return nil, fmt.Errorf("podman self-inspection needs %s; %s=1 starts a service per Backend, not for preflight",
			envPodmanSocket, envPodmanService)
	}
	eng, err := newPodmanEngine(context.Background(), access.Endpoint)
	if err != nil {
		return nil, err
	}
	return eng.selfInspect, nil
}
