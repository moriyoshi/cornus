...
// LoadSigner loads the one SSH signer selected by a key-auth profile. When an
// identity file and fingerprint are both present, the fingerprint pins the file's
// public key. With no identity file, the fingerprint selects an ssh-agent key.
// The returned cleanup closes the agent socket when one was opened and is always
// safe to call.
func LoadSigner(identityFile, keyFingerprint string, prompt func(keyPath string) ([]byte, error)) (ssh.Signer, func(), error) {
	cleanup := func() {}
	if identityFile != "" {
		signer, err := loadIdentity(identityFile, prompt)
		if err != nil {
			return nil, cleanup, err
		}
		if keyFingerprint != "" && ssh.FingerprintSHA256(signer.PublicKey()) != keyFingerprint {
			return nil, cleanup, fmt.Errorf("ssh key auth: identity file %s does not match fingerprint %q", identityFile, keyFingerprint)
		}
		return signer, cleanup, nil
	}
	if keyFingerprint == "" {
		return nil, cleanup, errors.New("ssh key auth: set an identity-file or key-fingerprint")
	}
	socket := os.Getenv("SSH_AUTH_SOCK")
	if socket == "" {
		return nil, cleanup, errors.New("ssh key auth: SSH_AUTH_SOCK is not set")
	}
	conn, err := net.Dial("unix", socket)
	if err != nil {
		return nil, cleanup, fmt.Errorf("ssh key auth: connect to ssh-agent: %w", err)
	}
	cleanup = func() { _ = conn.Close() }
	signers, err := agent.NewClient(conn).Signers()
	if err != nil {
		cleanup()
		return nil, func() {}, fmt.Errorf("ssh key auth: list ssh-agent keys: %w", err)
	}
	for _, signer := range signers {
		if ssh.FingerprintSHA256(signer.PublicKey()) == keyFingerprint {
			return signer, cleanup, nil
		}
	}
	cleanup()
	return nil, func() {}, fmt.Errorf("ssh key auth: key %q is not present in ssh-agent", keyFingerprint)
}
