...
// portRangePorts builds the pairwise expansion a "hostStart-hostEnd:contStart-…"
// range denotes, so the table below can state a multi-mapping expectation.
func portRangePorts(hostStart, hostEnd, contStart int, proto string) []Port {
	var out []Port
	for i := 0; i <= hostEnd-hostStart; i++ {
		out = append(out, Port{Host: hostStart + i, Container: contStart + i, Protocol: proto})
	}
	return out
}

// TestPortLongSyntax pins the long object form's validation. Historically a
// `published` string was parsed with the strconv error DISCARDED, and a failed
// parse (0) then fell through to "same as target" — so `published: bogus`
// silently produced a working but DIFFERENT mapping than the file asked for,
// and non-integral / wrongly-typed values were accepted the same way. Every
// field is now validated, with the sole surviving sentinel being an absent (or
// explicitly zero) `published`, which still means "publish on the target port".
func TestPortLongSyntax(t *testing.T) {
	tests := []struct {
		name    string
		entry   string
		want    []Port
		...
		// --- Valid shapes: regression guards for what already worked. ---
		{
			name:  "published string",
			entry: "- {target: 80, published: \"8080\"}",
			want:  []Port{{Host: 8080, Container: 80, Protocol: "tcp"}},
		},
		{
			name:  "published int",
			entry: "- {target: 80, published: 8080}",
		...
		{
			name:  "published omitted defaults to target",
			entry: "- {target: 80}",
			want:  []Port{{Host: 80, Container: 80, Protocol: "tcp"}},
		},
		{
			name:  "published zero int keeps the same-as-target sentinel",
			entry: "- {target: 80, published: 0}",
			want:  []Port{{Host: 80, Container: 80, Protocol: "tcp"}},
		},
		{
			name:  "published zero string keeps the same-as-target sentinel",
			entry: "- {target: 80, published: \"0\"}",
			want:  []Port{{Host: 80, Container: 80, Protocol: "tcp"}},
		},
		{
		...
		{
			name:  "explicit protocol is lowercased",
			entry: "- {target: 53, published: 5353, protocol: UDP}",
			want:  []Port{{Host: 5353, Container: 53, Protocol: "udp"}},
		},
		{
			name:  "host_ip preserved",
			entry: "- {target: 90, published: 9090, host_ip: 10.0.0.5}",
			want:  []Port{{Host: 9090, Container: 90, Protocol: "tcp", HostIP: "10.0.0.5"}},
		},
		{
			name:  "paired ranges expand pairwise",
			entry: "- {target: \"8000-8010\", published: \"8000-8010\"}",
			want:  portRangePorts(8000, 8010, 8000, "tcp"),
		},
		{
			name:  "published range with a single target narrows to the first host port",
			entry: "- {target: 80, published: \"8000-8010\"}",
			want:  []Port{{Host: 8000, Container: 80, Protocol: "tcp"}},
		},
		{
			name:  "lowest valid ports",
			entry: "- {target: 1, published: 1}",
			want:  []Port{{Host: 1, Container: 1, Protocol: "tcp"}},
		},
		{
			name:  "highest valid ports",
			entry: "- {target: 65535, published: 65535}",
			want:  []Port{{Host: 65535, Container: 65535, Protocol: "tcp"}},
		},

		// --- The bug: a bad `published` must fail loudly, not be rewritten. ---
		{
			name:    "published bogus",
			entry:   "- {target: 80, published: bogus}",
			wantErr: `published "bogus"`,
		},
		{
			name:    "published empty string",
			entry:   "- {target: 80, published: \"\"}",
			wantErr: "published is empty",
		},
		{
			name:    "published above the port range",
			entry:   "- {target: 80, published: \"65536\"}",
			wantErr: "65536",
		},
		{
			name:    "published above the port range as an int",
			entry:   "- {target: 80, published: 65536}",
			wantErr: "out of range",
		},
		{
			name:    "published negative",
			entry:   "- {target: 80, published: \"-1\"}",
			wantErr: `published "-1"`,
		},
		{
			name:    "published negative int",
			entry:   "- {target: 80, published: -1}",
			wantErr: "out of range",
		},
		{
			name:    "published float",
			entry:   "- {target: 80, published: 8080.5}",
			wantErr: "whole port number",
		},
		{
			name:    "published bool",
			entry:   "- {target: 80, published: true}",
			wantErr: "boolean",
		},
		{
			name:    "published list",
			entry:   "- {target: 80, published: [8080]}",
			wantErr: "published must be a port number or range",
		},
		{
			name:    "published mapping",
			entry:   "- {target: 80, published: {a: 1}}",
			wantErr: "published must be a port number or range",
		},
		{
			name:    "published range reversed",
			entry:   "- {target: \"8000-8010\", published: \"8010-8000\"}",
			wantErr: "end before start",
		},
		{
			name:    "published range length mismatch",
			entry:   "- {target: \"9000-9005\", published: \"8000-8010\"}",
			wantErr: "range lengths differ",
		},

		// --- The other fields are validated too. ---
		{
			name:    "target missing",
			entry:   "- {published: 8080}",
			wantErr: "target is required",
		},
		{
			name:    "target zero",
			entry:   "- {target: 0, published: 8080}",
			wantErr: `invalid target "0"`,
		},
		{
			name:    "target bogus",
			entry:   "- {target: bogus, published: 8080}",
			wantErr: `invalid target "bogus"`,
		},
		{
			name:    "target bool",
			entry:   "- {target: false, published: 8080}",
			wantErr: "target must be a port number or range",
		},
		{
			name:    "unsupported protocol",
			entry:   "- {target: 80, published: 8080, protocol: sctp}",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr != "" {
				if err == nil {
					t.Fatalf("Load succeeded, want an error containing %q (ports = %+v)",
						tt.wantErr, project.Services()["web"].Ports)
				}
			...
			got := []Port(project.Services()["web"].Ports)
