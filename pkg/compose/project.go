...
func LoadDocumentWithOptions(opts LoadOptions, files ...string) (*ProjectDocument, error) {
	...
	for _, f := range files {
		p, err := loadFile(f, opts.EnvFiles, warn, opts.OnFileRead)
		if err != nil {
			return nil, err
		}
		if p.Name != "" {
...
// (include.go).
func loadFile(f string, envFiles []string, warn func(service, field string), onFile func(string)) (*ProjectDocument, error) {
	p, err := parseFile(f, envFiles, warn, onFile)
	if err != nil {
		return nil, err
	}
	if err := resolveExtends(p, f, envFiles, warn, onFile); err != nil {
		return nil, err
	}
	...
	if err := processInclude(p, f, envFiles, warn, onFile); err != nil {
		return nil, err
	}
	return p, nil
}
...
func parseFile(f string, envFiles []string, warn func(service, field string), onFile func(string)) (*ProjectDocument, error) {
	if onFile != nil {
		onFile(f)
	}
