...
// silently dropping it. It must stay in sync with the Service struct and with
// ServiceDocument's json tags; TestSupportedServiceFieldsMatchesDocument and
// TestServiceAndDocumentFieldsMatch enforce that, so a drifting edit fails the
// build rather than relying on this comment being read.
...
type ServicePlan struct {
	...
	Provider *ProviderPlan // non-nil when the service is managed by an external provider plugin
	// Shells are the service's candidate interactive shells (`x-cornus-shells:`,
	// service level defaulting from the project level), most preferred first. Each
	// entry is a command STRING, not an argv ("/bin/bash", "/bin/busybox sh"); the
	// consumer splits it with the same shell-words parser Command/Entrypoint use.
	//
	// It lives on the PLAN and deliberately not in api.DeploySpec. It is not a
	// deployment instruction — no backend reads it, and putting it in the spec would
	// fold it into dockerhost's apply fingerprint, so editing a shell preference
	// would recreate every running container. Its one consumer is the `cornus web`
	// terminal, which is client-side, where the plan already is.
	Shells []string
}

// ProviderPlan is a translated provider service (compose-spec `provider:`): the
// external plugin type and the `--key=value` flags derived from its options. A
// ServicePlan with a non-nil Provider is driven by the plugin's up/down
// lifecycle (see composecli) rather than built and deployed to the server; its
// Spec carries only the resource name.
type ProviderPlan struct {
	Type  string   // plugin type: binary `docker-<type>` or `<type>` on PATH
	Flags []string // resolved `--key=value` option flags, in deterministic order
}
...
type LoadOptions struct {
	...
	EnvFiles []string
	// OnFileRead, when set, is called with the path of every compose file and env
	// file the load touches: the compose file(s) themselves, the sibling .env or
	// explicit --env-file entries used for interpolation, per-service `env_file:`
	// entries, and any `include:` / `extends` target files (and their env files).
	// It is invoked even for a missing OPTIONAL file (an absent sibling .env or a
	// non-required env_file) so a caller building a watch set is re-triggered when
	// such a file is later created. Paths are passed as the loader resolved them
	// (which may be relative to the process working directory); the caller
	// absolutizes and deduplicates. Used by `compose up --watch` to learn the set
	// of files whose edits should trigger a reload.
...
func LoadDocumentWithOptions(opts LoadOptions, files ...string) (*ProjectDocument, error) {
	...
	for _, f := range files {
		pf, err := loadFile(f, opts.EnvFiles, warn, opts.OnFileRead)
		if err != nil {
			return nil, err
		}
		p := pf.doc
		...
		if p.Credentials != nil {
			merged.Credentials = p.Credentials // last file with project-level credentials: wins
		}
		if p.Shells != nil {
			merged.Shells = p.Shells // last file with a project-level shell list: wins
		}
		// The per-file metadata (which keys this file actually wrote, and their
		// !reset / !override tags) drives the merge; see merge.go.
		svcMeta, secMeta := pf.services(), pf.meta.at("secrets")
		cfgMeta, volMeta, netMeta := pf.meta.at("configs"), pf.meta.at("volumes"), pf.meta.at("networks")
		for name, svc := range p.Services {
			if existing, ok := merged.Services[name]; ok {
				merged.Services[name] = mergeService(svcMeta.at(name), existing, svc)
			} else {
		...
		for name, sec := range p.Secrets {
			...
			if existing, ok := merged.Secrets[name]; ok {
				merged.Secrets[name] = mergeSecretDef(secMeta.at(name), existing, sec)
			} else {
		...
		for name, cfg := range p.Configs {
			...
			if existing, ok := merged.Configs[name]; ok {
				merged.Configs[name] = mergeConfigDef(cfgMeta.at(name), existing, cfg)
			} else {
		...
		for name, vol := range p.Volumes {
			...
			if existing, ok := merged.Volumes[name]; ok {
				merged.Volumes[name] = mergeVolumeDef(volMeta.at(name), existing, vol)
			} else {
		...
		for name, net := range p.Networks {
			...
			if existing, ok := merged.Networks[name]; ok {
				merged.Networks[name] = mergeNetworkDef(netMeta.at(name), existing, net)
			} else {
...
func NewProject(doc *ProjectDocument) *Project {
	return &Project{
		...
		credentials: credentialsFromDocument(doc.Credentials),
		shells:      append(StringList(nil), doc.Shells...),
	}
}
...
func credentialsFromDocument(d *CredentialsDocument) *Credentials {
	if d == nil {
		return nil
	}
	c := Credentials(*d)
...
func serviceFromDocument(d ServiceDocument) Service {
	return Service{
		...
		Credentials:     credentialsFromDocument(d.Credentials),
		Shells:          d.Shells,
	}
}
...
// Credentials returns the project-level `x-cornus-credentials:` default, nil
// when unset.
func (p *Project) Credentials() *Credentials {
	return p.credentials
}

// Shells returns the project-level `x-cornus-shells:` default, nil when unset.
func (p *Project) Shells() StringList {
	return p.shells
}
...
func (v *ProjectProfileView) Document() *ProjectDocument {
	...
	return &ProjectDocument{
		...
		Credentials: p.credentials.toDocument(),
		Shells:      p.shells,
	}
}
...
func (s Service) toDocument() ServiceDocument {
	return ServiceDocument{
		...
		Credentials:     s.Credentials.toDocument(),
		Shells:          s.Shells,
	}
}
...
// (include.go).
func loadFile(f string, envFiles []string, warn func(service, field string), onFile func(string)) (*parsedFile, error) {
	pf, err := parseFile(f, envFiles, warn, onFile)
	if err != nil {
		return nil, err
	}
	if err := resolveExtends(pf, f, envFiles, warn, onFile); err != nil {
		return nil, err
	}
	...
	if err := processInclude(pf, f, envFiles, warn, onFile); err != nil {
		return nil, err
	}
	return pf, nil
}

// parsedFile is one Compose file after parsing: the typed document plus the
// presence/tag metadata harvested from the file's YAML tree (see merge.go). The
// two travel together because the merge needs to know which keys the file
// actually wrote — the typed document alone cannot say whether `privileged:
// false` was written or merely absent.
type parsedFile struct {
	doc *ProjectDocument
	// meta is the metadata for the file's ROOT mapping, so a service's own
	// metadata is meta.at("services").at(name). It is nil for an empty document.
	meta *yamlMeta
}

// services returns the metadata for the file's `services:` mapping (nil-safe).
func (pf *parsedFile) services() *yamlMeta { return pf.meta.at("services") }
...
func parseFile(f string, envFiles []string, warn func(service, field string), onFile func(string)) (*parsedFile, error) {
	if onFile != nil {
		onFile(f)
	}
	...
	mapping, err := envMapping(dir, envFiles, onFile)
	if err != nil {
		return nil, fmt.Errorf("compose: %s: %w", f, err)
	}

	// Harvest which keys this file actually writes and any !reset / !override
	// tags they carry, from a yaml.v3 node pass (see merge.go). The typed decode
	// below then runs on the ORIGINAL bytes unless the file used those tags, in
	// which case it runs on an equivalent document with them resolved away.
	meta, decodeData, err := parseMergeMeta(data)
	if err != nil {
	...
	// run).
	var generic any
	if err := yaml.Unmarshal(decodeData, &generic); err != nil {
		return nil, fmt.Errorf("compose: parse %s: %w", f, err)
	}
	...
	for name, svc := range p.Services {
		if err := applyEnvFiles(&svc, dir, onFile); err != nil {
			return nil, fmt.Errorf("compose: %s: service %q: %w", f, name, err)
		}
		p.Services[name] = svc
	}
	return &parsedFile{doc: &p, meta: meta}, nil
}
...
func applyEnvFiles(svc *ServiceDocument, dir string, onFile func(string)) error {
	...
	for _, ref := range svc.EnvFile {
		...
		if onFile != nil {
			onFile(path) // record even a non-required missing file (watch retriggers on create)
		}
...
func (p *Project) Plan(projectName string) (map[string]ServicePlan, error) {
	...
	// Project-level credentials default: any service that declared no
	// `x-cornus-credentials:` of its own inherits a fresh copy (a service block
	// fully overrides — no field merge), so one block brokers the same credential
	// to a whole project. Each service that inherits it holds its own deploy-attach
	// session, so this is deliberately the blunt instrument: name the credential on
	// the one service that needs it when only one does. Validate the default once
	// up front.
	if p.credentials != nil {
		if _, err := translateCredentials(p.credentials); err != nil {
			return nil, fmt.Errorf("project credentials: %w", err)
		}
		for name, plan := range plans {
			if plan.Spec.Credentials != nil {
				continue
			}
			cs, _ := translateCredentials(p.credentials) // already validated above
			plan.Spec.Credentials = cs
			plans[name] = plan
		}
	}
	// Project-level shell candidates: any service that declared no
	// `x-cornus-shells:` of its own inherits a copy (a service list fully overrides
	// — no concatenation), so one block sets the terminal's preference for a whole
	// project. Copied per service so a later mutation of one plan cannot reach the
	// others through a shared backing array.
	if len(p.shells) > 0 {
		for name, plan := range plans {
			if len(plan.Shells) > 0 {
				continue
			}
			plan.Shells = append([]string(nil), p.shells...)
			plans[name] = plan
		}
	}
...
// translateCredentials converts a compose `x-cornus-credentials:` block into an
// api.CredentialSpec and validates it. A nil block, and a block whose source list
// is empty, both leave credentials off — an empty block would otherwise force a
// held deploy-attach session that brokers nothing.
//
// Validation is stricter here than the api type is: the compose file is the last
// place a typo can be reported cheaply, and every failure mode below otherwise
// surfaces as a workload that deploys green and then cannot read its credential.
func translateCredentials(c *Credentials) (*api.CredentialSpec, error) {
	if c == nil || len(c.Sources) == 0 {
		return nil, nil
	}
	seen := make(map[string]bool, len(c.Sources))
	sources := make([]api.CredentialSource, 0, len(c.Sources))
	for i, src := range c.Sources {
		// The name is the capability key the caretaker requests over the relay, so a
		// duplicate is ambiguous rather than additive.
		if src.Name == "" {
			return nil, fmt.Errorf("credentials.sources[%d]: name is required", i)
		}
		if seen[src.Name] {
			return nil, fmt.Errorf("credentials.sources[%d]: duplicate name %q", i, src.Name)
		}
		seen[src.Name] = true
		if src.Backend == "" {
			return nil, fmt.Errorf("credentials.sources[%d] (%s): backend is required", i, src.Name)
		}
		deliveries := make([]api.CredentialDelivery, 0, len(src.Deliveries))
		for j, d := range src.Deliveries {
			switch d.Kind {
			case "", "endpoint", "file", "env":
			default:
				return nil, fmt.Errorf("credentials.sources[%d] (%s): deliveries[%d]: unknown kind %q (want endpoint, file, or env)", i, src.Name, j, d.Kind)
			}
			// A file delivery with no path has nowhere to materialize, and an env
			// delivery with no variable name sets nothing; both are silent no-ops at
			// runtime rather than errors, so catch them here.
			if d.Kind == "file" && d.Path == "" {
				return nil, fmt.Errorf("credentials.sources[%d] (%s): deliveries[%d]: kind file needs a path", i, src.Name, j)
			}
			if d.Kind == "env" && d.EnvVar == "" {
				return nil, fmt.Errorf("credentials.sources[%d] (%s): deliveries[%d]: kind env needs env_var", i, src.Name, j)
			}
			deliveries = append(deliveries, api.CredentialDelivery{
				Kind:      d.Kind,
				Provider:  d.Provider,
		...
		// Copy the config map: a project-level block is translated once per
		// inheriting service, and a shared map would let one plan's mutation reach
		// every other service's spec.
		...
		sources = append(sources, api.CredentialSource{
			Name:       src.Name,
			Backend:    src.Backend,
			Config:     cfg,
			TTL:        src.TTL,
	...
	return &api.CredentialSpec{Sources: sources}, nil
}
...
func translateIngress(in *Ingress) (*api.IngressSpec, error) {
	...
	if in.Tunnel != nil && !in.Tunnel.Disabled() {
		is.Tunnel = &api.IngressTunnelOpt{
			Enabled:  true,
			HostMode: in.Tunnel.HostMode,
			Host:     in.Tunnel.Host,
		}
	}
...
func translateService(projectName, name string, svc Service, secrets map[string]SecretDef, configs map[string]ConfigDef, volumes map[string]VolumeDef, networks map[string]NetworkDef) (ServicePlan, error) {
	...
	// A provider service delegates its lifecycle to an external plugin (compose-
	// spec `provider:`) rather than being built/pulled and deployed as a
	// container. It is mutually exclusive with image/build/deploy: those describe
	// a container the provider replaces. Short-circuit with a ProviderPlan; the
	// composecli up/down loops invoke the plugin and inject its env into
	// dependents. Spec carries only the resource name (nothing is deployed).
	if svc.Provider != nil {
		if svc.Image != "" || svc.Build != nil || svc.Deploy != nil {
			return ServicePlan{}, fmt.Errorf("`provider` cannot be combined with image, build, or deploy")
		}
		if svc.Provider.Type == "" {
			return ServicePlan{}, fmt.Errorf("provider.type is required")
		}
		return ServicePlan{
			Service:  name,
			Resource: resource,
			Spec:     api.DeploySpec{Name: resource},
			Provider: &ProviderPlan{Type: svc.Provider.Type, Flags: svc.Provider.Options.Flags()},
		}, nil
	}
	...
	for _, vol := range svc.Volumes {
		...
		case vol.Source != "":
			spec.Mounts = append(spec.Mounts, api.Mount{
				...
				ReadOnly:         vol.ReadOnly,
				SELinux:          vol.SELinux,
				NoCreateHostPath: vol.NoCreateHostPath,
			})
		}
	}
	...
	spec.Telemetry = ts
	cs, err := translateCredentials(svc.Credentials)
	if err != nil {
		return ServicePlan{}, fmt.Errorf("service %q: %w", name, err)
	}
	spec.Credentials = cs

	plan := ServicePlan{Service: name, Resource: resource, Spec: spec, Shells: []string(svc.Shells)}
	if svc.Build != nil {
