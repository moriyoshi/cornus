...
// TestIngressTunnelOptIn covers the forms of `x-cornus-ingress.tunnel`.
func TestIngressTunnelOptIn(t *testing.T) {
	for _, tc := range []struct {
		name    string
		yaml    string
		want    *api.IngressTunnelOpt
		wantErr bool
	}{
		{
			name: "bare true enables with defaults",
			yaml: "      tunnel: true\n",
			want: &api.IngressTunnelOpt{Enabled: true},
		},
		{
			name: "empty object enables with defaults",
			yaml: "      tunnel: {}\n",
			want: &api.IngressTunnelOpt{Enabled: true},
		},
		{
			// Unlike the enclosing block, tunnel: has a natural "off" and a reader
			// would expect false to mean it.
			name: "explicit false disables",
			yaml: "      tunnel: false\n",
			want: nil,
		},
		{
			name: "object carries host mode and host",
			yaml: "      tunnel:\n        host_mode: rewrite\n        host: app.example.com\n",
			want: &api.IngressTunnelOpt{Enabled: true, HostMode: "rewrite", Host: "app.example.com"},
		},
		{
			name:    "unknown host mode is rejected at the boundary",
			yaml:    "      tunnel:\n        host_mode: nonsense\n",
			wantErr: true,
		},
	} {
...
`+tc.yaml)
			...
			if tc.wantErr {
				if err == nil {
					t.Fatal("expected an error for an invalid host mode")
				}
				return
			}
			if err != nil {
			...
			got := plans["web"].Spec.Ingress.Tunnel
			switch {
			case tc.want == nil && got != nil:
				t.Fatalf("tunnel = %+v, want none", got)
			case tc.want != nil && got == nil:
				t.Fatal("tunnel = none, want it enabled")
			case tc.want != nil && *got != *tc.want:
				t.Fatalf("tunnel = %+v, want %+v", *got, *tc.want)
			}
		})
	}
}
