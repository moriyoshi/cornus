...
func processInclude(p *ProjectDocument, file string, envFiles []string, warn func(service, field string), onFile func(string)) error {
	if len(p.Include) == 0 {
		return nil
	}
...
func (r *includeResolver) load(absFile string, envFiles []string, stack map[string]bool) (*ProjectDocument, error) {
	p, err := parseFile(absFile, envFiles, r.warn, r.onFile)
	if err != nil {
		return nil, fmt.Errorf("compose: include: %s: %w", absFile, err)
	}
	if err := resolveExtends(p, absFile, envFiles, r.warn, r.onFile); err != nil {
		return nil, err
	}
	stack[absFile] = true
