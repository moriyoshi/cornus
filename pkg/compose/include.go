...
func processInclude(pf *parsedFile, file string, envFiles []string, warn func(service, field string), onFile func(string)) error {
	if len(pf.doc.Include) == 0 {
		return nil
	}
...
type includeResolver struct {
	...
	onFile func(string) // records every file read (watch set); may be nil
}
...
func (r *includeResolver) expand(pf *parsedFile, absFile string, stack map[string]bool) error {
	p := pf.doc
	includes := p.Include
	p.Include = nil
	if len(includes) == 0 {
...
func (r *includeResolver) load(absFile string, envFiles []string, stack map[string]bool) (*parsedFile, error) {
	pf, err := parseFile(absFile, envFiles, r.warn, r.onFile)
	if err != nil {
		return nil, fmt.Errorf("compose: include: %s: %w", absFile, err)
	}
	if err := resolveExtends(pf, absFile, envFiles, r.warn, r.onFile); err != nil {
		return nil, err
	}
	stack[absFile] = true
	defer delete(stack, absFile)
	if err := r.expand(pf, absFile, stack); err != nil {
		return nil, err
	}
	return pf, nil
}
...
// mergeProjectInto deep-merges of's document on top of dst (the override wins),
// reusing the compose-spec field-level merge helpers (merge.go) and of's own
// per-file key-presence metadata. It mutates dst, creating its maps lazily. Used
// to fold included models together and to overlay the including file on top of
// them.
func mergeProjectInto(dst *ProjectDocument, of *parsedFile) {
	override := of.doc
	svcMeta, secMeta := of.services(), of.meta.at("secrets")
	volMeta, netMeta := of.meta.at("volumes"), of.meta.at("networks")
	if override.Name != "" {
	...
	for name, svc := range override.Services {
		...
		if existing, ok := dst.Services[name]; ok {
			dst.Services[name] = mergeService(svcMeta.at(name), existing, svc)
		} else {
	...
	for name, sec := range override.Secrets {
		...
		if existing, ok := dst.Secrets[name]; ok {
			dst.Secrets[name] = mergeSecretDef(secMeta.at(name), existing, sec)
		} else {
	...
	for name, vol := range override.Volumes {
		...
		if existing, ok := dst.Volumes[name]; ok {
			dst.Volumes[name] = mergeVolumeDef(volMeta.at(name), existing, vol)
		} else {
	...
	for name, net := range override.Networks {
		...
		if existing, ok := dst.Networks[name]; ok {
			dst.Networks[name] = mergeNetworkDef(netMeta.at(name), existing, net)
		} else {
