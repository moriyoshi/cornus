...
func resolveExtends(pf *parsedFile, file string, envFiles []string, warn func(service, field string), onFile func(string)) error {
	p := pf.doc
...
type extendsResolver struct {
	...
	files     map[string]*parsedFile     // abs file path -> raw parsed file (document + merge metadata)
...
func (r *extendsResolver) project(absFile string) (*parsedFile, error) {
	if p, ok := r.files[absFile]; ok {
		return p, nil
	}
	p, err := parseFile(absFile, r.envFiles, r.warn, r.onFile)
	if err != nil {
		return nil, err
	}
	r.files[absFile] = p
...
func (r *extendsResolver) resolveService(absFile, name string) (ServiceDocument, error) {
	...
	pf, err := r.project(absFile)
	if err != nil {
		return ServiceDocument{}, err
	}
	svc, ok := pf.doc.Services[name]
	if !ok {
	...
	// The extending service's own YAML keys win over the base, including a key
	// it explicitly wrote as false/empty and any !reset / !override tag (merge.go).
	result := mergeService(pf.meta.at("services").at(name), base, extending)
	result.Extends = nil
