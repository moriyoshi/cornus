...
type ProjectDocument struct {
	...
	// Credentials is a PROJECT-LEVEL `x-cornus-credentials:` block: the default
	// brokered-credential set for every service that does not declare its own. Like
	// Egress and Telemetry it ENABLES the feature for each inheriting service (a
	// service-level block fully overrides it, no field-level merge), so one block
	// gives a whole project the same credential — at the cost of a caretaker and a
	// held session per service, so prefer a service-level block when only one
	// service needs the secret. See Credentials.
	Credentials *CredentialsDocument `json:"x-cornus-credentials"`
	// Shells is a PROJECT-LEVEL `x-cornus-shells:` list: the default interactive-shell
	// candidates for every service that does not declare its own, most preferred
	// first. A service-level list fully overrides it (no concatenation), the same
	// wholesale rule Egress and Telemetry use. See ServiceDocument.Shells.
	Shells StringList `json:"x-cornus-shells"`
}
...
type ServiceDocument struct {
	...
	Telemetry *TelemetryDocument `json:"x-cornus-telemetry"`
	// Credentials is a service `x-cornus-credentials:` block: broker credentials
	// minted on the CLIENT into this container (kubernetes only). The secret never
	// enters the image, this file, or the pod spec; the caretaker serves it — or,
	// for the auth-proxy providers, injects it into requests the container never
	// sees. Because every source is minted client-side for the workload's lifetime,
	// a service that declares one always holds a deploy-attach session (in `up -d`
	// the project's background agent holds it). Maps to api.DeploySpec.Credentials.
	// See CredentialsDocument.
	Credentials *CredentialsDocument `json:"x-cornus-credentials"`
	// Shells is a service `x-cornus-shells:` list: candidate interactive shells for
	// this service, most preferred first. Each entry is a command STRING, not an
	// argv ("/bin/bash", "/bin/busybox sh") — it is split once, by the consumer,
	// with the same shell-words parser Command/Entrypoint use. Maps straight onto
	// api.DeploySpec.Shells, which no deploy backend reads: it is a hint for the
	// `cornus web` terminal, which probes these before its own candidate list. A
	// service list fully overrides the project-level one.
	Shells StringList `json:"x-cornus-shells"`
}
...
// CredentialsDocument is a service (or project-level) `x-cornus-credentials:`
// block: credentials minted on the CLIENT and brokered into the container by the
// caretaker, so a secret never enters the image, the compose file, or the pod
// spec. It uses the compose extension prefix (`x-`) so the file stays valid for
// standard compose tooling. It maps to api.CredentialSpec; see that type for
// field semantics.
//
// The block is the object form (a `sources:` list, mirroring the deploy spec so
// a spec's block pastes in unchanged) or the bare-sequence sugar, which is the
// same list with the wrapper elided:
//
...
// UnmarshalJSON accepts the object form (`{sources: [...]}`) and the bare-
// sequence sugar (`[...]`, the sources list with no wrapper). Both normalize to
// Sources, so `compose config` always renders the object form.
//
// The form is chosen by the value's SHAPE, not by trying one and falling back to
// the other: a sequence whose contents are malformed (say a delivery key the
// strict CredentialDelivery decoder rejects) must report that, and a fallback
// decode would bury it under "cannot unmarshal array into ...".
func (c *CredentialsDocument) UnmarshalJSON(data []byte) error {
	switch trimmed := bytes.TrimLeft(data, " \t\r\n"); {
	case len(trimmed) == 0:
		return fmt.Errorf("x-cornus-credentials: empty value")
	case trimmed[0] == '[':
		var list []CredentialSource
		if err := json.Unmarshal(data, &list); err != nil {
			return fmt.Errorf("x-cornus-credentials: %w", err)
		}
		c.Sources = list
		return nil
	case trimmed[0] == '{':
		...
		c.Sources = obj.Sources
		return nil
	default:
		return fmt.Errorf("x-cornus-credentials: want a mapping with `sources:` or a sequence of sources")
	}
}

// CredentialSource is one `sources:` entry: the client-side backend that
// mints the credential, plus how the container consumes it. Maps to
...
type CredentialSource struct {
	...
	TTL        string               `json:"ttl"`
	Deliveries []CredentialDelivery `json:"deliveries"`
}

// UnmarshalJSON decodes a source STRICTLY, extending the rule
// CredentialDelivery.UnmarshalJSON applies one level down: an unrecognised key
// here is an error, not a silent drop. Without this a source key that misses —
// `deliver:` for `deliveries:`, `well_known:` hoisted to source level, a typo in
// `backend:` — leaves the source with no deliveries or no backend config and
// still deploys green, which is the failure mode the strict delivery decoder
// exists to prevent.
func (s *CredentialSource) UnmarshalJSON(data []byte) error {
	type sourceAlias CredentialSource // shed this method, avoiding recursion
	...
	*s = CredentialSource(out)
	return nil
}

// CredentialConfig is a source's non-secret `config:` map. It takes the same two
// forms compose uses for every other string map (`{KEY: VALUE}` or
// `["KEY=VALUE"]`) and coerces scalar values to strings, so `duration: 3600`
// needs no quoting.
...
// CredentialDelivery is one `deliveries:` entry. Its fields are the
// api.CredentialDelivery fields under compose's snake_case spelling
// (`well_known`, `env_var`, `value_key` for the api's wellKnown / envVar /
// valueKey); the deploy spec's camelCase spellings are accepted too, so a block
// pasted from a deploy spec is not silently half-ignored. See UnmarshalJSON.
...
// deliveryAliases maps the deploy spec's camelCase delivery keys onto this
// document's snake_case ones, so a `deliveries:` entry copied out of a deploy
// spec keeps working. Only the three keys whose spellings differ are listed.
var deliveryAliases = map[string]string{
	"wellKnown": "well_known",
	"envVar":    "env_var",
	"valueKey":  "value_key",
}

// UnmarshalJSON decodes a delivery entry STRICTLY: an unrecognised key is an
// error, not a silent drop. Unlike the rest of a compose file — where an
// unsupported key is warned about and ignored — a mistyped delivery key would
// otherwise turn a credential the workload cannot read into a green deploy, so
// it fails at load. The deploy spec's camelCase spellings are folded onto their
// snake_case counterparts first (see deliveryAliases).
func (d *CredentialDelivery) UnmarshalJSON(data []byte) error {
	...
	for k, v := range raw {
		if canonical, ok := deliveryAliases[k]; ok {
			k = canonical
		}
		normalized[k] = v
	}
	// Re-encode the normalized keys and decode strictly, so the field-by-field
	// typing stays with the struct tags and only the key check is hand-rolled.
	buf, err := json.Marshal(normalized)
	if err != nil {
		return fmt.Errorf("deliveries: %w", err)
	}
	type deliveryAlias CredentialDelivery // shed this method, avoiding recursion
	dec := json.NewDecoder(bytes.NewReader(buf))
	dec.DisallowUnknownFields()
	var out deliveryAlias
	if err := dec.Decode(&out); err != nil {
		return fmt.Errorf("deliveries: %w", err)
	}
	*d = CredentialDelivery(out)
	return nil
}

// Credentials is the internal representation of an `x-cornus-credentials:` block
// — CredentialsDocument's data with the JSON tags stripped away. NewProject
// derives it from the document (see credentialsFromDocument).
type Credentials struct {
	Sources []CredentialSource
}

// toDocument rebuilds the `x-cornus-credentials:` document from the internal form
// (a nil receiver yields nil, so a service without credentials re-serializes
// cleanly).
func (c *Credentials) toDocument() *CredentialsDocument {
	if c == nil {
		return nil
	}
	return &CredentialsDocument{Sources: c.Sources}
}
...
type IngressDocument struct {
	...
	Tunnel      *IngressTunnel    `json:"tunnel"`
}
...
// IngressTunnel is the `x-cornus-ingress.tunnel` block asking the client to
// publish the ingress through a public tunnel after the deploy. It accepts the
// bare `tunnel: true` form as well as an object.
type IngressTunnel struct {
	HostMode string `json:"host_mode"`
	Host     string `json:"host"`
	// disabled records an explicit `tunnel: false`.
	disabled bool
}

// UnmarshalJSON accepts `tunnel: true` / `tunnel: {}` (enable with defaults)
// alongside the object form, mirroring how x-cornus-ingress itself decodes. A
// literal `false` yields nil, so it genuinely disables rather than enabling with
// defaults — unlike the enclosing block, `tunnel:` has a natural "off" and a
// reader would rightly expect false to mean it.
func (t *IngressTunnel) UnmarshalJSON(data []byte) error {
	var enabled bool
	...
	type tunnelAlias IngressTunnel
	var obj tunnelAlias
	if err := json.Unmarshal(data, &obj); err != nil {
		return fmt.Errorf("x-cornus-ingress.tunnel: %w", err)
	}
	*t = IngressTunnel(obj)
	return nil
}
...
// may expand into multiple mappings; so may the long object form
// {target, published, protocol, host_ip}, whose `target` and `published` accept
// the same single-port and range specs as the short form.
//
// Every field of the long form is validated: a `published` that is not a port
// number or range is an error naming the offending value, never a silent
// fallback to some other mapping. The one sentinel kept from earlier behaviour
// is an absent (or explicitly zero) `published`, which means "publish on the
// same host port as target" — see parsePublished.
func parsePortEntry(data json.RawMessage) ([]Port, error) {
	...
	proto, err := normalizePortProtocol(obj.Protocol)
	if err != nil {
		return nil, err
	}
	target, err := portFieldSpec("target", obj.Target)
	if err != nil {
		return nil, err
	}
	if target == "" {
	...
	contStart, contEnd, err := parsePortRange(target)
	if err != nil {
		return nil, fmt.Errorf("port: invalid target %q: %w", target, err)
	}
	if contStart == 0 {
		return nil, fmt.Errorf("port: invalid target %q: port must be 1-65535", target)
	}
	published, err := parsePublished(obj.Published, target)
	if err != nil {
		return nil, err
	}
	hostStart, hostEnd, err := parsePortRange(published)
	if err != nil {
		return nil, fmt.Errorf("port: invalid published %q: %w", published, err)
	}
	if hostStart == 0 {
		return nil, fmt.Errorf("port: invalid published %q: port must be 1-65535", published)
	}
	return expandPortPair(published+":"+target, hostStart, hostEnd, contStart, contEnd, proto, obj.HostIP)
}

// parsePublished resolves the long form's `published` to the port spec to parse.
// An absent key, or an explicit zero, keeps the historical sentinel meaning
// "publish on the same host port as target" (Docker would pick an ephemeral
// host port there, which cornus's fixed-host-port model cannot express). An
// explicitly empty string is rejected rather than folded into that sentinel: it
// is almost always an interpolation that resolved to nothing, and silently
// publishing on the target port is exactly the kind of quiet rewrite this parser
// must not do.
func parsePublished(v any, target string) (string, error) {
	if v == nil {
		return target, nil
	}
	spec, err := portFieldSpec("published", v)
	if err != nil {
		return "", err
	}
	switch spec {
	case "":
		return "", fmt.Errorf("port: published is empty; omit it to publish on the same port as target %q", target)
	case "0":
		return target, nil
	}
	return spec, nil
...
// portFieldSpec normalizes one long-form port field (`target` or `published`)
// to the textual port spec parsePortRange understands. Compose allows both a
// number and a string (interpolation always yields a string), and a string may
// carry a range ("8000-8010"). Any other YAML type — a fractional number, a
// boolean, a list, a mapping — is a defect in the file and is reported as such
// instead of being coerced or ignored. A nil value (absent key) yields "".
func portFieldSpec(field string, v any) (string, error) {
	switch n := v.(type) {
	case nil:
		return "", nil
	case string:
	...
	case float64:
		if n != float64(int64(n)) {
			return "", fmt.Errorf("port: %s %v must be a whole port number", field, n)
		}
		if n < 0 || n > 65535 {
			return "", fmt.Errorf("port: %s %v out of range (1-65535)", field, n)
		}
		return strconv.Itoa(int(n)), nil
	case bool:
		return "", fmt.Errorf("port: %s must be a port number or range, got boolean %v", field, n)
	default:
		return "", fmt.Errorf("port: %s must be a port number or range, got %T", field, v)
	}
}

// normalizePortProtocol validates the long form's `protocol` and defaults an
// omitted one to tcp, so a long-form mapping carries the same protocol string a
// short-form one does.
func normalizePortProtocol(s string) (string, error) {
	...
	case "tcp", "udp":
		return p, nil
	default:
		return "", fmt.Errorf("port: unsupported protocol %q (want tcp or udp)", s)
	}
}
...
func parsePortString(s string) ([]Port, error) {
	...
	return expandPortPair(s, hostStart, hostEnd, contStart, contEnd, proto, hostIP)
}

// expandPortPair pairs an already-parsed host range with a container range and
// expands them into the individual mappings they denote. spec is the mapping as
// the user wrote it ("8000-8010:80"), used only in diagnostics. Shared by the
// short and long forms so both expand ranges — and reject mismatched ones — the
// same way.
func expandPortPair(spec string, hostStart, hostEnd, contStart, contEnd int, proto, hostIP string) ([]Port, error) {
	...
	if hostLen != contLen {
		...
		if hostLen > 0 && contLen == 0 {
			...
			logging.FromContext(ctx).WarnContext(ctx, "host-port range narrowed to its first port; range selection is unsupported",
				slog.Group("compose", "port", spec, "chosen", hostStart))
			return []Port{{Host: hostStart, Container: contStart, Protocol: proto, HostIP: hostIP}}, nil
		}
		return nil, fmt.Errorf("port %q: range lengths differ (host %d, container %d)", spec, hostLen+1, contLen+1)
	}
...
// range. Port 0 is permitted here because it is a legitimate short-form value
// (a host port of 0 asks the backend for an ephemeral port); the long form's
// "same host port as target" sentinel is resolved by parsePublished before the
// spec ever reaches this function, which then requires 1-65535.
...
// Disabled reports an explicit `tunnel: false`.
func (t *IngressTunnel) Disabled() bool { return t != nil && t.disabled }
