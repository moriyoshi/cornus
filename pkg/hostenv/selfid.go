...
var (
	// reMountinfoID matches the container id in a runtime-managed per-container
	// bind source. Two spellings, because the runtimes disagree:
	//
	//	docker:  /var/lib/docker/containers/<id>/hosts
	//	podman:  /run/user/1000/containers/overlay-containers/<id>/userdata/hosts
	//
	// The anchor is load-bearing: overlay layer directories are also 64 hex
	// chars, and matching those would yield a confident wrong id. Both anchors
	// keep that property — podman's layers live under /overlay/<hash>/ and its
	// volumes under /containers/storage/volumes/, neither of which puts a bare
	// 64-hex segment straight after "containers/" or "overlay-containers/".
	//
	// Podman NEEDS this path to work, where Docker merely prefers it: Docker sets
	// a container's HOSTNAME to its own short id, so the hostname miner alone
	// finds it, but podman generates a random hostname unrelated to the id — mine
	// only that and every candidate is rejected by the daemon as "no such
	// container", leaving a co-located cornus unable to identify itself.
	reMountinfoID = regexp.MustCompile(`/(?:overlay-)?containers/([0-9a-f]{64})(?:/|$)`)
