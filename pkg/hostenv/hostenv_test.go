...
func TestLooksContainerized(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		// An incus instance matches NONE of the markers above: no marker file, no
		// Kubernetes env, no runtime-managed /containers/<64hex> bind, and a cgroup
		// path the 64-hex miners cannot read. Measured before this case existed:
		// `cornus daemon preflight` inside an instance reported "cornus runs on the
		// host; runtime paths need no translation". The guest API socket is the
		// marker there, and it is present in every instance.
		"incus guest socket": {host, "0::/incus.payload/cornus-srv\n", noEnv, func(p string) bool { return p == "/dev/incus/sock" }, true},
	} {
