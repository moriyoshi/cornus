...
type Mount struct {
	...
	// NoCreateHostPath disables auto-creating a missing caller-local bind source.
	// The default (false) matches Docker's create_host_path=true: a nonexistent
	// bind source is created as an empty directory before it is served over 9P.
	// Set from compose's `bind.create_host_path: false`. Ignored for non-local
	// (server-host) mounts.
	NoCreateHostPath bool `json:"noCreateHostPath,omitempty" yaml:"noCreateHostPath,omitempty"`
}
...
// Cooperative reports whether the proxy runs in the no-privilege loopback-alias
// mode rather than the default nftables-redirecting one. Nil is false: an absent
// proxy is not cooperative, and callers asking "is this the soft mode?" want a
// plain no.
//
// It exists so the mode word is compared in ONE place. The distinction is not
// cosmetic — enforcing installs a redirect that captures ALL of the app's
// outbound TCP, cooperative installs none — so anything deciding whether a
// feature can coexist with the proxy is really asking this question, and a
// backend that spelled the comparison itself would silently keep the old answer
// if the vocabulary ever grew a third mode.
func (p *ProxySpec) Cooperative() bool {
	return p != nil && strings.EqualFold(p.Mode, "cooperative")
}

// Enforcing reports whether the proxy redirects the workload's outbound TCP —
// the default when Mode is empty. Nil is false, so `spec.Proxy.Enforcing()`
// reads as "is there a redirecting proxy here", which is what callers guarding
// against the redirect actually need.
func (p *ProxySpec) Enforcing() bool {
	return p != nil && !p.Cooperative()
}
...
type IngressSpec struct {
	...
	// ClientEmulated marks an ingress the CLIENT realizes itself — a client-side
	// reverse proxy reached through the conduit (`--ingress-conduit=emulate`) — so
	// the deploy backend must NOT create a real cluster Ingress object or its managed
	// TLS Secret for it. The routing metadata (hosts, path, port, tls) is still
	// carried so the client can build the emulated proxy; the server just skips the
	// cluster-side objects. Unset for a native/real ingress, where the server DOES
	// create the Ingress the cluster controller (or a native-mode conduit tunnel)
	// routes on. Set client-side per conduit mode; the server only reads it.
	ClientEmulated bool `json:"clientEmulated,omitempty" yaml:"clientEmulated,omitempty"`
	// Tunnel, when enabled, asks the CLIENT to publish this ingress through a
	// public tunnel right after the deploy succeeds, and to print the URL. It is
	// carried on the spec so the intent can live in compose next to the ingress it
	// applies to.
	//
	// The server never acts on it: hosting a tunnel needs a credential, and the
	// spec is persisted in labels and echoed back in status, so a credential must
	// never ride it. The tunnel request remains the only channel for that.
	Tunnel *IngressTunnelOpt `json:"tunnel,omitempty" yaml:"tunnel,omitempty"`
}

// IngressTunnelOpt is the declarative form of "publish this ingress publicly".
type IngressTunnelOpt struct {
	Enabled bool `json:"enabled,omitempty" yaml:"enabled,omitempty"`
	// HostMode is one of the HostMode* constants; empty means HostModeAuto.
	HostMode string `json:"hostMode,omitempty" yaml:"hostMode,omitempty"`
	// Host selects which ingress hostname the tunnel fronts when more than one is
	// in scope.
	Host string `json:"host,omitempty" yaml:"host,omitempty"`
}

// Validate checks the tunnel opt-in's syntactically-decidable parts.
func (t *IngressTunnelOpt) Validate() error {
	if t == nil {
		return nil
	}
	switch t.HostMode {
	case "", HostModeAuto, HostModePassthrough, HostModeAlias, HostModeRewrite:
	default:
		return fmt.Errorf("ingress.tunnel: unknown hostMode %q (auto, passthrough, alias, rewrite)", t.HostMode)
	}
	return nil
}

// IngressRoute is one RESOLVED ingress rule: the hosts and path a request must
// match and the workload container port it reaches. It is what an IngressSpec
// becomes once the backend has derived and canonicalized its hosts and picked
// its target port, so a routing table can be rebuilt from it without re-deriving
// anything (and without the server-side defaults that were in effect at deploy
// time).
//
// The host backends persist it alongside the workload and report it back through
// deploy.IngressRouteReporter, which is what lets the server's ingress front door
// survive a restart.
type IngressRoute struct {
	// Workload is the deployment name the rule routes to.
	Workload string `json:"workload"`
	// Hosts are the canonical (lowercased, dot-trimmed) hostnames this rule serves.
	Hosts []string `json:"hosts"`
	// Path and PathType are the Kubernetes path-match rule; empty Path means "/"
	// and empty PathType means "Prefix".
	Path     string `json:"path,omitempty"`
	PathType string `json:"pathType,omitempty"`
	// TargetPort is the workload container port the rule reaches.
	TargetPort int `json:"targetPort"`
	// TLS records that the deployment asked for HTTPS. The server-side front door
	// serves plain HTTP (TLS is terminated at the tunnel provider's edge or by a
	// real cluster controller), so this is carried for reporting, not for
...
type IngressTLS struct {
	...
	// ManagedCertificates contains client-supplied TLS key pairs that the
	// Kubernetes backend materializes as owned kubernetes.io/tls Secrets. It is
	// an API transport field, not declarative YAML configuration: callers build
	// it from local certificate rules immediately before submitting a deploy.
	ManagedCertificates []ManagedIngressCertificate `json:"managedCertificates,omitempty" yaml:"-"`
}

// ManagedIngressCertificate is one Kubernetes TLS Secret desired by a native
// ingress. CertificatePEM and PrivateKeyPEM are sensitive transport payloads;
// they must never be included in logs, status responses, or diagnostics.
type ManagedIngressCertificate struct {
	Hosts          []string `json:"hosts" yaml:"-"`
	SecretName     string   `json:"secretName" yaml:"-"`
	CertificatePEM []byte   `json:"certificatePEM" yaml:"-"`
	PrivateKeyPEM  []byte   `json:"privateKeyPEM" yaml:"-"`
}
...
func (in *IngressSpec) Validate() error {
	...
	if err := in.Tunnel.Validate(); err != nil {
		return err
	}
...
// Knative's autoscaler owns the replica count and its Route owns exposure.
//
// Because the autoscaler owns the count, DeploySpec.Replicas is DROPPED for a
// ksvc — use MinScale/MaxScale to bound it. A spec that sets Replicas above 1
// alongside Knative is warned about, since the silent outcome is the opposite of
// what was asked for: an idle workload scales to zero rather than holding N
// instances. On a
...
type TelemetrySpec struct {
	...
	// ViaMux carries the Collector's exports over the caretaker's existing,
	// already-authenticated connection to the cornus server instead of letting it
	// dial the server directly.
	//
	// It only means anything when the destination IS cornus: there is no
	// connection to a third-party backend to ride. Its value is in the
	// environments where a direct dial is not available — a NetworkPolicy with no
	// egress, an isolated pod network, a server with no advertised URL — where it
	// is the difference between telemetry working and not. Pair it with the
	// server's re-export to reach an external backend from a workload that has no
	// route to one.
	//
	// It is a tri-state pointer so an explicit false survives: nil means "decide
	// for me", and the server turns it ON by default when it is the destination
	// and the backend can honor it (kubernetes). Setting it false forces the
	// direct HTTP dial even then.
	ViaMux *bool `json:"viaMux,omitempty" yaml:"viaMux,omitempty"`
}
...
// UsesMux reports whether the resolved spec carries its exports over the
// caretaker connection. Nil (undecided) reads as false here: the decision is made
// once, on the server, before any backend sees the spec.
func (t *TelemetrySpec) UsesMux() bool {
	return t != nil && t.ViaMux != nil && *t.ViaMux
}

// MuxDecided reports whether the user expressed a preference either way, so the
// server can default the undecided case without overriding an explicit choice.
func (t *TelemetrySpec) MuxDecided() bool { return t != nil && t.ViaMux != nil }
...
//
// An active spec with NO endpoint is valid here, and deliberately so: it means
// "export to whatever the server provides", which is how `x-cornus-telemetry: {}`
// reaches the built-in observability store without naming a URL. Whether that
// wish can be granted is not syntactically decidable — it depends on the server
// the deploy lands on — so the requirement is enforced where the answer is known,
// in deploy.BuildTelemetryWiring, after the server has had its chance to fill in
// its own endpoint.
...
// NeedsSession reports whether the spec brokers any credential, which requires a
// live deploy-attach session: every source is minted on the CLIENT, on demand,
// for the workload's lifetime (an env-kind delivery is minted once at deploy
// time, but still over that session). It is the credential counterpart of
// EgressSpec.NeedsRelay — the CLI uses both to decide whether a workload must
// hold a session rather than deploy statelessly. A nil spec needs none.
func (c *CredentialSpec) NeedsSession() bool {
...
type CredentialSource struct {
	...
	// Deliveries lists how the container consumes the credential. An empty list
	// is valid (the source is fetchable but not surfaced); typically one endpoint
	// and/or one file. Named for the noun, not the verb: `deliver` is already
	// this spec's HubExport routing-mode flag (see HubExport.Deliver).
	Deliveries []CredentialDelivery `json:"deliveries,omitempty" yaml:"deliveries,omitempty"`
}
...
type DeployStatus struct {
	...
	Origin *Origin `json:"origin,omitempty" yaml:"origin,omitempty"`
	// URL, when non-empty, is the address the workload is reachable at. Two
	// sources fill it: a backend that fronts the workload with a router of its own
	// (the kubernetes backend surfaces a Knative Service's status.url), and an
	// active ingress tunnel published for this deployment or its project — so
	// `cornus ps` and the web UI show the public URL without asking a second
	// endpoint. A backend-reported address wins, being the more specific fact.
	// Empty when the workload has no such address.
...
// TerminalExitCode reports the exit status the deployment's workload SETTLED
// with, and whether one is known at all. It is the single definition of "how did
// this deployment end" shared by everything that has to answer that question
// (the deploy-attach terminal event, the Docker API proxy's `docker wait`), so
// they cannot drift apart.
//
// It is deliberately conservative, because the only alternative to admitting
// "unknown" is fabricating a success:
//
//   - No instances at all — a backend that does not enumerate them — is unknown.
//   - Any instance still Running means the workload has not settled: unknown,
//     even if a sibling replica already terminated.
//   - A stopped instance whose backend cannot report a code (InstanceStatus.ExitCode
//     is nil: the barehost and incushost backends never fill it, and neither does
//     a pod that never started) contributes nothing.
//
// The answer is known once every instance has stopped and at least one reported
// a code. The first NON-ZERO code wins, so a partly-failed multi-replica
// deployment is never reported as a clean success; when every reported code is
// zero the answer is 0.
func (s DeployStatus) TerminalExitCode() (int, bool) {
...
type LogOptions struct {
	...
	Until string
	// Instance selects WHICH of a multi-replica deployment's instances to stream,
	// as a 0-based ordinal over the same ordering DeployStatus.Instances reports.
	//
	// The zero value is 0, which is the first instance — exactly what every
	// backend did before this field existed, so nothing that ignores it changes
	// behavior. An index past the end is ErrNotFound rather than a silent
	// fallback to instance 0: a caller enumerating replicas has to be able to
	// tell "that replica is gone" from "here is a different replica's output".
	//
	// There is deliberately no "all instances" value. Fanning several streams
	// into one would have to interleave them inside a single stdcopy framing, and
	// the caller is where that belongs — it already knows how it wants them
	// labelled. `compose logs --all-replicas` and the observability recorder both
	// open one call per instance.
	Instance int
}
...
type StatsOptions struct {
	Stream bool
	// Instance selects WHICH of a multi-replica deployment's instances to sample,
	// with the same contract as LogOptions.Instance: a 0-based ordinal over the
	// ordering DeployStatus.Instances reports, zero meaning the first, and an
	// index past the end being ErrNotFound rather than a silent fallback to 0.
	//
	// Metrics need this for the same reason logs did, and the failure mode is
	// worse here: without it a per-replica collector records instance 0's CPU and
	// memory once per replica, each stamped with a different ordinal. The result
	// is not an obviously empty series but N identical ones, which reads as a
	// perfectly balanced workload.
	Instance int
}

// ResourceSample is one point-in-time resource reading for a single instance, in
// a backend-neutral shape.
//
// It exists because StatsOptions/Backend.Stats speak Docker's stats JSON, which
// is a format chosen for the `docker stats` CLI rather than a faithful record:
// CPU arrives as a pair of cumulative readings for the caller to difference,
// there is nowhere to put a backend that can only report a RATE, and a one-shot
// read deliberately zeroes the previous sample. Recording telemetry through that
// format would mean re-parsing our own encoder's output and inheriting every one
// of those compromises.
//
// Absent and zero are kept distinct. A backend that cannot see network counters
// leaves Networks nil, which is not the same claim as "this container moved no
// bytes" — the first must produce no series at all, the second a series reading
// zero. Pointer fields carry that distinction for the families where a real
// backend genuinely lacks a source.
type ResourceSample struct {
	// Time is when the reading was taken.
	Time time.Time
	// CPUTime is cumulative processor time. Nil on a backend whose only CPU
	// source is an instantaneous rate (see CPUCores).
	CPUTime *CPUTime
	// CPUCores is instantaneous CPU consumption in cores (1.0 == one core fully
	// busy). Nil on a backend that reports cumulative time instead. Exactly one
	// of CPUTime and CPUCores is normally set; a reader wanting a rate should
	// prefer differencing CPUTime, which survives a missed sample.
	CPUCores *float64
	// MemUsage is current memory usage in bytes; MemLimit the enforced limit, or
	// zero when the instance is unlimited or the backend cannot report one.
	MemUsage uint64
	MemLimit uint64
	// Pids is the current process/thread count, zero when unavailable.
	Pids uint64
	// Networks holds cumulative per-interface counters. Nil (NOT an empty map)
	// when the backend cannot observe them at all — the kubernetes backend via
	// metrics-server, and gVisor-sandboxed bare instances whose netstack is
	// invisible from the host.
	Networks map[string]NetCounters
	// DiskRead and DiskWrite are cumulative block-I/O bytes. Nil when
	// unavailable.
	DiskRead  *uint64
	DiskWrite *uint64
}

// SampleField names one family of readings in a ResourceSample.
//
// It exists so a backend can state which families it can NEVER fill, separately
// from any particular reading. The distinction the pointer fields draw per sample
// — absent is not zero — has a second half a single sample cannot express:
// Networks nil in one reading might mean "not observed this time", while on the
// kubernetes backend it means "no source exists and none ever will". A consumer
// that wants to stop offering a chart nothing can ever fill needs the permanent
// answer, and inferring it from a run of empty samples would be a guess that gets
// an idle workload wrong.
//
// The names are the neutral shape's own vocabulary, not metric names: pkg/deploy
// has no business knowing what OpenTelemetry calls these. The mapping onto
// recorded metric families lives with the recorder that does the recording (see
// pkg/server/metricsrecorder.go).
type SampleField string

const (
	// SampleFieldCPUTime is ResourceSample.CPUTime, SampleFieldCPUCores
	// ResourceSample.CPUCores. A backend normally declares exactly one of them
	// unsupported, since a CPU source is either cumulative or a rate.
	SampleFieldCPUTime  SampleField = "cpu_time"
	SampleFieldCPUCores SampleField = "cpu_cores"
	// SampleFieldMemUsage is ResourceSample.MemUsage, SampleFieldMemLimit
	// ResourceSample.MemLimit. Declaring MemLimit unsupported is a claim about
	// the BACKEND, not about a workload: an instance with no limit configured is
	// a supported reading that happens to be absent, not an unobservable one.
	SampleFieldMemUsage SampleField = "mem_usage"
	SampleFieldMemLimit SampleField = "mem_limit"
	// SampleFieldPids is ResourceSample.Pids.
	SampleFieldPids SampleField = "pids"
	// SampleFieldNetwork is ResourceSample.Networks, SampleFieldDisk the
	// DiskRead/DiskWrite pair. Both are single fields here because neither has a
	// backend that can see one half and not the other.
	SampleFieldNetwork SampleField = "network"
	SampleFieldDisk    SampleField = "disk"
)

// CPUTime is cumulative processor time split by mode, in nanoseconds. Total is
// not necessarily User+System: several sources report a total that includes time
...
// NetCounters is one interface's cumulative traffic counters.
...
type ExecConfig struct {
	...
	// AnnouncePID asks the server to wrap Cmd so the process announces its own pid
	// as an OSC sequence before becoming the requested command (see
	// pkg/shells.WrapAnnouncePID). It exists because that pid is the CONTAINER's
	// number for the process, which is the only anchor an auxiliary exec can use to
	// read /proc — ExecState.Pid is a host pid, and is 0 on kubernetes and incus.
	//
	// It is a REQUEST, not a guarantee, and deliberately silent when refused: only
	// a POSIX shell can be wrapped, so anything else runs exactly as it would have.
	// A caller must therefore treat "no pid was announced" as normal and never wait
	// on one. The wrap never changes what the session runs, only how it is spelled.
	AnnouncePID bool
}
...
// FSOp names one structured filesystem operation. "Structured" is the whole
// point: the alternative — splicing `ls`/`mv`/`rm` into somebody else's image —
// cannot be trusted to report what it did. A distroless image has no shell at
// all, busybox and GNU disagree on directory-onto-directory, and an exec's exit
// code is only as reliable as the runtime's inspect call. Each op below has one
// meaning and one errno-derived answer, whoever serves it.
type FSOp string

const (
	// FSOpStat describes a single path. Symlinks are described, never followed.
	FSOpStat FSOp = "stat"
	// FSOpList reads a directory. Non-recursive; a symlink is reported as one.
	FSOpList FSOp = "list"
	// FSOpGet writes a tar of the path to the response body — the same framing
	// Backend.CopyFrom produces, so a caller can pipe one into the other.
	FSOpGet FSOp = "get"
	// FSOpPut extracts a tar read from the request body into the path, mirroring
	// Backend.CopyTo (including NoOverwriteDirNonDir).
	FSOpPut FSOp = "put"
	// FSOpMkdir creates a directory, parents included. Already-a-directory is
	// success, matching MkdirAll.
	FSOpMkdir FSOp = "mkdir"
	// FSOpRemove deletes a path; Recursive is required to delete a non-empty
	// directory. Removing what is not there is success (delete-if-exists, as
	// Backend.Delete and VolumeRemover already are).
	FSOpRemove FSOp = "remove"
	// FSOpRename renames Path to To. Both must be served by the same operator,
	// and the operator may still refuse across its own filesystem boundaries —
	// see FSErrCrossDevice.
	FSOpRename FSOp = "rename"
	// FSOpCopy copies Path to To entirely within the operator, so no bytes cross
	// the wire. This is what makes a volume-to-volume transfer cost nothing.
	FSOpCopy FSOp = "copy"
)

// FSOpRequest is one filesystem operation against a deployment, addressed by the
// path as the APP CONTAINER names it. The operator maps that onto whatever it can
// actually see; a path it cannot see is refused with FSErrUnsupported rather than
// guessed at, so the caller can fall back to relaying the bytes itself.
type FSOpRequest struct {
	Op   FSOp   `json:"op"`
	Path string `json:"path"`
	// To is the destination for rename and copy, also container-absolute.
	To string `json:"to,omitempty"`
	// Recursive permits FSOpRemove to delete a non-empty directory.
	Recursive bool `json:"recursive,omitempty"`
	// NoOverwriteDirNonDir carries CopyToOptions' flag of the same name through
	// FSOpPut. Set it: without it an incoming file lands on an existing directory
	// by deleting the whole tree.
	NoOverwriteDirNonDir bool `json:"noOverwriteDirNonDir,omitempty"`
	// CopyUIDGID carries CopyToOptions' flag of the same name through FSOpPut.
	CopyUIDGID bool `json:"copyUidGid,omitempty"`
}

// FSErr* are the machine-readable outcomes of a failed FSOpRequest. They exist
// because the caller's next move depends on WHICH failure it was: unsupported
// means "route this the long way round", not-found means "tell the user", and
// cross-device means "do it as copy-then-delete instead". A bare error string
// collapses all three into one.
const (
	FSErrNotFound    = "notfound"
	FSErrExists      = "exists"
	FSErrNotDir      = "notdir"
	FSErrIsDir       = "isdir"
	FSErrNotEmpty    = "notempty"
	FSErrReadOnly    = "readonly"
	FSErrCrossDevice = "crossdevice"
	// FSErrUnsupported means this operator does not serve this path at all —
	// the app's image layers, a path under no mount it was given, or a backend
	// with no operator. It is a POSITIVE answer, distinct from a transport
	// failure, and the caller is expected to fall back rather than report it.
	FSErrUnsupported = "unsupported"
)

// FSOpResponse is the answer to an FSOpRequest. Exactly one of Error and the
// result fields is meaningful.
type FSOpResponse struct {
	// Error is empty on success. When set, Code carries one of the FSErr*
	// constants (possibly empty for an error that fits none of them).
	Error string `json:"error,omitempty"`
	Code  string `json:"code,omitempty"`
	// Stat answers FSOpStat.
	Stat *PathStat `json:"stat,omitempty"`
	// Entries answers FSOpList. Nil for every other op — an empty directory
	// yields an empty slice, which is a different thing from "not a directory".
	Entries []PathStat `json:"entries,omitempty"`
	// Truncated reports that Entries is short of what the directory holds. A
	// caller that copies a truncated listing produces an incomplete tree and
	// calls it a success, so this must be checked, not just displayed.
	Truncated bool `json:"truncated,omitempty"`
	// Body reports that a tar stream follows this response (FSOpGet).
	Body bool `json:"body,omitempty"`
}

// FSOpUnsupported builds the refusal a backend with no operator returns. The
// message deliberately contains "unsupported": pkg/server maps that substring
// onto 501, and the whole fallback chain hangs off the caller seeing a 501
// rather than a 500.
func FSOpUnsupported(reason string) FSOpResponse {
	return FSOpResponse{Error: "filesystem operations are unsupported here: " + reason, Code: FSErrUnsupported}
}
...
type ServerInfo struct {
	// Version is the server's build version (the same string `cornus version`
	// prints). A client compares it to its own build to warn about a skew — a client
	// and server built from different generations can silently disagree (a client
	// that predates a feature sends a request the server then mishandles). Empty when
	// the server is older than this field or was built without a stamped version.
	...
	// Tunnel describes the server's public-tunnel capability, so a client can tell
	// whether asking for a tunnel will work before it tries — and, when it will
	// not, say which knob the OPERATOR has to turn. Nil on a server older than
	// this field.
	Tunnel *TunnelInfo `json:"tunnel,omitempty"`
	// Host reports the capabilities that depend on how the SERVER itself is
	// deployed, so `cornus setup`'s verification and the web UI can say "this
	// server cannot realize client-local mounts" before a deploy proves it. Nil
	// on a server older than this field.
	Host *HostCapabilities `json:"host,omitempty"`
	// Backend describes the server's DEPLOY backend: which one it is, and the
	// capabilities a client has to know BEFORE it plans rather than after a deploy
	// proves them. Nil on a server older than this field, or when the backend could
	// not be constructed to ask — a client must then assume nothing is unsupported
	// and behave exactly as it did before.
	Backend *BackendInfo `json:"backend,omitempty"`
}

// BackendInfo is the deploy backend's self-description, reported inside
// ServerInfo.
//
// A name plus capability flags and nothing else, for the same reason as
// HostCapabilities: /.cornus/v1/info is auth-exempt, so nothing here may describe
// the host's identity, layout, or configuration.
type BackendInfo struct {
	// Name is the backend's own identifier (deploy.Backend.Name(): "dockerhost",
	// "containerd", "bare", "incus", "kubernetes"), so a client's error message can
	// say WHICH target cannot do the thing it was asked for.
	Name string `json:"name,omitempty"`
	// ReportsHealth reports that the backend runs a container health probe and
	// fills InstanceStatus.Health (deploy.HealthReporter). False means a compose
	// `depends_on: condition: service_healthy` can NEVER be satisfied there, so the
	// CLI rejects such a dependency before deploying anything instead of waiting out
	// the full dependency timeout for a state the backend cannot produce.
	ReportsHealth bool `json:"reportsHealth"`
}

// HostCapabilities reports what the server's own deployment permits.
//
// Deliberately capability flags and nothing else. /.cornus/v1/info is
// auth-exempt (a client must be able to discover the registry host before it
// has credentials), so this must not carry the server's container id, its data
// directory, or any host path the preflight resolved — those stay in the server
// log and in `cornus daemon preflight`, both of which already require access to
// the host.
type HostCapabilities struct {
	// Containerized reports that the server runs inside a container.
	Containerized bool `json:"containerized,omitempty"`
	// ClientLocalMounts reports whether `--mount` can be realized at all here.
	// False means the server lacks the privileges for the kernel 9P mount, or
	// is containerized such that a mount it makes never reaches the runtime.
	ClientLocalMounts bool `json:"clientLocalMounts"`
}

// TunnelInfo reports what the server can do about public tunnels.
type TunnelInfo struct {
	// Backend is the configured tunnel provider (CORNUS_TUNNEL_BACKEND, default
	// "ngrok").
	Backend string `json:"backend,omitempty"`
	// Available reports that the backend was constructed successfully. False means
	// the provider is unusable on this server — most often because its package was
	// not linked in, or the name is unknown.
	Available bool `json:"available"`
	// CredentialRequired reports that a caller must inject a credential
	// (--authtoken) because the backend needs one and the server has no default.
	CredentialRequired bool `json:"credentialRequired,omitempty"`
	// SupportsIngress reports that this server implements ingress tunnels, letting
	// a client distinguish "not supported here" from "request rejected".
	SupportsIngress bool `json:"supportsIngress"`
}

// StorageUsage is the non-destructive disk-usage report returned by
// GET /.cornus/v1/storage (the read-only counterpart to POST /.cornus/v1/gc). CAS
// fields are the registry content store; the FileCache fields are present only
// when the per-file block cache is enabled.
type StorageUsage struct {
	// CASBlobs / CASBytes are the registry content-store footprint (blob count and
	// total bytes). Both zero in a pure re-export configuration (no CAS).
	CASBlobs int64 `json:"casBlobs"`
	CASBytes int64 `json:"casBytes"`
	// FileCacheBytes / FileCacheFiles are the per-file block-cache footprint,
	// omitted when the block cache is disabled.
	FileCacheBytes int64 `json:"fileCacheBytes,omitempty"`
	FileCacheFiles int   `json:"fileCacheFiles,omitempty"`
}
...
type IngressInfo struct {
	...
	// Emulated reports that the server realizes ingress itself, with its own
	// host/path routing table proxying to workloads (pkg/ingressmux), rather than
	// through a cluster ingress controller. It is what the host backends offer,
	// and it tells a client that an ingress tunnel can front this server even
	// though Controller is nil.
	Emulated bool `json:"emulated,omitempty"`
	// Listen is the address the emulated front door is bound to directly
	// (CORNUS_INGRESS_LISTEN), when the operator configured one. Empty means the
	// front door is only reachable through an ingress tunnel.
