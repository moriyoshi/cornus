...
func TestTelemetrySpecValidate(t *testing.T) {
	cases := []struct {
		...
		// An endpointless active spec is legal HERE on purpose: it means "export
		// to whatever the server provides", which is how `x-cornus-telemetry: {}`
		// reaches the built-in store. Whether that can be granted depends on the
		// server, so the requirement lives in deploy.BuildTelemetryWiring — see
		// TestBuildTelemetryWiringRequiresAnEndpoint.
		{"active without endpoint is deferred, not rejected", &TelemetrySpec{Enabled: true}, false},
