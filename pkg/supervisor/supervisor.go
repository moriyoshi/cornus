...
type Supervisor struct {
	...
	counted  int
	idleHook func()
	// rec, when set, records each child RUN to the process's flight recorder.
	// nil is a working no-op, which is what the client agent (no recorder) gets.
...
// SetRecorder installs the flight recorder each child run is recorded to.
//
// What it buys is the part of an incident that is otherwise invisible: a child
// that panicked and restarted eleven times leaves nothing behind once the
// process is gone, and "which services were live when it died" is exactly the
// snapshot a post-mortem starts from — an unfinished service record is precisely
// that. Set it before adding children; a nil recorder records nothing.
func (s *Supervisor) SetRecorder(rec *activity.Recorder) {
	s.mu.Lock()
	s.rec = rec
	s.mu.Unlock()
}

func (s *Supervisor) recorder() *activity.Recorder {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.rec
}
...
func (s *Supervisor) run(ctx context.Context, t *Token, svc Service, policy Policy) {
	...
	for attempt := 1; ; attempt++ {
		act := rec.Begin(activity.KindService, t.name, map[string]string{
			"policy":  policyName(policy),
			"counted": strconv.FormatBool(t.counted),
			"attempt": strconv.Itoa(attempt),
		})
		err := safeServe(ctx, svc)
		if ctx.Err() != nil {
			// Stopped because we asked it to (Remove, or supervisor shutdown).
			// Serve conventionally returns ctx.Err() here, and recording that as
			// a failure would make every clean shutdown look like an incident.
			act.End(nil)
			return
		}
...
// policyName renders a Policy for a record, so a reader does not have to know
// that 0 means RemoveOnExit.
