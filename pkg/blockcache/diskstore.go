...
type DiskStore struct {
	...
	chunkSize int64
	locks     stripedRW
	// scratch pools chunk-sized buffers for the read-modify-write and
	// hash-a-range paths. See getScratch.
	scratch sync.Pool
}
...
// getScratch returns a buffer of length n drawn from the per-store pool, backed
// by an array of at least chunkSize bytes.
//
// Every caller uses the buffer as PURELY LOCAL scratch: it is filled completely
// by readData (which reports an error unless it read len(buf) bytes, so no stale
// pooled byte can survive into a hash), consumed within the same call, and
// returned to the pool before returning. It is never stored in the index, never
// handed to a caller, and never aliased by a concurrent reader — unlike MemStore,
// DiskStore keeps no in-memory chunk that a reader could observe, so pooling here
// is safe independently of the per-key striped lock. The contents are NOT zeroed;
// callers must overwrite before reading.
//
// This removes the chunk-sized allocation every small partial write used to make
// (a 4 KiB SQLite page write allocated a fresh 1 MiB RMW buffer).
func (d *DiskStore) getScratch(n int64) []byte {
	if v, ok := d.scratch.Get().(*[]byte); ok {
		if b := *v; int64(cap(b)) >= n {
			return b[:n]
		}
	}
	c := n
...
func (d *DiskStore) putScratch(b []byte) {
	b = b[:cap(b)]
	d.scratch.Put(&b)
}
...
func (d *DiskStore) WriteChunk(id FileID, idx int, chunkOff int64, data []byte, chunkLen int64, callerHash uint64) error {
	...
	// resulting chunk against the caller's authoritative hash. cur is pooled
	// scratch (see getScratch) — readData overwrites it whole or errors out, and
	// it never escapes this call.
	cur := d.getScratch(chunkLen)
	defer d.putScratch(cur)
...
func (d *DiskStore) HashRange(id FileID, idx int, off, length int64) (uint64, bool, error) {
	...
	// Pooled scratch: filled whole by readData, hashed, and released here.
	buf := d.getScratch(length)
	defer d.putScratch(buf)
