...
func dockerSocketPath() string {
	...
	if err != nil {
		// An unparseable DOCKER_HOST is not this function's to report: its caller
		// already treats "" as "no socket to bind", which is the same conclusion
		// and the same remediation. newClient surfaces the parse error properly.
		return ""
	}
	return ep.UnixSocketPath()
}
...
func newClient() (*client, error) {
	...
	return &client{http: &http.Client{Transport: ep.Transport()}, host: ep.BaseURL()}, nil
}
