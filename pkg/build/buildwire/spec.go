...
const (
	tagP9  = '9'
	tagSSH = 'S'
	// tagLazy9P is wire's lazy-9P backing tag, NOT a second copy of it: the build
	// server opens an 'L'-tagged stream per on-demand lazy-context read, and the
	// caller's single accept loop (serveCallerStreams) must route those apart from
	// the SSH streams sharing the connection. It used to restate the byte with a
	// comment saying it mirrored wire's private constant, which put the writer and
	// the dispatcher in different packages holding independent literals.
	tagLazy9P = wire.TagLazy9P
)
