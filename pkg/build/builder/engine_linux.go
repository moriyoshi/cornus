...
// authProvider returns a session attachable seeded with request-scoped registry
// credentials and falling back to the ambient Docker config for every other
// host. Request credentials always replace same-host file entries.
func authProvider(extra map[string]RegistryCredential) session.Attachable {
	cfg := dockerconfig.LoadDefaultConfigFile(os.Stderr)
	seedRegistryAuth(cfg, extra)
	return authprovider.NewDockerAuthProvider(cfg, nil)
}

func seedRegistryAuth(cfg *dockerconfigfile.ConfigFile, extra map[string]RegistryCredential) {
	if cfg.AuthConfigs == nil {
		cfg.AuthConfigs = make(map[string]dockertypes.AuthConfig)
	}
