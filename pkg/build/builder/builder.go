...
type SolveInput struct {
	...
	// RegistryAuth adds credentials keyed by registry host. They override the
	// ambient Docker config for exactly those hosts; all other hosts keep using
	// the user's normal Docker credential configuration.
...
// RegistryCredential is plain cross-platform registry authentication data.
// BuildKit-specific config types stay in the linux-tagged engine file.
...
type Request struct {
	...
	SSH []SSHSource
	// RegistryAuth mirrors SolveInput.RegistryAuth.
	RegistryAuth map[string]RegistryCredential
