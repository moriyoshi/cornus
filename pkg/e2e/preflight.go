...
const (
	...
	CapDevcontainerCLI
	// CapPodman is the podman CLI plus a reachable API socket. The socket is
	// NOT discovered — the backend refuses to guess which daemon it drives, so
	// the probe checks the very endpoint the run will use.
	CapPodman
	...
	CapOCIRuntime
	// CapIncus is a reachable Incus daemon plus skopeo and umoci on PATH (Incus
	// converts OCI images with them). The incus target deploys against it.
	CapIncus
)
...
var capInfos = map[Capability]capInfo{
	...
	CapPodman:          {"podman", "install podman and expose its API socket (systemctl --user enable --now podman.socket), then set CORNUS_PODMAN_SOCKET"},
	...
	CapIncus:           {"incus", "install and start Incus (6.3+) with skopeo and umoci on PATH (Incus needs them to run OCI images); the socket usually needs root or the incus group. Only the incus target needs it"},
}
...
func scenarioNeeds(path string) ([]Capability, error) {
	...
	// A scenario can also reach the ssh tools by shelling out — auth-ssh-key.star
	// generates its key pairs with `sh(cmd = "ssh-keygen ...")` and never touches
	// ssh_agent(). Without this it slipped past preflight entirely and FAILED
	// mid-run on a host with no openssh-client, where the point of preflight is to
	// say so up front with a remedy.
	if strings.Contains(s, "ssh-keygen") || strings.Contains(s, "ssh-add") {
		caps = append(caps, CapSSHTools)
	}
...
// probeIncus reports whether the incus target can run: the `incus` CLI on PATH
// with a reachable daemon, plus skopeo and umoci (Incus flattens OCI images with
// them). It uses the CLI rather than the Go client to keep this probe daemon-free
// when incus is absent.
func probeIncus(ctx context.Context) (bool, string) {
	if _, err := exec.LookPath("incus"); err != nil {
		return false, "incus not on PATH"
	}
	var missing []string
	for _, b := range []string{"skopeo", "umoci"} {
		if _, err := exec.LookPath(b); err != nil {
			missing = append(missing, b)
		}
	}
	if len(missing) > 0 {
		return false, "incus present but missing " + strings.Join(missing, ", ") + " (Incus needs them to run OCI images)"
	}
	cctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	out, err := exec.CommandContext(cctx, "incus", "info").CombinedOutput()
	if err != nil {
		return false, "daemon unreachable: " + firstLine(string(out))
	}
	return true, "incus daemon reachable; skopeo + umoci on PATH"
}
...
// podmanProbeSocket resolves the socket the run will ACTUALLY use.
//
// It reads it off the target rather than off the environment alone, because the
// --podman-socket flag is populated into the struct and never mirrored into the
// environment — probing the stock path instead would green-light a run against a
// daemon the server never talks to. Same shape as containerdProbeAddress, and
// for the same reason.
func podmanProbeSocket(t Target) string {
	if pt, ok := t.(*PodmanTarget); ok {
		return pt.sock()
	}
	return os.Getenv("CORNUS_PODMAN_SOCKET")
}

// probePodman checks the podman CLI and the API socket the run will use.
//
// Both halves matter and they fail differently: without the binary there is
// nothing to inspect networks with, and without a reachable SOCKET the backend
// cannot deploy at all — and since cornus deliberately does not discover one,
// "no socket configured" is a distinct and common failure worth naming.
func probePodman(ctx context.Context, sock string) (bool, string) {
	if _, err := exec.LookPath("podman"); err != nil {
		return false, "podman not on PATH"
	}
	if sock == "" {
		return false, "no API socket configured (set CORNUS_PODMAN_SOCKET or pass --podman-socket); cornus does not probe for one"
	}
	if _, err := os.Stat(strings.TrimPrefix(sock, "unix://")); err != nil {
		return false, "socket " + sock + " not found"
	}
	cctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	// --url pins the CLI to the socket under test. Without it the CLI answers
	// from whatever daemon it defaults to, which on the rootless leg is a
	// DIFFERENT daemon than the run drives — a preflight that passes for a
	// runtime nothing in the run ever talks to.
	url := sock
	if !strings.Contains(url, "://") {
		url = "unix://" + url
	}
	out, err := exec.CommandContext(cctx, "podman", "--url", url, "version", "--format", "{{.Server.Version}}").CombinedOutput()
	if err != nil {
		return false, "podman at " + sock + " unreachable: " + firstLine(string(out))
	}
	return true, "podman " + strings.TrimSpace(string(out)) + " at " + sock
}
