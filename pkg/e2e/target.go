...
// PodmanTarget deploys via the podman backend against a podman API socket.
//
// The socket is NOT discovered. The backend itself refuses to guess (see
// pkg/deploy/dockerhost's resolvePodmanAccess), and a harness that quietly found
// a different daemon than the one under test would make a green run meaningless.
// So the target resolves it from CORNUS_PODMAN_SOCKET or the --podman-socket
// flag, and Setup fails loudly when neither names a reachable socket.
type PodmanTarget struct {
	Socket string
	// Rootless names the leg driving a ROOTLESS podman service. It changes no
	// setup step — the socket is still named explicitly — but it changes the
	// target's NAME, which is what scenarios gate on. That matters because
	// rootless is not a variation of the same topology: a workload's network
	// namespace is not routable from the host, so port-forward and tunnel behave
	// differently by design and a scenario has to know which it is looking at.
	Rootless bool

	gateway string
}

func (t *PodmanTarget) Name() string {
	if t.Rootless {
...
// sock resolves the endpoint this run will actually use, mirroring the backend's
// own precedence so the preflight probe and the server agree.
func (t *PodmanTarget) sock() string {
	if t.Socket != "" {
		return t.Socket
	}
	return os.Getenv("CORNUS_PODMAN_SOCKET")
}

// podmanURL renders the socket as the CLI's --url form.
func (t *PodmanTarget) podmanURL() string {
	s := t.sock()
	if strings.HasPrefix(s, "unix://") || strings.HasPrefix(s, "tcp://") || strings.HasPrefix(s, "ssh://") {
		return s
	}
	return "unix://" + s
}

func (t *PodmanTarget) Setup(ctx context.Context) error {
	if t.sock() == "" {
		return fmt.Errorf("podman target: no API socket configured; " +
			"pass --podman-socket or set CORNUS_PODMAN_SOCKET " +
			"(enable one with `systemctl --user enable --now podman.socket`)")
	}
	// --url pins the CLI to the socket this run will actually use. Without it the
	// CLI answers from whatever daemon it defaults to, which on the rootless leg
	// is a DIFFERENT daemon than the server drives — a green check for a runtime
	// nothing under test ever talks to.
	cmd := exec.CommandContext(ctx, "podman", "--url", t.podmanURL(), "version", "--format", "{{.Server.Version}}")
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("podman not reachable at %s: %v: %s", t.sock(), err, strings.TrimSpace(string(out)))
	}
	// The remote-companion path (CORNUS_PODMAN_REMOTE) puts a caretaker on the
	// default podman network, from which a host-run cornus is the bridge gateway
	// rather than 127.0.0.1 — the same discovery DockerTarget does. Best-effort:
	// an empty gateway just means serve() advertises nothing extra.
	out, err := exec.CommandContext(ctx, "podman", "--url", t.podmanURL(), "network", "inspect", "podman",
		"-f", "{{range .Subnets}}{{.Gateway}} {{end}}").CombinedOutput()
	if err == nil {
...
// AdvertiseHost returns the podman bridge gateway a companion can use to reach a
// host-run cornus server, or "" if it could not be discovered.
func (t *PodmanTarget) AdvertiseHost() string { return t.gateway }

func (t *PodmanTarget) Teardown(context.Context) error { return nil }

func (t *PodmanTarget) ServeEnv() []string {
	env := []string{
		"CORNUS_DEPLOY_BACKEND=podman",
		// Scenarios exercise host bind mounts through the stateless deploy path,
		// which the backend default-denies. Permit any absolute source for the test
		// server; production stays default-deny.
		"CORNUS_ALLOW_BIND_SOURCES=/",
		// The registry scenarios exercise the persistent CAS; an unset source
		// defaults to host-native re-export on podman, so opt out. A scenario that
		// wants re-export overrides this via serve(env=).
		"CORNUS_REGISTRY_SOURCE=off",
	}
	if s := t.sock(); s != "" {
		env = append(env, "CORNUS_PODMAN_SOCKET="+s)
	}
	return env
}

// PrepareImage is a no-op: the podman backend pulls from cornus's registry on
// the same host.
func (t *PodmanTarget) PrepareImage(context.Context, string) error { return nil }
...
// bareRuncStateRoot is the tmpfs state root (`--root`) the bare backend hands
// the OCI runtime. It is IMPORTED, not mirrored: Teardown must pass the runtime
// the same --root the backend used, or it lists a different (empty) state
// directory, finds no containers, and reports a successful cleanup having
// removed nothing. See barehost.RuncStateRoot for why that constant is exported.
const bareRuncStateRoot = barehost.RuncStateRoot
...
func (t *KubeTarget) Setup(ctx context.Context) error {
	...
	// `kind create cluster` returns once the control plane ANSWERS, not once the
	// node is schedulable — its own `--wait` defaults to 0s. The node still carries
	// node.kubernetes.io/not-ready until the CNI comes up, so the first pod of a
	// run lands on an unschedulable node and the deploy reports
	// "Unschedulable: 1 node(s) had untolerated taint". That was reproducing in
	// 4 of 4 containerized runs (deterministic on a fresh cluster, not a flake);
	// the client happened to retry and recover, so scenarios passed while eating a
	// failed first scheduling attempt, but a scenario treating its first deploy as
	// fatal would simply fail. Waiting here rather than passing `--wait` to `kind`
	// also covers the cluster-REUSE path, which skips creation entirely.
	if err := t.waitNodesReady(ctx); err != nil {
		return err
	}
	...
	// Install a real ingress controller when asked (E2E_INGRESS_NGINX=1), so the
	// ingress scenarios can exercise cornus dialling an actual controller instead
	// of only the server-side mux it falls back to. Without it those scenarios
	// still run — they just cover the fallback — so this stays opt-in, like
	// Knative above.
	if os.Getenv("E2E_INGRESS_NGINX") == "1" {
		if err := t.installIngressNginx(ctx); err != nil {
			return fmt.Errorf("install ingress-nginx: %w", err)
		}
	}
	// Install metrics-server when asked (E2E_METRICS_SERVER=1). Unlike the two
	// above, this is not an alternative path through the same feature: the
	// kubernetes backend has NO metric source without it (SampleMetrics reads
	// the metrics.k8s.io aggregated API, which simply does not exist), so it is
	// what makes observability-metrics.star's kube leg exercise anything at all.
	if os.Getenv("E2E_METRICS_SERVER") == "1" {
		if err := t.installMetricsServer(ctx); err != nil {
...
// nodeReadyTimeout bounds the wait for node readiness. A kind node typically
// goes Ready well inside 60s, so this is generous; exceeding it means the
// cluster is broken rather than slow, and failing here names that directly
// instead of letting every scenario fail with a confusing scheduling error.
const nodeReadyTimeout = 180 * time.Second

// waitNodesReady blocks until every node reports Ready. It polls `kubectl get`
// rather than using `kubectl wait --all`, which returns immediately (older
// versions error "no matching resources found") when the node list is still
// empty — precisely the window this is here to cover.
func (t *KubeTarget) waitNodesReady(ctx context.Context) error {
	const jsonpath = `jsonpath={.items[*].status.conditions[?(@.type=="Ready")].status}`
	deadline := time.Now().Add(nodeReadyTimeout)
	var last string
	for {
		out, err := t.run(ctx, "kubectl", "--kubeconfig", t.kubeconfig, "get", "nodes", "-o", jsonpath)
		if err == nil && nodesReady(out) {
			return nil
		}
		if err != nil {
			last = err.Error() + ": " + out
		} else {
			last = "node Ready conditions: " + out
		}
		if time.Now().After(deadline) {
			return fmt.Errorf("kube: nodes never became Ready within %s (%s)", nodeReadyTimeout, last)
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
...
// nodesReady reports whether the Ready-condition jsonpath output says every node
// is Ready. Empty output means the node list is not populated yet, which is NOT
// ready — the distinction that makes this safe to call immediately after create.
func nodesReady(out string) bool {
...
// installIngressNginx runs the shared install script against this target's
// cluster, pointing kubectl at the target's kubeconfig. Like installKnative it is
// repo-relative and fetches upstream manifests, so it needs network access.
func (t *KubeTarget) installIngressNginx(ctx context.Context) error {
	cmd := exec.CommandContext(ctx, "bash", "e2e/container/install-ingress-nginx.sh")
	cmd.Env = append(os.Environ(), "KUBECONFIG="+t.kubeconfig)
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("%v: %s", err, strings.TrimSpace(string(out)))
	}
...
// installMetricsServer runs the shared install script against this target's
// cluster, pointing kubectl at the target's kubeconfig. Its release manifest is
// vendored + checksummed under e2e/container/metrics-server/, so unlike the two
// above it applies from disk; the cluster still pulls the metrics-server image.
func (t *KubeTarget) installMetricsServer(ctx context.Context) error {
	cmd := exec.CommandContext(ctx, "bash", "e2e/container/install-metrics-server.sh")
	cmd.Env = append(os.Environ(), "KUBECONFIG="+t.kubeconfig)
	if out, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("%v: %s", err, strings.TrimSpace(string(out)))
	}
