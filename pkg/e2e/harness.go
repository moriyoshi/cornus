...
func (h *Harness) bDeploy(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	var ports, env, command, entrypoint, mounts, volumes, dns, hubExport, hubImport, ingress, knative, telemetry, healthcheck starlark.Value
	...
	if err := starlark.UnpackArgs("deploy", args, kwargs,
		...
		"telemetry?", &telemetry, "mem_limit?", &memLimit, "healthcheck?", &healthcheck); err != nil {
		return nil, err
	}
	if h.registryHost == "" {
	...
	// healthcheck=["CMD", ...] is the probe command; healthcheck={test, interval,
	// timeout, start_period, start_interval, retries} sets the timings too. A
	// scenario almost always wants an interval far below the 30s default, or it
	// spends its whole budget waiting for the first probe.
	if healthcheck != nil && healthcheck != starlark.None {
...
// parseHealthcheck turns the deploy() healthcheck kwarg into an api.Healthcheck.
//
// A LIST is the probe command in Docker's CMD form (["CMD", "true"]); a DICT
// additionally carries the timings, with `test` holding the command as a
// comma-separated string because starlark dict values here are strings (the same
// convention parseIngressSpec's `hosts` uses).
func parseHealthcheck(v starlark.Value) (*api.Healthcheck, error) {
	if _, isStr := v.(starlark.String); !isStr {
		if _, iterable := v.(starlark.Iterable); iterable {
			if _, isDict := v.(*starlark.Dict); !isDict {
				test, err := strSlice(v)
				if err != nil {
					return nil, fmt.Errorf("healthcheck: %w", err)
				}
				return &api.Healthcheck{Test: test}, nil
			}
		}
	}
	m, err := strMap(v)
	...
	for k, val := range m {
		...
		case "start_period":
			hc.StartPeriod = val
		case "start_interval":
			hc.StartInterval = val
		case "retries":
			n, err := strconv.Atoi(val)
			if err != nil {
				return nil, fmt.Errorf("healthcheck: retries %q: %w", val, err)
			}
			hc.Retries = n
		default:
			return nil, fmt.Errorf("healthcheck: unknown key %q", k)
		}
	}
	if len(hc.Test) == 0 {
		return nil, fmt.Errorf("healthcheck: no test command")
	}
	return hc, nil
}
