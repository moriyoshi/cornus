...
func (h *Harness) compose(sub string) func(*starlark.Thread, *starlark.Builtin, starlark.Tuple, []starlark.Tuple) (starlark.Value, error) {
	return func(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		...
		var detach bool
		if err := starlark.UnpackArgs("compose_"+sub, args, kwargs, "file", &file, "project?", &project, "detach?", &detach, "conduit?", &conduit); err != nil {
			return nil, err
		}
		cargs := []string{"-f", file}
		if project != "" {
...
func (h *Harness) bHTTPGet(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	var url, socks5Addr string
	var allowError, insecure, retry5xx bool
	retry := "15s"
	if err := starlark.UnpackArgs("http_get", args, kwargs, "url", &url, "retry?", &retry, "socks5?", &socks5Addr, "allow_error?", &allowError, "insecure?", &insecure, "retry_5xx?", &retry5xx); err != nil {
		return nil, err
	}
	...
	// terminates TLS with a locally-generated cert (e.g. the emulated ingress).
	httpClient := http.DefaultClient
	if socks5Addr != "" || insecure {
		tr := &http.Transport{}
		if socks5Addr != "" {
