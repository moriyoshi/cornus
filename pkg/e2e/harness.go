...
func predeclaredNames() map[string]bool {
	names := []string{
		"TARGET", "log", "sleep", "serve", "serve_container", "stop_server", "build", "ssh_agent", "sshd", "deploy",
		...
		"registry_roundtrip", "upstream_registry", "build_upload", "cornus", "cornus_stream", "dockerd_up", "docker_compose", "devcontainer_cli", "port_forward", "tunnel", "ingress_tunnel", "free_port",
		"web", "frontend_stub",
...
func (h *Harness) bHTTPGet(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	retry := "15s"
	if err := starlark.UnpackArgs("http_get", args, kwargs, "url", &url, "retry?", &retry, "socks5?", &socks5Addr, "allow_error?", &allowError, "insecure?", &insecure, "ca_file?", &caFile, "retry_5xx?", &retry5xx, "host?", &host); err != nil {
		return nil, err
	}
