...
func predeclaredNames() map[string]bool {
	names := []string{
		...
		"registry_roundtrip", "upstream_registry", "build_upload", "cornus", "cornus_stream", "cornus_bg", "dockerd_up", "docker_compose", "devcontainer_cli", "port_forward", "tunnel", "ingress_tunnel", "free_port",
...
func (h *Harness) bDeployAttach(_ *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	var ports, env, localMounts, specMounts, command, entrypoint starlark.Value
	var privileged bool
	replicas := 1
	timeout = "180s"
