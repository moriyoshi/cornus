...
// TestScenarioNeedsSSHToolsWhenShellingOut: a scenario can reach the ssh tools
// without touching ssh_agent() — auth-ssh-key.star generates its key pairs with
// `sh(cmd = "ssh-keygen ...")`. Before this, that scenario slipped past preflight
// entirely and failed MID-RUN on a host with no openssh-client, which is exactly
// the case preflight exists to catch up front with a remedy.
func TestScenarioNeedsSSHToolsWhenShellingOut(t *testing.T) {
	dir := t.TempDir()
	write := func(name, body string) string {
		p := filepath.Join(dir, name)
		if err := os.WriteFile(p, []byte(body), 0o644); err != nil {
	...
	for _, tc := range []struct{ name, body string }{
		{"keygen.star", "serve()\nsh(cmd = \"ssh-keygen -t ed25519 -f /tmp/k\")\n"},
		{"add.star", "serve()\nsh(cmd = \"ssh-add -l\")\n"},
	} {
		caps, err := scenarioNeeds(write(tc.name, tc.body))
		if err != nil {
			t.Fatal(err)
		}
		if !hasCap(caps, CapSSHTools) {
			t.Errorf("%s: shelling out to the ssh tools should need CapSSHTools", tc.name)
		}
	}

	// ...and a scenario that does neither still must not require them, or every
	// run would demand openssh-client for nothing.
	caps, err := scenarioNeeds(write("plain.star", "serve()\ndeploy(name=\"x\", image=\"alpine\")\n"))
	if err != nil {
		t.Fatal(err)
	}
	if hasCap(caps, CapSSHTools) {
		t.Error("a scenario using neither ssh_agent() nor the ssh binaries should not need ssh tools")
	}
}
