...
func TestRegistryPeerKeyIsNoOp(t *testing.T) {
	r := NewRegistry()
	if err := r.PublishPeerKey("replica-a", []byte("public")); err != nil {
		t.Fatal(err)
	}
	key, ok, err := r.PeerKey("replica-a")
	if err != nil {
		t.Fatal(err)
	}
	if ok || key != nil {
		t.Fatalf("process-local peer key = %q, ok=%v; want a miss", key, ok)
	}
}
