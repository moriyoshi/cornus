...
func TestStartUnknownMode(t *testing.T) {
	_, err := Start(context.Background(), fakeDialer{}, Config{Mode: "bogus"})
	if err == nil {
		t.Fatal("want error for unknown mode")
	}
	// The message must name the mode as unknown. A conduit URL never reaches here
	// (clientconn parses it into Mode+Socks5Listen), so an "unknown conduit mode
	// \"socks5://...\"" would mean a selector was smuggled through unparsed.
	if !strings.Contains(err.Error(), `unknown conduit mode "bogus"`) {
		t.Errorf("error = %v, want it to name the unknown mode", err)
	}
}

// TestValidate pins the loopback policy for the SOCKS5 conduit: refused by
// default at every bind spelling that reaches beyond this host, permitted with
// the Socks5AllowNonLoopback opt-in that `--allow-non-loopback` sets. Validate is
// the single enforcement point — Start calls it, and the CLI calls it up front so
// `compose up -d` fails at the flag instead of inside the background agent.
func TestValidate(t *testing.T) {
	nonLoopback := []string{"0.0.0.0:1080", "[::]:1080", ":1080", "192.168.1.5:1080", "*:1080"}
	for _, listen := range nonLoopback {
		t.Run("refused "+listen, func(t *testing.T) {
			cfg := Config{Mode: ModeSocks5, Socks5Listen: listen}
			err := cfg.Validate()
			if err == nil {
				t.Fatalf("Validate() = nil for %q, want a refusal", listen)
			}
	...
	// Loopback and the "" default need no opt-in, and the non-SOCKS5 modes have no
	// listener at all — the policy must not leak onto them.
	for _, cfg := range []Config{
		{Mode: ModeSocks5, Socks5Listen: "127.0.0.1:1080"},
		{Mode: ModeSocks5, Socks5Listen: ""},
		{Mode: ModePortForward, Socks5Listen: "0.0.0.0:1080"},
		{Mode: ModeNone, Socks5Listen: "0.0.0.0:1080"},
		{Mode: "", Socks5Listen: "0.0.0.0:1080"},
	} {
		if err := cfg.Validate(); err != nil {
	...
	if err := (Config{Mode: "bogus"}).Validate(); err == nil {
		t.Error("Validate() = nil for an unknown mode, want an error")
	}
}

// TestStartRefusesNonLoopbackBind is the neutralization guard for TestValidate:
// Start must not bind a non-loopback listener without the opt-in, whatever the
// caller did or did not validate beforehand.
func TestStartRefusesNonLoopbackBind(t *testing.T) {
	_, err := Start(context.Background(), fakeDialer{}, Config{Mode: ModeSocks5, Socks5Listen: "0.0.0.0:0"})
	if err == nil {
		t.Fatal("Start bound a non-loopback address without --allow-non-loopback")
	}
	if !strings.Contains(err.Error(), "non-loopback") {
		t.Errorf("error = %v, want the non-loopback refusal", err)
	}
}
