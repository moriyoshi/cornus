...
func Start(ctx context.Context, d portfwd.Dialer, cfg Config, opts ...Option) (Conduit, error) {
	...
	case ModeSocks5:
		// Reject an obviously non-loopback bind here, at the config layer, before
		// binding — a friendlier and earlier error than socks5.Start's post-bind
		// refusal. The session conduit has no non-loopback opt-in of its own (only
		// `cornus socks5 --allow-non-loopback` sets Socks5AllowNonLoopback), so this
		// fires for a misconfigured CORNUS_CONDUIT / profile listen. socks5.Start
		// stays the authoritative backstop for hostnames that only resolve off-host.
		if !cfg.Socks5AllowNonLoopback && socks5.LooksNonLoopback(cfg.Socks5Listen) {
			return nil, fmt.Errorf("refusing to bind the SOCKS5 conduit to non-loopback address %q: this proxy has no authentication and dials arbitrary destinations from this host, so off-host it is an open proxy for anyone who can reach it (use a loopback listen address, or run `cornus socks5 --allow-non-loopback` for an intentional off-host proxy)", cfg.Socks5Listen)
		}
