...
func (m Mode) Valid() bool {
	return m == "" || m == ModePortForward || m == ModeSocks5 || m == ModeNone
}
...
// Identity is the canonical, comparable identity of a resolved conduit configuration.
type Identity struct {
	Mode, Listen, Suffix, Rules, BareNames, Session     string
	IngressMode, IngressSuffix, IngressCA, IngressCAKey string
	Controller                                          string
	Certificates                                        string
}

func (c Config) Identity(session string) Identity {
	rules := ""
	for _, r := range c.Socks5Resolve {
		rules += r.Pattern + "=" + r.Replace + "\n"
	}
	bare := ""
	if c.Socks5BareServiceNames != nil {
		if *c.Socks5BareServiceNames {
			bare = "on"
		} else {
			bare = "off"
		}
	...
	local := ""
	if c.Socks5SessionLocal {
		local = session
	}
	var im, is, ica, ick, controller, certificates string
	if c.Ingress != nil {
		im, is, ica, ick = string(c.Ingress.Mode), c.Ingress.SuffixDomain, c.Ingress.CAFile, c.Ingress.CAKeyFile
		for _, cert := range c.Ingress.Certificates {
			certificates += cert.Pattern + "=" + cert.CertFile + "=" + cert.KeyFile + "\n"
		}
		if c.Ingress.Controller != nil {
			controller = fmt.Sprintf("%s|%s|%s|%d|%d", c.Ingress.Controller.KubeContext, c.Ingress.Controller.Namespace, c.Ingress.Controller.Service, c.Ingress.Controller.HTTPPort, c.Ingress.Controller.HTTPSPort)
		}
	}
	return Identity{string(c.Mode), c.Socks5Listen, c.Socks5Suffix, rules, bare, local, im, is, ica, ick, controller, certificates}
}
...
const (
	IngressOff     IngressMode = ""
	IngressNative  IngressMode = "native"
	IngressEmulate IngressMode = "emulate"
)

func (m IngressMode) Valid() bool {
	return m == IngressOff || m == IngressNative || m == IngressEmulate
}
...
func Start(ctx context.Context, d portfwd.Dialer, cfg Config, opts ...Option) (Conduit, error) {
	...
	case ModeSocks5:
		// A conduit with an agreed address goes through the rendezvous, so another
		// process asking for an address this one covers JOINS it rather than forking a
		// second proxy that then cannot bind. An ephemeral one is private by
		// construction and keeps the direct path.
		if o.registry != nil && !ephemeralListen(cfg) {
			return startRendezvous(ctx, d, cfg, o)
		}
	...
	default: // ModePortForward and "": Validate rejected every other mode.
		return &portForwardConduit{d: d, logf: o.logf}, nil
	}
}

// Validate reports whether c can be started: a known mode, and — in SOCKS5 mode —
// a bind address that is loopback unless the caller explicitly opted into off-host
// exposure with Socks5AllowNonLoopback (`--allow-non-loopback`). Start calls it, so
// it is never bypassed; callers that hand a config to something that starts the
// conduit LATER (the detached `compose up -d` agent) also call it up front, so a
// bad selector fails at the flag rather than inside a background process. The
// non-loopback rejection lands here, before binding, as a friendlier and earlier
// error than socks5.Start's post-bind refusal — which stays the authoritative
// backstop for hostnames that only resolve off-host.
func (c Config) Validate() error {
	switch c.Mode {
	case ModeNone, ModePortForward, "":
		return nil
	case ModeSocks5:
		if !c.Socks5AllowNonLoopback && socks5.LooksNonLoopback(c.Socks5Listen) {
			return fmt.Errorf("refusing to bind the SOCKS5 conduit to non-loopback address %q: this proxy has no authentication and dials arbitrary destinations from this host, so off-host it is an open proxy for anyone who can reach it (use a loopback listen address, or pass --allow-non-loopback to accept that exposure)", c.Socks5Listen)
		}
		return nil
	default:
		return fmt.Errorf("unknown conduit mode %q (want %q, %q, or %q)", c.Mode, ModePortForward, ModeSocks5, ModeNone)
	}
}
...
func (e *portForwardConduit) CAInfo() []string { return nil }
...
type socks5Conduit struct {
	...
	ingress *IngressConfig
	// ingressMux multiplexes emulated ingresses that share a host:port onto one
	// listener so overlapping paths resolve by longest match; built once by mux().
	ingressMux     *ingressemu.Mux
	ingressMuxOnce sync.Once
...
// Addr is the address the proxy actually bound, which is the address a browser
// must be pointed at. It is deliberately NOT on the Conduit interface — only a
// SOCKS5 conduit has a single answer, and port-forward has one address per
// forwarded port. Callers that want it type-assert for it (the client agent does,
// to name the holder of a contended bind address).
//
// It asks the live listener rather than the configuration because the everyday
// session-local spelling is an ephemeral "127.0.0.1:0", which has no answer until
// it is bound.
func (e *socks5Conduit) Addr() string { return e.proxy.Addr() }
func (e *socks5Conduit) CAInfo() []string {
	var info []string
	if e.ingress != nil {
		for _, cert := range e.ingress.Certificates {
			pattern := cert.Pattern
			if pattern == "" {
				pattern = "DNS SANs"
			}
			info = append(info, fmt.Sprintf("emulated ingress TLS certificate for %s: %s", pattern, cert.CertFile))
		}
	}
	if e.ca == nil {
		return info
	...
	if e.ca.CertPath() == "" {
		return append(info, "emulated ingress TLS fallback CA is in memory; trust the emitted certificate explicitly")
	}
	return append(info, "emulated ingress TLS fallback CA: "+e.ca.CertPath())
}
...
func (e *socks5Conduit) AddLocal(ctx context.Context, host string, port int, d socks5.LocalDialer) (bool, error) {
	...
	reg := e.router.RegisterLocal(host, port, d)
	go func() {
		<-ctx.Done()
		// Withdraw THIS claim, not whatever currently serves host:port. With several
		// publishers on one subject — two projects fronting the same ingress host —
		// withdrawing by key would remove the claim that is serving, which is usually
		// somebody else's.
		e.router.UnregisterLocal(reg.Handle)
	}()
	return true, nil
}
...
func (e *socks5Conduit) addNativeIngress(ctx context.Context, name string, in *api.IngressSpec, ports []api.PortMapping) ([]string, error) {
	...
	handles := make([]socks5.LocalHandle, 0, len(hosts)*len(regs))
	for _, h := range hosts {
		for port, ld := range regs {
			handles = append(handles, e.router.RegisterLocal(h, port, ld).Handle)
		}
	}
	go func() {
		<-ctx.Done()
		for _, h := range handles {
			e.router.UnregisterLocal(h)
		}
	}()
	return hosts, nil
}

// mux lazily builds (once) the shared emulated-ingress Mux, wiring its listener
// publish/withdraw to this conduit's SOCKS5 router.
func (e *socks5Conduit) mux() *ingressemu.Mux {
	e.ingressMuxOnce.Do(func() {
		e.ingressMux = ingressemu.NewMux(
			func(host string, port int, lis *memlisten.Listener) func() {
				reg := e.router.RegisterLocal(host, port, lis)
				return func() { e.router.UnregisterLocal(reg.Handle) }
			},
		)
	})
	return e.ingressMux
...
// addEmulatedIngress serves a client-side reverse proxy for the ingress through the
// shared ingress Mux, which publishes each host at every port it listens on (:80, plus
// :443 when the spec requests TLS) and folds ingresses that share a host:port onto one
// listener so overlapping paths resolve by longest match instead of clobbering.
func (e *socks5Conduit) addEmulatedIngress(ctx context.Context, name string, in *api.IngressSpec, ports []api.PortMapping) ([]string, error) {
	...
	hosts, cleanup, err := e.mux().Add(ingressemu.Config{
		Dialer:       e.d,
		Workload:     name,
		Spec:         in,
		Ports:        ports,
		...
		CA:           ca,
		Certificates: e.ingress.Certificates,
	})
	if err != nil {
		return nil, err
	}
...
func (noopConduit) CAInfo() []string { return nil }
