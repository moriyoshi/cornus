...
func TestStartRejectsNonLoopbackConduit(t *testing.T) {
	// The session conduit has no non-loopback opt-in, so a non-loopback listen is
	// refused at the config layer before any bind.
	for _, addr := range []string{"0.0.0.0:1080", ":1080", "192.168.1.5:1080"} {
		e, err := Start(context.Background(), fakeDialer{}, Config{Mode: ModeSocks5, Socks5Listen: addr})
		if err == nil {
			e.Close()
			t.Fatalf("Start(Socks5Listen=%q) = nil error, want refusal", addr)
		}
	}
}

func TestStartAllowsNonLoopbackWithOptIn(t *testing.T) {
	// `cornus socks5 --allow-non-loopback` sets Socks5AllowNonLoopback; the guard
	// must defer to socks5.Start, which then binds the address.
	e, err := Start(context.Background(), fakeDialer{}, Config{
		Mode:                   ModeSocks5,
		Socks5Listen:           "127.0.0.1:0", // loopback so the bind actually succeeds in CI
		Socks5AllowNonLoopback: true,
	})
	if err != nil {
		t.Fatalf("Start with opt-in errored: %v", err)
	}
	e.Close()
}
