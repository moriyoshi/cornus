...
// TestAgentRelayInstanceIsTheFirstInstance pins the wire format of the one key both ends
// of ssh-agent forwarding must agree on. The AGREEMENT itself is structural (a single
// constructor, called by pkg/server's registration and the kubernetes backend's
// Config.Instance), so what is left to pin here is that the string does not silently
// change shape — and that it addresses the FIRST instance, which is the replica exec
// always targets.
func TestAgentRelayInstanceIsTheFirstInstance(t *testing.T) {
	if got, want := AgentRelayInstance("web"), InstanceKey("web", 0); got != want {
		t.Errorf("AgentRelayInstance = %q, want %q — exec targets the first instance", got, want)
	}
	if got, want := AgentRelayInstance("web"), "web/0"; got != want {
		t.Errorf("AgentRelayInstance = %q, want %q", got, want)
	}
}
