...
//
// The counters answer four different failure modes that all look identical from
...
//   - Stale rising means the backend keeps re-serving one reading, so the poll
//     is finer than the source's own resolution. Nothing is broken; the series
//     simply cannot be denser than what the source publishes.
//
// Unsupported answers a fifth, which the counters cannot: the family was never
// collectable on this backend at all.
type MetricsStatus struct {
	...
	// has no source for, so they will stay empty however long a caller waits —
	// `container_network_io` on kubernetes, whose metrics-server source reports
	// only CPU and memory. Names are the recorded (dotted) spelling; the store
	// resolves those to their underscored PromQL form at query time, so a consumer
	// comparing against a query string should fold dots to underscores.
	//
	// Empty means "nothing known to be missing", which is also what an older
	// server that does not send the field looks like — a consumer must treat the
	// absence as no claim rather than as a claim of full support.
