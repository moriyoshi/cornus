...
func (s *store) QueryPromQL(ctx context.Context, expr string, start, end time.Time, step time.Duration) ([]Series, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("obsstore: promql: %w%s", err, duplicateTimestampHint(err))
	}
	return convertSeries(series), nil
}

// duplicateTimestampHint appends an explanation to the engine's
// duplicate-timestamp rejection, or "" for any other error.
//
// The engine refuses to evaluate a series holding two datapoints at one
// timestamp, and it refuses at READ time — so one bad write makes every later
// query of that metric fail, over every window, with a message that names
// neither the metric nor the writer. An operator who hits this sees only that
// their charts are empty while `observe status` reports readings recorded, which
// is why this is worth spelling out at the one place cornus can still see the
// query that provoked it.
//
// Matching on the message is admittedly brittle: the engine exports no sentinel
// for this, and a reworded upstream message costs the hint, not correctness —
// the original error is returned either way. Rejecting the duplicate at INGEST,
// where it is attributable to the export that carried it, belongs upstream and
// is filed as moriyoshi/imbh#27.
func duplicateTimestampHint(err error) string {
	if err == nil || !strings.Contains(err.Error(), "duplicate timestamps") {
		return ""
	}
	return " (a producer wrote two datapoints at one timestamp in one series," +
		" which makes every query of that metric fail until the offending points age out" +
		" of the retention window; check whether an exporter is re-sending one reading" +
