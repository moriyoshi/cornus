...
type blockServer struct {
	...
	opScratch  sync.Pool
	reqScratch sync.Pool // whole-frame buffers for large inbound requests (writes)
	scratch    []byte
}
...
func serveBlockServerFS(conn net.Conn, att p9.Attacher, chunkSize int64, opts ...BlockOpt) {
	...
	s.reqScratch.New = func() any { b := make([]byte, s.maxFrame); return &b }
	s.loop()
}
...
func (s *blockServer) readRequest() (frame, func(), error) {
	h, err := readFrameHeader(s.conn, s.maxFrame)
	if err != nil {
	...
	if h.plen <= bigRequest {
		...
		if _, err := blitReadFull(blitWireRead, s.conn, f.payload); err != nil {
			return frame{}, nil, err
		}
		return f, func() {}, nil
	}
	bp := s.reqScratch.Get().(*[]byte)
	f.payload = (*bp)[:h.plen]
	if _, err := blitReadFull(blitWireRead, s.conn, f.payload); err != nil {
		s.reqScratch.Put(bp)
		return frame{}, nil, err
