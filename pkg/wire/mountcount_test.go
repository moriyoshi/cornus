...
// TestBufferedReceiveDoesNotStageBulk pins the copy budget of the receive
// buffers, which is the one cost they add and the one thing no other test can
// see.
//
// A receive buffer collapses the header-plus-small-payload case to a single read.
// The price is that bufio only bypasses its buffer for a large read when the
// buffer is EMPTY, so a little of each bulk payload is copied through it. That
// price is bounded by one buffer per frame — UNLESS the buffer is sized at or
// above a frame, at which point the bypass never fires and every payload byte is
// staged. Nothing else would notice: the bytes still land in the right place,
// `blitWireRead` still totals the same, allocation does not move (the buffer is
// per-session and reused), and only wall time would drift.
//
// Requires -tags blitprof; the counters are compiled out otherwise.
func TestBufferedReceiveDoesNotStageBulk(t *testing.T) {
	if !BlitProfiling {
		t.Skip("build with -tags blitprof to check the receive-buffer copy budget")
	}
	const total = 4 << 20
	dir := t.TempDir()
	cl, _ := countingHarness(t, dir, nil)
	...
	// Correctness control: the payload really moved. Without it a harness where
	// every op failed would stage nothing and satisfy any ceiling.
	const slack = 256 << 10
	if wire := got[blitWireRead]; wire < 2*total || wire > 2*total+slack {
		t.Fatalf("wire-read = %d for %d moved each way; the workload did not run as expected", wire, total)
	}
	// Budget: one buffer per frame, not one payload. Measured ~0.8% of payload.
	staged := got[blitBufCopy]
	if staged > slack {
		t.Fatalf("buf-copy = %d bytes for %d moved (budget %d). The receive buffer is "+
			"staging bulk instead of letting bufio bypass it — check that blockReadBuf "+
			"(%d) is well below the frame size", staged, 2*total, slack, blockReadBuf)
	}
	// And a FLOOR, because a ceiling alone is satisfied by a counter that stopped
	// counting — which is not hypothetical: breaking the attribution so it never
	// reports a staged byte passes the ceiling above with room to spare. Every
	// frame header is read into an empty buffer smaller than the buffer size, so
	// bufio stages it by construction; across this many frames that cannot be zero.
	if staged == 0 {
		t.Fatal("buf-copy = 0 while a buffered reader is in use. Every frame header is " +
			"staged by construction, so this means the attribution stopped counting, " +
			"not that the buffer became free")
	}
}
