...
type blockOpts struct {
	...
	// not negotiated.
	readahead int64
	// ownerUID/ownerGID replace the ownership reported for every file in this
	// mount, and ownerSet distinguishes "report uid 0" from "report the file's
	// own uid" — 0 is a meaningful id, so the zero value cannot mean unset.
...
// WithReportedOwner makes the proxy report uid/gid as the owner of every file in
// the mount, instead of passing through the ids the caller's filesystem holds.
//
// It exists because the caller's ids and the workload's are unrelated id spaces.
// The export lives on the CALLER's machine and carries that machine's uids; the
// workload reads it inside a user namespace that maps a different range. The
// kernel resolves what the 9P server reports through the mounting namespace and
// then back out through the container's, so an id the container's map does not
// cover surfaces as 65534 — the OVERFLOW uid, not an owner — and no mode bit
// rescues it, because a userns root holds CAP_DAC_OVERRIDE only over ids inside
// its own map.
//
// So the useful answer is not a range shift but a flat statement of ownership:
// the workload owns what it was given, which is the same semantics the credential
// file delivery applies by chowning to the workload's ids. Callers pass the HOST
// ids that the workload's container-side ids map to (deploy.HostIDsFor).
func WithReportedOwner(uid, gid uint32) BlockOpt {
	return func(o *blockOpts) { o.ownerUID, o.ownerGID, o.ownerSet = uid, gid, true }
}
...
func (bc *blockClient) readPayload(f *frame, dst []byte, metaLen, plen int) (int, error) {
	...
	if _, err := blitReadFull(blitWireRead, bc.conn, f.payload); err != nil {
		return 0, err
	}
	return blitReadFull(blitWireRead, bc.conn, dst[:plen-metaLen])
}

func (bc *blockClient) readLoop() {
	for {
		h, err := readFrameHeader(bc.conn, bc.maxFrame)
		if err != nil {
			bc.fail(err)
			return
		}
