...
// BenchmarkInsertTxnDisk_* are the same workload over the PRODUCTION on-disk cache
// store (CORNUS_FILE_CACHE=1), so the disk backend's per-write allocation profile
// can be compared 1:1 against the MemStore variants above.
func BenchmarkInsertTxnDisk_Sync(b *testing.B) {
	benchInsert(b, yamux.SendSync, 4, 0, "NORMAL", diskStore(b))
}
func BenchmarkInsertTxnDisk_Batched(b *testing.B) {
	benchInsert(b, yamux.SendBatchedPipelined, 4, 0, "NORMAL", diskStore(b))
}
