...
func Backing9PSocketCached(sess *yamux.Session, name string, onRx, onTx func(int), cache *blockcache.Cache, opts ...BlockOpt) (string, func(), error) {
	mode := proxyPipe
	if cache != nil {
		mode = proxyReadCache
	}
	return backing9PSocket(sess, name, onRx, onTx, cache, mode, opts...)
}
...
// A nil cache selects NO-CACHE mode: the same protocol with nothing retained
// (reads miss-fill from the caller, writes are written through), for a mount that
// wants the block protocol without provisioning cache storage. Pair a CACHED mount
// with cache=mmap (async writeback).
func Backing9PSocketBlock(sess *yamux.Session, name string, onRx, onTx func(int), cache *blockcache.Cache, opts ...BlockOpt) (string, func(), error) {
	return backing9PSocket(sess, name, onRx, onTx, cache, proxyBlockWrite, opts...)
}
...
func backing9PSocket(sess *yamux.Session, name string, onRx, onTx func(int), cache *blockcache.Cache, mode proxyMode, opts ...BlockOpt) (string, func(), error) {
	...
	go func() {
		for {
			...
			go tunnel9P(conn, sess, name, onRx, onTx, cache, mode, opts...)
		}
	}()
	cleanup := func() {
		l.Close()
...
func tunnel9P(conn net.Conn, sess *yamux.Session, name string, onRx, onTx func(int), cache *blockcache.Cache, mode proxyMode, opts ...BlockOpt) {
	defer conn.Close()
	...
	case proxyBlockWrite:
		...
		// Caller options come AFTER the environment's, so an explicit
		// WithReportedOwner from the server wins over anything the operator set.
		ServeBlockProxy(c, stream, cache, name, append(BlockEnvOpts(), opts...)...)
	case proxyReadCache:
		// Terminate 9P in userspace and serve reads from the block cache.
		ServeCachingProxy(c, stream, cache, name)
	default:
		// Splice frames between the kernel mount and the caller's export. With an
		// owner set, the splice additionally rewrites the ownership in each
		// Rgetattr — the raw-9P equivalent of the block proxy's WithReportedOwner,
		// and far cheaper than terminating 9P just to change two fields.
		if o := resolveBlockOpts(opts); o.ownerSet {
			pipeMappingOwner(c, stream, o.ownerUID, o.ownerGID)
		} else {
			pipe(c, stream)
		}
	}
