...
const (
	...
	tagTelemetry     = 'T'
	tagFSOp          = 'S' // structured filesystem operation (see fsop.go)
)
...
// TagLazy9P identifies the per-context 9P backing stream for lazy bind mounts:
// the build server opens one per on-demand lazy-context read, and the caller's
// single accept loop must route it apart from the SSH streams sharing that
// connection.
//
// Exported because that accept loop lives in pkg/build/buildwire, which used to
// restate the byte as its own `tagLazy9P = 'L'` with a comment noting it
// "mirrors wire's private lazy-9P backing tag". Two literals for one wire byte:
// the writer is here and the dispatcher is there, so a change on this side would
// leave buildwire routing a tag nothing sends while lazy-context reads went
// unhandled. It is now one constant, dispatched through this name — the same
// arrangement the nine other cross-package tags already use.
const TagLazy9P = tagLazy9P
...
// TagTelemetry identifies a caretaker telemetry stream: the caretaker opens one
// per OTLP export, writes the signal name, then a length-prefixed export body,
// and reads a one-byte status back.
//
// It exists so a workload's telemetry can reach the server over the connection
// the caretaker ALREADY holds, instead of dialing the server's HTTP endpoint. In
// a pod with no route to that endpoint — a locked-down NetworkPolicy, an
// isolated network, no advertised URL — that is the difference between telemetry
// working and not. It also means the pod needs no credential of its own for the
// export: the connection is already authenticated.
const TagTelemetry = tagTelemetry

// telemetryMaxBody bounds one export body on the mux, mirroring the HTTP
// receiver's cap so a stream cannot be used to sidestep it.
const telemetryMaxBody = 16 << 20

// OpenTelemetry opens a caretaker telemetry stream, writes the signal name and
// the export body, and waits for the server's acknowledgement.
//
// One stream per export, rather than a long-lived one carrying many: yamux
// streams are cheap, and a per-export stream means a failed export cannot desync
// the framing for the next one. The ack is what lets the caretaker translate the
// result back into an HTTP status for the Collector, so its retry logic keeps
// working unchanged.
func OpenTelemetry(sess *yamux.Session, signal string, body []byte) error {
	if len(body) > telemetryMaxBody {
		return fmt.Errorf("telemetry export too large: %d bytes (limit %d)", len(body), telemetryMaxBody)
	}
	stream, err := openTagged(sess, tagTelemetry)
	if err != nil {
		return err
	}
	defer stream.Close()
	if _, err := io.WriteString(stream, signal+"\n"); err != nil {
		return err
	}
	var hdr [4]byte
	...
	return readTelemetryAck(stream)
}

// Telemetry acknowledgement bytes. They exist so the caretaker can map the
// server's outcome onto the HTTP status its local Collector expects: accepted,
// retry later, or give up on this payload.
const (
	TelemetryAckOK     = byte('o')
	TelemetryAckRetry  = byte('r')
	TelemetryAckReject = byte('x')
)

// telemetryAckTimeout bounds the wait for the server's verdict, so a server that
// stops answering cannot wedge the caretaker's export handler (and behind it the
// Collector's export queue). A var so tests can shorten it.
var telemetryAckTimeout = 30 * time.Second

// ErrTelemetryRetry reports that the server shed this export and the sender
// should retry, mirroring the HTTP receiver's 429.
var ErrTelemetryRetry = errors.New("telemetry: server is shedding load, retry")

// readTelemetryAck reads the server's one-byte verdict.
func readTelemetryAck(stream net.Conn) error {
	_ = stream.SetReadDeadline(time.Now().Add(telemetryAckTimeout))
	var ack [1]byte
	if _, err := io.ReadFull(stream, ack[:]); err != nil {
		return fmt.Errorf("telemetry: reading ack: %w", err)
	}
	switch ack[0] {
	case TelemetryAckOK:
		return nil
	case TelemetryAckRetry:
		return ErrTelemetryRetry
	default:
		return errors.New("telemetry: server rejected the export")
	}
}

// ReadTelemetryHeader reads a telemetry stream's signal line and body, after the
// tag byte has been consumed. The server side calls it.
func ReadTelemetryHeader(stream net.Conn) (signal string, body []byte, err error) {
	br := bufio.NewReader(stream)
	line, err := br.ReadString('\n')
	if err != nil {
		return "", nil, fmt.Errorf("telemetry: reading signal: %w", err)
	}
	signal = strings.TrimSpace(line)
	var hdr [4]byte
	if _, err := io.ReadFull(br, hdr[:]); err != nil {
		return "", nil, fmt.Errorf("telemetry: reading length: %w", err)
	}
	n := binary.BigEndian.Uint32(hdr[:])
	if n > telemetryMaxBody {
		return "", nil, fmt.Errorf("telemetry: export of %d bytes exceeds the %d limit", n, telemetryMaxBody)
	}
	body = make([]byte, n)
	if _, err := io.ReadFull(br, body); err != nil {
		return "", nil, fmt.Errorf("telemetry: reading body: %w", err)
	}
	return signal, body, nil
}

// WriteTelemetryAck sends the server's verdict for one export.
func WriteTelemetryAck(stream net.Conn, ack byte) error {
