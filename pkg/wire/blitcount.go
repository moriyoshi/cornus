...
const (
	...
	blitUserCopy
	// blitBufCopy: the SUBSET of blitWireRead that additionally passed through a
	// receive buffer instead of landing directly. It is a budget, not a defect:
	// buffering the header costs one buffer's worth per frame. It becomes a defect
	// when it approaches the payload, which is what happens if the buffer is ever
	// sized at or above a frame and bufio stops taking its large-read bypass.
	blitBufCopy
	numBlitKinds
)
