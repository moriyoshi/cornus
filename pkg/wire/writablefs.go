...
// linuxOAppend is the Linux O_APPEND open flag (02000). 9p2000.L open flags are
// Linux open flags carried on the wire, so this is a protocol constant, not the
// host OS's flag — using golang.org/x/sys/unix here would break cross-compilation
// to non-Unix targets (Windows), where unix.O_APPEND is undefined.
const linuxOAppend p9.OpenFlags = 0o2000
