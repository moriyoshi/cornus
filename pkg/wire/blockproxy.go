...
func ServeBlockProxy(kernelConn, remoteStream net.Conn, cache *blockcache.Cache, mount string, opts ...BlockOpt) {
	...
	// NO-CACHE mode: a nil cache means serve the mount over the block protocol
	// while retaining nothing. Correctness does not depend on the cache — the
	// caller is authoritative, reads miss-fill from it and writes are written
	// through — so this is the same protocol with the accelerator removed.
	//
	// The store is a NullStore rather than nil because the Rename and unlink
	// paths call into the store WITHOUT consulting cacheable, so a nil store
	// would panic there. noCache additionally keeps files out of the caching
	// read/write paths entirely, which is what makes this a true bypass
	// (rawReadAt) rather than a cache that always misses.
	noCache := cache == nil
	chunk := int64(blockcache.DefaultChunkSize)
	var store blockcache.Store = blockcache.NewNullStore()
	if !noCache {
...
type blockAttach struct {
	...
	// noCache serves the mount over the block protocol while retaining nothing;
	// files are never marked cacheable, so reads take rawReadAt and writes skip
	// the cache bookkeeping. See ServeBlockProxy.
	noCache bool
	// ownership reported to the kernel for every file, when ownerSet. See
	// WithReportedOwner.
...
func (f *blockProxyFile) WalkGetAttr(names []string) ([]p9.QID, p9.File, p9.AttrMask, p9.Attr, error) {
	...
	qids := getQIDs(r)
	mask := getAttrMask(r)
	attr := f.a.reportOwner(mask, getAttr(r))
	child := &blockProxyFile{a: f.a, h: newH, rel: joinRel(f.rel, names)}
	child.observe(mask, attr)
	return qids, child, mask, attr, nil
}

// reportOwner rewrites the ownership in an attribute response to the workload's
// own ids. It is a no-op unless WithReportedOwner was given, so every backend
// whose runtime does not remap ids keeps passing the caller's ids through
// untouched.
//
// It honours the mask rather than writing unconditionally: an attribute the
// client did not request carries no meaningful value, and rewriting it would put
// ownership into a response that never asked about ownership.
func (a *blockAttach) reportOwner(mask p9.AttrMask, attr p9.Attr) p9.Attr {
	if !a.ownerSet {
		return attr
	}
	if mask.UID {
		attr.UID = p9.UID(a.ownerUID)
	}
	if mask.GID {
		attr.GID = p9.GID(a.ownerGID)
	}
	return attr
}
...
// rawReadAt reads without caching (non-regular files / not opened cacheable, and
// every file in NO-CACHE mode).
//
// It issues a RANGED read of exactly the bytes asked for. It used to read the
// whole covering chunk and discard the remainder, which was fine while this was
// the rare path it was written for ("directories don't ReadAt") but is read
// amplification once no-cache mode makes it the hot path: with a 1 MiB chunk, a
// kernel read smaller than that fetched 1 MiB to serve it and kept nothing.
// Measured on the docker host-mount path, that cost roughly half the sequential
// read throughput of the raw 9P pipe.
//
