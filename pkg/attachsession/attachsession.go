...
// deploy logs (e.Log) and surface deploy errors (e.Err); dockerproxy uses it to
// capture the terminal event's exit code (docker logs arrive over a separate
// endpoint, so that is all it wants). f must not block.
