...
// session closes).
// It owns a dispatcher of its own; see runPortForwardAccept for why runCaretakerConn
// registers the handler on a shared one instead.
func serveIngress(ctx context.Context, sess *yamux.Session, targets map[string]hubTarget) error {
	d := newTagDispatch()
	registerIngress(d, targets)
	return d.run(ctx, sess)
}

// registerIngress wires the TagDeliver handler onto d.
func registerIngress(d *tagDispatch, targets map[string]hubTarget) {
	d.handle(wire.TagDeliver, func(stream net.Conn) { deliverIngress(stream, targets) })
}
