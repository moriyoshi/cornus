...
func TestFSOpRootsMatchOnPathBoundaries(t *testing.T) {
	...
	if root, _, ok := fsoppkg.Resolve(fsopRoots(normalizeFSOpRoots(nested)), "/data/inner/marker"); !ok || root.Path != database {
		t.Fatalf("nested root lost to its parent: %+v (ok=%v)", root, ok)
	}
}
...
func TestFSOpDropsMalformedRoots(t *testing.T) {
	...
	if _, _, ok := fsoppkg.Resolve(fsopRoots(got), "/anything"); ok {
