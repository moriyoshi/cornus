...
func TestResolveURL(t *testing.T) {
	...
	if got, want := resolveURL(cfg, cfg.bindAddr, nil, ln), "https://"+portStr+".tun.example"; got != want {
		t.Fatalf("resolveURL(template) = %q, want %q", got, want)
	}

	// No template falls back to tcp://host:port.
	cfg = sshConfig{addr: "sish.example:22"}
	if got, want := resolveURL(cfg, cfg.bindAddr, nil, ln), "tcp://sish.example:"+portStr; got != want {
...
// TestWithBindHost proves a requested hostname replaces the bind host while
// keeping the configured port — the mechanism by which an ingress tunnel asks a
// sish-style relay to publish it under the ingress hostname.
func TestWithBindHost(t *testing.T) {
	for _, tc := range []struct{ bindAddr, host, want string }{
		{"0.0.0.0:0", "app.example.com", "app.example.com:0"},
		{"0.0.0.0:8080", "app.example.com", "app.example.com:8080"},
		// An address with no parseable port lets the relay choose one.
		{"", "app.example.com", "app.example.com:0"},
		{"garbage", "app.example.com", "app.example.com:0"},
	} {
		if got := withBindHost(tc.bindAddr, tc.host); got != tc.want {
			t.Errorf("withBindHost(%q,%q) = %q, want %q", tc.bindAddr, tc.host, got, tc.want)
		}
	}
}
