...
func (provider) Start(ctx context.Context, cred tunnel.Credential, opts tunnel.Options) (tunnel.Session, error) {
	...
	// A requested hostname becomes the remote bind host: sish (and compatible
	// relays) publish a tunnel under the host it was asked to bind, so this is how
	// an ingress tunnel gets its own ingress hostname and needs no Host handling
	// at all. The relay is free to refuse and bind something else; the URL it
	// reports stays authoritative.
	bindAddr := cfg.bindAddr
	if opts.Domain != "" {
		bindAddr = withBindHost(bindAddr, opts.Domain)
	}
	ln, err := client.Listen("tcp", bindAddr)
	if err != nil {
		_ = client.Close()
		return nil, fmt.Errorf("ssh: remote-forward request (%s): %w", bindAddr, err)
	}

	url := resolveURL(cfg, bindAddr, client, ln)
...
// to tcp://host:port.
func resolveURL(cfg sshConfig, bindAddr string, client *ssh.Client, ln net.Listener) string {
...
// withBindHost replaces the host part of a "host:port" remote bind address,
// keeping its port. A bindAddr with no parseable port is replaced wholesale with
// "<host>:0", letting the relay choose.
func withBindHost(bindAddr, host string) string {
