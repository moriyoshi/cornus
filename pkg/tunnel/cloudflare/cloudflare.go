...
// Options.Domain is ignored: Cloudflare quick tunnels hand out a hostname of their own
// choosing and cannot be asked for a specific one. That is why Domain is
// documented as best-effort — the caller reads the URL the session reports
// and adapts (an ingress tunnel falls back to aliasing the hostname).
