...
func (provider) Start(ctx context.Context, cred tunnel.Credential, opts tunnel.Options) (tunnel.Session, error) {
	...
	if opts.Domain != "" {
		// A reserved/custom domain on the account. Requesting the ingress hostname
		// itself is what lets an ingress tunnel pass requests through untouched,
		// so it is worth asking for even though most accounts cannot grant it —
		// the caller adapts from the URL the session reports.
		eopts = append(eopts, ngrok.WithURL("https://"+opts.Domain))
	}
