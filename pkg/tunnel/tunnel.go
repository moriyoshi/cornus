...
type Options struct {
	...
	Hostname string
	// Domain is a REQUESTED public hostname, best-effort. An ingress tunnel asks
	// for the ingress host itself, because a tunnel whose public hostname already
	// is the ingress host needs no Host adjustment at all — the request arrives
	// exactly as the app expects it.
	//
	// Backends that cannot grant a specific hostname (Cloudflare quick tunnels,
	// Tailscale funnel) ignore it; the caller must treat the URL the Session
	// reports as authoritative and adapt, never assume the request was honored.
	Domain string
}
