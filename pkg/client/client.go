...
type Client struct {
	...
	tls   *tls.Config
	token string
	// tokenFunc, when set, supplies the bearer token per use instead of pinning the
	// one resolved at construction. A Client can outlive the credential it was built
	// with — the background agent caches a *Client for the life of a connection,
	// while an SSH-key session expires in an hour — so a baked-in string would go
	// stale with no way to refresh it. Providers own their own caching and locking;
	// see clientconn.sshKeyTokenProvider. Failures fall back to token.
	tokenFunc func() (string, error)
	// refresh, when set, obtains a REPLACEMENT credential after the server has
	// rejected the current one with 401. See WithCredentialRefresher.
	refresh func(context.Context) (string, error)
	// refreshMu guards token against a refresh racing concurrent requests. Only
	// the refresh path writes it; bearer() reads it.
...
// WithTokenFunc supplies the bearer token lazily, once per use, for credentials
// that expire while the Client is still alive. It takes precedence over WithToken,
// which remains the fallback when the provider errors or yields an empty token —
// presenting the last known credential and letting the server answer 401 gives a
// far better diagnostic than sending no Authorization header at all.
func WithTokenFunc(fn func() (string, error)) Option {
	return func(c *Client) { c.tokenFunc = fn }
}

// bearer resolves the credential for one request. Providers are expected to be
// cheap (a cached value, refreshed only near expiry).
func (c *Client) bearer() string {
	if c.tokenFunc != nil {
		if tok, err := c.tokenFunc(); err == nil && tok != "" {
			return tok
		}
	}
	c.refreshMu.RLock()
...
// WithCredentialRefresher supplies a function that obtains a REPLACEMENT
// credential once the server has rejected the current one with 401, so a
// credential that has become invalid before its stated expiry does not fail every
// command until it lapses.
//
// The case it exists for is a cached exchanged token (see clientconn's token
// exchange): the cache honours the expiry the SERVER reported, which is exactly
// right until the server stops accepting the token early — it restarted with a
// new signing key, the token was revoked, the scope mapping changed. Then every
// invocation reads the same dead token out of the cache and fails, and RE-RUNNING
// DOES NOT HELP, for as long as an hour. That is the shape of the bug: not a
// failure, but a failure that looks permanent and is not.
//
// Contract, and the parts that are deliberately narrow:
//
//   - It runs at most ONCE per request. No backoff loop; if the replacement is
//     also rejected, that is the caller's answer.
//   - The request is replayed only when the refresher returns a token DIFFERENT
//     from the one that was rejected. A refresher that hands back the same string
//     has not fixed anything, and replaying would only produce a second identical
//     401.
//   - Only rewindable bodies are replayed (nil, or a *bytes.Reader — what every
//     JSON call site passes). An arbitrary stream cannot be re-read, and buffering
//     one to make it retryable would turn an upload into an unbounded allocation.
//   - A 401 carrying a CornusSSH challenge is NOT a credential failure. It is the
//     first leg of the SSH-key proof handshake, where 401 is the protocol (see
//     sshProofRoundTrip). Refreshing there would re-exchange on every key-auth
//     handshake, which is why the discriminator is the WWW-Authenticate header
//     rather than a list of exempt paths.
//
// Replaying is safe on the server side regardless of method: a 401 is produced by
// the auth middleware before any handler runs, so the rejected request had no
// effect to duplicate.
func WithCredentialRefresher(fn func(context.Context) (string, error)) Option {
	return func(c *Client) { c.refresh = fn }
}
...
func (c *Client) RegistryToken() string { return c.bearer() }
...
func (c *Client) do(ctx context.Context, method, path string, body io.Reader) (*http.Response, error) {
	...
	if !c.refreshCredential(ctx, req, resp, body) {
		return resp, nil
	}
	// The rejected response is being replaced, so its body must not leak.
	_ = resp.Body.Close()
	_, resp, err = send()
	return resp, err
}

// refreshCredential handles a 401 by asking the refresher for a replacement
// credential, reporting whether the caller should replay the request. See
// WithCredentialRefresher for why each guard is here; violating any one of them
// turns a helpful retry into a silent loop, a double upload, or a re-exchange on
// every SSH-key handshake.
func (c *Client) refreshCredential(ctx context.Context, req *http.Request, resp *http.Response, body io.Reader) bool {
	if c.refresh == nil {
		return false
	}
	// A 401 that carries an SSH-key challenge is the proof handshake, not a stale
	// credential.
	if strings.HasPrefix(resp.Header.Get("WWW-Authenticate"), "CornusSSH") {
		return false
	}
	if !rewind(body) {
		return false
	...
	if err != nil || fresh == "" {
		// Report it and keep the server's 401: the refresher failing is not a
		// better error than "the server rejected your credential", and hiding the
		// original status would be worse.
		if err != nil {
			slog.Debug("refreshing the rejected credential failed", "error", err)
		}
		return false
	}
	sent := strings.TrimPrefix(req.Header.Get("Authorization"), "Bearer ")
	if fresh == sent {
		slog.Debug("the refreshed credential is identical to the rejected one; not replaying")
		return false
	}
	c.refreshMu.Lock()
	c.token = fresh
...
// rewind returns a body to its start so the request can be replayed, reporting
// whether that was possible. A nil body is trivially replayable.
...
func (c *Client) Logs(ctx context.Context, name string, opts api.LogOptions, w io.Writer) error {
	...
	if opts.Instance > 0 {
		// Only sent when non-default: a server that predates the field ignores an
		// unknown parameter, and omitting the zero value keeps the request
		// byte-identical to what it was for the single-replica common case.
		q.Set("instance", strconv.Itoa(opts.Instance))
	}
...
func (c *Client) Stats(ctx context.Context, name string, opts api.StatsOptions, w io.Writer) error {
	...
	// Only sent when non-zero, so a server that predates the parameter keeps
	// behaving identically for every caller that does not select a replica.
	if opts.Instance > 0 {
		q.Set("instance", strconv.Itoa(opts.Instance))
	}
...
func (c *Client) DeployAttach(ctx context.Context, spec api.DeploySpec, events func(deploywire.Event)) error {
	...
	localMounts, localDirs, err := resolveLocalMounts(spec.Mounts)
	if err != nil {
		return err
	}
	as.LocalMounts = localMounts
...
// resolveLocalMounts derives the deploy-attach LocalMounts and the name->dir map
// served over 9P from a spec's mounts. Each caller-local bind source is exported
// under a stable "m<index>" name; a single-file source exports its parent dir
// with the basename as Subpath, and a directory source exports as-is.
//
// A missing bind source is created as an empty directory, matching Docker
// Compose's default create_host_path behavior: Docker's daemon auto-creates a
// nonexistent bind source, so cornus does too — otherwise the 9P export would
// have no root and the kernel-9p mount fails opaquely ("connection reset by
// peer"), since the confined attacher cannot EvalSymlinks a path that isn't there.
// A mount with NoCreateHostPath (compose `bind.create_host_path: false`) opts out:
// the missing source is left absent and the server-side attach fails, as Docker
// does when create_host_path is false.
func resolveLocalMounts(mounts []api.Mount) ([]deploywire.LocalMount, map[string]string, error) {
	var localMounts []deploywire.LocalMount
	...
	for i, m := range mounts {
		...
		case os.IsNotExist(statErr) && !m.NoCreateHostPath:
			// Auto-create the missing bind source (Docker create_host_path parity).
			if err := os.MkdirAll(abs, 0o755); err != nil {
				return nil, nil, fmt.Errorf("mount %s: create host path %s: %w", m.Source, abs, err)
			}
			localDirs[name] = abs
		default:
			localDirs[name] = abs
		}
		localMounts = append(localMounts, lm)
	}
	return localMounts, localDirs, nil
}
...
// ingressTunnelQuery renders the scope selector an ingress-tunnel request needs.
func ingressTunnelQuery(project, deployment string) string {
	v := url.Values{}
	if project != "" {
		v.Set("project", project)
	}
	if deployment != "" {
		v.Set("deployment", deployment)
	}
	return "?" + v.Encode()
}

// IngressTunnelStart asks the server to host a public tunnel in front of its
// ingress (POST /.cornus/v1/ingress-tunnel), so a project's declared hosts and
// paths are reachable through one URL rather than a single workload port.
func (c *Client) IngressTunnelStart(ctx context.Context, req api.IngressTunnelRequest) (api.IngressTunnelStatus, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return api.IngressTunnelStatus{}, err
	}
	resp, err := c.do(ctx, http.MethodPost, "/.cornus/v1/ingress-tunnel", bytes.NewReader(body))
	if err != nil {
		return api.IngressTunnelStatus{}, err
	}
	defer resp.Body.Close()
	if resp.StatusCode == http.StatusNotFound {
		return api.IngressTunnelStatus{}, fmt.Errorf("this server does not support ingress tunnels (upgrade the cornus server)")
	}
	if resp.StatusCode != http.StatusOK {
		return api.IngressTunnelStatus{}, apiError(resp)
	}
	var st api.IngressTunnelStatus
	return st, json.NewDecoder(resp.Body).Decode(&st)
}

// IngressTunnelStatus reports the current ingress tunnel for a scope
// (GET /.cornus/v1/ingress-tunnel).
func (c *Client) IngressTunnelStatus(ctx context.Context, project, deployment string) (api.IngressTunnelStatus, error) {
	resp, err := c.do(ctx, http.MethodGet, "/.cornus/v1/ingress-tunnel"+ingressTunnelQuery(project, deployment), nil)
	if err != nil {
		return api.IngressTunnelStatus{}, err
	}
	defer resp.Body.Close()
...
// IngressTunnelStop tears an ingress tunnel down (DELETE /.cornus/v1/ingress-tunnel).
func (c *Client) IngressTunnelStop(ctx context.Context, project, deployment string) error {
	resp, err := c.do(ctx, http.MethodDelete, "/.cornus/v1/ingress-tunnel"+ingressTunnelQuery(project, deployment), nil)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
...
// IngressTunnelChannel opens the ssh-agent side-channel for an ingress tunnel,
// the scoped counterpart of TunnelChannel. The caller dials it before issuing the
// matching IngressTunnelStart with ForwardAgent set.
func (c *Client) IngressTunnelChannel(ctx context.Context, project, deployment, purpose string) (net.Conn, error) {
	ctx, span := observability.Tracer().Start(ctx, "cornus.client.ingresstunnelchannel")
	defer span.End()
	u := wsAttachURL(c.base, "/.cornus/v1/ingress-tunnel/channel/"+url.PathEscape(purpose)) + ingressTunnelQuery(project, deployment)
	conn, err := wire.DialConnControlHeaderCT(ctx, u, nil, c.dialHeader(ctx), c.clientTransport())
	if err != nil {
		span.RecordError(err)
		span.SetStatus(codes.Error, err.Error())
...
// Activity fetches the server's flight records (GET /.cornus/v1/activity).
//
// The records are NDJSON on the wire, one event per line, exactly as they sit on
// the server's disk — so a caller can stream them without the server having to
// buffer a whole history into one JSON document.
// It also returns the SERVING process's own instance id when the server reports
// one, so a caller can tell a still-open lifetime that is simply running from
// one whose process died mid-flight.
func (c *Client) Activity(ctx context.Context, since time.Time, kind string, unfinished bool) ([]activity.Event, string, error) {
	resp, err := c.do(ctx, http.MethodGet, activityPath(since, kind, unfinished, false), nil)
	if err != nil {
		return nil, "", err
	}
	defer resp.Body.Close()
	...
	live := resp.Header.Get("Cornus-Activity-Live-Instance")
	...
	for {
		...
		if err := dec.Decode(&e); err != nil {
			...
			return nil, "", fmt.Errorf("activity: decode: %w", err)
		}
		out = append(out, e)
	}
	return out, live, nil
...
func activityPath(since time.Time, kind string, unfinished, follow bool) string {
	q := url.Values{}
	if !since.IsZero() {
...
// ActivityFollow streams the server's flight records: the history, then every
// record as it is written, calling fn for each until ctx is cancelled or fn
// returns an error. It returns the serving process's instance id as soon as the
// response headers arrive, so a caller can label the live incarnation before any
// record has been seen.
//
// The follow stream is Server-Sent Events, not the NDJSON the one-shot read
// returns: a connection that stays open for hours and is idle most of the time
// needs a keep-alive and a media type intermediaries will not buffer, and SSE
// defines both. Each record arrives as one `activity` event whose data is the
// same JSON object the one-shot read yields.
func (c *Client) ActivityFollow(ctx context.Context, since time.Time, kind string, fn func(activity.Event) error) (string, error) {
	resp, err := c.do(ctx, http.MethodGet, activityPath(since, kind, false, true), nil)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
	...
	err = readSSE(resp.Body, func(event string, data []byte) error {
		if event != "" && event != "activity" {
			return nil // an event kind this build does not know about
		}
		var e activity.Event
		if err := json.Unmarshal(data, &e); err != nil {
			return fmt.Errorf("activity: decode: %w", err)
		}
		return fn(e)
	})
	return live, followErr(ctx, err)
}

// followErr normalizes how a cancelled follow reports itself.
//
// Cancellation is how this call normally ends, and it surfaces as a read error
// on a torn-down body. Over the default transport that error already IS
// context.Canceled, but this client also runs over a yamux-tunneled connection
// (an SSH conduit, a published tunnel), where the same cancellation arrives as
// whatever that stack says a closed stream looks like. The caller's context is
// the authority on why the read stopped; without this, a plain Ctrl-C through a
// tunnel would print an unrelated transport error and exit non-zero.
...
// readSSE parses a Server-Sent Events stream, calling fn once per dispatched
// message with its event name and data payload.
//
// Only the subset the activity stream uses is implemented: `event:` and `data:`
// fields, comment lines (the keep-alive) discarded, a blank line dispatching.
// Multi-line data is rejoined with newlines per the spec, even though the server
// never splits a record — a reader that silently dropped the continuation would
// corrupt exactly the long records worth reading.
func readSSE(r io.Reader, fn func(event string, data []byte) error) error {
	sc := bufio.NewScanner(r)
	// Records carry deployment names, image refs and attrs; the default 64 KiB
	// token limit is generous for one, but a hard failure on a long line would be
	// a silent truncation of the stream, so raise the ceiling.
	...
	for sc.Scan() {
		...
		case line == "":
			if len(data) > 0 {
				if err := fn(event, []byte(strings.Join(data, "\n"))); err != nil {
					return err
				}
			}
			event, data = "", nil
		case strings.HasPrefix(line, ":"):
			// A comment: the keep-alive, and nothing else in this protocol.
