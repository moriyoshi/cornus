...
func Run(req api.FSOpRequest, roots []Root, body io.Reader) (api.FSOpResponse, io.ReadCloser) {
	...
	case api.FSOpPut:
		...
		err := tarcopy.Unpack(t.root.Path, t.rel, body, tarcopy.UnpackOptions{
	...
	case api.FSOpRemove:
		return remove(t, req.Recursive), nil

	case api.FSOpRename, api.FSOpCopy:
		return transfer(req, roots, t), nil
	}
	return api.FSOpUnsupported("unknown operation " + string(req.Op)), nil
}

// listDir reads one directory. Symlinks are described, never followed — a listing that
...
func listDir(t target) api.FSOpResponse {
	host, err := fs.RootPath(t.root.Path, t.rel)
	if err != nil {
		return classify(err)
	}
	...
	if len(des) > maxEntries {
		des, resp.Truncated = des[:maxEntries], true
	}
	for _, de := range des {
		info, err := de.Info()
		if err != nil {
...
func remove(t target, recursive bool) api.FSOpResponse {
	if err := writable(t); err != nil {
		return classify(err)
	}
	if t.rel == "/" {
...
func transfer(req api.FSOpRequest, roots []Root, src target) api.FSOpResponse {
...
func renamePath(src, dst target) api.FSOpResponse {
...
func copyPath(src, dst target) api.FSOpResponse {
	if src.rel == "/" {
	...
	unpackErr := tarcopy.Unpack(dst.root.Path, destDir, tarStream, tarcopy.UnpackOptions{})
...
func writable(t target) error {
	if t.root.ReadOnly {
		return &statusErr{code: api.FSErrReadOnly, msg: t.root.Target + " is mounted read-only"}
	}
	return nil
}

// statusErr carries an FSErr* code for a refusal that is ours rather than the
// kernel's.
