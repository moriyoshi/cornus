...
func newRecordingMux() *recordingMux {
	...
	rm.Mux = NewMux(func(host string, port int, lis *memlisten.Listener) func() {
		key := fmt.Sprintf("%s:%d", host, port)
		rm.mu.Lock()
		rm.listeners[key] = lis
		rm.mu.Unlock()
		// The withdrawal closes over the key it published under, so it undoes exactly
		// this publication rather than whatever is registered under the same key later.
		return func() {
			rm.mu.Lock()
			delete(rm.listeners, key)
			rm.mu.Unlock()
		}
	})
	return rm
