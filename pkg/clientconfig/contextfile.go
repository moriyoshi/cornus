...
func classifyFields(c *Context) (all, sensitive []string) {
	fields := []struct {
		...
		{"key-auth", c.KeyAuth != nil, true},
		{"ssh-tunnel", c.SSHTunnel != nil, true},
		{"conduit", c.Conduit != nil, true},
		{"via-server", c.ViaServer != nil, false},
		// shells names binaries that get EXECUTED inside the workload when a
		// developer opens a terminal on it. It carries no credential, but an
		// auto-discovered working-tree file choosing that binary is a code-execution
		// primitive, so it is sensitive: stripped unless the user opted in.
		{"shells", len(c.Shells) > 0, true},
	}
...
func (c *Context) SuppliesCredential() bool {
	return c.Token != "" || c.KubeAuth != nil || c.KeyAuth != nil
}
...
func Merge(dst, src *Context) {
	...
	if src.KeyAuth != nil {
		...
		if src.KeyAuth.IdentityFile != "" {
			dst.KeyAuth.IdentityFile = src.KeyAuth.IdentityFile
		}
		if src.KeyAuth.KeyFingerprint != "" {
			dst.KeyAuth.KeyFingerprint = src.KeyAuth.KeyFingerprint
		}
		if src.KeyAuth.Name != "" {
			dst.KeyAuth.Name = src.KeyAuth.Name
		}
		if src.KeyAuth.Scope != "" {
			dst.KeyAuth.Scope = src.KeyAuth.Scope
		}
		if src.KeyAuth.TTL != "" {
			dst.KeyAuth.TTL = src.KeyAuth.TTL
		}
	}
	...
	if src.Conduit != nil {
		dst.Conduit = dst.Conduit.Merge(src.Conduit)
	}
	// Shells is a preference ORDER, so a given list replaces rather than appends:
	// concatenating would rank dst's entries above the ones src just asked for,
	// which is backwards for an override layer. Copied, so a later edit to either
	// side cannot reach through a shared backing array.
	if len(src.Shells) > 0 {
		dst.Shells = append([]string(nil), src.Shells...)
	}
}
