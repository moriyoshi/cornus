...
type Context struct {
	...
	// KeyAuth, when set, derives a short-lived bearer token by proving possession of
	// an SSH private key already enrolled with the server. It stores only a private
	// key path or an ssh-agent fingerprint, never private-key material. It takes
	// precedence over KubeAuth and Token but yields to CORNUS_TOKEN.
	KeyAuth *KeyAuth `json:"key-auth,omitempty"`
	// TokenExchange, when enabled, trades whatever credential the fields above
	// produced for a short-lived cornus credential via RFC 8693, and caches the
	// result. It is deliberately independent of which of them produced the subject
	// token: the exchange is the same round trip for a cluster ServiceAccount
	// token, an OIDC token, or a static one, and binding it to KubeAuth would have
	// to be undone the first time another source wanted it.
	TokenExchange *TokenExchange `json:"token-exchange,omitempty"`
	...
	// Tunnel carries defaults for public tunnels (cornus tunnel / cornus
	// ingress-tunnel) so a caller need not repeat them on every invocation. It
	// holds no secret: only a PATH to a credential file, never the credential.
	Tunnel *Tunnel `json:"tunnel,omitempty"`
	// Shells lists candidate interactive shells for the workloads reached through
	// this profile, most preferred first. Each entry is a command STRING, not an
	// argv ("/bin/bash", "/bin/busybox sh"); the consumer splits it once with the
	// same shell-words parser Compose uses for command/entrypoint. Consumed by
	// `cornus web`'s terminal, which probes a workload's own spec list first, then
	// these, then the browser's own list.
	//
	// It is classified SECURITY-SENSITIVE (see contextfile.go's classifyFields) even
	// though it carries no credential: it names a binary that gets EXECUTED inside
	// the workload, so an auto-discovered per-project override file — a working-tree
	// file anyone who can open a pull request may write — must not supply it without
	// explicit trust.
	Shells []string `json:"shells,omitempty"`
}

// KeyAuth selects the local SSH signer used to mint short-lived Cornus sessions.
// IdentityFile and KeyFingerprint are alternatives: the former loads a private
// key from disk, while the latter selects a key from SSH_AUTH_SOCK.
type KeyAuth struct {
...
// Tunnel holds per-profile defaults for public tunnels. It deliberately stores
// no credential — only the path to one — so a shared or checked-in profile can
// never leak an authtoken.
type Tunnel struct {
	// AuthTokenFile is the default --authtoken-file: a path to the tunnel-backend
	// credential. Empty means the caller passes one per invocation, or the server
	// has its own default (CORNUS_TUNNEL_AUTHTOKEN).
	AuthTokenFile string `json:"authtoken-file,omitempty"`
	// IngressHostMode is the default --host-mode for `cornus ingress-tunnel`:
	// "auto" (recommended), "passthrough", "alias", or "rewrite". Empty means auto.
	IngressHostMode string `json:"ingress-host-mode,omitempty"`
}
...
// Merge returns a deep copy of t overlaid with the non-zero fields of override.
func (t *Tunnel) Merge(override *Tunnel) *Tunnel {
	if override == nil {
		return t.Clone()
	}
	result := t.Clone()
	...
	if override.AuthTokenFile != "" {
		result.AuthTokenFile = override.AuthTokenFile
	}
	if override.IngressHostMode != "" {
		result.IngressHostMode = override.IngressHostMode
	}
	return result
}
...
// TokenExchange configures RFC 8693 token exchange for a profile.
//
// The benefit is not only latency. The credential that ends up on the wire is
// minted by the server, names its scope, and can be NARROWER than what the
// caller is entitled to — so a profile used for pulling images can hold a
// credential that cannot deploy, even though the same identity could have
// obtained one that could.
type TokenExchange struct {
	Enabled bool `json:"enabled,omitempty"`
	// Scope optionally narrows the credential (e.g. "registry:pull"). Empty means
	// whatever the server's scope map grants. A scope the policy does not grant is
	// refused rather than quietly reduced, so a profile that pins one fails loudly
	// when policy changes underneath it instead of silently gaining access.
	Scope string `json:"scope,omitempty"`
}
