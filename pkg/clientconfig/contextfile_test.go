...
func TestMerge(t *testing.T) {
	...
	t.Run("key auth merges field by field", func(t *testing.T) {
		dst := &Context{KeyAuth: &KeyAuth{IdentityFile: "/old", Scope: "api"}}
		Merge(dst, &Context{KeyAuth: &KeyAuth{TTL: "30m", Name: "laptop"}})
		if got := dst.KeyAuth; got.IdentityFile != "/old" || got.Scope != "api" || got.TTL != "30m" || got.Name != "laptop" {
			t.Fatalf("KeyAuth = %+v, want field-by-field merge", got)
		}
	})
}
...
func TestFieldClassification(t *testing.T) {
	...
	keyc := &Context{KeyAuth: &KeyAuth{IdentityFile: "/key"}}
	all, sensitive = FieldNames(keyc)
	if !reflect.DeepEqual(all, []string{"key-auth"}) || !reflect.DeepEqual(sensitive, all) || !keyc.SuppliesCredential() {
		t.Fatalf("key-auth classification = all %v sensitive %v credential %v", all, sensitive, keyc.SuppliesCredential())
	}
	StripSensitive(keyc)
	if keyc.KeyAuth != nil {
		t.Fatal("StripSensitive retained key-auth")
	}
}
