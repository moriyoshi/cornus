...
type deployAttacher interface {
	...
	// DeleteVolume removes the backend volume with the given logical name — the
	// VolumeSpec.Name toDeploySpec assigns a container's named volumes, which the
	// backend provisions with real, persistent storage. Delete-if-exists: removing
	// a volume that does not exist succeeds. It reports
	// client.ErrVolumeRemovalUnsupported when the server's deploy backend cannot
	// remove volumes (HTTP 501); `docker volume rm`/`prune` must surface that
	// rather than claim a removal that did not happen (volumes.go).
