...
// prefix so both versioned and unversioned Docker requests route correctly. The
// stripped version is preserved on the request context (requestAPIVersion),
// because it is not purely routing information: the stream media types are
// version-gated, so a handler still has to know what the client asked for.
func (p *Proxy) Handler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if v := versionPrefix.FindString(r.URL.Path); v != "" {
			r = withAPIVersion(r, strings.TrimPrefix(v, "/v"))
		}
