...
func (p *Proxy) start(w http.ResponseWriter, _ *http.Request, rec *containerRecord) {
	...
	// The event hook is how the container's exit status gets back here: the
	// server stamps it on the terminal control-stream event (see
	// deploywire.Event.ExitCode). Any previous run's code is dropped first — a
	// restarted container must not answer a wait with its old incarnation's code.
	rec.clearExitCode()
	sess := attachsession.Open(p.attacher, rec.spec, attachsession.WithEventHook(rec.observeExit))
...
// and streams the backend's stdcopy-multiplexed frames straight through, typed
// by streamContentType so a client can tell framed bytes from raw ones. The
// backend guarantees framing (dockerhost passes Docker's already-framed non-TTY
// bytes through; the kube backend wraps its raw stream in stdcopy stdout
// framing), so the proxy does not re-frame.
func (p *Proxy) logs(w http.ResponseWriter, r *http.Request, rec *containerRecord) {
	...
	w.Header().Set("Content-Type", streamContentType(r, rec.spec.TTY))
...
//
// The body carries the workload's REAL exit code (see awaitExit for where it
// comes from), or, when nothing can report one, unknownExitCode plus an Error —
// never a fabricated 0.
//
// RESIDUAL LIMITATION: two cases genuinely have no code to report, and both
// answer "unknown" rather than guessing.
//   - An explicit stop/kill/rm: cornus tears the whole deployment down, so the
//     workload never terminates in a way any backend observes. Docker would say
//     137 (SIGKILL) here; cornus does not know that the signal is what ended it,
//     and inventing 137 would be the same class of lie as inventing 0.
//   - A backend that cannot report exit codes at all: barehost and incushost
//     never fill api.InstanceStatus.ExitCode, so even a clean self-exit is
//     unknown there. dockerhost, containerdhost and kubernetes all report one.
func (p *Proxy) wait(w http.ResponseWriter, r *http.Request, rec *containerRecord) {
	...
	code, known := p.awaitExit(r.Context(), rec, sess)
	if r.Context().Err() != nil {
		return // client hung up; nothing to answer
	}
	if known {
		// Remember it, so a later inspect (and a repeat wait after the deployment
		// is gone from the backend) reports the same code.
		rec.setExitCode(code)
	}
	_ = json.NewEncoder(w).Encode(waitResult(code, known))
}

// exitPollInterval is how often a blocked wait re-reads the backend's status
// looking for a workload that has terminated. Polling is necessary because the
// deploy-attach session deliberately OUTLIVES the workload: the server holds it
// until the caller disconnects (so `docker logs` still works on an exited
// container, and so the mounts of a completed one-shot are not yanked out from
// under a client that has not asked for teardown), which means session.Done()
// alone never fires when a container exits on its own. The cost is one status
// call per second per *blocked wait*, not per session.
var exitPollInterval = time.Second

// awaitExit blocks until the container's exit status is decided, returning it
// and whether it is actually known. It returns as soon as EITHER the backend
// reports the workload terminated (a container that exited on its own — the
// `docker run` foreground case) OR the deploy-attach session ends (a stop/kill/
// rm driven by this proxy). A cancelled ctx returns unknown; the caller checks
// ctx itself and writes nothing.
func (p *Proxy) awaitExit(ctx context.Context, rec *containerRecord, sess *session) (int, bool) {
	if sess == nil {
		// Not running: answer from what is already known (docker's default
		// `condition=not-running` on an exited or never-started container).
		return p.exitStatus(ctx, rec)
	}
	...
	for {
		...
		case <-ctx.Done():
			return 0, false
		case <-tick.C:
			if st, err := p.attacher.Status(ctx, rec.deployment); err == nil {
...
// exitStatus resolves a not-running container's exit code from the two sources
// that survive the session: the code the server stamped on the terminal
// control-stream event (read off the backend just before it deleted the
// workload — the only source once the deployment is gone), then a direct backend
// status for a workload that outlived its session.
func (p *Proxy) exitStatus(ctx context.Context, rec *containerRecord) (int, bool) {
	if code, ok := rec.lastExitCode(); ok {
		return code, true
	}
	if st, err := p.attacher.Status(ctx, rec.deployment); err == nil {
		return st.TerminalExitCode()
	}
	return 0, false
}

// unknownExitCode is the StatusCode reported when nothing can say how the
// container ended. 125 is Docker's own "the daemon could not answer" code: the
// docker CLI substitutes exactly 125 whenever a wait response carries an Error
// (cli/command/container/utils.go), so pairing the two is the one answer the
// CLI, the API and a shell `$?` all read the same way.
//
// It must not be 0. Reporting an unknown outcome as success is precisely the bug
// this replaced: CI and wait-for-completion loops read that 0 and conclude a
// crashed container succeeded. There is no "unknown" in Docker's schema, so the
// honest encoding is a non-zero code plus the Error field explaining it.
const unknownExitCode = 125

// waitResult builds the docker wait body for a resolved (or unresolved) exit.
func waitResult(code int, known bool) waitResponse {
	if !known {
		return waitResponse{
			StatusCode: unknownExitCode,
			Error: &waitError{Message: "cornus: the deploy backend did not report an exit code for this container " +
				"(the workload was torn down while running, or its backend cannot report one); " +
				"the exit status is unknown, not 0"},
		}
	}
	return waitResponse{StatusCode: code}
}
...
func (p *Proxy) inspect(w http.ResponseWriter, rec *containerRecord) {
	state := rec.stateNow()
	// Docker's State.ExitCode is a plain int with no "unknown" encoding, so it
	// reports whatever has been established (by a wait, or by the terminal
	// deploy-attach event) and falls back to 0. RESIDUAL LIMITATION: a container
	// whose code nothing could report still inspects as ExitCode 0 — only
	// /wait can say "unknown" (see waitResult).
	code, _ := rec.lastExitCode()
