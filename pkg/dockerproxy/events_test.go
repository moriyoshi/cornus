...
func TestWaitNextExit(t *testing.T) {
	...
	select {
	case code := <-waitDone:
		// The fake backend reports no instances, so nothing can say how the
		// container ended — which answers unknownExitCode, NOT 0. (What the code
		// is when it IS known is covered in wait_test.go.)
		if code != unknownExitCode {
			t.Fatalf("wait status code = %d, want %d (unknown)", code, unknownExitCode)
		}
	case <-time.After(5 * time.Second):
