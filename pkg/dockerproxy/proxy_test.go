...
type fakeAttacher struct {
	...
	// attachCfgs records every AttachConfig the proxy asked for, so a test can
	// assert WHICH tunnel was requested (notably whether the proxy asked the
	// backend to replay the workload's output from its first byte).
	attachCfgs []api.AttachConfig

	// attachNeverEOF makes Attach return a stream that writes `output` and then
	// stays open forever without closing. That is real dockerd's behaviour for an
	// attach opened against a container that has ALREADY exited: unlike a stream
	// opened while the container ran, it never EOFs, so a proxy that waits for the
	// backend to end the stream waits forever.
	attachNeverEOF bool
	...
	// instances, when non-nil, is what Status reports for every deployment (the
	// default is an instance-less status, i.e. a backend that enumerates
	// nothing). Tests set it to model a workload that has terminated with a
	// known exit code while its deploy-attach session is still held.
	instances []api.InstanceStatus

	// doneExitCode, when non-nil, is stamped on a terminal Done event emitted
	// just before DeployAttach returns — exactly what the server does after
	// reading the workload's final status (deploywire.Event.ExitCode).
	doneExitCode *int
	...
	// deletedVolumes records the volume names DeleteVolume was called with (the
	// backend removals the proxy actually performed); volumeErr, when set, is
	// what DeleteVolume returns instead — e.g.
	// client.ErrVolumeRemovalUnsupported for a backend that cannot remove
	// volumes.
...
func (f *fakeAttacher) DeployAttach(ctx context.Context, spec api.DeploySpec, events func(deploywire.Event)) error {
	...
	f.mu.Unlock()
	if code != nil {
		// The server's terminal frame, delivered to the caller's event hook
		// before the attach returns (deploywire.Serve does the same).
		events(deploywire.Event{Done: true, ExitCode: code})
	}
	return err
}

// setInstances swaps what Status reports, so a test can make the workload
// "terminate" underneath a held session.
func (f *fakeAttacher) setInstances(insts []api.InstanceStatus) {
	f.mu.Lock()
	f.instances = insts
	f.mu.Unlock()
}
...
func (f *fakeAttacher) Status(_ context.Context, name string) (api.DeployStatus, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	return api.DeployStatus{Name: name, Instances: f.instances}, nil
}
...
func (f *fakeAttacher) Attach(_ context.Context, _ string, cfg api.AttachConfig) (net.Conn, error) {
	f.mu.Lock()
	f.attachCfgs = append(f.attachCfgs, cfg)
	never := f.attachNeverEOF
	out := f.output
	f.mu.Unlock()
	if never {
		return holdOpenConn(out), nil
	}
	return f.streamConn(), nil
}

// lastAttachCfg returns the most recent AttachConfig the proxy requested.
func (f *fakeAttacher) lastAttachCfg(t *testing.T) api.AttachConfig {
	t.Helper()
	f.mu.Lock()
	defer f.mu.Unlock()
	if len(f.attachCfgs) == 0 {
		t.Fatal("the proxy never opened an attach tunnel")
	}
	return f.attachCfgs[len(f.attachCfgs)-1]
}

// holdOpenConn writes payload and then keeps the connection open forever,
// modelling dockerd's attach to an already-exited container.
func holdOpenConn(payload string) net.Conn {
	a, b := net.Pipe()
	go func() {
		if payload != "" {
			_, _ = b.Write([]byte(payload))
		}
		select {} // never EOF, never close
	}()
	return a
}
...
// DeleteVolume records the backend volume removal (or fails with volumeErr),
// standing in for the server's DELETE /.cornus/v1/volume/{name}.
func (f *fakeAttacher) DeleteVolume(_ context.Context, name string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.volumeErr != nil {
		return f.volumeErr
...
// volumesDeleted snapshots the names DeleteVolume was called with.
func (f *fakeAttacher) volumesDeleted() []string {
	f.mu.Lock()
	defer f.mu.Unlock()
	return append([]string(nil), f.deletedVolumes...)
}
...
// stdcopy-framed log frames through unchanged. The container is non-TTY, so the
// frames are stdcopy-multiplexed and the media type has to say so
// (streamtype_test.go covers the type's version/TTY matrix directly).
func TestContainerLogs(t *testing.T) {
	...
	if ct := resp.Header.Get("Content-Type"); ct != mediaTypeMultiplexedStream {
		t.Fatalf("content-type = %q, want %q", ct, mediaTypeMultiplexedStream)
	}
