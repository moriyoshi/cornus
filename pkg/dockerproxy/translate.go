...
// sources are caller-local and serves those over 9P. Named volumes (Binds whose
// source is a volume name, and Mounts of Type=="volume") become spec.Volumes,
// keeping the project-scoped name the compose plugin already assigned.
func toDeploySpec(name string, req createRequest) api.DeploySpec {
	spec := api.DeploySpec{Name: name, Image: req.Image, Replicas: 1, Command: req.Cmd, Entrypoint: req.Entrypoint, TTY: req.Tty}
	...
	// Docker's DEFAULT (an unset policy) and an explicit "no" both mean "do not
	// restart", and both must be carried through explicitly. Leaving spec.Restart
	// empty is NOT equivalent: deploy.RestartPolicy defaults an empty value to
	// "unless-stopped", so dockerhost resurrected the container forever. A
	// restarting container inspects as State.Running=true, so InstanceStatus.ExitCode
	// stays nil, DeployStatus.TerminalExitCode never resolves, and `docker wait`
	// blocked forever — which is to say a plain `docker run -d` that exits on its own
	// could never report its exit status through the proxy at all.
	//
	// The attempt cap rides its own spec field rather than an "on-failure:N"
	// spelling: api.DeploySpec already keeps the policy word and the cap apart
	// (that is what pkg/compose's Restart.split produces for `on-failure:N`), and
	// the backends read the split form — dockerhost sends RestartMaxAttempts as
	// RestartPolicy.MaximumRetryCount, barehost feeds it to its supervisor's
	// MaxAttempts, kubernetes makes it a Job backoffLimit. Dropping it here made
	// `docker run --restart=on-failure:2` retry forever. Docker only honors the
	// count for "on-failure", so it is carried only there; a stray count on
	// another policy is ignored exactly as dockerd ignores it.
	spec.Restart = "no"
	if rp := req.HostConfig.RestartPolicy.Name; rp != "" {
		spec.Restart = rp
	}
	if spec.Restart == "on-failure" && req.HostConfig.RestartPolicy.MaximumRetryCount > 0 {
		spec.RestartMaxAttempts = req.HostConfig.RestartPolicy.MaximumRetryCount
	}
