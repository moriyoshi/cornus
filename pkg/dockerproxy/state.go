...
type containerRecord struct {
	...
	// exitCode is how the workload ended, once anything has been able to say:
	// either the deploy-attach control stream reported it (deploywire.Event.ExitCode
	// on the terminal event, which the server reads off the backend just before
	// deleting the workload) or wait resolved it from a backend status poll. nil
	// means nobody could say — which is NOT the same as 0 and must never be
	// reported as such. Guarded by mu.
	exitCode *int
}

// observeExit records an exit code reported over the deploy-attach control
// stream. Wired as the session's event hook, so it runs on the attach goroutine
// before the session's Done() closes — by the time a waiter wakes on Done the
// code is already visible here.
func (rec *containerRecord) observeExit(e deploywire.Event) {
	if e.ExitCode == nil {
		return
	}
	rec.setExitCode(*e.ExitCode)
}

// setExitCode records the container's exit status.
func (rec *containerRecord) setExitCode(code int) {
	rec.mu.Lock()
	defer rec.mu.Unlock()
	rec.exitCode = &code
}

// lastExitCode returns the recorded exit status, and whether one is recorded.
func (rec *containerRecord) lastExitCode() (int, bool) {
	rec.mu.Lock()
	defer rec.mu.Unlock()
	if rec.exitCode == nil {
		return 0, false
...
// clearExitCode drops a previous run's exit status. Called before opening a new
// session so a restarted container cannot answer a wait with the code its
// PREVIOUS incarnation exited with.
func (rec *containerRecord) clearExitCode() {
	rec.mu.Lock()
	defer rec.mu.Unlock()
	rec.exitCode = nil
}
