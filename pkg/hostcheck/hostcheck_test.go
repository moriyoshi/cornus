...
func TestHostNetworkWarnings(t *testing.T) {
	for name, tc := range map[string]struct {
		known, host bool
		want        Status
	}{
		"host netns":  {true, true, StatusOK},
		"own netns":   {true, false, StatusWarn},
		"cannot tell": {false, false, StatusWarn},
	} {
		env := translating(hostenv.RuntimeContainerd)
		env.HostNetworkKnown, env.HostNetwork = tc.known, tc.host
		...
		// Deferred, so never fatal however it comes out.
		if r.Failed() {
			t.Errorf("%s: Failed() = true; the host-network requirement is deferred", name)
		}
	}
}

// Only containerd's networking depends on the server's netns.
