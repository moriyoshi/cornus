...
// ProxyEnvFrom returns the subset of env that ProxyEnv owns, as sorted "K=V"
// strings ready to hand to a container's env. Empty when env carries none.
//
// It exists so a companion caretaker sharing the workload's network namespace
// can be pointed at the SAME loopback egress proxy the app was pointed at. In
// "proxy" mode the app is steered by env alone, so any sibling process that does
// not read those variables silently takes the native container path instead —
// which for the telemetry collector meant its OTLP exports bypassed a
// client-side egress policy the operator had explicitly configured. "transparent"
// mode has no such gap: its nftables redirect is netns-scoped, so it captures
// every process in the namespace whether or not it reads env.
func ProxyEnvFrom(env map[string]string) []string {
	sub := ProxyEnvSubset(env)
...
// ProxyEnvSubset is ProxyEnvFrom in map form, for backends whose container env is
// a map rather than a "K=V" list. Nil when env carries none.
func ProxyEnvSubset(env map[string]string) map[string]string {
	owned := []string{
